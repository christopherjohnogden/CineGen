...
- **Full-take shot ruler:** under the Generate viewer, a **S1 / S2 / S3** timeline sizes each beat by duration. Click a shot to seek the Full take there (and jump the Shots list). Times are **baked into that take** from the SEGMENT headings in the prompt that made the video, so rewriting Shots later does not move the cuts. Overlapping `from`/`to` clip at the next shot’s start (S2 to 13 / S3 from 12 → S3 at 12s). Isolate from the variant bar as before **(in progress)**
