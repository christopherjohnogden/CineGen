...
- Two-column workspace: the left column is the **production console** — clip identity with generate actions in the title row, 16:9 viewer, takes grouped by Full vs each isolated shot (native / held); the right column is the **prompt stack** — collapsible sections for **Director's notes** (open by default), Prompt, Edit body, Camera, Shots, Setup, Craft, and Style & constraints (the rest collapsed by default) (clicking S1 / S2 in the variant bar still opens Shots and jumps to that beat). Generate notes patch the clip like Shotlist so the compiled Prompt updates immediately
...
- **Shot grammar + camera movement** on Generate and Shotlist: size / bodies / dirty-clean / angle chips per beat; the variant bar labels each beat `S1 · MCU` / `S2 · WS` from the grammar (or the LLM cam line); a beat with no size of its own **holds the previous setup** (`S4 · CU` after an S3 CU, hover: same setup as S3). A **HARD CUT / REVERSE CUT / portrait** is a new setup, not a hold — only true continuations share a color wash; the selected shot keeps a gold inset ring. Chips light up from that cam line (`medium two shot` → MS + Two-shot, OTS + dirty from a shoulder-in-the-foreground). Each shot shows the LLM beat in plain language (cam + who + action + line) so you know what you are rewriting; **Reset** restores that first cam / action / line / chips. Scene coverage (master, singles, OTS, two-shot, insert) stamps empty beats; **Use this CU for every CU in the scene** copies a liked size. Hover a chip (or read the line under the row) for the plain-language framing — WS is a wide of full bodies in the room, CU is a face, OTS looks past one person to the other. Camera stays locked off unless you pick a move (dolly, track, crane, pan, tilt) or push the **Life** slider, which injects a filmic move when the frame feels static. Isolation allows that one move and still bans everything else. Acting take notes plus Whisper/Under/Full, Hold/Pick up/Overlap, and eyeline chips compile into `TAKE NOTE` / `DIRECTION`. Scene line of action locks screen direction **(in progress)**
- **Blocking map from a liked take:** pause the viewer, **Set as frame**, then **Make blocking map**. Higgsfield Nano Banana draws the tig-diagram schematic from that still and stores it as the clip's staging reference (or copies it to the whole scene). If CineGen misses the finished URL, **Load from Higgsfield** pulls the completed outline. **Clear this shot** empties the Frame and Map stills on this clip (saved storyboard cards stay). The Frame · map strip is a collapsed section like Prompt / Craft (header shows frame set / map ready / drawing). Generate still attaches the map last so photo refs keep the look **(in progress)**
- **Frame · map storyboard:** saved maps sit in a **Saved framings** strip under the current Frame / Map stills. **Set as frame** on an isolated take tags that shot; on **Full multishot** the playhead is mapped to S1 / S2 / S3… from beat timings so a pause in the CU stretch saves as that CU. The card keeps chips + cam line. Clicking it on another shot rewrites coverage; the blocking diagram still attaches last on Generate **(in progress)**
- **Shots storyboard picker:** each beat in Generate → Shots has a **Storyboard** grid of saved framings (still on top, source slate under it: `1A · S2 · CU`). Click a card to restage **that shot only** — chips, the shot line, leftover CU / two-shot copy in the action, and the compiled LENS / Edit body. Click again to unselect and restore the coverage from before the card. The gold ring is only that beat’s overlay, not the clip’s saved map. Empty state points at Set as frame in Frame · map **(in progress)**
...
- **Director's notes per clip**: the same notes box sits **above the compiled prompt** on each open clip — this clip is implied (no need to type 1A), and you can name shots as S1 / S2. **Apply notes with LLM** rewrites only that clip; other clips in the scene stay. The notes field **clears after a successful apply**. **Reset to original** (clip or scene) and per-shot **Reset** restore the first shotlist write — later notes / redo do not overwrite that snapshot. Scene-level notes remain for cross-clip notes. **Generate uses the same apply** on Full or an isolated shot (unnamed notes hit the visible shot) so the Prompt panel updates instead of a hidden Keep/Discard draft **(in progress)**
...
- **Per-scene shotlisting**: every scene block has its own Shotlist / Re-shotlist button plus inline scene **event** and **physical action** fields, so a single scene can be (re)broken without touching the rest of the show. **Coverage / line of action / camera movement** sit in a collapsible **Coverage** section, closed by default. **Re-shotlist scene** (and Shotlist show, once clips exist) asks for confirm first — it replaces those clips and drops their takes. Each shot row has **Redo** to rewrite just that camera; duration and dialogue stay. **Redo** on the clip (next to Copy prompt / Full multishot) rebuilds that clip's whole coverage — 1A stays 1A, other clips in the scene are untouched **(in progress)**
...
- **Shotlist page crash:** opening Shotlist threw `Can't find variable: getDirectorAdapter` — the adapter import was missing **(in progress)**
