...
> **Status: complete.** All nine tasks are implemented, tested and committed
> (`4f04bc4`, `5e83aca`, `17eb37c`, `c8e3e76`, `6efa63b`, `d4347b4`). See
> "What changed against this plan" at the end for where reality differed.
...
- [x] **Step 1: Write the failing test.** Attach 4 image files via `space-studio-attach-input`, select a Topview omni-reference video model, submit. Assert no `formError` and that all 4 urls reach the config. Assert the inverse for a media-tool model (`max: 1`) — 2 refs still error.
- [x] **Step 2: Add a `fieldCapacity(field)` helper** returning `field.maxItems ?? field.max ?? (field.multiple ? Infinity : 1)`.
- [x] **Step 3: Replace the guard** at 1507 with a capacity check, and make the write `capacity > 1 ? refs.map(r => r.url) : refs[0].url`.
- [x] **Step 4:** `npx vitest run tests/components/create/space-studio.test.tsx`. Commit.
...
- [x] **Step 1:** Gate the existing call behind an opt-in env var, default unchanged:
  `if (process.platform === 'darwin' && !app.isPackaged && process.env.CINEGEN_GPU !== '1') { … }`
- [x] **Step 2:** Verify `CINEGEN_GPU=1 npm run dev` reports a hardware renderer (`chrome://gpu` or a `WEBGL_debug_renderer_info` probe), and that plain `npm run dev` is byte-identical in behaviour to today.
- [x] **Step 3:** Document the flag in `README.md`. Commit.
...
- [x] **Step 1: Write the failing test** for `normalizeProjectSets` — drops non-objects, mints ids, coerces every field, never throws on garbage.
- [x] **Step 2:** Implement the types and `normalizeProjectSets`.
- [x] **Step 3:** Add `sets: ProjectSet[]` as the **last** member of `WorkspaceState`, and `sets?: ProjectSet[]` to `ProjectSnapshot`.
- [x] **Step 4:** Add `ADD_SET` / `UPDATE_SET` / `REMOVE_SET` to `WorkspaceAction`, `sets: []` to `createInitialWorkspaceState`, the three reducer cases, and `sets: ProjectSet[]` as a **non-optional** member of `HydratePayload` (this is what makes the compiler catch the three silent-corruption sites below).
- [x] **Step 5:** Add `sets` to the `HYDRATE` case return object (`workspace-state.ts:577-594`). *Missing this wipes sets on every load and every cloud tick.*
- [x] **Step 6:** Add the three actions to `WORKSPACE_PERSIST_ACTIONS` and to `UNDOABLE_ACTIONS` (two independent allowlists).
- [x] **Step 7:** In `persistWorkspace`, add `sets: state.sets` **inside** `dbState.workflow` and inside the JSON branch's workflow object.
- [x] **Step 8:** Add `state.sets` to the save-effect dependency array (`~1395-1410`). *Missing this makes saves fire only on unrelated changes.*
- [x] **Step 9:** Add `sets: normalizeProjectSets(...)` to **both** hydration branches **and** to `fromSnapshot` in the `watchCloudProject` effect (`~1242-1264`). *Missing the third loads sets then wipes them ~2 s later.*
- [x] **Step 10:** Add `'sets'` to the merge key list in `merge-project-update.ts:33`; add `sets: []` to `createDefaultCloudProject`'s workflow literal.
- [x] **Step 11:** In `project-db.ts`, add `sets?: unknown[]` to `FullProjectState['workflow']`, read it in `getWorkflowState`, and write it in `saveWorkflowState`'s `JSON.stringify`.
- [x] **Step 12:** Round-trip test: create → add a set → save → reload → assert survival. Commit.
...
- [x] **Step 1: Write the failing test** for `diagonalFov` — full-frame (36×24) gives ≈84.1° at 24 mm, ≈46.8° at 50 mm, ≈28.6° at 85 mm.
- [x] **Step 2:** Implement `src/lib/sets/optics.ts` (pure math only — no three.js import).
- [x] **Step 3:** Implement `scene.ts`: build a `THREE.Scene`, load the splat via Spark's `SplatMesh` from a `local-media://` URL, add a ground grid, add GLB mannequins on `THREE.Layers` 1 (splat on layer 0).
- [x] **Step 4:** Implement `SetViewer` — orbit/fly controls, aspect-letterboxed viewport, rule-of-thirds overlay, readouts of camera height / tilt / subject distance. Dispose the renderer and all geometries on unmount.
- [x] **Step 5:** Manual verification with `CINEGEN_GPU=1 npm run dev`. Commit.
...
- [x] **Step 1:** Widen `SpaceViewMode` to `'canvas' | 'studio' | 'sets'` (line 25), update `getInitialSpaceViewMode` (30), `handleViewModeChange` (190), the MCP `view` command's union check (**line 225** — currently throws on anything but canvas/studio), and add a third button to the switch (596-612).
- [x] **Step 2:** Import the splat via an Electron open dialog; store the absolute path only. Render the list with `set-card`.
- [x] **Step 3:** Implement `floor-plan.tsx` — a pure 2D SVG top-down of marks + camera frustums. Unit-test the projection math separately from rendering.
- [x] **Step 4:** Mount `SetViewer` through `React.lazy` with a `Suspense` fallback. Commit.
...
- [x] **Step 1:** Implement passes by toggling layer visibility — plate = layer 0 only; stand-in = layer 1 only; composite = both; depth = per Task 3's verdict.
- [x] **Step 2:** Render each via `SparkViewpoint({ target: { width, height, superXY: 2 } })` + `renderReadTarget()`, then pack RGBA into a PNG `Blob`.
- [x] **Step 3:** Persist via a **capability-probed** route: `media.writeTempImage` on desktop/local-web, `elements.upload` → `/api/uploads` on hosted web (hosted **refuses** `writeTempImage` with 422). Commit.
...
- [x] **Step 1: Write the failing test.** A 50 mm full-frame camera 4.2 m from a standing mannequin produces a block containing the 47° lens character, "about 4.2 metres from camera", and the mannequin-exclusion line.
- [x] **Step 2:** Implement `buildCameraPrompt(camera, standIns, set)` → `{ block: string, fovAnchor: FovAnchor }`.
- [x] **Step 3:** Always append verbatim: *"The gray mannequin in @image2 and @image4 is a placement guide only and must not appear in the shot."* Commit.
...
- [x] **Step 1:** Add the button to the **`promptChips` fragment** (`space-studio.tsx:2172`, immediately after the Elements chip block ending at 2193) — this is the one construct shared by the dock and the panel, so it needs a single insertion. `data-testid="space-studio-shape-shot"`, `disabled={!referencesActive}`, and hidden entirely when `!supportsReferences` (models without an omni reference field render no reference strip at all).
- [x] **Step 2:** Add `'both'` to `create-tab.tsx`'s transfer `content` union (currently sets media XOR prompt at 210-221) so one transfer can carry attachments *and* prompt.
- [x] **Step 3:** Give `StudioTransfer` an optional `promptMode: 'replace' | 'append'` and honour it at `space-studio.tsx:942` (currently an unconditional `setPrompt(transfer.prompt)`). Shape Shot uses `'append'`.
- [x] **Step 4:** On Attach — render passes at the generation's output resolution/aspect, build the transfer with `spaceId: state.activeSpaceId` (**it silently no-ops if this does not match**) and a fresh `id`, in order plate → composite → depth → stand-in. Save the camera to the Set.
- [x] **Step 5:** Mirror the camera block to a new `__studioShapeShot` config key and re-apply it in `recipe.ts` beside `presetId` (~line 227). *Without this the camera block is destroyed on Reuse, because `__studioPromptBody` is an unconditional override set to the undecorated body.*
- [x] **Step 6:** Integration test — open Shape Shot, attach, assert 4 references in order, the camera block present, `videoMode === 'references'`, and **no** start-frame write. Commit.
...
Four things were wrong in the plan and were corrected while building.

**1. Depth is in, not at risk.** The plan treated depth as a spike that might
fail and fall back to a 3-reference v1. The installed Spark 2.2.0 types ship
`modifiers.setDepthColor(splats, minDepth, maxDepth, reverse?)` — a first-class
depth pass. All four references ship. The docs site documents none of this; the
shipped types do, which is why reading `node_modules` beat reading the docs.

**2. `SparkViewpoint` does not exist in 2.2.0.** The offscreen render API the
plan described was folded into `SparkRenderer` itself: construct one with
`target: { width, height, superXY }` and call `renderReadTarget({ scene, camera })`.
Spark also ships `utils.flipPixels`, which replaced a hand-rolled row flip.

**3. The camera block survives Reuse with no mirror.** The plan said
`__studioPromptBody` is an unconditional override that would destroy an appended
camera block, and prescribed a `__studio*` mirror re-applied like `presetId`.
That is true of *submit-time decoration*, but Shape Shot appends the block into
the prompt **state**, so it is part of `trimmedPrompt` and is written to
`__studioPromptBody` along with everything else the user typed. The mirror was
still added, but for the camera *record* — which is what build step 6's
"regenerate from the same camera" actually needs.

**4. The four renders do not reach `extra_images` as a flat array.**
`referenceFieldFor` prefers element-list fields, so the submit path overwrites
that field with the composite `{ elementIds, elementVariationIds, urls }`. The
renders ride in `urls`. Task 9's assumption of a bare array was wrong; the
regression test asserts the real shape, plus `__studioAttachedRefs` for order.

## Verification

- 62 tests across `tests/lib/sets` and `tests/components/sets`, plus the guard
  regression in `tests/components/create/space-studio.test.tsx` (41 pass).
- `npx tsc --noEmit -p tsconfig.json` — clean.
- `npx vite build` — succeeds. three + Spark land in their own 2,976 kB chunk;
  `index` stays at 3,450 kB, unchanged from before this work. The lazy-import
  discipline held.
- The two pre-existing red gates are **unchanged**: `web/tsconfig.json` still has
  the same 2 × TS2741 (`generateCost`, `trimVideo`) and `parity.test.mjs` still
  fails only on `topview.recoverVideo`. Neither was touched by this work.

## Still open

- The viewer has not been exercised against a real splat file on a GPU. The
  scene, passes and depth path are typed against the real Spark API and unit
  tested, but `CINEGEN_GPU=1 npm run dev` with an actual `.ply` is the next step.
- Marks are persisted and drawn but there is no UI yet to place one; stand-ins
  snap to marks that already exist.
- Sets carry no thumbnail yet — `thumbnailUrl` is in the type and unused.
