...
ElevenLabs Audio nodes now run inside CineGen. Choose Connect ElevenLabs in the node or character voice panel to authorize CineGen’s own hosted MCP connection. CineGen uses OAuth with PKCE and its own client metadata, stores tokens encrypted, and discovers tools from the authenticated server. Claude’s private connection is not reused. The optional API-key connection remains available for operations that the hosted MCP does not expose; switching to it is explicit. Speech uses the saved character voice or a selected ElevenLabs voice. The adapter validates the advertised tool schema before submitting a take; unsupported tools and settings fail before spending credits. The API route uses Eleven v3 for speech and Eleven Sound Effects v2 for sound effects. The result plays on the node and is saved to Firebase, with a recoverable copy in Cloudflare R2. No Claude handoff is needed. Character editors can design voice previews, choose/save a voice, and generate sample dialogue inside the app.
