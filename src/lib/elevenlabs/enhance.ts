...
export async function enhanceAudioText(kind: AudioEnhanceKind, text: string, signal?: AbortSignal, requestId: string = crypto.randomUUID()): Promise<string> {
  // CineGen's hosted text enhancer uses no audio credits or additional sign-in.
  for (let attempt = 0; attempt < 45; attempt++) {
    if (signal?.aborted) throw new DOMException('Aborted', 'AbortError');
    const result = await elevenLabs.enhance({ requestId, kind, text });
    if (signal?.aborted) throw new DOMException('Aborted', 'AbortError');
