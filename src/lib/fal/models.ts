...
export const LOCAL_MODEL_REGISTRY: Record<string, ModelDefinition> = {
  'ltx-local': {
  ...
  'whisperx-local': {
    ...
    inputs: [
      { id: 'audio_url', portType: 'media' as any, label: 'Audio / Video', required: true, falParam: 'audio_url', fieldType: 'port' },
...
export const POD_MODEL_REGISTRY: Record<string, ModelDefinition> = {
  'pod-sdxl': {
...
/** Combined registry of all model definitions (fal + kie + local + runpod + pod). */
export const ALL_MODELS: Record<string, ModelDefinition> = { ...MODEL_REGISTRY, ...KIE_MODEL_REGISTRY, ...LOCAL_MODEL_REGISTRY, ...RUNPOD_MODEL_REGISTRY, ...POD_MODEL_REGISTRY };

export function getModelDefinition(nodeType: string): ModelDefinition | undefined {
  return ALL_MODELS[nodeType];
}

export function getAllModelNodeTypes(): string[] {
  return Object.keys(ALL_MODELS);
}

/** Return models filtered by provider ('fal' or 'kie'). */
export function getModelsByProvider(provider: 'fal' | 'kie'): Record<string, ModelDefinition> {
