...
function firstJobRecord(parsed) {
  for (const key of ["results", "jobs"]) {
    ...
    if (Array.isArray(value) && value.length > 0 && isRecord$1(value[0])) return value[0];
  }
  return parsed;
}
...
function extractMediaUrls(value, depth = 0) {
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    if (isRecord$1(entry) || Array.isArray(entry)) urls.push(...extractMediaUrls(entry, depth + 1));
  }
  return [...new Set(urls)];
}
...
function listRowsFromParsed(parsed) {
  if (Array.isArray(parsed)) return parsed.filter((entry) => isRecord$1(entry));
  if (!isRecord$1(parsed)) return [];
  for (const key of ["jobs", "results", "data", "items", "generations"]) {
    const rows = parsed[key];
    if (Array.isArray(rows)) return rows.filter((entry) => isRecord$1(entry));
  }
...
function registerWorkflowHandlers() {
  ipcMain.handle("workflow:run", async (_event, params) => {
    ...
    const { ALL_MODELS, resolveVideoModelEndpoint, sanitizeVideoInputsForEndpoint } = await import("./models-Ci756LlJ.js");
...
function topviewReferenceVideoMinPixels(submitModel) {
  if (/seedance-2-5/i.test(submitModel)) return SEEDANCE_2_5_REFERENCE_VIDEO_MIN_PIXELS;
  return /seedance-2/i.test(submitModel) ? SEEDANCE_REFERENCE_VIDEO_MIN_PIXELS : void 0;
}
...
CineGen could not create the temporary resized copy automatically.`);
  ...
  async submitWithSession(session2, params) {
    ...
    const built = buildTopviewVideoRequest({ config: config2, taskType, params, references: uploaded, boardId });
    const submitted = await this.callTool(session2, "topview_generate_video", built.req);
