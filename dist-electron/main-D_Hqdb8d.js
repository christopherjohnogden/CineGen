...
var Ml = (t, e, r) => e in t ? Ul(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : t[e] = r;
var U = (t, e, r) => Ml(t, typeof e != "symbol" ? e + "" : e, r);
import { BrowserWindow as Q, screen as ba, ipcMain as I, app as X, shell as va, safeStorage as hr, session as Ea, dialog as Ui, protocol as Ta, nativeImage as Dl, powerMonitor as sn } from "electron";
import j, { writeFile as $l, chmod as Fl, mkdir as et } from "node:fs/promises";
import B, { mkdirSync as Bl, writeFileSync as Hl, rmSync as Gl } from "node:fs";
import w, { join as Sa } from "node:path";
import z, { homedir as Vl } from "node:os";
import J, { randomBytes as zl, randomUUID as Pn } from "node:crypto";
import { Readable as Mi } from "node:stream";
import ti from "better-sqlite3";
import { createServer as Ia } from "node:http";
import { spawn as oe, execFile as tt } from "node:child_process";
import { promisify as fr } from "node:util";
import { lookup as Wl } from "node:dns/promises";
import { request as Xl } from "node:https";
import Jl, { isIP as ri } from "node:net";
import { createRequire as xa } from "node:module";
import { fileURLToPath as ni } from "node:url";
import { Worker as Aa } from "worker_threads";
const Kl = 1200, Yl = 150, Ql = 1e3, Di = 2800, ka = /* @__PURE__ */ new WeakMap(), ja = /* @__PURE__ */ new WeakMap(), jr = /* @__PURE__ */ new WeakMap(), Ra = /* @__PURE__ */ new WeakMap();
function Zl() {
...
const St = Zl(), Dr = w.join(import.meta.dirname, "."), Pa = w.join(Dr, "../dist"), ur = process.env.VITE_DEV_SERVER_URL;
function $i(t) {
  return ur ? t.loadURL(`${ur}?pm=1`) : t.loadFile(w.join(Pa, "index.html"), { query: { pm: "1" } });
}
function Fi(t) {
  return ur ? t.loadURL(ur) : t.loadFile(w.join(Pa, "index.html"));
}
function ln(t, e) {
  const r = jr.get(t) ?? /* @__PURE__ */ new Set();
  r.add(e), jr.set(t, r);
}
function gr(t, e) {
  var r;
  (r = jr.get(t)) == null || r.delete(e);
}
function Oa(t) {
  const e = jr.get(t);
  if (e) {
...
async function tu(t, e, r) {
  if (!t.isDestroyed())
    try {
      ...
      await On(t, e, r, "blank renderer DOM after resume");
    } catch (n) {
      console.warn(`[window] ${e} health check failed after wake:`, n), await On(t, e, r, "resume health check failed");
    }
}
function ru(t) {
  for (const e of Q.getAllWindows()) {
    ...
    const r = ka.get(e);
    if (!r) continue;
    const n = ja.get(e) ?? "window";
    Oa(e), Ra.set(e, Date.now() + Di + 1e3);
    let i = null;
    const o = setTimeout(() => {
      gr(e, o), !e.isDestroyed() && (console.log(`[window] ${n} wake recovery started: ${t}`), e.webContents.invalidate(), e.webContents.executeJavaScript(
        `(() => {
    ...
    const a = setTimeout(() => {
      gr(e, a), (async () => {
        try {
          await tu(e, n, r), i && (clearTimeout(i), gr(e, i), i = null);
        } catch (s) {
    ...
    ln(e, a), i = setTimeout(() => {
      gr(e, i), !e.isDestroyed() && On(e, n, r, `hard reload after ${t}`).catch((s) => {
...
function un() {
  const { width: t, height: e } = ba.getPrimaryDisplay().workAreaSize, r = 900, n = 580, i = new Q({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
    ...
    ...St ? { icon: St } : {},
    webPreferences: {
      preload: w.join(Dr, "preload.js"),
      nodeIntegration: !1,
      contextIsolation: !0,
      backgroundThrottling: !1
    }
  });
  return Na(i, "project-manager", $i), $i(i), i;
}
function nu() {
  const { width: t, height: e } = ba.getPrimaryDisplay().workAreaSize, r = 800, n = 395, i = new Q({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
  ...
  return i.loadFile(w.join(Dr, "splash.html")), i;
}
function Bi() {
  const t = new Q({
    width: 1440,
  ...
  return Na(t, "main", Fi), Fi(t), ur && t.webContents.openDevTools({ mode: "detach" }), t;
}
function ii() {
  return w.join(z.homedir(), "Documents", "CINEGEN");
}
function Nn() {
  return w.join(ii(), "projects.json");
}
function dr(t) {
  return w.join(ii(), t);
}
function at(t) {
  return w.join(dr(t), "project.json");
}
function qa() {
  return J.randomUUID();
}
...
async function La() {
  await j.mkdir(ii(), { recursive: !0 });
}
async function yr() {
  try {
    const t = await j.readFile(Nn(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function dn(t) {
  await La();
  const e = Nn() + ".tmp";
  await j.writeFile(e, JSON.stringify(t, null, 2), "utf-8"), await j.rename(e, Nn());
}
function iu(t, e) {
  const r = Ca(), n = {
    id: qa(),
...
function ou(t) {
  const e = w.join(dr(t), "project.json");
...
function au(t) {
  const e = w.join(dr(t), "project.db");
  if (!B.existsSync(e)) return null;
  try {
...
function su() {
  I.handle("project:list", async () => (await yr()).projects.map((e) => {
    const r = e.useSqlite ? au(e.id) : ou(e.id);
    ...
    const n = qa(), i = iu(n, r);
    await La(), await j.mkdir(dr(n), { recursive: !0 });
    const o = at(n) + ".tmp";
    await j.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await j.rename(o, at(n));
    const a = await yr();
    ...
    const r = await j.readFile(at(e), "utf-8");
    return JSON.parse(r);
  }), I.handle("project:save", async (t, e, r) => {
    let n;
    try {
      const l = await j.readFile(at(e), "utf-8");
      n = JSON.parse(l);
    } catch {
    ...
    await j.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await j.rename(o, at(e));
    const a = await yr(), s = a.projects.find((l) => l.id === e);
    return s && (s.updatedAt = i.project.updatedAt, s.assetCount = Array.isArray(i.assets) ? i.assets.length : 0, s.elementCount = Array.isArray(i.elements) ? i.elements.length : 0, r.project && r.project.name && (s.name = r.project.name), await dn(a)), i;
  }), I.handle("project:delete", async (t, e) => {
    await j.rm(dr(e), { recursive: !0, force: !0 });
    const r = await yr();
    r.projects = r.projects.filter((n) => n.id !== e), await dn(r);
  });
}
const Hi = 12e4, Ua = Sa(Vl(), "Documents", "CINEGEN"), qn = Sa(Ua, "mcp-bridge.json"), It = /* @__PURE__ */ new Map();
let Sr = !1, pe = null, _r = "";
function Ae(t, e, r) {
...
function uu(t, e, r) {
  return new Promise((n, i) => {
    const o = Pn(), a = setTimeout(() => {
      It.delete(o), i(new Error(`CineGen did not answer "${e}" within ${Math.round(Hi / 1e3)}s.`));
    }, Hi);
    It.set(o, { resolve: n, reject: i, timer: a }), t.webContents.send("mcp:invoke", { id: o, tool: e, args: r });
  });
}
function du(t) {
  I.on("mcp:ready", (e, r) => {
    ...
    const n = r != null && r.id ? It.get(r.id) : void 0;
    n && (clearTimeout(n.timer), It.delete(r.id), r.ok ? n.resolve(r.result) : n.reject(new Error(r.error || "The tool failed.")));
  }), I.handle("mcp:status", () => {
    var e;
    return {
      running: !!pe,
  ...
  }), _r = zl(24).toString("hex"), pe = Ia((e, r) => {
    (async () => {
      try {
        ...
        if (i.length !== _r.length || i !== _r) {
          Ae(r, 401, { ok: !1, error: "Bad or missing bridge token." });
          return;
        }
        if (e.method === "POST" && n.pathname === "/invoke") {
          const o = await lu(e), a = typeof (o == null ? void 0 : o.tool) == "string" ? o.tool : "";
          if (!a) {
          ...
          try {
            Ae(r, 200, { ok: !0, result: await uu(s, a, o.args ?? {}) });
          } catch (l) {
    ...
    try {
      Bl(Ua, { recursive: !0 }), Hl(
        qn,
        `${JSON.stringify({ port: e, token: _r, pid: process.pid, startedAt: (/* @__PURE__ */ new Date()).toISOString() }, null, 2)}
`,
        { mode: 384 }
...
function Ma() {
  return Gi || (Gi = 1, (function(t) {
    var e = Mt && Mt.__awaiter || function(i, o, a, s) {
      function l(u) {
...
function it() {
  if (zi) return we;
  zi = 1;
  var t = we && we.__awaiter || function(s, l, u, d) {
    function c(p) {
  ...
  Object.defineProperty(we, "__esModule", { value: !0 }), we.ValidationError = we.ApiError = void 0, we.defaultResponseHandler = o, we.resultResponseHandler = a;
  const e = oi(), r = "x-fal-request-id";
...
function $r() {
  return Xi || (Xi = 1, (function(t) {
    var e = Dt && Dt.__awaiter || function(s, l, u, d) {
      function c(p) {
        return p instanceof u ? p : new u(function(f) {
          f(p);
        });
...
var $t = {};
const pu = "@fal-ai/client", mu = "1.9.4", hu = {
  name: pu,
  version: mu
};
var Ji;
function ai() {
  if (Ji) return $t;
  Ji = 1, Object.defineProperty($t, "__esModule", { value: !0 }), $t.isBrowser = t, $t.getUserAgent = r;
  function t() {
...
function pr() {
  ...
  const r = $r(), n = ai(), i = Ve(), o = typeof navigator < "u" && (navigator == null ? void 0 : navigator.userAgent) === "Cloudflare-Workers";
  function a(l) {
    return t(this, void 0, void 0, function* () {
      var u;
      const { targetUrl: d, input: c, config: p, options: f = {} } = l, { credentials: m, requestMiddleware: y, responseHandler: h, fetch: g } = p, _ = Object.assign(Object.assign({}, p.retry), f.retry || {}), b = () => t(this, void 0, void 0, function* () {
        ...
        const E = (0, n.isBrowser)() ? {} : { "User-Agent": (0, n.getUserAgent)() }, A = typeof m == "function" ? m() : m, { method: q, url: N, headers: M } = yield y({
          ...
          url: d,
          headers: l.headers
        }), F = A ? { Authorization: `Key ${A}` } : {}, G = Object.assign(Object.assign(Object.assign(Object.assign({}, F), { Accept: "application/json", "Content-Type": "application/json" }), E), M ?? {}), { responseHandler: V, retry: R } = f, O = e(f, ["responseHandler", "retry"]), H = yield g(N, Object.assign(Object.assign(Object.assign(Object.assign({}, O), { method: q, headers: Object.assign(Object.assign({}, G), (x = O.headers) !== null && x !== void 0 ? x : {}) }), !o && { mode: "cors" }), { signal: f.signal, body: q.toLowerCase() !== "get" && c ? JSON.stringify(c) : void 0 }));
        return yield (V ?? h)(H);
      });
      let v;
...
function Da() {
  return Qi || (Qi = 1, (function(t) {
    var e = Ft && Ft.__awaiter || function(m, y, h, g) {
      function _(b) {
    ...
    function p(m, y, h) {
      return e(this, void 0, void 0, function* () {
        ...
        for (let M = 0; M < x; M++) {
          const F = M * T, G = Math.min(F + T, m.size), V = m.slice(F, G), R = M + 1, O = `${E.origin}${E.pathname}/${R}${E.search}`;
          A.push(yield c(O, V, y));
        }
...
`;
  ...
  return wr.createParser = t, wr;
}
var Bt = {}, eo;
function $a() {
  return eo || (eo = 1, (function(t) {
    var e = Bt && Bt.__awaiter || function(a, s, l, u) {
      function d(c) {
...
function Fa() {
  ...
  const n = /* @__PURE__ */ gu(), i = $a(), o = pr(), a = it(), s = Ve(), l = "text/event-stream", u = 15 * 1e3;
  class d {
    constructor(f, m, y) {
      var h;
      this.listeners = /* @__PURE__ */ new Map(), this.buffer = [], this.currentData = void 0, this.lastEventTimestamp = 0, this.streamClosed = !1, this._requestId = null, this.abortController = new AbortController(), this.start = () => t(this, void 0, void 0, function* () {
        ...
        try {
          if (E === "client") {
            const q = (0, s.ensureEndpointIdFormat)(v), N = (g = (0, s.resolveEndpointPath)(v, void 0, "/stream")) !== null && g !== void 0 ? g : "", F = yield (A ? () => A(`${q}${N}`) : () => (console.warn('[fal.stream] Using the default token provider is deprecated. Please provide a `tokenProvider` function when using `connectionMode: "client"`. See https://docs.fal.ai/fal-client/authentication for more information.'), (0, i.getTemporaryAuthToken)(v, this.config)))(), { fetch: G } = this.config, V = new URL(this.url);
            V.searchParams.set("fal_jwt_token", F);
            const R = yield G(V.toString(), {
        ...
        if (!((_ = g.headers.get("content-type")) !== null && _ !== void 0 ? _ : "").startsWith(l)) {
          const N = v.getReader(), M = () => {
            N.read().then(({ done: F, value: G }) => {
              if (F) {
                this.emit("done", this.currentData);
                return;
              }
              this.buffer.push(G), this.currentData = G, this.emit("data", G), M();
            });
          };
          M();
          return;
        ...
        const T = new TextDecoder("utf-8"), x = g.body.getReader(), E = (0, n.createParser)((N) => {
          if (N.type === "event") {
            ...
            try {
              ...
              this.buffer.push(F), this.currentData = F, this.emit("data", F), this.emit("message", F);
            } catch (F) {
              this.emit("error", F);
            }
          }
...
function yu() {
  ...
  const r = oi(), n = pr(), i = it(), o = $r(), a = Da(), s = Fa(), l = Ve(), u = 500, d = {
    maxRetries: 3,
    baseDelay: 1e3,
    maxDelay: 6e4,
        ...
        return t(this, void 0, void 0, function* () {
          ...
          return new Promise((T, x) => {
            ...
            const M = () => t(this, void 0, void 0, function* () {
              var F;
              try {
                const G = yield y.status(h, {
                  requestId: _,
                  logs: (F = g.logs) !== null && F !== void 0 ? F : !1,
                  abortSignal: g.abortSignal
                });
                if (g.onQueueUpdate && g.onQueueUpdate(G), G.status === "COMPLETED") {
                  N(), T(G);
                  return;
                }
                A = setTimeout(M, q);
...
const bu = new TextEncoder(), vu = 50;
function Eu(t, e, r) {
  bu.encodeInto(t, e.subarray(r));
}
function Tu(t, e, r) {
  t.length > vu ? Eu(t, e, r) : wu(t, e, r);
}
const Su = 4096;
function Ba(t, e, r) {
  let n = e;
  ...
  for (; n < i; ) {
    ...
    if ((s & 128) === 0)
      ...
      o.push(s);
    o.length >= Su && (a += String.fromCharCode(...o), o.length = 0);
  }
  return o.length > 0 && (a += String.fromCharCode(...o)), a;
}
const Iu = new TextDecoder(), xu = 200;
function Au(t, e, r) {
  const n = t.subarray(e, e + r);
  return Iu.decode(n);
}
function ku(t, e, r) {
  return r > xu ? Au(t, e, r) : Ba(t, e, r);
}
class Kt {
  constructor(e, r) {
    U(this, "type");
...
const Va = -1, Pu = 4294967296 - 1, Ou = 17179869184 - 1;
function za({ sec: t, nsec: e }) {
  if (t >= 0 && e >= 0 && t <= Ou)
    if (e === 0 && t <= Pu) {
    ...
    return n.setUint32(0, e), Ha(n, 4, t), r;
  }
}
function Wa(t) {
...
function Ja(t) {
  ...
  switch (t.byteLength) {
    ...
    case 12: {
      const r = Ga(e, 4), n = e.getUint32(0);
      return { sec: r, nsec: n };
    }
    default:
...
const Nu = {
  ...
  tryToEncode(e, r) {
    ...
    return e instanceof Kt ? e : null;
  }
  decode(e, r, n) {
    const i = r < 0 ? this.builtInDecoders[-1 - r] : this.decoders[r];
    return i ? i(e, r, n) : new Kt(r, e);
  }
};
U(Mr, "defaultCodec", new Mr());
let cr = Mr;
function qu(t) {
  return t instanceof ArrayBuffer || typeof SharedArrayBuffer < "u" && t instanceof SharedArrayBuffer;
}
function Cn(t) {
  return t instanceof Uint8Array ? t : ArrayBuffer.isView(t) ? new Uint8Array(t.buffer, t.byteOffset, t.byteLength) : qu(t) ? new Uint8Array(t) : Uint8Array.from(t);
}
const Cu = 100, Lu = 2048;
class Fr {
  constructor(e) {
    ...
    U(this, "bytes");
    U(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? cr.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.maxDepth = (e == null ? void 0 : e.maxDepth) ?? Cu, this.initialBufferSize = (e == null ? void 0 : e.initialBufferSize) ?? Lu, this.sortKeys = (e == null ? void 0 : e.sortKeys) ?? !1, this.forceFloat32 = (e == null ? void 0 : e.forceFloat32) ?? !1, this.ignoreUndefined = (e == null ? void 0 : e.ignoreUndefined) ?? !1, this.forceIntegerToFloat = (e == null ? void 0 : e.forceIntegerToFloat) ?? !1, this.pos = 0, this.view = new DataView(new ArrayBuffer(this.initialBufferSize)), this.bytes = new Uint8Array(this.view.buffer);
  }
  clone() {
    return new Fr({
  ...
  encodeString(e) {
    const n = _u(e);
    this.ensureBufferSizeToWrite(5 + n), this.writeStringHeader(n), Tu(e, this.bytes, this.pos), this.pos += n;
  }
  encodeObject(e, r) {
  ...
  writeI64(e) {
    this.ensureBufferSizeToWrite(8), Ha(this.view, this.pos, e), this.pos += 8;
  }
  writeBigUint64(e) {
...
function Uu(t, e) {
  return new Fr(e).encodeSharedRef(t);
}
function pn(t) {
...
class $u {
  ...
  decode(e, r, n) {
    ...
    if (i != null)
      return this.hit++, i;
    this.miss++;
    const o = Ba(e, r, n), a = Uint8Array.prototype.slice.call(e, r, r + n);
    return this.store(a, o), o;
  }
}
const Ln = "array", er = "map_key", Ya = "map_value", Fu = (t) => {
...
class Bu {
  ...
  pushArrayState(e) {
    ...
    r.type = Ln, r.position = 0, r.size = e, r.array = new Array(e);
  }
  pushMapState(e) {
    const r = this.getUninitializedStateFromPool();
    r.type = er, r.readCount = 0, r.size = e, r.map = {};
  }
...
const Gt = -1, li = new DataView(new ArrayBuffer(0)), Hu = new Uint8Array(li.buffer);
try {
  li.getInt8(0);
} catch (t) {
...
const no = new RangeError("Insufficient data"), Gu = new $u();
class ze {
  constructor(e) {
    U(this, "extensionCodec");
    U(this, "context");
    ...
    U(this, "stack", new Bu());
    U(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? cr.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.rawStrings = (e == null ? void 0 : e.rawStrings) ?? !1, this.maxStrLength = (e == null ? void 0 : e.maxStrLength) ?? Ht, this.maxBinLength = (e == null ? void 0 : e.maxBinLength) ?? Ht, this.maxArrayLength = (e == null ? void 0 : e.maxArrayLength) ?? Ht, this.maxMapLength = (e == null ? void 0 : e.maxMapLength) ?? Ht, this.maxExtLength = (e == null ? void 0 : e.maxExtLength) ?? Ht, this.keyDecoder = (e == null ? void 0 : e.keyDecoder) !== void 0 ? e.keyDecoder : Gu, this.mapKeyConverter = (e == null ? void 0 : e.mapKeyConverter) ?? Fu;
  }
  clone() {
    return new ze({
      extensionCodec: this.extensionCodec,
  ...
  appendBuffer(e) {
    if (this.headByte === Gt && !this.hasRemaining(1))
      ...
      const r = this.bytes.subarray(this.pos), n = Cn(e), i = new Uint8Array(r.length + n.length);
  ...
  *decodeMulti(e) {
  ...
  async decodeAsync(e) {
    ...
    try {
      ...
      throw new RangeError(`Insufficient data in parsing ${pn(i)} at ${a} (${o} in the current buffer)`);
    } finally {
      this.entered = !1;
    }
  }
  ...
  doDecodeSync() {
    e: for (; ; ) {
      ...
      if (e >= 224)
        ...
        throw new le(`Unrecognized type byte: ${pn(e)}`);
      this.complete();
      const n = this.stack;
      for (; n.length > 0; ) {
        const i = n.top();
        if (i.type === Ln)
          ...
          i.key = this.mapKeyConverter(r), i.type = Ya;
          continue e;
        } else if (i.map[i.key] = r, i.readCount++, i.readCount === i.size)
          r = i.map, n.release(i);
        else {
          i.key = null, i.type = er;
          continue e;
        }
      }
      return r;
  ...
  readArraySize() {
    ...
    switch (e) {
      ...
      default: {
        if (e < 160)
          return e - 144;
        throw new le(`Unrecognized array type byte: ${pn(e)}`);
      }
    }
  }
  pushMapState(e) {
  ...
  decodeUtf8String(e, r) {
    ...
    const n = this.pos + r;
    let i;
    return this.stateIsMapKey() && ((o = this.keyDecoder) != null && o.canBeCached(e)) ? i = this.keyDecoder.decode(this.bytes, n, e) : i = ku(this.bytes, n, e), this.pos += r + e, i;
  }
  stateIsMapKey() {
    return this.stack.length > 0 ? this.stack.top().type === er : !1;
  }
  /**
   * @throws {@link RangeError}
   */
  ...
  readU64() {
    const e = Ru(this.view, this.pos);
    return this.pos += 8, e;
  }
  readI64() {
    const e = Ga(this.view, this.pos);
    return this.pos += 8, e;
  }
  readU64AsBigInt() {
...
function Vu(t, e) {
  return new ze(e).decode(t);
}
function zu(t, e) {
  return new ze(e).decodeMulti(t);
}
function Wu(t) {
...
async function Ju(t, e) {
  const r = ui(t);
  return new ze(e).decodeAsync(r);
}
function Ku(t, e) {
  const r = ui(t);
  return new ze(e).decodeArrayStream(r);
}
function Yu(t, e) {
  const r = ui(t);
  return new ze(e).decodeStream(r);
}
const Qu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  ExtensionCodec: cr,
  decode: Vu,
  decodeArrayStream: Ku,
  decodeAsync: Ju,
  decodeMulti: zu,
  decodeMultiStream: Yu,
  decodeTimestampExtension: Ka,
  decodeTimestampToTimeSpec: Ja,
  encode: Uu,
  encodeDateToTimeSpec: Wa,
  encodeTimeSpecToTimestamp: za,
  encodeTimestampExtension: Xa
}, Symbol.toStringTag, { value: "Module" })), Zu = /* @__PURE__ */ fu(Qu);
var fe = {}, io;
function ed() {
  if (io) return fe;
  ...
  let r = {}, n = () => !0, i = () => ({}), o = (k) => k, a = (k, P, C, L) => k.apply(C, L) && P.apply(C, L), s = (k, P, C, [L, W]) => P.call(C, k.call(C, L, W), W), l = (k, P) => Object.freeze(Object.create(k, P));
  function u(k, P, C) {
    return k.reduce((L, W) => function(...ne) {
      return C(L, W, this, ne);
    }, P);
  }
  function d(k) {
  ...
  function g(k, P, ...C) {
    let L = u(h(m, C).map((ne) => ne.fn), n, a), W = u(h(c, C).map((ne) => ne.fn), o, s);
    return l(this, {
      from: t(k),
      to: t(P),
      guards: t(L),
  ...
  function T(k, P, C) {
    return V(P, k, C, this.immediates) || k;
  }
  function x(k) {
  ...
  let q = {
    enter(k, P, C) {
      ...
      return F.isPrototypeOf(L) ? l(N, {
        machine: t(L),
        transitions: t(this.transitions)
      }).enter(k, P, C) : (L.then((W) => P.send({ type: "done", data: W })).catch((W) => P.send({ type: "error", error: W })), k);
    }
  }, N = {
    enter(k, P, C) {
      if (P.child = H(this.machine, (L) => {
        ...
        return delete P.child, V(P, k, { type: "done", data: L }, this.transitions.get("done"));
      }
      return k;
    }
  };
  ...
  function G(k, P, C = i) {
    return typeof k != "string" && (C = P || i, P = k, k = Object.keys(P)[0]), r._create && r._create(k, P), l(F, {
      context: t(C),
      current: t(k),
      states: t(P)
    });
  ...
  function V(k, P, C, L) {
    ...
    for (let { to: ne, guards: xe, reducers: ue } of L)
      if (xe(W, C)) {
        k.context = ue.call(k, W, C);
        let Oe = P.original || P, Ot = l(Oe, {
          current: t(ne),
          original: { value: Oe }
        });
        return r._onEnter && r._onEnter(P, ne, k.context, W, C), Ot.state.value.enter(Ot, k, C);
      }
  }
  function R(k, P) {
    let C = P.type || P, { machine: L } = k, { value: W, name: ne } = L.state;
    return W.transitions.has(C) ? V(k, L, P, W.transitions.get(C)) || L : (r._send && r._send(C, ne), L);
  }
  let O = {
    send(k) {
  ...
  function H(k, P, C, L) {
    let W = Object.create(O, {
      machine: e(k),
      context: e(k.context(C, L)),
      onChange: t(P)
    });
    return W.send = W.send.bind(W), W.machine = W.machine.state.value.enter(W.machine, W, L), W;
  }
  return fe.action = f, fe.createMachine = G, fe.d = r, fe.guard = y, fe.immediate = S, fe.interpret = H, fe.invoke = M, fe.reduce = p, fe.state = A, fe.transition = v, fe;
}
var oo;
function td() {
  if (oo) return lt;
  oo = 1;
  var t = lt && lt.__awaiter || function(R, O, H, k) {
    function P(C) {
    ...
    return new (H || (H = Promise))(function(C, L) {
      ...
      function xe(ue) {
        ue.done ? C(ue.value) : P(ue.value).then(W, ne);
      }
      xe((k = k.apply(R, O || [])).next());
    });
  };
  Object.defineProperty(lt, "__esModule", { value: !0 }), lt.createRealtimeClient = V;
  const e = Zu, r = ed(), n = $a(), i = it(), o = ai(), a = Ve(), s = () => ({
  ...
  function g(R, { token: O, maxBuffering: H, path: k }) {
    ...
    const L = (0, a.ensureEndpointIdFormat)(R), W = (P = (0, a.resolveEndpointPath)(R, k, "/realtime")) !== null && P !== void 0 ? P : "";
    return `wss://fal.run/${L}${W}?${C.toString()}`;
  }
  const _ = 128;
  function b(R) {
  ...
  function G({ data: R, decodeMessage: O, onResult: H, onError: k, send: P }) {
    const C = (L) => {
      if (b(L)) {
        P({
          type: "unauthorized",
    ...
    Promise.resolve(O ? O(R) : R).then(C).catch((L) => {
      var W;
      k(new i.ApiError({
        message: (W = L == null ? void 0 : L.message) !== null && W !== void 0 ? W : "Failed to decode realtime message",
        status: 400
      }));
    });
  }
    ...
    return {
      connect(O, H) {
        const {
          ...
          maxBuffering: C,
          path: L,
          throttleInterval: W = _,
          encodeMessage: ne,
          decodeMessage: xe,
          tokenProvider: ue,
          tokenExpirationSeconds: Oe
        } = H;
        if (k && !(0, o.isBrowser)())
          return A;
        const Ot = ne ?? ((We) => F(We)), Qr = xe ?? ((We) => M(We));
        let mr, Li, Nt, qt = 0;
        T.set(P, {
          decodeMessage: Qr,
          onError: H.onError,
        ...
        const Zr = () => T.get(P), Ct = x(P, W, ({ context: We, machine: Xe, send: Ne }) => {
          var en;
          const { enqueuedMessage: tn, token: rn, websocket: nn } = We;
          if (Li = tn, Xe.current === "active" && tn && (nn == null ? void 0 : nn.readyState) === WebSocket.OPEN && Ne({ type: "send", message: tn }), Xe.current === "authRequired" && rn === void 0 && mr !== Xe.current) {
            Ne({ type: "initiateAuth" }), qt++;
            const Se = qt, de = (0, a.ensureEndpointIdFormat)(O), _e = (en = (0, a.resolveEndpointPath)(O, L, "/realtime")) !== null && en !== void 0 ? en : "", Je = ue ? () => ue(`${de}${_e}`) : () => (console.warn("[fal.realtime] Using the default token provider is deprecated. Please provide a `tokenProvider` function to `fal.realtime.connect()`. See https://docs.fal.ai/model-apis/client#client-side-usage-with-token-provider for more information."), (0, n.getTemporaryAuthToken)(O, R)), Lt = ue ? Oe : n.TOKEN_EXPIRATION_SECONDS, on = Lt !== void 0 ? () => {
              clearTimeout(Nt);
              const Ut = Math.round(Lt * 0.9 * 1e3);
              Nt = setTimeout(() => {
                Se === qt && Je().then((an) => {
                  Se === qt && (queueMicrotask(() => {
                    Ne({ type: "authenticated", token: an });
                  }), on());
                }).catch(() => {
                  if (Se !== qt)
                    return;
                  const an = Math.round(Lt * 0.05 * 1e3);
                  Nt = setTimeout(() => {
                    on();
                  }, an);
                });
            ...
            Je().then((Ut) => {
              queueMicrotask(() => {
                Ne({ type: "authenticated", token: Ut });
              }), on();
            }).catch((Ut) => {
              queueMicrotask(() => {
                Ne({ type: "unauthorized", error: Ut });
              });
            });
          }
          if (Xe.current === "connecting" && mr !== Xe.current && rn !== void 0) {
            const Se = new WebSocket(g(O, { token: rn, maxBuffering: C, path: L }));
            Se.onopen = () => {
              var de, _e;
              Ne({ type: "connected", websocket: Se });
              const Je = (_e = (de = Ct.context) === null || de === void 0 ? void 0 : de.enqueuedMessage) !== null && _e !== void 0 ? _e : Li;
              Je && (Se.send(Ot(Je)), Ct.context = Object.assign(Object.assign({}, Ct.context), { enqueuedMessage: void 0 }));
            }, Se.onclose = (de) => {
              if (de.code !== v.NORMAL_CLOSURE) {
                const { onError: _e = E } = Zr();
                _e(new i.ApiError({
                  message: `Error closing the connection: ${de.reason}`,
                  status: de.code
                }));
              }
              Ne({ type: "connectionClosed", code: de.code });
            }, Se.onerror = (de) => {
              const { onError: _e = E } = Zr();
              _e(new i.ApiError({ message: "Unknown error", status: 500 }));
            }, Se.onmessage = (de) => {
              const { decodeMessage: _e = Qr, onResult: Je, onError: Lt = E } = Zr();
              G({
                data: de.data,
                decodeMessage: _e,
                onResult: Je,
          ...
          mr === "active" && Xe.current !== "active" && (clearTimeout(Nt), Nt = void 0), mr = Xe.current;
        });
        return {
          send: (We) => {
            Ct.throttledSend({
...
function so() {
  if (ao) return st;
  ao = 1;
  var t = st && st.__awaiter || function(d, c, p, f) {
    function m(y) {
  ...
  Object.defineProperty(st, "__esModule", { value: !0 }), st.createFalClient = u;
  const e = si(), r = oi(), n = yu(), i = td(), o = pr(), a = it(), s = Da(), l = Fa();
  function u(d = {}) {
...
function rd() {
  if (lo) return Vt;
  lo = 1, Object.defineProperty(Vt, "__esModule", { value: !0 }), Vt.isQueueStatus = t, Vt.isCompletedQueueStatus = e;
  function t(r) {
...
function nd() {
  return uo || (uo = 1, (function(t) {
    var e = Ke && Ke.__createBinding || (Object.create ? (function(u, d, c, p) {
      p === void 0 && (p = c);
    ...
    })), r = Ke && Ke.__exportStar || function(u, d) {
...
const id = /* @__PURE__ */ JSON.parse('[{"display_name":"3D Rigging","job_set_type":"3d_rigging","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height_meters","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true}]},{"display_name":"Brain Activity","job_set_type":"brain_activity","type":"text","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Bytedance Image Upscale","job_set_type":"bytedance_image_upscale","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"4k","required":false,"enum":["2k","4k"]}]},{"display_name":"Bytedance Video Upscale","job_set_type":"bytedance_video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"fps","type":"integer","default":24,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"model_version","type":"string","default":"standard","required":false,"enum":["standard","pro"]},{"name":"preset","type":"string","default":"common","required":false,"enum":["common","aigc","short_series","ugc","old_film"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1080p","2k","4k"]}]},{"display_name":"Cinematic Studio 2.5","job_set_type":"cinematic_studio_2_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"auto","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio 3.0","job_set_type":"cinematic_studio_3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Cinematic Studio Image","job_set_type":"cinematic_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"string","default":null,"required":true},{"name":"camera_lens_id","type":"string","default":null,"required":true},{"name":"camera_model_id","type":"string","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio Soul Cast","job_set_type":"cinematic_studio_soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinematic Studio Soul Location","job_set_type":"cinematic_studio_soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Cinematic Studio Video","job_set_type":"cinematic_studio_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"slow_motion","type":"boolean","default":false,"required":false},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Cinematic Studio Video 3.5","job_set_type":"cinematic_studio_video_3_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"camera_style","type":"object","default":null,"required":false},{"name":"color_grading","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"light_scheme","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"style_id","type":"object","default":null,"required":false},{"name":"style_prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinema Studio 4.0","job_set_type":"cinematic_studio_video_4_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"color_palette","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"era_id","type":"object","default":null,"required":false},{"name":"extension_mode","type":"object","default":null,"required":false},{"name":"film_era","type":"null","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"null","default":null,"required":false},{"name":"genre_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"light","type":"object","default":null,"required":false},{"name":"light_custom","type":"object","default":null,"required":false},{"name":"light_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"model","type":"string","default":"default","required":false,"enum":["default","video_edit","video_extension"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"pacing_id","type":"object","default":null,"required":false},{"name":"preset_id","type":"null","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"speedramp","type":"object","default":"auto","required":false},{"name":"use_blur","type":"boolean","default":false,"required":false},{"name":"use_eye_mask","type":"boolean","default":false,"required":false},{"name":"use_transparency","type":"boolean","default":false,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Cinematic Studio Video V2","job_set_type":"cinematic_studio_video_v2","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"cfg_scale","type":"number","default":0.5,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","western","suspense","intimate","spectacle"]},{"name":"kling_element_ids","type":"array","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Clipify","job_set_type":"clipify","type":"video","params":[{"name":"clip_aspect","type":"string","default":"9:16","required":false,"enum":["9:16","1:1","16:9"]},{"name":"clips_num","type":"integer","default":10,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"max_height","type":"integer","default":1080,"required":false},{"name":"segment_seconds","type":"integer","default":10,"required":false},{"name":"subtitle_case","type":"string","default":"as-is","required":false,"enum":["lower","upper","as-is"]},{"name":"subtitle_font","type":"string","default":"notosans","required":false},{"name":"subtitle_highlight_hex","type":"string","default":"#FFE84D","required":false},{"name":"subtitle_position","type":"string","default":"bottom","required":false,"enum":["bottom","center","top"]},{"name":"track_face_crop","type":"boolean","default":true,"required":false},{"name":"urls","type":"array","default":null,"required":true}]},{"display_name":"Draw To Video","job_set_type":"draw_to_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"ref_image","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"sketch","type":"object","default":null,"required":true},{"name":"video","type":"object","default":null,"required":true}]},{"display_name":"dubbing","job_set_type":"dubbing","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"target_language","type":"string","default":null,"required":true,"enum":["eng","cmn","fra","hin","ita","jpn","kor","por","rus","tur","spa","deu","ara","pol","ind","fil","swe","fin"]}]},{"display_name":"Explainer Video","job_set_type":"explainer_video","type":"video","params":[{"name":"height","type":"integer","default":null,"required":true},{"name":"items","type":"array","default":null,"required":true},{"name":"subtitles","type":"object","default":null,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"FLUX.2","job_set_type":"flux_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"pro","required":false,"enum":["pro","flex","max"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"FLUX.2 Pro Outpaint","job_set_type":"flux_2_pro_outpaint","type":"image","params":[{"name":"expand_bottom","type":"integer","default":0,"required":false},{"name":"expand_left","type":"integer","default":0,"required":false},{"name":"expand_right","type":"integer","default":0,"required":false},{"name":"expand_top","type":"integer","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"FLUX 3 Video","job_set_type":"flux_3_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","2:1","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Flux Kontext","job_set_type":"flux_kontext","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Gemini Omni Flash","job_set_type":"gemini_omni","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"integer","default":8,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false}]},{"display_name":"GPT Image 2","job_set_type":"gpt_image_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"high","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Grok Image","job_set_type":"grok_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","1:2","2:1","3:2","2:3","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","quality"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Grok Video","job_set_type":"grok_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Grok Video 1.5","job_set_type":"grok_video_v15","type":"video","params":[{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Happy Horse Video","job_set_type":"happy_horse_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Hunyuan 3D v3.1 Text to 3D","job_set_type":"hunyuan3d_v3_1_text_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Hunyuan3D v3 Image to 3D","job_set_type":"hunyuan3d_v3_image_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"integer","default":500000,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"string","default":"Normal","required":false,"enum":["Normal","LowPoly","Geometry"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"polygon_type","type":"string","default":"triangle","required":false,"enum":["triangle","quadrilateral"]}]},{"display_name":"Image Auto","job_set_type":"image_auto","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Image Background Remover","job_set_type":"image_background_remover","type":"image","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Image Decompose","job_set_type":"image_decompose","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"granular","required":false,"enum":["granular","standard"]}]},{"display_name":"Image to 3D","job_set_type":"image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Inworld Text to Speech","job_set_type":"inworld_text_to_speech","type":"audio","params":[{"name":"prompt","type":"string","default":null,"required":true},{"name":"voice","type":"string","default":null,"required":true}]},{"display_name":"Kimodo","job_set_type":"kimodo","type":"3d","params":[{"name":"diffusion_steps","type":"integer","default":10,"required":false},{"name":"duration","type":"object","default":null,"required":false},{"name":"durations","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_version","type":"string","default":"ardy-core","required":false,"enum":["ardy-core","ardy-core-h8"]},{"name":"prompt","type":"object","default":null,"required":false},{"name":"prompts","type":"object","default":null,"required":false},{"name":"seed","type":"integer","default":42,"required":false}]},{"display_name":"Kling O1 Image","job_set_type":"kling_omni_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","auto","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Kling 2.6 Video","job_set_type":"kling2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Kling v3.0","job_set_type":"kling3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std","4k"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]}]},{"display_name":"Kling 3.0 Motion Control","job_set_type":"kling3_0_motion_control","type":"video","params":[{"name":"background_source","type":"string","default":"input_image","required":false,"enum":["input_image","input_video"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","pro"]}]},{"display_name":"Kling 3.0 Turbo","job_set_type":"kling3_0_turbo","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"LLM Generation","job_set_type":"llm_text","type":"video","params":[{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":null,"required":true},{"name":"reasoning_effort","type":"object","default":null,"required":false},{"name":"system_prompt","type":"string","default":"","required":false},{"name":"user_prompt","type":"string","default":"","required":false}]},{"display_name":"Marketing Studio Image","job_set_type":"marketing_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Marketing Studio Video","job_set_type":"marketing_studio_video","type":"video","params":[{"name":"ad_reference_id","type":"object","default":null,"required":false},{"name":"aspect_ratio","type":"string","default":"9:16","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"avatar_ids","type":"array","default":null,"required":false},{"name":"avatars","type":"array","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"hook_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"ugc","required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"setting_id","type":"object","default":null,"required":false},{"name":"specific_mode","type":"string","default":"default","required":false,"enum":["default","web_product","from_storyboard"]},{"name":"storyboard_id","type":"object","default":null,"required":false},{"name":"web_product_ids","type":"array","default":null,"required":false},{"name":"web_product_type","type":"object","default":null,"required":false}]},{"display_name":"Meshy 5 Remesh","job_set_type":"meshy_v5_remesh","type":"3d","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true},{"name":"origin_at","type":"object","default":null,"required":false},{"name":"resize_height","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Meshy 6 Text to 3D","job_set_type":"meshy_v6_text_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"enable_prompt_expansion","type":"boolean","default":false,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"full","required":false},{"name":"model_type","type":"string","default":"standard","required":false},{"name":"pose_mode","type":"string","default":"","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"boolean","default":true,"required":false},{"name":"symmetry_mode","type":"string","default":"auto","required":false},{"name":"target_polycount","type":"integer","default":30000,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"string","default":"triangle","required":false}]},{"display_name":"MiniMax H3","job_set_type":"minimax_h3","type":"video","params":[{"name":"aigc_watermark","type":"boolean","default":false,"required":false},{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":4,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"resolution","type":"string","default":"2K","required":false,"enum":["768P","2K"]},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Minimax Hailuo","job_set_type":"minimax_hailuo","type":"video","params":[{"name":"duration","type":"string","default":6,"required":false,"enum":["6","10"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"minimax-2.3","required":false,"enum":["minimax","minimax-fast","minimax-2.3","minimax-2.3-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"768","required":false,"enum":["512","768","1080"]}]},{"display_name":"Mirelo Text to Audio","job_set_type":"mirelo_text_to_audio","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"MS Image","job_set_type":"ms_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"avatars","type":"array","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"brand_kit_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"low","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Multi-Image to 3D","job_set_type":"multi_image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Nano Banana","job_set_type":"nano_banana","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_ai_stylist","type":"image","params":[{"name":"background_preset_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"outfit_preset_ids","type":"array","default":null,"required":false},{"name":"pose_preset_id","type":"object","default":null,"required":false},{"name":"user_outfit_ids","type":"array","default":null,"required":false}]},{"display_name":"Nano Banana 2 Lite","job_set_type":"nano_banana_2_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"thinking","type":"string","default":"HIGH","required":false,"enum":["MINIMAL","HIGH"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_relight","type":"image","params":[{"name":"brightness","type":"integer","default":null,"required":true},{"name":"color","type":"string","default":null,"required":true},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"light_quality","type":"string","default":null,"required":true,"enum":["hard","sharp","soft"]},{"name":"light_source","type":"string","default":null,"required":true,"enum":["mdl","mdr","mul","mur","bml","fml","fmr","bmm","mml","mmr","fmm","bmr","mdm","mum","bdr","fdl","bur","ful","bdl","fdr","bul","fur","bdm","fdm","bum","fum"]},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_shots","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_skin_enhancer","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"preset_id","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana 2","job_set_type":"nano_banana_flash","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"OpenAI Hazel","job_set_type":"openai_hazel","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","auto"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"medium","required":false,"enum":["low","medium","high"]}]},{"display_name":"Outpaint","job_set_type":"outpaint","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"21:9","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Qwen Audio 3.0 TTS Flash","job_set_type":"qwen_audio_tts","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"instruction","type":"object","default":null,"required":false},{"name":"language","type":"object","default":null,"required":false},{"name":"pitch_rate","type":"number","default":1,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","22050","24000","44100","48000"]},{"name":"seed","type":"integer","default":0,"required":false},{"name":"speech_rate","type":"number","default":1,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"integer","default":50,"required":false}]},{"display_name":"Angles","job_set_type":"qwen_camera_control","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"move_forward_level","type":"integer","default":0,"required":false},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"rotate_degree","type":"integer","default":0,"required":false},{"name":"vertical_angle","type":"integer","default":0,"required":false},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Recraft V4.1","job_set_type":"recraft_v4_1","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:4","4:3","4:5","5:4","3:2","2:3","16:9","9:16","21:9"]},{"name":"background_color","type":"object","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"colors","type":"array","default":null,"required":false},{"name":"model_type","type":"string","default":"standard","required":false,"enum":["standard","vector","utility","utility_vector"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Reframe","job_set_type":"reframe","type":"video","params":[{"name":"aspect_ratio","type":"string","default":null,"required":true,"enum":["21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"3D Objects","job_set_type":"sam_3_3d","type":"3d","params":[{"name":"detection_threshold","type":"object","default":null,"required":false},{"name":"export_textured_glb","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"3D Body","job_set_type":"sam_3_3d_body","type":"3d","params":[{"name":"export_meshes","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"include_3d_keypoints","type":"boolean","default":true,"required":false},{"name":"include_mhr_params","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Remove Background","job_set_type":"sam_3_video","type":"video","params":[{"name":"apply_mask","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"frames_count","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false}]},{"display_name":"Seed Audio 1.0","job_set_type":"seed_audio","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"expression_intensity","type":"integer","default":5,"required":false},{"name":"format","type":"string","default":"wav","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"loudness_rate","type":"integer","default":0,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mood","type":"number","default":0,"required":false},{"name":"pitch_rate","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","24000","32000","44100","48000"]},{"name":"speech_rate","type":"integer","default":0,"required":false},{"name":"voice_id","type":"object","default":null,"required":false},{"name":"voice_style","type":"object","default":null,"required":false},{"name":"voice_type","type":"object","default":null,"required":false},{"name":"voices","type":"array","default":null,"required":false}]},{"display_name":"Seedance 2.0","job_set_type":"seedance_2_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]}]},{"display_name":"Seedance 2.0 Mini","job_set_type":"seedance_2_0_mini","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p"]}]},{"display_name":"Seedance 2.5","job_set_type":"seedance_2_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"audio_references","type":"array","default":null,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"end_image","type":"object","default":null,"required":false},{"name":"extension_mode","type":"string","default":null,"required":false,"enum":["backward","forward"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"image_references","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"t2v","required":false,"enum":["t2v","omni_reference","video_edit","video_extension"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"start_image","type":"object","default":null,"required":false},{"name":"video_references","type":"array","default":null,"required":false}]},{"display_name":"Seedance 1.5 Pro","job_set_type":"seedance1_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"duration","type":"string","default":4,"required":false,"enum":["4","8","12"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Seedream 4.5","job_set_type":"seedream_v4_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","4:3","16:9","3:2","21:9","3:4","9:16","2:3"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Lite","job_set_type":"seedream_v5_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Pro","job_set_type":"seedream_v5_pro","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"height","type":"object","default":null,"required":false},{"name":"is_inpaint","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","1.5k","2k"]},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Sonilo Music","job_set_type":"sonilo_music","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Soul Cast","job_set_type":"soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"soul_cinema_studio","job_set_type":"soul_cinema_studio","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Soul Cinematic","job_set_type":"soul_cinematic","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]}]},{"display_name":"Soul Location","job_set_type":"soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Sync Lipsync 3","job_set_type":"sync_so","type":"video","params":[{"name":"active_speaker_detection","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_audio","type":"object","default":null,"required":true},{"name":"input_video","type":"object","default":null,"required":true},{"name":"occlusion_detection_enabled","type":"boolean","default":false,"required":false},{"name":"sync_mode","type":"string","default":"bounce","required":false,"enum":["bounce","loop","cut_off","silence","remap"]},{"name":"temperature","type":"number","default":0.5,"required":false}]},{"display_name":"Higgsfield Soul 2.0","job_set_type":"text2image_soul_v2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"Text to Speech V2","job_set_type":"text2speech_v2","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"emotion","type":"object","default":null,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["mp3","wav"]},{"name":"language_boost","type":"string","default":"auto","required":false,"enum":["auto","af","ar","bg","ca","cs","da","de","el","en","es","fa","fi","fil","fr","he","hi","hr","hu","id","it","ja","ko","ms","nl","nn","no","pl","pt","ro","ru","sk","sl","sv","ta","th","tr","uk","vi","yue","zh"]},{"name":"model","type":"string","default":null,"required":true,"enum":["elevenlabs","minimax","seed_speech","vibe_voice","cozy_voice"]},{"name":"pitch","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":32000,"required":false,"enum":["8000","16000","22050","24000","32000","44100"]},{"name":"speed","type":"number","default":1,"required":false},{"name":"stability","type":"object","default":null,"required":false},{"name":"text_normalization","type":"boolean","default":false,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"number","default":1,"required":false}]},{"display_name":"Topaz","job_set_type":"topaz_image","type":"image","params":[{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"Standard V2","required":false,"enum":["Standard V2","Low Resolution V2","CGI","High Fidelity V2","Text Refine"]},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"sharpen","type":"number","default":0,"required":false}]},{"display_name":"Topaz","job_set_type":"topaz_image_generative","type":"image","params":[{"name":"autoprompt","type":"boolean","default":true,"required":false},{"name":"creativity","type":"integer","default":1,"required":false},{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"Redefine","required":false,"enum":["Standard MAX","Redefine","Recovery","Recovery V2"]},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"sharpen","type":"number","default":0,"required":false},{"name":"texture","type":"integer","default":1,"required":false}]},{"display_name":"Topaz","job_set_type":"topaz_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"enhancement","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"frame_interpolation","type":"object","default":null,"required":false},{"name":"frame_rate","type":"number","default":30,"required":false},{"name":"frames_count","type":"object","default":null,"required":false},{"name":"input_height","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":false},{"name":"input_video_size","type":"integer","default":0,"required":false},{"name":"input_width","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"1080p","required":false,"enum":["1080p","2160p"]}]},{"display_name":"Text to 3D","job_set_type":"tripo_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"negative_prompt","type":"object","default":null,"required":false},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]}]},{"display_name":"Tripo H3.1 Image to 3D","job_set_type":"tripo_h3_1_image_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Tripo H3.1 Multiview to 3D","job_set_type":"tripo_h3_1_multiview_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Google Veo 3","job_set_type":"veo3","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"veo-3-fast","required":false,"enum":["veo-3-preview","veo-3-fast"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Google Veo 3.1","job_set_type":"veo3_1","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"model","type":"string","default":"veo-3-1-fast","required":false,"enum":["veo-3-1-preview","veo-3-1-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high","ultra"]}]},{"display_name":"Google Veo 3.1 Lite","job_set_type":"veo3_1_lite","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","auto"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Video Background Remover","job_set_type":"video_background_remover","type":"video","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Video Deflicker","job_set_type":"video_deflicker","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"Video Upscale","job_set_type":"video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"voice_change","job_set_type":"voice_change","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":"preset","required":false,"enum":["preset","element"]}]},{"display_name":"Wan 2.6 Video","job_set_type":"wan2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10","15"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 2.7","job_set_type":"wan2_7","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 3.0","job_set_type":"wan3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enable_thinking","type":"boolean","default":false,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Z Image","job_set_type":"z_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"prompt","type":"string","default":null,"required":true}]}]'), od = {
  ...
  "sketch",
  "texture_image_url"
]), ud = /* @__PURE__ */ new Set(["input_images"]), dd = /* @__PURE__ */ new Set(["input_video", "video"]), cd = /* @__PURE__ */ new Set(["input_audio"]);
function di(t) {
  return t.split(/[_-]+/).filter(Boolean).map((e) => e.charAt(0).toUpperCase() + e.slice(1)).join(" ");
}
function fd(t) {
  return sd[t] ?? `hf-${t.replaceAll("_", "-")}`;
}
function pd(t) {
...
const _d = ["prompt", "user_prompt", "instruction"];
function wd(t) {
  for (const e of _d) {
...
function vd(t = Qa) {
  const e = {};
  for (const r of t) {
    const n = fd(r.job_set_type), i = pd(r.type);
    if (e[n]) throw new Error(`Duplicate Higgsfield node type: ${n}`);
    const o = wd(r.params.flatMap((s) => [
      gd(r, hd(r, s)),
      ...yd(r, s)
    ])), a = o.findIndex((s) => s.id === "prompt") + 1;
    o.splice(a, 0, ...bd(r)), e[n] = {
      id: r.job_set_type,
      nodeType: n,
      name: r.display_name,
      category: i,
      ...
      inputs: o,
      outputType: i,
      outputs: [{ id: i, portType: i, label: i === "model3d" ? "3D Model" : di(i) }],
      provider: "higgsfield",
...
const Td = {
  ...
  video: "--video",
  audio: "--audio"
}, Sd = /^[A-Za-z][A-Za-z0-9_]*$/, Id = /* @__PURE__ */ new Set(["json", "wait", "no_color"]);
function xd(t) {
  const e = ["generate", "create", t.model], r = {
    ...Ed(t.model, { ...t.extra, ...t.params }) ?? {}
  }, n = (o, a) => {
    if (a == null) return;
    if (!Sd.test(o) || Id.has(o))
  ...
  for (const o of t.medias ?? [])
    o.value && e.push(Td[o.role], o.value);
...
const Ad = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;
function kd(t) {
...
function jd(t) {
  for (const e of ["results", "jobs"]) {
    const r = t[e];
    if (Array.isArray(r) && r.length > 0 && Be(r[0])) return r[0];
  }
  return t;
}
function fi(t) {
  ...
  const r = (a) => Array.isArray(a) ? { results: a } : Be(a) ? a : { result: a };
  let n = null;
  try {
    n = r(JSON.parse(e));
  } catch {
...
`);
  ...
  return (r = e.match(Ad)) == null ? void 0 : r[0];
}
function Be(t) {
  return !!t && typeof t == "object" && !Array.isArray(t);
}
const Rd = /* @__PURE__ */ new Set(["params", "prompt", "input_images", "inputs", "extra", "request"]), fo = [
  "url",
  "urls",
  "video_url",
  "image_url",
...
function ht(t, e = 0) {
  ...
  if (Array.isArray(t))
    return [...new Set(t.flatMap((n) => ht(n, e + 1)))];
  if (!Be(t)) return [];
  const r = [];
  for (const n of fo)
    t[n] !== void 0 && r.push(...ht(t[n], e + 1));
  if (typeof t.result_json == "string" && t.result_json.trim())
    try {
      r.push(...ht(JSON.parse(t.result_json), e + 1));
    } catch {
    }
  for (const n of po)
    t[n] !== void 0 && r.push(...ht(t[n], e + 1));
  for (const [n, i] of Object.entries(t))
    Rd.has(n) || fo.includes(n) || po.includes(n) || n === "result_json" || (Be(i) || Array.isArray(i)) && r.push(...ht(i, e + 1));
  return [...new Set(r)];
}
const Pd = /https?:\/\/[^\s"'<>\\]+/gi;
function Za(t) {
...
function pi(t, e) {
  var l;
  const r = ht(t.parsed), n = r[0], i = Ir(t.parsed), o = t.record.duration ?? ((l = t.record.output) == null ? void 0 : l.duration), a = e.mediaType, s = r.map((u) => ({ kind: a, url: u }));
...
function ts(t) {
  return t instanceof Un ? { stdout: t.stdout, stderr: t.stderr } : { stdout: "", stderr: "" };
}
...
const mo = 1260 * 1e3, Ld = 9e4, ho = 4, mn = "Higgsfield CLI not found. Install @higgsfield/cli, then run higgsfield auth login — or connect Higgsfield in Settings.";
function Ud(t) {
...
function Md(t, e, r) {
  return new Promise((n, i) => {
    var u, d;
    const o = oe(t, e, { env: Cd() });
    let a = "", s = "";
    const l = setTimeout(() => {
      o.kill("SIGTERM"), i(new Un("Higgsfield CLI timed out", a, s));
    }, r);
...
async function He(t, e = 6e4) {
  const r = t.includes("--json") ? t : [...t, "--json"], n = qd(), i = ut ? [ut, ...n.filter((l) => l !== ut)] : n;
  if (i.length === 0) throw new Error(mn);
  let o;
  for (const l of i)
    try {
      const u = await Md(l, r, e);
      return ut = l, u;
    } catch (u) {
      if (Ud(u)) {
        ut === l && (ut = null), o = u;
  ...
  throw new Error(s ? `${mn} Tried: ${a}. ${s}` : `${mn} Tried: ${a}.`);
}
const Dd = 6e4, rs = {
  "image/png": ".png",
  "image/jpeg": ".jpg",
...
function $d(t, e, r) {
  const n = e ? rs[e.split(";", 1)[0].trim().toLowerCase()] : void 0;
  if (n) return n;
...
async function Fd(t) {
  ...
  for (const [n, i] of t.entries()) {
    ...
    if (/^https?:\/\//i.test(o)) {
      const a = await fetch(o, { signal: AbortSignal.timeout(Dd) });
      if (!a.ok) throw new Error(`Failed to download media input (HTTP ${a.status}): ${o}`);
      const s = $d(o, a.headers.get("content-type"), i.role), l = w.join(z.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${s}`);
      ...
      const s = o.slice(5, a), l = s.replace(/;base64$/i, ""), u = o.slice(a + 1), d = /;base64$/i.test(s) ? Buffer.from(u, "base64") : Buffer.from(decodeURIComponent(u)), c = rs[l.toLowerCase()] ?? ns(i.role), p = w.join(z.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${c}`);
      await B.promises.writeFile(p, d), e.push(p), r.push({ ...i, value: p });
    } else
...
async function $n(t) {
  const { medias: e, tempPaths: r } = await Fd(t.medias);
  let n;
  try {
    n = await Bd({ ...t, medias: e });
  } finally {
    for (const o of r)
      B.promises.unlink(o).catch(() => {
      });
  }
  if (ci(n, t.mediaType)) return n;
...
async function Bd(t) {
  ...
  for (let n = 1; n <= 3; n++)
    try {
      const i = await He(e, Ld), o = await Dn(i, t);
      if (o) return o;
      const a = co(i);
      if (a) return { jobId: a };
      ...
      r = i;
      const { stdout: o, stderr: a } = ts(i), s = await Dn(o, t);
      if (s) return s;
      const l = co(o, a, _t(i));
      if (l) return { jobId: l };
      if (!Mn(_t(i)) || n === 3) throw i;
      await is(1500 * n);
    }
...
async function os(t, e) {
  try {
    const r = await ss({ size: 50 }), n = Hd(r, t);
    if (!n) return;
    const i = pi({
...
async function mi(t, e) {
  const r = await He(["generate", "get", t], 2e4), n = fi(r);
...
async function Gd(t) {
  try {
    ...
    if (!/unknown|unexpected|unrecognized/i.test(_t(e))) throw e;
    return He(
...
async function as(t, e) {
  ...
  for (let i = 1; i <= ho; i++)
    try {
      ...
      r = o;
      const { stdout: a } = ts(o), s = await Dn(a, e);
      if (s) return s;
      try {
        const d = await mi(t, e);
        if (d) return d;
      } catch (d) {
        if (!Mn(_t(d))) throw d;
      }
      const l = _t(o);
      if (!(Mn(l) || /timed out/i.test(l) || /still running/i.test(l) || /without a media URL/i.test(l))) throw o;
      if (i === ho) break;
      await is(2e3 * i);
    }
  ...
  if (n) return n;
  throw new Error(
    `${_t(r)} The job was submitted (${t}) and may still finish on Higgsfield.`
  );
}
function go(t) {
  if (Array.isArray(t)) return t.filter((e) => Be(e));
  if (!Be(t)) return [];
  for (const e of ["jobs", "results", "data", "items", "generations"]) {
    const r = t[e];
    if (Array.isArray(r)) return r.filter((n) => Be(n));
  }
...
async function ss(t) {
  ...
  const n = (await He(e, 2e4)).trim();
  try {
    return go(JSON.parse(n));
  } catch {
    const i = Math.max(n.lastIndexOf("["), n.lastIndexOf("{"));
    return i < 0 ? [] : go(JSON.parse(n.slice(i)));
  }
}
async function Vd(t, e) {
  const r = await mi(t, e);
  return r && ci(r, e.mediaType) ? r : as(t, e);
}
async function yo() {
  try {
    const t = await He(["account", "status"], 15e3);
...
function zd() {
  I.handle("higgsfield:account-status", async () => _o(await yo())), I.handle("higgsfield:quick-edit", async (t, e) => {
    ...
    if (e.drawnFramePath && e.referenceMode === "frame")
      ...
      try {
        ...
        console.log("[higgsfield:quick-edit] extracted refs:", s.paths), i = wo({
    ...
    if (e.jobId) {
      ...
      if (e.wait === !1) {
        const o = await mi(e.jobId, i);
        if (o && ci(o, e.outputType)) return o;
        throw new Error("Higgsfield job is still running");
      }
      return Vd(e.jobId, i);
    }
  ...
  }), I.handle("higgsfield:generate-list", async (t, e) => ss(e)), I.handle("higgsfield:auth-login", async () => {
    try {
      await He(["auth", "login"], 300 * 1e3);
    } catch (t) {
      return { connected: !1, error: t instanceof Error ? t.message : String(t) };
    }
    return _o(await yo());
  }), I.handle("higgsfield:auth-logout", async () => {
    await He(["auth", "logout"], 15e3).catch(() => {
    });
  });
}
const Wd = {
...
}, Kd = "notrius/ltx-2.5-serverless:cu130@sha256:73d1621ef915ae6a149f2a32f6c317dfc89f12075ed4b3abd7df707420267205", hi = "https://rest.runpod.io/v1", Yd = "https://api.runpod.io/v2", ls = "https://api.runpod.io/graphql", gi = 8e3, hn = 256 * 1024, Qd = 1800, Zd = 200, Fn = 15e3, us = 6500, ec = 12e4, yi = 12e3, ds = 14 * 1024 * 1024, tc = 100 * 1024 * 1024, bo = 1024 * 1024, vo = 4, cs = Object.freeze(["sdxl", "qwen-image-edit"]), rc = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABACAIAAAAlC+aJAAAAYElEQVR4nO3PQQ0AIBDAMMD4WUcEj4ZkVbDtmVk/OzrgVQNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgPaBRFyAf0dnk7yAAAAAElFTkSuQmCC", nc = "balanced", Eo = Object.freeze({
...
function ve(t, e) {
  if (typeof t != "string" || !t.trim())
    throw new $(`${e} is required.`, "MISSING_CONFIGURATION", 422);
  return t.trim();
}
function Br(t, e) {
  const r = ve(t, e);
  if (!/^[A-Za-z0-9_-]{1,191}$/.test(r))
    throw new $(`${e} is invalid.`, "INVALID_CONFIGURATION", 422);
  return r;
}
async function oc(t) {
...
async function fs(t, e, r = {}, n = Fn) {
  ...
  let s;
  const l = new _i(n), u = new Promise((c, p) => {
    s = setTimeout(() => {
      i.abort(l), p(l);
    }, n);
  }), d = (async () => {
    const c = await t(e, { ...r, signal: i.signal }), p = await oc(c);
...
function ac(t) {
  try {
    return new URL(t).pathname === "/health" ? us : Fn;
  } catch {
    return Fn;
  }
}
...
async function Te(t, e, r, n, i = [200, 201, 202, 204], o = ac(e)) {
  let a;
  try {
    a = await fs(t, e, r, o);
  } catch (u) {
    throw u instanceof _i ? new $(`${n} RunPod did not respond before the request timed out.`, "PROVIDER_TIMEOUT", 504) : new $(u instanceof Error ? u.message : n, "PROVIDER_UNREACHABLE", 502);
  }
  const { response: s, payload: l } = a;
  if (!i.includes(s.status))
    throw new $(Hr(l, `${n} (${s.status})`), "PROVIDER_ERROR", s.status);
  return l;
}
function wi(t, e = !1) {
  return {
...
async function lc(t, e, r) {
  var a;
  const n = new AbortController(), i = setTimeout(() => n.abort(), Qd);
  let o;
  try {
    const s = new URL(`${Yd}/pods/${encodeURIComponent(r)}/logs`);
    ...
    for (; !c.done && d.length < hn && (d += u.decode(c.value, { stream: !0 }), !(d.length >= hn)); ) {
      let p;
      const f = new Promise((m) => {
        p = setTimeout(() => m({ quiet: !0 }), Zd);
      });
      if (c = await Promise.race([o.read(), f]), clearTimeout(p), c != null && c.quiet)
        break;
    }
    return d += u.decode(), sc(d.slice(0, hn));
  } catch {
    return [];
  } finally {
...
async function To(t, e, r, n) {
  const i = `mutation { secretCreate(input: { name: ${JSON.stringify(r)}, value: ${JSON.stringify(n)}, description: "Temporary CineGen LTX-2.5 session credential" }) { id name } }`, o = new URL(ls);
  ...
  if (s.length || typeof (u == null ? void 0 : u.id) != "string")
    throw new $(Hr(a, "RunPod could not create the encrypted session secret."));
  return u.id;
}
async function ps(t, e, r) {
  const n = `mutation { secretDelete(id: ${JSON.stringify(r)}) }`, i = new URL(ls);
...
function hc(t) {
  return `https://${t}-${gi}.proxy.runpod.net`;
}
function bi(t, e) {
  const r = Br(e, "RunPod session ID"), n = new URL(ve(t, "RunPod session URL"));
  if (n.protocol !== "https:" || n.username || n.password || n.hostname !== `${r}-${gi}.proxy.runpod.net`)
    throw new $("RunPod session URL is invalid.", "INVALID_CONFIGURATION", 422);
...
function yc(t) {
  const e = t === void 0 ? nc : t;
  if (typeof e != "string" || !Object.hasOwn(Eo, e))
    throw new $("Choose a valid LTX-2.5 GPU profile: economy, balanced, or performance.", "INVALID_GPU_PROFILE", 422);
  return { name: e, config: Eo[e] };
}
function _c(t) {
  if (t === void 0)
    return [];
  if (!Array.isArray(t))
    throw new $("Image models must be an array.", "INVALID_IMAGE_MODELS", 422);
  const e = [];
  for (const r of t) {
    if (typeof r != "string" || !cs.includes(r))
      throw new $("Choose only supported session image models: SDXL or Qwen Image Edit.", "INVALID_IMAGE_MODELS", 422);
...
async function bc(t, e = fetch) {
  const r = ve(t.runpodKey, "RunPod API key"), n = ve(t.huggingFaceToken, "Hugging Face read token");
  if (!/^hf_[A-Za-z0-9]+$/.test(n))
    throw new $("Enter a valid Hugging Face read token.", "INVALID_HUGGINGFACE_TOKEN", 422);
  const i = yc(t.gpuProfile), o = _c(t.imageModels), a = gc(), s = `cinegen_ltx25_hf_${a}`, l = `cinegen_ltx25_session_${a}`, u = `${crypto.randomUUID()}${crypto.randomUUID()}`.replace(/-/g, ""), d = [];
  try {
    d.push(await To(e, r, s, n)), d.push(await To(e, r, l, u));
    const c = await Te(e, `${hi}/pods`, {
      method: "POST",
      headers: wi(r, !0),
      body: JSON.stringify({
        ...
        containerDiskInGb: wc(i.config, o),
        volumeInGb: 0,
        ports: [`${gi}/http`],
        supportPublicIp: !0,
        interruptible: !1,
    ...
    }, "RunPod could not create the LTX-2.5 session Pod."), p = ms(c);
    if (!p.id)
      throw new $("RunPod created a Pod without returning its ID.");
    return {
      podId: p.id,
      podUrl: hc(p.id),
      podAuthToken: u,
...
async function vc(t, e = fetch) {
  const r = ve(t.runpodKey, "RunPod API key"), n = ve(t.podAuthToken, "RunPod session token"), i = bi(t.podUrl, t.podId);
  let o;
  try {
    o = await Te(e, `${hi}/pods/${i.podId}`, {
      headers: wi(r)
    }, "RunPod could not read the LTX-2.5 session.");
  } catch (c) {
    if (c instanceof $ && c.statusCode === 404)
      return {
        status: "ended",
        phase: "ended",
        podId: i.podId,
  ...
  if (a.desiredStatus === "RUNNING")
    try {
      const { response: c, payload: p } = await fs(e, `${i.url}/health`, {
      ...
      s = c instanceof _i ? { kind: "timeout" } : { kind: "unreachable" };
    }
  const l = await lc(e, r, i.podId), u = cc(l);
  if (!u && uc(l))
    try {
      const c = await hs({
        runpodKey: r,
  ...
  let d;
  return (s == null ? void 0 : s.kind) === "timeout" ? d = "RunPod reports the Pod is running, but its private gateway did not answer within 7 seconds. It may still be starting; check again shortly. Billing continues while the Pod runs." : (s == null ? void 0 : s.kind) === "response" && (s.status === 502 || s.status === 504) ? d = `RunPod reports the Pod is running, but its private gateway returned ${s.status}. The container may still be starting; check again shortly. Billing continues while the Pod runs.` : d = pc(s == null ? void 0 : s.body) ?? mc(l), !d && (s == null ? void 0 : s.kind) === "unreachable" ? d = "RunPod reports the Pod is running, but its private gateway is not reachable yet. The container may still be starting; check again shortly. Billing continues while the Pod runs." : !d && (s == null ? void 0 : s.kind) === "response" && s.status >= 400 && (d = `RunPod reports the Pod is running, but its private gateway returned HTTP ${s.status}. Check again shortly. Billing continues while the Pod runs.`), {
    status: "downloading",
    phase: "downloading",
    podId: i.podId,
    podUrl: i.url,
...
async function hs(t, e = fetch) {
  const r = ve(t.runpodKey, "RunPod API key"), n = Br(t.podId, "RunPod session ID");
  await Te(e, `${hi}/pods/${n}`, {
    method: "DELETE",
    headers: wi(r)
...
function Tc(t) {
  const e = JSON.parse(JSON.stringify(Wd)), r = ve(t.prompt, "Video prompt");
  if (r.length > yi)
    throw new $("The LTX-2.5 video prompt is too long.", "PROMPT_TOO_LONG", 422);
  const n = Math.min(20, Math.max(1, Math.round(Number(t.durationSec) || 5))), i = ["16:9", "9:16", "1:1"].includes(t.aspectRatio ?? "") ? t.aspectRatio : "16:9", o = t.resolution === "480p" || t.resolution === "1080p" ? t.resolution : "720p", a = Ec(i, o);
  return e["398:376"].inputs.value = r, e[395].inputs.image = "cinegen-source.png", e["398:362"].inputs.value = n, e["398:372"].inputs.value = a.width, e["398:360"].inputs.value = a.height, e["398:361"].inputs.value = 24, e["398:380"].inputs.sampling_mode = "on", e["398:380"].inputs["sampling_mode.seed"] = Math.floor(Math.random() * 999999998) + 1, e["398:383"].inputs.value = t.generateAudio !== !1, e["398:363"].inputs.value = gs(t).length === 0, e["398:338"].inputs.noise_seed = Math.floor(Math.random() * 999999999999998) + 1, e["398:339"].inputs.noise_seed = Math.floor(Math.random() * 999999999999998) + 1, e;
}
function Sc(t) {
  const e = gs(t), r = e.find((a) => a.startsWith("data:image/")) ?? (e.length ? "" : rc), n = /^data:(image\/(?:png|jpeg|webp|gif|bmp|avif));base64,([A-Za-z0-9+/]*={0,2})$/i.exec(r);
  if (!n || !n[2] || n[2].length % 4 === 1)
    throw new $("The LTX-2.5 reference image could not be prepared.", "INVALID_REFERENCE", 422);
  const i = n[2].endsWith("==") ? 2 : n[2].endsWith("=") ? 1 : 0;
  if (Math.floor(n[2].length * 0.75) - i > ds)
    throw new $("The first LTX-2.5 reference image is larger than 14 MB.", "REFERENCE_TOO_LARGE", 413);
  return r;
}
function ys(t) {
  if (typeof t != "string" || !cs.includes(t))
    throw new $("Choose SDXL or Qwen Image Edit for this image job.", "INVALID_IMAGE_MODEL", 422);
  return t;
}
function So(t) {
  try {
...
function Io(t, e) {
  ...
  if (!Number.isInteger(r) || r < 256 || r > 2048)
    throw new $("Image width and height must be whole pixels from 256 to 2048.", "INVALID_DIMENSIONS", 422);
...
function Ic(t) {
  ...
  if (!Number.isSafeInteger(e) || e < 0)
    throw new $("Image seed must be a non-negative whole number.", "INVALID_SEED", 422);
  return e;
}
function xc(t, e) {
  ...
  if (!r || !r[2] || r[2].length % 4 === 1)
    throw new $(`Qwen reference image ${e + 1} could not be prepared.`, "INVALID_REFERENCE", 422);
  const n = r[2].endsWith("==") ? 2 : r[2].endsWith("=") ? 1 : 0;
  if (Math.floor(r[2].length * 0.75) - n > ds)
    throw new $(`Qwen reference image ${e + 1} is larger than 14 MB.`, "REFERENCE_TOO_LARGE", 413);
  return t.trim();
}
function Ac(t, e) {
  const r = Array.isArray(t.referenceImages) ? t.referenceImages.filter((n) => typeof n == "string" && n.trim()) : [];
  if (e === "sdxl" && r.length)
    throw new $("SDXL session jobs are text-to-image and do not accept reference images.", "INVALID_REFERENCE", 422);
  if (e === "qwen-image-edit" && (r.length < 1 || r.length > 3))
    throw new $("Qwen Image Edit requires one to three reference images.", "INVALID_REFERENCE_COUNT", 422);
  return r.map(xc);
}
function kc(t, e) {
  const r = ve(t, e);
  if (r.length > yi)
    throw new $(`${e} is too long.`, "PROMPT_TOO_LONG", 422);
  return r;
}
function jc(t, e, r, n, i, o) {
  const a = JSON.parse(JSON.stringify(Xd));
...
function Rc(t, e, r, n) {
  const i = JSON.parse(JSON.stringify(Jd));
...
function Pc(t) {
  const e = ys(t.model), r = kc(t.prompt, "Image prompt"), n = typeof t.negativePrompt == "string" ? t.negativePrompt.trim().slice(0, yi) : e === "sdxl" ? "text, watermark, logo, low quality, distorted" : "", i = Io(t.width, 1024), o = Io(t.height, 1024), a = Ic(t.seed), s = Ac(t, e), l = e === "sdxl" ? jc(t, r, n, i, o, a) : Rc(r, n, a, s);
  return {
    model: e,
    label: Gr(e),
    workflow: l,
...
function qc(t, e) {
  const r = _s(t);
  if (r)
    throw new $(r, "GENERATION_FAILED", 502);
  for (const { record: n, parentKey: i } of Vr(t)) {
    const o = De(n.video_url, n.videoUrl);
    if (o)
      return { url: o, durationSec: e, model: "LTX-2.5" };
    const a = De(n.video_base64, n.videoBase64);
    if (a)
      return { data: a, mediaType: Rr(n), durationSec: e, model: "LTX-2.5" };
    if (!Oc(n, i))
      continue;
    const s = De(n.url, n.download_url, n.downloadUrl), l = De(n.data, n.base64), u = s ?? (String(n.type ?? "").toLowerCase() === "url" ? l : void 0);
    if (u)
      return { url: u, durationSec: e, model: "LTX-2.5" };
    const d = String(n.type ?? "").toLowerCase() === "url" ? void 0 : l;
    if (d)
      return { data: d, mediaType: Rr(n), durationSec: e, model: "LTX-2.5" };
  }
  throw new $("LTX-2.5 completed without returning a video.", "INVALID_PROVIDER_RESPONSE", 502);
}
function Cc(t, e) {
  const r = Gr(e), n = _s(t);
  if (n)
    throw new $(n, "GENERATION_FAILED", 502);
  for (const { record: i, parentKey: o } of Vr(t)) {
    const a = De(i.image_url, i.imageUrl);
    if (a)
      return { url: a, model: r };
    const s = De(i.image_base64, i.imageBase64);
    if (s)
      return { data: s, mediaType: Rr(i, "image/png"), model: r };
    if (!Nc(i, o))
      continue;
    const l = De(i.url, i.download_url, i.downloadUrl), u = De(i.data, i.base64), d = l ?? (String(i.type ?? "").toLowerCase() === "url" ? u : void 0);
    if (d)
      return { url: d, model: r };
    const c = String(i.type ?? "").toLowerCase() === "url" ? void 0 : u;
    if (c)
      return { data: c, mediaType: Rr(i, "image/png"), model: r };
  }
  throw new $(`${r} completed without returning an image.`, "INVALID_PROVIDER_RESPONSE", 502);
}
function ws(t, e = "video", r = "LTX-2.5") {
  for (const { record: n, parentKey: i } of Vr(t)) {
    if (i !== "artifact")
      continue;
    ...
    if (!/^[A-Za-z0-9_-]{1,191}$/.test(o) || !Number.isSafeInteger(a) || a <= 0 || a > tc || s && !s.startsWith(`${e}/`))
      throw new $(`${r} returned invalid artifact metadata.`, "INVALID_PROVIDER_RESPONSE", 502);
    return { id: o, byteSize: a, mediaType: s };
  }
}
function Lc(t, e = "LTX-2.5", r = "video") {
  if (typeof t != "string" || !t || t.length % 4 === 1 || !/^[A-Za-z0-9+/]*={0,2}$/.test(t))
    throw new $(`${e} returned an invalid ${r} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
  if (typeof Buffer < "u")
    return new Uint8Array(Buffer.from(t, "base64"));
  if (typeof atob != "function")
    throw new $(`This CineGen runtime cannot decode the generated ${r}.`, "RUNTIME_UNSUPPORTED", 500);
  let n;
  try {
    n = atob(t);
  } catch {
...
function Uc(t, e, r = "video", n = "LTX-2.5") {
  ...
  if (r === "image") {
    ...
    if (l)
      return "image/webp";
    throw new $(`${n} returned an unsupported image file.`, "INVALID_PROVIDER_RESPONSE", 502);
  }
  if (!i && !o)
    throw new $(`${n} returned an unsupported video file.`, "INVALID_PROVIDER_RESPONSE", 502);
  return i ? "video/webm" : e === "video/quicktime" ? "video/quicktime" : "video/mp4";
}
function Mc(t, e = "media") {
  if (typeof Buffer < "u")
    return Buffer.from(t.buffer, t.byteOffset, t.byteLength).toString("base64");
  if (typeof btoa != "function")
    throw new $(`This CineGen runtime cannot encode the generated ${e}.`, "RUNTIME_UNSUPPORTED", 500);
  const r = [], n = 32 * 1024;
...
async function bs(t, e, r, n, i = "video", o = "LTX-2.5") {
  ...
  for (let c = 0; c < l.length; c += vo) {
    const p = l.slice(c, c + vo), f = await Promise.all(p.map(async ({ offset: m, length: y }) => {
      ...
      if (!g || typeof g != "object")
        throw new $(`${o} returned an invalid ${i} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
      const _ = typeof g.id == "string" ? g.id : "", b = Number(g.offset), v = Number(g.byteSize ?? g.byte_size), S = typeof (g.mediaType ?? g.media_type) == "string" ? String(g.mediaType ?? g.media_type).trim() : "", T = Lc(g.data, o, i);
      if (_ !== n.id || b !== m || v !== n.byteSize || n.mediaType && S !== n.mediaType || T.byteLength !== y)
        throw new $(`${o} returned an inconsistent ${i} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
      return { offset: m, bytes: T };
    }));
    for (const m of f)
      m.offset === 0 && (s = m.bytes.slice(0, 12)), a.set(m.bytes, m.offset);
  }
  const u = Uc(s ?? new Uint8Array(), n.mediaType, i, o), d = Mc(a, i);
  return await Dc(t, e, r, n.id), { data: d, mediaType: u };
}
function vs(t) {
  if (!t || typeof t != "object")
    return !1;
...
async function $c(t, e, r, n) {
  const i = await Es(t, e, r), o = i == null ? void 0 : i.capabilities, a = Array.isArray(i == null ? void 0 : i.installedModels) ? i.installedModels : [];
  ...
  if (!a.includes(n))
    throw new $(
      `${Gr(n)} was not installed when this Pod was created. Start a new session with that image model selected.`,
...
function Bc(t) {
  return t instanceof $ && (t.code === "PROVIDER_TIMEOUT" || t.code === "PROVIDER_UNREACHABLE" || t.statusCode === 502 || t.statusCode === 504);
}
async function Hc(t, e, r, n) {
  try {
...
async function Ts(t, e, r, n, i, o, a) {
  var c;
  const s = Fc(), l = ((c = i == null ? void 0 : i.capabilities) == null ? void 0 : c.idempotentSubmissions) === !0, u = JSON.stringify({ input: { ...o, cinegen_job_id: s } }), d = () => Te(t, `${e.url}/run`, {
    method: "POST",
  ...
  for (let p = 0; p < (l ? 2 : 1); p += 1)
    try {
      ...
      if (!m)
        throw new $("The session did not return a generation job ID.", "INVALID_PROVIDER_RESPONSE", 502);
      if (l && m !== s)
        throw new $("The session returned a different generation job ID.", "INVALID_PROVIDER_RESPONSE", 502);
      return { payload: f, jobId: m };
    } catch (f) {
      if (!l || !Bc(f))
        throw f;
      const m = await Hc(t, e, r, s);
      if (m)
        return { payload: m, jobId: s };
      if (p === 1)
        throw f;
    }
  throw new $("CineGen could not confirm the generation submission.", "PROVIDER_TIMEOUT", 504);
}
async function Gc(t, e = fetch) {
  const r = bi(t.podUrl, t.podId), n = ve(t.podAuthToken, "RunPod session token"), i = { Authorization: `Bearer ${n}`, Accept: "application/json", "Content-Type": "application/json" };
  if (t.jobId) {
    const u = Br(t.jobId, "RunPod generation job ID"), d = await Te(e, `${r.url}/status/${u}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    ...
    if (c === "FAILED")
      return { jobId: u, status: "failed", phase: "error", error: Hr(d, "LTX-2.5 generation failed.") };
    if (c === "COMPLETED") {
      const p = Math.min(20, Math.max(1, Math.round(Number(d.durationSec) || 5)));
      try {
        const f = ws(d.output), m = f ? { ...await bs(e, r, n, f), durationSec: p, model: "LTX-2.5" } : qc(d.output, p);
  ...
  if (!t.input)
    throw new $("Video generation input is required.", "MISSING_INPUT", 422);
  const o = Math.min(20, Math.max(1, Math.round(Number(t.input.durationSec) || 5))), a = {
    workflow: Tc(t.input),
    images: [{ name: "cinegen-source.png", image: Sc(t.input) }],
    cinegen_duration_sec: o,
    cinegen_task: "ltx-2.5"
  }, s = await Es(e, r, n), { jobId: l } = await Ts(e, r, n, i, s, a, "CineGen could not submit the LTX-2.5 generation.");
...
async function Vc(t, e = fetch) {
  ...
  if (t.jobId) {
    const u = Br(t.jobId, "RunPod generation job ID"), d = await Te(e, `${r.url}/status/${u}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    }, "CineGen could not read the session image generation status."), c = So(d == null ? void 0 : d.task), p = So(t.model ?? ((l = t.input) == null ? void 0 : l.model));
    if (c && p && c !== p)
    ...
    const m = Gr(f), y = String(d.status ?? "").toUpperCase();
    if (y === "IN_QUEUE")
    ...
    if (y === "FAILED")
      return { jobId: u, status: "failed", phase: "error", error: Hr(d, `${m} generation failed.`) };
    if (y === "COMPLETED")
      try {
        const h = ws(d.output, "image", m), g = h ? { ...await bs(e, r, n, h, "image", m), model: m } : Cc(d.output, f);
  ...
  if (!t.input)
    throw new $("Image generation input is required.", "MISSING_INPUT", 422);
  const o = Pc(t.input), a = await $c(e, r, n, o.model), { jobId: s } = await Ts(e, r, n, i, a, {
    workflow: o.workflow,
    images: o.images,
...
const zc = bc, Wc = vc, Xc = hs, Jc = Gc, Kc = Vc, gt = "https://api.kie.ai/api/v1", Yc = 3e3, Qc = 120, Zc = {
  runway: `${gt}/runway/generate`,
  veo: `${gt}/veo/generate`,
  "4o-image": `${gt}/gpt4o-image/generate`,
  "suno-music": `${gt}/generate`
};
function ef(t) {
  for (const [e, r] of Object.entries(Zc))
    if (t.startsWith(e)) return r;
}
async function tf(t, e, r) {
  const n = ef(t), i = n ?? `${gt}/jobs/createTask`, o = n ? { ...e, callBackUrl: "" } : { model: t, input: e, callBackUrl: "" }, a = await fetch(i, {
    method: "POST",
    headers: {
...
async function rf(t, e) {
  for (let r = 0; r < Qc; r++) {
    await new Promise((a) => setTimeout(a, Yc));
    const n = await fetch(`${gt}/jobs/recordInfo?taskId=${t}`, {
...
function tr(t, e, r = !1) {
  if (typeof t == "string") {
    const a = Ss(t).trim(), s = r ? lf(a, e) : e;
    return a ? [{ value: a, role: s }] : [];
  }
  if (Array.isArray(t))
    return t.flatMap((a) => tr(a, e, r));
  if (!t || typeof t != "object") return [];
  const n = t, i = sf(n, e);
  if (Array.isArray(n.allUrls))
    return n.allUrls.flatMap((a) => tr(
      a,
      i.role,
      r && !i.explicit
    ));
...
function df(t, e, r) {
  const n = [], i = {}, o = t === "seedance_2_5" ? "image" : uf(r);
  for (const [a, s] of Object.entries(e)) {
    if (s == null) continue;
...
async function ff(t, e, r) {
  const n = await $n(df(t, e, r));
  return cf(n);
}
const xo = "https://api.runpod.ai/v2", pf = 3e3, mf = 120;
async function hf(t, e, r) {
  ...
  for (let o = 0; o < mf; o++) {
    await new Promise((l) => setTimeout(l, pf));
...
const wt = 14 * 1024 * 1024, ko = 100 * 1024 * 1024, jo = 100 * 1024 * 1024, _f = 45e3;
function Ei(t) {
...
async function bf(t) {
  ...
  if (Number.isFinite(e) && e > wt)
    throw new Error("The LTX-2.5 first-frame reference is larger than 14 MB.");
  if (!t.body) {
    const o = Buffer.from(await t.arrayBuffer());
    if (o.byteLength > wt) throw new Error("The LTX-2.5 first-frame reference is larger than 14 MB.");
    return o;
  }
...
async function Is(t) {
  ...
  if (t.startsWith("local-media://file")) {
    const e = Ss(t), r = await j.stat(e);
    if (!r.isFile()) throw new Error("The LTX-2.5 first-frame reference is not a file.");
    if (r.size > wt) throw new Error("The LTX-2.5 first-frame reference is larger than 14 MB.");
    const n = await j.readFile(e);
    return Ro(n);
  }
  if (/^https?:\/\//i.test(t)) {
    const e = wf(t), r = new AbortController(), n = setTimeout(() => r.abort(), _f);
    let i;
    try {
...
async function Tf(t) {
  ...
  const r = await Promise.all(e.map(async (n, i) => {
    const o = n.trim().startsWith("data:") ? n : await Is(n.trim());
    return Ef(o, i);
  }));
  return {
    ...t,
...
async function If(t) {
  ...
  if (!i.length || i.byteLength > jo)
    throw new Error("RunPod returned an invalid or oversized image file.");
  const o = Ei(i), a = o === "image/png" ? ".png" : o === "image/jpeg" ? ".jpg" : o === "image/webp" ? ".webp" : void 0;
...
async function Po(t) {
  const e = decodeURIComponent(t.replace("local-media://file", "")), r = await j.readFile(e), n = xf(e), i = new Blob([r], { type: n }), o = new File([i], w.basename(e), { type: n });
  return K.fal.storage.upload(o);
}
async function Pr(t) {
  const e = {};
  for (const [r, n] of Object.entries(t))
    typeof n == "string" && n.startsWith("local-media://file") ? e[r] = await Po(n) : Array.isArray(n) ? e[r] = await Promise.all(
      n.map(async (i) => typeof i == "string" && i.startsWith("local-media://file") ? Po(i) : i && typeof i == "object" && !Array.isArray(i) ? Pr(i) : i)
    ) : n && typeof n == "object" && !Array.isArray(n) ? e[r] = await Pr(n) : e[r] = n;
  return e;
}
...
function Af() {
  I.handle("workflow:run", async (e, r) => {
    const {
      ...
      outputType: c,
      inputs: p
    } = r, { ALL_MODELS: f, resolveVideoModelEndpoint: m, sanitizeVideoInputsForEndpoint: y } = await import("./models-CEFXpnAn.js"), h = f[d] ?? Object.values(f).find(
    ...
    if (!h) {
      if (d.startsWith("fal-ai/")) {
        ...
        Bn(E);
        const A = await Pr(p), q = await Oo(d, A, E);
        return q.data ?? q;
      }
    ...
    const g = h.provider;
    let _ = p;
    g !== "higgsfield" && (n && Bn(n), _ = await Pr(p));
    ...
    if (g === "kie") {
      ...
      T = await ff(b, _, c ?? (E === "video" ? "video" : E === "audio" ? "audio" : E === "text" ? "text" : E === "3d" || E === "model3d" || E === "model" ? "3d" : "image"));
    } else {
      const E = n;
      if (!E) throw new Error("No fal.ai API key provided. Add one in Settings.");
      T = await Oo(b, _, E);
  ...
  I.handle("workflow:poll-job", async (e, r) => {
    ...
    return n;
  }), I.handle("pod:start", async (e, r) => await Ao(r.runpodKey, r.podId, "start")), I.handle("pod:stop", async (e, r) => await Ao(r.runpodKey, r.podId, "stop")), I.handle("pod:status", async (e, r) => await yf(r.runpodKey, r.podId)), I.handle("pod:setup-ltx25", async (e, r) => await zc(r)), I.handle("pod:status-ltx25", async (e, r) => await Wc(r)), I.handle("pod:terminate-ltx25", async (e, r) => await Xc(r)), I.handle("pod:generate-ltx25", async (e, r) => {
    const n = r.input ? await vf(r.input) : void 0;
    return await Sf(await Jc({ ...r, input: n }));
  }), I.handle("pod:generate-session-image", async (e, r) => {
    const n = r.input ? await Tf(r.input) : void 0;
    return await If(await Kc({ ...r, input: n }));
  });
}
const xt = fr(tt), xs = "https://mcp.artlist.io/mcp", At = "artlist", kf = 1200 * 1e3, jf = [
...
async function br() {
  for (const t of jf)
    try {
      const { stdout: e } = await xt(t, ["--version"], {
        env: kt(),
        timeout: 8e3
      });
...
function rr(t) {
  ...
  const e = t, r = ["result", "data", "output"].flatMap((n) => {
    const i = e[n];
    if (i && typeof i == "object") return rr(i);
    if (typeof i == "string")
      try {
        return rr(JSON.parse(i));
      } catch {
        return [];
      }
    return [];
...
function Of(t) {
  ...
  for (const u of n) {
    ...
    for (const p of [d, u])
      if (p)
        try {
          i.unshift(...rr(JSON.parse(p)));
        } catch {
        }
    const c = (s = u.match(/\{[\s\S]*\}/)) == null ? void 0 : s[0];
    if (c)
      try {
        i.unshift(...rr(JSON.parse(c)));
      } catch {
      }
  }
  for (const u of i) {
    const d = Ye(u, ["url", "videoUrl", "video_url", "downloadUrl", "download_url", "mediaUrl", "media_url"]);
    ...
    return {
      ...
      ...Ye(u, ["generationId", "generation_id", "id"]) ? { generationId: Ye(u, ["generationId", "generation_id", "id"]) } : {},
      ...Ye(u, ["accountUrl", "account_url", "sessionUrl", "session_url"]) ? { accountUrl: Ye(u, ["accountUrl", "account_url", "sessionUrl", "session_url"]) } : {},
      ...Ye(u, ["model", "modelId", "model_id"]) ? { model: Ye(u, ["model", "modelId", "model_id"]) } : {}
    };
  }
  const o = (l = n.join(`
...
async function nr(t) {
  try {
    const { stdout: e, stderr: r } = await xt(t, ["mcp", "get", At], {
      env: kt(),
      timeout: 2e4
    }), n = `${e}
...
async function Nf(t) {
  (await nr(t)).configured || await xt(t, [
    "mcp",
    "add",
    "--transport",
    "http",
...
function Cf(t, e = process.platform, r = "/tmp/cinegen-artlist-login.command") {
  if (e === "darwin") {
    const n = [
      "#!/bin/zsh",
...
`);
  ...
  return { file: t, args: ["mcp", "login", At], detached: !1 };
}
function Lf(t) {
...
async function Mf(t, e) {
  const r = w.join(X.getPath("userData"), "artlist-mcp-workspace");
  await et(r, { recursive: !0 });
  const n = Pf(e), { stdout: i } = await xt(t, [
    "-p",
    n,
    "--output-format",
    "json",
...
function Df() {
  I.handle("artlist:account-status", async () => {
    const t = await br();
    return t ? nr(t) : { connected: !1, configured: !1, error: "Claude Code is required for the Artlist MCP connection." };
    ...
    const e = Cf(
      t,
      process.platform,
      w.join(X.getPath("userData"), "artlist-login.command")
    );
    try {
      e.script && (await $l(e.script.path, e.script.contents, { mode: 448 }), await Fl(e.script.path, 448)), await xt(e.file, e.args, {
        env: kt(),
        timeout: e.detached ? 2e4 : 300 * 1e3,
        maxBuffer: 2 * 1024 * 1024
      });
    } catch (r) {
      throw new Error(Lf(r));
    }
    return e.detached ? Uf(t) : nr(t);
  }), I.handle("artlist:auth-logout", async () => {
    const t = await br();
    t && await xt(t, ["mcp", "logout", At], {
      env: kt(),
      timeout: 2e4
    });
    ...
    if (!(await nr(r)).connected) throw new Error("Connect your Artlist account in Settings before generating.");
    if (!((i = e == null ? void 0 : e.prompt) != null && i.trim())) throw new Error("Artlist generation requires a prompt.");
    return Mf(r, e);
  });
}
const As = xa(import.meta.url);
function ks(t) {
  return X.isPackaged ? t.replace("app.asar", "app.asar.unpacked") : t;
}
function Ee() {
  const t = As("ffmpeg-static");
  return ks(t);
}
function Ti() {
  const t = As("ffprobe-static").path;
  return ks(t);
}
function js() {
  if (X.isPackaged)
    return w.join(process.resourcesPath, "vendor", "fpcalc");
  const t = w.dirname(ni(import.meta.url));
  return w.resolve(t, "..", "vendor", "fpcalc", "fpcalc");
}
const Rs = fr(tt), $f = 20 * 1e3, Ff = 300 * 1e3;
function Bf(t) {
...
function Vf(t, e) {
  return Ps(t, `scale=${e.width}:${e.height}:flags=lanczos`);
}
function zf(t, e) {
  const r = `max(1,sqrt(${e}/(iw*ih)))`, n = `ceil(iw*${r}/2)*2`, i = `ceil(ih*${r}/2)*2`;
  return Ps(t, `scale='${n}':'${i}':flags=lanczos`);
}
const Wf = [
  "GPT Image 2",
  "Nano Banana 2",
...
], Kf = ["16:9", "9:16", "1:1", "4:3", "3:4"], Yf = ["1K", "2K", "4K"], Qf = ["16:9", "9:16", "1:1", "4:3", "3:4", "21:9"], Zf = ["720", "1080"], ep = Array.from({ length: 27 }, (t, e) => String(e + 4));
function Ie(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function Hn(t, e = []) {
  return Array.isArray(t) ? t.forEach((r) => Hn(r, e)) : Ie(t) && (e.push(t), Object.values(t).forEach((r) => Hn(r, e))), e;
}
function tp(t) {
  for (const e of Hn(t))
...
function np(t, e) {
  ...
  return (Array.isArray(n) ? n : Ie(n) ? ["values", "options", "enum", "allowedValues"].map((o) => n[o]).find(Array.isArray) ?? [] : []).map(rp).filter((o) => o != null);
}
function Os(t) {
  ...
  for (const r of [])
    if (!(r.outputType !== "image" && r.outputType !== "video" && r.outputType !== "audio"))
      for (const n of tp(r.config)) {
        ...
        for (const s of op(n)) a.accepts.add(s);
        ...
        for (const s of ["aspectRatio", "resolution", "duration", "quality", "sound"]) {
          const l = np(n, s);
          l.length && (a.accepts.add(s), a.options.set(s, Os([...a.options.get(s) ?? [], ...l])));
        }
        e.set(o, a);
      }
  return [...e.values()];
...
function qo(t, e) {
  ...
  return Os(r).map((n) => ({ value: String(n), label: String(n) }));
}
function op(t) {
...
function bt(t, e, r) {
  ...
  return n.length ? qo(n, []) : t.live && !t.accepts.has(e) ? [] : qo([], r);
}
function vt(t, e, r) {
  var i;
...
function ap(t) {
  const e = bt(t, "aspectRatio", Kf), r = bt(t, "resolution", Yf), n = bt(t, "quality", []), i = t.taskTypes.size === 0 || t.taskTypes.has("text_to_image"), o = t.taskTypes.size === 0 || t.taskTypes.has("image_edit"), a = [
  ...
  return o && a.push(
    ...
    fieldType: "select",
    default: vt(e, "16:9", t.defaults.aspectRatio),
    options: e
  }), r.length && a.push({
    id: "resolution",
    portType: "text",
    ...
    falParam: "resolution",
    fieldType: "select",
    default: vt(r, "2K", t.defaults.resolution),
    options: r
  }), n.length && a.push({
    id: "quality",
    portType: "text",
    ...
    falParam: "quality",
    fieldType: "select",
    default: vt(n, "medium", t.defaults.quality),
    options: n
  }), {
    id: `topview/image/${t.submitModel ?? t.displayName}`,
    nodeType: `topview-image-${zr(t.displayName)}`,
    name: t.displayName,
...
function sp(t) {
  const e = bt(t, "aspectRatio", Qf), r = bt(t, "resolution", Zf), n = bt(t, "duration", ep), i = t.options.get("sound") ?? [], o = t.nativeAudio === !0 || i.some((d) => String(d).toLowerCase() === "on"), a = t.taskTypes.size === 0 || t.taskTypes.has("omni_reference"), s = t.taskTypes.size === 0 || t.taskTypes.has("image_to_video"), l = [...t.taskTypes].map((d) => d.replaceAll("_", " ")).join(" · "), u = [
    { id: "prompt", portType: "text", label: "Prompt", required: !0, falParam: "prompt", fieldType: "port" }
  ];
  return a ? u.push(
    ...
    falParam: "duration",
    fieldType: "select",
    default: Number(vt(n, "5", t.defaults.duration)),
    options: n
  }), e.length && u.push({
    id: "aspect_ratio",
    portType: "text",
    ...
    falParam: "resolution",
    fieldType: "select",
    default: vt(r, "720", t.defaults.resolution),
    options: r
  }), o && u.push({
    id: "generate_audio",
    portType: "number",
    ...
    nodeType: `topview-video-${zr(t.displayName)}`,
    name: t.displayName,
    category: "video",
...
function lp(t) {
  ...
  return e === "music" ? r.push(
    ...
    nodeType: `topview-audio-${zr(t.displayName)}`,
    name: t.displayName,
    category: "audio",
...
function up() {
  return [
    ...
    ...Jf.map(({ displayName: t, catalogType: e }) => ({
      displayName: t,
      outputType: "audio",
      catalogType: e,
...
function B_(t) {
  const e = ip(), r = e.length ? e : up();
  return Object.fromEntries(r.map((n) => {
    const i = n.outputType === "image" ? ap(n) : n.outputType === "video" ? sp(n) : lp(n);
    return [i.nodeType, i];
  }));
}
const dp = "https://mcp.topview.ai/mcp", gn = "https://mcp.topview.ai", cp = "https://www.topview.ai/mcp_oauth/oauth/authorize", fp = "https://www.topview.ai/mcp_oauth/oauth/token", pp = "https://www.topview.ai/mcp_oauth/oauth/register", Co = "https://www.topview.ai/mcp_oauth/oauth/userinfo", mp = 300 * 1e3, yn = 1200 * 1e3, hp = 90 * 1e3, gp = 30 * 1e3, jt = 45 * 1024 * 1024, Lo = 50;
function D(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function _n(t) {
...
class _p {
  constructor() {
    this.root = w.join(X.getPath("userData"), "integrations", "topview");
  }
  availabilityError() {
    try {
      return hr.isEncryptionAvailable() ? process.platform === "linux" && hr.getSelectedStorageBackend() === "basic_text" ? "Topview sign-in requires a Linux secret store such as GNOME Keyring or KWallet." : void 0 : "Secure credential storage is unavailable on this device. Configure the operating-system keychain, then restart CineGen.";
    } catch {
  ...
  async read(e) {
    try {
      ...
      const n = hr.decryptString(Buffer.from(r.data, "base64")), i = JSON.parse(n);
      if (!D(i)) throw new Error("Topview credentials are invalid. Connect the account again.");
      return i;
    } catch (r) {
  ...
  async write(e, r) {
    ...
    const n = JSON.stringify({
      version: 1,
      data: hr.encryptString(JSON.stringify(r)).toString("base64")
...
function Gn(t, e, r) {
  const n = D(e) ? e.error_description ?? e.message ?? e.error : e;
...
async function bn(t, e, r) {
  ...
  const i = await Ns(n);
  if (!n.ok) throw Gn(n.status, i, r);
  if (!D(i)) throw new Error(`${r} Topview returned an invalid response.`);
  return i;
}
function wp(t, e) {
  const r = [];
...
`);
  ...
  return r.find((n) => D(n) && n.id === e) ?? r.find((n) => D(n) && (n.result !== void 0 || n.error !== void 0)) ?? r.at(-1);
}
function rt(t, e = [], r = 0) {
  if (r > 14 || t === null || t === void 0) return e;
  if (Array.isArray(t))
    for (const n of t) rt(n, e, r + 1);
  else if (D(t)) {
    e.push(t);
    for (const n of Object.values(t)) rt(n, e, r + 1);
  }
  return e;
}
function Or(t, e = [], r = 0) {
  ...
  else if (Array.isArray(t)) for (const n of t) Or(n, e, r + 1);
  else if (D(t)) for (const n of Object.values(t)) Or(n, e, r + 1);
  return e;
}
function ae(t) {
  const e = [t];
...
function dt(t) {
  ...
  return e && /^https?:\/\//i.test(e) ? e : Or(t).find((r) => /^https?:\/\//i.test(r) && (/\.(?:mp4|mov|webm|png|jpe?g|webp|avif)(?:[?#]|$)/i.test(r) || /cloudfront|cdn|output|result/i.test(r)));
}
function $o(t) {
  const e = te(t, [
    "fileId",
...
function qs(t, e) {
  ...
  return r.includes("array") && Array.isArray(t) ? t.map((n) => qs(n, e.items)) : r.includes("object") && D(t) ? Cs(e, t) : t;
}
function Cs(t, e) {
  if (!D(t)) return { ...e };
  const r = D(t.properties) ? t.properties : {}, i = (D(r.req) ? r.req : void 0) ?? t, o = D(i.properties) ? i.properties : {}, a = i.additionalProperties === !1 && Object.keys(o).length > 0, s = {};
  for (const [l, u] of Object.entries(e))
    a && !Object.hasOwn(o, l) || (s[l] = qs(u, o[l]));
  return s;
}
function vp(t, e) {
  var i;
  const r = D((i = t.inputSchema) == null ? void 0 : i.properties) ? t.inputSchema.properties : {}, n = Cs(t.inputSchema, e);
...
function Ep(t) {
  const r = (Ls(t, /^(?:boards|list|items|records|data|rows)$/i) ?? []).filter(D).map((i) => ({
...
function Tp(t) {
  return (Ls(t, /^models$/i) ?? []).filter(D);
}
function Us(t, e) {
  const r = t.submitParameterOptions;
  if (D(r)) {
    const n = r[e];
    if (Array.isArray(n)) return n.map(vn);
    if (D(n)) {
      for (const i of ["values", "options", "enum", "allowedValues"])
        if (Array.isArray(n[i])) return n[i].map(vn);
    }
  }
  if (Array.isArray(r)) {
    const n = r.find((i) => D(i) && (i.name === e || i.key === e || i.field === e));
    if (D(n)) {
      for (const i of ["values", "options", "enum", "allowedValues"])
        if (Array.isArray(n[i])) return n[i].map(vn);
    }
...
function vn(t) {
  return D(t) ? t.value ?? t.key ?? t.id ?? t.name : t;
}
function Si(t) {
  return D(t.requiredSubmitFields) ? Object.entries(t.requiredSubmitFields).filter(([, e]) => e === !0 || D(e)).map(([e]) => e) : Array.isArray(t.requiredSubmitFields) ? t.requiredSubmitFields.map((e) => {
...
function Sp(t) {
  ...
  return D(e) ? Object.keys(e) : Array.isArray(e) ? e.map((r) => {
    if (!D(r)) return "";
    const n = r.name ?? r.key ?? r.field;
    return typeof n == "string" ? n : "";
  }).filter(Boolean) : [];
...
function yt(t, e) {
  const r = D(t.defaultSubmitParameters) ? t.defaultSubmitParameters : {};
  return Object.hasOwn(r, e) || Si(t).includes(e) || Sp(t).includes(e);
}
function Vn(t) {
  if (t.nativeAudio === !1 || t.supportsNativeAudio === !1) return !1;
  if (t.nativeAudio === !0 || t.supportsNativeAudio === !0) return !0;
  const e = Us(t, "sound");
  if (e.length) return $s(e, "on") !== void 0;
  if ((D(t.defaultSubmitParameters) ? t.defaultSubmitParameters : {}).sound === "on" || yt(t, "sound")) return !0;
}
function Et(t) {
...
function Ms(t, e, r = !1) {
  ...
  if (i && i !== "auto") {
    const s = n.find((l) => Et(l).some((u) => u.toLowerCase() === i.toLowerCase()));
    if (!s)
  ...
  const o = te(t, ["preferredSubmitModel", "preferred_submit_model"]), a = n.find((s) => Et(s).includes(o ?? "")) ?? n.find((s) => s.preferred === !0) ?? n[0];
  if (r && Vn(a) === !1)
    throw new Error(`Topview's default model "${Et(a)[0] ?? "selected"}" does not support native sound. Disable sound or explicitly choose another model.`);
  return a;
}
function Ip(t) {
...
function Fo(t) {
  var a;
  const e = t.references.length ? "image_edit" : "text_to_image", r = Ms(t.config, t.params.model), n = (a = Et(r)[0]) == null ? void 0 : a.trim();
  if (!n) throw new Error("Topview returned an image model without a submit identifier.");
  const i = D(r.defaultSubmitParameters) ? r.defaultSubmitParameters : {}, o = {
    taskType: e,
    model: n,
    prompt: Ds(t.params.prompt),
  ...
  for (const [s, l, u] of [
    ...
    yt(r, s) && (o[s] = zn({ model: r, field: s, requested: l, fallback: u }));
  for (const s of Si(r))
...
const xp = "Keep the frame free of on-screen text, captions, and subtitles.";
function Ds(t) {
...
function Ap(t, e) {
  const r = t.submitParameterOptions;
  let n;
  if (D(r) ? n = r[e] : Array.isArray(r) && (n = r.find((s) => D(s) && (s.name === e || s.key === e || s.field === e))), !D(n)) return {};
...
function zn(t) {
  const e = D(t.model.defaultSubmitParameters) ? t.model.defaultSubmitParameters : {}, r = t.requested !== void 0, n = e[t.field] !== void 0 && e[t.field] !== null, i = t.requested ?? e[t.field] ?? t.fallback;
  if (i === void 0) {
    if (t.required) throw new Error(`Topview model configuration requires "${t.field}", but did not provide a usable default.`);
    return;
  }
  const o = Us(t.model, t.field);
  if (o.length) {
    const s = $s(o, i);
    if (s === void 0) {
      if (!r && !n) return o[0];
      throw new Error(`Topview model "${Et(t.model)[0] ?? "selected"}" does not allow ${t.field}=${String(i)}. Allowed values: ${o.map(String).join(", ")}.`);
    }
    return s;
  }
  const a = Ap(t.model, t.field);
  if (a.min !== void 0 || a.max !== void 0 || a.step !== void 0) {
    ...
    if (!Number.isFinite(s) || a.min !== void 0 && s < a.min || a.max !== void 0 && s > a.max || a.step !== void 0 && a.min !== void 0 && Math.abs((s - a.min) / a.step - Math.round((s - a.min) / a.step)) > 1e-9)
      throw new Error(`Topview model "${Et(t.model)[0] ?? "selected"}" does not allow ${t.field}=${String(i)}.`);
    return s;
  }
  return i;
}
...
function Bo(t) {
  var p, f;
  const e = Ms(t.config, t.params.model, t.params.generateAudio === !0), r = String(e.submitModel ?? "").trim();
  if (!r) throw new Error("Topview returned a video model without a submit identifier.");
  const n = D(e.defaultSubmitParameters) ? e.defaultSubmitParameters : {}, i = new Set(Si(e)), o = t.params.durationSec === void 0 ? void 0 : Math.round(t.params.durationSec);
  if (o !== void 0 && (!Number.isFinite(o) || o === 0 || o < -1))
    throw new Error("Topview video duration must be a positive whole number of seconds.");
  let a = Ds(t.params.prompt);
  const s = {
    ...n,
    taskType: t.taskType,
    model: r,
  ...
  const l = (m, y, h) => {
    ...
    const _ = zn({ model: e, field: m, requested: y, fallback: h, required: i.has(m) });
    _ !== void 0 && (s[m] = _);
  };
  if (l("resolution", t.params.resolution === void 0 ? void 0 : Number.parseInt(t.params.resolution, 10), 720), yt(e, "duration") && l("duration", o, 5), l("generatingCount", void 0, 1), t.taskType !== "image_to_video" ? l("aspectRatio", (p = t.params.aspectRatio) == null ? void 0 : p.trim(), "16:9") : (t.params.aspectRatio !== void 0 || i.has("aspectRatio")) && l("aspectRatio", (f = t.params.aspectRatio) == null ? void 0 : f.trim(), "16:9"), Vn(e) !== !1 && (yt(e, "sound") || t.params.generateAudio === !0))
    s.sound = zn({
      model: e,
      field: "sound",
...
function En(t, e, r) {
  const n = (r ?? "").split(";", 1)[0].trim().toLowerCase(), i = n ? jp[n] : void 0, o = (() => {
    try {
  ...
  if (!s || !Ho(e).has(s)) {
    ...
    throw new Error(`Topview received an unsupported ${l} reference format. Supported formats: ${[...Ho(e)].join(", ")}.`);
  }
  if (n && !i)
...
function Pp(t) {
  const e = t.startsWith("[") && t.endsWith("]") ? t.slice(1, -1) : t, r = ri(e.split("%", 1)[0]);
  if (r === 4) return Go(e);
  if (r !== 6) return !1;
  const n = Rp(e);
  return n ? n.slice(0, 10).every((o) => o === 0) && n[10] === 255 && n[11] === 255 ? Go(n.slice(12).join(".")) : !(n[0] < 32 || n[0] > 63 || n[0] === 32 && n[1] === 1 && (n[2] & 254) === 0 || n[0] === 32 && n[1] === 1 && n[2] === 0 && n[3] === 2 || n[0] === 32 && n[1] === 1 && n[2] === 13 && n[3] === 184 || n[0] === 32 && n[1] === 1 && (n[2] & 240) === 16 || n[0] === 32 && n[1] === 1 && (n[2] & 240) === 32 || n[0] === 32 && n[1] === 2 || n[0] === 63 && (n[1] & 240) === 240) : !1;
}
async function Op(t) {
  ...
  const n = ri(e), i = n ? [{ address: e, family: n }] : await Wl(e, { all: !0, verbatim: !0 });
  if (!i.length || i.some((o) => !Pp(o.address)))
...
async function Hs(t, e = 0) {
  ...
  return new Promise((i, o) => {
    const a = Xl({
      ...
      method: "GET",
      servername: ri(r.hostname.replace(/^\[|\]$/g, "")) ? void 0 : r.hostname,
      headers: {
      ...
      if (l >= 300 && l < 400) {
        ...
        Hs(new URL(p, r).href, e + 1).then(i, o);
        return;
      }
      if (l < 200 || l >= 300) {
...
async function Np(t, e) {
  ...
  if (r.startsWith("data:")) {
    ...
    if (s.length > jt) throw new Error("This reference exceeds CineGen's 45 MB Topview upload safety limit.");
    const l = En("", e, a[1]);
    return { bytes: s, format: l, contentType: ir[l] };
  }
  let n;
  ...
  if (n) {
    ...
    if (a.size > jt) throw new Error("This reference exceeds CineGen's 45 MB Topview upload safety limit.");
    const s = En(n, e);
    return { bytes: await j.readFile(n), format: s, contentType: ir[s], filePath: n };
  }
  const i = await Hs(r), o = En(i.finalUrl, e, i.contentType);
  return { bytes: i.bytes, format: o, contentType: ir[o] };
}
const qp = 407696, Nr = 409600;
function Gs(t) {
  const e = zr(t);
  return e.includes("seedance-2-5") ? qp : e.includes("seedance-2") ? Nr : void 0;
}
function Wn(t) {
  return t.toLocaleString("en-US");
}
function Cp(t) {
  const e = Gs(t.submitModel);
  if (e === void 0) return;
  const r = t.width * t.height;
  if (!(r >= e))
    return `This reference video is ${t.width}x${t.height}, which is ${Wn(r)} pixels per frame. "${t.submitModel}" requires at least ${Wn(e)}. Re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice), then attach it again.`;
}
function Vo(t) {
  var e;
  if (/video\s+pixel\s+count/i.test(t)) {
    const r = (e = /greater\s+than\s+or\s+equal\s+to\s+(\d+)/i.exec(t)) == null ? void 0 : e[1], n = r ? Number.parseInt(r, 10) : Nr;
    return `Seedance rejected a reference video for being too small. It needs at least ${Wn(n)} pixels per frame, so re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice) and attach it again. A 640x360 clip is the usual cause.`;
  }
...
function Lp(t) {
  var r;
  const e = (r = rt(t).find((n) => D(n.headers))) == null ? void 0 : r.headers;
  return D(e) ? Object.fromEntries(Object.entries(e).filter((n) => typeof n[1] == "string")) : {};
}
class Up {
  constructor() {
    this.store = new _p();
  ...
  async tokenExchange(e, r) {
    ...
    return n.set("client_id", r.client_id), r.client_secret && n.set("client_secret", r.client_secret), bn(fp, {
      method: "POST",
  ...
  async teamConnection() {
    ...
    try {
      const n = JSON.parse(await j.readFile(
        w.join(X.getPath("home"), ".topview", "credentials.json"),
        "utf8"
      ));
      if (D(n) && typeof n.api_key == "string" && n.api_key.trim() && typeof n.uid == "string" && n.uid.trim())
        return {
  ...
  async mcpRequest(e, r, n) {
    var a;
    const i = new AbortController(), o = setTimeout(() => i.abort(), hp);
    (a = o.unref) == null || a.call(o);
    try {
      const s = await fetch(dp, {
        method: "POST",
        headers: {
        ...
        signal: i.signal
      }), l = await s.text(), u = (s.headers.get("content-type") || "").includes("text/event-stream") ? wp(l, r.id) : l ? (() => {
        try {
          return JSON.parse(l);
        } catch {
          return l;
      ...
      if (!s.ok) throw Gn(s.status, u, "Topview MCP request failed.");
      if (D(u) && u.error !== void 0) throw Gn(400, u.error, "Topview MCP returned an error.");
      return {
        payload: D(u) ? u : {},
  ...
  async session() {
    ...
    for (let l = 0; l < Lo; l += 1) {
      ...
      i = u.sessionId || i;
      const d = D(u.payload.result) ? u.payload.result : {}, c = Array.isArray(d.tools) ? d.tools.filter((f) => D(f) && typeof f.name == "string") : [];
    ...
    throw new Error(`Topview returned more than ${Lo} MCP tool pages.`);
  }
  async callTool(e, r, n) {
    ...
    const a = D(o.payload) ? o.payload.result : void 0;
    if (D(a) && a.isError === !0) {
      const s = ae(a), u = (te(s, [
        "errorMsg",
        "error_msg",
        ...
        "failure_reason",
        "message"
      ]) ?? Or(a).join(" ").slice(0, 700)).trim() || `Topview could not run ${r}.`, d = Vo(u);
      throw new Error(d ? `${u}
...
${d}` : u);
  ...
  async chooseBoard(e) {
    ...
    const i = await this.callTool(e, "topview_create_board", { name: "CineGen" }), o = te(ae(i), ["boardId", "board_id", "id"]);
  ...
  async uploadReference(e, r, n) {
    ...
    let i = await Np(r.value, r.role), o;
    if (r.role === "video" && n && Gs(n) !== void 0) {
      const f = await Gf(i), m = f && Cp({ submitModel: n, ...f });
      if (f && m) {
        const y = Hf(f, Nr);
        try {
          const h = await Vf(i, y);
          if (h.length > jt)
            throw new Error("The resized reference exceeds CineGen's 45 MB Topview upload safety limit.");
          i = { bytes: h, format: "mp4", contentType: ir.mp4 }, o = `Upscaled reference video ${f.width}x${f.height} → ${y.width}x${y.height}`, console.info(
...
CineGen could not create the temporary resized copy automatically.`);
        ...
        try {
          const y = await zf(i, Nr);
          if (y.length > jt)
            throw new Error("The prepared reference exceeds CineGen's 45 MB Topview upload safety limit.");
          i = { bytes: y, format: "mp4", contentType: ir.mp4 }, o = "Prepared reference video at Seedance-compatible resolution", console.info("[topview] prepared Seedance reference video with the area-based compatibility fallback");
        } catch (y) {
    ...
    const a = await this.callTool(e, "ta_upload_credential", {
      format: i.format,
      needAccelerateUrl: !1
    }), s = ae(a), l = te(s, ["fileId", "file_id"]), u = te(s, ["uploadUrl", "upload_url", "accelerateUrl", "accelerate_url"]);
    ...
    const d = (te(s, ["method", "httpMethod", "http_method"]) || "PUT").toUpperCase(), c = await fetch(u, {
      method: d,
      headers: { ...Lp(s), ...i.contentType ? { "Content-Type": i.contentType } : {} },
      body: i.bytes
    });
    ...
    if (bp(ae(p)) === !1) throw new Error("Topview could not verify an uploaded element reference.");
  ...
  async accountStatus() {
    ...
    try {
      ...
      const n = await this.accessToken(), i = await bn(Co, {
        method: "GET",
      ...
      if (o.tools.some((u) => u.name === "topview_get_credit"))
        try {
          l = Do(ae(await this.callTool(o, "topview_get_credit", {}))) ?? l;
        } catch {
        }
      return {
        connected: !0,
      ...
      return { connected: !1, configured: !0, error: Uo(r, "Topview connection expired.") };
    }
  }
  async modelCatalog() {
    ...
    for (const i of r)
      try {
        const o = ae(await this.callTool(e, "topview_get_generation_config", {
  ...
  async authLogin() {
    ...
    const i = `http://127.0.0.1:${n.port}/oauth/callback`, o = _n(J.randomBytes(48)), a = _n(J.createHash("sha256").update(o).digest()), s = _n(J.randomBytes(32));
    let l, u;
    ...
    }), c = setTimeout(() => u == null ? void 0 : u(new Error("Topview sign-in timed out. Try connecting again.")), mp);
    ...
    try {
      ...
      y.search = new URLSearchParams({
        ...
        resource: gn
      }).toString(), await va.openExternal(y.href);
      ...
      try {
        const S = await fetch(Co, {
          headers: { Accept: "application/json", Authorization: `Bearer ${v.access_token}` }
        }), T = await Ns(S);
        S.ok && D(T) && await this.store.write("profile", T);
      } catch {
      }
      return Mo(l, !0, "You can close this window and return to CineGen."), this.accountStatus();
    } catch (f) {
      throw l && !l.writableEnded && Mo(l, !1, Uo(f, "Topview sign-in did not complete.")), f;
    } finally {
  ...
  async submitWithSession(e, r) {
    ...
    const n = Fs(r.medias), i = kp(r.medias), o = await this.chooseBoard(e), a = ae(await this.callTool(e, "topview_get_generation_config", {
      type: "video",
      taskType: i
    })), s = Bo({
      config: a,
      ...
      params: r,
      boardId: o,
      references: n.map((f, m) => ({ ...f, fileId: `preflight-${m + 1}` }))
    }), l = [];
    for (const f of n)
      l.push(await this.uploadReference(e, f, s.model));
    const u = Bo({ config: a, taskType: i, params: r, references: l, boardId: o }), d = await this.callTool(e, "topview_generate_video", u.req), c = ae(d), p = te(c, ["taskId", "task_id", "generationId", "generation_id"]);
    ...
    return {
      result: {
        ...
        ...l.some((f) => f.preparation) ? {
          referencePreparation: l.flatMap((f) => f.preparation ?? []).join("; ")
        } : {}
      },
      documents: c
    };
  ...
  async queryWithSession(e, r) {
    ...
    const n = await this.callTool(e, "topview_query_task", {
      ...
      needCloudFrontUrl: !0
    }), i = ae(n), o = zt(i), a = dt(i), s = /fail|error|cancel/.test(o), l = !!a || /success|complete|done/.test(o), u = s ? "fail" : l && a ? "success" : l ? "fail" : /^(init|created|queued)$/.test(o) ? "init" : "running", d = te(i, ["boardTaskId", "board_task_id"]), c = te(i, [
      "errorMsg",
      "error_msg",
      "errorMessage",
      "error_message",
      "failureReason",
      "failure_reason"
    ]), p = c ? Vo(c) : void 0, f = u !== "fail" ? void 0 : c ? p ? `${c}
...
${p}` : c : l ? "Topview completed the task without returning a video URL." : "Topview could not complete this video.";
  ...
  async generate(e) {
    const r = await this.session(), n = await this.submitWithSession(r, e), i = dt(n.documents);
    if (i)
      return {
        url: i,
        mediaType: "video",
  ...
  async generateImage(e) {
    ...
    const r = await this.session(), n = Ip(e.medias), i = n.length ? "image_edit" : "text_to_image", o = await this.chooseBoard(r), a = ae(await this.callTool(r, "topview_get_generation_config", {
      type: "image",
      taskType: i
    }));
    Fo({
    ...
    const l = Fo({ config: a, params: e, references: s, boardId: o }), u = await this.callTool(r, "topview_generate_image", l.req), d = ae(u), c = te(d, ["taskId", "task_id", "generationId", "generation_id"]), p = dt(d);
    if (p)
      return {
        url: p,
        mediaType: "image",
        taskId: c,
        model: l.model,
        referenceValue: $o(d) ?? await this.reusableGeneratedImageReference(r, p),
    ...
    for (; Date.now() < m; ) {
      ...
      if (g) {
        ...
        return {
          ...
          taskId: c,
          model: l.model,
          referenceValue: $o(h) ?? await this.reusableGeneratedImageReference(r, g),
  ...
  async generateAudio(e) {
    ...
    let l = ae(await this.callTool(r, o, s));
    const u = te(l, ["taskId", "task_id", "generationId", "generation_id"]), d = dt(l);
    ...
    const c = Date.now() + yn;
    for (; Date.now() < c; ) {
      await new Promise((m) => setTimeout(m, 3e3)), l = ae(await this.callTool(r, "topview_query_task", {
        taskType: a,
        taskId: u,
        needCloudFrontUrl: !0
      }));
...
let zo;
function Vs() {
  return zo ?? (zo = new Up()), zo;
}
function Mp() {
  return Vs().teamConnection();
}
function Dp() {
  const t = Vs();
...
const Wo = {
  ...
  high: { crf: 16, scale: 1 }
}, se = /* @__PURE__ */ new Map(), or = /* @__PURE__ */ new Map();
function $p(t, e) {
...
async function Bp(t, e) {
  const r = se.get(t);
  if (!r) return;
  const n = Ee(), i = Wo[e.preset || "standard"] || Wo.standard, o = e.fps || 30, a = e.outputPath || w.join(process.cwd(), `export_${t}.mp4`);
  ...
  return u.push(
    ...
    (h = m.stderr) == null || h.on("data", (g) => {
      ...
      if (b) {
        const v = Fp(b, e.totalDuration);
        if (v !== null) {
          const S = se.get(t);
          S && (se.set(t, { ...S, progress: v }), $p(t, v));
        }
      }
...
function Hp() {
  I.handle("export:start", async (t, e) => {
    ...
    return se.set(i.id, i), Bp(i.id, e).catch((o) => {
...
function Tn(t) {
  const e = w.extname(t).toLowerCase();
  return Gp[e] ?? "application/octet-stream";
}
function Xo(t) {
  try {
    const e = new URL(t);
...
async function zp(t) {
  ...
  return await new Promise((i, o) => {
    var l;
    const a = oe(r, n, { stdio: ["ignore", "ignore", "pipe"] });
    let s = "";
...
function Wp() {
  I.handle(
    ...
    async (t, e, r) => {
      if (!r) {
        const a = w.join(X.getPath("userData"), "media", "elements");
        await j.mkdir(a, { recursive: !0 });
        const s = w.join(a, Vp(e.name));
    ...
    async (t, e, r) => {
      ...
      const i = await zp(n);
      try {
        const o = await j.readFile(i), s = `${w.basename(n, w.extname(n))}.m4a`, l = Tn(i), u = new Blob([o], { type: l }), d = new File([u], s, { type: l });
    ...
    async (t, e, r) => {
      ...
      const n = Xo(e);
      if (n) {
        const i = await j.readFile(n), o = w.basename(n), a = Tn(n), s = new Blob([i], { type: a }), l = new File([s], o, { type: a });
      ...
      if (e.startsWith("http://") || e.startsWith("https://")) {
        ...
        const s = await j.readFile(a), l = w.basename(a), u = Tn(a), d = new Blob([s], { type: u }), c = new File([d], l, { type: u }), p = await K.fal.storage.upload(c);
...
class Kp {
  constructor(e) {
    zs(e);
    const r = w.join(nt(e), "project.db");
   ...
   */
  initSchema() {
    this.db.exec(Xp), this.db.exec(Jp);
  }
  /**
...
function Xs(t) {
  ...
  if (!r) return null;
  const n = e.type === "character" || e.type === "location" || e.type === "prop" || e.type === "vehicle" ? e.type : "character", i = typeof e.folderId == "string" && e.folderId ? e.folderId : typeof e.folder_id == "string" && e.folder_id ? e.folder_id : void 0, o = Qp(e.variations), a = typeof e.activeVariationId == "string" && o.some((l) => l.id === e.activeVariationId) ? e.activeVariationId : (s = o[0]) == null ? void 0 : s.id;
  return {
    id: r,
...
function Ks(t) {
  if (!t || typeof t != "object") return Ws();
  const e = t, r = Array.isArray(e.folders) ? e.folders.map(Zp).filter((o) => o !== null) : [], n = new Set(r.map((o) => o.id)), i = Array.isArray(e.elements) ? e.elements.map(Xs).filter((o) => o !== null) : [];
  return {
    version: 1,
    folders: r,
...
function em(t, e) {
  const r = Ws(), n = new Map(r.elements.map((o) => [o.id, o])), i = [...r.folders];
  for (const o of e) {
    const a = o.elements.map(Xs).filter((l) => l !== null);
...
function Ys() {
  return w.join(ot(), "elements-library.json");
}
function rm() {
  return w.join(ot(), "projects.json");
}
function Jo(t) {
  return w.join(ot(), t);
}
async function Xn(t) {
  await j.mkdir(ot(), { recursive: !0 });
  const e = Ys(), r = `${e}.tmp`;
  await j.writeFile(r, JSON.stringify(t, null, 2), "utf-8"), await j.rename(r, e);
}
async function nm() {
  try {
    const t = await j.readFile(Ys(), "utf-8");
    return Ks(JSON.parse(t));
  } catch {
    return null;
  }
}
function im(t) {
  const e = w.join(Jo(t.id), "project.db"), r = w.join(Jo(t.id), "project.json");
...
async function om() {
  ...
  let e = [];
  try {
    const i = JSON.parse(await j.readFile(rm(), "utf-8"));
    e = Array.isArray(i.projects) ? i.projects : [];
  } catch {
    e = [];
  }
  const r = e.map(im), n = em(null, r);
  return await Xn(n), n;
}
function am() {
  I.handle(
    ...
    async (t, e) => {
      ...
      if (e != null && e.projectId && e.projectName) {
        const n = tm(r, e.projectId, e.projectName);
        n !== r && (await Xn(n), r = n);
      }
      return r;
    }
...
function lm(t, e, r) {
  const n = sm(r.persona), i = r.persona ? Qs[r.persona] : void 0, o = [];
  let a = 0;
  if (e.length === 0)
  ...
  return t.timelinePlacements.some((s) => s.timelineId === r.activeTimelineId) && r.activeTimelineId && (a += n.activeTimeline, o.push("already on the active timeline")), t.words.length > 0 && (a += n.wordTiming), t.emotion && (a += n.hasEmotion), t.delivery && (a += n.hasDelivery, o.push("has vocal delivery notes")), i && (Yo(t.energy, i.preferredEnergy) && (a += n.energyMatch, o.push(`${t.energy} energy fits ${r.persona}`)), Yo(t.pace, i.preferredPace) && (a += n.paceMatch, o.push(`${t.pace} pace fits ${r.persona}`)), t.emotion && i.emotionBias.some((s) => t.emotion.toLowerCase().includes(s)) && (a += n.emotionBias, o.push(`${t.emotion} emotion favored by ${r.persona}`))), t.emotion && r.queryEmotions.some((s) => t.emotion.toLowerCase().includes(s) || s.includes(t.emotion.toLowerCase())) && (a += n.emotionQueryMatch, o.push(`emotion (${t.emotion}) matches the query`)), t.notable && t.notable.length > 0 && (a += n.notableSignal * t.notable.length, o.push(`notable: ${t.notable.slice(0, 2).join("; ")}`)), { score: a, reasons: o };
}
function um(t) {
...
function mm(t) {
  ...
  return pm.filter((r) => e.includes(r));
}
function hm(t, e, r = 24) {
  const n = typeof r == "number" ? { limit: r } : r, i = n.limit ?? 24, o = fm(e), a = {
    activeTimelineId: t.activeTimelineId,
    persona: n.persona,
    queryEmotions: mm(e)
  };
  return t.moments.map((s) => ({ moment: s, ...lm(s, o, a) })).filter((s) => s.score > 0).sort((s, l) => l.score - s.score || s.moment.sourceStart - l.moment.sourceStart).slice(0, i).map(({ moment: s, score: l, reasons: u }) => ({
    id: s.id,
    assetId: s.assetId,
...
const Ue = "google/gemini-2.5-flash";
function ct(t, e, r) {
...
async function Wr(t, e) {
  ...
  const r = ym(e);
  if (!r) return null;
  const n = await j.readFile(r), i = gm(r), o = new Blob([n], { type: i }), a = new File([o], w.basename(r), { type: i });
...
function Qo(t, e) {
  const n = (Array.isArray(t.objects) ? t.objects : Array.isArray(t.detections) ? t.detections : Array.isArray(t.items) ? t.items : Array.isArray(t.regions) ? t.regions : Array.isArray(t.subjects) ? t.subjects : typeof t.label == "string" || typeof t.name == "string" || typeof t.object == "string" ? [t] : []).map((o) => {
    ...
    const h = ct(c, 0.02, 1), g = ct(p, 0.02, 1), _ = [
      ct(u, h / 2, 1 - h / 2),
      ct(d, g / 2, 1 - g / 2),
      h,
      g
    ], b = Y(a.score ?? a.confidence ?? a.probability), v = b !== null ? ct(b, 0, 1) : 0.75, S = Y(a.priority ?? a.salience ?? a.importance), T = S !== null ? ct(S, 0, 1) : v;
    return {
      label: l,
      box: _,
      score: v,
...
async function Zo(t, e, r, n, i) {
  ...
  const a = (await K.fal.subscribe("fal-ai/any-llm/vision", {
    ...
    logs: !0
  })).data, s = xr(a.output) ?? xr(a.text) ?? xr(a);
...
async function el(t) {
  ...
  const e = (await Promise.all(
    t.framePaths.slice(0, 6).map((c) => Wr(t.apiKey, c).catch(() => null))
  )).filter((c) => !!c);
  if (e.length === 0)
    return {
      assetId: t.assetId,
      status: "missing",
      model: ((a = t.model) == null ? void 0 : a.trim()) || Ue,
  ...
  const n = (await K.fal.subscribe("fal-ai/any-llm/vision", {
    input: {
      model: ((s = t.model) == null ? void 0 : s.trim()) || Ue,
      prompt: [
...
`),
    ...
    logs: !0
  })).data, i = he(n.output) || he(n.text) || "", o = Zs(i);
  if (!o)
    return {
      assetId: t.assetId,
      status: "failed",
      model: ((l = t.model) == null ? void 0 : l.trim()) || Ue,
  ...
  try {
    ...
    return {
      ...
      model: ((u = t.model) == null ? void 0 : u.trim()) || Ue,
    ...
    return {
      assetId: t.assetId,
      status: "failed",
      model: ((d = t.model) == null ? void 0 : d.trim()) || Ue,
...
async function tl(t) {
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await _m(t.apiKey, t.videoPath).catch(() => null);
  if (!e)
...
async function wm(t) {
  var o;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await Wr(t.apiKey, t.imagePath).catch(() => null);
  if (!e)
    throw new Error("Could not upload the image file for analysis.");
  K.fal.config({ credentials: t.apiKey });
  const n = (await K.fal.subscribe("fal-ai/any-llm/vision", {
    input: {
      model: ((o = t.model) == null ? void 0 : o.trim()) || Ue,
...
async function bm(t) {
  var a, s;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = Math.min(12, Math.max(1, Math.round(t.maxObjects ?? 6))), r = await Wr(t.apiKey, t.imagePath).catch(() => null);
  if (!r)
    return {
      status: "missing",
      model: ((a = t.model) == null ? void 0 : a.trim()) || Ue,
  ...
  const n = ((s = t.model) == null ? void 0 : s.trim()) || Ue, i = [
...
`);
  try {
    const l = await Zo(t.apiKey, r, n, e, i), u = l ? Qo(l, e) : [];
    if (u.length > 0)
    ...
    const d = await Zo(t.apiKey, r, n, e, o), c = d ? Qo(d, e) : [];
...
function vm() {
  I.handle("vision:index-asset", async (t, e) => el(e)), I.handle("vision:detect-objects", async (t, e) => bm(e));
}
const Em = "anthropic/claude-sonnet-4.6";
function ie(t) {
  const e = Number(t);
...
function Tm(t) {
  if (!t || typeof t != "object") return;
  const e = t, r = ie(e.prompt_tokens) ?? 0, n = ie(e.completion_tokens) ?? 0, i = ie(e.total_tokens) ?? r + n, o = ie(e.cost) ?? 0;
  if (!(r <= 0 && n <= 0 && i <= 0 && o <= 0))
...
async function Rt(t) {
  ...
  K.fal.config({ credentials: t.apiKey });
  const e = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || Em,
    prompt: t.prompt,
...
function jm(t, e) {
  ...
  return {
    brief: {
      ...
      targetDurationSeconds: Math.max(5, ie(r.targetDurationSeconds) ?? e.targetDurationSeconds),
      variantCount: xm(r.variantCount, e.variantCount),
      persona: Im(r.persona),
      ...
      confidence: Math.min(1, Math.max(0, ie(r.confidence) ?? e.confidence)),
      rationale: typeof r.rationale == "string" && r.rationale.trim() ? r.rationale.trim() : e.rationale
    },
    clarifyingQuestions: km(r.clarifyingQuestions)
  };
}
function Rm(t, e, r) {
  const n = { ...t, ...e ?? {} };
...
function Pm(t) {
  if (!t || typeof t != "object") return null;
  const e = t, r = ea(e.source_start), n = ea(e.source_end);
...
function Om(t, e) {
  if (!t || typeof t != "object") return null;
  const r = t, n = Array.isArray(r.segments) ? r.segments.map(Pm).filter((i) => !!i) : [];
...
function Nm(t) {
  ...
  return Array.isArray(e.variants) ? e.variants.flatMap((r, n) => {
    var a;
    if (!r || typeof r != "object") return [];
    const i = r, o = Array.isArray(i.proposals) ? i.proposals.map((s) => Om(s, `AI Cut ${n + 1}`)).filter((s) => !!s) : [];
    return o.length === 0 ? [] : [{
...
function qm(t, e) {
  ...
  for (const s of n) {
    ...
    u && i.set(u, {
      overall: ie(l.overall) ?? 78,
      storyArc: ie(l.storyArc) ?? 78,
      pacing: ie(l.pacing) ?? 78,
      clarity: ie(l.clarity) ?? 78,
      visualFit: ie(l.visualFit) ?? 78,
      completeness: ie(l.completeness) ?? 78,
      formatFit: ie(l.formatFit) ?? 78,
...
async function Mm(t) {
  var o;
  const e = Am(t.request, t.index), r = [
...
async function ta(t, e, r, n, i = {}) {
  ...
  let a = hm(t, o, { limit: 20, persona: r.persona });
  if (i.rerank && i.apiKey && a.length > 1)
    try {
      const l = um({ query: o, brief: r, candidates: a }), u = await Rt({
        apiKey: i.apiKey,
        model: i.model,
...
async function Dm(t) {
  ...
  const e = (d, c) => {
    ...
    if (!p) return null;
    try {
      const f = JSON.parse(p), y = Nm({ variants: [f] })[0];
      return y ? {
        variant: y,
        usage: c
      } : null;
...
${JSON.stringify(l.map((h) => ({ title: h.title, strategy: h.strategy, summary: h.summary })), null, 2)}` : "",
      ...
      "",
      "Retrieved moments:",
      rl(t.retrievalSummary.topMoments),
      "",
      "Reference timelines:",
...
`),
    ...
    s = sr(s, y.usage), y.variant && l.push({
      ...y.variant,
      id: `variant_${d + 1}`
    });
  }
...
async function $m(t) {
  ...
  const e = [
    ...
    rl(t.retrievalSummary.topMoments.slice(0, 10)),
    "",
    "Variants:",
...
async function Fm(t) {
  ...
  n = sr(n, i.usage);
  const o = Rm(i.brief, t.briefOverride, t.questionAnswers), a = await ta(e, r, o, []);
  if (!t.confirmedBrief)
    return {
      stage: "brief",
  ...
  if (n = sr(n, u.usage), u.variants.length === 0)
    return {
      stage: "brief",
...
`)) >= 0; ) {
      ...
      if (m = m.slice(b + 1), !!v)
        try {
          ...
          S.done && (l = ie(S.prompt_eval_count) ?? 0, u = ie(S.eval_count) ?? 0);
        } catch {
        }
    }
  }
...
async function Gm() {
  try {
    const t = await fetch(`${nl}/api/tags`);
...
function Vm() {
  I.handle("llm:chat", async (t, e) => {
    ...
    const n = Array.isArray(e.messages) ? e.messages : [], i = Sm(n);
    ...
    const r = e.requestId || crypto.randomUUID(), n = await Hm(r, e);
    return {
      message: n.message,
      ...n.usage ? { usage: n.usage } : {}
    };
  }), I.handle("llm:local-models", async () => Gm()), I.handle("llm:run-cut-workflow", async (t, e) => Fm(e));
}
const il = fr(tt), zm = [
  w.join(z.homedir(), ".local/bin/claude"),
  "/opt/homebrew/bin/claude",
...
async function ol() {
  ...
  for (const t of zm)
    try {
      const { stdout: e } = await il(t, ["--version"], {
        env: xi(),
        timeout: 8e3
      });
...
function th() {
  return w.join(X.getPath("userData"), "claude-code-workspace");
}
function rh(t) {
...
async function ih(t, e) {
  ...
  const n = ((h = e.model) == null ? void 0 : h.trim()) || "sonnet", i = !!e.resumeSessionId && !e.injectProjectContext, o = rh(e), a = [
    "-p",
    i ? e.userMessage.trim() : nh(e, o),
    "--output-format",
    "stream-json",
    "--verbose",
    "--max-turns",
  ...
  if (o ? (a.push("--safe-mode", "--effort", "low", "--include-partial-messages"), !i && ((g = e.systemPrompt) != null && g.trim()) && a.push("--system-prompt", e.systemPrompt.trim())) : a.push("--include-partial-messages"), i && e.resumeSessionId) {
    if (a.push("--resume", e.resumeSessionId), !o) {
      const v = [(_ = e.systemPrompt) == null ? void 0 : _.trim(), Xm].filter(Boolean).join(`
...
` : "", S = e.purpose === "enhance-prompt" ? Jm : Wm;
    a.push("--append-system-prompt", `${v}${e.systemPrompt.trim()}

${S}`);
  }
  const s = Qm(), l = o ? th() : void 0;
  l && await et(l, { recursive: !0 });
  ...
  return new Promise((v, S) => {
    ...
    Qe = { child: T, requestId: t };
    let x = "", E = !1;
    const A = (M) => {
      E || (E = !0, Qe = null, s == null || s.webContents.send("llm:claude-code-stream", { requestId: t, done: !0 }), M(), T.killed || T.kill());
    };
...
`)) >= 0; ) {
        ...
        if (x = x.slice(F + 1), !!G)
          try {
            ...
            if (V.type === "system" && V.subtype === "init" && typeof V.session_id == "string" && (c = V.session_id), V.type === "assistant" && V.error === "authentication_failed" && (p = !0), V.type === "result") {
              y = V;
              const H = na(V);
            ...
            V.type === "assistant" && !f ? (u = O, s == null || s.webContents.send("llm:claude-code-stream", { requestId: t, token: O })) : V.type === "result" && !u.trim() && (u = O, s == null || s.webContents.send("llm:claude-code-stream", { requestId: t, token: O }));
          } catch {
          }
      }
...
function oh() {
  I.handle("llm:claude-code-detect", async () => {
    ...
    const r = e.requestId || J.randomUUID(), n = await ih(r, e);
    return {
      message: n.message,
    ...
    (Qe == null ? void 0 : Qe.requestId) === e && (Qe.child.kill("SIGTERM"), Qe = null);
  });
}
const al = fr(tt), ah = {
  "claude-code": [
...
async function Jr(t) {
  if (Er.has(t))
    return Er.get(t) ?? null;
  for (const e of ah[t])
    try {
      const { stdout: r } = await al(e, ["--version"], {
        env: Xr(),
        timeout: 8e3
      });
      if (r.trim())
...
function uh() {
  I.handle("llm:cli-detect", async () => ({ providers: await sh() }));
}
function dh(t, e) {
...
let Re = null;
function ch() {
  return w.join(X.getPath("userData"), "codex-workspace");
}
function fh(t) {
  return t.purpose === "json-job" ? !0 : t.purpose === "copilot" || t.purpose === "enhance-prompt" ? !1 : !t.injectProjectContext && !t.resumeSessionId && !(t.messages && t.messages.length > 0);
}
...
` : "", l = t.purpose === "enhance-prompt" ? dl : ul;
      r.push(`${s}${t.systemPrompt.trim()}

${l}`);
    } else
      r.push(t.systemPrompt.trim());
  const n = (t.messages ?? []).filter((s) => s.content.trim()), i = n.length > 0 ? Kn(n) : `${t.userMessage.trim()}
...
async function gh(t, e) {
  var y;
  const r = await Jr("codex");
  if (!r)
    throw new Error("Codex CLI is not installed. Install it from https://developers.openai.com/codex");
  if (!e.userMessage.trim())
    throw new Error("No chat message provided.");
  const n = ((y = e.model) == null ? void 0 : y.trim()) || "gpt-5.3-codex", i = !!e.resumeSessionId && !e.injectProjectContext, o = fh(e), a = i ? e.userMessage.trim() : ph(e, o), s = o ? ch() : void 0;
  s && await et(s, { recursive: !0 });
...
`)) >= 0; ) {
        ...
        if (b = b.slice(A + 1), !!q)
          try {
            ...
            const F = hh(N);
            if (F) {
              const G = F.startsWith(m) ? F.slice(m.length) : F;
              m = F, d = F, G && (u == null || u.webContents.send("llm:codex-stream", { requestId: t, token: G }));
            }
          } catch {
          }
      }
...
function yh() {
  I.handle("llm:codex-chat", async (t, e) => {
    const r = e.requestId || J.randomUUID(), n = await gh(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
...
function wh(t) {
  ...
  const r = e, n = r.prompt_tokens_details && typeof r.prompt_tokens_details == "object" && !Array.isArray(r.prompt_tokens_details) ? r.prompt_tokens_details : {}, i = Wt(r.prompt_tokens ?? r.input_tokens), o = Wt(r.completion_tokens ?? r.output_tokens), a = Wt(n.cached_tokens), s = Wt(n.cache_write_tokens), l = Wt(r.total_tokens) || i + o;
  if (!(i <= 0 && o <= 0 && l <= 0))
    return { promptTokens: i, completionTokens: o, totalTokens: l, cachedTokens: a, cacheWriteTokens: s };
}
function bh(t) {
  const e = t.promptTokens > _h ? ia.long : ia.short, r = Math.min(t.cachedTokens, t.promptTokens), n = Math.min(t.cacheWriteTokens, Math.max(0, t.promptTokens - r)), o = (Math.max(0, t.promptTokens - r - n) * e.input + r * e.cached + n * e.cacheWrite + t.completionTokens * e.output) / 1e6;
  return { ...t, cost: Math.round(o * 1e8) / 1e8 };
}
const vh = "https://api.openai.com/v1/chat/completions", Eh = "gpt-5.6-luna", Th = 6e4;
function Sh(t, e = []) {
...
function Ih(t) {
  ...
  r && e.push({ role: "system", content: r }), e.push({ role: "user", content: Sh(t.userMessage, t.imageUrls) });
  const n = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || Eh,
    messages: e,
    reasoning_effort: t.reasoningEffort ?? "low",
    max_completion_tokens: Number.isFinite(t.maxCompletionTokens) ? Math.max(1, Math.floor(t.maxCompletionTokens)) : Th
  };
...
async function kh(t) {
  ...
  if (!i.ok)
    throw new Error(xh(a, `OpenAI request failed (${i.status}).`));
  const s = wh(a);
  return {
    message: Ah(a),
    ...s ? { usage: bh(s) } : {}
...
const Rh = "http://localhost:3000", cl = "https://cinegen-cloud-studio.cogden.chatgpt.site", Ai = "persist:cinegen-team-workspace", Ph = 8e3, Yn = "__CINEGEN_TEAM_PROVIDER__", $e = { origin: cl, source: "hosted" }, xn = { origin: Rh, source: "local-web" };
let Pe = null, ft = null, Xt = null;
function qr() {
  return {
    supported: !0,
    scope: "workspace",
...
async function Kr(t, e, r, n) {
  const i = new AbortController(), o = setTimeout(() => i.abort(), Ph);
  try {
    const a = `${t.origin}/api/rpc/${encodeURIComponent(e)}/${encodeURIComponent(r)}`, s = {
      ...
      redirect: "error",
      signal: i.signal
    }, l = t.source === "hosted" ? await Ea.fromPartition(Ai).fetch(a, s) : await fetch(a, s);
    return { response: l, payload: await Oh(l) };
  } finally {
    clearTimeout(o);
  }
}
...
async function lr(t) {
  try {
    const { response: e, payload: r } = await Kr(t, "providers", "status", []);
    return !e.ok || !(r != null && r.ok) || !r.result ? null : fl(r.result, t);
  } catch {
    return null;
  }
}
async function Yr() {
  ...
  const t = await lr($e);
  if (t)
    return Pe = $e, { target: $e, status: t };
  const e = await lr(xn);
  return e ? (Pe = xn, { target: xn, status: e }) : null;
}
async function Nh(t, e, r) {
  ...
  const { response: i, payload: o } = await Kr(n.target, t, e, r);
  if (!i.ok || !(o != null && o.ok))
...
async function oa(t, e) {
  ...
  if (!r)
    throw new Error("Connect CineGen Desktop to the hosted team workspace in Settings first.");
  const { response: n, payload: i } = await Kr(r.target, "providers", t, [e]);
...
async function qh() {
  ...
  if (!e || e.target.source !== "hosted") {
    const a = await pl();
    (i = a.desktop) != null && i.connected && a.desktop.source === "hosted" && (e = { target: $e, status: a });
  }
  if (!e || e.target.source !== "hosted")
  ...
  const { response: r, payload: n } = await Kr(e.target, "topview", "importTeamConnection", [t]);
...
async function Ch(t) {
  return Nh("llm", "openaiChat", [{ ...t, apiKey: Yn }]);
}
async function pl() {
  const t = await lr($e);
  if (t)
    return Pe = $e, t;
  if (Xt)
    return ft && !ft.isDestroyed() && ft.focus(), Xt;
  const e = new Q({
    width: 720,
    height: 820,
    minWidth: 520,
  ...
  ft = e;
  const r = `${cl}/signin-with-chatgpt?return_to=${encodeURIComponent("/")}`, n = new Promise((i, o) => {
    ...
    const d = () => {
      ...
      if (!(s || a)) {
        ...
        try {
          const m = await lr($e);
          if (a || !m) return;
          Pe = $e, c(m), e.isDestroyed() || e.close();
        } finally {
          s = !1;
        }
      }
...
function Lh() {
  I.handle("team-providers:status", async () => {
    const t = await Yr();
    return (t == null ? void 0 : t.status) ?? qr();
  }), I.handle("team-providers:connect", () => pl()), I.handle("team-providers:disconnect", async () => (await Ea.fromPartition(Ai).clearStorageData({ storages: ["cookies"] }), Pe = null, qr())), I.handle("team-providers:save", async (t, e) => oa("save", e)), I.handle("team-providers:remove", async (t, e) => oa("remove", e)), I.handle("team-providers:share-topview", () => qh());
}
function Uh(t) {
  const e = w.extname(t).toLowerCase();
...
function Mh(t) {
  ...
  const r = jh(e) ?? (e.startsWith("/") || /^[A-Za-z]:[\\/]/.test(e) ? e : null);
  if (!r || !B.existsSync(r)) return null;
  const n = B.readFileSync(r);
  return `data:${Uh(r)};base64,${n.toString("base64")}`;
}
function Dh(t) {
  return Array.isArray(t) ? t.flatMap((e) => {
...
function $h() {
  I.handle("llm:openai-chat", async (t, e) => {
    const r = e && typeof e == "object" && !Array.isArray(e) ? e : {}, n = typeof r.apiKey == "string" ? r.apiKey : "", i = typeof r.userMessage == "string" ? r.userMessage : "", o = Dh(r.imageUrls);
    return n === Yn ? Ch({
      ...r,
      apiKey: Yn,
      userMessage: i,
...
async function hl(t, e, r, n) {
  const i = Ee(), o = Math.max(0, e), a = Math.max(0.1, Math.min(r, Fh));
  try {
    return await ml(i, [
      "-y",
      "-ss",
...
function An(t, e, r) {
  ...
  const n = w.extname(t) || (e.mediaType === "image" ? ".jpg" : ".mp4"), i = w.join(r, `${Qn(e)}-source${n}`), o = yl(t, i);
...
async function ki(t, e) {
  ...
  for (const i of t) {
    ...
    if (i.trimStartSec !== void 0 && i.trimDurationSec !== void 0) {
      const d = w.join(r, `${Qn(i)}.mp4`), c = await hl(
        o,
        i.trimStartSec,
        i.trimDurationSec,
        d
    ...
    const s = (i.framePaths ?? []).map((d) => Tt(d)).find(Boolean);
    if (s) {
      const d = An(s, {
        ...i,
        mediaType: "image",
    ...
    const l = w.join(r, `${Qn(i)}.jpg`), u = await Ar(o, i.trimStartSec ?? 0, l);
    u && n.push({
      label: i.label,
      kind: i.kind,
      mediaType: "image",
...
function wl(t) {
  ...
  if (e.startsWith("file://"))
    try {
      return Tt(decodeURIComponent(new URL(e).pathname));
    } catch {
      return null;
    }
  return Tt(e);
...
async function Bh(t, e) {
  ...
  if (e.mode === "first-last") {
    const d = w.join(n, `${i}-first.jpg`), c = w.join(n, `${i}-last.jpg`), p = await Ar(r, o, d), f = await Ar(r, Math.max(o, a - 0.05), c), m = [], y = [];
  ...
  if (e.mode === "segment") {
    const d = w.join(n, `${i}-segment.mp4`), c = Math.max(0.1, a > o ? a - o : e.maxSegmentSec ?? 30), p = await hl(r, o, Math.min(c, e.maxSegmentSec ?? 30), d);
    if (!p) throw new Error("Failed to extract clip segment");
    return { paths: [p], roles: ["image"] };
  }
  const s = e.frameTimeSec ?? (a > o ? (o + a) / 2 : o), l = w.join(n, `${i}-frame.jpg`), u = await Ar(r, s, l);
...
const Gh = 18e4, Vh = 600 * 1e3;
async function bl(t) {
  var d;
  const e = await Jr("gemini");
  if (!e)
    throw new Cr("Gemini CLI is not installed.");
  const r = Tt(t.mediaPath);
  if (!r)
  ...
  let i = r, o = !1;
  if (gl(r)) {
    const c = w.extname(r) || ".mp4", p = w.join(n, `${J.randomUUID()}${c}`), f = yl(r, p);
    if (!f)
  ...
  return new Promise((c, p) => {
    var T, x;
    const f = oe(e, l, { env: sl(), cwd: n, stdio: ["ignore", "pipe", "pipe"] });
    let m = "", y = "", h = "", g = !1, _ = !1;
    const b = (E) => {
...
`)) >= 0; ) {
      ...
      const A = m.trim();
      if (!A) {
        const q = Jn(y.trim()) || `Gemini CLI exited with code ${E ?? "unknown"}`;
        b(() => p(new Error(q)));
        return;
      }
      if (Hh(A)) {
        b(() => p(new Cr("Gemini CLI declined to analyze the media.")));
        return;
      }
      b(() => c(A));
    });
...
const aa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  GeminiMediaUnavailableError: Cr,
  analyzeMediaWithGeminiCli: bl,
  buildGeminiUserMessageWithVisualRefs: _l,
  cleanupEphemeralVisualRefs: ji,
  prepareClipReference: Bh,
  prepareCopilotVisualRefs: ki,
  resolveLocalSourcePath: wl
}, Symbol.toStringTag, { value: "Module" }));
let ye = null;
const zh = 9e4, Wh = 18e4, Xh = 8e3;
function vl() {
  return w.join(X.getPath("userData"), "gemini-cli-workspace");
}
function Jh() {
...
${t.purpose === "enhance-prompt" ? dl : ul}`);
    } else
...
async function sa(t, e, r) {
  var h;
  const n = await Jr("gemini");
  if (!n)
    throw new Error("Gemini CLI is not installed. Install it with: npm install -g @google/gemini-cli");
  const i = ((h = e.model) == null ? void 0 : h.trim().replace(/^[^/]+\//, "")) || "gemini-2.5-flash", o = r.canResume ? Yh(e) : Kh(e), a = o.length > Xh, s = vl();
  ...
  const u = ll();
  let d = "", c = "", p, f;
  const m = 900 * 1e3, y = r.hasVisualRefs ? Wh : zh;
  return new Promise((g, _) => {
    ...
    let v = "", S = !1, T = !1;
    const x = (G) => {
      S || (S = !0, clearTimeout(E), clearTimeout(A), ji(r.preparedVisualRefs), G());
    }, E = setTimeout(() => {
    ...
    (M = b.stdout) == null || M.on("data", (G) => {
      v += G.toString();
      let V;
      for (; (V = v.indexOf(`
`)) >= 0; ) {
        const R = v.slice(0, V).trim();
        if (v = v.slice(V + 1), !!R)
          try {
            ...
            if (O.type === "error" && typeof O.message == "string") {
              const k = O.message;
              c += k, !d.trim() && eg(k) && (ye = null, b.kill("SIGTERM"), x(() => _(new Error(Jn(k)))));
            }
      ...
      const V = d.trim();
      if (!V) {
        const R = Jn(c.trim()) || `Gemini CLI exited with code ${G ?? "unknown"}`;
        x(() => _(new Error(R)));
        return;
      }
      x(() => g({
...
async function rg(t, e) {
  if (!e.userMessage.trim())
    throw new Error("No chat message provided.");
  const r = vl(), n = Jh();
  await et(r, { recursive: !0 }), await et(n, { recursive: !0 });
  const i = await ki(e.visualRefs ?? [], n);
  ...
  const o = i.length > 0, a = {
    ...e,
    userMessage: _l(e.userMessage, i)
...
function ng() {
  I.handle("llm:gemini-chat", async (t, e) => {
    const r = e.requestId || J.randomUUID(), n = await rg(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
...
const ig = `You are a music prompt engineer. Your job is to write a detailed, evocative text prompt that will be used to generate music with an AI music model (ElevenLabs/Suno).
...
function ag() {
  I.handle("music:generate-prompt", async (t, e) => {
    ...
    if (!r) throw new Error("No fal.ai API key provided.");
    K.fal.config({ credentials: r });
    const n = e.frameUrls && e.frameUrls.length > 0, i = og(e, !!n), o = {
...
function sg() {
  ...
  }), I.handle("shell:open-path", async (t, e) => await va.openPath(e));
}
const lg = {
...
function Pt(t, e) {
  const r = lg[e], n = Object.entries(t).filter(
    ([a]) => a !== "id" && (!r || r.has(a))
  );
...
function Sl(t, e, r) {
  const { setClauses: n, values: i } = Pt(r, "projects");
...
function Lr(t, e, r) {
  const { setClauses: n, values: i } = Pt(r, "assets");
...
function cg(t, e, r) {
  const { setClauses: n, values: i } = Pt(r, "media_folders");
...
function pg(t, e, r) {
  const { setClauses: n, values: i } = Pt(r, "timelines");
  ...
  t.transaction(() => {
    ...
    for (const n of r)
      t.run(
        ...
        [Yt(), n.clip_id, n.time, n.property, n.value]
      );
  });
}
function Eg(t, e) {
...
function jg(t, e, r) {
  const { setClauses: n, values: i } = Pt(r, "elements");
...
function Ng(t, e, r) {
  const { setClauses: n, values: i } = Pt(r, "export_jobs");
  return t.run(`UPDATE export_jobs SET ${n} WHERE id = ?`, [...i, e]);
}
function la(t, e) {
  const r = Tl(t, e);
  if (!r) throw new Error(`Project not found: ${e}`);
  const n = Il(t, e), i = ug(t, e), o = Ig(t, e), a = Ag(t, e), s = Pg(t, e), u = fg(t, e).map((c) => {
    const p = hg(t, c.id), f = yg(t, c.id), m = Eg(t, c.id), y = f.map((h) => ({
      ...h,
      keyframes: bg(t, h.id)
    }));
...
function qg(t, e, r) {
  t.transaction(() => {
    ...
    for (const f of r.timelines) {
      if (l.has(f.id))
        pg(t, f.id, { name: f.name, duration: f.duration });
      else {
...
function Ce(t) {
  let e = Fe.get(t);
  return e || (e = new Kp(t), Fe.set(t, e)), e;
}
function jl() {
  return w.join(ot(), "projects.json");
}
async function Ri() {
  try {
    const t = await j.readFile(jl(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function Pi(t) {
  await j.mkdir(ot(), { recursive: !0 }), await j.writeFile(jl(), JSON.stringify(t, null, 2), "utf-8");
}
async function Cg(t) {
  const e = await Ri(), r = e.projects.findIndex((n) => n.id === t.id);
  r >= 0 ? e.projects[r] = t : e.projects.push(t), await Pi(e);
}
async function Lg(t) {
  const e = await Ri();
  e.projects = e.projects.filter((r) => r.id !== t), await Pi(e);
}
function Ug() {
  I.handle("db:project:create", async (t, e) => {
    ...
    const r = Ce(e), n = la(r, e);
    for (const i of n.assets)
      if (i.file_ref && !i.source_url) {
        const o = i.status;
        B.existsSync(i.file_ref) ? i.status === "offline" && (i.status = "online") : i.status = "offline", i.status !== o && Lr(r, i.id, { status: i.status });
      }
    return n;
  }), I.handle("db:project:save", async (t, e, r) => {
    const n = Ce(e);
    qg(n, e, r);
    const i = ar(), o = await Ri(), a = o.projects.find((s) => s.id === e);
    return a && (a.name = r.project.name, a.updatedAt = i, a.assetCount = r.assets.length, a.elementCount = r.elements.length, await Pi(o)), { ok: !0 };
  }), I.handle("db:project:delete", async (t, e) => {
    const r = Fe.get(e);
    r && (r.close(), Fe.delete(e));
    const n = nt(e);
    try {
    ...
    return await Lg(e), { ok: !0 };
  }), I.handle("db:project:close", async (t, e) => {
    const r = Fe.get(e);
    return r && (r.close(), Fe.delete(e)), { ok: !0 };
  }), I.handle(
    "db:project:update",
    async (t, e, r) => {
      const n = Ce(e);
      return Sl(n, e, r), { ok: !0 };
    }
  ), I.handle("db:asset:insert", async (t, e) => {
    const r = Ce(e.project_id);
    return xl(r, e), { ok: !0 };
  }), I.handle(
    "db:asset:update",
    async (t, e, r, n) => {
      const i = Ce(e);
      return Lr(i, r, n), { ok: !0 };
    }
...
const Dg = /* @__PURE__ */ new Set([".mp4", ".mov", ".avi", ".mkv", ".webm", ".mxf", ".m4v"]), $g = /* @__PURE__ */ new Set([".wav", ".mp3", ".aac", ".flac", ".ogg", ".m4a"]);
function kn(t, e) {
  const r = w.extname(t).toLowerCase();
  return Dg.has(r) ? "video" : $g.has(r) ? "audio" : r ? "image" : e;
}
function Fg(t, e) {
  if (t)
    try {
...
function kr(t) {
  const e = nt(t.projectId), r = w.join(e, ".cache"), n = J.randomUUID(), i = {
    id: n,
  ...
  if (be(i).catch((o) => console.error("[generated-asset-persist] Metadata failed:", o)), t.type === "audio" || t.type === "video") {
...
async function ua(t) {
  ...
  const i = nt(e), o = w.join(i, "media", "generated");
  await j.mkdir(o, { recursive: !0 });
  const a = await Bg(o, r);
  if (a)
    return kr({
      assetId: r,
      projectId: e,
  ...
  const s = t.extension || Fg(t.remoteUrl ?? t.localPathHint, n), l = w.join(o, `${r}${s}`), u = Gg(t.localPathHint);
  if (u)
    return await Vg(u, l), kr({
      assetId: r,
      projectId: e,
      inputPath: l,
      type: kn(l, n)
  ...
  return await j.writeFile(l, Buffer.from(p)), kr({
    assetId: r,
    projectId: e,
    inputPath: l,
    type: kn(l, n)
...
let me = null;
const Le = /* @__PURE__ */ new Map(), Qt = /* @__PURE__ */ new Map(), zg = w.dirname(ni(import.meta.url));
function Rl() {
  let t = w.join(zg, "workers", "media-worker.js");
  return t.includes("app.asar") && (t = t.replace("app.asar", "app.asar.unpacked")), t;
}
function Wg() {
  return me || (me = new Aa(Rl()), me.on("message", (t) => {
    switch (t.type) {
      case "ready":
      ...
      case "job:complete": {
        ...
        Qt.delete(t.jobId);
        const r = Le.get(t.jobId);
        r && (r.resolve(t.result), Le.delete(t.jobId));
        break;
      }
      case "job:error": {
        const e = Qt.get(t.jobId);
        ...
        Qt.delete(t.jobId);
        const r = Le.get(t.jobId);
        r && (r.reject(new Error(t.error)), Le.delete(t.jobId));
        break;
      }
    ...
    for (const [e, r] of Le)
      r.reject(new Error("Worker exited")), Le.delete(e);
  }), me.postMessage({
    type: "config",
    ffmpegPath: Ee(),
    ffprobePath: Ti(),
...
function be(t) {
  return t.type === "sync_compute_offset" || t.type === "sync_batch_match" ? Xg(t) : new Promise((e, r) => {
    Le.set(t.id, { resolve: e, reject: r }), Qt.set(t.id, { assetId: t.assetId, jobType: t.type }), Wg().postMessage({ type: "job:submit", job: t });
  });
}
function Xg(t) {
  return new Promise((e, r) => {
...
function Kg() {
  I.handle("media:import", async (t, e) => {
    const { filePaths: r, projectId: n, mode: i } = e, o = nt(n), a = [], s = [];
    for (const l of r) {
  ...
  }), I.handle("media:submit-job", async (t, e) => be(e)), I.handle("media:cancel-job", async (t, e) => {
    const r = me;
    return r && r.postMessage({ type: "job:cancel", jobId: e }), Le.delete(e), { ok: !0 };
  }), I.handle("media:extract-frame", async (t, e) => {
    const { inputPath: r, timeSec: n } = e, i = Ee(), o = w.join(z.tmpdir(), `cinegen-frame-${J.randomUUID()}.jpg`);
    ...
    const { inputPath: r, startTimeSec: n, durationSec: i } = e, o = Ee(), a = w.join(z.tmpdir(), `cinegen-clip-${J.randomUUID()}.mp4`), s = Math.max(0, n), l = Math.max(0.1, i);
    return new Promise((u) => {
      ...
      tt(o, d, { timeout: Math.max(12e4, Math.ceil(l * 4e3)) }, (c, p, f) => {
        if (c || !B.existsSync(a)) {
          u(null);
          return;
        }
    ...
    if (a) {
      ...
      be(p).catch((f) => console.error("[media-import] Thumbnail failed:", f));
    }
    if (s) {
      ...
      be(p).catch((f) => console.error("[media-import] Waveform failed:", f));
    }
    if (l) {
      ...
      be(p).catch((f) => console.error("[media-import] Filmstrip failed:", f));
    }
    if (o) {
      ...
      be(p).catch((f) => console.error("[media-import] Proxy failed:", f));
    }
    return { ok: !0 };
  }), I.handle(
...
const Zg = xa(import.meta.url), ey = w.dirname(ni(import.meta.url));
function ty() {
  return X.isPackaged ? w.join(process.resourcesPath, "native", "cinegen_avfoundation.node") : w.resolve(ey, "../native/avfoundation/build/Release/cinegen_avfoundation.node");
}
let re = null, ei = null;
if (process.platform === "darwin")
  try {
    const t = ty();
    re = Zg(t), console.log("[native-video] AVFoundation addon loaded:", t);
  } catch (t) {
    ei = t instanceof Error ? t.message : String(t), console.error("[native-video] Failed to load AVFoundation addon:", ei);
  }
function qe() {
  return re != null;
}
...
function ay() {
  I.handle("native-video:is-available", () => ({
    ...
    return !r || !qe() ? !1 : ny(e, r.getNativeWindowHandle());
  }), I.on("native-video:set-surface-rect", (t, e) => {
    qe() && iy(e.surfaceId, e.x, e.y, e.width, e.height);
  }), I.on("native-video:set-surface-hidden", (t, e) => {
    qe() && ca(e.surfaceId, e.hidden);
  }), I.on("native-video:clear-surface", (t, e) => {
    qe() && fa(e);
  }), I.on("native-video:sync-surface", (t, e) => {
    qe() && oy(e.surfaceId, e.descriptors);
  }), I.on("native-video:destroy-surface", (t, e) => {
    qe() && da(e);
  });
}
const sy = "python3.12", Pl = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), ly = w.join(Pl, ".venv", "bin", "python");
function uy(...t) {
  return X.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const dy = uy("scripts", "whisperx", "cinegen_infer.py"), cy = "fal-ai/whisper", pa = "3", fy = {
  ".mp4": "video/mp4",
  ".m4v": "video/mp4",
  ".mov": "video/quicktime",
  ".webm": "video/webm",
...
function py(t) {
  const e = w.extname(t).toLowerCase();
  return fy[e] ?? "application/octet-stream";
}
function Ur(t) {
  const e = Number(t);
...
function Ol(t) {
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    r || (r = {
      ...
      words: []
    }), r.words.push(o), r.end = o.end, r.text = my(r.text, o.word), !r.speaker && o.speaker && (r.speaker = o.speaker);
...
function yy(t) {
  const e = (t == null ? void 0 : t.data) ?? t, r = typeof (e == null ? void 0 : e.text) == "string" ? e.text : "", n = e == null ? void 0 : e.chunks, i = e, o = Array.isArray(n) ? n.flatMap((d) => {
    ...
    const c = typeof d.text == "string" ? d.text.trim() : "", p = d.timestamp, f = Array.isArray(p) ? Ur(p[0]) : void 0, m = Array.isArray(p) ? Ur(p[1]) : void 0, y = hy(d.speaker);
...
function Ge(t, e) {
  var r;
  (r = by()) == null || r.webContents.send("transcription:progress", {
    jobId: t.jobId,
    assetId: t.assetId,
    engine: t.engine,
    ...e
...
async function vy(t) {
  try {
    const e = Ce(t.projectId), r = Il(e, t.projectId).find((o) => o.id === t.assetId), i = {
    ...
    Lr(e, t.assetId, { metadata: JSON.stringify(i) });
  } catch (e) {
    console.error("[transcription] failed to save to db:", e);
  }
}
async function Oi(t) {
  t.status = "done", t.segments = gy(t.segments), t.fullText.trim() || (t.fullText = t.segments.map((e) => e.text).filter(Boolean).join(" ")), await vy(t), Ge(t, {
    type: "done",
    text: t.fullText,
    segments: t.segments,
    language: t.language
...
function Ze(t, e) {
  t.status = "error", t.error = e, Ge(t, { type: "error", error: e });
}
function Ey(t, e) {
  const r = e.model ?? "large", n = e.language ?? "auto";
  t.model = r, (async () => {
    const i = w.join(z.tmpdir(), `cinegen-whisper-${t.jobId}.py`);
    await j.writeFile(i, wy, "utf-8");
    const o = oe(sy, [i, e.filePath, r, n], {
      stdio: ["ignore", "pipe", "pipe"]
    });
    t.status = "running", Ge(t, { type: "status", status: "running" }), o.stdout.on("data", (a) => {
...
`)) {
        ...
        if (l)
          try {
            ...
            if (u.type === "segment") {
              ...
              t.segments.push(d), Ge(t, { type: "segment", ...d });
      ...
      if (await j.unlink(i).catch(() => {
        ...
        Ze(t, `whisper process exited with code ${a}`);
        return;
      }
      await Oi(t);
    }), o.on("error", async (a) => {
    ...
    Ze(t, i instanceof Error ? i.message : String(i));
  });
}
function Ty(t, e) {
  t.model = "base";
  ...
  t.status = "running", Ge(t, { type: "status", status: "running" });
  let o;
  i.stdout.on("data", (a) => {
    for (const s of a.toString().split(`
`)) {
      ...
      if (l)
        try {
          const u = JSON.parse(l);
          u.type === "progress" ? (u.output_text !== void 0 && (t.fullText = u.output_text), u.segments && (t.segments = u.segments), u.language !== void 0 && (t.language = u.language), Ge(t, {
            type: "progress",
            stage: u.stage,
            message: u.message,
          ...
          })) : u.type === "done" ? (u.output_text !== void 0 && (t.fullText = u.output_text), u.segments && (t.segments = u.segments), u.language !== void 0 && (t.language = u.language), o = u.transcript_path) : u.type === "error" && Ze(t, u.error ?? "WhisperX error");
        } catch {
        }
    }
  }), i.stderr.on("data", () => {
    ...
    if (t.status !== "error") {
      if (a !== 0) {
        Ze(t, `whisperx process exited with code ${a}`);
        return;
      }
      if (o)
        try {
...
function Sy(t, e) {
  (async () => {
    ...
    t.model = pa, t.status = "running", Ge(t, { type: "status", status: "running", stage: "uploading", message: "Preparing audio for cloud transcription" }), K.fal.config({ credentials: e.apiKey });
    const r = await _y(e.filePath);
    let n = "";
    try {
      const s = await j.readFile(r), u = `${w.basename(e.filePath, w.extname(e.filePath))}.m4a`, d = py(r), c = new Blob([s], { type: d }), p = new File([c], u, { type: d });
      n = await K.fal.storage.upload(p);
    } finally {
      await j.unlink(r).catch(() => {
      });
    }
    Ge(t, { type: "status", status: "running", stage: "transcribing", message: "Running cloud transcription" });
    const i = {
      audio_url: n,
      task: "transcribe",
      chunk_level: "word",
    ...
    }, o = await K.fal.subscribe(cy, { input: i, logs: !0 }), a = yy(o);
    t.fullText = a.text, t.segments = a.segments, t.language = a.language, await Oi(t);
  })().catch((r) => {
    Ze(t, r instanceof Error ? r.message : String(r));
  });
}
function Iy() {
  I.handle("transcription:start", async (t, e) => {
    ...
    return ma.set(u, d), s === "whisperx-local" ? Ty(d, { filePath: i, language: a }) : s === "whisper-cloud" ? Sy(d, { filePath: i, language: a, apiKey: l }) : Ey(d, { filePath: i, model: o, language: a }), { jobId: u };
...
const Ni = w.join(z.homedir(), "Desktop", "Coding", "ltx"), xy = w.join(Ni, ".venv", "bin", "python"), Ay = w.join(Ni, "cinegen_infer.py"), qi = w.join(z.homedir(), "Desktop", "Coding", "qwen-edit"), ky = w.join(qi, ".venv", "bin", "python"), jy = w.join(qi, "cinegen_infer.py"), Nl = w.join(z.homedir(), "Desktop", "Coding", "layer-decompose"), Ry = w.join(Nl, ".venv", "bin", "python"), ql = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), Py = w.join(ql, ".venv", "bin", "python");
function Cl(...t) {
  return X.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const Oy = Cl("scripts", "layer-decompose", "cinegen_infer.py"), Ny = Cl("scripts", "whisperx", "cinegen_infer.py"), qy = {
...
function pt(t, e) {
  var r;
  (r = Cy()) == null || r.webContents.send("local-model:progress", { jobId: t, ...e });
}
...
function Ly() {
  I.handle("local-model:run", async (t, e) => {
    ...
    if (e.nodeType === "qwen-edit-local") {
      ...
      let p = null;
      if (r.image_url) {
        const m = await Tr(String(r.image_url), n);
      ...
      if (r.audio_url) {
        ...
        const m = await Tr(String(r.audio_url), n);
      ...
      p && (f.HF_TOKEN = p), o = oe(Py, c, {
        cwd: ql,
        stdio: ["ignore", "pipe", "pipe"],
        env: f
      });
    } else {
      const s = String(r.prompt ?? ""), l = String(r.resolution ?? "896x512"), { height: u, width: d } = qy[l] ?? { height: 512, width: 896 }, c = Number(r.frame_rate ?? 24), p = Number(r.duration_secs ?? 4), f = Math.round(p * c / 8) * 8 + 1, m = Math.max(9, f), y = Number(r.seed ?? 42), h = !!r.enhance_prompt;
      let g = null;
      if (r.image_url) {
        const b = await Tr(String(r.image_url), n);
      ...
      g && _.push("--image_path", g), h && _.push("--enhance_prompt"), o = oe(xy, _, {
        cwd: Ni,
        stdio: ["ignore", "pipe", "pipe"]
      });
    }
    return i.status = "running", pt(n, { type: "status", status: "running" }), o.stdout.on("data", (s) => {
...
`)) {
        ...
        if (u)
          try {
            ...
            d.type === "progress" ? (i.stage = d.stage, d.output_text !== void 0 && (i.outputText = d.output_text), d.segments && (i.segments = d.segments), d.language !== void 0 && (i.language = d.language), pt(n, {
              type: "progress",
              stage: d.stage,
              message: d.message,
            ...
            })) : d.type === "done" ? (i.status = "done", i.outputPath = d.output_path, i.outputText = d.output_text, i.transcriptPath = d.transcript_path, i.segments = d.segments, i.language = d.language, pt(n, {
              type: "done",
            ...
            })) : d.type === "error" && (i.status = "error", i.error = d.error, pt(n, { type: "error", error: d.error }));
          } catch {
          }
      }
    }), o.stderr.on("data", () => {
    }), o.on("error", (s) => {
      i.status = "error", i.error = s.message, pt(n, { type: "error", error: s.message });
    }), o.on("close", (s) => {
      a && j.unlink(a).catch(() => {
      }), s !== 0 && i.status !== "done" && (i.status = "error", i.error = i.error ?? `Process exited with code ${s}`, pt(n, { type: "error", error: i.error }));
    }), { jobId: n };
...
const Ci = w.join(z.homedir(), "Desktop", "Coding", "Sam3"), Uy = w.join(Ci, ".venv", "bin", "python"), My = w.join(Ci, "cinegen_server.py"), Dy = 120 * 1e3, $y = 500, Fy = 60;
class By {
  constructor() {
  ...
  async start() {
    var e, r;
    return this.proc && !this.proc.killed ? this.port : (this.port = await this.findFreePort(), console.log(`[sam3] Starting server on port ${this.port}`), this.proc = oe(Uy, [My, "--port", String(this.port)], {
      cwd: Ci,
      stdio: ["ignore", "pipe", "pipe"],
      env: {
        ...process.env,
  ...
  async waitForHealth() {
    ...
    for (let e = 0; e < Fy; e++) {
      ...
      await new Promise((r) => setTimeout(r, $y));
    }
...
const Zt = new By();
function Hy() {
  I.handle("sam3:start", async () => ({ port: await Zt.ensureRunning() })), I.handle("sam3:stop", async () => {
    await Zt.stop();
  }), I.handle("sam3:port", () => ({ port: Zt.getPort(), running: Zt.isRunning() }));
}
function Gy() {
  Zt.stop();
}
...
async function zy(t) {
  const e = t.workspaceDir ?? w.join(X.getPath("userData"), "gemini-cli-workspace"), r = await ki(t.visualRefs, e);
  if (r.length === 0)
    throw new Error("Could not load the attached clip or asset files for visual analysis.");
  try {
    const n = [];
    for (const i of r) {
      const o = Vy(t.prompt, i.label, i.mediaType), a = i.mediaType === "video" ? await tl({
        apiKey: t.apiKey,
...
function Wy() {
  I.handle("copilot:analyze-visual-refs", async (t, e) => zy(e));
}
const Xy = 1, Jy = -30, Ky = 0.3;
function Yy(t) {
  const e = [];
  let r = null;
  for (const n of t.split(/\r?\n/)) {
...
function t_(t) {
  ...
  return (Array.isArray(r) ? r : r && typeof r == "object" && Array.isArray(r.segments) ? r.segments : []).flatMap((i) => {
    ...
    return a === void 0 || s === void 0 || s <= a ? [] : [{
      ...
      delivery: mt(o.delivery),
      emotion: mt(o.emotion),
      energy: mt(o.energy),
      pace: mt(o.pace),
      notable: e_(o.notable),
      content: mt(o.content),
      shotType: mt(o.shotType),
...
function r_(t) {
  return new Promise((e) => {
    const r = [
      ...
      t,
      "-af",
      `silencedetect=noise=${Jy}dB:d=${Ky}`,
      "-f",
      "null",
      "-"
    ], n = oe(Ee(), r);
...
const Ll = "gemini-2.5-flash", n_ = "fal-ai/video-understanding";
async function i_(t, e) {
  var n;
  const r = ((n = t.model) == null ? void 0 : n.trim()) || Ll;
  try {
...
async function o_(t) {
  ...
  const e = {
    ...
    status: "failed",
    version: Xy,
    model: ((r = t.model) == null ? void 0 : r.trim()) || Ll,
    silenceMap: [],
    segments: [],
  ...
  try {
    const n = await r_(t.mediaPath).catch(() => ""), i = Yy(n), o = Qy({ assetName: t.assetName, transcript: t.transcript }), { rawText: a, model: s } = await i_(t, o), l = t_(a);
    return {
      ...e,
      model: s,
      status: "ready",
...
function a_() {
  I.handle("acoustic:analyze-asset", async (t, e) => o_(e));
}
const s_ = process.platform === "darwin" && !X.isPackaged;
s_ && (X.disableHardwareAcceleration(), X.commandLine.appendSwitch("disable-gpu-compositing"), console.log("[app] hardware acceleration disabled for macOS dev wake stability"));
...
Ta.registerSchemesAsPrivileged([
  {
    scheme: "local-media",
    privileges: {
      standard: !0,
...
let Z = null, Jt = null, ee = null, Me = null;
const l_ = Date.now(), ya = "cinegen-desktop", _a = "CineGen", u_ = ".cinegen-user-data-migrated.json", wa = "CineGen", d_ = 700;
...
function f_() {
  try {
    const t = X.getPath("appData"), e = w.join(t, ya), r = w.join(t, _a);
    return X.getPath("userData") !== r && X.setPath("userData", r), console.log("[app] userData path:", r), { preferredUserDataPath: r, legacyUserDataPath: e };
  } catch (t) {
    console.error("[app] failed to configure userData path:", t);
    const e = X.getPath("appData"), r = w.join(e, _a), n = w.join(e, ya);
    return { preferredUserDataPath: r, legacyUserDataPath: n };
  }
}
const p_ = f_();
try {
  X.setName(wa), process.platform === "darwin" && X.setAboutPanelOptions({
    applicationName: wa,
...
async function m_() {
  const { preferredUserDataPath: t, legacyUserDataPath: e } = p_;
...
function g_(t) {
  const e = w.extname(t).toLowerCase();
  return c_[e] ?? "application/octet-stream";
}
function y_(t, e) {
...
X.whenReady().then(async () => {
  if (await m_(), process.platform === "darwin") {
    const r = h_();
    console.log("[dock] icon candidates:", r);
    for (const n of r)
      try {
        const i = Dl.createFromPath(n);
        if (console.log("[dock] testing icon:", n, "empty?", i.isEmpty()), !i.isEmpty()) {
          await Promise.resolve(X.dock.setIcon(i)), console.log("[dock] applied icon:", n);
          break;
        }
      } catch (i) {
  ...
  Ta.handle("local-media", async (r) => {
    try {
      ...
      const o = i.size, a = g_(n), s = y_(r.headers, "range");
      ...
      if (s) {
        ...
        const f = p - c + 1, m = B.createReadStream(n, { start: c, end: p }), y = Mi.toWeb(m);
      ...
      const l = B.createReadStream(n), u = Mi.toWeb(l);
  ...
  }), su(), du(() => Z), Af(), zd(), Df(), Dp(), Hp(), Wp(), am(), Vm(), oh(), uh(), yh(), $h(), Lh(), ng(), ag(), sg(), Ug(), Kg(), Qg(be), vm(), Wy(), a_(), ay(), Iy(), Ly(), Hy(), await b_(), I.handle("pm:open-project", async (r, n, i) => n === "__close__" ? (ee == null || ee.close(), ee = null, { ok: !0 }) : ((!Z || Z.isDestroyed()) && (Z = Bi()), Z.once("ready-to-show", () => {
    Z == null || Z.maximize(), Z == null || Z.show(), Z == null || Z.webContents.send("pm:open-project", n, i);
  }), Z.webContents.getURL() !== "" && (Z.maximize(), Z.show(), Z.webContents.send("pm:open-project", n, i)), ee == null || ee.close(), ee = null, { ok: !0 })), I.handle("pm:open", async () => ee && !ee.isDestroyed() ? (ee.focus(), { ok: !0 }) : (ee = un(), ee.on("closed", () => {
    ee = null;
  }), { ok: !0 })), Jt = nu(), Z = Bi();
  const t = 3e3;
  Z.once("ready-to-show", () => {
    const r = Date.now() - l_, n = Math.max(0, t - r);
    setTimeout(() => {
      Jt == null || Jt.close(), Jt = null, ee = un(), ee.on("closed", () => {
        ee = null;
      });
    }, n);
  }), X.on("activate", () => {
    Q.getAllWindows().length === 0 && (ee = un(), ee.on("closed", () => {
      ee = null;
    }));
  });
  const e = (r) => {
    Me && (clearTimeout(Me), Me = null), Me = setTimeout(() => {
      Me = null, console.log(`[app] Wake recovery triggered by ${r}`), ru(r);
    }, d_);
  };
  sn.on("resume", () => {
    Rn("resume"), e("resume");
  }), sn.on("unlock-screen", () => {
    Rn("unlock-screen"), e("unlock-screen");
  }), sn.on("suspend", () => {
    Rn("suspend");
  });
});
X.on("before-quit", () => {
  cu(), Me && (clearTimeout(Me), Me = null), Yg(), Mg(), Gy();
});
