...
function registerWorkflowHandlers() {
  ipcMain.handle("workflow:run", async (_event, params) => {
    ...
    const { ALL_MODELS, resolveVideoModelEndpoint, sanitizeVideoInputsForEndpoint } = await import("./models-Cfen1Ws8.js");
...
function topviewRequiresInheritedVideoDuration(message) {
  return /duration/i.test(message) && /must\s+be\s+`?-1`?/i.test(message);
}
function buildTopviewVideoRequest(args) {
  ...
  if (args.inheritInputVideoDuration) {
