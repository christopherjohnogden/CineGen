...
import { ipcMain as I, app as W, shell as Jr, BrowserWindow as Q, screen as Fa, safeStorage as Sr, session as Ba, dialog as ro, protocol as Va, nativeImage as fd, powerMonitor as _n } from "electron";
...
function xd() {
  I.handle("claude-mcp:status", () => bn()), I.handle("claude-mcp:setup", async () => {
    wn();
    const t = bt();
    ...
    if (wn(), !it(bt().config)) throw new Error("Connect Claude Desktop first to create its configuration.");
...
const jt = Rd(), Kr = w.join(import.meta.dirname, "."), es = w.join(Kr, "../dist"), wr = process.env.VITE_DEV_SERVER_URL;
function ao(t) {
...
function Nd(t) {
  for (const e of Q.getAllWindows()) {
    ...
    vn(e, a), i = setTimeout(() => {
      Ir(e, i), !e.isDestroyed() && zn(e, n, r, `hard reload after ${t}`).catch((s) => {
...
const uo = 12e4, as = Et(Ga(), "Documents", "CINEGEN"), Xn = Et(as, "mcp-bridge.json"), Rt = /* @__PURE__ */ new Map();
let Nr = !1, me = null, Ar = "";
function je(t, e, r) {
...
function Er() {
  ...
  const r = Yr(), n = Si(), i = Je(), o = typeof navigator < "u" && (navigator == null ? void 0 : navigator.userAgent) === "Cloudflare-Workers";
  function a(l) {
    return t(this, void 0, void 0, function* () {
      var d;
...
function Xd() {
  ...
  const r = Ti(), n = Er(), i = lt(), o = Yr(), a = ls(), s = us(), l = Je(), d = 500, u = {
    maxRetries: 3,
    baseDelay: 1e3,
    maxDelay: 6e4,
...
const Kn = "array", ar = "map_key", bs = "map_value", _u = (t) => {
  ...
  *decodeMulti(e) {
  ...
  async decodeAsync(e) {
    ...
    try {
      ...
      throw new RangeError(`Insufficient data in parsing ${xn(i)} at ${a} (${o} in the current buffer)`);
    } finally {
      this.entered = !1;
    }
  }
  ...
  doDecodeSync() {
    e: for (; ; ) {
      ...
      if (e >= 224)
        ...
        throw new le(`Unrecognized type byte: ${xn(e)}`);
      this.complete();
      const n = this.stack;
      for (; n.length > 0; ) {
        const i = n.top();
  ...
  readArraySize() {
    ...
    switch (e) {
      ...
      default: {
        if (e < 160)
          return e - 144;
        throw new le(`Unrecognized array type byte: ${xn(e)}`);
      }
    }
  }
  pushMapState(e) {
  ...
  function G({ config: R }) {
    return {
      connect(P, V) {
        ...
        if (k && !(0, o.isBrowser)())
          return A;
        const Mt = ne ?? ((Ye) => B(Ye)), un = ke ?? ((Ye) => M(Ye));
        let Tr, to, Dt, $t = 0;
        S.set(O, {
          decodeMessage: un,
          onError: V.onError,
        ...
        const cn = () => S.get(O), Ft = x(O, X, ({ context: Ye, machine: Qe, send: Ue }) => {
          var fn;
          const { enqueuedMessage: pn, token: mn, websocket: hn } = Ye;
          if (to = pn, Qe.current === "active" && pn && (hn == null ? void 0 : hn.readyState) === WebSocket.OPEN && Ue({ type: "send", message: pn }), Qe.current === "authRequired" && mn === void 0 && Tr !== Qe.current) {
            Ue({ type: "initiateAuth" }), $t++;
            const xe = $t, ce = (0, a.ensureEndpointIdFormat)(P), ve = (fn = (0, a.resolveEndpointPath)(P, L, "/realtime")) !== null && fn !== void 0 ? fn : "", Ze = ue ? () => ue(`${ce}${ve}`) : () => (console.warn("[fal.realtime] Using the default token provider is deprecated. Please provide a `tokenProvider` function to `fal.realtime.connect()`. See https://docs.fal.ai/model-apis/client#client-side-usage-with-token-provider for more information."), (0, n.getTemporaryAuthToken)(P, R)), Bt = ue ? Le : n.TOKEN_EXPIRATION_SECONDS, gn = Bt !== void 0 ? () => {
              clearTimeout(Dt);
              const Vt = Math.round(Bt * 0.9 * 1e3);
              Dt = setTimeout(() => {
                xe === $t && Ze().then((yn) => {
                  xe === $t && (queueMicrotask(() => {
                    Ue({ type: "authenticated", token: yn });
                  }), gn());
                }).catch(() => {
                  if (xe !== $t)
                    return;
                  const yn = Math.round(Bt * 0.05 * 1e3);
                  Dt = setTimeout(() => {
                    gn();
                  }, yn);
                });
          ...
          if (Qe.current === "connecting" && Tr !== Qe.current && mn !== void 0) {
            const xe = new WebSocket(g(P, { token: mn, maxBuffering: q, path: L }));
            xe.onopen = () => {
              var ce, ve;
              ...
              const { decodeMessage: ve = un, onResult: Ze, onError: Bt = T } = cn();
              H({
                data: ce.data,
                decodeMessage: ve,
                onResult: Ze,
...
const Co = 1260 * 1e3, uc = 9e4, qo = 4, An = "Higgsfield CLI not found. Install @higgsfield/cli, then run higgsfield auth login — or connect Higgsfield in Settings.";
function cc(t) {
...
async function qe(t, e = 6e4) {
  ...
  if (i.length === 0) throw new Error(An);
  let o;
  for (const l of i)
    try {
  ...
  throw new Error(s ? `${An} Tried: ${a}. ${s}` : `${An} Tried: ${a}.`);
}
const pc = 6e4, ks = {
  "image/png": ".png",
  "image/jpeg": ".jpg",
...
async function gc(t) {
  ...
  for (let n = 1; n <= 3; n++)
    try {
      const i = await qe(e, uc), o = await Zn(i, t);
      if (o) return o;
      const a = Oo(i);
      if (a) return { jobId: a };
      ...
      r = i;
      const { stdout: o, stderr: a } = As(i), s = await Zn(o, t);
      if (s) return s;
      const l = Oo(o, a, Tt(i));
      if (l) return { jobId: l };
      if (!Qn(Tt(i)) || n === 3) throw i;
      await Rs(1500 * n);
    }
...
async function Ps(t, e) {
  ...
  for (let i = 1; i <= qo; i++)
    try {
      ...
      r = o;
      const { stdout: a } = As(o), s = await Zn(a, e);
      if (s) return s;
      try {
...
function Uc(t) {
  try {
    return new URL(t).pathname === "/health" ? qs : ti;
  } catch {
    return ti;
  }
}
...
async function Dc(t, e, r) {
  ...
  try {
    ...
    for (; !c.done && u.length < kn && (u += d.decode(c.value, { stream: !0 }), !(u.length >= kn)); ) {
      let p;
    ...
    return u += d.decode(), Mc(u.slice(0, kn));
  } catch {
    return [];
  } finally {
...
function Mi(t, e) {
  const r = Zr(e, "RunPod session ID"), n = new URL(Se(t, "RunPod session URL"));
...
async function Fs(t, e = fetch) {
  const r = Se(t.runpodKey, "RunPod API key"), n = Zr(t.podId, "RunPod session ID");
...
async function Ef(t, e = fetch) {
  ...
  if (t.jobId) {
    const d = Zr(t.jobId, "RunPod generation job ID"), u = await Ie(e, `${r.url}/status/${d}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    ...
    if (c === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: en(u, "LTX-2.5 generation failed.") };
    if (c === "COMPLETED") {
...
async function Tf(t, e = fetch) {
  ...
  if (t.jobId) {
    ...
    const m = tn(f), y = String(u.status ?? "").toUpperCase();
    if (y === "IN_QUEUE")
    ...
    if (y === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: en(u, `${m} generation failed.`) };
    if (y === "COMPLETED")
      try {
...
function ip() {
  I.handle("workflow:run", async (e, r) => {
    const {
      ...
      outputType: c,
      inputs: p
    } = r, { ALL_MODELS: f, resolveVideoModelEndpoint: m, sanitizeVideoInputsForEndpoint: y } = await import("./models-BQd30hJX.js"), h = f[u] ?? Object.values(f).find(
...
function ii(t, e = []) {
  return Array.isArray(t) ? t.forEach((r) => ii(r, e)) : Ae(t) && (e.push(t), Object.values(t).forEach((r) => ii(r, e))), e;
}
function $p(t) {
  for (const e of ii(t))
...
const Qp = "https://mcp.topview.ai/mcp", jn = "https://mcp.topview.ai", Zp = "https://www.topview.ai/mcp_oauth/oauth/authorize", em = "https://www.topview.ai/mcp_oauth/oauth/token", tm = "https://www.topview.ai/mcp_oauth/oauth/register", na = "https://www.topview.ai/mcp_oauth/oauth/userinfo", rm = 300 * 1e3, Rn = 1200 * 1e3, nm = 90 * 1e3, im = 30 * 1e3, Ct = 45 * 1024 * 1024, ia = 50;
function $(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function On(t) {
...
function ul(t, e) {
  ...
  if ($(r)) {
    const n = r[e];
    if (Array.isArray(n)) return n.map(qn);
    if ($(n)) {
      for (const i of ["values", "options", "enum", "allowedValues"])
        if (Array.isArray(n[i])) return n[i].map(qn);
    }
  }
  if (Array.isArray(r)) {
...
function la(t) {
  ...
  for (const [s, l, d] of [
    ...
    ur(r, s) && (o[s] = si({ model: r, field: s, requested: l, fallback: d }));
  for (const s of Vi(r))
...
function Ln(t) {
  ...
  if (l("resolution", t.params.resolution === void 0 ? void 0 : Number.parseInt(t.params.resolution, 10), 720), t.inheritInputVideoDuration || o === ra ? s.duration = ra : ur(e, "duration") && l("duration", o, 5), l("generatingCount", void 0, 1), t.taskType !== "image_to_video" ? l("aspectRatio", (p = t.params.aspectRatio) == null ? void 0 : p.trim(), "16:9") : (t.params.aspectRatio !== void 0 || i.has("aspectRatio")) && l("aspectRatio", (f = t.params.aspectRatio) == null ? void 0 : f.trim(), "16:9"), ai(e) !== !1 && (ur(e, "sound") || t.params.generateAudio === !0))
    s.sound = si({
      model: e,
      field: "sound",
...
async function ca(t, e) {
  ...
  const i = await gl(r), o = Un(i.finalUrl, e, i.contentType);
...
function Em(t) {
  ...
  if (!(r >= e))
    return `This reference video is ${t.width}x${t.height}, which is ${li(r)} pixels per frame. "${t.submitModel}" requires at least ${li(e)}. Re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice), then attach it again.`;
}
function fa(t) {
  var e;
  if (/video\s+pixel\s+count/i.test(t)) {
    ...
    return `Seedance rejected a reference video for being too small. It needs at least ${li(n)} pixels per frame, so re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice) and attach it again. A 640x360 clip is the usual cause.`;
  }
  if (ol(t))
...
class Sm {
  ...
  async tokenExchange(e, r) {
    ...
    return n.set("client_id", r.client_id), r.client_secret && n.set("client_secret", r.client_secret), Cn(em, {
      method: "POST",
  ...
  async mcpRequest(e, r, n) {
    ...
    try {
      ...
      if (!s.ok) throw oi(s.status, d, "Topview MCP request failed.");
      if ($(d) && d.error !== void 0) throw oi(400, d.error, "Topview MCP returned an error.");
      return {
        payload: $(d) ? d : {},
...
CineGen could not create the temporary resized copy automatically.`);
  ...
  async accountStatus() {
    ...
    try {
      ...
      const n = await this.accessToken(), i = await Cn(na, {
        method: "GET",
  ...
  async authLogin() {
    ...
    const i = `http://127.0.0.1:${n.port}/oauth/callback`, o = On(J.randomBytes(48)), a = On(J.createHash("sha256").update(o).digest()), s = On(J.randomBytes(32));
    let l, d;
    ...
    try {
      ...
      throw l && !l.writableEnded && oa(l, !1, Pn(f, "Topview sign-in did not complete.")), f;
    } finally {
  ...
  async submitVideoRequest(e, r) {
    ...
    try {
      ...
      const o = Ln({ ...r, inheritInputVideoDuration: !0 });
      return {
        built: o,
...
function Cm() {
  I.handle(
    ...
    async (t, e, r) => {
      ...
      const i = await Nm(n);
      try {
        const o = await j.readFile(i), s = `${w.basename(n, w.extname(n))}.m4a`, l = Mn(i), d = new Blob([o], { type: l }), u = new File([d], s, { type: l });
    ...
    async (t, e, r) => {
      ...
      const n = ha(e);
      if (n) {
        const i = await j.readFile(n), o = w.basename(n), a = Mn(n), s = new Blob([i], { type: a }), l = new File([s], o, { type: a });
      ...
      if (e.startsWith("http://") || e.startsWith("https://")) {
        ...
        const s = await j.readFile(a), l = w.basename(a), d = Mn(a), u = new Blob([s], { type: d }), c = new File([u], l, { type: d }), p = await K.fal.storage.upload(c);
...
${l}`);
  ...
  const n = (t.messages ?? []).filter((s) => s.content.trim()), i = n.length > 0 ? ci(n) : `${t.userMessage.trim()}
...
const _g = "http://localhost:3000", Ml = "https://cinegen-film.vercel.app", zi = "persist:cinegen-team-workspace", wg = 8e3, fi = "__CINEGEN_TEAM_PROVIDER__", He = { origin: Ml, source: "hosted" }, Fn = { origin: _g, source: "local-web" };
...
async function hr(t) {
  try {
    const { response: e, payload: r } = await ln(t, "providers", "status", []);
...
async function vg(t, e, r) {
  ...
  const { response: i, payload: o } = await ln(n.target, t, e, r);
  if (!i.ok || !(o != null && o.ok))
...
async function xa(t, e) {
  ...
  if (!r)
    throw new Error("Connect CineGen Desktop to the hosted team workspace in Settings first.");
  const { response: n, payload: i } = await ln(r.target, "providers", t, [e]);
...
async function Eg() {
  ...
  const { response: r, payload: n } = await ln(e.target, "topview", "importTeamConnection", [t]);
...
async function Tg(t) {
  return vg("llm", "openaiChat", [{ ...t, apiKey: fi }]);
}
async function $l() {
  const t = await hr(He);
  if (t)
...
const jg = /* @__PURE__ */ new Set(["authCancel", "authLogin", "authStatus", "accountStatus", "connect", "disconnect", "voices", "generate", "job", "design", "saveVoice", "enhance"]);
function Rg() {
...
async function Bl(t, e, r, n) {
  const i = be(), o = Math.max(0, e), a = Math.max(0.1, Math.min(r, Pg));
  try {
    return await Fl(i, [
      "-y",
      "-ss",
...
function Bn(t, e, r) {
  ...
  const n = w.extname(t) || (e.mediaType === "image" ? ".jpg" : ".mp4"), i = w.join(r, `${pi(e)}-source${n}`), o = Hl(t, i);
...
async function Wi(t, e) {
  ...
  for (const i of t) {
    ...
    if (i.trimStartSec !== void 0 && i.trimDurationSec !== void 0) {
      const u = w.join(r, `${pi(i)}.mp4`), c = await Bl(
        o,
        i.trimStartSec,
        i.trimDurationSec,
        u
    ...
    const l = w.join(r, `${pi(i)}.jpg`), d = await Ur(o, i.trimStartSec ?? 0, l);
    d && n.push({
      label: i.label,
      kind: i.kind,
      mediaType: "image",
...
const qg = 18e4, Lg = 600 * 1e3;
async function Wl(t) {
  var u;
  const e = await sn("gemini");
  if (!e)
...
`)) >= 0; ) {
      ...
      const A = m.trim();
      if (!A) {
        const C = ui(y.trim()) || `Gemini CLI exited with code ${T ?? "unknown"}`;
        b(() => p(new Error(C)));
        return;
      }
      if (Cg(A)) {
...
async function ka(t, e, r) {
  var h;
  const n = await sn("gemini");
  if (!n)
    throw new Error("Gemini CLI is not installed. Install it with: npm install -g @google/gemini-cli");
  const i = ((h = e.model) == null ? void 0 : h.trim().replace(/^[^/]+\//, "")) || "gemini-2.5-flash", o = r.canResume ? Bg(e) : Fg(e), a = o.length > Dg, s = Xl();
  ...
  const d = ql();
  let u = "", c = "", p, f;
  const m = 900 * 1e3, y = r.hasVisualRefs ? Mg : Ug;
...
`)) >= 0; ) {
        ...
        if (v = v.slice(G + 1), !!R)
          try {
            ...
            if (P.type === "error" && typeof P.message == "string") {
              const k = P.message;
              c += k, !u.trim() && Gg(k) && (we = null, b.kill("SIGTERM"), x(() => _(new Error(ui(k)))));
            }
      ...
      const G = u.trim();
      if (!G) {
        const R = ui(c.trim()) || `Gemini CLI exited with code ${H ?? "unknown"}`;
        x(() => _(new Error(R)));
        return;
      }
      x(() => g({
...
function Xg() {
  I.handle("llm:gemini-chat", async (t, e) => {
    const r = e.requestId || J.randomUUID(), n = await Wg(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
...
const Jg = `You are a music prompt engineer. Your job is to write a detailed, evocative text prompt that will be used to generate music with an AI music model (ElevenLabs/Suno).
...
function Yg() {
  I.handle("music:generate-prompt", async (t, e) => {
    ...
    if (!r) throw new Error("No fal.ai API key provided.");
    K.fal.config({ credentials: r });
    const n = e.frameUrls && e.frameUrls.length > 0, i = Kg(e, !!n), o = {
...
function Ut(t, e) {
  const r = Zg[e], n = Object.entries(t).filter(
    ([a]) => a !== "id" && (!r || r.has(a))
  );
...
function ja(t, e) {
  ...
  const n = Ql(t, e), i = ey(t, e), o = gy(t, e), a = _y(t, e), s = Ey(t, e), d = ny(t, e).map((c) => {
    const p = ay(t, c.id), f = ly(t, c.id), m = py(t, c.id), y = f.map((h) => ({
      ...h,
      keyframes: cy(t, h.id)
    }));
...
function Iy(t, e, r) {
  t.transaction(() => {
    ...
    for (const f of r.timelines) {
      if (l.has(f.id))
        iy(t, f.id, { name: f.name, duration: f.duration });
      else {
...
const Ry = /* @__PURE__ */ new Set([".mp4", ".mov", ".avi", ".mkv", ".webm", ".mxf", ".m4v"]), Oy = /* @__PURE__ */ new Set([".wav", ".mp3", ".aac", ".flac", ".ogg", ".m4a"]);
function Vn(t, e) {
  const r = w.extname(t).toLowerCase();
  return Ry.has(r) ? "video" : Oy.has(r) ? "audio" : r ? "image" : e;
}
function Py(t, e) {
  if (t)
    try {
...
async function Ra(t) {
  ...
  const s = t.extension || Py(t.remoteUrl ?? t.localPathHint, n), l = w.join(o, `${r}${s}`), d = qy(t.localPathHint);
  if (d)
    return await Ly(d, l), Mr({
      assetId: r,
      projectId: e,
      inputPath: l,
      type: Vn(l, n)
...
let he = null;
const $e = /* @__PURE__ */ new Map(), ir = /* @__PURE__ */ new Map(), Uy = w.dirname(bi(import.meta.url));
function nd() {
  let t = w.join(Uy, "workers", "media-worker.js");
...
function Te(t) {
  return t.type === "sync_compute_offset" || t.type === "sync_batch_match" ? Dy(t) : new Promise((e, r) => {
    $e.set(t.id, { resolve: e, reject: r }), ir.set(t.id, { assetId: t.assetId, jobType: t.type }), My().postMessage({ type: "job:submit", job: t });
  });
}
function Dy(t) {
  return new Promise((e, r) => {
...
const Hy = Wa(import.meta.url), Gy = w.dirname(bi(import.meta.url));
function zy() {
  return W.isPackaged ? w.join(process.resourcesPath, "native", "cinegen_avfoundation.node") : w.resolve(Gy, "../native/avfoundation/build/Release/cinegen_avfoundation.node");
}
let re = null, hi = null;
if (process.platform === "darwin")
  try {
    const t = zy();
    re = Hy(t), console.log("[native-video] AVFoundation addon loaded:", t);
  } catch (t) {
    hi = t instanceof Error ? t.message : String(t), console.error("[native-video] Failed to load AVFoundation addon:", hi);
  }
function Me() {
  return re != null;
}
...
function Yy() {
  I.handle("native-video:is-available", () => ({
    ...
    return !r || !Me() ? !1 : Xy(e, r.getNativeWindowHandle());
  }), I.on("native-video:set-surface-rect", (t, e) => {
    Me() && Jy(e.surfaceId, e.x, e.y, e.width, e.height);
    ...
    Me() && Ky(e.surfaceId, e.descriptors);
  }), I.on("native-video:destroy-surface", (t, e) => {
    Me() && Oa(e);
  });
}
const Qy = "python3.12", id = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), Zy = w.join(id, ".venv", "bin", "python");
function e_(...t) {
  return W.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const t_ = e_("scripts", "whisperx", "cinegen_infer.py"), r_ = "fal-ai/whisper", Ca = "3", n_ = {
  ".mp4": "video/mp4",
  ".m4v": "video/mp4",
  ".mov": "video/quicktime",
  ".webm": "video/webm",
...
function i_(t) {
  const e = w.extname(t).toLowerCase();
  return n_[e] ?? "application/octet-stream";
}
function Wr(t) {
  const e = Number(t);
...
function od(t) {
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    r || (r = {
      ...
      words: []
    }), r.words.push(o), r.end = o.end, r.text = o_(r.text, o.word), !r.speaker && o.speaker && (r.speaker = o.speaker);
...
function l_(t) {
  const e = (t == null ? void 0 : t.data) ?? t, r = typeof (e == null ? void 0 : e.text) == "string" ? e.text : "", n = e == null ? void 0 : e.chunks, i = e, o = Array.isArray(n) ? n.flatMap((u) => {
    ...
    const c = typeof u.text == "string" ? u.text.trim() : "", p = u.timestamp, f = Array.isArray(p) ? Wr(p[0]) : void 0, m = Array.isArray(p) ? Wr(p[1]) : void 0, y = a_(u.speaker);
...
function Xe(t, e) {
  var r;
  (r = c_()) == null || r.webContents.send("transcription:progress", {
    jobId: t.jobId,
    assetId: t.assetId,
    engine: t.engine,
    ...e
...
async function Yi(t) {
  t.status = "done", t.segments = s_(t.segments), t.fullText.trim() || (t.fullText = t.segments.map((e) => e.text).filter(Boolean).join(" ")), await f_(t), Xe(t, {
    type: "done",
    text: t.fullText,
    segments: t.segments,
    language: t.language
...
function p_(t, e) {
  ...
  t.model = r, (async () => {
    ...
    await j.writeFile(i, u_, "utf-8");
    const o = ae(Qy, [i, e.filePath, r, n], {
      stdio: ["ignore", "pipe", "pipe"]
    });
...
function h_(t, e) {
  (async () => {
    ...
    const r = await d_(e.filePath);
    let n = "";
    try {
      const s = await j.readFile(r), d = `${w.basename(e.filePath, w.extname(e.filePath))}.m4a`, u = i_(r), c = new Blob([s], { type: u }), p = new File([c], d, { type: u });
    ...
    }, o = await K.fal.subscribe(r_, { input: i, logs: !0 }), a = l_(o);
...
function g_() {
  I.handle("transcription:start", async (t, e) => {
    ...
    return qa.set(d, u), s === "whisperx-local" ? m_(u, { filePath: i, language: a }) : s === "whisper-cloud" ? h_(u, { filePath: i, language: a, apiKey: l }) : p_(u, { filePath: i, model: o, language: a }), { jobId: d };
...
const Qi = w.join(z.homedir(), "Desktop", "Coding", "ltx"), y_ = w.join(Qi, ".venv", "bin", "python"), __ = w.join(Qi, "cinegen_infer.py"), Zi = w.join(z.homedir(), "Desktop", "Coding", "qwen-edit"), w_ = w.join(Zi, ".venv", "bin", "python"), b_ = w.join(Zi, "cinegen_infer.py"), ad = w.join(z.homedir(), "Desktop", "Coding", "layer-decompose"), v_ = w.join(ad, ".venv", "bin", "python"), sd = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), E_ = w.join(sd, ".venv", "bin", "python");
function ld(...t) {
  return W.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const T_ = ld("scripts", "layer-decompose", "cinegen_infer.py"), S_ = ld("scripts", "whisperx", "cinegen_infer.py"), I_ = {
...
function yt(t, e) {
  var r;
  (r = x_()) == null || r.webContents.send("local-model:progress", { jobId: t, ...e });
}
...
function A_() {
  I.handle("local-model:run", async (t, e) => {
    ...
    if (e.nodeType === "qwen-edit-local") {
      ...
      p && (f.HF_TOKEN = p), o = ae(E_, c, {
        cwd: sd,
        stdio: ["ignore", "pipe", "pipe"],
        env: f
      });
    } else {
      const s = String(r.prompt ?? ""), l = String(r.resolution ?? "896x512"), { height: d, width: u } = I_[l] ?? { height: 512, width: 896 }, c = Number(r.frame_rate ?? 24), p = Number(r.duration_secs ?? 4), f = Math.round(p * c / 8) * 8 + 1, m = Math.max(9, f), y = Number(r.seed ?? 42), h = !!r.enhance_prompt;
      let g = null;
      if (r.image_url) {
      ...
      g && _.push("--image_path", g), h && _.push("--enhance_prompt"), o = ae(y_, _, {
        cwd: Qi,
        stdio: ["ignore", "pipe", "pipe"]
      });
    }
...
const eo = w.join(z.homedir(), "Desktop", "Coding", "Sam3"), k_ = w.join(eo, ".venv", "bin", "python"), j_ = w.join(eo, "cinegen_server.py"), R_ = 120 * 1e3, O_ = 500, P_ = 60;
class N_ {
  constructor() {
  ...
  async start() {
    var e, r;
    return this.proc && !this.proc.killed ? this.port : (this.port = await this.findFreePort(), console.log(`[sam3] Starting server on port ${this.port}`), this.proc = ae(k_, [j_, "--port", String(this.port)], {
      cwd: eo,
      stdio: ["ignore", "pipe", "pipe"],
      env: {
        ...process.env,
  ...
  async waitForHealth() {
    ...
    for (let e = 0; e < P_; e++) {
      ...
      await new Promise((r) => setTimeout(r, O_));
    }
...
async function U_(t) {
  ...
  try {
    const n = [];
    for (const i of r) {
      const o = L_(t.prompt, i.label, i.mediaType), a = i.mediaType === "video" ? await kl({
        apiKey: t.apiKey,
...
function M_() {
  I.handle("copilot:analyze-visual-refs", async (t, e) => U_(e));
}
const D_ = 1, $_ = -30, F_ = 0.3;
function B_(t) {
  const e = [];
  let r = null;
  for (const n of t.split(/\r?\n/)) {
...
function z_(t) {
  ...
  return (Array.isArray(r) ? r : r && typeof r == "object" && Array.isArray(r.segments) ? r.segments : []).flatMap((i) => {
    ...
    const o = i, a = Hn(o.start), s = Hn(o.end);
...
const dd = "gemini-2.5-flash", X_ = "fal-ai/video-understanding";
...
async function K_(t) {
  ...
  try {
    const n = await W_(t.mediaPath).catch(() => ""), i = B_(n), o = V_({ assetName: t.assetName, transcript: t.transcript }), { rawText: a, model: s } = await J_(t, o), l = z_(a);
    return {
      ...e,
      model: s,
      status: "ready",
...
function Y_() {
  I.handle("acoustic:analyze-asset", async (t, e) => K_(e));
}
const Q_ = process.platform === "darwin" && !W.isPackaged;
Q_ && (W.disableHardwareAcceleration(), W.commandLine.appendSwitch("disable-gpu-compositing"), console.log("[app] hardware acceleration disabled for macOS dev wake stability"));
...
const Z_ = Date.now(), Ma = "cinegen-desktop", Da = "CineGen", ew = ".cinegen-user-data-migrated.json", $a = "CineGen", tw = 700;
...
async function ow() {
  const { preferredUserDataPath: t, legacyUserDataPath: e } = iw;
...
function sw(t) {
  const e = w.extname(t).toLowerCase();
  return rw[e] ?? "application/octet-stream";
}
function lw(t, e) {
...
W.whenReady().then(async () => {
  if (await ow(), process.platform === "darwin") {
    const r = aw();
  ...
  Va.handle("local-media", async (r) => {
    try {
      ...
      const o = i.size, a = sw(n), s = lw(r.headers, "range");
  ...
  }), Md(), Fd(() => Z), xd(), ip(), Tc(), gp(), xm(), Rm(), Cm(), Xm(), Nh(), Wh(), Yh(), og(), kg(), Sg(), Rg(), Og(), Xg(), Yg(), Qg(), ky(), Fy(), Vy(Te), dh(), M_(), Y_(), Yy(), g_(), A_(), C_(), await cw(), I.handle("pm:open-project", async (r, n, i) => n === "__close__" ? (ee == null || ee.close(), ee = null, { ok: !0 }) : ((!Z || Z.isDestroyed()) && (Z = lo()), Z.once("ready-to-show", () => {
    Z == null || Z.maximize(), Z == null || Z.show(), Z == null || Z.webContents.send("pm:open-project", n, i);
  }), Z.webContents.getURL() !== "" && (Z.maximize(), Z.show(), Z.webContents.send("pm:open-project", n, i)), ee == null || ee.close(), ee = null, { ok: !0 })), I.handle("pm:open", async () => ee && !ee.isDestroyed() ? (ee.focus(), { ok: !0 }) : (ee = En(), ee.on("closed", () => {
    ee = null;
  }), { ok: !0 })), tr = Cd(), Z = lo();
  const t = 3e3;
  Z.once("ready-to-show", () => {
    const r = Date.now() - Z_, n = Math.max(0, t - r);
    setTimeout(() => {
      tr == null || tr.close(), tr = null, ee = En(), ee.on("closed", () => {
        ee = null;
      });
    }, n);
  }), W.on("activate", () => {
    Q.getAllWindows().length === 0 && (ee = En(), ee.on("closed", () => {
      ee = null;
    }));
  });
  const e = (r) => {
  ...
  _n.on("resume", () => {
    ...
    Gn("unlock-screen"), e("unlock-screen");
  }), _n.on("suspend", () => {
    Gn("suspend");
  });
});
W.on("before-quit", () => {
  Bd(), Be && (clearTimeout(Be), Be = null), By(), jy(), q_();
});
