  ...
  const appendParam = (name2, value) => {
    ...
    if (typeof value === "string") {
      ...
      try {
        ...
        if (json === void 0) throw new Error("not JSON serializable");
        serialized = json;
      } catch (error) {
...
function listRowsFromParsed(parsed) {
  if (Array.isArray(parsed)) return parsed.filter((entry) => isRecord(entry));
  if (!isRecord(parsed)) return [];
  for (const key of ["jobs", "results", "data", "items", "generations"]) {
    const rows = parsed[key];
    if (Array.isArray(rows)) return rows.filter((entry) => isRecord(entry));
  }
...
function registerWorkflowHandlers() {
  ipcMain.handle("workflow:run", async (_event, params) => {
    ...
    const { ALL_MODELS, resolveVideoModelEndpoint, sanitizeVideoInputsForEndpoint } = await import("./models-CiFR2oEL.js");
