...
const FAST_SYNC_ACCEPT_CONFIDENCE = 0.48;
...
async function computeSampledWaveformSyncOffset(sourceVideoPath, targetAudioPath, ffmpegPath, ffprobePath, fpcalcPath, tempFiles) {
  ...
  console.log(
    `[audio-sync] Sampled fingerprint pass: ${sourceWindows.length} source windows x ${targetWindows.length} target windows`
  );
  const [sourceFingerprintWindows, targetFingerprintWindows] = await Promise.all([
    extractFingerprintWindows(sourceVideoPath, ffmpegPath, fpcalcPath, sourceWindows, tempFiles),
    extractFingerprintWindows(targetAudioPath, ffmpegPath, fpcalcPath, targetWindows, tempFiles)
  ]);
  const votes = [];
  for (const sourceWindow of sourceFingerprintWindows) {
    for (const targetWindow of targetFingerprintWindows) {
      const correlation = crossCorrelateFingerprints(
        sourceWindow.fingerprint,
        targetWindow.fingerprint,
        Math.max(sourceWindow.durationSeconds, targetWindow.durationSeconds)
      );
