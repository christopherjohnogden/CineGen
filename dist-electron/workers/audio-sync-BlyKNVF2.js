...
function crossCorrelatePcm(source, target, sampleRate, maxOffsetSeconds = 300) {
  ...
  const coarseStepSamples = Math.floor(sampleRate * 0.25);
  const fineStepSamples = Math.floor(sampleRate * 0.01);
  ...
  for (let a = 0; a < numAnchors; a++) {
    ...
    const computeNCC = (pos) => {
      ...
      return covar / Math.sqrt(anchorVar * tVar);
    };
    for (let pos = searchStart; pos <= searchEnd; pos += coarseStepSamples) {
    ...
    const fineStart = Math.max(searchStart, bestPos - coarseStepSamples);
    const fineEnd = Math.min(searchEnd, bestPos + coarseStepSamples);
    for (let pos = fineStart; pos <= fineEnd; pos += fineStepSamples) {
      const ncc = computeNCC(pos);
      if (ncc > bestNCC) {
        bestNCC = ncc;
        bestPos = pos;
  ...
  console.log("[audio-sync] Anchor votes:", votes.map(
    (v) => `offset=${v.offsetSeconds.toFixed(2)}s ncc=${v.ncc.toFixed(3)}`
  ).join(", "));
  if (votes.length === 0) {
  ...
  console.log(`[audio-sync] Result: offset=${weightedOffset.toFixed(3)}s confidence=${avgConfidence.toFixed(3)} (${bestGroup.length} anchors agreed)`);
  return {
