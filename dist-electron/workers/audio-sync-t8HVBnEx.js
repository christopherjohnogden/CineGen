...
function crossCorrelatePcm(source, target, sampleRate, maxOffsetSeconds = 120) {
  ...
  for (let shift = -maxShift; shift <= maxShift; shift++) {
    let sumSrc = 0, sumTgt = 0;
    let sumSrc2 = 0, sumTgt2 = 0;
    let sumProduct = 0;
    let n = 0;
    for (let i = 0; i < sourceEnergy.length; i++) {
      const j = i + shift;
      if (j < 0 || j >= targetEnergy.length) continue;
      const s = sourceEnergy[i];
      const t = targetEnergy[j];
      sumSrc += s;
      sumTgt += t;
      sumSrc2 += s * s;
      sumTgt2 += t * t;
    ...
    if (n < 20) continue;
    const meanSrc = sumSrc / n;
    const meanTgt = sumTgt / n;
    const varSrc = sumSrc2 / n - meanSrc * meanSrc;
    const varTgt = sumTgt2 / n - meanTgt * meanTgt;
    const covariance = sumProduct / n - meanSrc * meanTgt;
    const denom = Math.sqrt(varSrc * varTgt);
    if (denom < 1e-10) continue;
    const ncc = covariance / denom;
    if (ncc > bestNCC) {
      bestNCC = ncc;
      bestOffset = shift;
    }
  ...
  const confidence = Math.min(1, Math.max(0, (bestNCC + 1) / 2));
  return {
    offsetIndex: bestOffset,
    confidence,
