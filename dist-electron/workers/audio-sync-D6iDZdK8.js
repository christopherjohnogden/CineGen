...
function crossCorrelatePcm(source, target, sampleRate) {
  ...
  for (let a = 0; a < numAnchors; a++) {
    ...
    if (bestNCC > 0.15) {
      ...
      votes.push({ offsetSeconds: rawOffset, ncc: bestNCC, anchorIdx: a });
    }
  }
  console.log("[audio-sync] Anchor votes:", votes.map(
    (v) => `a${v.anchorIdx}: offset=${v.offsetSeconds.toFixed(2)}s ncc=${v.ncc.toFixed(3)}`
  ).join(" | "));
  if (votes.length === 0) {
    console.log("[audio-sync] No valid anchor matches found");
    return { offsetIndex: 0, confidence: 0, _offsetSeconds: 0 };
  }
  ...
  const totalW = bestGroup.reduce((s, v) => s + v.ncc, 0);
  const weightedOffset = bestGroup.reduce((s, v) => s + v.offsetSeconds * v.ncc, 0) / totalW;
  const avgConfidence = totalW / bestGroup.length;
  const finalOffset = flipped ? weightedOffset : -weightedOffset;
  console.log(`[audio-sync] Result: offset=${finalOffset.toFixed(3)}s confidence=${avgConfidence.toFixed(3)} (${bestGroup.length}/${votes.length} anchors agreed)${flipped ? " [flipped]" : ""}`);
  return {
    offsetIndex: 0,
    confidence: Math.min(1, avgConfidence),
    _offsetSeconds: finalOffset
  };
}
