...
const FAST_SYNC_WINDOW_SECONDS = 45;
const FAST_SYNC_MIN_PAIR_CONFIDENCE = 0.3;
...
function crossCorrelatePcm(source, target, sampleRate) {
  ...
  const numAnchors = Math.min(6, Math.max(2, Math.floor(usableLen / anchorSamples)));
  if (numAnchors === 0 || longer.length < anchorSamples) {
    return { offsetIndex: 0, confidence: 0, _offsetSeconds: 0 };
  }
...
async function extractPcmWindows(inputPath, ffmpegPath, windows, tempFiles) {
  return Promise.all(
    windows.map(async (window) => {
      const pcmPath = await extractAudioToTempPcm(inputPath, ffmpegPath, {
        startSeconds: window.startSeconds,
        durationSeconds: window.durationSeconds
      });
      tempFiles.push(pcmPath);
      const pcm = await readPcmAsFloat32(pcmPath);
      return { ...window, pcm };
    })
  );
}
async function computeSampledWaveformSyncOffset(sourceVideoPath, targetAudioPath, ffmpegPath, ffprobePath, tempFiles) {
  ...
  const sourceWindows = buildAnalysisWindows(sourceDuration);
  const targetWindows = buildAnalysisWindows(targetDuration);
  if (sourceWindows.length === 0 || targetWindows.length === 0) {
    console.log("[audio-sync] Sampled pass skipped: no analysis windows");
    return null;
  }
  console.log(
    `[audio-sync] Sampled pass: ${sourceWindows.length} source windows x ${targetWindows.length} target windows`
  );
  const [sourcePcmWindows, targetPcmWindows] = await Promise.all([
    extractPcmWindows(sourceVideoPath, ffmpegPath, sourceWindows, tempFiles),
    extractPcmWindows(targetAudioPath, ffmpegPath, targetWindows, tempFiles)
  ]);
  const votes = [];
  for (const sourceWindow of sourcePcmWindows) {
    for (const targetWindow of targetPcmWindows) {
      const correlation = crossCorrelatePcm(sourceWindow.pcm, targetWindow.pcm, SYNC_SAMPLE_RATE);
      if (correlation.confidence < FAST_SYNC_MIN_PAIR_CONFIDENCE) continue;
      votes.push({
        offsetSeconds: correlation._offsetSeconds + sourceWindow.startSeconds - targetWindow.startSeconds,
        confidence: correlation.confidence,
        label: `${sourceWindow.label}->${targetWindow.label}`
      });
    }
  }
  return summarizeOffsetVotes(votes, "Sampled");
}
