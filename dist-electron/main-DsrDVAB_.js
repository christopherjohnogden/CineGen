...
var Ad = (t, e, r) => e in t ? xd(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : t[e] = r;
var M = (t, e, r) => Ad(t, typeof e != "symbol" ? e + "" : e, r);
import { ipcMain as I, app as z, shell as Kr, BrowserWindow as Z, screen as Ja, safeStorage as Ar, net as kd, nativeImage as Ka, session as Ya, dialog as so, protocol as Qa, powerMonitor as bn } from "electron";
import $, { existsSync as ot, readFileSync as _r, mkdirSync as Yr, writeFileSync as Ti, copyFileSync as jd, chmodSync as Rd, renameSync as Pd, rmSync as Za } from "node:fs";
import w, { dirname as Od, join as Et } from "node:path";
import H, { homedir as es } from "node:os";
import { execFile as qe, spawn as ae } from "node:child_process";
import { promisify as Ut } from "node:util";
import X, { randomUUID as wr, randomBytes as Nd } from "node:crypto";
import j, { writeFile as ts, chmod as Cd, mkdir as st, mkdtemp as qd, rm as Ld } from "node:fs/promises";
import { Readable as lo } from "node:stream";
import Ei from "better-sqlite3";
import { createServer as rs } from "node:http";
import { lookup as Ud } from "node:dns/promises";
import { request as Md } from "node:https";
import Dd, { isIP as Si } from "node:net";
import { createRequire as ns } from "node:module";
import { fileURLToPath as Qr } from "node:url";
import { Worker as is } from "worker_threads";
function br(t) {
...
function vt() {
  return {
    config: Et(es(), "Library", "Application Support", "Claude", "claude_desktop_config.json"),
    source: Et(z.getAppPath(), "dist-electron", "cinegen-mcp.cjs"),
    installed: Et(z.getPath("userData"), "mcp", "cinegen-mcp.cjs")
  };
}
function as() {
...
async function Bd(t) {
  ...
  try {
    await Fd(process.execPath, ["-e", e, t], {
...
function Tn() {
  const t = vt(), e = Ii(t.config).config, r = os(e), n = ot(t.source) && ot(t.installed) && _r(t.source).equals(_r(t.installed));
  return {
    supported: process.platform === "darwin",
    configured: !!r,
    needsRepair: !!r && (!$d(r, as()) || !n),
    serverAvailable: ot(t.source),
    configPath: t.config
  };
}
function Vd() {
  I.handle("claude-mcp:status", () => Tn()), I.handle("claude-mcp:setup", async () => {
    vn();
    const t = vt();
    if (Ii(t.config), !ot(t.source)) throw new Error("This build is missing the MCP server. Install the latest CineGen Mac build.");
    await Bd(t.source), Yr(Et(z.getPath("userData"), "mcp"), { recursive: !0 }), Ti(t.installed, _r(t.source), { mode: 384 });
    const e = uo(t.config, as());
    return { ...Tn(), ...e };
    ...
    if (vn(), !ot(vt().config)) throw new Error("Connect Claude Desktop first to create its configuration.");
    Kr.showItemInFolder(vt().config);
  });
}
function Gd() {
  if (z.isPackaged) return;
  const t = w.join(z.getPath("userData"), "dev-browser-session");
  Yr(t, { recursive: !0 }), z.setPath("sessionData", t);
}
const zd = 1200, Hd = 150, Wd = 1e3, co = 2800, ss = /* @__PURE__ */ new WeakMap(), ls = /* @__PURE__ */ new WeakMap(), $r = /* @__PURE__ */ new WeakMap(), ds = /* @__PURE__ */ new WeakMap();
function Xd() {
...
const Rt = Xd(), Zr = w.join(import.meta.dirname, "."), us = w.join(Zr, "../dist"), vr = process.env.VITE_DEV_SERVER_URL;
function fo(t) {
  return vr ? t.loadURL(`${vr}?pm=1`) : t.loadFile(w.join(us, "index.html"), { query: { pm: "1" } });
}
function po(t) {
  return vr ? t.loadURL(vr) : t.loadFile(w.join(us, "index.html"));
}
function En(t, e) {
...
async function Kd(t, e, r) {
  if (!t.isDestroyed())
    try {
      ...
      await Yn(t, e, r, "blank renderer DOM after resume");
    } catch (n) {
      console.warn(`[window] ${e} health check failed after wake:`, n), await Yn(t, e, r, "resume health check failed");
    }
}
function Yd(t) {
  for (const e of Z.getAllWindows()) {
    ...
    const r = ss.get(e);
    if (!r) continue;
    const n = ls.get(e) ?? "window";
    cs(e), ds.set(e, Date.now() + co + 1e3);
    let i = null;
    const o = setTimeout(() => {
    ...
    const a = setTimeout(() => {
      kr(e, a), (async () => {
        try {
          await Kd(e, n, r), i && (clearTimeout(i), kr(e, i), i = null);
        } catch (s) {
    ...
    En(e, a), i = setTimeout(() => {
      kr(e, i), !e.isDestroyed() && Yn(e, n, r, `hard reload after ${t}`).catch((s) => {
...
function Sn() {
  const { width: t, height: e } = Ja.getPrimaryDisplay().workAreaSize, r = 900, n = 580, i = new Z({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
    ...
    ...Rt ? { icon: Rt } : {},
    webPreferences: {
      preload: w.join(Zr, "preload.js"),
      nodeIntegration: !1,
      contextIsolation: !0,
      backgroundThrottling: !1
    }
  });
  return fs(i, "project-manager", fo), fo(i), i;
}
function Qd() {
  const { width: t, height: e } = Ja.getPrimaryDisplay().workAreaSize, r = 800, n = 395, i = new Z({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
  ...
  return i.loadFile(w.join(Zr, "splash.html")), i;
}
function mo() {
  const t = new Z({
    width: 1440,
  ...
  return fs(t, "main", po), po(t), vr && t.webContents.openDevTools({ mode: "detach" }), t;
}
function xi() {
  return w.join(H.homedir(), "Documents", "CINEGEN");
}
function Qn() {
...
async function hs() {
  await j.mkdir(xi(), { recursive: !0 });
}
async function jr() {
  try {
    const t = await j.readFile(Qn(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function In(t) {
  await hs();
  const e = Qn() + ".tmp";
  await j.writeFile(e, JSON.stringify(t, null, 2), "utf-8"), await j.rename(e, Qn());
}
function Zd(t, e) {
  const r = ms(), n = {
    id: ps(),
...
function eu(t) {
  ...
  try {
    const r = $.readFileSync(e, "utf-8"), i = (JSON.parse(r).assets ?? []).find(
...
function tu(t) {
  ...
  if (!$.existsSync(e)) return null;
  try {
    const r = new Ei(e, { readonly: !0 }), n = r.prepare(
...
function ru() {
  I.handle("project:list", async () => (await jr()).projects.map((e) => {
    const r = e.useSqlite ? tu(e.id) : eu(e.id);
    ...
    const n = ps(), i = Zd(n, r);
    await hs(), await j.mkdir(Tr(n), { recursive: !0 });
    const o = ct(n) + ".tmp";
    await j.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await j.rename(o, ct(n));
    const a = await jr();
    ...
    const r = await j.readFile(ct(e), "utf-8");
    return JSON.parse(r);
  }), I.handle("project:save", async (t, e, r) => {
    let n;
    try {
      const l = await j.readFile(ct(e), "utf-8");
      n = JSON.parse(l);
    } catch {
    ...
    await j.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await j.rename(o, ct(e));
    const a = await jr(), s = a.projects.find((l) => l.id === e);
    return s && (s.updatedAt = i.project.updatedAt, s.assetCount = Array.isArray(i.assets) ? i.assets.length : 0, s.elementCount = Array.isArray(i.elements) ? i.elements.length : 0, r.project && r.project.name && (s.name = r.project.name), await In(a)), i;
  }), I.handle("project:delete", async (t, e) => {
    await j.rm(Tr(e), { recursive: !0, force: !0 });
    const r = await jr();
    r.projects = r.projects.filter((n) => n.id !== e), await In(r);
  });
}
const ho = 12e4, gs = Et(es(), "Documents", "CINEGEN"), Zn = Et(gs, "mcp-bridge.json"), Pt = /* @__PURE__ */ new Map();
let qr = !1, he = null, Rr = "";
function Re(t, e, r) {
...
function iu(t, e, r) {
  return new Promise((n, i) => {
    const o = wr(), a = setTimeout(() => {
      Pt.delete(o), i(new Error(`CineGen did not answer "${e}" within ${Math.round(ho / 1e3)}s.`));
    }, ho);
...
function ou(t) {
  ...
  }), Rr = Nd(24).toString("hex"), he = rs((e, r) => {
    (async () => {
      try {
        ...
        if (e.method === "POST" && n.pathname === "/invoke") {
          const o = await nu(e), a = typeof (o == null ? void 0 : o.tool) == "string" ? o.tool : "";
          if (!a) {
          ...
          try {
            Re(r, 200, { ok: !0, result: await iu(s, a, o.args ?? {}) });
          } catch (l) {
...
function au() {
  ...
  Pt.clear(), he == null || he.close(), he = null;
  try {
    Za(Zn, { force: !0 });
  } catch {
  }
...
function dt() {
  if (_o) return Ee;
  _o = 1;
  var t = Ee && Ee.__awaiter || function(s, l, d, u) {
    function c(p) {
...
function Ke() {
  ...
  Object.defineProperty(pe, "__esModule", { value: !0 }), pe.ensureEndpointIdFormat = e, pe.parseEndpointId = n, pe.resolveEndpointPath = i, pe.isValidUrl = o, pe.throttle = a, pe.isReact = l, pe.isPlainObject = d, pe.sleep = u;
  function e(c) {
    if (c.split("/").length > 1)
      return c;
...
function en() {
  return bo || (bo = 1, (function(t) {
    var e = Ht && Ht.__awaiter || function(s, l, d, u) {
      function c(p) {
        return p instanceof d ? p : new d(function(f) {
          f(p);
        });
...
var Wt = {};
const lu = "@fal-ai/client", du = "1.9.4", uu = {
  name: lu,
  version: du
};
var vo;
...
function Sr() {
  if (Eo) return Oe;
  Eo = 1;
  var t = Oe && Oe.__awaiter || function(l, d, u, c) {
    function p(f) {
  ...
  const r = en(), n = ki(), i = Ke(), o = typeof navigator < "u" && (navigator == null ? void 0 : navigator.userAgent) === "Cloudflare-Workers";
  function a(l) {
    return t(this, void 0, void 0, function* () {
      var d;
      const { targetUrl: u, input: c, config: p, options: f = {} } = l, { credentials: m, requestMiddleware: y, responseHandler: h, fetch: g } = p, _ = Object.assign(Object.assign({}, p.retry), f.retry || {}), b = () => t(this, void 0, void 0, function* () {
        var T, S, x;
        const E = (0, n.isBrowser)() ? {} : { "User-Agent": (0, n.getUserAgent)() }, k = typeof m == "function" ? m() : m, { method: O, url: q, headers: D } = yield y({
          method: ((S = (T = l.method) !== null && T !== void 0 ? T : f.method) !== null && S !== void 0 ? S : "post").toUpperCase(),
          url: u,
          headers: l.headers
        }), G = k ? { Authorization: `Key ${k}` } : {}, K = Object.assign(Object.assign(Object.assign(Object.assign({}, G), { Accept: "application/json", "Content-Type": "application/json" }), E), D ?? {}), { responseHandler: W, retry: R } = f, U = e(f, ["responseHandler", "retry"]), N = yield g(q, Object.assign(Object.assign(Object.assign(Object.assign({}, U), { method: O, headers: Object.assign(Object.assign({}, K), (x = U.headers) !== null && x !== void 0 ? x : {}) }), !o && { mode: "cors" }), { signal: f.signal, body: O.toLowerCase() !== "get" && c ? JSON.stringify(c) : void 0 }));
        return yield (W ?? h)(N);
      });
      let v;
  ...
  function s(l, d = {}) {
    var u, c;
    const p = ((u = d.method) !== null && u !== void 0 ? u : "post").toLowerCase(), f = ((c = d.path) !== null && c !== void 0 ? c : "").replace(/^\//, "").replace(/\/{2,}/, "/"), m = d.input, y = Object.assign(Object.assign({}, d.query || {}), p === "get" ? m : {}), h = Object.keys(y).length > 0 ? `?${new URLSearchParams(y).toString()}` : "";
    if ((0, i.isValidUrl)(l))
      return `${l.endsWith("/") ? l : `${l}/`}${f}${h}`;
    const g = (0, i.ensureEndpointIdFormat)(l);
    return `${`https://${d.subdomain ? `${d.subdomain}.` : ""}fal.run/${g}/${f}`.replace(/\/$/, "")}${h}`;
  }
  return Oe;
}
var Xt = {}, So;
function _s() {
  return So || (So = 1, (function(t) {
    var e = Xt && Xt.__awaiter || function(m, y, h, g) {
      function _(b) {
    ...
    function p(m, y, h) {
      return e(this, void 0, void 0, function* () {
        const { fetch: g, responseHandler: _ } = y, b = m.type || "application/octet-stream", { upload_url: v, file_url: T } = yield u(m, y, b, h), S = 10 * 1024 * 1024, x = Math.ceil(m.size / S), E = new URL(v), k = [];
        for (let D = 0; D < x; D++) {
          const G = D * S, K = Math.min(G + S, m.size), W = m.slice(G, K), R = D + 1, U = `${E.origin}${E.pathname}/${R}${E.search}`;
          k.push(yield c(U, W, y));
        }
        const O = `${E.origin}${E.pathname}/complete${E.search}`, q = yield g(O, {
          method: "POST",
          headers: {
    ...
    function f({ config: m }) {
      const y = {
        upload: (h, g) => e(this, void 0, void 0, function* () {
          ...
          const b = h.type || "application/octet-stream", { fetch: v, responseHandler: T } = m, { upload_url: S, file_url: x } = yield d(h, m, b, _), E = yield v(S, {
            method: "PUT",
            body: h,
            headers: {
        ...
        transformInput: (h) => e(this, void 0, void 0, function* () {
          ...
          if ((0, i.isPlainObject)(h)) {
            const _ = Object.entries(h).map((v) => e(this, [v], void 0, function* ([T, S]) {
...
` && (_ = T - h);
    ...
    function f(m, y, h, g) {
      ...
      const T = y + v, S = g - v, x = m.slice(T, T + S).toString();
      if (b === "data")
        u += x ? "".concat(x, `
`) : `
`;
  ...
  return xo || (xo = 1, (function(t) {
    var e = Jt && Jt.__awaiter || function(a, s, l, d) {
      ...
      return new (l || (l = Promise))(function(c, p) {
        ...
        function y(h) {
          h.done ? c(h.value) : u(h.value).then(f, m);
        }
...
function bs() {
  ...
  var t = _e && _e.__awaiter || function(p, f, m, y) {
    ...
    return new (m || (m = Promise))(function(g, _) {
      ...
      function T(S) {
        S.done ? g(S.value) : h(S.value).then(b, v);
      }
    ...
    function T(k) {
      k.value instanceof e ? Promise.resolve(k.value.v).then(S, x) : E(g[0][2], k);
    }
    function S(k) {
      v("next", k);
    }
    ...
    function E(k, O) {
      k(O), g.shift(), g.length && v(g[0][0], g[0][1]);
    }
  };
  Object.defineProperty(_e, "__esModule", { value: !0 }), _e.FalStream = void 0, _e.createStreamingClient = c;
  const n = /* @__PURE__ */ cu(), i = ws(), o = Sr(), a = dt(), s = Ke(), l = "text/event-stream", d = 15 * 1e3;
  class u {
    constructor(f, m, y) {
      var h;
      this.listeners = /* @__PURE__ */ new Map(), this.buffer = [], this.currentData = void 0, this.lastEventTimestamp = 0, this.streamClosed = !1, this._requestId = null, this.abortController = new AbortController(), this.start = () => t(this, void 0, void 0, function* () {
        ...
        try {
          if (E === "client") {
            const O = (0, s.ensureEndpointIdFormat)(v), q = (g = (0, s.resolveEndpointPath)(v, void 0, "/stream")) !== null && g !== void 0 ? g : "", G = yield (k ? () => k(`${O}${q}`) : () => (console.warn('[fal.stream] Using the default token provider is deprecated. Please provide a `tokenProvider` function when using `connectionMode: "client"`. See https://docs.fal.ai/fal-client/authentication for more information.'), (0, i.getTemporaryAuthToken)(v, this.config)))(), { fetch: K } = this.config, W = new URL(this.url);
            W.searchParams.set("fal_jwt_token", G);
            const R = yield K(W.toString(), {
        ...
        if (!((_ = g.headers.get("content-type")) !== null && _ !== void 0 ? _ : "").startsWith(l)) {
          const q = v.getReader(), D = () => {
            q.read().then(({ done: G, value: K }) => {
              if (G) {
                this.emit("done", this.currentData);
                return;
              }
              this.buffer.push(K), this.currentData = K, this.emit("data", K), D();
            });
          };
          D();
          return;
        }
        const S = new TextDecoder("utf-8"), x = g.body.getReader(), E = (0, n.createParser)((q) => {
          if (q.type === "event") {
            const D = q.data;
            try {
              const G = JSON.parse(D);
              this.buffer.push(G), this.currentData = G, this.emit("data", G), this.emit("message", G);
            } catch (G) {
              this.emit("error", G);
            }
          }
        }), k = (b = this.options.timeout) !== null && b !== void 0 ? b : d, O = () => t(this, void 0, void 0, function* () {
          const { value: q, done: D } = yield x.read();
          this.lastEventTimestamp = Date.now(), E.feed(S.decode(q)), Date.now() - this.lastEventTimestamp > k && this.emit("error", new a.ApiError({
            message: `Event stream timed out after ${(k / 1e3).toFixed(0)} seconds with no messages.`,
            status: 408,
            requestId: this._requestId || void 0
          })), D ? this.emit("done", this.currentData) : O().catch(this.handleError);
        });
    ...
    return {
      stream(m, y) {
        return t(this, void 0, void 0, function* () {
          ...
          return new u(m, p, Object.assign(Object.assign({}, y), { input: h }));
        });
      }
    };
  }
...
function fu() {
  ...
  const r = Ai(), n = Sr(), i = dt(), o = en(), a = _s(), s = bs(), l = Ke(), d = 500, u = {
    maxRetries: 3,
    baseDelay: 1e3,
    maxDelay: 6e4,
    ...
    const y = {
      submit(h, g) {
        return t(this, void 0, void 0, function* () {
          const { webhookUrl: _, priority: b, hint: v, startTimeout: T, headers: S, storageSettings: x } = g, E = e(g, ["webhookUrl", "priority", "hint", "startTimeout", "headers", "storageSettings"]), k = g.input ? yield m.transformInput(g.input) : void 0, O = Object.fromEntries(Object.entries(S ?? {}).map(([q, D]) => [
            q.toLowerCase(),
            D
          ]));
      ...
      subscribeToStatus(h, g) {
        return t(this, void 0, void 0, function* () {
          ...
          if (g.mode === "streaming") {
            const S = yield y.streamStatus(h, {
              requestId: _,
              logs: g.logs,
              connectionMode: "connectionMode" in g ? g.connectionMode : void 0
            }), x = [];
            b && (v = setTimeout(() => {
              throw S.abort(), y.cancel(h, { requestId: _ }).catch(T), new Error(`Client timed out waiting for the request to complete after ${b}ms`);
            }, b)), S.on("data", (k) => {
              g.onQueueUpdate && ("logs" in k && Array.isArray(k.logs) && k.logs.length > 0 && x.push(...k.logs), g.onQueueUpdate("logs" in k ? Object.assign(Object.assign({}, k), { logs: x }) : k));
            });
          ...
          return new Promise((S, x) => {
            var E;
            let k;
            const O = "pollInterval" in g && typeof g.pollInterval == "number" && (E = g.pollInterval) !== null && E !== void 0 ? E : d, q = () => {
              v && clearTimeout(v), k && clearTimeout(k);
            };
            b && (v = setTimeout(() => {
              q(), y.cancel(h, { requestId: _ }).catch(T), x(new Error(`Client timed out waiting for the request to complete after ${b}ms`));
            }, b));
            const D = () => t(this, void 0, void 0, function* () {
              var G;
              try {
                const K = yield y.status(h, {
                  requestId: _,
                  logs: (G = g.logs) !== null && G !== void 0 ? G : !1,
                  abortSignal: g.abortSignal
                });
                if (g.onQueueUpdate && g.onQueueUpdate(K), K.status === "COMPLETED") {
                  q(), S(K);
                  return;
                }
                k = setTimeout(D, O);
...
const hu = new TextEncoder(), gu = 50;
function yu(t, e, r) {
  hu.encodeInto(t, e.subarray(r));
}
function _u(t, e, r) {
  t.length > gu ? yu(t, e, r) : mu(t, e, r);
}
const wu = 4096;
function vs(t, e, r) {
  let n = e;
...
const bu = new TextDecoder(), vu = 200;
function Tu(t, e, r) {
  const n = t.subarray(e, e + r);
  return bu.decode(n);
}
function Eu(t, e, r) {
  return r > vu ? Tu(t, e, r) : vs(t, e, r);
}
class nr {
  constructor(e, r) {
    M(this, "type");
...
const Ss = -1, xu = 4294967296 - 1, Au = 17179869184 - 1;
function Is({ sec: t, nsec: e }) {
  if (t >= 0 && e >= 0 && t <= Au)
    if (e === 0 && t <= xu) {
    ...
    return n.setUint32(0, e), Ts(n, 4, t), r;
  }
}
function xs(t) {
...
function ks(t) {
  ...
  switch (t.byteLength) {
    ...
    case 12: {
      const r = Es(e, 4), n = e.getUint32(0);
      return { sec: r, nsec: n };
    }
    default:
...
function ei(t) {
  return t instanceof Uint8Array ? t : ArrayBuffer.isView(t) ? new Uint8Array(t.buffer, t.byteOffset, t.byteLength) : ju(t) ? new Uint8Array(t) : Uint8Array.from(t);
}
const Ru = 100, Pu = 2048;
class tn {
  constructor(e) {
    ...
    M(this, "bytes");
    M(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? Er.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.maxDepth = (e == null ? void 0 : e.maxDepth) ?? Ru, this.initialBufferSize = (e == null ? void 0 : e.initialBufferSize) ?? Pu, this.sortKeys = (e == null ? void 0 : e.sortKeys) ?? !1, this.forceFloat32 = (e == null ? void 0 : e.forceFloat32) ?? !1, this.ignoreUndefined = (e == null ? void 0 : e.ignoreUndefined) ?? !1, this.forceIntegerToFloat = (e == null ? void 0 : e.forceIntegerToFloat) ?? !1, this.pos = 0, this.view = new DataView(new ArrayBuffer(this.initialBufferSize)), this.bytes = new Uint8Array(this.view.buffer);
  }
  clone() {
    return new tn({
  ...
  encodeString(e) {
    const n = pu(e);
    this.ensureBufferSizeToWrite(5 + n), this.writeStringHeader(n), _u(e, this.bytes, this.pos), this.pos += n;
  }
  encodeObject(e, r) {
  ...
  writeU64(e) {
    this.ensureBufferSizeToWrite(8), Su(this.view, this.pos, e), this.pos += 8;
  }
  writeI64(e) {
...
function Ou(t, e) {
  return new tn(e).encodeSharedRef(t);
}
function kn(t) {
...
class qu {
  ...
  decode(e, r, n) {
    ...
    if (i != null)
      return this.hit++, i;
    this.miss++;
    const o = vs(e, r, n), a = Uint8Array.prototype.slice.call(e, r, r + n);
    return this.store(a, o), o;
  }
}
const ti = "array", sr = "map_key", Rs = "map_value", Lu = (t) => {
...
class Uu {
  ...
  pushArrayState(e) {
    ...
    r.type = ti, r.position = 0, r.size = e, r.array = new Array(e);
  }
  pushMapState(e) {
    const r = this.getUninitializedStateFromPool();
...
const Yt = -1, Ri = new DataView(new ArrayBuffer(0)), Mu = new Uint8Array(Ri.buffer);
try {
  Ri.getInt8(0);
} catch (t) {
...
const jo = new RangeError("Insufficient data"), Du = new qu();
class Ye {
  constructor(e) {
    M(this, "extensionCodec");
    M(this, "context");
    ...
    M(this, "stack", new Uu());
    M(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? Er.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.rawStrings = (e == null ? void 0 : e.rawStrings) ?? !1, this.maxStrLength = (e == null ? void 0 : e.maxStrLength) ?? Kt, this.maxBinLength = (e == null ? void 0 : e.maxBinLength) ?? Kt, this.maxArrayLength = (e == null ? void 0 : e.maxArrayLength) ?? Kt, this.maxMapLength = (e == null ? void 0 : e.maxMapLength) ?? Kt, this.maxExtLength = (e == null ? void 0 : e.maxExtLength) ?? Kt, this.keyDecoder = (e == null ? void 0 : e.keyDecoder) !== void 0 ? e.keyDecoder : Du, this.mapKeyConverter = (e == null ? void 0 : e.mapKeyConverter) ?? Lu;
  }
  clone() {
    return new Ye({
      extensionCodec: this.extensionCodec,
  ...
  appendBuffer(e) {
    if (this.headByte === Yt && !this.hasRemaining(1))
      ...
      const r = this.bytes.subarray(this.pos), n = ei(e), i = new Uint8Array(r.length + n.length);
  ...
  *decodeMulti(e) {
  ...
  async decodeAsync(e) {
    ...
    try {
      ...
      throw new RangeError(`Insufficient data in parsing ${kn(i)} at ${a} (${o} in the current buffer)`);
    } finally {
      this.entered = !1;
    }
  }
  ...
  doDecodeSync() {
    e: for (; ; ) {
      ...
      if (e >= 224)
        ...
        throw new de(`Unrecognized type byte: ${kn(e)}`);
      this.complete();
      const n = this.stack;
      for (; n.length > 0; ) {
        const i = n.top();
        if (i.type === ti)
          ...
          i.key = this.mapKeyConverter(r), i.type = Rs;
          continue e;
  ...
  readArraySize() {
    ...
    switch (e) {
      ...
      default: {
        if (e < 160)
          return e - 144;
        throw new de(`Unrecognized array type byte: ${kn(e)}`);
      }
    }
  }
  pushMapState(e) {
  ...
  decodeUtf8String(e, r) {
    ...
    const n = this.pos + r;
    let i;
    return this.stateIsMapKey() && ((o = this.keyDecoder) != null && o.canBeCached(e)) ? i = this.keyDecoder.decode(this.bytes, n, e) : i = Eu(this.bytes, n, e), this.pos += r + e, i;
  }
  stateIsMapKey() {
...
const Wu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  ExtensionCodec: Er,
  decode: $u,
  decodeArrayStream: zu,
  decodeAsync: Gu,
  decodeMulti: Fu,
  decodeMultiStream: Hu,
  decodeTimestampExtension: js,
  decodeTimestampToTimeSpec: ks,
  encode: Ou,
  encodeDateToTimeSpec: xs,
  encodeTimeSpecToTimestamp: Is,
  encodeTimestampExtension: As
}, Symbol.toStringTag, { value: "Module" })), Xu = /* @__PURE__ */ su(Wu);
var me = {}, Ro;
function Ju() {
  if (Ro) return me;
  ...
  let r = {}, n = () => !0, i = () => ({}), o = (P) => P, a = (P, A, L, C) => P.apply(L, C) && A.apply(L, C), s = (P, A, L, [C, V]) => A.call(L, P.call(L, C, V), V), l = (P, A) => Object.freeze(Object.create(P, A));
  function d(P, A, L) {
  ...
  let c = {}, p = u.bind(c), f = (P) => p((A, L) => !!~P(A, L) && A), m = {}, y = u.bind(m);
  function h(P, A) {
    return A.filter((L) => P.isPrototypeOf(L));
  }
  function g(P, A, ...L) {
    let C = d(h(m, L).map((Y) => Y.fn), n, a), V = d(h(c, L).map((Y) => Y.fn), o, s);
    return l(this, {
      from: t(P),
      to: t(A),
      guards: t(C),
  ...
  function S(P, A, L) {
    return W(A, P, L, this.immediates) || P;
  }
  function x(P) {
    let A = /* @__PURE__ */ new Map();
    for (let L of P)
      A.has(L.from) || A.set(L.from, []), A.get(L.from).push(L);
    return A;
  }
  let E = { enter: o };
  function k(...P) {
    let A = h(_, P), L = h(b, P), C = {
      final: t(P.length === 0),
      transitions: t(x(A))
    };
    return L.length && (C.immediates = t(L), C.enter = t(S)), l(E, C);
  }
  let O = {
    enter(P, A, L) {
      let C = this.fn.call(A, A.context, L);
      return G.isPrototypeOf(C) ? l(q, {
        machine: t(C),
        transitions: t(this.transitions)
      }).enter(P, A, L) : (C.then((V) => A.send({ type: "done", data: V })).catch((V) => A.send({ type: "error", error: V })), P);
    }
  }, q = {
    enter(P, A, L) {
      if (A.child = N(this.machine, (C) => {
        A.onChange(C), A.child == C && C.machine.state.value.final && (delete A.child, A.send({ type: "done", data: C.context }));
      }, A.context, L), A.child.machine.state.value.final) {
        let C = A.child.context;
        return delete A.child, W(A, P, { type: "done", data: C }, this.transitions.get("done"));
      }
      return P;
    }
  };
  ...
  function K(P, A, L = i) {
    return typeof P != "string" && (L = A || i, A = P, P = Object.keys(A)[0]), r._create && r._create(P, A), l(G, {
      context: t(L),
      current: t(P),
      states: t(A)
    });
  ...
  function W(P, A, L, C) {
    ...
    for (let { to: Y, guards: je, reducers: ce } of C)
      if (je(V, L)) {
        P.context = ce.call(P, V, L);
        let Ue = A.original || A, Dt = l(Ue, {
          current: t(Y),
          original: { value: Ue }
        });
        return r._onEnter && r._onEnter(A, Y, P.context, V, L), Dt.state.value.enter(Dt, P, L);
      }
  }
  function R(P, A) {
    let L = A.type || A, { machine: C } = P, { value: V, name: Y } = C.state;
    return V.transitions.has(L) ? W(P, C, A, V.transitions.get(L)) || C : (r._send && r._send(L, Y), C);
  }
  let U = {
    send(P) {
      this.machine = R(this, P), this.onChange(this);
    }
  };
  function N(P, A, L, C) {
    let V = Object.create(U, {
      machine: e(P),
      context: e(P.context(L, C)),
      onChange: t(A)
    });
    return V.send = V.send.bind(V), V.machine = V.machine.state.value.enter(V.machine, V, C), V;
  }
  return me.action = f, me.createMachine = K, me.d = r, me.guard = y, me.immediate = T, me.interpret = N, me.invoke = D, me.reduce = p, me.state = k, me.transition = v, me;
}
var Po;
function Ku() {
  if (Po) return pt;
  Po = 1;
  var t = pt && pt.__awaiter || function(R, U, N, P) {
    function A(L) {
    ...
    return new (N || (N = Promise))(function(L, C) {
      ...
      function je(ce) {
        ce.done ? L(ce.value) : A(ce.value).then(V, Y);
      }
      je((P = P.apply(R, U || [])).next());
    });
  };
  Object.defineProperty(pt, "__esModule", { value: !0 }), pt.createRealtimeClient = W;
  const e = Xu, r = Ju(), n = ws(), i = dt(), o = ki(), a = Ke(), s = () => ({
  ...
  function u(R, U) {
    return Object.assign(Object.assign({}, R), { enqueuedMessage: U.message });
  }
  function c(R) {
  ...
  function p(R, U) {
    return R.websocket && R.websocket.readyState === WebSocket.OPEN ? (U.message instanceof Uint8Array || typeof U.message == "string" ? R.websocket.send(U.message) : R.websocket.send((0, e.encode)(U.message)), Object.assign(Object.assign({}, R), { enqueuedMessage: void 0 })) : Object.assign(Object.assign({}, R), { enqueuedMessage: U.message });
  }
  function f(R) {
    return Object.assign(Object.assign({}, R), { token: void 0 });
  }
  function m(R, U) {
    return Object.assign(Object.assign({}, R), { token: U.token });
  }
  function y(R, U) {
    return Object.assign(Object.assign({}, R), { websocket: U.websocket });
  }
  ...
  function g(R, { token: U, maxBuffering: N, path: P }) {
    var A;
    if (N !== void 0 && (N < 1 || N > 60))
      throw new Error("The `maxBuffering` must be between 1 and 60 (inclusive)");
    const L = new URLSearchParams({
      fal_jwt_token: U
    });
    N !== void 0 && L.set("max_buffering", N.toFixed(0));
    const C = (0, a.ensureEndpointIdFormat)(R), V = (A = (0, a.resolveEndpointPath)(R, P, "/realtime")) !== null && A !== void 0 ? A : "";
    return `wss://fal.run/${C}${V}?${L.toString()}`;
  }
  const _ = 128;
  function b(R) {
  ...
  function x(R, U, N) {
    if (!T.has(R)) {
      const P = (0, r.interpret)(h, N);
      T.set(R, Object.assign(Object.assign({}, P), { throttledSend: U > 0 ? (0, a.throttle)(P.send, U, !0) : P.send }));
    }
    return T.get(R);
  }
  const E = () => {
  ...
  function O(R) {
    return R.status !== "error" && R.type !== "x-fal-message" && !q(R);
  }
  function q(R) {
  ...
  function D(R) {
    return t(this, void 0, void 0, function* () {
      if (typeof R == "string")
        return JSON.parse(R);
      const U = (N) => t(this, void 0, void 0, function* () {
        return N instanceof Uint8Array ? N : N instanceof Blob ? new Uint8Array(yield N.arrayBuffer()) : new Uint8Array(N);
      });
      return R instanceof ArrayBuffer || R instanceof Uint8Array ? (0, e.decode)(yield U(R)) : R instanceof Blob ? (0, e.decode)(yield U(R)) : R;
    });
  }
  function G(R) {
    return R instanceof Uint8Array ? R : (0, e.encode)(R);
  }
  function K({ data: R, decodeMessage: U, onResult: N, onError: P, send: A }) {
    const L = (C) => {
      if (b(C)) {
        A({
          type: "unauthorized",
    ...
    Promise.resolve(U ? U(R) : R).then(L).catch((C) => {
      var V;
      P(new i.ApiError({
  ...
  function W({ config: R }) {
    return {
      connect(U, N) {
        ...
        if (P && !(0, o.isBrowser)())
          return k;
        const Dt = Y ?? ((Qe) => G(Qe)), fn = je ?? ((Qe) => D(Qe));
        let xr, ao, $t, Ft = 0;
        S.set(A, {
          decodeMessage: fn,
          onError: N.onError,
        ...
        const pn = () => S.get(A), Bt = x(A, V, ({ context: Qe, machine: Ze, send: Me }) => {
          var mn;
          const { enqueuedMessage: hn, token: gn, websocket: yn } = Qe;
          if (ao = hn, Ze.current === "active" && hn && (yn == null ? void 0 : yn.readyState) === WebSocket.OPEN && Me({ type: "send", message: hn }), Ze.current === "authRequired" && gn === void 0 && xr !== Ze.current) {
            Me({ type: "initiateAuth" }), Ft++;
            const Ae = Ft, fe = (0, a.ensureEndpointIdFormat)(U), Te = (mn = (0, a.resolveEndpointPath)(U, C, "/realtime")) !== null && mn !== void 0 ? mn : "", et = ce ? () => ce(`${fe}${Te}`) : () => (console.warn("[fal.realtime] Using the default token provider is deprecated. Please provide a `tokenProvider` function to `fal.realtime.connect()`. See https://docs.fal.ai/model-apis/client#client-side-usage-with-token-provider for more information."), (0, n.getTemporaryAuthToken)(U, R)), Vt = ce ? Ue : n.TOKEN_EXPIRATION_SECONDS, _n = Vt !== void 0 ? () => {
              clearTimeout($t);
              const Gt = Math.round(Vt * 0.9 * 1e3);
              $t = setTimeout(() => {
                Ae === Ft && et().then((wn) => {
                  Ae === Ft && (queueMicrotask(() => {
                    Me({ type: "authenticated", token: wn });
                  }), _n());
                }).catch(() => {
                  if (Ae !== Ft)
                    return;
                  const wn = Math.round(Vt * 0.05 * 1e3);
                  $t = setTimeout(() => {
                    _n();
                  }, wn);
                });
          ...
          if (Ze.current === "connecting" && xr !== Ze.current && gn !== void 0) {
            const Ae = new WebSocket(g(U, { token: gn, maxBuffering: L, path: C }));
            Ae.onopen = () => {
              var fe, Te;
              ...
              const { decodeMessage: Te = fn, onResult: et, onError: Vt = E } = pn();
              K({
                data: fe.data,
                decodeMessage: Te,
                onResult: et,
...
function No() {
  if (Oo) return ft;
  Oo = 1;
  var t = ft && ft.__awaiter || function(u, c, p, f) {
    function m(y) {
  ...
  const e = ji(), r = Ai(), n = fu(), i = Ku(), o = Sr(), a = dt(), s = _s(), l = bs();
  function d(u = {}) {
...
function Qu() {
  return qo || (qo = 1, (function(t) {
    var e = tt && tt.__createBinding || (Object.create ? (function(d, u, c, p) {
      p === void 0 && (p = c);
    ...
    })), r = tt && tt.__exportStar || function(d, u) {
...
const ec = /* @__PURE__ */ JSON.parse('[{"display_name":"3D Rigging","job_set_type":"3d_rigging","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height_meters","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true}]},{"display_name":"Brain Activity","job_set_type":"brain_activity","type":"text","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Bytedance Image Upscale","job_set_type":"bytedance_image_upscale","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"4k","required":false,"enum":["2k","4k"]}]},{"display_name":"Bytedance Video Upscale","job_set_type":"bytedance_video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"fps","type":"integer","default":24,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"model_version","type":"string","default":"standard","required":false,"enum":["standard","pro"]},{"name":"preset","type":"string","default":"common","required":false,"enum":["common","aigc","short_series","ugc","old_film"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1080p","2k","4k"]}]},{"display_name":"Cinematic Studio 2.5","job_set_type":"cinematic_studio_2_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"auto","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio 3.0","job_set_type":"cinematic_studio_3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Cinematic Studio Image","job_set_type":"cinematic_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"string","default":null,"required":true},{"name":"camera_lens_id","type":"string","default":null,"required":true},{"name":"camera_model_id","type":"string","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio Soul Cast","job_set_type":"cinematic_studio_soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinematic Studio Soul Location","job_set_type":"cinematic_studio_soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Cinematic Studio Video","job_set_type":"cinematic_studio_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"slow_motion","type":"boolean","default":false,"required":false},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Cinematic Studio Video 3.5","job_set_type":"cinematic_studio_video_3_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"camera_style","type":"object","default":null,"required":false},{"name":"color_grading","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"light_scheme","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"style_id","type":"object","default":null,"required":false},{"name":"style_prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinema Studio 4.0","job_set_type":"cinematic_studio_video_4_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"color_palette","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"era_id","type":"object","default":null,"required":false},{"name":"extension_mode","type":"object","default":null,"required":false},{"name":"film_era","type":"null","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"null","default":null,"required":false},{"name":"genre_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"light","type":"object","default":null,"required":false},{"name":"light_custom","type":"object","default":null,"required":false},{"name":"light_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"model","type":"string","default":"default","required":false,"enum":["default","video_edit","video_extension"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"pacing_id","type":"object","default":null,"required":false},{"name":"preset_id","type":"null","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"speedramp","type":"object","default":"auto","required":false},{"name":"use_blur","type":"boolean","default":false,"required":false},{"name":"use_eye_mask","type":"boolean","default":false,"required":false},{"name":"use_transparency","type":"boolean","default":false,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Cinematic Studio Video V2","job_set_type":"cinematic_studio_video_v2","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"cfg_scale","type":"number","default":0.5,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","western","suspense","intimate","spectacle"]},{"name":"kling_element_ids","type":"array","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Clipify","job_set_type":"clipify","type":"video","params":[{"name":"clip_aspect","type":"string","default":"9:16","required":false,"enum":["9:16","1:1","16:9"]},{"name":"clips_num","type":"integer","default":10,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"max_height","type":"integer","default":1080,"required":false},{"name":"segment_seconds","type":"integer","default":10,"required":false},{"name":"subtitle_case","type":"string","default":"as-is","required":false,"enum":["lower","upper","as-is"]},{"name":"subtitle_font","type":"string","default":"notosans","required":false},{"name":"subtitle_highlight_hex","type":"string","default":"#FFE84D","required":false},{"name":"subtitle_position","type":"string","default":"bottom","required":false,"enum":["bottom","center","top"]},{"name":"track_face_crop","type":"boolean","default":true,"required":false},{"name":"urls","type":"array","default":null,"required":true}]},{"display_name":"Draw To Video","job_set_type":"draw_to_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"ref_image","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"sketch","type":"object","default":null,"required":true},{"name":"video","type":"object","default":null,"required":true}]},{"display_name":"dubbing","job_set_type":"dubbing","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"target_language","type":"string","default":null,"required":true,"enum":["eng","cmn","fra","hin","ita","jpn","kor","por","rus","tur","spa","deu","ara","pol","ind","fil","swe","fin"]}]},{"display_name":"Explainer Video","job_set_type":"explainer_video","type":"video","params":[{"name":"height","type":"integer","default":null,"required":true},{"name":"items","type":"array","default":null,"required":true},{"name":"subtitles","type":"object","default":null,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"FLUX.2","job_set_type":"flux_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"pro","required":false,"enum":["pro","flex","max"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"FLUX.2 Pro Outpaint","job_set_type":"flux_2_pro_outpaint","type":"image","params":[{"name":"expand_bottom","type":"integer","default":0,"required":false},{"name":"expand_left","type":"integer","default":0,"required":false},{"name":"expand_right","type":"integer","default":0,"required":false},{"name":"expand_top","type":"integer","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"FLUX 3 Video","job_set_type":"flux_3_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","2:1","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Flux Kontext","job_set_type":"flux_kontext","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Gemini Omni Flash","job_set_type":"gemini_omni","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"integer","default":8,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false}]},{"display_name":"GPT Image 2","job_set_type":"gpt_image_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"high","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"GPT Image 2.5","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","16:9","9:16","21:9","27:16","16:27","9:8","8:9"]},{"name":"background","type":"string","default":null,"required":false,"enum":["auto","opaque","transparent"]},{"name":"image_references","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"low","required":false,"enum":["low","medium","high","xhigh","max"]},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]},{"name":"variant","type":"string","default":"flare","required":false,"enum":["flare","sunburst"]}],"type":"image","job_set_type":"gpt_image_2_5"},{"display_name":"Grok Image","job_set_type":"grok_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","1:2","2:1","3:2","2:3","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","quality"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Grok Video","job_set_type":"grok_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Grok Video 1.5","job_set_type":"grok_video_v15","type":"video","params":[{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Happy Horse Video","job_set_type":"happy_horse_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Genjutsu","job_set_type":"hf_mult_motion_control","type":"video","params":[{"name":"image_references","type":"array","default":null,"required":false},{"name":"prompt","type":"string","nullable":true,"default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"video_references","type":"array","default":null,"required":false}],"rules":[{"cel":"size(params.image_references) >= 1","message":"at least one image_references entry is required"},{"cel":"size(params.video_references) == 1","message":"exactly one video_references entry is required"}]},{"display_name":"Genjutsu","job_set_type":"hf_mult_replace_object","type":"video","params":[{"name":"image_references","type":"array","default":null,"required":false},{"name":"prompt","type":"string","nullable":true,"default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"video_references","type":"array","default":null,"required":false}],"rules":[{"cel":"size(params.image_references) >= 1","message":"at least one image_references entry is required"},{"cel":"size(params.video_references) == 1","message":"exactly one video_references entry is required"}]},{"display_name":"Hunyuan 3D v3.1 Text to 3D","job_set_type":"hunyuan3d_v3_1_text_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Hunyuan3D v3 Image to 3D","job_set_type":"hunyuan3d_v3_image_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"integer","default":500000,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"string","default":"Normal","required":false,"enum":["Normal","LowPoly","Geometry"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"polygon_type","type":"string","default":"triangle","required":false,"enum":["triangle","quadrilateral"]}]},{"display_name":"Image Auto","job_set_type":"image_auto","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Image Background Remover","job_set_type":"image_background_remover","type":"image","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Image Decompose","job_set_type":"image_decompose","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"granular","required":false,"enum":["granular","standard"]}]},{"display_name":"Image to 3D","job_set_type":"image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Inworld Text to Speech","job_set_type":"inworld_text_to_speech","type":"audio","params":[{"name":"prompt","type":"string","default":null,"required":true},{"name":"voice","type":"string","default":null,"required":true}]},{"display_name":"Kimodo","job_set_type":"kimodo","type":"3d","params":[{"name":"diffusion_steps","type":"integer","default":10,"required":false},{"name":"duration","type":"object","default":null,"required":false},{"name":"durations","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_version","type":"string","default":"ardy-core","required":false,"enum":["ardy-core","ardy-core-h8"]},{"name":"prompt","type":"object","default":null,"required":false},{"name":"prompts","type":"object","default":null,"required":false},{"name":"seed","type":"integer","default":42,"required":false}]},{"display_name":"Kling O1 Image","job_set_type":"kling_omni_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","auto","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Kling 2.6 Video","job_set_type":"kling2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Kling v3.0","job_set_type":"kling3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std","4k"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]}]},{"display_name":"Kling 3.0 Motion Control","job_set_type":"kling3_0_motion_control","type":"video","params":[{"name":"background_source","type":"string","default":"input_image","required":false,"enum":["input_image","input_video"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","pro"]}]},{"display_name":"Kling 3.0 Turbo","job_set_type":"kling3_0_turbo","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"LLM Generation","job_set_type":"llm_text","type":"video","params":[{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":null,"required":true},{"name":"reasoning_effort","type":"object","default":null,"required":false},{"name":"system_prompt","type":"string","default":"","required":false},{"name":"user_prompt","type":"string","default":"","required":false}]},{"display_name":"Marketing Studio Image","job_set_type":"marketing_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Marketing Studio Video","job_set_type":"marketing_studio_video","type":"video","params":[{"name":"ad_reference_id","type":"object","default":null,"required":false},{"name":"aspect_ratio","type":"string","default":"9:16","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"avatar_ids","type":"array","default":null,"required":false},{"name":"avatars","type":"array","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"hook_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"ugc","required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"setting_id","type":"object","default":null,"required":false},{"name":"specific_mode","type":"string","default":"default","required":false,"enum":["default","web_product","from_storyboard"]},{"name":"storyboard_id","type":"object","default":null,"required":false},{"name":"web_product_ids","type":"array","default":null,"required":false},{"name":"web_product_type","type":"object","default":null,"required":false}]},{"display_name":"Meshy 5 Remesh","job_set_type":"meshy_v5_remesh","type":"3d","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true},{"name":"origin_at","type":"object","default":null,"required":false},{"name":"resize_height","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Meshy 6 Text to 3D","job_set_type":"meshy_v6_text_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"enable_prompt_expansion","type":"boolean","default":false,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"full","required":false},{"name":"model_type","type":"string","default":"standard","required":false},{"name":"pose_mode","type":"string","default":"","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"boolean","default":true,"required":false},{"name":"symmetry_mode","type":"string","default":"auto","required":false},{"name":"target_polycount","type":"integer","default":30000,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"string","default":"triangle","required":false}]},{"display_name":"MiniMax H3","job_set_type":"minimax_h3","type":"video","params":[{"name":"aigc_watermark","type":"boolean","default":false,"required":false},{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":4,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"resolution","type":"string","default":"2K","required":false,"enum":["768P","2K"]},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Minimax Hailuo","job_set_type":"minimax_hailuo","type":"video","params":[{"name":"duration","type":"string","default":6,"required":false,"enum":["6","10"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"minimax-2.3","required":false,"enum":["minimax","minimax-fast","minimax-2.3","minimax-2.3-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"768","required":false,"enum":["512","768","1080"]}]},{"display_name":"Mirelo Text to Audio","job_set_type":"mirelo_text_to_audio","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"MS Image","job_set_type":"ms_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"avatars","type":"array","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"brand_kit_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"low","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Multi-Image to 3D","job_set_type":"multi_image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Nano Banana","job_set_type":"nano_banana","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_ai_stylist","type":"image","params":[{"name":"background_preset_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"outfit_preset_ids","type":"array","default":null,"required":false},{"name":"pose_preset_id","type":"object","default":null,"required":false},{"name":"user_outfit_ids","type":"array","default":null,"required":false}]},{"display_name":"Nano Banana 2 Lite","job_set_type":"nano_banana_2_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"thinking","type":"string","default":"HIGH","required":false,"enum":["MINIMAL","HIGH"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_relight","type":"image","params":[{"name":"brightness","type":"integer","default":null,"required":true},{"name":"color","type":"string","default":null,"required":true},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"light_quality","type":"string","default":null,"required":true,"enum":["hard","sharp","soft"]},{"name":"light_source","type":"string","default":null,"required":true,"enum":["mdl","mdr","mul","mur","bml","fml","fmr","bmm","mml","mmr","fmm","bmr","mdm","mum","bdr","fdl","bur","ful","bdl","fdr","bul","fur","bdm","fdm","bum","fum"]},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_shots","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_skin_enhancer","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"preset_id","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana 2","job_set_type":"nano_banana_flash","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"OpenAI Hazel","job_set_type":"openai_hazel","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","auto"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"medium","required":false,"enum":["low","medium","high"]}]},{"display_name":"Outpaint","job_set_type":"outpaint","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"21:9","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Qwen Audio 3.0 TTS Flash","job_set_type":"qwen_audio_tts","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"instruction","type":"object","default":null,"required":false},{"name":"language","type":"object","default":null,"required":false},{"name":"pitch_rate","type":"number","default":1,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","22050","24000","44100","48000"]},{"name":"seed","type":"integer","default":0,"required":false},{"name":"speech_rate","type":"number","default":1,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"integer","default":50,"required":false}]},{"display_name":"Angles","job_set_type":"qwen_camera_control","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"move_forward_level","type":"integer","default":0,"required":false},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"rotate_degree","type":"integer","default":0,"required":false},{"name":"vertical_angle","type":"integer","default":0,"required":false},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Recraft V4.1","job_set_type":"recraft_v4_1","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:4","4:3","4:5","5:4","3:2","2:3","16:9","9:16","21:9"]},{"name":"background_color","type":"object","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"colors","type":"array","default":null,"required":false},{"name":"model_type","type":"string","default":"standard","required":false,"enum":["standard","vector","utility","utility_vector"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Reframe","job_set_type":"reframe","type":"video","params":[{"name":"aspect_ratio","type":"string","default":null,"required":true,"enum":["21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"3D Objects","job_set_type":"sam_3_3d","type":"3d","params":[{"name":"detection_threshold","type":"object","default":null,"required":false},{"name":"export_textured_glb","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"3D Body","job_set_type":"sam_3_3d_body","type":"3d","params":[{"name":"export_meshes","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"include_3d_keypoints","type":"boolean","default":true,"required":false},{"name":"include_mhr_params","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Remove Background","job_set_type":"sam_3_video","type":"video","params":[{"name":"apply_mask","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"frames_count","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false}]},{"display_name":"Seed Audio 1.0","job_set_type":"seed_audio","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"expression_intensity","type":"integer","default":5,"required":false},{"name":"format","type":"string","default":"wav","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"loudness_rate","type":"integer","default":0,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mood","type":"number","default":0,"required":false},{"name":"pitch_rate","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","24000","32000","44100","48000"]},{"name":"speech_rate","type":"integer","default":0,"required":false},{"name":"voice_id","type":"object","default":null,"required":false},{"name":"voice_style","type":"object","default":null,"required":false},{"name":"voice_type","type":"object","default":null,"required":false},{"name":"voices","type":"array","default":null,"required":false}]},{"display_name":"Seedance 2.0","job_set_type":"seedance_2_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]}]},{"display_name":"Seedance 2.0 Mini","job_set_type":"seedance_2_0_mini","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p"]}]},{"display_name":"Seedance 2.5","job_set_type":"seedance_2_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"audio_references","type":"array","default":null,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"end_image","type":"object","default":null,"required":false},{"name":"extension_mode","type":"string","default":null,"required":false,"enum":["backward","forward"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"image_references","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"t2v","required":false,"enum":["t2v","omni_reference","video_edit","video_extension"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"start_image","type":"object","default":null,"required":false},{"name":"video_references","type":"array","default":null,"required":false}]},{"display_name":"Seedance 1.5 Pro","job_set_type":"seedance1_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"duration","type":"string","default":4,"required":false,"enum":["4","8","12"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Seedream 4.5","job_set_type":"seedream_v4_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","4:3","16:9","3:2","21:9","3:4","9:16","2:3"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Lite","job_set_type":"seedream_v5_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Pro","job_set_type":"seedream_v5_pro","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"height","type":"object","default":null,"required":false},{"name":"is_inpaint","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","1.5k","2k"]},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Sonilo Music","job_set_type":"sonilo_music","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Soul Cast","job_set_type":"soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"soul_cinema_studio","job_set_type":"soul_cinema_studio","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Soul Cinematic","job_set_type":"soul_cinematic","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]}]},{"display_name":"Soul Location","job_set_type":"soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Sync Lipsync 3","job_set_type":"sync_so","type":"video","params":[{"name":"input_video","type":"object","default":null,"required":true},{"name":"input_audio","type":"object","default":null,"required":true},{"name":"sync_mode","type":"string","default":"bounce","required":false,"enum":["bounce","loop","cut_off","silence","remap"]},{"name":"folder_id","type":"string","default":null,"required":false}]},{"display_name":"Higgsfield Soul 2.0","job_set_type":"text2image_soul_v2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"Text to Speech V2","job_set_type":"text2speech_v2","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"emotion","type":"object","default":null,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["mp3","wav"]},{"name":"language_boost","type":"string","default":"auto","required":false,"enum":["auto","af","ar","bg","ca","cs","da","de","el","en","es","fa","fi","fil","fr","he","hi","hr","hu","id","it","ja","ko","ms","nl","nn","no","pl","pt","ro","ru","sk","sl","sv","ta","th","tr","uk","vi","yue","zh"]},{"name":"model","type":"string","default":null,"required":true,"enum":["elevenlabs","minimax","seed_speech","vibe_voice","cozy_voice"]},{"name":"pitch","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":32000,"required":false,"enum":["8000","16000","22050","24000","32000","44100"]},{"name":"speed","type":"number","default":1,"required":false},{"name":"stability","type":"object","default":null,"required":false},{"name":"text_normalization","type":"boolean","default":false,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"number","default":1,"required":false}]},{"display_name":"Topaz","params":[{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"string","default":null,"required":false},{"name":"image_references","type":"array","default":null,"required":true},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"sharpen","type":"number","default":0,"required":false},{"name":"variant","type":"string","default":"Standard V2","required":false,"enum":["Standard V2","Low Resolution V2","CGI","High Fidelity V2","Text Refine"]}],"type":"image","job_set_type":"topaz_image"},{"display_name":"Topaz","job_set_type":"topaz_image_generative","type":"image","params":[{"name":"autoprompt","type":"boolean","default":true,"required":false},{"name":"creativity","type":"integer","default":1,"required":false},{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"Redefine","required":false,"enum":["Standard MAX","Redefine","Recovery","Recovery V2"]},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"sharpen","type":"number","default":0,"required":false},{"name":"texture","type":"integer","default":1,"required":false}]},{"display_name":"Topaz","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"enhancement","type":"object","default":null,"required":false},{"name":"frame_interpolation","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"1080p","required":false,"enum":["1080p","2160p"]},{"name":"video_references","type":"array","default":null,"required":true}],"type":"video","job_set_type":"topaz_video"},{"display_name":"Text to 3D","job_set_type":"tripo_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"negative_prompt","type":"object","default":null,"required":false},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]}]},{"display_name":"Tripo H3.1 Image to 3D","job_set_type":"tripo_h3_1_image_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Tripo H3.1 Multiview to 3D","job_set_type":"tripo_h3_1_multiview_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Google Veo 3","job_set_type":"veo3","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"veo-3-fast","required":false,"enum":["veo-3-preview","veo-3-fast"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Google Veo 3.1","job_set_type":"veo3_1","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"model","type":"string","default":"veo-3-1-fast","required":false,"enum":["veo-3-1-preview","veo-3-1-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high","ultra"]}]},{"display_name":"Google Veo 3.1 Lite","job_set_type":"veo3_1_lite","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","auto"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Video Background Remover","job_set_type":"video_background_remover","type":"video","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Video Deflicker","job_set_type":"video_deflicker","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"Video Upscale","job_set_type":"video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"voice_change","job_set_type":"voice_change","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":"preset","required":false,"enum":["preset","element"]}]},{"display_name":"Wan 2.6 Video","job_set_type":"wan2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10","15"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 2.7","job_set_type":"wan2_7","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 3.0","job_set_type":"wan3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enable_thinking","type":"boolean","default":false,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Z Image","job_set_type":"z_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"prompt","type":"string","default":null,"required":true}]}]'), tc = {
  ...
  "sketch",
  "texture_image_url"
]), oc = /* @__PURE__ */ new Set(["input_images", "image_references"]), ac = /* @__PURE__ */ new Set(["input_video", "video"]), sc = /* @__PURE__ */ new Set(["input_audio"]);
function Oi(t) {
  return t.split(/[_-]+/).filter(Boolean).map((e) => e.charAt(0).toUpperCase() + e.slice(1)).join(" ");
}
function lc(t) {
  return nc[t] ?? `hf-${t.replaceAll("_", "-")}`;
}
function dc(t) {
...
function uc(t, e) {
  let r, n, i = !1;
  if (ic.has(e.name) ? (r = "image", n = t.type === "video" && e.name === "input_image" ? "start_image" : "image") : oc.has(e.name) ? (r = "image", n = "image", i = !0) : e.name === "video_references" ? (r = "video", n = "video", i = !0) : e.name === "audio_references" ? (r = "audio", n = "audio", i = !0) : ac.has(e.name) ? (r = "video", n = "video") : sc.has(e.name) ? (r = "audio", n = "audio") : e.name === "model_url" ? r = "model3d" : e.name === "urls" ? (r = "media", i = !0) : e.name === "medias" && (i = !0, t.type === "image" || t.type === "3d" ? (r = "image", n = "image") : t.type === "text" ? (r = "video", n = "video") : r = "media"), !!r)
    return {
      id: e.name,
      portType: r,
      label: Oi(e.name),
...
const mc = ["prompt", "user_prompt", "instruction"];
function hc(t) {
  for (const e of mc) {
...
function yc(t = Os) {
  ...
  const e = {};
  for (const n of t) {
    const i = lc(n.job_set_type), o = dc(n.type);
    if (e[i]) throw new Error(`Duplicate Higgsfield node type: ${i}`);
    const a = hc(n.params.flatMap((l) => [
      fc(n, cc(n, l)),
      ...pc(n, l)
    ])), s = a.findIndex((l) => l.id === "prompt") + 1;
    a.splice(s, 0, ...gc(n)), e[i] = {
      id: n.job_set_type,
      nodeType: i,
...
const _c = {
  ...
  video: "--video",
  audio: "--audio"
}, Cs = /^[A-Za-z][A-Za-z0-9_]*$/, qs = /* @__PURE__ */ new Set(["json", "wait", "no_color"]);
function wc(t) {
  const e = ["generate", "create", t.model], r = {
    ...Ns(t.model, { ...t.extra, ...t.params }) ?? {}
  }, n = (o, a) => {
    if (a == null) return;
    if (!Cs.test(o) || qs.has(o))
  ...
  if (Ps(t.model)) {
    if (Zu(t.model, r, t.medias ?? []), t.model === "sync_so") throw new Error("Sync Lipsync 3 is available through Higgsfield MCP in the CineGen website. The current Higgsfield desktop CLI does not expose this model. No generation was submitted.");
  ...
  for (const o of t.medias ?? []) {
    if (!o.value) continue;
    const a = it(t.model) ? o.role === "video" ? "--video-references" : "--image-references" : _c[o.role];
    e.push(a, o.value);
  }
...
const bc = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;
function vc(t) {
...
function Ci(t) {
  ...
  const i = Tc(n), o = i.job_id ?? i.id ?? i.jobId;
  return {
...
`);
  ...
  return (r = e.match(bc)) == null ? void 0 : r[0];
}
function We(t) {
  return !!t && typeof t == "object" && !Array.isArray(t);
}
const Ec = /* @__PURE__ */ new Set(["params", "prompt", "input_images", "inputs", "extra", "request"]), Uo = [
  "url",
  "urls",
  "video_url",
  "image_url",
...
function bt(t, e = 0) {
  ...
  for (const [n, i] of Object.entries(t))
    Ec.has(n) || Uo.includes(n) || Mo.includes(n) || n === "result_json" || (We(i) || Array.isArray(i)) && r.push(...bt(i, e + 1));
  return [...new Set(r)];
}
const Sc = /https?:\/\/[^\s"'<>\\]+/gi;
function Ls(t) {
...
function Ms(t) {
  return t instanceof ri ? { stdout: t.stdout, stderr: t.stderr } : { stdout: "", stderr: "" };
}
...
function kc() {
  const t = H.homedir(), e = [w.join(t, ".npm-global/bin"), w.join(t, ".local/bin"), "/opt/homebrew/bin", "/usr/local/bin"];
  return { ...process.env, PATH: [...e, process.env.PATH ?? ""].filter(Boolean).join(w.delimiter), NO_COLOR: "1" };
}
const Do = 1260 * 1e3, jc = 9e4, $o = 4, jn = "Higgsfield CLI not found. Install @higgsfield/cli, then run higgsfield auth login — or connect Higgsfield in Settings.";
function Rc(t) {
...
function Pc(t, e, r) {
  return new Promise((n, i) => {
    var d, u;
    const o = ae(t, e, { env: kc() });
    let a = "", s = "";
    const l = setTimeout(() => {
      o.kill("SIGTERM"), i(new ri("Higgsfield CLI timed out", a, s));
    }, r);
...
async function Le(t, e = 6e4) {
  const r = t.includes("--json") ? t : [...t, "--json"], n = Ac(), i = mt ? [mt, ...n.filter((l) => l !== mt)] : n;
  if (i.length === 0) throw new Error(jn);
  let o;
  for (const l of i)
    try {
      const d = await Pc(l, r, e);
      return mt = l, d;
    } catch (d) {
      if (Rc(d)) {
        mt === l && (mt = null), o = d;
  ...
  throw new Error(s ? `${jn} Tried: ${a}. ${s}` : `${jn} Tried: ${a}.`);
}
const Oc = 6e4, Ds = {
  "image/png": ".png",
  "image/jpeg": ".jpg",
...
function Nc(t, e, r) {
  const n = e ? Ds[e.split(";", 1)[0].trim().toLowerCase()] : void 0;
  if (n) return n;
  const i = w.extname(t.split(/[?#]/, 1)[0]).toLowerCase();
  return i && i.length <= 5 ? i : $s(r);
}
async function Cc(t) {
  ...
  for (const [n, i] of t.entries()) {
    ...
    if (/^https?:\/\//i.test(o)) {
      ...
      const s = Nc(o, a.headers.get("content-type"), i.role), l = w.join(H.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${s}`);
      await $.promises.writeFile(l, Buffer.from(await a.arrayBuffer())), e.push(l), r.push({ ...i, value: l });
    } else if (o.startsWith("data:")) {
      const a = o.indexOf(",");
      if (a < 0) throw new Error("Malformed data: URI media input");
      const s = o.slice(5, a), l = s.replace(/;base64$/i, ""), d = o.slice(a + 1), u = /;base64$/i.test(s) ? Buffer.from(d, "base64") : Buffer.from(decodeURIComponent(d)), c = Ds[l.toLowerCase()] ?? $s(i.role), p = w.join(H.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${c}`);
      await $.promises.writeFile(p, u), e.push(p), r.push({ ...i, value: p });
    } else
...
async function oi(t) {
  const { medias: e, tempPaths: r } = await Cc(t.medias);
  let n;
  try {
    n = await qc({ ...t, medias: e });
  } finally {
    for (const o of r)
      $.promises.unlink(o).catch(() => {
      });
  }
...
async function qc(t) {
  ...
  for (let n = 1; n <= 3; n++)
    try {
      const i = await Le(e, jc), o = await ii(i, t);
      if (o) return o;
      const a = Lo(i);
      if (a) return { jobId: a };
      ...
      r = i;
      const { stdout: o, stderr: a } = Ms(i), s = await ii(o, t);
      if (s) return s;
      const l = Lo(o, a, St(i));
      if (l) return { jobId: l };
      if (!ni(St(i)) || n === 3) throw i;
      await Fs(1500 * n);
    }
...
async function Bs(t, e) {
  try {
    const r = await Gs({ size: 50 }), n = Lc(r, t);
    if (!n) return;
    const i = qi({
...
async function Li(t, e) {
  ...
  const o = Ls(r);
  return o[0] ? { ...i, url: o[0], urls: o, outputs: o.map((a) => ({ kind: e.mediaType, url: a })) } : Bs(t, e);
}
async function Uc(t) {
  try {
    return await Le(
...
async function Vs(t, e) {
  ...
  for (let i = 1; i <= $o; i++)
    try {
      ...
      r = o;
      const { stdout: a } = Ms(o), s = await ii(a, e);
      if (s) return s;
      try {
        const u = await Li(t, e);
        if (u) return u;
      } catch (u) {
        if (!ni(St(u))) throw u;
      }
      const l = St(o);
      if (!(ni(l) || /timed out/i.test(l) || /still running/i.test(l) || /without a media URL/i.test(l))) throw o;
      if (i === $o) break;
      await Fs(2e3 * i);
    }
  const n = await Bs(t, e);
  if (n) return n;
  throw new Error(
...
async function Mc(t, e) {
  ...
  return r && Ni(r, e.mediaType) ? r : Vs(t, e);
}
function Dc(t, e) {
  const r = ["generate", "cost", t];
  for (const [n, i] of Object.entries(Ns(t, e) ?? {}))
    i == null || i === "" || !Cs.test(n) || qs.has(n) || (typeof i == "string" ? r.push(`--${n}`, i) : typeof i == "number" && Number.isFinite(i) ? r.push(`--${n}`, String(i)) : typeof i == "boolean" && r.push(`--${n}`, i ? "true" : "false"));
...
function Bc() {
  I.handle("higgsfield:account-status", async () => Vo(await Bo())), I.handle("higgsfield:quick-edit", async (t, e) => {
    const { prepareClipReference: r, resolveLocalSourcePath: n } = await Promise.resolve().then(() => qa);
    ...
    const { resolveLocalSourcePath: r } = await Promise.resolve().then(() => qa);
    if (e.jobId) {
  ...
  }), I.handle("higgsfield:generate-list", async (t, e) => Gs(e)), I.handle("higgsfield:generate-cost", async (t, e) => Fc(e == null ? void 0 : e.model, (e == null ? void 0 : e.params) ?? {})), I.handle("higgsfield:auth-login", async () => {
    try {
...
}, Hc = "notrius/ltx-2.5-serverless:cu130@sha256:73d1621ef915ae6a149f2a32f6c317dfc89f12075ed4b3abd7df707420267205", Ui = "https://rest.runpod.io/v1", Wc = "https://api.runpod.io/v2", zs = "https://api.runpod.io/graphql", Mi = 8e3, Rn = 256 * 1024, Xc = 1800, Jc = 200, ai = 15e3, Hs = 6500, Kc = 12e4, Di = 12e3, Ws = 14 * 1024 * 1024, Yc = 100 * 1024 * 1024, zo = 1024 * 1024, Ho = 4, Xs = Object.freeze(["sdxl", "qwen-image-edit"]), Qc = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABACAIAAAAlC+aJAAAAYElEQVR4nO3PQQ0AIBDAMMD4WUcEj4ZkVbDtmVk/OzrgVQNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgPaBRFyAf0dnk7yAAAAAElFTkSuQmCC", Zc = "balanced", Wo = Object.freeze({
...
function Ie(t, e) {
  if (typeof t != "string" || !t.trim())
    throw new B(`${e} is required.`, "MISSING_CONFIGURATION", 422);
  return t.trim();
}
function rn(t, e) {
  const r = Ie(t, e);
  if (!/^[A-Za-z0-9_-]{1,191}$/.test(r))
    throw new B(`${e} is invalid.`, "INVALID_CONFIGURATION", 422);
  return r;
}
async function tf(t) {
...
async function Js(t, e, r = {}, n = ai) {
  ...
  let s;
  const l = new $i(n), d = new Promise((c, p) => {
    s = setTimeout(() => {
      i.abort(l), p(l);
    }, n);
  }), u = (async () => {
    const c = await t(e, { ...r, signal: i.signal }), p = await tf(c);
...
function rf(t) {
  try {
    return new URL(t).pathname === "/health" ? Hs : ai;
  } catch {
    return ai;
  }
}
...
async function xe(t, e, r, n, i = [200, 201, 202, 204], o = rf(e)) {
  let a;
  try {
    a = await Js(t, e, r, o);
  } catch (d) {
    throw d instanceof $i ? new B(`${n} RunPod did not respond before the request timed out.`, "PROVIDER_TIMEOUT", 504) : new B(d instanceof Error ? d.message : n, "PROVIDER_UNREACHABLE", 502);
  }
  const { response: s, payload: l } = a;
  if (!i.includes(s.status))
    throw new B(nn(l, `${n} (${s.status})`), "PROVIDER_ERROR", s.status);
  return l;
}
function Fi(t, e = !1) {
  return {
...
async function of(t, e, r) {
  var a;
  const n = new AbortController(), i = setTimeout(() => n.abort(), Xc);
  let o;
  try {
    const s = new URL(`${Wc}/pods/${encodeURIComponent(r)}/logs`);
    ...
    for (; !c.done && u.length < Rn && (u += d.decode(c.value, { stream: !0 }), !(u.length >= Rn)); ) {
      let p;
      const f = new Promise((m) => {
        p = setTimeout(() => m({ quiet: !0 }), Jc);
      });
      if (c = await Promise.race([o.read(), f]), clearTimeout(p), c != null && c.quiet)
        break;
    }
    return u += d.decode(), nf(u.slice(0, Rn));
  } catch {
    return [];
  } finally {
...
async function Xo(t, e, r, n) {
  const i = `mutation { secretCreate(input: { name: ${JSON.stringify(r)}, value: ${JSON.stringify(n)}, description: "Temporary CineGen LTX-2.5 session credential" }) { id name } }`, o = new URL(zs);
  ...
  if (s.length || typeof (d == null ? void 0 : d.id) != "string")
    throw new B(nn(a, "RunPod could not create the encrypted session secret."));
  return d.id;
}
async function Ks(t, e, r) {
  const n = `mutation { secretDelete(id: ${JSON.stringify(r)}) }`, i = new URL(zs);
...
function Bi(t, e) {
  const r = rn(e, "RunPod session ID"), n = new URL(Ie(t, "RunPod session URL"));
  if (n.protocol !== "https:" || n.username || n.password || n.hostname !== `${r}-${Mi}.proxy.runpod.net`)
    throw new B("RunPod session URL is invalid.", "INVALID_CONFIGURATION", 422);
...
function mf(t) {
  const e = t === void 0 ? Zc : t;
  if (typeof e != "string" || !Object.hasOwn(Wo, e))
    throw new B("Choose a valid LTX-2.5 GPU profile: economy, balanced, or performance.", "INVALID_GPU_PROFILE", 422);
...
function hf(t) {
  if (t === void 0)
    return [];
  if (!Array.isArray(t))
    throw new B("Image models must be an array.", "INVALID_IMAGE_MODELS", 422);
  const e = [];
  for (const r of t) {
    if (typeof r != "string" || !Xs.includes(r))
      throw new B("Choose only supported session image models: SDXL or Qwen Image Edit.", "INVALID_IMAGE_MODELS", 422);
...
async function yf(t, e = fetch) {
  const r = Ie(t.runpodKey, "RunPod API key"), n = Ie(t.huggingFaceToken, "Hugging Face read token");
  if (!/^hf_[A-Za-z0-9]+$/.test(n))
    throw new B("Enter a valid Hugging Face read token.", "INVALID_HUGGINGFACE_TOKEN", 422);
  const i = mf(t.gpuProfile), o = hf(t.imageModels), a = pf(), s = `cinegen_ltx25_hf_${a}`, l = `cinegen_ltx25_session_${a}`, d = `${crypto.randomUUID()}${crypto.randomUUID()}`.replace(/-/g, ""), u = [];
  try {
    u.push(await Xo(e, r, s, n)), u.push(await Xo(e, r, l, d));
    const c = await xe(e, `${Ui}/pods`, {
      method: "POST",
      headers: Fi(r, !0),
      body: JSON.stringify({
        ...
        containerDiskInGb: gf(i.config, o),
        volumeInGb: 0,
        ports: [`${Mi}/http`],
        supportPublicIp: !0,
        interruptible: !1,
    ...
    }, "RunPod could not create the LTX-2.5 session Pod."), p = Ys(c);
    if (!p.id)
      throw new B("RunPod created a Pod without returning its ID.");
    return {
      podId: p.id,
      podUrl: ff(p.id),
      podAuthToken: d,
    ...
    throw await Promise.allSettled(u.map((p) => Ks(e, r, p))), c;
  }
}
async function _f(t, e = fetch) {
  ...
  try {
    ...
    if (c instanceof B && c.statusCode === 404)
      return {
        status: "ended",
        phase: "ended",
        podId: i.podId,
  ...
  if (a.desiredStatus === "RUNNING")
    try {
      const { response: c, payload: p } = await Js(e, `${i.url}/health`, {
      ...
      s = c instanceof $i ? { kind: "timeout" } : { kind: "unreachable" };
    }
  const l = await of(e, r, i.podId), d = lf(l);
  if (!d && af(l))
    try {
      const c = await Qs({
        runpodKey: r,
  ...
  let u;
  return (s == null ? void 0 : s.kind) === "timeout" ? u = "RunPod reports the Pod is running, but its private gateway did not answer within 7 seconds. It may still be starting; check again shortly. Billing continues while the Pod runs." : (s == null ? void 0 : s.kind) === "response" && (s.status === 502 || s.status === 504) ? u = `RunPod reports the Pod is running, but its private gateway returned ${s.status}. The container may still be starting; check again shortly. Billing continues while the Pod runs.` : u = uf(s == null ? void 0 : s.body) ?? cf(l), !u && (s == null ? void 0 : s.kind) === "unreachable" ? u = "RunPod reports the Pod is running, but its private gateway is not reachable yet. The container may still be starting; check again shortly. Billing continues while the Pod runs." : !u && (s == null ? void 0 : s.kind) === "response" && s.status >= 400 && (u = `RunPod reports the Pod is running, but its private gateway returned HTTP ${s.status}. Check again shortly. Billing continues while the Pod runs.`), {
    status: "downloading",
    phase: "downloading",
    podId: i.podId,
    podUrl: i.url,
...
async function Qs(t, e = fetch) {
  const r = Ie(t.runpodKey, "RunPod API key"), n = rn(t.podId, "RunPod session ID");
  ...
  return (await Promise.allSettled(i.map((s) => Ks(e, r, s)))).filter((s) => s.status === "rejected").length ? { ok: !0, warning: "The Pod was deleted and billing stopped, but one temporary RunPod secret could not be removed." } : { ok: !0 };
}
function wf(t, e) {
...
function bf(t) {
  const e = JSON.parse(JSON.stringify(Vc)), r = Ie(t.prompt, "Video prompt");
  if (r.length > Di)
    throw new B("The LTX-2.5 video prompt is too long.", "PROMPT_TOO_LONG", 422);
  const n = Math.min(20, Math.max(1, Math.round(Number(t.durationSec) || 5))), i = ["16:9", "9:16", "1:1"].includes(t.aspectRatio ?? "") ? t.aspectRatio : "16:9", o = t.resolution === "480p" || t.resolution === "1080p" ? t.resolution : "720p", a = wf(i, o);
...
function vf(t) {
  const e = Zs(t), r = e.find((a) => a.startsWith("data:image/")) ?? (e.length ? "" : Qc), n = /^data:(image\/(?:png|jpeg|webp|gif|bmp|avif));base64,([A-Za-z0-9+/]*={0,2})$/i.exec(r);
  if (!n || !n[2] || n[2].length % 4 === 1)
    throw new B("The LTX-2.5 reference image could not be prepared.", "INVALID_REFERENCE", 422);
  const i = n[2].endsWith("==") ? 2 : n[2].endsWith("=") ? 1 : 0;
  if (Math.floor(n[2].length * 0.75) - i > Ws)
    throw new B("The first LTX-2.5 reference image is larger than 14 MB.", "REFERENCE_TOO_LARGE", 413);
  return r;
}
function el(t) {
  if (typeof t != "string" || !Xs.includes(t))
    throw new B("Choose SDXL or Qwen Image Edit for this image job.", "INVALID_IMAGE_MODEL", 422);
  return t;
}
function Jo(t) {
  try {
...
function Ko(t, e) {
  ...
  if (!Number.isInteger(r) || r < 256 || r > 2048)
    throw new B("Image width and height must be whole pixels from 256 to 2048.", "INVALID_DIMENSIONS", 422);
...
function Tf(t) {
  ...
  if (!Number.isSafeInteger(e) || e < 0)
    throw new B("Image seed must be a non-negative whole number.", "INVALID_SEED", 422);
  return e;
}
function Ef(t, e) {
  ...
  if (!r || !r[2] || r[2].length % 4 === 1)
    throw new B(`Qwen reference image ${e + 1} could not be prepared.`, "INVALID_REFERENCE", 422);
  const n = r[2].endsWith("==") ? 2 : r[2].endsWith("=") ? 1 : 0;
  if (Math.floor(r[2].length * 0.75) - n > Ws)
    throw new B(`Qwen reference image ${e + 1} is larger than 14 MB.`, "REFERENCE_TOO_LARGE", 413);
  return t.trim();
}
function Sf(t, e) {
  const r = Array.isArray(t.referenceImages) ? t.referenceImages.filter((n) => typeof n == "string" && n.trim()) : [];
  if (e === "sdxl" && r.length)
    throw new B("SDXL session jobs are text-to-image and do not accept reference images.", "INVALID_REFERENCE", 422);
  if (e === "qwen-image-edit" && (r.length < 1 || r.length > 3))
    throw new B("Qwen Image Edit requires one to three reference images.", "INVALID_REFERENCE_COUNT", 422);
  return r.map(Ef);
}
function If(t, e) {
  const r = Ie(t, e);
  if (r.length > Di)
    throw new B(`${e} is too long.`, "PROMPT_TOO_LONG", 422);
  return r;
}
function xf(t, e, r, n, i, o) {
  const a = JSON.parse(JSON.stringify(Gc));
...
function Af(t, e, r, n) {
  const i = JSON.parse(JSON.stringify(zc));
...
function kf(t) {
  const e = el(t.model), r = If(t.prompt, "Image prompt"), n = typeof t.negativePrompt == "string" ? t.negativePrompt.trim().slice(0, Di) : e === "sdxl" ? "text, watermark, logo, low quality, distorted" : "", i = Ko(t.width, 1024), o = Ko(t.height, 1024), a = Tf(t.seed), s = Sf(t, e), l = e === "sdxl" ? xf(t, r, n, i, o, a) : Af(r, n, a, s);
  return {
    model: e,
    label: on(e),
    workflow: l,
...
function Pf(t, e) {
  const r = tl(t);
  if (r)
    throw new B(r, "GENERATION_FAILED", 502);
  for (const { record: n, parentKey: i } of an(t)) {
    ...
    if (u)
      return { data: u, mediaType: Fr(n), durationSec: e, model: "LTX-2.5" };
  }
  throw new B("LTX-2.5 completed without returning a video.", "INVALID_PROVIDER_RESPONSE", 502);
}
function Of(t, e) {
  const r = on(e), n = tl(t);
  if (n)
    throw new B(n, "GENERATION_FAILED", 502);
  for (const { record: i, parentKey: o } of an(t)) {
    ...
    if (!Rf(i, o))
      continue;
    const l = Ge(i.url, i.download_url, i.downloadUrl), d = Ge(i.data, i.base64), u = l ?? (String(i.type ?? "").toLowerCase() === "url" ? d : void 0);
    if (u)
      return { url: u, model: r };
    const c = String(i.type ?? "").toLowerCase() === "url" ? void 0 : d;
    if (c)
      return { data: c, mediaType: Fr(i, "image/png"), model: r };
  }
  throw new B(`${r} completed without returning an image.`, "INVALID_PROVIDER_RESPONSE", 502);
}
function rl(t, e = "video", r = "LTX-2.5") {
  for (const { record: n, parentKey: i } of an(t)) {
    if (i !== "artifact")
      continue;
    ...
    if (!/^[A-Za-z0-9_-]{1,191}$/.test(o) || !Number.isSafeInteger(a) || a <= 0 || a > Yc || s && !s.startsWith(`${e}/`))
      throw new B(`${r} returned invalid artifact metadata.`, "INVALID_PROVIDER_RESPONSE", 502);
    return { id: o, byteSize: a, mediaType: s };
  }
}
function Nf(t, e = "LTX-2.5", r = "video") {
  if (typeof t != "string" || !t || t.length % 4 === 1 || !/^[A-Za-z0-9+/]*={0,2}$/.test(t))
    throw new B(`${e} returned an invalid ${r} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
  if (typeof Buffer < "u")
    return new Uint8Array(Buffer.from(t, "base64"));
  if (typeof atob != "function")
    throw new B(`This CineGen runtime cannot decode the generated ${r}.`, "RUNTIME_UNSUPPORTED", 500);
  let n;
  try {
    n = atob(t);
  } catch {
...
function Cf(t, e, r = "video", n = "LTX-2.5") {
  ...
  if (r === "image") {
    ...
    if (l)
      return "image/webp";
    throw new B(`${n} returned an unsupported image file.`, "INVALID_PROVIDER_RESPONSE", 502);
  }
  if (!i && !o)
    throw new B(`${n} returned an unsupported video file.`, "INVALID_PROVIDER_RESPONSE", 502);
  return i ? "video/webm" : e === "video/quicktime" ? "video/quicktime" : "video/mp4";
}
function qf(t, e = "media") {
  if (typeof Buffer < "u")
    return Buffer.from(t.buffer, t.byteOffset, t.byteLength).toString("base64");
  if (typeof btoa != "function")
    throw new B(`This CineGen runtime cannot encode the generated ${e}.`, "RUNTIME_UNSUPPORTED", 500);
  const r = [], n = 32 * 1024;
...
async function nl(t, e, r, n, i = "video", o = "LTX-2.5") {
  const a = new Uint8Array(n.byteSize);
  let s;
  const l = [];
  for (let c = 0; c < n.byteSize; c += zo) {
    const p = Math.min(zo, n.byteSize - c);
    l.push({ offset: c, length: p });
  }
  for (let c = 0; c < l.length; c += Ho) {
    const p = l.slice(c, c + Ho), f = await Promise.all(p.map(async ({ offset: m, length: y }) => {
      ...
      if (!g || typeof g != "object")
        throw new B(`${o} returned an invalid ${i} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
      const _ = typeof g.id == "string" ? g.id : "", b = Number(g.offset), v = Number(g.byteSize ?? g.byte_size), T = typeof (g.mediaType ?? g.media_type) == "string" ? String(g.mediaType ?? g.media_type).trim() : "", S = Nf(g.data, o, i);
      if (_ !== n.id || b !== m || v !== n.byteSize || n.mediaType && T !== n.mediaType || S.byteLength !== y)
        throw new B(`${o} returned an inconsistent ${i} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
      return { offset: m, bytes: S };
    }));
    for (const m of f)
      m.offset === 0 && (s = m.bytes.slice(0, 12)), a.set(m.bytes, m.offset);
  }
  const d = Cf(s ?? new Uint8Array(), n.mediaType, i, o), u = qf(a, i);
  return await Lf(t, e, r, n.id), { data: u, mediaType: d };
}
function il(t) {
  if (!t || typeof t != "object")
    return !1;
...
async function Uf(t, e, r, n) {
  const i = await ol(t, e, r), o = i == null ? void 0 : i.capabilities, a = Array.isArray(i == null ? void 0 : i.installedModels) ? i.installedModels : [];
  ...
  if (!a.includes(n))
    throw new B(
      `${on(n)} was not installed when this Pod was created. Start a new session with that image model selected.`,
...
function Df(t) {
  return t instanceof B && (t.code === "PROVIDER_TIMEOUT" || t.code === "PROVIDER_UNREACHABLE" || t.statusCode === 502 || t.statusCode === 504);
}
...
async function al(t, e, r, n, i, o, a) {
  var c;
  const s = Mf(), l = ((c = i == null ? void 0 : i.capabilities) == null ? void 0 : c.idempotentSubmissions) === !0, d = JSON.stringify({ input: { ...o, cinegen_job_id: s } }), u = () => xe(t, `${e.url}/run`, {
    method: "POST",
  ...
  for (let p = 0; p < (l ? 2 : 1); p += 1)
    try {
      ...
      if (!m)
        throw new B("The session did not return a generation job ID.", "INVALID_PROVIDER_RESPONSE", 502);
      if (l && m !== s)
        throw new B("The session returned a different generation job ID.", "INVALID_PROVIDER_RESPONSE", 502);
      return { payload: f, jobId: m };
    } catch (f) {
      if (!l || !Df(f))
        throw f;
      const m = await $f(t, e, r, s);
      if (m)
        return { payload: m, jobId: s };
      if (p === 1)
        throw f;
    }
  throw new B("CineGen could not confirm the generation submission.", "PROVIDER_TIMEOUT", 504);
}
async function Ff(t, e = fetch) {
  ...
  if (t.jobId) {
    const d = rn(t.jobId, "RunPod generation job ID"), u = await xe(e, `${r.url}/status/${d}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    ...
    if (c === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: nn(u, "LTX-2.5 generation failed.") };
    if (c === "COMPLETED") {
      const p = Math.min(20, Math.max(1, Math.round(Number(u.durationSec) || 5)));
      try {
        const f = rl(u.output), m = f ? { ...await nl(e, r, n, f), durationSec: p, model: "LTX-2.5" } : Pf(u.output, p);
  ...
  if (!t.input)
    throw new B("Video generation input is required.", "MISSING_INPUT", 422);
  const o = Math.min(20, Math.max(1, Math.round(Number(t.input.durationSec) || 5))), a = {
    workflow: bf(t.input),
    images: [{ name: "cinegen-source.png", image: vf(t.input) }],
    cinegen_duration_sec: o,
    cinegen_task: "ltx-2.5"
  }, s = await ol(e, r, n), { jobId: l } = await al(e, r, n, i, s, a, "CineGen could not submit the LTX-2.5 generation.");
  return { jobId: l, status: "queued", phase: "rendering", message: "LTX-2.5 generation queued." };
}
async function Bf(t, e = fetch) {
  var l;
  const r = Bi(t.podUrl, t.podId), n = Ie(t.podAuthToken, "RunPod session token"), i = { Authorization: `Bearer ${n}`, Accept: "application/json", "Content-Type": "application/json" };
  if (t.jobId) {
    const d = rn(t.jobId, "RunPod generation job ID"), u = await xe(e, `${r.url}/status/${d}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    }, "CineGen could not read the session image generation status."), c = Jo(u == null ? void 0 : u.task), p = Jo(t.model ?? ((l = t.input) == null ? void 0 : l.model));
    if (c && p && c !== p)
    ...
    const m = on(f), y = String(u.status ?? "").toUpperCase();
    if (y === "IN_QUEUE")
    ...
    if (y === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: nn(u, `${m} generation failed.`) };
    if (y === "COMPLETED")
      try {
        const h = rl(u.output, "image", m), g = h ? { ...await nl(e, r, n, h, "image", m), model: m } : Of(u.output, f);
        return { jobId: d, status: "completed", phase: "ready", output: g };
      } catch (h) {
        return { jobId: d, status: "failed", phase: "error", error: h instanceof Error ? h.message : `${m} generation failed.` };
      }
    return { jobId: d, status: "in_progress", phase: "rendering", message: `${m} is preparing the image…` };
  }
  if (!t.input)
    throw new B("Image generation input is required.", "MISSING_INPUT", 422);
  const o = kf(t.input), a = await Uf(e, r, n, o.model), { jobId: s } = await al(e, r, n, i, a, {
    workflow: o.workflow,
    images: o.images,
...
const Vf = yf, Gf = _f, zf = Qs, Hf = Ff, Wf = Bf, Tt = "https://api.kie.ai/api/v1", Xf = 3e3, Jf = 120, Kf = {
...
function Yf(t) {
  for (const [e, r] of Object.entries(Kf))
    if (t.startsWith(e)) return r;
}
async function Qf(t, e, r) {
  const n = Yf(t), i = n ?? `${Tt}/jobs/createTask`, o = n ? { ...e, callBackUrl: "" } : { model: t, input: e, callBackUrl: "" }, a = await fetch(i, {
    method: "POST",
    headers: {
...
async function Zf(t, e) {
  for (let r = 0; r < Jf; r++) {
    await new Promise((a) => setTimeout(a, Xf));
...
function lr(t, e, r = !1) {
  if (typeof t == "string") {
    const a = sl(t).trim(), s = r ? ip(a, e) : e;
...
function ap(t, e, r) {
  const n = [], i = {}, o = t === "seedance_2_5" ? "image" : op(r);
  for (const [a, s] of Object.entries(e)) {
    if (s == null) continue;
...
async function lp(t, e, r) {
  const n = await oi(ap(t, e, r));
  return sp(n);
}
const Yo = "https://api.runpod.ai/v2", dp = 3e3, up = 120;
async function cp(t, e, r) {
  ...
  for (let o = 0; o < up; o++) {
    await new Promise((l) => setTimeout(l, dp));
    ...
    if (s.status === "COMPLETED") {
      ...
      if (d && !d.startsWith("http") && !d.startsWith("local-media://")) {
        const u = d.includes(",") ? d.split(",")[1] : d, c = w.join(H.tmpdir(), `cinegen-runpod-${Date.now()}.png`);
        return await j.writeFile(c, Buffer.from(u, "base64")), { output: { ...l, image_url: `local-media://file${c}` } };
      }
      return { output: l };
    }
    if (s.status === "FAILED")
...
const It = 14 * 1024 * 1024, Zo = 100 * 1024 * 1024, ea = 100 * 1024 * 1024, mp = 45e3;
function Gi(t) {
...
async function ll(t) {
  ...
  if (/^https?:\/\//i.test(t)) {
    const e = hp(t), r = new AbortController(), n = setTimeout(() => r.abort(), mp);
    let i;
    try {
...
async function yp(t) {
  ...
  return {
    ...t,
    referenceImages: e ? [await ll(e)] : void 0
  };
}
function _p(t, e) {
  var a;
...
async function wp(t) {
  ...
  const r = await Promise.all(e.map(async (n, i) => {
    const o = n.trim().startsWith("data:") ? n : await ll(n.trim());
    return _p(o, i);
  }));
  return {
    ...t,
...
async function bp(t) {
  ...
  const o = await j.mkdtemp(w.join(H.tmpdir(), "cinegen-ltx25-")), a = w.join(o, `result${i}`);
...
async function vp(t) {
  ...
  const s = await j.mkdtemp(w.join(H.tmpdir(), "cinegen-runpod-image-")), l = w.join(s, `result${a}`);
...
async function ra(t) {
  const e = decodeURIComponent(t.replace("local-media://file", "")), r = await j.readFile(e), n = Tp(e), i = new Blob([r], { type: n }), o = new File([i], w.basename(e), { type: n });
  return J.fal.storage.upload(o);
}
async function Br(t) {
  const e = {};
...
async function na(t, e, r) {
  var n;
  si(r), console.log("[fal] Calling model:", t, "with input:", JSON.stringify(e, null, 2));
  try {
    return await J.fal.subscribe(t, { input: e, logs: !0 });
  } catch (i) {
...
function Ep() {
  I.handle("workflow:run", async (e, r) => {
    const {
      ...
      outputType: c,
      inputs: p
    } = r, { ALL_MODELS: f, resolveVideoModelEndpoint: m, sanitizeVideoInputsForEndpoint: y } = await import("./models-CkrrX5Ho.js"), h = f[u] ?? Object.values(f).find(
      (E) => E.id === u || E.altId === u || E.nodeType === u
    );
    if (!h) {
      if (u.startsWith("fal-ai/")) {
        ...
        si(E);
        const k = await Br(p), O = await na(u, k, E);
        return O.data ?? O;
      }
    ...
    const g = h.provider;
    let _ = p;
    g !== "higgsfield" && (n && si(n), _ = await Br(p));
    let b = u.includes("/") ? u : h.id;
    const v = h.nodeType ?? u, T = Object.keys(_).some(
    ...
    if (g === "kie") {
      ...
      const k = a || h.runpodEndpointId || "";
      S = await cp(k, _, E);
    } else if (g === "higgsfield") {
      const E = h.outputType;
      S = await lp(b, _, c ?? (E === "video" ? "video" : E === "audio" ? "audio" : E === "text" ? "text" : E === "3d" || E === "model3d" || E === "model" ? "3d" : "image"));
    } else {
      const E = n;
      if (!E) throw new Error("No fal.ai API key provided. Add one in Settings.");
      S = await na(b, _, E);
  ...
  I.handle("workflow:poll-job", async (e, r) => {
    ...
    return n;
  }), I.handle("pod:start", async (e, r) => await Qo(r.runpodKey, r.podId, "start")), I.handle("pod:stop", async (e, r) => await Qo(r.runpodKey, r.podId, "stop")), I.handle("pod:status", async (e, r) => await pp(r.runpodKey, r.podId)), I.handle("pod:setup-ltx25", async (e, r) => await Vf(r)), I.handle("pod:status-ltx25", async (e, r) => await Gf(r)), I.handle("pod:terminate-ltx25", async (e, r) => await zf(r)), I.handle("pod:generate-ltx25", async (e, r) => {
    const n = r.input ? await yp(r.input) : void 0;
    return await bp(await Hf({ ...r, input: n }));
  }), I.handle("pod:generate-session-image", async (e, r) => {
    const n = r.input ? await wp(r.input) : void 0;
    return await vp(await Wf({ ...r, input: n }));
  });
}
const Ot = Ut(qe), dl = "https://mcp.artlist.io/mcp", Nt = "artlist", Sp = 1200 * 1e3, Ip = [
  w.join(H.homedir(), ".local/bin/claude"),
...
function Ct() {
  ...
  return {
    ...process.env,
    PATH: [w.join(H.homedir(), ".local/bin"), "/opt/homebrew/bin", "/usr/local/bin", t].filter(Boolean).join(w.delimiter)
  };
}
async function Or() {
  for (const t of Ip)
...
function kp(t) {
  ...
  for (const d of n) {
    ...
    if (c)
      try {
        i.unshift(...dr(JSON.parse(c)));
      } catch {
      }
  }
  for (const d of i) {
    const u = rt(d, ["url", "videoUrl", "video_url", "downloadUrl", "download_url", "mediaUrl", "media_url"]);
...
function Pp(t, e = process.platform, r = "/tmp/cinegen-artlist-login.command") {
  if (e === "darwin") {
    const n = [
      "#!/bin/zsh",
...
async function Cp(t, e) {
  const r = w.join(z.getPath("userData"), "artlist-mcp-workspace");
  await st(r, { recursive: !0 });
  const n = Ap(e), { stdout: i } = await Ot(t, [
    "-p",
    n,
    "--output-format",
    "json",
...
function qp() {
  I.handle("artlist:account-status", async () => {
    ...
    const e = Pp(
      t,
      process.platform,
      w.join(z.getPath("userData"), "artlist-login.command")
    );
    try {
      e.script && (await ts(e.script.path, e.script.contents, { mode: 448 }), await Cd(e.script.path, 448)), await Ot(e.file, e.args, {
        env: Ct(),
...
function Lp(t, e, r, n) {
  ...
  if (t === "Image Upscale" || t === "Video Upscale") throw new Error(ul(t));
...
const cl = ["Avatar 4", "Avatar 4 Fast", "Video Lip Sync", "Image Upscale", "Video Upscale"];
function Up(t) {
...
function $p(t, e) {
  const r = Dp(e), n = Array.from(String(t ?? "")).length;
  if (r !== void 0 && n > r)
...
function Bp(t, e) {
  if (!pl(t.model)) throw new Error("Topview Clip Edit is available for Seedance 2.5. Choose that model or switch back to generation.");
...
function Gp(t) {
  ...
  if (i)
    for (const o of n(i.start, i.end).filter((a) => a.type === "trak")) {
      ...
      if (!d) continue;
      const u = t[d.start], c = u === 0 ? 76 : u === 1 ? 88 : -1;
      if (c < 0 || d.start + c + 8 > d.end) continue;
      const p = e.getUint32(d.start + c) / 65536, f = e.getUint32(d.start + c + 4) / 65536;
...
function zp(t) {
  ...
  if (e.width * e.height < Vp) throw new Error(`This source video is ${e.width}×${e.height}, below Topview’s Clip Edit minimum of 407,696 pixels per frame. Use a 720p or larger source video. Choosing 1080p output does not resize the input. No generation was submitted.`);
}
const Hp = [
...
function oa(t) {
  return Hp.every((e) => t.includes(e));
}
function we(t) {
  var r, n, i;
...
const li = "cinegen-canvas:";
function cr(t) {
  if (!(typeof t != "string" || !t.startsWith(li))) {
    try {
      const e = JSON.parse(atob(t.slice(li.length).replace(/-/g, "+").replace(/_/g, "/")));
...
function Wp(t) {
  const e = li + btoa(JSON.stringify(t)).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  return cr(e), e;
}
function Ne(t) {
...
function Xp(t, e, r) {
  ...
  if (n && r.filter((c) => Ne(c) === "video").length !== 1) throw new Error("Clip Edit needs exactly one source video.");
  const i = t.inputRoles ?? t.inputs ?? [], o = {};
  for (const c of r) {
    const p = `reference_${Ne(c)}`;
    if (o[p] = (o[p] ?? 0) + 1, !i.some((f) => f.role === p)) throw new Error(`Topview ${t.displayName ?? t.model} does not accept ${Ne(c)} references in this mode.`);
  }
  for (const c of i) {
    const p = o[c.role] ?? 0;
    if (p < (c.min ?? 0) || p > (c.max ?? 1 / 0)) throw new Error(`Topview ${t.displayName ?? t.model} allows ${c.min ?? 0}–${c.max} ${c.role.replace("reference_", "")} references in this mode.`);
  }
  for (const c of t.constraints ?? [])
    if (c.type === "require_any_roles" && !c.roles.some((p) => o[p]))
  ...
  for (const [c, p] of Object.entries(l)) {
    ...
    if (!Object.hasOwn(a, c)) throw new Error(`Topview Canvas does not expose the requested ${c} setting for this model.`);
    const f = a[c].enum ?? ((u = t.parameterEnums) == null ? void 0 : u[c]), m = a[c].type === "number" ? Number(String(p).replace(/p$/i, "")) : p;
    if (f && !f.includes(m)) throw new Error(`Topview Canvas does not support ${c}=${p}. Available: ${f.join(", ")}.`);
    s[c] = m;
  }
  for (const c of t.requiredParameters ?? [])
    if (s[c] === void 0) throw new Error(`Topview requires ${c} for this generation.`);
  return s;
}
async function aa(t) {
  var y, h, g;
  const { call: e, request: r, references: n } = t;
  $p(r.prompt, t.submitSchema);
  ...
  const d = Xp(l, r, n), c = (we(await e("get_topview_canvas_state", { canvasId: o, fields: ["nodes.geometry"] })).nodes ?? []).reduce((_, b) => {
    var v, T, S, x;
    return Math.max(
      _,
  ...
  for (const [_, b] of n.entries()) {
    ...
    if (r.omniReferenceTaskType === "edit" && Ne(b) === "video" && zp(v.bytes), Ne(b) === "audio" && (!["mp3", "wav"].includes(v.format.toLowerCase()) || v.bytes.byteLength > 15 * 1024 * 1024))
  ...
  return {
    taskId: Wp({ canvasId: o, nodeId: m.nodeId, taskId: m.taskId }),
    status: "running",
    model: r.model,
...
const ml = ns(import.meta.url);
function hl(t) {
  return z.isPackaged ? t.replace("app.asar", "app.asar.unpacked") : t;
}
function ve() {
  const t = ml("ffmpeg-static");
  return hl(t);
}
function Hi() {
  const t = ml("ffprobe-static").path;
  return hl(t);
}
function gl() {
  if (z.isPackaged)
    return w.join(process.resourcesPath, "vendor", "fpcalc");
  const t = w.dirname(Qr(import.meta.url));
  return w.resolve(t, "..", "vendor", "fpcalc", "fpcalc");
}
const yl = Ut(qe), Kp = 20 * 1e3, Yp = 300 * 1e3;
function Qp(t) {
...
async function em(t) {
  try {
    ...
    const e = await j.mkdtemp(w.join(H.tmpdir(), "cinegen-probe-"));
    try {
      const r = w.join(e, `reference.${t.format}`);
      return await j.writeFile(r, t.bytes), await sa(r);
    } finally {
...
async function _l(t, e) {
  const r = await j.mkdtemp(w.join(H.tmpdir(), "cinegen-resize-"));
  try {
...
], lm = ["16:9", "9:16", "1:1", "4:3", "3:4"], dm = ["1K", "2K", "4K"], um = ["16:9", "9:16", "1:1", "4:3", "3:4", "21:9"], cm = ["720", "1080"], fm = Array.from({ length: 27 }, (t, e) => String(e + 4));
function ke(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function di(t, e = []) {
  return Array.isArray(t) ? t.forEach((r) => di(r, e)) : ke(t) && (e.push(t), Object.values(t).forEach((r) => di(r, e))), e;
}
function pm(t) {
  for (const e of di(t))
...
function hm(t, e) {
  ...
  return (Array.isArray(n) ? n : ke(n) ? ["values", "options", "enum", "allowedValues"].map((o) => n[o]).find(Array.isArray) ?? [] : []).map(mm).filter((o) => o != null);
}
function wl(t) {
  ...
  for (const r of (t == null ? void 0 : t.configs) ?? [])
    if (!(r.outputType !== "image" && r.outputType !== "video" && r.outputType !== "audio"))
      for (const n of pm(r.config)) {
        ...
        for (const s of gm(n)) a.accepts.add(s);
        ...
        for (const s of ["aspectRatio", "resolution", "duration", "quality", "sound"]) {
          const l = hm(n, s);
          l.length && (a.accepts.add(s), a.options.set(s, wl([...a.options.get(s) ?? [], ...l])));
        }
        e.set(o, a);
      }
  return [...e.values()];
...
function la(t, e) {
  ...
  return wl(r).map((n) => ({ value: String(n), label: String(n) }));
}
function gm(t) {
...
function ym(t) {
  const e = xt(t, "aspectRatio", lm), r = xt(t, "resolution", dm), n = xt(t, "quality", []), i = t.taskTypes.size === 0 || t.taskTypes.has("text_to_image"), o = t.taskTypes.size === 0 || t.taskTypes.has("image_edit"), a = [
  ...
  return o && a.push(
    ...
    nodeType: `topview-image-${sn(t.displayName)}`,
    name: t.displayName,
...
function _m(t) {
  const e = pl(t.submitModel ?? t.displayName), r = xt(t, "aspectRatio", um), n = xt(t, "resolution", cm), i = xt(t, "duration", fm), o = t.options.get("sound") ?? [], a = t.nativeAudio === !0 || o.some((p) => String(p).toLowerCase() === "on"), s = t.taskTypes.size === 0 || t.taskTypes.has("omni_reference"), l = s && (t.audioReferences ?? zi(t)), d = t.taskTypes.size === 0 || t.taskTypes.has("image_to_video"), u = [...t.taskTypes].map((p) => p.replaceAll("_", " ")).join(" · "), c = [
    { id: "prompt", portType: "text", label: "Prompt", required: !0, falParam: "prompt", fieldType: "port" }
  ];
  return e && c.push(
    {
  ...
  })) : d && c.push({ id: "image_url", portType: "image", label: "Start Frame", required: !1, falParam: "image_url", fieldType: "port", mediaRole: "start_image" }), d && s && c.push(
    ...
    nodeType: `topview-video-${sn(t.displayName)}`,
    name: t.displayName,
    category: "video",
    description: `Topview video generation${l ? " with image, video, and audio input references" : ""}${a ? " with native audio" : ""}${u ? ` · ${u}` : ""}`,
    outputType: "video",
    provider: "topview",
    responseMapping: { path: "url" },
    inputs: c
...
function wm(t) {
  ...
  return e === "music" ? r.push(
    ...
    nodeType: `topview-audio-${sn(t.displayName)}`,
    name: t.displayName,
    category: "audio",
...
function bm() {
  return [
    ...
    ...sm.map(({ displayName: t, catalogType: e }) => ({
      displayName: t,
      outputType: "audio",
      catalogType: e,
...
function fb(t) {
  const e = bl(t), n = (e.length ? e : bm()).map((i) => i.outputType === "image" ? ym(i) : i.outputType === "video" ? _m(i) : wm(i));
  return Object.fromEntries([...n, ...Mp(t == null ? void 0 : t.tools, (t == null ? void 0 : t.authMode) === "api_key")].map((i) => [i.nodeType, i]));
}
const da = -1, vm = "[-\\u2010-\\u2015\\u2212]", Tm = new RegExp(
  `duration\`?\\s*(?:must|should)\\s+be\\s*\`?\\s*${vm}\\s*1\\b`,
  "i"
), Em = /task\s+as\s+video\s+editing|duration\s+follows?\s+the\s+input\s+video/i;
function vl(t) {
  return /duration/i.test(t) ? Tm.test(t) || Em.test(t) : !1;
}
const Sm = "https://mcp.topview.ai/mcp", On = "https://mcp.topview.ai", Im = "https://www.topview.ai/mcp_oauth/oauth/authorize", xm = "https://www.topview.ai/mcp_oauth/oauth/token", Am = "https://www.topview.ai/mcp_oauth/oauth/register", ua = "https://www.topview.ai/mcp_oauth/oauth/userinfo", km = 300 * 1e3, Nn = 1200 * 1e3, jm = 90 * 1e3, Rm = 30 * 1e3, qt = 45 * 1024 * 1024, ca = 50;
function F(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function Cn(t) {
...
class Om {
  constructor() {
    this.root = w.join(z.getPath("userData"), "integrations", "topview");
  }
  availabilityError() {
    try {
...
function Vr(t, e = [], r = 0) {
  ...
  else if (F(t)) for (const n of Object.values(t)) Vr(n, e, r + 1);
  return e;
}
function ie(t) {
  const e = [t];
...
function El(t, e) {
  ...
  return r.includes("array") && Array.isArray(t) ? t.map((n) => El(n, e.items)) : r.includes("object") && F(t) ? Sl(e, t) : t;
}
function Sl(t, e) {
  if (!F(t)) return { ...e };
  ...
  for (const [l, d] of Object.entries(e))
    a && !Object.hasOwn(o, l) || (s[l] = El(d, o[l]));
  return s;
}
function qm(t, e) {
  var i;
  const r = F((i = t.inputSchema) == null ? void 0 : i.properties) ? t.inputSchema.properties : {}, n = Sl(t.inputSchema, e);
...
function Lm(t) {
  const r = (Il(t, /^(?:boards|list|items|records|data|rows)$/i) ?? []).filter(F).map((i) => ({
...
function Um(t) {
  return (Il(t, /^models$/i) ?? []).filter(F);
}
function xl(t, e) {
  const r = t.submitParameterOptions;
  if (F(r)) {
    const n = r[e];
    if (Array.isArray(n)) return n.map(Mn);
    if (F(n)) {
      for (const i of ["values", "options", "enum", "allowedValues"])
        if (Array.isArray(n[i])) return n[i].map(Mn);
    }
  }
  if (Array.isArray(r)) {
...
function fr(t, e) {
  ...
  return Object.hasOwn(r, e) || Wi(t).includes(e) || Mm(t).includes(e);
}
function ci(t) {
  if (t.nativeAudio === !1 || t.supportsNativeAudio === !1) return !1;
  if (t.nativeAudio === !0 || t.supportsNativeAudio === !0) return !0;
  const e = xl(t, "sound");
  if (e.length) return jl(e, "on") !== void 0;
  if ((F(t.defaultSubmitParameters) ? t.defaultSubmitParameters : {}).sound === "on" || fr(t, "sound")) return !0;
}
function kt(t) {
...
function ha(t) {
  var a;
  const e = t.references.length ? "image_edit" : "text_to_image", r = Al(t.config, t.params.model), n = (a = kt(r)[0]) == null ? void 0 : a.trim();
  ...
  for (const [s, l, d] of [
    ...
    fr(r, s) && (o[s] = fi({ model: r, field: s, requested: l, fallback: d }));
  for (const s of Wi(r))
...
const $m = "Keep the frame free of on-screen text, captions, and subtitles.";
function kl(t) {
...
function Dn(t) {
  var p, f;
  const e = Al(t.config, t.params.model, t.params.generateAudio === !0), r = String(e.submitModel ?? "").trim();
  if (!r) throw new Error("Topview returned a video model without a submit identifier.");
  const n = F(e.defaultSubmitParameters) ? e.defaultSubmitParameters : {}, i = new Set(Wi(e)), o = t.params.durationSec === void 0 ? void 0 : Math.round(t.params.durationSec);
  if (o !== void 0 && (!Number.isFinite(o) || o === 0 || o < -1))
    throw new Error("Topview video duration must be a positive whole number of seconds.");
  let a = kl(t.params.prompt);
  const s = {
    ...n,
    taskType: t.taskType,
    model: r,
  ...
  const l = (m, y, h) => {
    ...
    const _ = fi({ model: e, field: m, requested: y, fallback: h, required: i.has(m) });
    _ !== void 0 && (s[m] = _);
  };
  if (l("resolution", t.params.resolution === void 0 ? void 0 : Number.parseInt(t.params.resolution, 10), 720), t.inheritInputVideoDuration || o === da ? s.duration = da : fr(e, "duration") && l("duration", o, 5), l("generatingCount", void 0, 1), t.taskType !== "image_to_video" ? l("aspectRatio", (p = t.params.aspectRatio) == null ? void 0 : p.trim(), "16:9") : (t.params.aspectRatio !== void 0 || i.has("aspectRatio")) && l("aspectRatio", (f = t.params.aspectRatio) == null ? void 0 : f.trim(), "16:9"), ci(e) !== !1 && (fr(e, "sound") || t.params.generateAudio === !0))
    s.sound = fi({
      model: e,
      field: "sound",
...
${a}`, s.prompt = a, g.length && (s.inputImages = g), _.length && (s.inputVideos = _), b.length && (s.inputAudios = b);
  ...
  const u = Number(s.duration ?? n.duration), c = Number.isFinite(u) && u > 0 ? u : void 0;
...
function $n(t, e, r) {
  const n = (r ?? "").split(";", 1)[0].trim().toLowerCase(), i = n ? Vm[n] : void 0, o = (() => {
    try {
  ...
  if (!s || !ga(e).has(s)) {
    ...
    throw new Error(`Topview received an unsupported ${l} reference format. Supported formats: ${[...ga(e)].join(", ")}.`);
  }
  if (n && !i)
...
function zm(t) {
  const e = t.startsWith("[") && t.endsWith("]") ? t.slice(1, -1) : t, r = Si(e.split("%", 1)[0]);
  if (r === 4) return ya(e);
  if (r !== 6) return !1;
  const n = Gm(e);
...
async function Hm(t) {
  ...
  const n = Si(e), i = n ? [{ address: e, family: n }] : await Ud(e, { all: !0, verbatim: !0 });
  if (!i.length || i.some((o) => !zm(o.address)))
...
async function Pl(t, e = 0) {
  ...
  return new Promise((i, o) => {
    const a = Md({
      ...
      method: "GET",
      servername: Si(r.hostname.replace(/^\[|\]$/g, "")) ? void 0 : r.hostname,
      headers: {
      ...
      if (l >= 300 && l < 400) {
        ...
        Pl(new URL(p, r).href, e + 1).then(i, o);
        return;
      }
      if (l < 200 || l >= 300) {
...
async function Fn(t, e) {
  ...
  if (n) {
    ...
    const s = $n(n, e);
    return { bytes: await j.readFile(n), format: s, contentType: pr[s], filePath: n };
  }
  const i = await Pl(r), o = $n(i.finalUrl, e, i.contentType);
  return { bytes: i.bytes, format: o, contentType: pr[o] };
}
const Wm = 407696, Gr = 409600;
function Ol(t) {
  const e = sn(t);
  return e.includes("seedance-2-5") ? Wm : e.includes("seedance-2") ? Gr : void 0;
}
function mi(t) {
  return t.toLocaleString("en-US");
}
function Xm(t) {
  const e = Ol(t.submitModel);
  if (e === void 0) return;
  const r = t.width * t.height;
  if (!(r >= e))
    return `This reference video is ${t.width}x${t.height}, which is ${mi(r)} pixels per frame. "${t.submitModel}" requires at least ${mi(e)}. Re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice), then attach it again.`;
}
function _a(t) {
  var e;
  if (/video\s+pixel\s+count/i.test(t)) {
    ...
    return `Seedance rejected a reference video for being too small. It needs at least ${mi(n)} pixels per frame, so re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice) and attach it again. A 640x360 clip is the usual cause.`;
  }
  if (vl(t))
...
function Jm(t) {
  var r;
  const e = (r = lt(t).find((n) => F(n.headers))) == null ? void 0 : r.headers;
...
class Km {
  ...
  async tokenExchange(e, r) {
    ...
    return n.set("client_id", r.client_id), r.client_secret && n.set("client_secret", r.client_secret), Un(xm, {
      method: "POST",
  ...
  async teamConnection() {
    ...
    try {
      const n = JSON.parse(await j.readFile(
        w.join(z.getPath("home"), ".topview", "credentials.json"),
        "utf8"
      ));
      if (F(n) && typeof n.api_key == "string" && n.api_key.trim() && typeof n.uid == "string" && n.uid.trim())
        return {
  ...
  async mcpRequest(e, r, n) {
    var a;
    const i = new AbortController(), o = setTimeout(() => i.abort(), jm);
    (a = o.unref) == null || a.call(o);
    try {
      const s = await fetch(Sm, {
        method: "POST",
        headers: {
        ...
        signal: i.signal
      }), l = await s.text(), d = (s.headers.get("content-type") || "").includes("text/event-stream") ? Nm(l, r.id) : l ? (() => {
        try {
          return JSON.parse(l);
        } catch {
          return l;
      ...
      if (!s.ok) throw ui(s.status, d, "Topview MCP request failed.");
      if (F(d) && d.error !== void 0) throw ui(400, d.error, "Topview MCP returned an error.");
      return {
        payload: F(d) ? d : {},
  ...
  async callTool(e, r, n) {
    ...
    if (F(a) && a.isError === !0) {
      const s = ie(a), d = (re(s, [
        ...
        "failure_reason",
        "message"
      ]) ?? Vr(a).join(" ").slice(0, 700)).trim() || `Topview could not run ${r}.`, u = _a(d);
      throw new Error(u ? `${d}
...
${u}` : d);
  ...
  async uploadReference(e, r, n) {
    ...
    let i = await Fn(r.value, r.role), o;
    if (r.role === "video" && n && Ol(n) !== void 0) {
      const f = await em(i), m = f && Xm({ submitModel: n, ...f });
      if (f && m) {
        const y = Zp(f, Gr);
        try {
          const h = await tm(i, y);
          if (h.length > qt)
            throw new Error("The resized reference exceeds CineGen's 45 MB Topview upload safety limit.");
          i = { bytes: h, format: "mp4", contentType: pr.mp4 }, o = `Upscaled reference video ${f.width}x${f.height} → ${y.width}x${y.height}`, console.info(
...
CineGen could not create the temporary resized copy automatically.`);
    ...
    const u = (re(s, ["method", "httpMethod", "http_method"]) || "PUT").toUpperCase(), c = await fetch(d, {
      method: u,
      headers: { ...Jm(s), ...i.contentType ? { "Content-Type": i.contentType } : {} },
      body: i.bytes
    });
    ...
    if (Cm(ie(p)) === !1) throw new Error("Topview could not verify an uploaded element reference.");
  ...
  async accountStatus() {
    ...
    try {
      ...
      const n = await this.accessToken(), i = await Un(ua, {
        method: "GET",
      ...
      return { connected: !1, configured: !0, error: qn(r, "Topview connection expired.") };
    }
  }
  async modelCatalog() {
  ...
  async authLogin() {
    ...
    const i = `http://127.0.0.1:${n.port}/oauth/callback`, o = Cn(X.randomBytes(48)), a = Cn(X.createHash("sha256").update(o).digest()), s = Cn(X.randomBytes(32));
    let l, d;
    const u = new Promise((f, m) => {
      d = m, r.on("request", (y, h) => {
    ...
    }), c = setTimeout(() => d == null ? void 0 : d(new Error("Topview sign-in timed out. Try connecting again.")), km);
    ...
    try {
      ...
      const h = await u, g = h.searchParams.get("error_description") || h.searchParams.get("error");
      ...
      try {
        ...
        T.ok && F(S) && await this.store.write("profile", S);
      } catch {
      }
      return fa(l, !0, "You can close this window and return to CineGen."), this.accountStatus();
    } catch (f) {
      throw l && !l.writableEnded && fa(l, !1, qn(f, "Topview sign-in did not complete.")), f;
    } finally {
  ...
  async submitVideoRequest(e, r) {
    ...
    try {
      ...
      const o = Dn({ ...r, inheritInputVideoDuration: !0 });
      return {
        built: o,
  ...
  async submitWithSession(e, r) {
    var f, m, y;
    if (Up(r == null ? void 0 : r.model)) {
      const h = fl(r.model, e.tools.map((x) => x.name));
      if (h) throw new Error(h);
      const g = pi(r.medias);
      ia(r.model, r.prompt ?? "", g);
      const _ = [];
      for (const x of g) _.push(await this.uploadReference(e, x));
    ...
    if (Fp({ ...r })) {
      const h = Bp({ ...r }, n);
      Pn(void 0, h, !1, oa(e.tools.map((_) => _.name)));
      const g = await aa({
        ...
        load: async (_) => {
          const b = await Fn(_.value, _.role);
    ...
    const i = Bm(r.medias), o = await this.chooseBoard(e), a = ie(await this.callTool(e, "topview_get_generation_config", {
      type: "video",
      taskType: i
    })), s = Dn({
      config: a,
      ...
      params: r,
      boardId: o,
      references: n.map((h, g) => ({ ...h, fileId: `preflight-${g + 1}` }))
    });
    if (Pn(
      (m = e.tools.find((h) => h.name === "topview_generate_video")) == null ? void 0 : m.inputSchema,
      s.req,
      !1,
      oa(e.tools.map((h) => h.name))
    ) === "canvas-mcp") {
      const h = await aa({
        call: (g, _) => this.callTool(e, g, _),
        references: n,
        submitSchema: (y = e.tools.find((g) => g.name === "submit_topview_canvas_generation_task")) == null ? void 0 : y.inputSchema,
        request: { ...s.req, prompt: r.prompt, ...r.generateAudio !== void 0 ? { sound: r.generateAudio } : {} },
        load: async (g) => {
          const _ = await Fn(g.value, g.role);
    ...
    const d = [];
    for (const h of n)
      d.push(await this.uploadReference(e, h, s.model));
    const { built: u, documents: c } = await this.submitVideoRequest(e, {
      config: a,
      taskType: i,
      params: r,
      references: d,
      boardId: o
    }), p = re(c, ["taskId", "task_id", "generationId", "generation_id"]);
    ...
    return {
      result: {
        ...
        ...d.some((h) => h.preparation) ? {
          referencePreparation: d.flatMap((h) => h.preparation ?? []).join("; ")
        } : {}
      },
      documents: c
    };
  ...
  async queryWithSession(e, r) {
    ...
    const n = await this.callTool(e, "topview_query_task", {
      ...
      needCloudFrontUrl: !0,
      shortenUrls: !1
    }), i = ie(n), o = Zt(i), a = ht(i), s = /fail|error|cancel/.test(o), l = !!a || /success|complete|done/.test(o), d = s ? "fail" : l && a ? "success" : l ? "fail" : /^(init|created|queued)$/.test(o) ? "init" : "running", u = re(i, ["boardTaskId", "board_task_id"]), c = re(i, [
      "errorMsg",
      "error_msg",
      "errorMessage",
      "error_message",
      "failureReason",
      "failure_reason"
    ]), p = c ? _a(c) : void 0, f = d !== "fail" ? void 0 : c ? p ? `${c}
...
${p}` : c : l ? "Topview completed the task without returning a video URL." : "Topview could not complete this video.";
    return {
      ...
      ...a ? { url: a } : {},
      ...f ? { error: f } : {},
      ...cr(r.taskId) ? {} : { boardUrl: `https://www.topview.ai/board/${encodeURIComponent(r.boardId)}${u ? `?boardResultId=${encodeURIComponent(u)}` : ""}` }
    };
  }
  async query(e) {
  ...
  async generate(e) {
    const r = await this.session(), n = await this.submitWithSession(r, e), i = ht(n.documents);
    if (i)
      return {
        url: i,
        mediaType: "video",
  ...
  async generateImage(e) {
    ...
    const r = await this.session(), n = Dm(e.medias), i = n.length ? "image_edit" : "text_to_image", o = await this.chooseBoard(r), a = ie(await this.callTool(r, "topview_get_generation_config", {
      type: "image",
      taskType: i
    }));
    ha({
    ...
    const l = ha({ config: a, params: e, references: s, boardId: o }), d = await this.callTool(r, "topview_generate_image", l.req), u = ie(d), c = re(u, ["taskId", "task_id", "generationId", "generation_id"]), p = ht(u);
    if (p)
      return {
        url: p,
        mediaType: "image",
        taskId: c,
        model: l.model,
        referenceValue: ma(u) ?? await this.reusableGeneratedImageReference(r, p),
    ...
    if (/fail|error|cancel/.test(f))
      throw new Error(re(u, ["errorMsg", "error_msg", "errorMessage", "error_message"]) ?? "Topview could not complete this image.");
    const m = Date.now() + Nn;
    for (; Date.now() < m; ) {
      ...
      const y = await this.callTool(r, "topview_query_task", {
        ...
        taskId: c,
        needCloudFrontUrl: !0
      }), h = ie(y), g = ht(h), _ = Zt(h);
      if (g) {
        const b = re(h, ["boardTaskId", "board_task_id"]);
        return {
          url: g,
          mediaType: "image",
          taskId: c,
          model: l.model,
          referenceValue: ma(h) ?? await this.reusableGeneratedImageReference(r, g),
          boardUrl: `https://www.topview.ai/board/${encodeURIComponent(o)}${b ? `?boardResultId=${encodeURIComponent(b)}` : ""}`
        };
      }
      if (/fail|error|cancel/.test(_))
        throw new Error(re(h, ["errorMsg", "error_msg", "errorMessage", "error_message"]) ?? "Topview could not complete this image.");
    }
  ...
  async generateAudio(e) {
    ...
    const d = re(l, ["taskId", "task_id", "generationId", "generation_id"]), u = ht(l);
    if (u) return { url: u, mediaType: "audio", taskId: d, model: e.model, boardUrl: `https://www.topview.ai/board/${encodeURIComponent(n)}` };
...
let wa;
function Nl() {
  return wa ?? (wa = new Km()), wa;
}
function Ym() {
  return Nl().teamConnection();
}
function Qm() {
  const t = Nl();
...
async function th(t, e) {
  ...
  for (let p = 0; p < s.length; p++) {
    ...
    m !== 1 && h.push(`setpts=${(1 / m).toFixed(4)}*PTS`), i.scale !== 1 && h.push(`scale=iw*${i.scale}:ih*${i.scale}`), h.push(`fps=${o}`), d.push(`[${p}:v]${h.join(",")}[v${p}]`);
    const g = f.duration / m;
    if (f.type === "image")
      d.push(`anullsrc=r=48000:cl=stereo,atrim=duration=${g.toFixed(4)}[a${p}]`);
    else {
      const _ = [];
  ...
  return d.push(
    ...
    (h = m.stderr) == null || h.on("data", (g) => {
      ...
      if (b) {
        const v = eh(b, e.totalDuration);
        if (v !== null) {
          const T = le.get(t);
          T && (le.set(t, { ...T, progress: v }), Zm(t, v));
        }
      }
...
function rh() {
  I.handle("export:start", async (t, e) => {
    ...
    return le.set(i.id, i), th(i.id, e).catch((o) => {
...
function Bn(t) {
  const e = w.extname(t).toLowerCase();
  return nh[e] ?? "application/octet-stream";
}
function va(t) {
  try {
    const e = new URL(t);
...
function ah() {
  I.handle(
    ...
    async (t, e, r) => {
      ...
      const i = await oh(n);
      try {
        const o = await j.readFile(i), s = `${w.basename(n, w.extname(n))}.m4a`, l = Bn(i), d = new Blob([o], { type: l }), u = new File([d], s, { type: l });
        return { url: await J.fal.storage.upload(u) };
      } finally {
    ...
    async (t, e, r) => {
      ...
      const n = va(e);
      if (n) {
        const i = await j.readFile(n), o = w.basename(n), a = Bn(n), s = new Blob([i], { type: a }), l = new File([s], o, { type: a });
        return { url: await J.fal.storage.upload(l) };
      }
      ...
      if (e.startsWith("http://") || e.startsWith("https://")) {
        ...
        const s = await j.readFile(a), l = w.basename(a), d = Bn(a), u = new Blob([s], { type: d }), c = new File([u], l, { type: d }), p = await J.fal.storage.upload(c);
...
function Cl(t) {
  ...
  for (const n of r)
    $.mkdirSync(n, { recursive: !0 });
}
class dh {
  constructor(e) {
    Cl(e);
    const r = w.join(Xe(e), "project.db");
    this.db = new Ei(r), this.db.pragma("journal_mode = WAL"), this.db.pragma("foreign_keys = ON"), this.initSchema();
  }
  /**
   ...
   */
  initSchema() {
    this.db.exec(sh), this.db.exec(lh);
  }
  /**
...
function Ll(t) {
  ...
  if (!r) return null;
  const n = e.type === "character" || e.type === "location" || e.type === "prop" || e.type === "vehicle" ? e.type : "character", i = typeof e.folderId == "string" && e.folderId ? e.folderId : typeof e.folder_id == "string" && e.folder_id ? e.folder_id : void 0, o = fh(e.variations), a = typeof e.activeVariationId == "string" && o.some((l) => l.id === e.activeVariationId) ? e.activeVariationId : (s = o[0]) == null ? void 0 : s.id;
  return {
    id: r,
    name: typeof e.name == "string" ? e.name : "Untitled",
    type: n,
    description: typeof e.description == "string" ? e.description : "",
    voice: n === "character" ? uh(e.voice) : void 0,
    images: Ul(e.images),
...
function Ml(t) {
  if (!t || typeof t != "object") return ql();
  const e = t, r = Array.isArray(e.folders) ? e.folders.map(ph).filter((o) => o !== null) : [], n = new Set(r.map((o) => o.id)), i = Array.isArray(e.elements) ? e.elements.map(Ll).filter((o) => o !== null) : [];
  return {
    version: 1,
    folders: r,
...
function mh(t, e) {
  const r = ql(), n = new Map(r.elements.map((o) => [o.id, o])), i = [...r.folders];
  for (const o of e) {
    const a = o.elements.map(Ll).filter((l) => l !== null);
...
async function yh() {
  try {
    const t = await j.readFile(Dl(), "utf-8");
...
function _h(t) {
  ...
  if (t.useSqlite || $.existsSync(e))
    try {
      const n = new Ei(e, { readonly: !0 }), i = n.prepare("SELECT * FROM elements").all();
  ...
  if ($.existsSync(r))
    try {
      const n = JSON.parse($.readFileSync(r, "utf-8"));
...
async function wh() {
  ...
  let e = [];
  try {
    const i = JSON.parse(await j.readFile(gh(), "utf-8"));
    e = Array.isArray(i.projects) ? i.projects : [];
  } catch {
    e = [];
  }
  const r = e.map(_h), n = mh(null, r);
  return await hi(n), n;
}
function bh() {
  I.handle(
    ...
    async (t, e) => {
      ...
      if (e != null && e.projectId && e.projectName) {
        const n = hh(r, e.projectId, e.projectName);
        n !== r && (await hi(n), r = n);
      }
      return r;
    }
...
function Th(t, e, r) {
  const n = vh(r.persona), i = r.persona ? $l[r.persona] : void 0, o = [];
  let a = 0;
  if (e.length === 0)
...
function kh(t) {
  ...
  return Ah.filter((r) => e.includes(r));
}
function jh(t, e, r = 24) {
  const n = typeof r == "number" ? { limit: r } : r, i = n.limit ?? 24, o = xh(e), a = {
    activeTimelineId: t.activeTimelineId,
    persona: n.persona,
    queryEmotions: kh(e)
  };
  return t.moments.map((s) => ({ moment: s, ...Th(s, o, a) })).filter((s) => s.score > 0).sort((s, l) => l.score - s.score || s.moment.sourceStart - l.moment.sourceStart).slice(0, i).map(({ moment: s, score: l, reasons: d }) => ({
    id: s.id,
    assetId: s.assetId,
...
async function ln(t, e) {
  ...
  if (e.startsWith("data:")) {
    ...
    return J.fal.config({ credentials: t }), J.fal.storage.upload(p);
  }
  const r = Ph(e);
  if (!r) return null;
  const n = await j.readFile(r), i = Rh(r), o = new Blob([n], { type: i }), a = new File([o], w.basename(r), { type: i });
  return J.fal.config({ credentials: t }), J.fal.storage.upload(a);
}
...
function Ia(t, e) {
  const n = (Array.isArray(t.objects) ? t.objects : Array.isArray(t.detections) ? t.detections : Array.isArray(t.items) ? t.items : Array.isArray(t.regions) ? t.regions : Array.isArray(t.subjects) ? t.subjects : typeof t.label == "string" || typeof t.name == "string" || typeof t.object == "string" ? [t] : []).map((o) => {
    ...
    f && f.length >= 4 && (d = Q(f[0]), u = Q(f[1]), c = Q(f[2]), p = Q(f[3]));
    ...
    if ((d === null || u === null || c === null || p === null) && m && m.length >= 4) {
      ...
      [x, E, k, O].every((q) => q !== null) && (d = (x + k) / 2, u = (E + O) / 2, c = k - x, p = O - E);
    }
    const y = Array.isArray(a.box_3d) ? a.box_3d : Array.isArray(a.box3d) ? a.box3d : null;
    if ((d === null || u === null || c === null || p === null) && y && y.length >= 6) {
      const x = Q(y[0]), E = Q(y[1]), k = Q(y[3]), O = Q(y[4]), q = Q(y[5]);
      [x, E, k, O, q].every((D) => D !== null) && (d = x, u = E, c = Math.max(k, O), p = Math.max(O, q));
    }
    if (d === null || u === null || c === null || p === null) {
      ...
      [x, E, k, O].every((q) => q !== null) && (d = x, u = E, c = k, p = O);
    }
    if (d === null || u === null || c === null || p === null) {
      const x = Q(a.x_min ?? a.left), E = Q(a.y_min ?? a.top), k = Q(a.x_max ?? a.right), O = Q(a.y_max ?? a.bottom);
      [x, E, k, O].every((q) => q !== null) && (d = (x + k) / 2, u = (E + O) / 2, c = k - x, p = O - E);
    }
    if ([d, u, c, p].some((x) => x === null || !Number.isFinite(x))) return null;
    const h = gt(c, 0.02, 1), g = gt(p, 0.02, 1), _ = [
      gt(d, h / 2, 1 - h / 2),
      gt(u, g / 2, 1 - g / 2),
      h,
      g
    ], b = Q(a.score ?? a.confidence ?? a.probability), v = b !== null ? gt(b, 0, 1) : 0.75, T = Q(a.priority ?? a.salience ?? a.importance), S = T !== null ? gt(T, 0, 1) : v;
    return {
      label: l,
      box: _,
      score: v,
...
async function xa(t, e, r, n, i) {
  J.fal.config({ credentials: t });
  const a = (await J.fal.subscribe("fal-ai/any-llm/vision", {
    input: {
      model: r,
      prompt: i,
      image_urls: [e],
...
async function Bl(t) {
  ...
  const e = (await Promise.all(
    t.framePaths.slice(0, 6).map((c) => ln(t.apiKey, c).catch(() => null))
  )).filter((c) => !!c);
  if (e.length === 0)
    return {
      assetId: t.assetId,
  ...
  J.fal.config({ credentials: t.apiKey });
  const n = (await J.fal.subscribe("fal-ai/any-llm/vision", {
    input: {
...
`),
    ...
    logs: !0
  })).data, i = ye(n.output) || ye(n.text) || "", o = Fl(i);
  if (!o)
    return {
      assetId: t.assetId,
      status: "failed",
  ...
  try {
    ...
    return {
      assetId: t.assetId,
      status: "failed",
      model: ((u = t.model) == null ? void 0 : u.trim()) || Be,
...
async function Vl(t) {
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await Oh(t.apiKey, t.videoPath).catch(() => null);
  if (!e)
    throw new Error("Could not upload the video file for analysis.");
  J.fal.config({ credentials: t.apiKey });
  const n = (await J.fal.subscribe("fal-ai/video-understanding", {
    input: {
      video_url: e,
...
async function Nh(t) {
  var o;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await ln(t.apiKey, t.imagePath).catch(() => null);
  if (!e)
...
async function Ch(t) {
  var a, s;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = Math.min(12, Math.max(1, Math.round(t.maxObjects ?? 6))), r = await ln(t.apiKey, t.imagePath).catch(() => null);
  if (!r)
    return {
      status: "missing",
      model: ((a = t.model) == null ? void 0 : a.trim()) || Be,
...
`);
  try {
    ...
    const u = await xa(t.apiKey, r, n, e, o), c = u ? Ia(u, e) : [];
...
function qh() {
  I.handle("vision:index-asset", async (t, e) => Bl(e)), I.handle("vision:detect-objects", async (t, e) => Ch(e));
}
const Lh = "anthropic/claude-sonnet-4.6";
function oe(t) {
  const e = Number(t);
...
async function Lt(t) {
  ...
  J.fal.config({ credentials: t.apiKey });
  const e = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || Lh,
    prompt: t.prompt,
  ...
  const n = (await J.fal.subscribe("openrouter/router", { input: e, logs: !0 })).data;
  return {
...
function Vh(t, e) {
  ...
  return {
    brief: {
      ...
      variantCount: $h(r.variantCount, e.variantCount),
      persona: Dh(r.persona),
    ...
    clarifyingQuestions: Bh(r.clarifyingQuestions)
  };
}
function Gh(t, e, r) {
  const n = { ...t, ...e ?? {} };
...
function Hh(t, e) {
  if (!t || typeof t != "object") return null;
  const r = t, n = Array.isArray(r.segments) ? r.segments.map(zh).filter((i) => !!i) : [];
...
function Wh(t) {
  ...
  return Array.isArray(e.variants) ? e.variants.flatMap((r, n) => {
    var a;
    if (!r || typeof r != "object") return [];
    const i = r, o = Array.isArray(i.proposals) ? i.proposals.map((s) => Hh(s, `AI Cut ${n + 1}`)).filter((s) => !!s) : [];
    return o.length === 0 ? [] : [{
...
async function Qh(t) {
  var o;
  const e = Fh(t.request, t.index), r = [
...
async function ka(t, e, r, n, i = {}) {
  ...
  let a = jh(t, o, { limit: 20, persona: r.persona });
  if (i.rerank && i.apiKey && a.length > 1)
    try {
      const l = Eh({ query: o, brief: r, candidates: a }), d = await Lt({
        apiKey: i.apiKey,
        model: i.model,
...
async function Zh(t) {
  ...
  const e = (u, c) => {
    ...
    if (!p) return null;
    try {
      const f = JSON.parse(p), y = Wh({ variants: [f] })[0];
      return y ? {
        variant: y,
        usage: c
      } : null;
...
${JSON.stringify(l.map((h) => ({ title: h.title, strategy: h.strategy, summary: h.summary })), null, 2)}` : "",
      ...
      "",
      "Retrieved moments:",
      Gl(t.retrievalSummary.topMoments),
      "",
      "Reference timelines:",
...
async function eg(t) {
  ...
  const e = [
    ...
    Gl(t.retrievalSummary.topMoments.slice(0, 10)),
    "",
    "Variants:",
...
async function tg(t) {
  ...
  n = gr(n, i.usage);
  const o = Gh(i.brief, t.briefOverride, t.questionAnswers), a = await ka(e, r, o, []);
  if (!t.confirmedBrief)
    return {
      stage: "brief",
...
`)) >= 0; ) {
      ...
      if (m = m.slice(b + 1), !!v)
        try {
          ...
          if (x)
            for (const E of x)
              u ? (c += E, c.endsWith("</think>") && (u = !1, c = "")) : (c += E, c === "<think>" ? (u = !0, c = "") : "<think>".startsWith(c) || (s += c, a == null || a.webContents.send("llm:local-stream", { requestId: t, token: c }), c = ""));
...
async function ig() {
  try {
    const t = await fetch(`${zl}/api/tags`);
...
function og() {
  I.handle("llm:chat", async (t, e) => {
    ...
    const n = Array.isArray(e.messages) ? e.messages : [], i = Mh(n);
    ...
    const r = e.requestId || crypto.randomUUID(), n = await ng(r, e);
    return {
      message: n.message,
      ...n.usage ? { usage: n.usage } : {}
    };
  }), I.handle("llm:local-models", async () => ig()), I.handle("llm:run-cut-workflow", async (t, e) => tg(e));
}
const ag = 18, ja = 24 * 1024 * 1024;
function Hl(t = []) {
  if (t.length > ag) throw new Error("Too many canvas previews in one request. Please try again.");
  let e = 0;
  return t.map((r) => {
    ...
    if (e += i.length, e > ja) throw new Error("Canvas previews are too large to send. Please try again.");
    const o = n[1];
...
async function Wl(t = []) {
  ...
  const r = await qd(w.join(H.tmpdir(), "cinegen-assistant-")), n = () => Ld(r, { recursive: !0, force: !0 });
  try {
    const i = [];
    for (const [o, a] of e.entries()) {
      const s = w.join(r, `${o + 1}.${a.mime.split("/")[1]}`);
      await ts(s, a.bytes, { mode: 384 }), i.push({ label: a.label, kind: "asset", mediaType: "image", mediaPath: s, ephemeral: !1 });
    }
...
async function Jl() {
  ...
  for (const t of lg)
    try {
      const { stdout: e } = await Xl(t, ["--version"], {
        env: Ji(),
        timeout: 8e3
      });
...
function yg() {
  return w.join(z.getPath("userData"), "claude-code-workspace");
}
function _g(t) {
...
async function bg(t, e) {
  ...
  const n = ((_ = e.model) == null ? void 0 : _.trim()) || "sonnet", i = !!e.resumeSessionId && !e.injectProjectContext && !((b = e.images) != null && b.length), o = _g(e), a = i ? e.userMessage.trim() : wg(e, o), s = (v = e.images) != null && v.length ? sg(a, e.images) : void 0, l = [
    "-p",
  ...
  if (o ? (l.push("--safe-mode", "--effort", "low", "--include-partial-messages"), !i && ((T = e.systemPrompt) != null && T.trim()) && l.push("--system-prompt", e.systemPrompt.trim())) : l.push("--include-partial-messages"), i && e.resumeSessionId) {
    if (l.push("--resume", e.resumeSessionId), !o) {
      const E = [(S = e.systemPrompt) == null ? void 0 : S.trim(), ug].filter(Boolean).join(`
...
` : "", k = e.purpose === "enhance-prompt" ? cg : dg;
    l.push("--append-system-prompt", `${E}${e.systemPrompt.trim()}

${k}`);
  }
  const d = mg(), u = o ? yg() : void 0;
  u && await st(u, { recursive: !0 });
  let c = "", p = "", f, m = !1, y = !1, h, g;
  return new Promise((E, k) => {
    ...
    s && ((K = O.stdin) == null || K.end(s)), nt = { child: O, requestId: t };
    let q = "", D = !1;
    const G = (U) => {
      D || (D = !0, nt = null, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, done: !0 }), U(), O.killed || O.kill());
    };
    (W = O.stdout) == null || W.on("data", (U) => {
      q += U.toString();
      let N;
      for (; (N = q.indexOf(`
`)) >= 0; ) {
        const P = q.slice(0, N).trim();
        if (q = q.slice(N + 1), !!P)
          try {
            ...
            if (A.type === "system" && A.subtype === "init" && typeof A.session_id == "string" && (f = A.session_id), A.type === "assistant" && A.error === "authentication_failed" && (m = !0), A.type === "result") {
              g = A;
              const V = Pa(A);
              V && !c.trim() && (c = V, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: V }));
              const Y = c.trim();
              if (Y && !m && !Y.includes("Not logged in")) {
                G(() => E({ message: Y, sessionId: f, usage: h, resumed: i }));
                return;
              }
            }
            const L = Ra(A);
            ...
            if (A.type === "stream_event") {
              y = !0, c += C, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: C });
              continue;
            }
            A.type === "assistant" && !y ? (c = C, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: C })) : A.type === "result" && !c.trim() && (c = C, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: C }));
          } catch {
          }
      }
    }), (R = O.stderr) == null || R.on("data", (U) => {
      p += U.toString();
    }), O.on("error", (U) => {
      G(() => k(U));
    }), O.on("close", (U) => {
...
function vg() {
  I.handle("llm:claude-code-detect", async () => {
    ...
    const r = e.requestId || X.randomUUID(), n = await bg(r, e);
    return {
      message: n.message,
...
const Ki = Ut(qe), Tg = {
  ...
  codex: [
    w.join(H.homedir(), ".npm-global/bin/codex"),
    w.join(H.homedir(), ".local/bin/codex"),
    "/opt/homebrew/bin/codex",
    "/usr/local/bin/codex",
    "codex"
  ],
  gemini: [
    w.join(H.homedir(), ".npm-global/bin/gemini"),
    w.join(H.homedir(), ".local/bin/gemini"),
...
`).trim();
  return i ? Ag(i) : `Codex exited with code ${e ?? "unknown"}`;
}
function jg(t) {
...
let se = null;
function Rg() {
  return w.join(z.getPath("userData"), "codex-workspace");
}
function Pg(t) {
  return t.purpose === "json-job" ? !0 : t.purpose === "copilot" || t.purpose === "enhance-prompt" ? !1 : !t.injectProjectContext && !t.resumeSessionId && !(t.messages && t.messages.length > 0);
}
...
` : "", l = t.purpose === "enhance-prompt" ? Zl : Ql;
      r.push(`${s}${t.systemPrompt.trim()}

${l}`);
    } else
      r.push(t.systemPrompt.trim());
  const n = (t.messages ?? []).filter((s) => s.content.trim()), i = n.length > 0 ? yi(n) : `${t.userMessage.trim()}
...
async function qg(t, e) {
  var _;
  const r = await dn("codex");
  if (!r)
    throw new Error("Codex CLI is not installed. Install it from https://developers.openai.com/codex");
  if (!e.userMessage.trim())
    throw new Error("No chat message provided.");
  const n = jg(e.model), i = !!e.resumeSessionId && !e.injectProjectContext && !((_ = e.images) != null && _.length), o = Pg(e), a = i ? e.userMessage.trim() : Og(e, o), s = Rg();
  ...
  n && l.push("-m", n);
  const d = await Wl(e.images);
  for (const b of d.refs) l.push("--image", b.mediaPath);
  d.refs.length && !o && l.push("--"), i && e.resumeSessionId && l.push("resume", e.resumeSessionId), o || l.push("-");
  const u = Yl();
  let c = "", p = "", f = "", m = !1, y, h, g = "";
  try {
    return await new Promise((b, v) => {
      ...
      (x = T.stdin) == null || x.on("error", () => {
      }), (E = T.stdin) == null || E.write(a), (k = T.stdin) == null || k.end(), se = { child: T, requestId: t, provider: "codex" };
      let S = "";
      (O = T.stdout) == null || O.on("data", (D) => {
        S += D.toString();
        let G;
        for (; (G = S.indexOf(`
`)) >= 0; ) {
          const K = S.slice(0, G).trim();
          if (S = S.slice(G + 1), !!K)
            try {
              const W = JSON.parse(K);
              W.type === "thread.started" && typeof W.thread_id == "string" && (y = W.thread_id);
              const R = Ng(W);
              if (R && (h = R), W.type === "turn.failed") {
                m = !0;
                const N = W.error;
                f = (N == null ? void 0 : N.message) ?? "Codex turn failed.";
              } else W.type === "error" && typeof W.message == "string" && (f = W.message);
              const U = Cg(W);
              if (U) {
                const N = U.startsWith(g) ? U.slice(g.length) : U;
                g = U, c = U, N && (u == null || u.webContents.send("llm:codex-stream", { requestId: t, token: N }));
              }
            } catch {
            }
        }
      }), (q = T.stderr) == null || q.on("data", (D) => {
        p += D.toString();
      }), T.on("error", (D) => {
        (se == null ? void 0 : se.requestId) === t && (se = null), v(D);
      }), T.on("close", (D) => {
        (se == null ? void 0 : se.requestId) === t && (se = null), u == null || u.webContents.send("llm:codex-stream", { requestId: t, done: !0 });
        const G = c.trim();
        if (!G || D !== 0 || m) {
          v(new Error(kg(f || p, D)));
          return;
        }
        b({ message: G, sessionId: y, usage: h, resumed: i });
      });
    });
  } finally {
    await d.cleanup();
...
function Lg() {
  I.handle("llm:codex-chat", async (t, e) => {
    const r = e.requestId || X.randomUUID(), n = await qg(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
...
const Oa = 32 * 1024 * 1024;
async function Ug(t) {
  if (typeof t != "string") throw new Error("Invalid canvas image source.");
  const e = new URL(t);
  let r;
  if (e.protocol === "file:" || e.protocol === "local-media:" && e.hostname === "file") {
    const s = e.protocol === "file:" ? Qr(e) : decodeURIComponent(e.pathname), l = await j.stat(s);
    if (!l.isFile() || l.size > Oa) throw new Error("Canvas image source is too large or unavailable.");
    r = await j.readFile(s);
  } else if (e.protocol === "https:" || e.protocol === "http:") {
    const s = await kd.fetch(t, { signal: AbortSignal.timeout(12e3), credentials: "omit" });
    if (!s.ok || !s.body) throw new Error("Canvas image source is unavailable.");
    const l = s.body.getReader(), d = [];
    let u = 0;
    try {
      for (; ; ) {
        const { done: c, value: p } = await l.read();
        if (c) break;
        if (u += p.byteLength, u > Oa) throw new Error("Canvas image source is too large.");
        d.push(p);
      }
    } finally {
      await l.cancel();
  ...
  const n = Ka.createFromBuffer(r);
  if (n.isEmpty()) throw new Error("Canvas source is not a readable image.");
  const { width: i, height: o } = n.getSize(), a = Math.min(1, 1280 / Math.max(i, o));
  return `data:image/jpeg;base64,${n.resize({ width: Math.max(1, Math.round(i * a)), height: Math.max(1, Math.round(o * a)) }).toJPEG(82).toString("base64")}`;
}
function Mg() {
  I.handle("llm:canvas-image-preview", (t, e) => Ug(e));
}
const Dg = 272e3, Na = {
...
function Fg(t) {
  const e = t.promptTokens > Dg ? Na.long : Na.short, r = Math.min(t.cachedTokens, t.promptTokens), n = Math.min(t.cacheWriteTokens, Math.max(0, t.promptTokens - r)), o = (Math.max(0, t.promptTokens - r - n) * e.input + r * e.cached + n * e.cacheWrite + t.completionTokens * e.output) / 1e6;
  return { ...t, cost: Math.round(o * 1e8) / 1e8 };
}
const Bg = "https://api.openai.com/v1/chat/completions", Vg = "gpt-5.6-luna", Gg = 6e4;
function zg(t, e = []) {
...
function Hg(t) {
  ...
  r && e.push({ role: "system", content: r }), e.push({ role: "user", content: zg(t.userMessage, t.imageUrls) });
  const n = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || Vg,
    messages: e,
    reasoning_effort: t.reasoningEffort ?? "low",
    max_completion_tokens: Number.isFinite(t.maxCompletionTokens) ? Math.max(1, Math.floor(t.maxCompletionTokens)) : Gg
  };
...
async function Jg(t) {
  ...
  if (!i.ok)
    throw new Error(Wg(a, `OpenAI request failed (${i.status}).`));
  const s = $g(a);
  return {
    message: Xg(a),
    ...s ? { usage: Fg(s) } : {}
...
const Yg = "http://localhost:3000", ed = "https://cinegen-film.vercel.app", Yi = "persist:cinegen-team-workspace", Qg = 8e3, _i = "__CINEGEN_TEAM_PROVIDER__", ze = { origin: ed, source: "hosted" }, Hn = { origin: Yg, source: "local-web" };
...
async function un(t, e, r, n) {
  const i = new AbortController(), o = setTimeout(() => i.abort(), Qg);
  try {
    const a = `${t.origin}/api/rpc/${encodeURIComponent(e)}/${encodeURIComponent(r)}`, s = {
      ...
      redirect: "error",
      signal: i.signal
    }, l = t.source === "hosted" ? await Ya.fromPartition(Yi).fetch(a, s) : await fetch(a, s);
    return { response: l, payload: await Zg(l) };
  } finally {
    clearTimeout(o);
  }
}
...
async function yr(t) {
  try {
    const { response: e, payload: r } = await un(t, "providers", "status", []);
    return !e.ok || !(r != null && r.ok) || !r.result ? null : td(r.result, t);
  } catch {
    return null;
  }
}
async function cn() {
  ...
  const e = await yr(Hn);
  return e ? (Ce = Hn, { target: Hn, status: e }) : null;
}
async function ey(t, e, r) {
  ...
  const { response: i, payload: o } = await un(n.target, t, e, r);
  if (!i.ok || !(o != null && o.ok))
...
async function Ca(t, e) {
  ...
  if (!r)
    throw new Error("Connect CineGen Desktop to the hosted team workspace in Settings first.");
  const { response: n, payload: i } = await un(r.target, "providers", t, [e]);
...
async function ty() {
  ...
  const { response: r, payload: n } = await un(e.target, "topview", "importTeamConnection", [t]);
...
async function ry(t) {
  return ey("llm", "openaiChat", [{ ...t, apiKey: _i }]);
}
async function rd() {
  const t = await yr(ze);
  if (t)
    ...
    const u = () => {
      ...
      if (!(s || a)) {
        ...
        try {
          const m = await yr(ze);
          if (a || !m) return;
          Ce = ze, c(m), e.isDestroyed() || e.close();
        } finally {
          s = !1;
        }
      }
...
function ny() {
  ...
  }), I.handle("team-providers:connect", () => rd()), I.handle("team-providers:disconnect", async () => (await Ya.fromPartition(Yi).clearStorageData({ storages: ["cookies", "indexdb", "localstorage"] }), Ce = null, zr())), I.handle("team-providers:save", async (t, e) => Ca("save", e)), I.handle("team-providers:remove", async (t, e) => Ca("remove", e)), I.handle("team-providers:share-topview", () => ty());
}
function iy(t) {
  const e = w.extname(t).toLowerCase();
...
function oy(t) {
  ...
  const r = Kg(e) ?? (e.startsWith("/") || /^[A-Za-z]:[\\/]/.test(e) ? e : null);
  if (!r || !$.existsSync(r)) return null;
  const n = $.readFileSync(r);
  return `data:${iy(r)};base64,${n.toString("base64")}`;
}
function ay(t) {
  return Array.isArray(t) ? t.flatMap((e) => {
...
function sy() {
  I.handle("llm:openai-chat", async (t, e) => {
    const r = e && typeof e == "object" && !Array.isArray(e) ? e : {}, n = typeof r.apiKey == "string" ? r.apiKey : "", i = typeof r.userMessage == "string" ? r.userMessage : "", o = ay(r.imageUrls);
    return n === _i ? ry({
      ...r,
      apiKey: _i,
      userMessage: i,
...
const ly = /* @__PURE__ */ new Set(["authCancel", "authLogin", "authStatus", "accountStatus", "connect", "disconnect", "voices", "generate", "job", "design", "saveVoice", "enhance"]);
function dy() {
  I.handle("elevenlabs:request", async (t, e, r, n) => {
    var a;
    if (!ly.has(e) || typeof n != "string" || !n || n.length > 12e3) throw new Error("Invalid ElevenLabs request.");
...
async function id(t, e, r, n) {
  const i = ve(), o = Math.max(0, e), a = Math.max(0.1, Math.min(r, cy));
  try {
    return await nd(i, [
      "-y",
      "-ss",
      ...
      "+faststart",
      n
    ], { timeout: Math.max(12e4, Math.ceil(a * 4e3)) }), $.existsSync(n) ? n : null;
  } catch {
    return null;
  }
}
async function Mr(t, e, r) {
  ...
  try {
    return await nd(n, [
      ...
      "2",
      r
    ], { timeout: 15e3 }), $.existsSync(r) ? r : null;
  } catch {
    return null;
  }
}
...
function Wn(t, e, r) {
  ...
  const n = w.extname(t) || (e.mediaType === "image" ? ".jpg" : ".mp4"), i = w.join(r, `${wi(e)}-source${n}`), o = ad(t, i);
...
async function Qi(t, e) {
  ...
  for (const i of t) {
    ...
    if (i.trimStartSec !== void 0 && i.trimDurationSec !== void 0) {
      const u = w.join(r, `${wi(i)}.mp4`), c = await id(
        o,
        i.trimStartSec,
        i.trimDurationSec,
        u
    ...
    const s = (i.framePaths ?? []).map((u) => jt(u)).find(Boolean);
    if (s) {
      const u = Wn(s, {
        ...i,
        mediaType: "image",
    ...
    const l = w.join(r, `${wi(i)}.jpg`), d = await Mr(o, i.trimStartSec ?? 0, l);
    d && n.push({
      label: i.label,
      kind: i.kind,
      mediaType: "image",
...
async function fy(t, e) {
  ...
  const n = w.join(H.tmpdir(), "cinegen-higgsfield-refs");
  $.mkdirSync(n, { recursive: !0 });
  const i = X.randomBytes(6).toString("hex"), o = Math.max(0, e.sourceStartSec ?? 0), a = e.sourceEndSec ?? o;
  if (e.mode === "first-last") {
    const u = w.join(n, `${i}-first.jpg`), c = w.join(n, `${i}-last.jpg`), p = await Mr(r, o, u), f = await Mr(r, Math.max(o, a - 0.05), c), m = [], y = [];
  ...
  if (e.mode === "segment") {
    const u = w.join(n, `${i}-segment.mp4`), c = Math.max(0.1, a > o ? a - o : e.maxSegmentSec ?? 30), p = await id(r, o, Math.min(c, e.maxSegmentSec ?? 30), u);
...
const my = 18e4, hy = 600 * 1e3;
async function dd(t) {
  var u;
  const e = await dn("gemini");
  if (!e)
    throw new Hr("Gemini CLI is not installed.");
  const r = jt(t.mediaPath);
  if (!r)
    throw new Error(`Media file not found: ${t.mediaPath}`);
  const n = w.join(H.tmpdir(), "cinegen-gemini-acoustic");
  await st(n, { recursive: !0 });
  let i = r, o = !1;
  if (od(r)) {
    const c = w.extname(r) || ".mp4", p = w.join(n, `${X.randomUUID()}${c}`), f = ad(r, p);
    if (!f)
  ...
  return new Promise((c, p) => {
    var S, x;
    const f = ae(e, l, { env: Kl(), cwd: n, stdio: ["ignore", "pipe", "pipe"] });
    let m = "", y = "", h = "", g = !1, _ = !1;
    const b = (E) => {
      g || (g = !0, clearTimeout(v), clearTimeout(T), d(), E());
    }, v = setTimeout(() => {
    ...
    (S = f.stdout) == null || S.on("data", (E) => {
      ...
      for (; (k = h.indexOf(`
`)) >= 0; ) {
        const O = h.slice(0, k).trim();
        if (h = h.slice(k + 1), !!O)
          try {
            const q = JSON.parse(O);
            q.type === "message" && q.role === "assistant" && typeof q.content == "string" && q.content && (_ = !0, m += q.content), q.type === "error" && typeof q.message == "string" && (y += q.message);
          } catch {
          }
      }
...
const qa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  analyzeMediaWithGeminiCli: dd,
  buildGeminiUserMessageWithVisualRefs: sd,
  cleanupEphemeralVisualRefs: Zi,
  prepareClipReference: fy,
  prepareCopilotVisualRefs: Qi,
  resolveLocalSourcePath: ld
}, Symbol.toStringTag, { value: "Module" }));
let be = null;
const gy = 9e4, yy = 18e4, _y = 8e3;
function ud() {
  return w.join(z.getPath("userData"), "gemini-cli-workspace");
}
function wy() {
  return w.join(H.tmpdir(), "cinegen-gemini-visual-refs");
}
function by(t) {
  var n;
  const e = [];
...
${t.purpose === "enhance-prompt" ? Zl : Ql}`);
    } else
...
async function La(t, e, r) {
  var h, g;
  const n = await dn("gemini");
  if (!n)
    throw new Error("Gemini CLI is not installed. Install it with: npm install -g @google/gemini-cli");
  const i = ((h = e.model) == null ? void 0 : h.trim().replace(/^[^/]+\//, "")) || "gemini-2.5-flash", o = r.canResume ? vy(e) : by(e), a = o.length > _y, s = ud();
  await st(s, { recursive: !0 });
  const l = [
    "--skip-trust",
    ...a ? ["-p", ""] : ["-p", o],
    ...
    r.hasVisualRefs && !((g = e.images) != null && g.length) ? "yolo" : "default"
  ];
  if (r.hasVisualRefs) {
  ...
  const d = Yl();
  let u = "", c = "", p, f;
  const m = 900 * 1e3, y = r.hasVisualRefs ? yy : gy;
  return new Promise((_, b) => {
    ...
    a && ((q = v.stdin) == null || q.write(o), (D = v.stdin) == null || D.end()), be = { child: v, requestId: t, provider: "gemini" };
    let T = "", S = !1, x = !1;
    const E = (W) => {
      S || (S = !0, clearTimeout(k), clearTimeout(O), Zi(r.preparedVisualRefs), W());
    }, k = setTimeout(() => {
    ...
    (G = v.stdout) == null || G.on("data", (W) => {
      T += W.toString();
      let R;
      for (; (R = T.indexOf(`
`)) >= 0; ) {
        const U = T.slice(0, R).trim();
        if (T = T.slice(R + 1), !!U)
          try {
            const N = JSON.parse(U);
            N.type === "init" && typeof N.session_id == "string" && (p = N.session_id);
            const P = Ty(N);
            if (P && (f = P), N.type === "tool_use" && (d == null || d.webContents.send("llm:gemini-stream", {
              requestId: t,
              status: Ey(N.tool_name)
            })), N.type === "message" && N.role === "assistant" && typeof N.content == "string") {
              const A = N.content;
              A && (x = !0, u += A, d == null || d.webContents.send("llm:gemini-stream", { requestId: t, token: A }));
            }
            if (N.type === "error" && typeof N.message == "string") {
              const A = N.message;
              c += A, !u.trim() && Sy(A) && (be = null, v.kill("SIGTERM"), E(() => b(new Error(gi(A)))));
            }
            if (N.type === "result" && N.status === "error") {
              const A = typeof N.error == "string" ? N.error : typeof N.message == "string" ? N.message : "Gemini CLI returned an error.";
              c += A;
            }
          } catch {
          }
      }
    }), (K = v.stderr) == null || K.on("data", (W) => {
      c += W.toString();
    }), v.on("error", (W) => {
      be = null, E(() => b(W));
    }), v.on("close", (W) => {
      ...
      const R = u.trim();
      if (!R) {
        const U = gi(c.trim()) || `Gemini CLI exited with code ${W ?? "unknown"}`;
        E(() => b(new Error(U)));
        return;
      }
      E(() => _({
...
async function xy(t, e) {
  ...
  try {
    ...
    const a = o.length > 0, s = sd(e.userMessage, o), l = e.messages ? [...e.messages] : void 0;
    if (a && (l != null && l.length)) {
      const c = l.findLastIndex((p) => p.role === "user");
      c >= 0 ? l[c] = { ...l[c], content: s } : l.push({ role: "user", content: s });
    }
    const d = {
      ...e,
      userMessage: s,
      messages: l
    }, u = !!e.resumeSessionId && !e.injectProjectContext && !a;
    try {
      return await La(t, d, {
        canResume: u,
        hasVisualRefs: a,
...
function Ay() {
  I.handle("llm:gemini-chat", async (t, e) => {
    const r = e.requestId || X.randomUUID(), n = await xy(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
...
const ky = `You are a music prompt engineer. Your job is to write a detailed, evocative text prompt that will be used to generate music with an AI music model (ElevenLabs/Suno).
...
function Ry() {
  I.handle("music:generate-prompt", async (t, e) => {
    ...
    if (!r) throw new Error("No fal.ai API key provided.");
    J.fal.config({ credentials: r });
    const n = e.frameUrls && e.frameUrls.length > 0, i = jy(e, !!n), o = {
      model: "google/gemini-flash-1.5",
      system_prompt: ky,
      prompt: i,
      max_tokens: 300
    }, a = n ? "fal-ai/any-llm/vision" : "fal-ai/any-llm";
    return n && (o.image_urls = e.frameUrls), { prompt: ((await J.fal.subscribe(a, { input: o, logs: !0 })).data.output ?? "").trim() };
  });
}
function Py() {
...
function Mt(t, e) {
  const r = Oy[e], n = Object.entries(t).filter(
    ([a]) => a !== "id" && (!r || r.has(a))
  );
...
function Ua(t, e) {
  ...
  const n = md(t, e), i = Ny(t, e), o = Jy(t, e), a = Yy(t, e), s = t_(t, e), d = Ly(t, e).map((c) => {
    const p = Dy(t, c.id), f = Fy(t, c.id), m = Hy(t, c.id), y = f.map((h) => ({
      ...h,
      keyframes: Gy(t, h.id)
    }));
...
function i_(t, e, r) {
  t.transaction(() => {
    ...
    for (const f of r.timelines) {
      if (l.has(f.id))
        Uy(t, f.id, { name: f.name, duration: f.duration });
      else {
...
function $e(t) {
  let e = He.get(t);
  return e || (e = new dh(t), He.set(t, e)), e;
}
function _d() {
  return w.join(ut(), "projects.json");
}
async function eo() {
  try {
    const t = await j.readFile(_d(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function to(t) {
  await j.mkdir(ut(), { recursive: !0 }), await j.writeFile(_d(), JSON.stringify(t, null, 2), "utf-8");
}
async function o_(t) {
...
function s_() {
  I.handle("db:project:create", async (t, e) => {
    ...
    const r = $e(e), n = Ua(r, e);
    for (const i of n.assets)
      if (i.file_ref && !i.source_url) {
        const o = i.status;
        $.existsSync(i.file_ref) ? i.status === "offline" && (i.status = "online") : i.status = "offline", i.status !== o && Wr(r, i.id, { status: i.status });
      }
    return n;
  }), I.handle("db:project:save", async (t, e, r) => {
    const n = $e(e);
    i_(n, e, r);
    const i = hr(), o = await eo(), a = o.projects.find((s) => s.id === e);
...
const d_ = /* @__PURE__ */ new Set([".mp4", ".mov", ".avi", ".mkv", ".webm", ".mxf", ".m4v"]), u_ = /* @__PURE__ */ new Set([".wav", ".mp3", ".aac", ".flac", ".ogg", ".m4a"]);
function Xn(t, e) {
  const r = w.extname(t).toLowerCase();
  return d_.has(r) ? "video" : u_.has(r) ? "audio" : r ? "image" : e;
}
function c_(t, e) {
  if (t)
    try {
...
async function Ma(t) {
  ...
  const s = t.extension || c_(t.remoteUrl ?? t.localPathHint, n), l = w.join(o, `${r}${s}`), d = m_(t.localPathHint);
  if (d)
    return await h_(d, l), Dr({
      assetId: r,
      projectId: e,
      inputPath: l,
      type: Xn(l, n)
  ...
  return await j.writeFile(l, Buffer.from(p)), Dr({
    assetId: r,
    projectId: e,
    inputPath: l,
    type: Xn(l, n)
...
let ge = null;
const Fe = /* @__PURE__ */ new Map(), or = /* @__PURE__ */ new Map(), g_ = w.dirname(Qr(import.meta.url));
function wd() {
  let t = w.join(g_, "workers", "media-worker.js");
  return t.includes("app.asar") && (t = t.replace("app.asar", "app.asar.unpacked")), t;
}
function y_() {
  return ge || (ge = new is(wd()), ge.on("message", (t) => {
    switch (t.type) {
      case "ready":
...
function Se(t) {
  return t.type === "sync_compute_offset" || t.type === "sync_batch_match" ? __(t) : new Promise((e, r) => {
    Fe.set(t.id, { resolve: e, reject: r }), or.set(t.id, { assetId: t.assetId, jobType: t.type }), y_().postMessage({ type: "job:submit", job: t });
  });
}
function __(t) {
  return new Promise((e, r) => {
  ...
  I.handle("media:import", async (t, e) => {
    ...
    for (const l of r) {
      ...
      if (i === "copy") {
        ...
        await j.copyFile(l, y), u = y;
      }
      const c = w_(l), p = X.randomUUID();
      s.push({
        assetId: d,
        metadataJobId: p,
        inputPath: u,
    ...
    const { inputPath: r, timeSec: n } = e, i = ve(), o = w.join(H.tmpdir(), `cinegen-frame-${X.randomUUID()}.jpg`);
    ...
    const n = r[1] === "jpeg" ? "jpg" : r[1], i = Buffer.from(r[2], "base64"), o = w.join(H.tmpdir(), `cinegen-frame-chat-${X.randomUUID()}.${n}`);
    ...
    return new Promise((u) => {
      ...
      qe(ve(), c, { timeout: Math.max(12e4, Math.ceil(s * 4e3)) }, (p) => {
        if (p || !$.existsSync(d)) {
          u(null);
          return;
        }
    ...
    const { inputPath: r, startTimeSec: n, durationSec: i } = e, o = ve(), a = w.join(H.tmpdir(), `cinegen-clip-${X.randomUUID()}.mp4`), s = Math.max(0, n), l = Math.max(0.1, i);
    return new Promise((d) => {
      ...
      qe(o, u, { timeout: Math.max(12e4, Math.ceil(l * 4e3)) }, (c, p, f) => {
        if (c || !$.existsSync(a)) {
          d(null);
          return;
        }
    ...
    } = e, d = Xe(n), u = w.join(d, ".cache");
    if (a) {
      const c = w.join(u, "thumbnails");
      $.mkdirSync(c, { recursive: !0 });
      const p = {
        id: X.randomUUID(),
        type: "generate_thumbnail",
        assetId: r,
...
const E_ = ns(import.meta.url), S_ = w.dirname(Qr(import.meta.url));
function I_() {
  return z.isPackaged ? w.join(process.resourcesPath, "native", "cinegen_avfoundation.node") : w.resolve(S_, "../native/avfoundation/build/Release/cinegen_avfoundation.node");
}
let ne = null, vi = null;
if (process.platform === "darwin")
  try {
    const t = I_();
    ne = E_(t), console.log("[native-video] AVFoundation addon loaded:", t);
  } catch (t) {
    vi = t instanceof Error ? t.message : String(t), console.error("[native-video] Failed to load AVFoundation addon:", vi);
  }
function De() {
  return ne != null;
}
...
function R_() {
  I.handle("native-video:is-available", () => ({
    ...
    return !r || !De() ? !1 : A_(e, r.getNativeWindowHandle());
  }), I.on("native-video:set-surface-rect", (t, e) => {
    De() && k_(e.surfaceId, e.x, e.y, e.width, e.height);
  }), I.on("native-video:set-surface-hidden", (t, e) => {
    De() && $a(e.surfaceId, e.hidden);
  }), I.on("native-video:clear-surface", (t, e) => {
    De() && Fa(e);
  }), I.on("native-video:sync-surface", (t, e) => {
    De() && j_(e.surfaceId, e.descriptors);
  }), I.on("native-video:destroy-surface", (t, e) => {
    De() && Da(e);
  });
}
const P_ = "python3.12", bd = w.join(H.homedir(), "Desktop", "Coding", "whisperx"), O_ = w.join(bd, ".venv", "bin", "python");
function N_(...t) {
  return z.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const C_ = N_("scripts", "whisperx", "cinegen_infer.py"), q_ = "fal-ai/whisper", Ba = "3", L_ = {
  ".mp4": "video/mp4",
  ".m4v": "video/mp4",
  ".mov": "video/quicktime",
  ".webm": "video/webm",
...
function U_(t) {
  const e = w.extname(t).toLowerCase();
  return L_[e] ?? "application/octet-stream";
}
function Xr(t) {
  const e = Number(t);
...
function vd(t) {
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    r || (r = {
      ...
      words: []
    }), r.words.push(o), r.end = o.end, r.text = M_(r.text, o.word), !r.speaker && o.speaker && (r.speaker = o.speaker);
...
function F_(t) {
  const e = (t == null ? void 0 : t.data) ?? t, r = typeof (e == null ? void 0 : e.text) == "string" ? e.text : "", n = e == null ? void 0 : e.chunks, i = e, o = Array.isArray(n) ? n.flatMap((u) => {
    ...
    const c = typeof u.text == "string" ? u.text.trim() : "", p = u.timestamp, f = Array.isArray(p) ? Xr(p[0]) : void 0, m = Array.isArray(p) ? Xr(p[1]) : void 0, y = D_(u.speaker);
...
function Je(t, e) {
  var r;
  (r = G_()) == null || r.webContents.send("transcription:progress", {
    jobId: t.jobId,
    assetId: t.assetId,
    engine: t.engine,
    ...e
...
async function z_(t) {
  try {
    const e = $e(t.projectId), r = md(e, t.projectId).find((o) => o.id === t.assetId), i = {
...
async function ro(t) {
  t.status = "done", t.segments = $_(t.segments), t.fullText.trim() || (t.fullText = t.segments.map((e) => e.text).filter(Boolean).join(" ")), await z_(t), Je(t, {
    type: "done",
    text: t.fullText,
    segments: t.segments,
    language: t.language
...
function H_(t, e) {
  ...
  t.model = r, (async () => {
    const i = w.join(H.tmpdir(), `cinegen-whisper-${t.jobId}.py`);
    await j.writeFile(i, V_, "utf-8");
    const o = ae(P_, [i, e.filePath, r, n], {
        ...
        if (l)
          try {
            ...
            if (d.type === "segment") {
              ...
              t.segments.push(u), Je(t, { type: "segment", ...u });
...
function X_(t, e) {
  (async () => {
    ...
    t.model = Ba, t.status = "running", Je(t, { type: "status", status: "running", stage: "uploading", message: "Preparing audio for cloud transcription" }), J.fal.config({ credentials: e.apiKey });
    const r = await B_(e.filePath);
    let n = "";
    try {
      const s = await j.readFile(r), d = `${w.basename(e.filePath, w.extname(e.filePath))}.m4a`, u = U_(r), c = new Blob([s], { type: u }), p = new File([c], d, { type: u });
      n = await J.fal.storage.upload(p);
    } finally {
    ...
    }, o = await J.fal.subscribe(q_, { input: i, logs: !0 }), a = F_(o);
...
function J_() {
  I.handle("transcription:start", async (t, e) => {
    ...
    return Va.set(d, u), s === "whisperx-local" ? W_(u, { filePath: i, language: a }) : s === "whisper-cloud" ? X_(u, { filePath: i, language: a, apiKey: l }) : H_(u, { filePath: i, model: o, language: a }), { jobId: d };
...
const no = w.join(H.homedir(), "Desktop", "Coding", "ltx"), K_ = w.join(no, ".venv", "bin", "python"), Y_ = w.join(no, "cinegen_infer.py"), io = w.join(H.homedir(), "Desktop", "Coding", "qwen-edit"), Q_ = w.join(io, ".venv", "bin", "python"), Z_ = w.join(io, "cinegen_infer.py"), Td = w.join(H.homedir(), "Desktop", "Coding", "layer-decompose"), ew = w.join(Td, ".venv", "bin", "python"), Ed = w.join(H.homedir(), "Desktop", "Coding", "whisperx"), tw = w.join(Ed, ".venv", "bin", "python");
function Sd(...t) {
  return z.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const rw = Sd("scripts", "layer-decompose", "cinegen_infer.py"), nw = Sd("scripts", "whisperx", "cinegen_infer.py"), iw = {
...
function _t(t, e) {
  var r;
  (r = ow()) == null || r.webContents.send("local-model:progress", { jobId: t, ...e });
}
async function Cr(t, e) {
  if (t.startsWith("http://") || t.startsWith("https://")) {
    const r = w.extname(new URL(t).pathname) || ".jpg", n = w.join(H.tmpdir(), `cinegen-img-${e}${r}`), i = await fetch(t);
...
function aw() {
  I.handle("local-model:run", async (t, e) => {
    ...
    if (e.nodeType === "qwen-edit-local") {
      ...
      p && (f.HF_TOKEN = p), o = ae(tw, c, {
        cwd: Ed,
        stdio: ["ignore", "pipe", "pipe"],
        env: f
      });
    } else {
      const s = String(r.prompt ?? ""), l = String(r.resolution ?? "896x512"), { height: d, width: u } = iw[l] ?? { height: 512, width: 896 }, c = Number(r.frame_rate ?? 24), p = Number(r.duration_secs ?? 4), f = Math.round(p * c / 8) * 8 + 1, m = Math.max(9, f), y = Number(r.seed ?? 42), h = !!r.enhance_prompt;
      let g = null;
      if (r.image_url) {
      ...
      g && _.push("--image_path", g), h && _.push("--enhance_prompt"), o = ae(K_, _, {
        cwd: no,
        stdio: ["ignore", "pipe", "pipe"]
      });
    }
...
`)) {
        ...
        if (d)
          try {
            ...
            u.type === "progress" ? (i.stage = u.stage, u.output_text !== void 0 && (i.outputText = u.output_text), u.segments && (i.segments = u.segments), u.language !== void 0 && (i.language = u.language), _t(n, {
              type: "progress",
              stage: u.stage,
              message: u.message,
            ...
            })) : u.type === "done" ? (i.status = "done", i.outputPath = u.output_path, i.outputText = u.output_text, i.transcriptPath = u.transcript_path, i.segments = u.segments, i.language = u.language, _t(n, {
              type: "done",
            ...
            })) : u.type === "error" && (i.status = "error", i.error = u.error, _t(n, { type: "error", error: u.error }));
          } catch {
          }
      }
    }), o.stderr.on("data", () => {
...
const oo = w.join(H.homedir(), "Desktop", "Coding", "Sam3"), sw = w.join(oo, ".venv", "bin", "python"), lw = w.join(oo, "cinegen_server.py"), dw = 120 * 1e3, uw = 500, cw = 60;
class fw {
  constructor() {
  ...
  async start() {
    var e, r;
    return this.proc && !this.proc.killed ? this.port : (this.port = await this.findFreePort(), console.log(`[sam3] Starting server on port ${this.port}`), this.proc = ae(sw, [lw, "--port", String(this.port)], {
      cwd: oo,
      stdio: ["ignore", "pipe", "pipe"],
      env: {
        ...process.env,
  ...
  async waitForHealth() {
    ...
    for (let e = 0; e < cw; e++) {
      ...
      await new Promise((r) => setTimeout(r, uw));
    }
...
async function gw(t) {
  const e = t.workspaceDir ?? w.join(z.getPath("userData"), "gemini-cli-workspace"), r = await Qi(t.visualRefs, e);
  if (r.length === 0)
    throw new Error("Could not load the attached clip or asset files for visual analysis.");
  try {
    const n = [];
    for (const i of r) {
      const o = hw(t.prompt, i.label, i.mediaType), a = i.mediaType === "video" ? await Vl({
        apiKey: t.apiKey,
...
function yw() {
  I.handle("copilot:analyze-visual-refs", async (t, e) => gw(e));
}
const _w = 1, ww = -30, bw = 0.3;
function vw(t) {
  const e = [];
  let r = null;
  for (const n of t.split(/\r?\n/)) {
...
`);
  const n = r.map((i) => `[${za(i.start)}-${za(i.end)}] ${i.text}`).join(`
`);
  return [
...
function Iw(t) {
  ...
  return (Array.isArray(r) ? r : r && typeof r == "object" && Array.isArray(r.segments) ? r.segments : []).flatMap((i) => {
    ...
    const o = i, a = Jn(o.start), s = Jn(o.end);
...
function xw(t) {
  return new Promise((e) => {
    const r = [
      ...
      t,
      "-af",
      `silencedetect=noise=${ww}dB:d=${bw}`,
      "-f",
      "null",
      "-"
    ], n = ae(ve(), r);
...
const Id = "gemini-2.5-flash", Aw = "fal-ai/video-understanding";
async function kw(t, e) {
  var n;
  const r = ((n = t.model) == null ? void 0 : n.trim()) || Id;
  try {
    return { rawText: await dd({
...
async function jw(t) {
  ...
  const e = {
    ...
    status: "failed",
    version: _w,
    model: ((r = t.model) == null ? void 0 : r.trim()) || Id,
    silenceMap: [],
    segments: [],
  ...
  try {
    const n = await xw(t.mediaPath).catch(() => ""), i = vw(n), o = Tw({ assetName: t.assetName, transcript: t.transcript }), { rawText: a, model: s } = await kw(t, o), l = Iw(a);
    return {
      ...e,
      model: s,
      status: "ready",
...
function Rw() {
  I.handle("acoustic:analyze-asset", async (t, e) => jw(e));
}
const Pw = process.platform === "darwin" && !z.isPackaged;
Pw && (z.disableHardwareAcceleration(), z.commandLine.appendSwitch("disable-gpu-compositing"), console.log("[app] hardware acceleration disabled for macOS dev wake stability"));
z.commandLine.appendSwitch("disable-renderer-backgrounding");
z.commandLine.appendSwitch("disable-backgrounding-occluded-windows");
Qa.registerSchemesAsPrivileged([
  {
    scheme: "local-media",
    privileges: {
      standard: !0,
...
const Ow = Date.now(), Ha = "cinegen-desktop", Wa = "CineGen", Nw = ".cinegen-user-data-migrated.json", Xa = "CineGen", Cw = 700;
...
function Lw() {
  try {
    const t = z.getPath("appData"), e = w.join(t, Ha), r = w.join(t, Wa);
    return z.getPath("userData") !== r && z.setPath("userData", r), console.log("[app] userData path:", r), { preferredUserDataPath: r, legacyUserDataPath: e };
  } catch (t) {
    console.error("[app] failed to configure userData path:", t);
    const e = z.getPath("appData"), r = w.join(e, Wa), n = w.join(e, Ha);
...
Gd();
try {
  z.setName(Xa), process.platform === "darwin" && z.setAboutPanelOptions({
    applicationName: Xa,
    applicationVersion: z.getVersion(),
...
async function Mw() {
  const { preferredUserDataPath: t, legacyUserDataPath: e } = Uw;
  if (t === e || !$.existsSync(e)) return;
  const r = w.join(t, Nw);
  if (!$.existsSync(r))
    try {
...
function Dw() {
  ...
  for (const n of e)
    for (const i of t) {
      ...
      $.existsSync(o) && r.push(o);
    }
  return r;
}
function $w(t) {
  const e = w.extname(t).toLowerCase();
  return qw[e] ?? "application/octet-stream";
}
function Fw(t, e) {
...
async function Gw() {
  var n, i, o, a;
  const t = w.join(process.cwd(), ".data", "dev", "project.json"), e = w.join(H.homedir(), "Documents", "CINEGEN"), r = w.join(e, "projects.json");
  try {
    await j.access(t);
  } catch {
    return;
  ...
  try {
    const s = await j.readFile(t, "utf-8"), l = JSON.parse(s), d = ((n = l.project) == null ? void 0 : n.id) || X.randomUUID(), u = ((i = l.project) == null ? void 0 : i.name) || "Migrated Project";
    await j.mkdir(w.join(e, d), { recursive: !0 }), await j.writeFile(
    ...
    await j.writeFile(r, JSON.stringify(c, null, 2), "utf-8"), console.log(`[migration] Migrated legacy project "${u}" to ${e}/${d}`);
  } catch (s) {
    console.error("[migration] Failed to migrate legacy data:", s);
  }
}
z.whenReady().then(async () => {
  if (await Mw(), process.platform === "darwin") {
    const r = Dw();
    console.log("[dock] icon candidates:", r);
    for (const n of r)
      try {
        const i = Ka.createFromPath(n);
        if (console.log("[dock] testing icon:", n, "empty?", i.isEmpty()), !i.isEmpty()) {
          await Promise.resolve(z.dock.setIcon(i)), console.log("[dock] applied icon:", n);
          break;
        }
      } catch (i) {
  ...
  Qa.handle("local-media", async (r) => {
    try {
      ...
      const o = i.size, a = $w(n), s = Fw(r.headers, "range");
      ...
      if (s) {
        ...
        const f = p - c + 1, m = $.createReadStream(n, { start: c, end: p }), y = lo.toWeb(m);
      ...
      const l = $.createReadStream(n), d = lo.toWeb(l);
  ...
  }), ru(), ou(() => ee), Vd(), Ep(), Bc(), qp(), Qm(), rh(), ah(), bh(), og(), vg(), xg(), Lg(), Mg(), sy(), ny(), dy(), uy(), Ay(), Ry(), Py(), s_(), b_(), T_(Se), qh(), yw(), Rw(), R_(), J_(), aw(), pw(), await Gw(), I.handle("pm:open-project", async (r, n, i) => n === "__close__" ? (te == null || te.close(), te = null, { ok: !0 }) : ((!ee || ee.isDestroyed()) && (ee = mo()), ee.once("ready-to-show", () => {
    ee == null || ee.maximize(), ee == null || ee.show(), ee == null || ee.webContents.send("pm:open-project", n, i);
  }), ee.webContents.getURL() !== "" && (ee.maximize(), ee.show(), ee.webContents.send("pm:open-project", n, i)), te == null || te.close(), te = null, { ok: !0 })), I.handle("pm:open", async () => te && !te.isDestroyed() ? (te.focus(), { ok: !0 }) : (te = Sn(), te.on("closed", () => {
    te = null;
  }), { ok: !0 })), rr = Qd(), ee = mo();
  const t = 3e3;
  ee.once("ready-to-show", () => {
    const r = Date.now() - Ow, n = Math.max(0, t - r);
    setTimeout(() => {
      rr == null || rr.close(), rr = null, te = Sn(), te.on("closed", () => {
        te = null;
      });
    }, n);
  }), z.on("activate", () => {
    Z.getAllWindows().length === 0 && (te = Sn(), te.on("closed", () => {
      te = null;
    }));
  });
  const e = (r) => {
    Ve && (clearTimeout(Ve), Ve = null), Ve = setTimeout(() => {
      Ve = null, console.log(`[app] Wake recovery triggered by ${r}`), Yd(r);
    }, Cw);
  };
  bn.on("resume", () => {
    Kn("resume"), e("resume");
  }), bn.on("unlock-screen", () => {
    Kn("unlock-screen"), e("unlock-screen");
  }), bn.on("suspend", () => {
    Kn("suspend");
  });
});
z.on("before-quit", () => {
  au(), Ve && (clearTimeout(Ve), Ve = null), v_(), l_(), mw();
});
