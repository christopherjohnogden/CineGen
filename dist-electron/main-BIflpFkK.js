...
var Id = (t, e, r) => e in t ? Sd(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : t[e] = r;
var M = (t, e, r) => Id(t, typeof e != "symbol" ? e + "" : e, r);
import { ipcMain as I, app as G, shell as Kr, BrowserWindow as Z, screen as Xa, safeStorage as Ar, session as Ja, dialog as so, protocol as Ka, nativeImage as xd, powerMonitor as wn } from "electron";
import D, { existsSync as ot, readFileSync as _r, mkdirSync as Yr, writeFileSync as vi, copyFileSync as Ad, chmodSync as kd, renameSync as jd, rmSync as Ya } from "node:fs";
import w, { dirname as Rd, join as Et } from "node:path";
import z, { homedir as Qa } from "node:os";
import { execFile as qe, spawn as ae } from "node:child_process";
import { promisify as Ut } from "node:util";
import X, { randomUUID as wr, randomBytes as Pd } from "node:crypto";
import R, { writeFile as Za, chmod as Od, mkdir as st, mkdtemp as Nd, rm as Cd } from "node:fs/promises";
import { Readable as lo } from "node:stream";
import Ti from "better-sqlite3";
import { createServer as es } from "node:http";
import { lookup as qd } from "node:dns/promises";
import { request as Ld } from "node:https";
import Ud, { isIP as Ei } from "node:net";
import { createRequire as ts } from "node:module";
import { fileURLToPath as Si } from "node:url";
import { Worker as rs } from "worker_threads";
function br(t) {
  return !!t && typeof t == "object" && !Array.isArray(t);
}
function Ii(t) {
  if (!ot(t)) return { raw: null, config: {} };
  const e = _r(t, "utf8");
  let r;
  try {
    r = JSON.parse(e);
  ...
  if (!br(r) || r.mcpServers !== void 0 && !br(r.mcpServers))
    throw new Error("Claude’s configuration has an unexpected format. No changes were made.");
  return { raw: e, config: r };
}
function ns(t) {
  return br(t.mcpServers) ? t.mcpServers.cinegen : void 0;
}
function Md(t, e) {
  if (!br(t) || !br(t.env)) return !1;
  const r = t.env;
  return t.command === e.command && JSON.stringify(t.args) === JSON.stringify(e.args) && Object.entries(e.env).every(([n, i]) => r[n] === i);
}
function uo(t, e) {
  const r = Ii(t), n = { ...r.config.mcpServers };
  if (e ? n.cinegen = e : delete n.cinegen, !e && !ns(r.config)) return {};
  const i = { ...r.config, mcpServers: n };
  Yr(Rd(t), { recursive: !0 });
  const o = r.raw === null ? void 0 : `${t}.cinegen-backup-${Date.now()}-${wr()}`, a = `${t}.cinegen-${wr()}.tmp`;
  try {
    if (vi(a, `${JSON.stringify(i, null, 2)}
`, { mode: 384, flag: "wx" }), (ot(t) ? _r(t, "utf8") : null) !== r.raw) throw new Error("Claude’s configuration changed during setup. Try again.");
    o && (Ad(t, o), kd(o, 384)), jd(a, t);
  } finally {
    Ya(a, { force: !0 });
  }
  return { backupPath: o };
...
function vt() {
  return {
    config: Et(Qa(), "Library", "Application Support", "Claude", "claude_desktop_config.json"),
    source: Et(G.getAppPath(), "dist-electron", "cinegen-mcp.cjs"),
    installed: Et(G.getPath("userData"), "mcp", "cinegen-mcp.cjs")
  };
}
function is() {
  return { command: process.execPath, args: [vt().installed], env: { ELECTRON_RUN_AS_NODE: "1" } };
}
function bn() {
...
async function $d(t) {
  ...
  try {
    await Dd(process.execPath, ["-e", e, t], {
...
function vn() {
  const t = vt(), e = Ii(t.config).config, r = ns(e), n = ot(t.source) && ot(t.installed) && _r(t.source).equals(_r(t.installed));
  return {
    supported: process.platform === "darwin",
    configured: !!r,
    needsRepair: !!r && (!Md(r, is()) || !n),
    serverAvailable: ot(t.source),
    configPath: t.config
  };
}
function Fd() {
  I.handle("claude-mcp:status", () => vn()), I.handle("claude-mcp:setup", async () => {
    bn();
    const t = vt();
    if (Ii(t.config), !ot(t.source)) throw new Error("This build is missing the MCP server. Install the latest CineGen Mac build.");
    await $d(t.source), Yr(Et(G.getPath("userData"), "mcp"), { recursive: !0 }), vi(t.installed, _r(t.source), { mode: 384 });
    const e = uo(t.config, is());
    return { ...vn(), ...e };
  }), I.handle("claude-mcp:remove", () => {
    bn();
    const t = uo(vt().config, null);
    return { ...vn(), ...t };
  }), I.handle("claude-mcp:reveal", () => {
    if (bn(), !ot(vt().config)) throw new Error("Connect Claude Desktop first to create its configuration.");
    Kr.showItemInFolder(vt().config);
  });
}
function Bd() {
  if (G.isPackaged) return;
  const t = w.join(G.getPath("userData"), "dev-browser-session");
  Yr(t, { recursive: !0 }), G.setPath("sessionData", t);
}
const Vd = 1200, Gd = 150, zd = 1e3, co = 2800, os = /* @__PURE__ */ new WeakMap(), as = /* @__PURE__ */ new WeakMap(), $r = /* @__PURE__ */ new WeakMap(), ss = /* @__PURE__ */ new WeakMap();
function Hd() {
...
const Rt = Hd(), Qr = w.join(import.meta.dirname, "."), ls = w.join(Qr, "../dist"), vr = process.env.VITE_DEV_SERVER_URL;
function fo(t) {
  return vr ? t.loadURL(`${vr}?pm=1`) : t.loadFile(w.join(ls, "index.html"), { query: { pm: "1" } });
}
function po(t) {
  return vr ? t.loadURL(vr) : t.loadFile(w.join(ls, "index.html"));
}
function Tn(t, e) {
  const r = $r.get(t) ?? /* @__PURE__ */ new Set();
  r.add(e), $r.set(t, r);
}
function kr(t, e) {
  var r;
  (r = $r.get(t)) == null || r.delete(e);
}
function ds(t) {
  const e = $r.get(t);
  if (e) {
...
function Wd(t) {
  return new Promise((e, r) => {
    ...
    const i = () => {
      ...
      n || !u || l === -3 || (n = !0, i(), r(new Error(`did-fail-load ${l}: ${d}`)));
    };
...
async function Xd(t, e, r) {
  if (!t.isDestroyed())
    try {
      ...
      await Kn(t, e, r, "blank renderer DOM after resume");
    } catch (n) {
      console.warn(`[window] ${e} health check failed after wake:`, n), await Kn(t, e, r, "resume health check failed");
    }
}
function Jd(t) {
  for (const e of Z.getAllWindows()) {
    if (e.isDestroyed()) continue;
    const r = os.get(e);
    if (!r) continue;
    const n = as.get(e) ?? "window";
    ds(e), ss.set(e, Date.now() + co + 1e3);
    let i = null;
    const o = setTimeout(() => {
      kr(e, o), !e.isDestroyed() && (console.log(`[window] ${n} wake recovery started: ${t}`), e.webContents.invalidate(), e.webContents.executeJavaScript(
        `(() => {
    ...
    const a = setTimeout(() => {
      kr(e, a), (async () => {
        try {
          await Xd(e, n, r), i && (clearTimeout(i), kr(e, i), i = null);
        } catch (s) {
    ...
    Tn(e, a), i = setTimeout(() => {
      kr(e, i), !e.isDestroyed() && Kn(e, n, r, `hard reload after ${t}`).catch((s) => {
...
function En() {
  const { width: t, height: e } = Xa.getPrimaryDisplay().workAreaSize, r = 900, n = 580, i = new Z({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
    ...
    ...Rt ? { icon: Rt } : {},
    webPreferences: {
      preload: w.join(Qr, "preload.js"),
      nodeIntegration: !1,
      contextIsolation: !0,
      backgroundThrottling: !1
    }
  });
  return us(i, "project-manager", fo), fo(i), i;
}
function Kd() {
  const { width: t, height: e } = Xa.getPrimaryDisplay().workAreaSize, r = 800, n = 395, i = new Z({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
  ...
  return i.loadFile(w.join(Qr, "splash.html")), i;
}
function mo() {
  const t = new Z({
    width: 1440,
  ...
  return us(t, "main", po), po(t), vr && t.webContents.openDevTools({ mode: "detach" }), t;
}
function xi() {
  return w.join(z.homedir(), "Documents", "CINEGEN");
}
function Yn() {
  return w.join(xi(), "projects.json");
}
function Tr(t) {
  return w.join(xi(), t);
}
function ct(t) {
  return w.join(Tr(t), "project.json");
}
function cs() {
  return X.randomUUID();
}
...
async function ps() {
  await R.mkdir(xi(), { recursive: !0 });
}
async function jr() {
  try {
    const t = await R.readFile(Yn(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function Sn(t) {
  await ps();
  const e = Yn() + ".tmp";
  await R.writeFile(e, JSON.stringify(t, null, 2), "utf-8"), await R.rename(e, Yn());
}
function Yd(t, e) {
  const r = fs(), n = {
    id: cs(),
...
function Qd(t) {
  const e = w.join(Tr(t), "project.json");
...
function Zd(t) {
  const e = w.join(Tr(t), "project.db");
  if (!D.existsSync(e)) return null;
  try {
    const r = new Ti(e, { readonly: !0 }), n = r.prepare(
...
function eu() {
  I.handle("project:list", async () => (await jr()).projects.map((e) => {
    const r = e.useSqlite ? Zd(e.id) : Qd(e.id);
    ...
    const n = cs(), i = Yd(n, r);
    await ps(), await R.mkdir(Tr(n), { recursive: !0 });
    const o = ct(n) + ".tmp";
    await R.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await R.rename(o, ct(n));
    const a = await jr();
    ...
    const r = await R.readFile(ct(e), "utf-8");
    return JSON.parse(r);
  }), I.handle("project:save", async (t, e, r) => {
    let n;
    try {
      const l = await R.readFile(ct(e), "utf-8");
      n = JSON.parse(l);
    } catch {
    ...
    await R.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await R.rename(o, ct(e));
    const a = await jr(), s = a.projects.find((l) => l.id === e);
    return s && (s.updatedAt = i.project.updatedAt, s.assetCount = Array.isArray(i.assets) ? i.assets.length : 0, s.elementCount = Array.isArray(i.elements) ? i.elements.length : 0, r.project && r.project.name && (s.name = r.project.name), await Sn(a)), i;
  }), I.handle("project:delete", async (t, e) => {
    await R.rm(Tr(e), { recursive: !0, force: !0 });
    const r = await jr();
    r.projects = r.projects.filter((n) => n.id !== e), await Sn(r);
  });
}
const go = 12e4, ms = Et(Qa(), "Documents", "CINEGEN"), Qn = Et(ms, "mcp-bridge.json"), Pt = /* @__PURE__ */ new Map();
let qr = !1, ge = null, Rr = "";
function Re(t, e, r) {
...
function ru(t, e, r) {
  return new Promise((n, i) => {
    const o = wr(), a = setTimeout(() => {
      Pt.delete(o), i(new Error(`CineGen did not answer "${e}" within ${Math.round(go / 1e3)}s.`));
    }, go);
    Pt.set(o, { resolve: n, reject: i, timer: a }), t.webContents.send("mcp:invoke", { id: o, tool: e, args: r });
  });
}
function nu(t) {
  I.on("mcp:ready", (e, r) => {
    ...
    const n = r != null && r.id ? Pt.get(r.id) : void 0;
    n && (clearTimeout(n.timer), Pt.delete(r.id), r.ok ? n.resolve(r.result) : n.reject(new Error(r.error || "The tool failed.")));
  }), I.handle("mcp:status", () => {
    var e;
    return {
      running: !!ge,
      workspaceReady: qr,
      port: ge ? ((e = ge.address()) == null ? void 0 : e.port) ?? 0 : 0,
      discoveryFile: Qn
    };
  }), Rr = Pd(24).toString("hex"), ge = es((e, r) => {
    (async () => {
      try {
        ...
        if (i.length !== Rr.length || i !== Rr) {
          Re(r, 401, { ok: !1, error: "Bad or missing bridge token." });
          return;
        }
        if (e.method === "POST" && n.pathname === "/invoke") {
          const o = await tu(e), a = typeof (o == null ? void 0 : o.tool) == "string" ? o.tool : "";
          if (!a) {
  ...
  }), ge.listen(0, "127.0.0.1", () => {
    var r;
    const e = ((r = ge == null ? void 0 : ge.address()) == null ? void 0 : r.port) ?? 0;
    try {
      Yr(ms, { recursive: !0 }), vi(
        Qn,
        `${JSON.stringify({ port: e, token: Rr, pid: process.pid, startedAt: (/* @__PURE__ */ new Date()).toISOString() }, null, 2)}
`,
        { mode: 384 }
...
function iu() {
  ...
  Pt.clear(), ge == null || ge.close(), ge = null;
  try {
    Ya(Qn, { force: !0 });
  } catch {
  }
...
function gs() {
  return ho || (ho = 1, (function(t) {
    var e = zt && zt.__awaiter || function(i, o, a, s) {
      function l(d) {
        return d instanceof a ? d : new a(function(c) {
          c(d);
        });
      }
      return new (a || (a = Promise))(function(d, c) {
        function u(m) {
          try {
            f(s.next(m));
          } catch (y) {
        ...
        function f(m) {
          m.done ? d(m.value) : l(m.value).then(u, p);
        }
...
function dt() {
  if (_o) return Ee;
  _o = 1;
  var t = Ee && Ee.__awaiter || function(s, l, d, c) {
    function u(p) {
    ...
    return new (d || (d = Promise))(function(p, f) {
      ...
      function g(h) {
        h.done ? p(h.value) : u(h.value).then(m, y);
      }
      g((c = c.apply(s, l || [])).next());
    });
  };
  ...
  const e = Ai(), r = "x-fal-request-id";
  class n extends Error {
    constructor({ message: l, status: d, body: c, requestId: u, timeoutType: p }) {
      super(l), this.name = "ApiError", this.status = d, this.body = c, this.requestId = u || "", this.timeoutType = p;
    }
    /**
  ...
  function o(s) {
    return t(this, void 0, void 0, function* () {
      var l;
      const { status: d, statusText: c } = s, u = (l = s.headers.get("Content-Type")) !== null && l !== void 0 ? l : "", p = s.headers.get(r) || void 0, f = s.headers.get(e.REQUEST_TIMEOUT_TYPE_HEADER) || void 0;
      if (!s.ok) {
      ...
      return u.includes("application/json") ? s.json() : u.includes("text/html") ? s.text() : u.includes("application/octet-stream") ? s.arrayBuffer() : s.text();
    });
  }
  function a(s) {
    return t(this, void 0, void 0, function* () {
...
function Ke() {
  if (wo) return pe;
  wo = 1;
  var t = pe && pe.__awaiter || function(u, p, f, m) {
    function y(g) {
    ...
    return new (f || (f = Promise))(function(g, h) {
      ...
      function v(T) {
        T.done ? g(T.value) : y(T.value).then(_, b);
      }
      v((m = m.apply(u, p || [])).next());
    });
  };
  Object.defineProperty(pe, "__esModule", { value: !0 }), pe.ensureEndpointIdFormat = e, pe.parseEndpointId = n, pe.resolveEndpointPath = i, pe.isValidUrl = o, pe.throttle = a, pe.isReact = l, pe.isPlainObject = d, pe.sleep = c;
  function e(u) {
    if (u.split("/").length > 1)
      return u;
    const [, f, m] = /^([0-9]+)-([a-zA-Z0-9-]+)$/.exec(u) || [];
    if (f && m)
      return `${f}/${m}`;
    throw new Error(`Invalid app id: ${u}. Must be in the format <appOwner>/<appId>`);
  }
  ...
  function a(u, p, f = !1) {
    let m, y;
    return (...g) => {
      !y && f ? (u(...g), y = Date.now()) : (m && clearTimeout(m), m = setTimeout(() => {
        Date.now() - y >= p && (u(...g), y = Date.now());
      }, p - (Date.now() - y)));
    };
  }
  let s;
  function l() {
    if (s === void 0) {
      ...
      s = !!u && (u.includes("node_modules/react-dom/") || u.includes("node_modules/next/"));
    }
    return s;
  }
  function d(u) {
    return !!u && Object.getPrototypeOf(u) === Object.prototype;
  }
  function c(u) {
    return t(this, void 0, void 0, function* () {
      return new Promise((p) => setTimeout(p, u));
    });
  }
  return pe;
}
...
function Zr() {
  return bo || (bo = 1, (function(t) {
    var e = Ht && Ht.__awaiter || function(s, l, d, c) {
      function u(p) {
        return p instanceof d ? p : new d(function(f) {
          f(p);
        });
    ...
    function o(s, l, d, c, u) {
      const p = Math.min(l * Math.pow(c, s), d);
      if (u) {
    ...
    function a(s, l, d) {
      return e(this, void 0, void 0, function* () {
        ...
        for (let p = 0; p <= l.maxRetries; p++) {
          c.totalAttempts++;
          try {
            return { result: yield s(), metrics: c };
          } catch (f) {
            if (u = f, c.lastError = f, p === l.maxRetries || !i(f, l.retryableStatusCodes))
              throw f;
            const m = o(p, l.baseDelay, l.maxDelay, l.backoffMultiplier, l.enableJitter);
            c.totalDelay += m, d && d(p + 1, f, m), yield (0, n.sleep)(m);
          }
        }
        throw u;
      });
...
var Wt = {};
const au = "@fal-ai/client", su = "1.9.4", lu = {
  name: au,
  version: su
};
var vo;
function ki() {
  if (vo) return Wt;
  vo = 1, Object.defineProperty(Wt, "__esModule", { value: !0 }), Wt.isBrowser = t, Wt.getUserAgent = r;
  function t() {
...
function ji() {
  return To || (To = 1, (function(t) {
    Object.defineProperty(t, "__esModule", { value: !0 }), t.credentialsFromEnv = void 0, t.resolveDefaultFetch = o, t.createConfig = d, t.getRestApiUrl = c;
    const e = gs(), r = dt(), n = Zr(), i = ki();
    function o() {
      if (typeof fetch > "u")
    ...
    const l = {
      ...
      requestMiddleware: (u) => Promise.resolve(u),
      responseHandler: r.defaultResponseHandler,
      retry: n.DEFAULT_RETRY_OPTIONS
    };
    function d(u) {
      var p;
      let f = Object.assign(Object.assign(Object.assign({}, l), u), {
        fetch: (p = u.fetch) !== null && p !== void 0 ? p : o(),
        // Merge retry configuration with defaults
        retry: Object.assign(Object.assign({}, n.DEFAULT_RETRY_OPTIONS), u.retry || {})
      });
      u.proxyUrl && (f = Object.assign(Object.assign({}, f), { requestMiddleware: (0, e.withMiddleware)(f.requestMiddleware, (0, e.withProxy)({ targetUrl: u.proxyUrl })) }));
...
function Sr() {
  if (Eo) return Oe;
  Eo = 1;
  var t = Oe && Oe.__awaiter || function(l, d, c, u) {
    function p(f) {
      return f instanceof c ? f : new c(function(m) {
        m(f);
      });
    }
    return new (c || (c = Promise))(function(f, m) {
      ...
      h((u = u.apply(l, d || [])).next());
    });
  }, e = Oe && Oe.__rest || function(l, d) {
    var c = {};
    for (var u in l) Object.prototype.hasOwnProperty.call(l, u) && d.indexOf(u) < 0 && (c[u] = l[u]);
    if (l != null && typeof Object.getOwnPropertySymbols == "function")
      for (var p = 0, u = Object.getOwnPropertySymbols(l); p < u.length; p++)
        d.indexOf(u[p]) < 0 && Object.prototype.propertyIsEnumerable.call(l, u[p]) && (c[u[p]] = l[u[p]]);
    return c;
  };
  Object.defineProperty(Oe, "__esModule", { value: !0 }), Oe.dispatchRequest = a, Oe.buildUrl = s;
  const r = Zr(), n = ki(), i = Ke(), o = typeof navigator < "u" && (navigator == null ? void 0 : navigator.userAgent) === "Cloudflare-Workers";
  function a(l) {
    return t(this, void 0, void 0, function* () {
      var d;
      const { targetUrl: c, input: u, config: p, options: f = {} } = l, { credentials: m, requestMiddleware: y, responseHandler: g, fetch: h } = p, _ = Object.assign(Object.assign({}, p.retry), f.retry || {}), b = () => t(this, void 0, void 0, function* () {
        var T, S, x;
        const E = (0, n.isBrowser)() ? {} : { "User-Agent": (0, n.getUserAgent)() }, k = typeof m == "function" ? m() : m, { method: O, url: N, headers: B } = yield y({
          method: ((S = (T = l.method) !== null && T !== void 0 ? T : f.method) !== null && S !== void 0 ? S : "post").toUpperCase(),
          url: c,
          headers: l.headers
        }), H = k ? { Authorization: `Key ${k}` } : {}, W = Object.assign(Object.assign(Object.assign(Object.assign({}, H), { Accept: "application/json", "Content-Type": "application/json" }), E), B ?? {}), { responseHandler: J, retry: j } = f, L = e(f, ["responseHandler", "retry"]), U = yield h(N, Object.assign(Object.assign(Object.assign(Object.assign({}, L), { method: O, headers: Object.assign(Object.assign({}, W), (x = L.headers) !== null && x !== void 0 ? x : {}) }), !o && { mode: "cors" }), { signal: f.signal, body: O.toLowerCase() !== "get" && u ? JSON.stringify(u) : void 0 }));
        return yield (J ?? g)(U);
      });
      let v;
      for (let T = 0; T <= _.maxRetries; T++)
        try {
          ...
          if (v = S, T === _.maxRetries || !(0, r.isRetryableError)(S, _.retryableStatusCodes) || ((d = f.signal) === null || d === void 0 ? void 0 : d.aborted))
            throw S;
          const E = (0, r.calculateBackoffDelay)(T, _.baseDelay, _.maxDelay, _.backoffMultiplier, _.enableJitter);
          yield (0, i.sleep)(E);
        }
      throw v;
    });
  ...
  function s(l, d = {}) {
    var c, u;
    const p = ((c = d.method) !== null && c !== void 0 ? c : "post").toLowerCase(), f = ((u = d.path) !== null && u !== void 0 ? u : "").replace(/^\//, "").replace(/\/{2,}/, "/"), m = d.input, y = Object.assign(Object.assign({}, d.query || {}), p === "get" ? m : {}), g = Object.keys(y).length > 0 ? `?${new URLSearchParams(y).toString()}` : "";
    if ((0, i.isValidUrl)(l))
...
function hs() {
  return So || (So = 1, (function(t) {
    var e = Xt && Xt.__awaiter || function(m, y, g, h) {
      function _(b) {
        return b instanceof g ? b : new g(function(v) {
          v(b);
        });
      }
      return new (g || (g = Promise))(function(b, v) {
        function T(E) {
          try {
            x(h.next(E));
          } catch (k) {
        ...
        function x(E) {
          E.done ? b(E.value) : _(E.value).then(T, S);
        }
        x((h = h.apply(m, y || [])).next());
      });
    };
    ...
    function u(m, y, g) {
      return e(this, arguments, void 0, function* (h, _, b, v = 3) {
        if (v === 0)
        ...
        try {
          ...
          return yield u(h, _, b, v - 1);
        }
      });
    }
    function p(m, y, g) {
      return e(this, void 0, void 0, function* () {
        const { fetch: h, responseHandler: _ } = y, b = m.type || "application/octet-stream", { upload_url: v, file_url: T } = yield c(m, y, b, g), S = 10 * 1024 * 1024, x = Math.ceil(m.size / S), E = new URL(v), k = [];
        for (let B = 0; B < x; B++) {
          const H = B * S, W = Math.min(H + S, m.size), J = m.slice(H, W), j = B + 1, L = `${E.origin}${E.pathname}/${j}${E.search}`;
          k.push(yield u(L, J, y));
        }
        const O = `${E.origin}${E.pathname}/complete${E.search}`, N = yield h(O, {
          method: "POST",
          headers: {
    ...
    function f({ config: m }) {
      const y = {
        upload: (g, h) => e(this, void 0, void 0, function* () {
          ...
          const b = g.type || "application/octet-stream", { fetch: v, responseHandler: T } = m, { upload_url: S, file_url: x } = yield d(g, m, b, _), E = yield v(S, {
            method: "PUT",
            body: g,
            headers: {
        ...
        transformInput: (g) => e(this, void 0, void 0, function* () {
          ...
          if ((0, i.isPlainObject)(g)) {
            const _ = Object.entries(g).map((v) => e(this, [v], void 0, function* ([T, S]) {
              return [T, yield y.transformInput(S)];
...
function du() {
  if (Io) return Pr;
  Io = 1, Object.defineProperty(Pr, "__esModule", {
    value: !0
  });
  function t(n) {
    let i, o, a, s, l, d, c;
    ...
    function u() {
      i = !0, o = "", a = 0, s = -1, l = void 0, d = void 0, c = "";
    }
    function p(m) {
...
` && ++g, h = !1);
        ...
        for (let T = a; _ < 0 && T < y; ++T)
          v = o[T], v === ":" && b < 0 ? b = T - g : v === "\r" ? (h = !0, _ = T - g) : v === `
` && (_ = T - g);
        if (_ < 0) {
          a = y - g, s = b;
          break;
    ...
    function f(m, y, g, h) {
      ...
      let v = 0;
      _ ? v = h : m[y + g + 1] === " " ? v = g + 2 : v = g + 1;
      const T = y + v, S = h - v, x = m.slice(T, T + S).toString();
      if (b === "data")
        c += x ? "".concat(x, `
`) : `
`;
  ...
  return Pr.createParser = t, Pr;
}
var Jt = {}, xo;
function ys() {
  return xo || (xo = 1, (function(t) {
    var e = Jt && Jt.__awaiter || function(a, s, l, d) {
      function c(u) {
        return u instanceof l ? u : new l(function(p) {
          p(u);
        });
      }
      return new (l || (l = Promise))(function(u, p) {
        function f(g) {
          try {
            y(d.next(g));
          } catch (h) {
        ...
        function y(g) {
          g.done ? u(g.value) : c(g.value).then(f, m);
        }
...
function _s() {
  ...
  var t = _e && _e.__awaiter || function(p, f, m, y) {
    ...
    return new (m || (m = Promise))(function(h, _) {
      ...
      function T(S) {
        S.done ? h(S.value) : g(S.value).then(b, v);
      }
    ...
    function _(k) {
      return function(O) {
        return Promise.resolve(O).then(k, x);
      };
    }
    function b(k, O) {
      y[k] && (g[k] = function(N) {
        return new Promise(function(B, H) {
          h.push([k, N, B, H]) > 1 || v(k, N);
        });
      }, O && (g[k] = O(g[k])));
    }
    function v(k, O) {
    ...
    function T(k) {
      k.value instanceof e ? Promise.resolve(k.value.v).then(S, x) : E(h[0][2], k);
    }
    function S(k) {
      v("next", k);
    }
    ...
    function E(k, O) {
      k(O), h.shift(), h.length && v(h[0][0], h[0][1]);
    }
  };
  Object.defineProperty(_e, "__esModule", { value: !0 }), _e.FalStream = void 0, _e.createStreamingClient = u;
  const n = /* @__PURE__ */ du(), i = ys(), o = Sr(), a = dt(), s = Ke(), l = "text/event-stream", d = 15 * 1e3;
  class c {
    constructor(f, m, y) {
      var g;
      this.listeners = /* @__PURE__ */ new Map(), this.buffer = [], this.currentData = void 0, this.lastEventTimestamp = 0, this.streamClosed = !1, this._requestId = null, this.abortController = new AbortController(), this.start = () => t(this, void 0, void 0, function* () {
        var h, _, b;
        const { endpointId: v, options: T } = this, { input: S, method: x = "post", connectionMode: E = "server", tokenProvider: k } = T;
        try {
          if (E === "client") {
            const O = (0, s.ensureEndpointIdFormat)(v), N = (h = (0, s.resolveEndpointPath)(v, void 0, "/stream")) !== null && h !== void 0 ? h : "", H = yield (k ? () => k(`${O}${N}`) : () => (console.warn('[fal.stream] Using the default token provider is deprecated. Please provide a `tokenProvider` function when using `connectionMode: "client"`. See https://docs.fal.ai/fal-client/authentication for more information.'), (0, i.getTemporaryAuthToken)(v, this.config)))(), { fetch: W } = this.config, J = new URL(this.url);
            J.searchParams.set("fal_jwt_token", H);
            const j = yield W(J.toString(), {
            ...
            return this._requestId = j.headers.get("x-fal-request-id"), yield this.handleResponse(j);
          }
        ...
        if (!((_ = h.headers.get("content-type")) !== null && _ !== void 0 ? _ : "").startsWith(l)) {
          const N = v.getReader(), B = () => {
            N.read().then(({ done: H, value: W }) => {
              if (H) {
                this.emit("done", this.currentData);
                return;
              }
              this.buffer.push(W), this.currentData = W, this.emit("data", W), B();
            });
          };
          B();
          return;
        }
        const S = new TextDecoder("utf-8"), x = h.body.getReader(), E = (0, n.createParser)((N) => {
          if (N.type === "event") {
            const B = N.data;
            try {
              const H = JSON.parse(B);
              this.buffer.push(H), this.currentData = H, this.emit("data", H), this.emit("message", H);
            } catch (H) {
              this.emit("error", H);
            }
          }
        }), k = (b = this.options.timeout) !== null && b !== void 0 ? b : d, O = () => t(this, void 0, void 0, function* () {
          const { value: N, done: B } = yield x.read();
          this.lastEventTimestamp = Date.now(), E.feed(S.decode(N)), Date.now() - this.lastEventTimestamp > k && this.emit("error", new a.ApiError({
            message: `Event stream timed out after ${(k / 1e3).toFixed(0)} seconds with no messages.`,
            status: 408,
            requestId: this._requestId || void 0
          })), B ? this.emit("done", this.currentData) : O().catch(this.handleError);
        });
  ...
  _e.FalStream = c;
  function u({ config: p, storage: f }) {
    return {
      stream(m, y) {
        return t(this, void 0, void 0, function* () {
          const g = y.input ? yield f.transformInput(y.input) : void 0;
          return new c(m, p, Object.assign(Object.assign({}, y), { input: g }));
        });
      }
    };
  }
...
function uu() {
  if (ko) return Pe;
  ko = 1;
  var t = Pe && Pe.__awaiter || function(f, m, y, g) {
    function h(_) {
    ...
    return new (y || (y = Promise))(function(_, b) {
      ...
      function S(x) {
        x.done ? _(x.value) : h(x.value).then(v, T);
      }
      S((g = g.apply(f, m || [])).next());
    });
  ...
  const r = Ai(), n = Sr(), i = dt(), o = Zr(), a = hs(), s = _s(), l = Ke(), d = 500, c = {
    maxRetries: 3,
    baseDelay: 1e3,
    maxDelay: 6e4,
    ...
    const y = {
      submit(g, h) {
        return t(this, void 0, void 0, function* () {
          const { webhookUrl: _, priority: b, hint: v, startTimeout: T, headers: S, storageSettings: x } = h, E = e(h, ["webhookUrl", "priority", "hint", "startTimeout", "headers", "storageSettings"]), k = h.input ? yield m.transformInput(h.input) : void 0, O = Object.fromEntries(Object.entries(S ?? {}).map(([N, B]) => [
            N.toLowerCase(),
            B
          ]));
          return (0, n.dispatchRequest)({
            method: h.method,
            targetUrl: (0, n.buildUrl)(g, Object.assign(Object.assign({}, E), { subdomain: "queue", query: _ ? { fal_webhook: _ } : void 0 })),
      ...
      status(g, h) {
        return t(this, arguments, void 0, function* (_, { requestId: b, logs: v = !1, abortSignal: T }) {
          const S = (0, l.parseEndpointId)(_), x = S.namespace ? `${S.namespace}/` : "";
          return (0, n.dispatchRequest)({
            method: "get",
            targetUrl: (0, n.buildUrl)(`${x}${S.owner}/${S.alias}`, {
              subdomain: "queue",
              query: { logs: v ? "1" : "0" },
      ...
      streamStatus(g, h) {
        return t(this, arguments, void 0, function* (_, { requestId: b, logs: v = !1, connectionMode: T }) {
          const S = (0, l.parseEndpointId)(_), x = S.namespace ? `${S.namespace}/` : "", E = {
            logs: v ? "1" : "0"
          }, k = (0, n.buildUrl)(`${x}${S.owner}/${S.alias}`, {
            subdomain: "queue",
      ...
      subscribeToStatus(g, h) {
        return t(this, void 0, void 0, function* () {
          ...
          if (h.mode === "streaming") {
            const S = yield y.streamStatus(g, {
              requestId: _,
              logs: h.logs,
              connectionMode: "connectionMode" in h ? h.connectionMode : void 0
            }), x = [];
            b && (v = setTimeout(() => {
              throw S.abort(), y.cancel(g, { requestId: _ }).catch(T), new Error(`Client timed out waiting for the request to complete after ${b}ms`);
            }, b)), S.on("data", (k) => {
              h.onQueueUpdate && ("logs" in k && Array.isArray(k.logs) && k.logs.length > 0 && x.push(...k.logs), h.onQueueUpdate("logs" in k ? Object.assign(Object.assign({}, k), { logs: x }) : k));
            });
            const E = yield S.done();
            return v && clearTimeout(v), E;
          }
          return new Promise((S, x) => {
            var E;
            let k;
            const O = "pollInterval" in h && typeof h.pollInterval == "number" && (E = h.pollInterval) !== null && E !== void 0 ? E : d, N = () => {
              v && clearTimeout(v), k && clearTimeout(k);
            };
            b && (v = setTimeout(() => {
              N(), y.cancel(g, { requestId: _ }).catch(T), x(new Error(`Client timed out waiting for the request to complete after ${b}ms`));
            }, b));
            const B = () => t(this, void 0, void 0, function* () {
              var H;
              try {
                const W = yield y.status(g, {
                  requestId: _,
                  logs: (H = h.logs) !== null && H !== void 0 ? H : !1,
                  abortSignal: h.abortSignal
                });
                if (h.onQueueUpdate && h.onQueueUpdate(W), W.status === "COMPLETED") {
                  N(), S(W);
                  return;
                }
                k = setTimeout(B, O);
      ...
      result(g, h) {
        return t(this, arguments, void 0, function* (_, { requestId: b, abortSignal: v }) {
          const T = (0, l.parseEndpointId)(_), S = T.namespace ? `${T.namespace}/` : "";
          return (0, n.dispatchRequest)({
            method: "get",
            targetUrl: (0, n.buildUrl)(`${S}${T.owner}/${T.alias}`, {
              subdomain: "queue",
              path: `/requests/${b}`
            }),
...
const pu = new TextEncoder(), mu = 50;
function gu(t, e, r) {
  pu.encodeInto(t, e.subarray(r));
}
function hu(t, e, r) {
  t.length > mu ? gu(t, e, r) : fu(t, e, r);
}
const yu = 4096;
function ws(t, e, r) {
  let n = e;
  ...
  for (; n < i; ) {
    ...
    if ((s & 128) === 0)
      ...
      const l = t[n++] & 63, d = t[n++] & 63, c = t[n++] & 63;
      let u = (s & 7) << 18 | l << 12 | d << 6 | c;
      u > 65535 && (u -= 65536, o.push(u >>> 10 & 1023 | 55296), u = 56320 | u & 1023), o.push(u);
    } else
      o.push(s);
    o.length >= yu && (a += String.fromCharCode(...o), o.length = 0);
  }
  return o.length > 0 && (a += String.fromCharCode(...o)), a;
}
const _u = new TextDecoder(), wu = 200;
function bu(t, e, r) {
  const n = t.subarray(e, e + r);
  return _u.decode(n);
}
function vu(t, e, r) {
  return r > wu ? bu(t, e, r) : ws(t, e, r);
}
class nr {
  constructor(e, r) {
    M(this, "type");
...
const Ts = -1, Su = 4294967296 - 1, Iu = 17179869184 - 1;
function Es({ sec: t, nsec: e }) {
  if (t >= 0 && e >= 0 && t <= Iu)
    if (e === 0 && t <= Su) {
    ...
    return n.setUint32(0, e), bs(n, 4, t), r;
  }
}
function Ss(t) {
...
function xs(t) {
  ...
  switch (t.byteLength) {
    ...
    case 12: {
      const r = vs(e, 4), n = e.getUint32(0);
      return { sec: r, nsec: n };
    }
    default:
      throw new de(`Unrecognized data size for timestamp (expected 4, 8, or 12): ${t.length}`);
  }
}
function As(t) {
  const e = xs(t);
...
const xu = {
  ...
  tryToEncode(e, r) {
    ...
    return e instanceof nr ? e : null;
  }
  decode(e, r, n) {
    const i = r < 0 ? this.builtInDecoders[-1 - r] : this.decoders[r];
    return i ? i(e, r, n) : new nr(r, e);
  }
};
M(Jr, "defaultCodec", new Jr());
let Er = Jr;
function Au(t) {
  return t instanceof ArrayBuffer || typeof SharedArrayBuffer < "u" && t instanceof SharedArrayBuffer;
}
function Zn(t) {
  return t instanceof Uint8Array ? t : ArrayBuffer.isView(t) ? new Uint8Array(t.buffer, t.byteOffset, t.byteLength) : Au(t) ? new Uint8Array(t) : Uint8Array.from(t);
}
const ku = 100, ju = 2048;
class en {
  constructor(e) {
    ...
    M(this, "bytes");
    M(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? Er.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.maxDepth = (e == null ? void 0 : e.maxDepth) ?? ku, this.initialBufferSize = (e == null ? void 0 : e.initialBufferSize) ?? ju, this.sortKeys = (e == null ? void 0 : e.sortKeys) ?? !1, this.forceFloat32 = (e == null ? void 0 : e.forceFloat32) ?? !1, this.ignoreUndefined = (e == null ? void 0 : e.ignoreUndefined) ?? !1, this.forceIntegerToFloat = (e == null ? void 0 : e.forceIntegerToFloat) ?? !1, this.pos = 0, this.view = new DataView(new ArrayBuffer(this.initialBufferSize)), this.bytes = new Uint8Array(this.view.buffer);
  }
  clone() {
    return new en({
  ...
  encodeString(e) {
    const n = cu(e);
    this.ensureBufferSizeToWrite(5 + n), this.writeStringHeader(n), hu(e, this.bytes, this.pos), this.pos += n;
  }
  encodeObject(e, r) {
  ...
  writeU64(e) {
    this.ensureBufferSizeToWrite(8), Tu(this.view, this.pos, e), this.pos += 8;
  }
  writeI64(e) {
    this.ensureBufferSizeToWrite(8), bs(this.view, this.pos, e), this.pos += 8;
  }
  writeBigUint64(e) {
...
class Nu {
  ...
  decode(e, r, n) {
    ...
    if (i != null)
      return this.hit++, i;
    this.miss++;
    const o = ws(e, r, n), a = Uint8Array.prototype.slice.call(e, r, r + n);
    return this.store(a, o), o;
  }
}
const ei = "array", sr = "map_key", ks = "map_value", Cu = (t) => {
  if (typeof t == "string" || typeof t == "number")
    return t;
  throw new de("The type of key must be string or number but " + typeof t);
};
class qu {
  constructor() {
    M(this, "stack", []);
  ...
  pushArrayState(e) {
    ...
    r.type = ei, r.position = 0, r.size = e, r.array = new Array(e);
  }
  pushMapState(e) {
    const r = this.getUninitializedStateFromPool();
    r.type = sr, r.readCount = 0, r.size = e, r.map = {};
  }
...
const Yt = -1, Ri = new DataView(new ArrayBuffer(0)), Lu = new Uint8Array(Ri.buffer);
try {
  Ri.getInt8(0);
} catch (t) {
...
const jo = new RangeError("Insufficient data"), Uu = new Nu();
class Ye {
  constructor(e) {
    M(this, "extensionCodec");
    M(this, "context");
    ...
    M(this, "stack", new qu());
    M(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? Er.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.rawStrings = (e == null ? void 0 : e.rawStrings) ?? !1, this.maxStrLength = (e == null ? void 0 : e.maxStrLength) ?? Kt, this.maxBinLength = (e == null ? void 0 : e.maxBinLength) ?? Kt, this.maxArrayLength = (e == null ? void 0 : e.maxArrayLength) ?? Kt, this.maxMapLength = (e == null ? void 0 : e.maxMapLength) ?? Kt, this.maxExtLength = (e == null ? void 0 : e.maxExtLength) ?? Kt, this.keyDecoder = (e == null ? void 0 : e.keyDecoder) !== void 0 ? e.keyDecoder : Uu, this.mapKeyConverter = (e == null ? void 0 : e.mapKeyConverter) ?? Cu;
  }
  clone() {
    return new Ye({
      extensionCodec: this.extensionCodec,
  ...
  reinitializeState() {
    this.totalPos = 0, this.headByte = Yt, this.stack.reset();
  }
  setBuffer(e) {
    const r = Zn(e);
  ...
  appendBuffer(e) {
    if (this.headByte === Yt && !this.hasRemaining(1))
      this.setBuffer(e);
    else {
      const r = this.bytes.subarray(this.pos), n = Zn(e), i = new Uint8Array(r.length + n.length);
  ...
  *decodeMulti(e) {
  ...
  async decodeAsync(e) {
    ...
    try {
      ...
      throw new RangeError(`Insufficient data in parsing ${An(i)} at ${a} (${o} in the current buffer)`);
    } finally {
      this.entered = !1;
    }
  }
  ...
  doDecodeSync() {
    e: for (; ; ) {
      ...
      if (e >= 224)
        ...
        throw new de(`Unrecognized type byte: ${An(e)}`);
      this.complete();
      const n = this.stack;
      for (; n.length > 0; ) {
        const i = n.top();
        if (i.type === ei)
          ...
          if (r === "__proto__")
            throw new de("The key __proto__ is not allowed");
          i.key = this.mapKeyConverter(r), i.type = ks;
          continue e;
        } else if (i.map[i.key] = r, i.readCount++, i.readCount === i.size)
          r = i.map, n.release(i);
        else {
          i.key = null, i.type = sr;
          continue e;
        }
      }
      return r;
  ...
  readHeadByte() {
    return this.headByte === Yt && (this.headByte = this.readU8()), this.headByte;
  }
  complete() {
    this.headByte = Yt;
  }
  readArraySize() {
    ...
    switch (e) {
      ...
      default: {
        if (e < 160)
          return e - 144;
        throw new de(`Unrecognized array type byte: ${An(e)}`);
      }
    }
  }
  pushMapState(e) {
    if (e > this.maxMapLength)
      throw new de(`Max length exceeded: map length (${e}) > maxMapLengthLength (${this.maxMapLength})`);
    this.stack.pushMapState(e);
  }
  pushArrayState(e) {
    if (e > this.maxArrayLength)
      throw new de(`Max length exceeded: array length (${e}) > maxArrayLength (${this.maxArrayLength})`);
  ...
  decodeUtf8String(e, r) {
    ...
    if (e > this.maxStrLength)
      throw new de(`Max length exceeded: UTF-8 byte length (${e}) > maxStrLength (${this.maxStrLength})`);
    if (this.bytes.byteLength < this.pos + r + e)
      throw jo;
    const n = this.pos + r;
    let i;
    return this.stateIsMapKey() && ((o = this.keyDecoder) != null && o.canBeCached(e)) ? i = this.keyDecoder.decode(this.bytes, n, e) : i = vu(this.bytes, n, e), this.pos += r + e, i;
  }
  stateIsMapKey() {
    return this.stack.length > 0 ? this.stack.top().type === sr : !1;
  }
  /**
   * @throws {@link RangeError}
   */
  decodeBinary(e, r) {
    if (e > this.maxBinLength)
      throw new de(`Max length exceeded: bin length (${e}) > maxBinLength (${this.maxBinLength})`);
  ...
  decodeExtension(e, r) {
    if (e > this.maxExtLength)
      throw new de(`Max length exceeded: ext length (${e}) > maxExtLength (${this.maxExtLength})`);
  ...
  readU64() {
    const e = Eu(this.view, this.pos);
    return this.pos += 8, e;
  }
  readI64() {
    const e = vs(this.view, this.pos);
    return this.pos += 8, e;
  }
  readU64AsBigInt() {
...
function Mu(t, e) {
  return new Ye(e).decode(t);
}
function Du(t, e) {
  return new Ye(e).decodeMulti(t);
}
function $u(t) {
...
async function Bu(t, e) {
  const r = Pi(t);
  return new Ye(e).decodeAsync(r);
}
function Vu(t, e) {
  const r = Pi(t);
  return new Ye(e).decodeArrayStream(r);
}
function Gu(t, e) {
  const r = Pi(t);
  return new Ye(e).decodeStream(r);
}
const zu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  ExtensionCodec: Er,
  decode: Mu,
  decodeArrayStream: Vu,
  decodeAsync: Bu,
  decodeMulti: Du,
  decodeMultiStream: Gu,
  decodeTimestampExtension: As,
  decodeTimestampToTimeSpec: xs,
  encode: Ru,
  encodeDateToTimeSpec: Ss,
  encodeTimeSpecToTimestamp: Es,
  encodeTimestampExtension: Is
}, Symbol.toStringTag, { value: "Module" })), Hu = /* @__PURE__ */ ou(zu);
var me = {}, Ro;
function Wu() {
  if (Ro) return me;
  Ro = 1, Object.defineProperty(me, "__esModule", { value: !0 });
  function t(P) {
    return { enumerable: !0, value: P };
  }
  function e(P) {
    return { enumerable: !0, writable: !0, value: P };
  }
  let r = {}, n = () => !0, i = () => ({}), o = (P) => P, a = (P, A, q, C) => P.apply(q, C) && A.apply(q, C), s = (P, A, q, [C, V]) => A.call(q, P.call(q, C, V), V), l = (P, A) => Object.freeze(Object.create(P, A));
  function d(P, A, q) {
    return P.reduce((C, V) => function(...Y) {
      return q(C, V, this, Y);
    }, A);
  }
  function c(P) {
    return l(this, { fn: t(P) });
  }
  let u = {}, p = c.bind(u), f = (P) => p((A, q) => !!~P(A, q) && A), m = {}, y = c.bind(m);
  function g(P, A) {
    return A.filter((q) => P.isPrototypeOf(q));
  }
  function h(P, A, ...q) {
    let C = d(g(m, q).map((Y) => Y.fn), n, a), V = d(g(u, q).map((Y) => Y.fn), o, s);
    return l(this, {
      from: t(P),
      to: t(A),
      guards: t(C),
  ...
  let _ = {}, b = {}, v = h.bind(_), T = h.bind(b, null);
  function S(P, A, q) {
    return J(A, P, q, this.immediates) || P;
  }
  function x(P) {
    let A = /* @__PURE__ */ new Map();
    for (let q of P)
      A.has(q.from) || A.set(q.from, []), A.get(q.from).push(q);
    return A;
  }
  let E = { enter: o };
  function k(...P) {
    let A = g(_, P), q = g(b, P), C = {
      final: t(P.length === 0),
      transitions: t(x(A))
    };
    return q.length && (C.immediates = t(q), C.enter = t(S)), l(E, C);
  }
  let O = {
    enter(P, A, q) {
      let C = this.fn.call(A, A.context, q);
      return H.isPrototypeOf(C) ? l(N, {
        machine: t(C),
        transitions: t(this.transitions)
      }).enter(P, A, q) : (C.then((V) => A.send({ type: "done", data: V })).catch((V) => A.send({ type: "error", error: V })), P);
    }
  }, N = {
    enter(P, A, q) {
      if (A.child = U(this.machine, (C) => {
        A.onChange(C), A.child == C && C.machine.state.value.final && (delete A.child, A.send({ type: "done", data: C.context }));
      }, A.context, q), A.child.machine.state.value.final) {
        let C = A.child.context;
        return delete A.child, J(A, P, { type: "done", data: C }, this.transitions.get("done"));
      }
      return P;
    }
  };
  ...
  function W(P, A, q = i) {
    return typeof P != "string" && (q = A || i, A = P, P = Object.keys(A)[0]), r._create && r._create(P, A), l(H, {
      context: t(q),
      current: t(P),
      states: t(A)
    });
  ...
  function J(P, A, q, C) {
    let { context: V } = P;
    for (let { to: Y, guards: je, reducers: ce } of C)
      if (je(V, q)) {
        P.context = ce.call(P, V, q);
        let Ue = A.original || A, Dt = l(Ue, {
          current: t(Y),
          original: { value: Ue }
        });
        return r._onEnter && r._onEnter(A, Y, P.context, V, q), Dt.state.value.enter(Dt, P, q);
      }
  }
  function j(P, A) {
    let q = A.type || A, { machine: C } = P, { value: V, name: Y } = C.state;
    return V.transitions.has(q) ? J(P, C, A, V.transitions.get(q)) || C : (r._send && r._send(q, Y), C);
  }
  let L = {
    send(P) {
      this.machine = j(this, P), this.onChange(this);
    }
  };
  function U(P, A, q, C) {
    let V = Object.create(L, {
      machine: e(P),
      context: e(P.context(q, C)),
      onChange: t(A)
    });
    return V.send = V.send.bind(V), V.machine = V.machine.state.value.enter(V.machine, V, C), V;
  }
  return me.action = f, me.createMachine = W, me.d = r, me.guard = y, me.immediate = T, me.interpret = U, me.invoke = B, me.reduce = p, me.state = k, me.transition = v, me;
}
var Po;
function Xu() {
  if (Po) return pt;
  Po = 1;
  var t = pt && pt.__awaiter || function(j, L, U, P) {
    function A(q) {
      return q instanceof U ? q : new U(function(C) {
        C(q);
      });
    }
    return new (U || (U = Promise))(function(q, C) {
      function V(ce) {
        try {
          je(P.next(ce));
        } catch (Ue) {
      ...
      function je(ce) {
        ce.done ? q(ce.value) : A(ce.value).then(V, Y);
      }
      je((P = P.apply(j, L || [])).next());
    });
  };
  Object.defineProperty(pt, "__esModule", { value: !0 }), pt.createRealtimeClient = J;
  const e = Hu, r = Wu(), n = ys(), i = dt(), o = ki(), a = Ke(), s = () => ({
    enqueuedMessage: void 0
  });
  function l(j) {
    return j.token !== void 0;
  }
  function d(j) {
    return !l(j);
  }
  function c(j, L) {
    return Object.assign(Object.assign({}, j), { enqueuedMessage: L.message });
  }
  function u(j) {
    return j.websocket && j.websocket.readyState === WebSocket.OPEN && j.websocket.close(), Object.assign(Object.assign({}, j), { websocket: void 0 });
  }
  function p(j, L) {
    return j.websocket && j.websocket.readyState === WebSocket.OPEN ? (L.message instanceof Uint8Array || typeof L.message == "string" ? j.websocket.send(L.message) : j.websocket.send((0, e.encode)(L.message)), Object.assign(Object.assign({}, j), { enqueuedMessage: void 0 })) : Object.assign(Object.assign({}, j), { enqueuedMessage: L.message });
  }
  function f(j) {
    return Object.assign(Object.assign({}, j), { token: void 0 });
  }
  function m(j, L) {
    return Object.assign(Object.assign({}, j), { token: L.token });
  }
  function y(j, L) {
    return Object.assign(Object.assign({}, j), { websocket: L.websocket });
  }
  const g = (0, r.createMachine)("idle", {
    idle: (0, r.state)((0, r.transition)("send", "connecting", (0, r.reduce)(c)), (0, r.transition)("expireToken", "idle", (0, r.reduce)(f)), (0, r.transition)("close", "idle", (0, r.reduce)(u))),
    connecting: (0, r.state)((0, r.transition)("connecting", "connecting"), (0, r.transition)("connected", "active", (0, r.reduce)(y)), (0, r.transition)("connectionClosed", "idle", (0, r.reduce)(u)), (0, r.transition)("send", "connecting", (0, r.reduce)(c)), (0, r.transition)("close", "idle", (0, r.reduce)(u)), (0, r.immediate)("authRequired", (0, r.guard)(d))),
    authRequired: (0, r.state)((0, r.transition)("initiateAuth", "authInProgress"), (0, r.transition)("send", "authRequired", (0, r.reduce)(c)), (0, r.transition)("close", "idle", (0, r.reduce)(u))),
    authInProgress: (0, r.state)((0, r.transition)("authenticated", "connecting", (0, r.reduce)(m)), (0, r.transition)("unauthorized", "idle", (0, r.reduce)(f), (0, r.reduce)(u)), (0, r.transition)("send", "authInProgress", (0, r.reduce)(c)), (0, r.transition)("close", "idle", (0, r.reduce)(u))),
    active: (0, r.state)((0, r.transition)("send", "active", (0, r.reduce)(p)), (0, r.transition)("authenticated", "active", (0, r.reduce)(m)), (0, r.transition)("unauthorized", "idle", (0, r.reduce)(f)), (0, r.transition)("connectionClosed", "idle", (0, r.reduce)(u)), (0, r.transition)("close", "idle", (0, r.reduce)(u))),
    failed: (0, r.state)((0, r.transition)("send", "failed"), (0, r.transition)("close", "idle", (0, r.reduce)(u)))
  }, s);
  function h(j, { token: L, maxBuffering: U, path: P }) {
    var A;
    if (U !== void 0 && (U < 1 || U > 60))
      throw new Error("The `maxBuffering` must be between 1 and 60 (inclusive)");
    const q = new URLSearchParams({
      fal_jwt_token: L
    });
    U !== void 0 && q.set("max_buffering", U.toFixed(0));
    const C = (0, a.ensureEndpointIdFormat)(j), V = (A = (0, a.resolveEndpointPath)(j, P, "/realtime")) !== null && A !== void 0 ? A : "";
    return `wss://fal.run/${C}${V}?${q.toString()}`;
  }
  const _ = 128;
  function b(j) {
    return j.status === "error" && j.error === "Unauthorized";
  }
  const v = {
    NORMAL_CLOSURE: 1e3
  ...
  function x(j, L, U) {
    if (!T.has(j)) {
      const P = (0, r.interpret)(g, U);
      T.set(j, Object.assign(Object.assign({}, P), { throttledSend: L > 0 ? (0, a.throttle)(P.send, L, !0) : P.send }));
    }
    return T.get(j);
  }
  const E = () => {
  ...
  function O(j) {
    return j.status !== "error" && j.type !== "x-fal-message" && !N(j);
  }
  function N(j) {
    return j.type === "x-fal-error";
  }
  function B(j) {
    return t(this, void 0, void 0, function* () {
      if (typeof j == "string")
        return JSON.parse(j);
      const L = (U) => t(this, void 0, void 0, function* () {
        return U instanceof Uint8Array ? U : U instanceof Blob ? new Uint8Array(yield U.arrayBuffer()) : new Uint8Array(U);
      });
      return j instanceof ArrayBuffer || j instanceof Uint8Array ? (0, e.decode)(yield L(j)) : j instanceof Blob ? (0, e.decode)(yield L(j)) : j;
    });
  }
  function H(j) {
    return j instanceof Uint8Array ? j : (0, e.encode)(j);
  }
  function W({ data: j, decodeMessage: L, onResult: U, onError: P, send: A }) {
    const q = (C) => {
      if (b(C)) {
        A({
          type: "unauthorized",
    ...
    Promise.resolve(L ? L(j) : j).then(q).catch((C) => {
      var V;
      P(new i.ApiError({
        message: (V = C == null ? void 0 : C.message) !== null && V !== void 0 ? V : "Failed to decode realtime message",
        status: 400
      }));
    });
  }
    ...
    return {
      connect(L, U) {
        const {
          ...
          clientOnly: P = (0, a.isReact)() && !(0, o.isBrowser)(),
          connectionKey: A = crypto.randomUUID(),
          maxBuffering: q,
          path: C,
          ...
          decodeMessage: je,
          tokenProvider: ce,
          tokenExpirationSeconds: Ue
        } = U;
        if (P && !(0, o.isBrowser)())
          return k;
        const Dt = Y ?? ((Qe) => H(Qe)), cn = je ?? ((Qe) => B(Qe));
        let xr, ao, $t, Ft = 0;
        S.set(A, {
          decodeMessage: cn,
          onError: U.onError,
        ...
        const fn = () => S.get(A), Bt = x(A, V, ({ context: Qe, machine: Ze, send: Me }) => {
          var pn;
          const { enqueuedMessage: mn, token: gn, websocket: hn } = Qe;
          if (ao = mn, Ze.current === "active" && mn && (hn == null ? void 0 : hn.readyState) === WebSocket.OPEN && Me({ type: "send", message: mn }), Ze.current === "authRequired" && gn === void 0 && xr !== Ze.current) {
            Me({ type: "initiateAuth" }), Ft++;
            const Ae = Ft, fe = (0, a.ensureEndpointIdFormat)(L), Te = (pn = (0, a.resolveEndpointPath)(L, C, "/realtime")) !== null && pn !== void 0 ? pn : "", et = ce ? () => ce(`${fe}${Te}`) : () => (console.warn("[fal.realtime] Using the default token provider is deprecated. Please provide a `tokenProvider` function to `fal.realtime.connect()`. See https://docs.fal.ai/model-apis/client#client-side-usage-with-token-provider for more information."), (0, n.getTemporaryAuthToken)(L, j)), Vt = ce ? Ue : n.TOKEN_EXPIRATION_SECONDS, yn = Vt !== void 0 ? () => {
              clearTimeout($t);
              const Gt = Math.round(Vt * 0.9 * 1e3);
              $t = setTimeout(() => {
                Ae === Ft && et().then((_n) => {
                  Ae === Ft && (queueMicrotask(() => {
                    Me({ type: "authenticated", token: _n });
                  }), yn());
                }).catch(() => {
                  if (Ae !== Ft)
                    return;
                  const _n = Math.round(Vt * 0.05 * 1e3);
                  $t = setTimeout(() => {
                    yn();
                  }, _n);
                });
            ...
            et().then((Gt) => {
              queueMicrotask(() => {
                Me({ type: "authenticated", token: Gt });
              }), yn();
            }).catch((Gt) => {
              queueMicrotask(() => {
                Me({ type: "unauthorized", error: Gt });
              });
            });
          }
          if (Ze.current === "connecting" && xr !== Ze.current && gn !== void 0) {
            const Ae = new WebSocket(h(L, { token: gn, maxBuffering: q, path: C }));
            Ae.onopen = () => {
              var fe, Te;
              Me({ type: "connected", websocket: Ae });
              const et = (Te = (fe = Bt.context) === null || fe === void 0 ? void 0 : fe.enqueuedMessage) !== null && Te !== void 0 ? Te : ao;
              et && (Ae.send(Dt(et)), Bt.context = Object.assign(Object.assign({}, Bt.context), { enqueuedMessage: void 0 }));
            }, Ae.onclose = (fe) => {
              if (fe.code !== v.NORMAL_CLOSURE) {
                const { onError: Te = E } = fn();
                Te(new i.ApiError({
                  message: `Error closing the connection: ${fe.reason}`,
                  status: fe.code
                }));
              }
              Me({ type: "connectionClosed", code: fe.code });
            }, Ae.onerror = (fe) => {
              const { onError: Te = E } = fn();
              Te(new i.ApiError({ message: "Unknown error", status: 500 }));
            }, Ae.onmessage = (fe) => {
              const { decodeMessage: Te = cn, onResult: et, onError: Vt = E } = fn();
              W({
                data: fe.data,
                decodeMessage: Te,
                onResult: et,
          ...
          xr === "active" && Ze.current !== "active" && (clearTimeout($t), $t = void 0), xr = Ze.current;
        });
        return {
          send: (Qe) => {
            Bt.throttledSend({
...
function No() {
  if (Oo) return ft;
  Oo = 1;
  var t = ft && ft.__awaiter || function(c, u, p, f) {
    function m(y) {
    ...
    return new (p || (p = Promise))(function(y, g) {
      ...
      function b(v) {
        v.done ? y(v.value) : m(v.value).then(h, _);
      }
      b((f = f.apply(c, u || [])).next());
    });
  };
  Object.defineProperty(ft, "__esModule", { value: !0 }), ft.createFalClient = d;
  const e = ji(), r = Ai(), n = uu(), i = Xu(), o = Sr(), a = dt(), s = hs(), l = _s();
  function d(c = {}) {
    const u = (0, e.createConfig)(c), p = (0, s.createStorageClient)({ config: u }), f = (0, n.createQueueClient)({ config: u, storage: p }), m = (0, l.createStreamingClient)({ config: u, storage: p }), y = (0, i.createRealtimeClient)({ config: u });
    return {
      queue: f,
      realtime: y,
      storage: p,
      ...
      run(g) {
        return t(this, arguments, void 0, function* (h, _ = {}) {
          ...
          return (0, o.dispatchRequest)({
            ...
            config: Object.assign(Object.assign({}, u), { responseHandler: a.resultResponseHandler }),
            options: {
...
function Ju() {
  if (Co) return Qt;
  Co = 1, Object.defineProperty(Qt, "__esModule", { value: !0 }), Qt.isQueueStatus = t, Qt.isCompletedQueueStatus = e;
  function t(r) {
...
function Ku() {
  return qo || (qo = 1, (function(t) {
    var e = tt && tt.__createBinding || (Object.create ? (function(d, c, u, p) {
      p === void 0 && (p = u);
      ...
      p === void 0 && (p = u), d[p] = c[u];
    })), r = tt && tt.__exportStar || function(d, c) {
      for (var u in d) u !== "default" && !Object.prototype.hasOwnProperty.call(c, u) && e(c, d, u);
    };
    ...
    Object.defineProperty(t, "parseEndpointId", { enumerable: !0, get: function() {
      ...
      let c = (0, n.createFalClient)();
      return {
        config(u) {
          c = (0, n.createFalClient)(u);
        },
        get queue() {
          return c.queue;
        },
...
var K = Ku();
function it(t) {
  return ["hf_mult_motion_control", "hf_mult_replace_object"].includes(String(t));
}
function js(t) {
  return it(t) || ["topaz_image", "topaz_video", "sync_so"].includes(String(t));
}
function Yu(t, e, r) {
  if (!js(t)) return;
  const n = r.filter((a) => /image/.test(a.role ?? "")), i = r.filter((a) => /video/.test(a.role ?? "")), o = r.filter((a) => /audio/.test(a.role ?? ""));
  if (it(t)) {
    if (r.some((a) => !["image", "image_references", "video", "video_references"].includes(a.role ?? "")))
      throw new Error("Genjutsu accepts reference images and one source video. Audio and frame references are not supported.");
    if (i.length !== 1) throw new Error("Genjutsu requires exactly one source video.");
    if (!n.length) throw new Error("Genjutsu requires at least one reference image.");
    if (e.resolution !== void 0 && !["480p", "720p", "1080p"].includes(String(e.resolution)))
      throw new Error("Genjutsu output must be 480p, 720p, or 1080p.");
  } else if (t === "topaz_image") {
...
const Qu = /* @__PURE__ */ JSON.parse('[{"display_name":"3D Rigging","job_set_type":"3d_rigging","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height_meters","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true}]},{"display_name":"Brain Activity","job_set_type":"brain_activity","type":"text","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Bytedance Image Upscale","job_set_type":"bytedance_image_upscale","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"4k","required":false,"enum":["2k","4k"]}]},{"display_name":"Bytedance Video Upscale","job_set_type":"bytedance_video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"fps","type":"integer","default":24,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"model_version","type":"string","default":"standard","required":false,"enum":["standard","pro"]},{"name":"preset","type":"string","default":"common","required":false,"enum":["common","aigc","short_series","ugc","old_film"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1080p","2k","4k"]}]},{"display_name":"Cinematic Studio 2.5","job_set_type":"cinematic_studio_2_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"auto","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio 3.0","job_set_type":"cinematic_studio_3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Cinematic Studio Image","job_set_type":"cinematic_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"string","default":null,"required":true},{"name":"camera_lens_id","type":"string","default":null,"required":true},{"name":"camera_model_id","type":"string","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio Soul Cast","job_set_type":"cinematic_studio_soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinematic Studio Soul Location","job_set_type":"cinematic_studio_soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Cinematic Studio Video","job_set_type":"cinematic_studio_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"slow_motion","type":"boolean","default":false,"required":false},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Cinematic Studio Video 3.5","job_set_type":"cinematic_studio_video_3_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"camera_style","type":"object","default":null,"required":false},{"name":"color_grading","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"light_scheme","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"style_id","type":"object","default":null,"required":false},{"name":"style_prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinema Studio 4.0","job_set_type":"cinematic_studio_video_4_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"color_palette","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"era_id","type":"object","default":null,"required":false},{"name":"extension_mode","type":"object","default":null,"required":false},{"name":"film_era","type":"null","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"null","default":null,"required":false},{"name":"genre_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"light","type":"object","default":null,"required":false},{"name":"light_custom","type":"object","default":null,"required":false},{"name":"light_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"model","type":"string","default":"default","required":false,"enum":["default","video_edit","video_extension"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"pacing_id","type":"object","default":null,"required":false},{"name":"preset_id","type":"null","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"speedramp","type":"object","default":"auto","required":false},{"name":"use_blur","type":"boolean","default":false,"required":false},{"name":"use_eye_mask","type":"boolean","default":false,"required":false},{"name":"use_transparency","type":"boolean","default":false,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Cinematic Studio Video V2","job_set_type":"cinematic_studio_video_v2","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"cfg_scale","type":"number","default":0.5,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","western","suspense","intimate","spectacle"]},{"name":"kling_element_ids","type":"array","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Clipify","job_set_type":"clipify","type":"video","params":[{"name":"clip_aspect","type":"string","default":"9:16","required":false,"enum":["9:16","1:1","16:9"]},{"name":"clips_num","type":"integer","default":10,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"max_height","type":"integer","default":1080,"required":false},{"name":"segment_seconds","type":"integer","default":10,"required":false},{"name":"subtitle_case","type":"string","default":"as-is","required":false,"enum":["lower","upper","as-is"]},{"name":"subtitle_font","type":"string","default":"notosans","required":false},{"name":"subtitle_highlight_hex","type":"string","default":"#FFE84D","required":false},{"name":"subtitle_position","type":"string","default":"bottom","required":false,"enum":["bottom","center","top"]},{"name":"track_face_crop","type":"boolean","default":true,"required":false},{"name":"urls","type":"array","default":null,"required":true}]},{"display_name":"Draw To Video","job_set_type":"draw_to_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"ref_image","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"sketch","type":"object","default":null,"required":true},{"name":"video","type":"object","default":null,"required":true}]},{"display_name":"dubbing","job_set_type":"dubbing","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"target_language","type":"string","default":null,"required":true,"enum":["eng","cmn","fra","hin","ita","jpn","kor","por","rus","tur","spa","deu","ara","pol","ind","fil","swe","fin"]}]},{"display_name":"Explainer Video","job_set_type":"explainer_video","type":"video","params":[{"name":"height","type":"integer","default":null,"required":true},{"name":"items","type":"array","default":null,"required":true},{"name":"subtitles","type":"object","default":null,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"FLUX.2","job_set_type":"flux_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"pro","required":false,"enum":["pro","flex","max"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"FLUX.2 Pro Outpaint","job_set_type":"flux_2_pro_outpaint","type":"image","params":[{"name":"expand_bottom","type":"integer","default":0,"required":false},{"name":"expand_left","type":"integer","default":0,"required":false},{"name":"expand_right","type":"integer","default":0,"required":false},{"name":"expand_top","type":"integer","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"FLUX 3 Video","job_set_type":"flux_3_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","2:1","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Flux Kontext","job_set_type":"flux_kontext","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Gemini Omni Flash","job_set_type":"gemini_omni","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"integer","default":8,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false}]},{"display_name":"GPT Image 2","job_set_type":"gpt_image_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"high","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"GPT Image 2.5","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","16:9","9:16","21:9","27:16","16:27","9:8","8:9"]},{"name":"background","type":"string","default":null,"required":false,"enum":["auto","opaque","transparent"]},{"name":"image_references","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"low","required":false,"enum":["low","medium","high","xhigh","max"]},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]},{"name":"variant","type":"string","default":"flare","required":false,"enum":["flare","sunburst"]}],"type":"image","job_set_type":"gpt_image_2_5"},{"display_name":"Grok Image","job_set_type":"grok_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","1:2","2:1","3:2","2:3","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","quality"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Grok Video","job_set_type":"grok_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Grok Video 1.5","job_set_type":"grok_video_v15","type":"video","params":[{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Happy Horse Video","job_set_type":"happy_horse_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Genjutsu","job_set_type":"hf_mult_motion_control","type":"video","params":[{"name":"image_references","type":"array","default":null,"required":false},{"name":"prompt","type":"string","nullable":true,"default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"video_references","type":"array","default":null,"required":false}],"rules":[{"cel":"size(params.image_references) >= 1","message":"at least one image_references entry is required"},{"cel":"size(params.video_references) == 1","message":"exactly one video_references entry is required"}]},{"display_name":"Genjutsu","job_set_type":"hf_mult_replace_object","type":"video","params":[{"name":"image_references","type":"array","default":null,"required":false},{"name":"prompt","type":"string","nullable":true,"default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"video_references","type":"array","default":null,"required":false}],"rules":[{"cel":"size(params.image_references) >= 1","message":"at least one image_references entry is required"},{"cel":"size(params.video_references) == 1","message":"exactly one video_references entry is required"}]},{"display_name":"Hunyuan 3D v3.1 Text to 3D","job_set_type":"hunyuan3d_v3_1_text_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Hunyuan3D v3 Image to 3D","job_set_type":"hunyuan3d_v3_image_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"integer","default":500000,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"string","default":"Normal","required":false,"enum":["Normal","LowPoly","Geometry"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"polygon_type","type":"string","default":"triangle","required":false,"enum":["triangle","quadrilateral"]}]},{"display_name":"Image Auto","job_set_type":"image_auto","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Image Background Remover","job_set_type":"image_background_remover","type":"image","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Image Decompose","job_set_type":"image_decompose","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"granular","required":false,"enum":["granular","standard"]}]},{"display_name":"Image to 3D","job_set_type":"image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Inworld Text to Speech","job_set_type":"inworld_text_to_speech","type":"audio","params":[{"name":"prompt","type":"string","default":null,"required":true},{"name":"voice","type":"string","default":null,"required":true}]},{"display_name":"Kimodo","job_set_type":"kimodo","type":"3d","params":[{"name":"diffusion_steps","type":"integer","default":10,"required":false},{"name":"duration","type":"object","default":null,"required":false},{"name":"durations","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_version","type":"string","default":"ardy-core","required":false,"enum":["ardy-core","ardy-core-h8"]},{"name":"prompt","type":"object","default":null,"required":false},{"name":"prompts","type":"object","default":null,"required":false},{"name":"seed","type":"integer","default":42,"required":false}]},{"display_name":"Kling O1 Image","job_set_type":"kling_omni_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","auto","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Kling 2.6 Video","job_set_type":"kling2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Kling v3.0","job_set_type":"kling3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std","4k"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]}]},{"display_name":"Kling 3.0 Motion Control","job_set_type":"kling3_0_motion_control","type":"video","params":[{"name":"background_source","type":"string","default":"input_image","required":false,"enum":["input_image","input_video"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","pro"]}]},{"display_name":"Kling 3.0 Turbo","job_set_type":"kling3_0_turbo","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"LLM Generation","job_set_type":"llm_text","type":"video","params":[{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":null,"required":true},{"name":"reasoning_effort","type":"object","default":null,"required":false},{"name":"system_prompt","type":"string","default":"","required":false},{"name":"user_prompt","type":"string","default":"","required":false}]},{"display_name":"Marketing Studio Image","job_set_type":"marketing_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Marketing Studio Video","job_set_type":"marketing_studio_video","type":"video","params":[{"name":"ad_reference_id","type":"object","default":null,"required":false},{"name":"aspect_ratio","type":"string","default":"9:16","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"avatar_ids","type":"array","default":null,"required":false},{"name":"avatars","type":"array","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"hook_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"ugc","required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"setting_id","type":"object","default":null,"required":false},{"name":"specific_mode","type":"string","default":"default","required":false,"enum":["default","web_product","from_storyboard"]},{"name":"storyboard_id","type":"object","default":null,"required":false},{"name":"web_product_ids","type":"array","default":null,"required":false},{"name":"web_product_type","type":"object","default":null,"required":false}]},{"display_name":"Meshy 5 Remesh","job_set_type":"meshy_v5_remesh","type":"3d","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true},{"name":"origin_at","type":"object","default":null,"required":false},{"name":"resize_height","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Meshy 6 Text to 3D","job_set_type":"meshy_v6_text_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"enable_prompt_expansion","type":"boolean","default":false,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"full","required":false},{"name":"model_type","type":"string","default":"standard","required":false},{"name":"pose_mode","type":"string","default":"","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"boolean","default":true,"required":false},{"name":"symmetry_mode","type":"string","default":"auto","required":false},{"name":"target_polycount","type":"integer","default":30000,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"string","default":"triangle","required":false}]},{"display_name":"MiniMax H3","job_set_type":"minimax_h3","type":"video","params":[{"name":"aigc_watermark","type":"boolean","default":false,"required":false},{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":4,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"resolution","type":"string","default":"2K","required":false,"enum":["768P","2K"]},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Minimax Hailuo","job_set_type":"minimax_hailuo","type":"video","params":[{"name":"duration","type":"string","default":6,"required":false,"enum":["6","10"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"minimax-2.3","required":false,"enum":["minimax","minimax-fast","minimax-2.3","minimax-2.3-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"768","required":false,"enum":["512","768","1080"]}]},{"display_name":"Mirelo Text to Audio","job_set_type":"mirelo_text_to_audio","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"MS Image","job_set_type":"ms_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"avatars","type":"array","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"brand_kit_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"low","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Multi-Image to 3D","job_set_type":"multi_image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Nano Banana","job_set_type":"nano_banana","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_ai_stylist","type":"image","params":[{"name":"background_preset_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"outfit_preset_ids","type":"array","default":null,"required":false},{"name":"pose_preset_id","type":"object","default":null,"required":false},{"name":"user_outfit_ids","type":"array","default":null,"required":false}]},{"display_name":"Nano Banana 2 Lite","job_set_type":"nano_banana_2_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"thinking","type":"string","default":"HIGH","required":false,"enum":["MINIMAL","HIGH"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_relight","type":"image","params":[{"name":"brightness","type":"integer","default":null,"required":true},{"name":"color","type":"string","default":null,"required":true},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"light_quality","type":"string","default":null,"required":true,"enum":["hard","sharp","soft"]},{"name":"light_source","type":"string","default":null,"required":true,"enum":["mdl","mdr","mul","mur","bml","fml","fmr","bmm","mml","mmr","fmm","bmr","mdm","mum","bdr","fdl","bur","ful","bdl","fdr","bul","fur","bdm","fdm","bum","fum"]},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_shots","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_skin_enhancer","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"preset_id","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana 2","job_set_type":"nano_banana_flash","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"OpenAI Hazel","job_set_type":"openai_hazel","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","auto"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"medium","required":false,"enum":["low","medium","high"]}]},{"display_name":"Outpaint","job_set_type":"outpaint","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"21:9","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Qwen Audio 3.0 TTS Flash","job_set_type":"qwen_audio_tts","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"instruction","type":"object","default":null,"required":false},{"name":"language","type":"object","default":null,"required":false},{"name":"pitch_rate","type":"number","default":1,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","22050","24000","44100","48000"]},{"name":"seed","type":"integer","default":0,"required":false},{"name":"speech_rate","type":"number","default":1,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"integer","default":50,"required":false}]},{"display_name":"Angles","job_set_type":"qwen_camera_control","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"move_forward_level","type":"integer","default":0,"required":false},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"rotate_degree","type":"integer","default":0,"required":false},{"name":"vertical_angle","type":"integer","default":0,"required":false},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Recraft V4.1","job_set_type":"recraft_v4_1","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:4","4:3","4:5","5:4","3:2","2:3","16:9","9:16","21:9"]},{"name":"background_color","type":"object","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"colors","type":"array","default":null,"required":false},{"name":"model_type","type":"string","default":"standard","required":false,"enum":["standard","vector","utility","utility_vector"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Reframe","job_set_type":"reframe","type":"video","params":[{"name":"aspect_ratio","type":"string","default":null,"required":true,"enum":["21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"3D Objects","job_set_type":"sam_3_3d","type":"3d","params":[{"name":"detection_threshold","type":"object","default":null,"required":false},{"name":"export_textured_glb","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"3D Body","job_set_type":"sam_3_3d_body","type":"3d","params":[{"name":"export_meshes","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"include_3d_keypoints","type":"boolean","default":true,"required":false},{"name":"include_mhr_params","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Remove Background","job_set_type":"sam_3_video","type":"video","params":[{"name":"apply_mask","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"frames_count","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false}]},{"display_name":"Seed Audio 1.0","job_set_type":"seed_audio","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"expression_intensity","type":"integer","default":5,"required":false},{"name":"format","type":"string","default":"wav","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"loudness_rate","type":"integer","default":0,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mood","type":"number","default":0,"required":false},{"name":"pitch_rate","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","24000","32000","44100","48000"]},{"name":"speech_rate","type":"integer","default":0,"required":false},{"name":"voice_id","type":"object","default":null,"required":false},{"name":"voice_style","type":"object","default":null,"required":false},{"name":"voice_type","type":"object","default":null,"required":false},{"name":"voices","type":"array","default":null,"required":false}]},{"display_name":"Seedance 2.0","job_set_type":"seedance_2_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]}]},{"display_name":"Seedance 2.0 Mini","job_set_type":"seedance_2_0_mini","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p"]}]},{"display_name":"Seedance 2.5","job_set_type":"seedance_2_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"audio_references","type":"array","default":null,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"end_image","type":"object","default":null,"required":false},{"name":"extension_mode","type":"string","default":null,"required":false,"enum":["backward","forward"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"image_references","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"t2v","required":false,"enum":["t2v","omni_reference","video_edit","video_extension"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"start_image","type":"object","default":null,"required":false},{"name":"video_references","type":"array","default":null,"required":false}]},{"display_name":"Seedance 1.5 Pro","job_set_type":"seedance1_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"duration","type":"string","default":4,"required":false,"enum":["4","8","12"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Seedream 4.5","job_set_type":"seedream_v4_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","4:3","16:9","3:2","21:9","3:4","9:16","2:3"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Lite","job_set_type":"seedream_v5_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Pro","job_set_type":"seedream_v5_pro","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"height","type":"object","default":null,"required":false},{"name":"is_inpaint","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","1.5k","2k"]},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Sonilo Music","job_set_type":"sonilo_music","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Soul Cast","job_set_type":"soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"soul_cinema_studio","job_set_type":"soul_cinema_studio","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Soul Cinematic","job_set_type":"soul_cinematic","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]}]},{"display_name":"Soul Location","job_set_type":"soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Sync Lipsync 3","job_set_type":"sync_so","type":"video","params":[{"name":"input_video","type":"object","default":null,"required":true},{"name":"input_audio","type":"object","default":null,"required":true},{"name":"sync_mode","type":"string","default":"bounce","required":false,"enum":["bounce","loop","cut_off","silence","remap"]},{"name":"folder_id","type":"string","default":null,"required":false}]},{"display_name":"Higgsfield Soul 2.0","job_set_type":"text2image_soul_v2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"Text to Speech V2","job_set_type":"text2speech_v2","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"emotion","type":"object","default":null,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["mp3","wav"]},{"name":"language_boost","type":"string","default":"auto","required":false,"enum":["auto","af","ar","bg","ca","cs","da","de","el","en","es","fa","fi","fil","fr","he","hi","hr","hu","id","it","ja","ko","ms","nl","nn","no","pl","pt","ro","ru","sk","sl","sv","ta","th","tr","uk","vi","yue","zh"]},{"name":"model","type":"string","default":null,"required":true,"enum":["elevenlabs","minimax","seed_speech","vibe_voice","cozy_voice"]},{"name":"pitch","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":32000,"required":false,"enum":["8000","16000","22050","24000","32000","44100"]},{"name":"speed","type":"number","default":1,"required":false},{"name":"stability","type":"object","default":null,"required":false},{"name":"text_normalization","type":"boolean","default":false,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"number","default":1,"required":false}]},{"display_name":"Topaz","params":[{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"string","default":null,"required":false},{"name":"image_references","type":"array","default":null,"required":true},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"sharpen","type":"number","default":0,"required":false},{"name":"variant","type":"string","default":"Standard V2","required":false,"enum":["Standard V2","Low Resolution V2","CGI","High Fidelity V2","Text Refine"]}],"type":"image","job_set_type":"topaz_image"},{"display_name":"Topaz","job_set_type":"topaz_image_generative","type":"image","params":[{"name":"autoprompt","type":"boolean","default":true,"required":false},{"name":"creativity","type":"integer","default":1,"required":false},{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"Redefine","required":false,"enum":["Standard MAX","Redefine","Recovery","Recovery V2"]},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"sharpen","type":"number","default":0,"required":false},{"name":"texture","type":"integer","default":1,"required":false}]},{"display_name":"Topaz","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"enhancement","type":"object","default":null,"required":false},{"name":"frame_interpolation","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"1080p","required":false,"enum":["1080p","2160p"]},{"name":"video_references","type":"array","default":null,"required":true}],"type":"video","job_set_type":"topaz_video"},{"display_name":"Text to 3D","job_set_type":"tripo_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"negative_prompt","type":"object","default":null,"required":false},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]}]},{"display_name":"Tripo H3.1 Image to 3D","job_set_type":"tripo_h3_1_image_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Tripo H3.1 Multiview to 3D","job_set_type":"tripo_h3_1_multiview_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Google Veo 3","job_set_type":"veo3","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"veo-3-fast","required":false,"enum":["veo-3-preview","veo-3-fast"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Google Veo 3.1","job_set_type":"veo3_1","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"model","type":"string","default":"veo-3-1-fast","required":false,"enum":["veo-3-1-preview","veo-3-1-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high","ultra"]}]},{"display_name":"Google Veo 3.1 Lite","job_set_type":"veo3_1_lite","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","auto"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Video Background Remover","job_set_type":"video_background_remover","type":"video","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Video Deflicker","job_set_type":"video_deflicker","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"Video Upscale","job_set_type":"video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"voice_change","job_set_type":"voice_change","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":"preset","required":false,"enum":["preset","element"]}]},{"display_name":"Wan 2.6 Video","job_set_type":"wan2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10","15"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 2.7","job_set_type":"wan2_7","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 3.0","job_set_type":"wan3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enable_thinking","type":"boolean","default":false,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Z Image","job_set_type":"z_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"prompt","type":"string","default":null,"required":true}]}]'), Zu = {
  ...
  "sketch",
  "texture_image_url"
]), nc = /* @__PURE__ */ new Set(["input_images", "image_references"]), ic = /* @__PURE__ */ new Set(["input_video", "video"]), oc = /* @__PURE__ */ new Set(["input_audio"]);
function Oi(t) {
  return t.split(/[_-]+/).filter(Boolean).map((e) => e.charAt(0).toUpperCase() + e.slice(1)).join(" ");
}
function ac(t) {
  return tc[t] ?? `hf-${t.replaceAll("_", "-")}`;
}
function sc(t) {
...
function lc(t, e) {
  let r, n, i = !1;
  if (rc.has(e.name) ? (r = "image", n = t.type === "video" && e.name === "input_image" ? "start_image" : "image") : nc.has(e.name) ? (r = "image", n = "image", i = !0) : e.name === "video_references" ? (r = "video", n = "video", i = !0) : e.name === "audio_references" ? (r = "audio", n = "audio", i = !0) : ic.has(e.name) ? (r = "video", n = "video") : oc.has(e.name) ? (r = "audio", n = "audio") : e.name === "model_url" ? r = "model3d" : e.name === "urls" ? (r = "media", i = !0) : e.name === "medias" && (i = !0, t.type === "image" || t.type === "3d" ? (r = "image", n = "image") : t.type === "text" ? (r = "video", n = "video") : r = "media"), !!r)
    return {
      id: e.name,
      portType: r,
      label: Oi(e.name),
...
function uc(t, e) {
  var r, n, i, o, a;
  if (it(t.job_set_type)) {
    if (e.id === "image_references") return { ...e, required: !0, label: "Reference images", description: "At least one image showing the subject or object to use." };
    if (e.id === "video_references") return { ...e, required: !0, multiple: !1, label: "Source video", description: "Exactly one video to transfer motion from or edit." };
  }
...
const fc = ["prompt", "user_prompt", "instruction"];
function pc(t) {
  for (const e of fc) {
...
function gc(t = Rs) {
  ...
  const e = {};
  for (const n of t) {
    const i = ac(n.job_set_type), o = sc(n.type);
    if (e[i]) throw new Error(`Duplicate Higgsfield node type: ${i}`);
    const a = pc(n.params.flatMap((l) => [
      uc(n, dc(n, l)),
      ...cc(n, l)
    ])), s = a.findIndex((l) => l.id === "prompt") + 1;
    a.splice(s, 0, ...mc(n)), e[i] = {
      id: n.job_set_type,
      nodeType: i,
      name: n.job_set_type === "hf_mult_motion_control" ? "Genjutsu · Motion Transfer" : n.job_set_type === "hf_mult_replace_object" ? "Genjutsu · Object Replacement" : n.job_set_type === "topaz_image" ? "Topaz Image Upscale" : n.job_set_type === "topaz_video" ? "Topaz Video Upscale" : n.display_name,
      category: o,
      description: n.job_set_type === "hf_mult_motion_control" ? "Transfer motion from one source video to subjects in one or more reference images. Optional prompt; 480p, 720p, or 1080p." : n.job_set_type === "hf_mult_replace_object" ? "Replace objects in one source video using one or more reference images. Optional prompt; 480p, 720p, or 1080p." : `Higgsfield ${n.type.toUpperCase()} model`,
      inputs: a,
      outputType: o,
      outputs: [{ id: o, portType: o, label: o === "model3d" ? "3D Model" : Oi(o) }],
      provider: "higgsfield",
...
const hc = {
  ...
  video: "--video",
  audio: "--audio"
}, Os = /^[A-Za-z][A-Za-z0-9_]*$/, Ns = /* @__PURE__ */ new Set(["json", "wait", "no_color"]);
function yc(t) {
  const e = ["generate", "create", t.model], r = {
    ...Ps(t.model, { ...t.extra, ...t.params }) ?? {}
  }, n = (o, a) => {
    if (a == null) return;
    if (!Os.test(o) || Ns.has(o))
  ...
  if (js(t.model)) {
    if (Yu(t.model, r, t.medias ?? []), t.model === "sync_so") throw new Error("Sync Lipsync 3 is available through Higgsfield MCP in the CineGen website. The current Higgsfield desktop CLI does not expose this model. No generation was submitted.");
  ...
  for (const o of t.medias ?? []) {
    if (!o.value) continue;
    const a = it(t.model) ? o.role === "video" ? "--video-references" : "--image-references" : hc[o.role];
    e.push(a, o.value);
  }
  t.aspectRatio !== void 0 && !it(t.model) && (delete r.aspect_ratio, n("aspect_ratio", t.aspectRatio)), t.durationSec !== void 0 && !it(t.model) && (delete r.duration, t.durationSec > 0 && n("duration", t.durationSec)), t.count !== void 0 && !it(t.model) && (delete r.count, t.count >= 1 && n("count", t.count));
  for (const [o, a] of Object.entries(r))
    n(o, a);
  return t.wait !== !1 && e.push("--wait"), e.push("--json"), e;
}
...
const _c = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;
function wc(t) {
...
function bc(t) {
  for (const e of ["results", "jobs"]) {
    const r = t[e];
    if (Array.isArray(r) && r.length > 0 && We(r[0])) return r[0];
  }
  return t;
}
function Ci(t) {
  ...
  const r = (a) => Array.isArray(a) ? { results: a } : We(a) ? a : { result: a };
  let n = null;
  try {
    n = r(JSON.parse(e));
  } catch {
  ...
  const i = bc(n), o = i.job_id ?? i.id ?? i.jobId;
  return {
...
`);
  ...
  return (r = e.match(_c)) == null ? void 0 : r[0];
}
function We(t) {
  return !!t && typeof t == "object" && !Array.isArray(t);
}
const vc = /* @__PURE__ */ new Set(["params", "prompt", "input_images", "inputs", "extra", "request"]), Uo = [
  "url",
  "urls",
  "video_url",
  "image_url",
...
function bt(t, e = 0) {
  ...
  if (Array.isArray(t))
    return [...new Set(t.flatMap((n) => bt(n, e + 1)))];
  if (!We(t)) return [];
  const r = [];
  for (const n of Uo)
    t[n] !== void 0 && r.push(...bt(t[n], e + 1));
  if (typeof t.result_json == "string" && t.result_json.trim())
    try {
      r.push(...bt(JSON.parse(t.result_json), e + 1));
    } catch {
    }
  for (const n of Mo)
    t[n] !== void 0 && r.push(...bt(t[n], e + 1));
  for (const [n, i] of Object.entries(t))
    vc.has(n) || Uo.includes(n) || Mo.includes(n) || n === "result_json" || (We(i) || Array.isArray(i)) && r.push(...bt(i, e + 1));
  return [...new Set(r)];
}
const Tc = /https?:\/\/[^\s"'<>\\]+/gi;
function Cs(t) {
...
function qi(t, e) {
  var l;
  const r = bt(t.parsed), n = r[0], i = Lr(t.parsed), o = t.record.duration ?? ((l = t.record.output) == null ? void 0 : l.duration), a = e.mediaType, s = r.map((d) => ({ kind: a, url: d }));
...
function Ls(t) {
  return t instanceof ti ? { stdout: t.stdout, stderr: t.stderr } : { stdout: "", stderr: "" };
}
...
const Do = 1260 * 1e3, Ac = 9e4, $o = 4, kn = "Higgsfield CLI not found. Install @higgsfield/cli, then run higgsfield auth login — or connect Higgsfield in Settings.";
function kc(t) {
...
function jc(t, e, r) {
  return new Promise((n, i) => {
    var d, c;
    const o = ae(t, e, { env: xc() });
    let a = "", s = "";
    const l = setTimeout(() => {
      o.kill("SIGTERM"), i(new ti("Higgsfield CLI timed out", a, s));
    }, r);
    (d = o.stdout) == null || d.on("data", (u) => {
      a += u.toString();
      ...
      if (clearTimeout(l), u === 0) {
        n(a);
        return;
      }
      const p = s.trim() || a.trim() || `Higgsfield CLI exited with code ${u}`, f = /session expired/i.test(p) ? 'Higgsfield is not connected. Run "higgsfield auth login" or connect it in Settings.' : p;
      i(new ti(f, a, s));
    });
  });
}
async function Le(t, e = 6e4) {
  const r = t.includes("--json") ? t : [...t, "--json"], n = Ic(), i = mt ? [mt, ...n.filter((l) => l !== mt)] : n;
  if (i.length === 0) throw new Error(kn);
  let o;
  for (const l of i)
    try {
      const d = await jc(l, r, e);
      return mt = l, d;
    } catch (d) {
      if (kc(d)) {
        mt === l && (mt = null), o = d;
  ...
  throw new Error(s ? `${kn} Tried: ${a}. ${s}` : `${kn} Tried: ${a}.`);
}
const Rc = 6e4, Us = {
  "image/png": ".png",
  "image/jpeg": ".jpg",
...
function Pc(t, e, r) {
  const n = e ? Us[e.split(";", 1)[0].trim().toLowerCase()] : void 0;
  if (n) return n;
  const i = w.extname(t.split(/[?#]/, 1)[0]).toLowerCase();
  return i && i.length <= 5 ? i : Ms(r);
}
async function Oc(t) {
  ...
  for (const [n, i] of t.entries()) {
    ...
    if (/^https?:\/\//i.test(o)) {
      const a = await fetch(o, { signal: AbortSignal.timeout(Rc) });
      if (!a.ok) throw new Error(`Failed to download media input (HTTP ${a.status}): ${o}`);
      const s = Pc(o, a.headers.get("content-type"), i.role), l = w.join(z.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${s}`);
      ...
      const s = o.slice(5, a), l = s.replace(/;base64$/i, ""), d = o.slice(a + 1), c = /;base64$/i.test(s) ? Buffer.from(d, "base64") : Buffer.from(decodeURIComponent(d)), u = Us[l.toLowerCase()] ?? Ms(i.role), p = w.join(z.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${u}`);
      await D.promises.writeFile(p, c), e.push(p), r.push({ ...i, value: p });
    } else
...
async function ii(t) {
  const { medias: e, tempPaths: r } = await Oc(t.medias);
  let n;
  try {
    n = await Nc({ ...t, medias: e });
  } finally {
    for (const o of r)
      D.promises.unlink(o).catch(() => {
      });
  }
  if (Ni(n, t.mediaType)) return n;
...
async function Nc(t) {
  ...
  for (let n = 1; n <= 3; n++)
    try {
      const i = await Le(e, Ac), o = await ni(i, t);
      if (o) return o;
      const a = Lo(i);
      if (a) return { jobId: a };
      ...
      r = i;
      const { stdout: o, stderr: a } = Ls(i), s = await ni(o, t);
      if (s) return s;
      const l = Lo(o, a, St(i));
      if (l) return { jobId: l };
      if (!ri(St(i)) || n === 3) throw i;
      await Ds(1500 * n);
    }
...
async function $s(t, e) {
  try {
    const r = await Bs({ size: 50 }), n = Cc(r, t);
    if (!n) return;
    const i = qi({
...
async function Li(t, e) {
  const r = await Le(["generate", "get", t], 2e4), n = Ci(r);
  ...
  const o = Cs(r);
  return o[0] ? { ...i, url: o[0], urls: o, outputs: o.map((a) => ({ kind: e.mediaType, url: a })) } : $s(t, e);
}
async function qc(t) {
  try {
    return await Le(
    ...
    if (!/unknown|unexpected|unrecognized/i.test(St(e))) throw e;
    return Le(
...
async function Fs(t, e) {
  ...
  for (let i = 1; i <= $o; i++)
    try {
      ...
      r = o;
      const { stdout: a } = Ls(o), s = await ni(a, e);
      if (s) return s;
      try {
        const c = await Li(t, e);
        if (c) return c;
      } catch (c) {
        if (!ri(St(c))) throw c;
      }
      const l = St(o);
      if (!(ri(l) || /timed out/i.test(l) || /still running/i.test(l) || /without a media URL/i.test(l))) throw o;
      if (i === $o) break;
      await Ds(2e3 * i);
    }
  const n = await $s(t, e);
  if (n) return n;
  throw new Error(
    `${St(r)} The job was submitted (${t}) and may still finish on Higgsfield.`
  );
}
function Fo(t) {
  if (Array.isArray(t)) return t.filter((e) => We(e));
  if (!We(t)) return [];
  for (const e of ["jobs", "results", "data", "items", "generations"]) {
    const r = t[e];
    if (Array.isArray(r)) return r.filter((n) => We(n));
  }
...
async function Bs(t) {
  ...
  const n = (await Le(e, 2e4)).trim();
  try {
    return Fo(JSON.parse(n));
  } catch {
    const i = Math.max(n.lastIndexOf("["), n.lastIndexOf("{"));
    return i < 0 ? [] : Fo(JSON.parse(n.slice(i)));
  }
}
async function Lc(t, e) {
  const r = await Li(t, e);
  return r && Ni(r, e.mediaType) ? r : Fs(t, e);
}
function Uc(t, e) {
  const r = ["generate", "cost", t];
  for (const [n, i] of Object.entries(Ps(t, e) ?? {}))
    i == null || i === "" || !Os.test(n) || Ns.has(n) || (typeof i == "string" ? r.push(`--${n}`, i) : typeof i == "number" && Number.isFinite(i) ? r.push(`--${n}`, String(i)) : typeof i == "boolean" && r.push(`--${n}`, i ? "true" : "false"));
...
async function Bo() {
  try {
    const t = await Le(["account", "status"], 15e3);
...
function $c() {
  I.handle("higgsfield:account-status", async () => Vo(await Bo())), I.handle("higgsfield:quick-edit", async (t, e) => {
    const { prepareClipReference: r, resolveLocalSourcePath: n } = await Promise.resolve().then(() => Ca);
    ...
    if (e.drawnFramePath && e.referenceMode === "frame")
      ...
      try {
        ...
        console.log("[higgsfield:quick-edit] extracted refs:", s.paths), i = Go({
    ...
    const { resolveLocalSourcePath: r } = await Promise.resolve().then(() => Ca);
    if (e.jobId) {
      const i = { model: e.model, mediaType: e.outputType };
      if (e.wait === !1) {
        const o = await Li(e.jobId, i);
        if (o && Ni(o, e.outputType)) return o;
        throw new Error("Higgsfield job is still running");
      }
      return Lc(e.jobId, i);
    }
  ...
  }), I.handle("higgsfield:generate-list", async (t, e) => Bs(e)), I.handle("higgsfield:generate-cost", async (t, e) => Dc(e == null ? void 0 : e.model, (e == null ? void 0 : e.params) ?? {})), I.handle("higgsfield:auth-login", async () => {
    try {
      await Le(["auth", "login"], 300 * 1e3);
    } catch (t) {
      return { connected: !1, error: t instanceof Error ? t.message : String(t) };
    }
    return Vo(await Bo());
  }), I.handle("higgsfield:auth-logout", async () => {
    await Le(["auth", "logout"], 15e3).catch(() => {
    });
  });
}
const Fc = {
...
}, Gc = "notrius/ltx-2.5-serverless:cu130@sha256:73d1621ef915ae6a149f2a32f6c317dfc89f12075ed4b3abd7df707420267205", Ui = "https://rest.runpod.io/v1", zc = "https://api.runpod.io/v2", Vs = "https://api.runpod.io/graphql", Mi = 8e3, jn = 256 * 1024, Hc = 1800, Wc = 200, oi = 15e3, Gs = 6500, Xc = 12e4, Di = 12e3, zs = 14 * 1024 * 1024, Jc = 100 * 1024 * 1024, zo = 1024 * 1024, Ho = 4, Hs = Object.freeze(["sdxl", "qwen-image-edit"]), Kc = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABACAIAAAAlC+aJAAAAYElEQVR4nO3PQQ0AIBDAMMD4WUcEj4ZkVbDtmVk/OzrgVQNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgPaBRFyAf0dnk7yAAAAAElFTkSuQmCC", Yc = "balanced", Wo = Object.freeze({
...
async function Ws(t, e, r = {}, n = oi) {
  ...
  let s;
  const l = new $i(n), d = new Promise((u, p) => {
    s = setTimeout(() => {
      i.abort(l), p(l);
    }, n);
  }), c = (async () => {
    const u = await t(e, { ...r, signal: i.signal }), p = await Zc(u);
    return { response: u, payload: p };
  })();
  try {
...
function ef(t) {
  try {
    return new URL(t).pathname === "/health" ? Gs : oi;
  } catch {
    return oi;
  }
}
...
async function xe(t, e, r, n, i = [200, 201, 202, 204], o = ef(e)) {
  let a;
  try {
    a = await Ws(t, e, r, o);
  } catch (d) {
    throw d instanceof $i ? new F(`${n} RunPod did not respond before the request timed out.`, "PROVIDER_TIMEOUT", 504) : new F(d instanceof Error ? d.message : n, "PROVIDER_UNREACHABLE", 502);
  }
  const { response: s, payload: l } = a;
  if (!i.includes(s.status))
    throw new F(rn(l, `${n} (${s.status})`), "PROVIDER_ERROR", s.status);
  return l;
}
function Fi(t, e = !1) {
  return {
...
async function rf(t, e, r) {
  var a;
  const n = new AbortController(), i = setTimeout(() => n.abort(), Hc);
  let o;
  try {
    const s = new URL(`${zc}/pods/${encodeURIComponent(r)}/logs`);
    ...
    let c = "", u = await o.read();
    for (; !u.done && c.length < jn && (c += d.decode(u.value, { stream: !0 }), !(c.length >= jn)); ) {
      let p;
      const f = new Promise((m) => {
        p = setTimeout(() => m({ quiet: !0 }), Wc);
      });
      if (u = await Promise.race([o.read(), f]), clearTimeout(p), u != null && u.quiet)
        break;
    }
    return c += d.decode(), tf(c.slice(0, jn));
  } catch {
    return [];
  } finally {
...
async function Xo(t, e, r, n) {
  const i = `mutation { secretCreate(input: { name: ${JSON.stringify(r)}, value: ${JSON.stringify(n)}, description: "Temporary CineGen LTX-2.5 session credential" }) { id name } }`, o = new URL(Vs);
  ...
  if (s.length || typeof (d == null ? void 0 : d.id) != "string")
    throw new F(rn(a, "RunPod could not create the encrypted session secret."));
  return d.id;
}
async function Xs(t, e, r) {
  const n = `mutation { secretDelete(id: ${JSON.stringify(r)}) }`, i = new URL(Vs);
...
function uf(t) {
  return `https://${t}-${Mi}.proxy.runpod.net`;
}
function Bi(t, e) {
  const r = tn(e, "RunPod session ID"), n = new URL(Ie(t, "RunPod session URL"));
  if (n.protocol !== "https:" || n.username || n.password || n.hostname !== `${r}-${Mi}.proxy.runpod.net`)
...
async function gf(t, e = fetch) {
  ...
  const i = ff(t.gpuProfile), o = pf(t.imageModels), a = cf(), s = `cinegen_ltx25_hf_${a}`, l = `cinegen_ltx25_session_${a}`, d = `${crypto.randomUUID()}${crypto.randomUUID()}`.replace(/-/g, ""), c = [];
  try {
    c.push(await Xo(e, r, s, n)), c.push(await Xo(e, r, l, d));
    const u = await xe(e, `${Ui}/pods`, {
      method: "POST",
      headers: Fi(r, !0),
      body: JSON.stringify({
        ...
        containerDiskInGb: mf(i.config, o),
        volumeInGb: 0,
        ports: [`${Mi}/http`],
        supportPublicIp: !0,
        interruptible: !1,
    ...
    }, "RunPod could not create the LTX-2.5 session Pod."), p = Js(u);
    if (!p.id)
    ...
    throw await Promise.allSettled(c.map((p) => Xs(e, r, p))), u;
  }
}
async function hf(t, e = fetch) {
  const r = Ie(t.runpodKey, "RunPod API key"), n = Ie(t.podAuthToken, "RunPod session token"), i = Bi(t.podUrl, t.podId);
  let o;
  try {
    o = await xe(e, `${Ui}/pods/${i.podId}`, {
      headers: Fi(r)
    }, "RunPod could not read the LTX-2.5 session.");
  } catch (u) {
    if (u instanceof F && u.statusCode === 404)
      return {
        status: "ended",
        phase: "ended",
        podId: i.podId,
  ...
  if (a.desiredStatus === "RUNNING")
    try {
      const { response: u, payload: p } = await Ws(e, `${i.url}/health`, {
      ...
      if (u.ok && (p == null ? void 0 : p.ready) === !0)
        return rl(p) ? {
          status: "ready",
          phase: "ready",
          podId: i.podId,
      ...
      s = { kind: "response", status: u.status, body: p };
    } catch (u) {
      s = u instanceof $i ? { kind: "timeout" } : { kind: "unreachable" };
    }
  const l = await rf(e, r, i.podId), d = af(l);
  if (!d && nf(l))
    try {
      const u = await Ks({
        runpodKey: r,
      ...
      return {
        ...
        podId: i.podId,
        podUrl: i.url,
        message: u.warning ? "The LTX-2.5 container could not start. CineGen deleted the failed Pod and billing stopped, but RunPod could not remove one temporary secret. Check RunPod Secrets." : "The LTX-2.5 container could not start. CineGen deleted the failed Pod and temporary secrets; billing stopped.",
        costPerHr: null,
        gpu: a.gpu
      };
    } catch {
  ...
  let c;
  return (s == null ? void 0 : s.kind) === "timeout" ? c = "RunPod reports the Pod is running, but its private gateway did not answer within 7 seconds. It may still be starting; check again shortly. Billing continues while the Pod runs." : (s == null ? void 0 : s.kind) === "response" && (s.status === 502 || s.status === 504) ? c = `RunPod reports the Pod is running, but its private gateway returned ${s.status}. The container may still be starting; check again shortly. Billing continues while the Pod runs.` : c = lf(s == null ? void 0 : s.body) ?? df(l), !c && (s == null ? void 0 : s.kind) === "unreachable" ? c = "RunPod reports the Pod is running, but its private gateway is not reachable yet. The container may still be starting; check again shortly. Billing continues while the Pod runs." : !c && (s == null ? void 0 : s.kind) === "response" && s.status >= 400 && (c = `RunPod reports the Pod is running, but its private gateway returned HTTP ${s.status}. Check again shortly. Billing continues while the Pod runs.`), {
    status: "downloading",
    phase: "downloading",
    podId: i.podId,
    podUrl: i.url,
    message: c ?? "Downloading weights and loading LTX-2.5 into the GPU…",
    costPerHr: a.costPerHr,
    gpu: a.gpu
  };
}
async function Ks(t, e = fetch) {
  const r = Ie(t.runpodKey, "RunPod API key"), n = tn(t.podId, "RunPod session ID");
  await xe(e, `${Ui}/pods/${n}`, {
    method: "DELETE",
    headers: Fi(r)
  ...
  return (await Promise.allSettled(i.map((s) => Xs(e, r, s)))).filter((s) => s.status === "rejected").length ? { ok: !0, warning: "The Pod was deleted and billing stopped, but one temporary RunPod secret could not be removed." } : { ok: !0 };
}
function yf(t, e) {
...
function _f(t) {
  const e = JSON.parse(JSON.stringify(Fc)), r = Ie(t.prompt, "Video prompt");
  if (r.length > Di)
    throw new F("The LTX-2.5 video prompt is too long.", "PROMPT_TOO_LONG", 422);
  const n = Math.min(20, Math.max(1, Math.round(Number(t.durationSec) || 5))), i = ["16:9", "9:16", "1:1"].includes(t.aspectRatio ?? "") ? t.aspectRatio : "16:9", o = t.resolution === "480p" || t.resolution === "1080p" ? t.resolution : "720p", a = yf(i, o);
  return e["398:376"].inputs.value = r, e[395].inputs.image = "cinegen-source.png", e["398:362"].inputs.value = n, e["398:372"].inputs.value = a.width, e["398:360"].inputs.value = a.height, e["398:361"].inputs.value = 24, e["398:380"].inputs.sampling_mode = "on", e["398:380"].inputs["sampling_mode.seed"] = Math.floor(Math.random() * 999999998) + 1, e["398:383"].inputs.value = t.generateAudio !== !1, e["398:363"].inputs.value = Ys(t).length === 0, e["398:338"].inputs.noise_seed = Math.floor(Math.random() * 999999999999998) + 1, e["398:339"].inputs.noise_seed = Math.floor(Math.random() * 999999999999998) + 1, e;
}
function wf(t) {
  const e = Ys(t), r = e.find((a) => a.startsWith("data:image/")) ?? (e.length ? "" : Kc), n = /^data:(image\/(?:png|jpeg|webp|gif|bmp|avif));base64,([A-Za-z0-9+/]*={0,2})$/i.exec(r);
...
function vf(t, e) {
  ...
  if (Math.floor(r[2].length * 0.75) - n > zs)
...
function Sf(t, e, r, n, i, o) {
  const a = JSON.parse(JSON.stringify(Bc));
...
function If(t, e, r, n) {
  const i = JSON.parse(JSON.stringify(Vc));
...
function xf(t) {
  const e = Qs(t.model), r = Ef(t.prompt, "Image prompt"), n = typeof t.negativePrompt == "string" ? t.negativePrompt.trim().slice(0, Di) : e === "sdxl" ? "text, watermark, logo, low quality, distorted" : "", i = Ko(t.width, 1024), o = Ko(t.height, 1024), a = bf(t.seed), s = Tf(t, e), l = e === "sdxl" ? Sf(t, r, n, i, o, a) : If(r, n, a, s);
  return {
    model: e,
    label: nn(e),
    workflow: l,
    images: s.map((d, c) => ({
      name: `cinegen-qwen-reference-${c + 1}.png`,
      image: d
    })),
...
function Fr(t, e = "video/mp4") {
  return Vi(t) || e;
}
function Af(t, e) {
  if (e === "videos" || e === "video" || Vi(t).startsWith("video/"))
    return !0;
  const n = t.filename ?? t.name;
  return typeof n == "string" && /\.(?:mp4|webm|mov|mkv|avi|m4v)(?:$|[?#])/i.test(n);
}
function kf(t, e) {
  if (e === "images" || e === "image" || Vi(t).startsWith("image/"))
    return !0;
  const n = t.filename ?? t.name;
...
function jf(t, e) {
  ...
  for (const { record: n, parentKey: i } of on(t)) {
    const o = Ge(n.video_url, n.videoUrl);
    if (o)
      return { url: o, durationSec: e, model: "LTX-2.5" };
    const a = Ge(n.video_base64, n.videoBase64);
    if (a)
      return { data: a, mediaType: Fr(n), durationSec: e, model: "LTX-2.5" };
    if (!Af(n, i))
      continue;
    const s = Ge(n.url, n.download_url, n.downloadUrl), l = Ge(n.data, n.base64), d = s ?? (String(n.type ?? "").toLowerCase() === "url" ? l : void 0);
    if (d)
      return { url: d, durationSec: e, model: "LTX-2.5" };
    const c = String(n.type ?? "").toLowerCase() === "url" ? void 0 : l;
    if (c)
      return { data: c, mediaType: Fr(n), durationSec: e, model: "LTX-2.5" };
  }
...
function Rf(t, e) {
  ...
  for (const { record: i, parentKey: o } of on(t)) {
    const a = Ge(i.image_url, i.imageUrl);
    if (a)
      return { url: a, model: r };
    const s = Ge(i.image_base64, i.imageBase64);
    if (s)
      return { data: s, mediaType: Fr(i, "image/png"), model: r };
    if (!kf(i, o))
      continue;
    const l = Ge(i.url, i.download_url, i.downloadUrl), d = Ge(i.data, i.base64), c = l ?? (String(i.type ?? "").toLowerCase() === "url" ? d : void 0);
    if (c)
      return { url: c, model: r };
    const u = String(i.type ?? "").toLowerCase() === "url" ? void 0 : d;
    if (u)
      return { data: u, mediaType: Fr(i, "image/png"), model: r };
  }
  throw new F(`${r} completed without returning an image.`, "INVALID_PROVIDER_RESPONSE", 502);
}
function el(t, e = "video", r = "LTX-2.5") {
  for (const { record: n, parentKey: i } of on(t)) {
    if (i !== "artifact")
      continue;
...
function Of(t, e, r = "video", n = "LTX-2.5") {
...
async function tl(t, e, r, n, i = "video", o = "LTX-2.5") {
  const a = new Uint8Array(n.byteSize);
  let s;
  const l = [];
  for (let u = 0; u < n.byteSize; u += zo) {
    const p = Math.min(zo, n.byteSize - u);
    l.push({ offset: u, length: p });
  }
  for (let u = 0; u < l.length; u += Ho) {
    const p = l.slice(u, u + Ho), f = await Promise.all(p.map(async ({ offset: m, length: y }) => {
      ...
      const h = await xe(t, g.toString(), {
        headers: { Authorization: `Bearer ${r}`, Accept: "application/json" }
      }, `CineGen could not download the ${o} ${i} chunk.`);
      if (!h || typeof h != "object")
        throw new F(`${o} returned an invalid ${i} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
      const _ = typeof h.id == "string" ? h.id : "", b = Number(h.offset), v = Number(h.byteSize ?? h.byte_size), T = typeof (h.mediaType ?? h.media_type) == "string" ? String(h.mediaType ?? h.media_type).trim() : "", S = Pf(h.data, o, i);
      if (_ !== n.id || b !== m || v !== n.byteSize || n.mediaType && T !== n.mediaType || S.byteLength !== y)
        throw new F(`${o} returned an inconsistent ${i} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
      return { offset: m, bytes: S };
    }));
    for (const m of f)
      m.offset === 0 && (s = m.bytes.slice(0, 12)), a.set(m.bytes, m.offset);
  }
  const d = Of(s ?? new Uint8Array(), n.mediaType, i, o), c = Nf(a, i);
  return await Cf(t, e, r, n.id), { data: c, mediaType: d };
}
function rl(t) {
  if (!t || typeof t != "object")
    return !1;
...
async function qf(t, e, r, n) {
  const i = await nl(t, e, r), o = i == null ? void 0 : i.capabilities, a = Array.isArray(i == null ? void 0 : i.installedModels) ? i.installedModels : [];
  ...
  if (!a.includes(n))
    throw new F(
      `${nn(n)} was not installed when this Pod was created. Start a new session with that image model selected.`,
...
async function il(t, e, r, n, i, o, a) {
  var u;
  const s = Lf(), l = ((u = i == null ? void 0 : i.capabilities) == null ? void 0 : u.idempotentSubmissions) === !0, d = JSON.stringify({ input: { ...o, cinegen_job_id: s } }), c = () => xe(t, `${e.url}/run`, {
    method: "POST",
  ...
  for (let p = 0; p < (l ? 2 : 1); p += 1)
    try {
      const f = await c(), m = typeof (f == null ? void 0 : f.id) == "string" ? f.id : "";
      if (!m)
...
async function Df(t, e = fetch) {
  const r = Bi(t.podUrl, t.podId), n = Ie(t.podAuthToken, "RunPod session token"), i = { Authorization: `Bearer ${n}`, Accept: "application/json", "Content-Type": "application/json" };
  if (t.jobId) {
    const d = tn(t.jobId, "RunPod generation job ID"), c = await xe(e, `${r.url}/status/${d}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    }, "CineGen could not read the LTX-2.5 generation status."), u = String(c.status ?? "").toUpperCase();
    if (u === "IN_QUEUE")
    ...
    if (u === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: rn(c, "LTX-2.5 generation failed.") };
    if (u === "COMPLETED") {
      const p = Math.min(20, Math.max(1, Math.round(Number(c.durationSec) || 5)));
      try {
        const f = el(c.output), m = f ? { ...await tl(e, r, n, f), durationSec: p, model: "LTX-2.5" } : jf(c.output, p);
  ...
  const o = Math.min(20, Math.max(1, Math.round(Number(t.input.durationSec) || 5))), a = {
    workflow: _f(t.input),
    images: [{ name: "cinegen-source.png", image: wf(t.input) }],
    cinegen_duration_sec: o,
    cinegen_task: "ltx-2.5"
  }, s = await nl(e, r, n), { jobId: l } = await il(e, r, n, i, s, a, "CineGen could not submit the LTX-2.5 generation.");
  return { jobId: l, status: "queued", phase: "rendering", message: "LTX-2.5 generation queued." };
}
async function $f(t, e = fetch) {
  var l;
  const r = Bi(t.podUrl, t.podId), n = Ie(t.podAuthToken, "RunPod session token"), i = { Authorization: `Bearer ${n}`, Accept: "application/json", "Content-Type": "application/json" };
  if (t.jobId) {
    const d = tn(t.jobId, "RunPod generation job ID"), c = await xe(e, `${r.url}/status/${d}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    }, "CineGen could not read the session image generation status."), u = Jo(c == null ? void 0 : c.task), p = Jo(t.model ?? ((l = t.input) == null ? void 0 : l.model));
    if (u && p && u !== p)
    ...
    const m = nn(f), y = String(c.status ?? "").toUpperCase();
    if (y === "IN_QUEUE")
    ...
    if (y === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: rn(c, `${m} generation failed.`) };
    if (y === "COMPLETED")
      try {
        const g = el(c.output, "image", m), h = g ? { ...await tl(e, r, n, g, "image", m), model: m } : Rf(c.output, f);
  ...
  const o = xf(t.input), a = await qf(e, r, n, o.model), { jobId: s } = await il(e, r, n, i, a, {
    workflow: o.workflow,
    images: o.images,
...
const Ff = gf, Bf = hf, Vf = Ks, Gf = Df, zf = $f, Tt = "https://api.kie.ai/api/v1", Hf = 3e3, Wf = 120, Xf = {
  runway: `${Tt}/runway/generate`,
  veo: `${Tt}/veo/generate`,
  "4o-image": `${Tt}/gpt4o-image/generate`,
  "suno-music": `${Tt}/generate`
};
function Jf(t) {
  for (const [e, r] of Object.entries(Xf))
    if (t.startsWith(e)) return r;
}
async function Kf(t, e, r) {
  const n = Jf(t), i = n ?? `${Tt}/jobs/createTask`, o = n ? { ...e, callBackUrl: "" } : { model: t, input: e, callBackUrl: "" }, a = await fetch(i, {
    method: "POST",
    headers: {
...
async function Yf(t, e) {
  for (let r = 0; r < Wf; r++) {
    await new Promise((a) => setTimeout(a, Hf));
    const n = await fetch(`${Tt}/jobs/recordInfo?taskId=${t}`, {
...
function lr(t, e, r = !1) {
  if (typeof t == "string") {
    const a = ol(t).trim(), s = r ? rp(a, e) : e;
    return a ? [{ value: a, role: s }] : [];
  }
  if (Array.isArray(t))
    return t.flatMap((a) => lr(a, e, r));
  if (!t || typeof t != "object") return [];
  const n = t, i = tp(n, e);
  if (Array.isArray(n.allUrls))
    return n.allUrls.flatMap((a) => lr(
      a,
      i.role,
      r && !i.explicit
    ));
...
function ip(t, e, r) {
  const n = [], i = {}, o = t === "seedance_2_5" ? "image" : np(r);
  for (const [a, s] of Object.entries(e)) {
    if (s == null) continue;
...
async function ap(t, e, r) {
  const n = await ii(ip(t, e, r));
  return op(n);
}
const Yo = "https://api.runpod.ai/v2", sp = 3e3, lp = 120;
async function dp(t, e, r) {
  ...
  for (let o = 0; o < lp; o++) {
    await new Promise((l) => setTimeout(l, sp));
    const a = await fetch(`${Yo}/${t}/status/${i}`, {
    ...
    if (s.status === "COMPLETED") {
      ...
      if (d && !d.startsWith("http") && !d.startsWith("local-media://")) {
        const c = d.includes(",") ? d.split(",")[1] : d, u = w.join(z.tmpdir(), `cinegen-runpod-${Date.now()}.png`);
        return await R.writeFile(u, Buffer.from(c, "base64")), { output: { ...l, image_url: `local-media://file${u}` } };
      }
      return { output: l };
    }
    if (s.status === "FAILED")
...
async function cp(t, e) {
  ...
  const s = (c = (d = a.runtime) == null ? void 0 : d.ports) == null ? void 0 : c.find((u) => u.privatePort === 8e3 && u.isIpPublic);
  return {
...
const It = 14 * 1024 * 1024, Zo = 100 * 1024 * 1024, ea = 100 * 1024 * 1024, fp = 45e3;
function Gi(t) {
...
async function mp(t) {
  ...
  if (Number.isFinite(e) && e > It)
    throw new Error("The LTX-2.5 first-frame reference is larger than 14 MB.");
  if (!t.body) {
    const o = Buffer.from(await t.arrayBuffer());
    if (o.byteLength > It) throw new Error("The LTX-2.5 first-frame reference is larger than 14 MB.");
    return o;
  }
...
async function al(t) {
  ...
  if (t.startsWith("local-media://file")) {
    const e = ol(t), r = await R.stat(e);
    if (!r.isFile()) throw new Error("The LTX-2.5 first-frame reference is not a file.");
    if (r.size > It) throw new Error("The LTX-2.5 first-frame reference is larger than 14 MB.");
    const n = await R.readFile(e);
    return ta(n);
  }
  if (/^https?:\/\//i.test(t)) {
    const e = pp(t), r = new AbortController(), n = setTimeout(() => r.abort(), fp);
    let i;
    try {
...
async function yp(t) {
  ...
  const r = await Promise.all(e.map(async (n, i) => {
    const o = n.trim().startsWith("data:") ? n : await al(n.trim());
    return hp(o, i);
  }));
  return {
    ...t,
...
async function _p(t) {
  ...
  if (!r || r.length > Math.ceil(Zo / 3) * 4 + 8)
  ...
  if (n.byteLength > Zo) throw new Error("The LTX-2.5 video is larger than CineGen can import automatically.");
  ...
  const o = await R.mkdtemp(w.join(z.tmpdir(), "cinegen-ltx25-")), a = w.join(o, `result${i}`);
...
async function wp(t) {
  var u, p;
  const e = (p = (u = t.output) == null ? void 0 : u.data) == null ? void 0 : p.trim();
  if (!e) return t;
  const r = /^data:image\/[A-Za-z0-9.+-]+;base64,([\s\S]+)$/i.exec(e), n = ((r == null ? void 0 : r[1]) ?? e).replace(/\s+/g, "");
  if (!n || n.length > Math.ceil(ea / 3) * 4 + 8 || !/^[A-Za-z0-9+/]*={0,2}$/.test(n) || n.length % 4 === 1)
    throw new Error("RunPod returned an invalid or oversized image file.");
  const i = Buffer.from(n, "base64");
  if (!i.length || i.byteLength > ea)
    throw new Error("RunPod returned an invalid or oversized image file.");
  const o = Gi(i), a = o === "image/png" ? ".png" : o === "image/jpeg" ? ".jpg" : o === "image/webp" ? ".webp" : void 0;
  if (!a || !o) throw new Error("RunPod returned an unsupported image file.");
  const s = await R.mkdtemp(w.join(z.tmpdir(), "cinegen-runpod-image-")), l = w.join(s, `result${a}`);
  await R.writeFile(l, i, { flag: "wx", mode: 384 });
  const { data: d, ...c } = t.output;
  return {
    ...t,
    output: {
      ...c,
...
async function ra(t) {
  const e = decodeURIComponent(t.replace("local-media://file", "")), r = await R.readFile(e), n = bp(e), i = new Blob([r], { type: n }), o = new File([i], w.basename(e), { type: n });
  return K.fal.storage.upload(o);
}
async function Br(t) {
  const e = {};
  for (const [r, n] of Object.entries(t))
    typeof n == "string" && n.startsWith("local-media://file") ? e[r] = await ra(n) : Array.isArray(n) ? e[r] = await Promise.all(
      n.map(async (i) => typeof i == "string" && i.startsWith("local-media://file") ? ra(i) : i && typeof i == "object" && !Array.isArray(i) ? Br(i) : i)
    ) : n && typeof n == "object" && !Array.isArray(n) ? e[r] = await Br(n) : e[r] = n;
  return e;
}
async function na(t, e, r) {
  var n;
  ai(r), console.log("[fal] Calling model:", t, "with input:", JSON.stringify(e, null, 2));
  try {
...
function vp() {
  I.handle("workflow:run", async (e, r) => {
    const {
      ...
      outputType: u,
      inputs: p
    } = r, { ALL_MODELS: f, resolveVideoModelEndpoint: m, sanitizeVideoInputsForEndpoint: y } = await import("./models-BvjNCn_8.js"), g = f[c] ?? Object.values(f).find(
      (E) => E.id === c || E.altId === c || E.nodeType === c
    );
    if (!g) {
      if (c.startsWith("fal-ai/")) {
        const E = n;
        if (!E) throw new Error("No fal.ai API key provided. Add one in Settings.");
        ai(E);
        const k = await Br(p), O = await na(c, k, E);
        return O.data ?? O;
      }
    ...
    const h = g.provider;
    let _ = p;
    h !== "higgsfield" && (n && ai(n), _ = await Br(p));
    let b = c.includes("/") ? c : g.id;
    const v = g.nodeType ?? c, T = Object.keys(_).some(
    ...
    if (h === "kie") {
      ...
      const E = g.podRoute ?? b;
      S = await up(s, E, _);
    } else if (h === "runpod") {
      const E = o;
      if (!E) throw new Error("No RunPod API key provided. Add one in Settings.");
      const k = a || g.runpodEndpointId || "";
      S = await dp(k, _, E);
    } else if (h === "higgsfield") {
      const E = g.outputType;
      S = await ap(b, _, u ?? (E === "video" ? "video" : E === "audio" ? "audio" : E === "text" ? "text" : E === "3d" || E === "model3d" || E === "model" ? "3d" : "image"));
    } else {
      const E = n;
      if (!E) throw new Error("No fal.ai API key provided. Add one in Settings.");
      S = await na(b, _, E);
  ...
  I.handle("workflow:poll-job", async (e, r) => {
    ...
    return n;
  }), I.handle("pod:start", async (e, r) => await Qo(r.runpodKey, r.podId, "start")), I.handle("pod:stop", async (e, r) => await Qo(r.runpodKey, r.podId, "stop")), I.handle("pod:status", async (e, r) => await cp(r.runpodKey, r.podId)), I.handle("pod:setup-ltx25", async (e, r) => await Ff(r)), I.handle("pod:status-ltx25", async (e, r) => await Bf(r)), I.handle("pod:terminate-ltx25", async (e, r) => await Vf(r)), I.handle("pod:generate-ltx25", async (e, r) => {
    const n = r.input ? await gp(r.input) : void 0;
    return await _p(await Gf({ ...r, input: n }));
  }), I.handle("pod:generate-session-image", async (e, r) => {
    const n = r.input ? await yp(r.input) : void 0;
    return await wp(await zf({ ...r, input: n }));
  });
}
const Ot = Ut(qe), sl = "https://mcp.artlist.io/mcp", Nt = "artlist", Tp = 1200 * 1e3, Ep = [
...
async function Or() {
  for (const t of Ep)
    try {
      const { stdout: e } = await Ot(t, ["--version"], {
        env: Ct(),
        timeout: 8e3
      });
...
function dr(t) {
  ...
  const e = t, r = ["result", "data", "output"].flatMap((n) => {
    const i = e[n];
    if (i && typeof i == "object") return dr(i);
    if (typeof i == "string")
      try {
        return dr(JSON.parse(i));
      } catch {
        return [];
      }
    return [];
...
function xp(t) {
  ...
  if (r && typeof r == "object") {
    const d = r;
    for (const c of ["result", "text", "message"])
      typeof d[c] == "string" && n.unshift(d[c]);
  }
  const i = dr(r);
  for (const d of n) {
    const c = (a = d.match(/```(?:json)?\s*([\s\S]*?)```/i)) == null ? void 0 : a[1];
    for (const p of [c, d])
      if (p)
        try {
          i.unshift(...dr(JSON.parse(p)));
        } catch {
        }
    const u = (s = d.match(/\{[\s\S]*\}/)) == null ? void 0 : s[0];
    if (u)
      try {
        i.unshift(...dr(JSON.parse(u)));
      } catch {
      }
  }
  for (const d of i) {
    const c = rt(d, ["url", "videoUrl", "video_url", "downloadUrl", "download_url", "mediaUrl", "media_url"]);
    if (!c || !/^https?:\/\//i.test(c)) continue;
    const u = Number(d.durationSec ?? d.duration_sec ?? d.duration);
    return {
      url: c,
      mediaType: "video",
      ...Number.isFinite(u) && u > 0 ? { durationSec: u } : {},
      ...rt(d, ["generationId", "generation_id", "id"]) ? { generationId: rt(d, ["generationId", "generation_id", "id"]) } : {},
      ...rt(d, ["accountUrl", "account_url", "sessionUrl", "session_url"]) ? { accountUrl: rt(d, ["accountUrl", "account_url", "sessionUrl", "session_url"]) } : {},
      ...rt(d, ["model", "modelId", "model_id"]) ? { model: rt(d, ["model", "modelId", "model_id"]) } : {}
    };
  }
  const o = (l = n.join(`
...
async function ur(t) {
  try {
    const { stdout: e, stderr: r } = await Ot(t, ["mcp", "get", Nt], {
      env: Ct(),
      timeout: 2e4
    }), n = `${e}
...
async function Ap(t) {
  (await ur(t)).configured || await Ot(t, [
    "mcp",
    "add",
    "--transport",
    "http",
...
function jp(t, e = process.platform, r = "/tmp/cinegen-artlist-login.command") {
  if (e === "darwin") {
    const n = [
      "#!/bin/zsh",
...
`);
  ...
  return { file: t, args: ["mcp", "login", Nt], detached: !1 };
}
function Rp(t) {
...
async function Op(t, e) {
  const r = w.join(G.getPath("userData"), "artlist-mcp-workspace");
  await st(r, { recursive: !0 });
  const n = Ip(e), { stdout: i } = await Ot(t, [
    "-p",
    n,
    "--output-format",
    "json",
...
function Np() {
  I.handle("artlist:account-status", async () => {
    ...
    const e = jp(
      t,
      process.platform,
      w.join(G.getPath("userData"), "artlist-login.command")
    );
    try {
      e.script && (await Za(e.script.path, e.script.contents, { mode: 448 }), await Od(e.script.path, 448)), await Ot(e.file, e.args, {
        env: Ct(),
        timeout: e.detached ? 2e4 : 300 * 1e3,
        maxBuffer: 2 * 1024 * 1024
      });
    } catch (r) {
      throw new Error(Rp(r));
    }
    return e.detached ? Pp(t) : ur(t);
  }), I.handle("artlist:auth-logout", async () => {
    const t = await Or();
    t && await Ot(t, ["mcp", "logout", Nt], {
      env: Ct(),
      timeout: 2e4
    });
...
function Cp(t, e, r, n) {
  const i = (d) => /audio/.test(d.role ?? "") ? "audio" : /video/.test(d.role ?? "") ? "video" : "image", o = t === "Video Lip Sync" || t === "Video Upscale" ? "video" : "image", a = r.filter((d) => i(d) === o), s = r.filter((d) => i(d) === "audio"), l = t.startsWith("Avatar") || t === "Video Lip Sync";
  ...
  if (t === "Image Upscale" || t === "Video Upscale") throw new Error(ll(t));
...
const dl = ["Avatar 4", "Avatar 4 Fast", "Video Lip Sync", "Image Upscale", "Video Upscale"];
function qp(t) {
...
const ue = (t) => t && typeof t == "object" && !Array.isArray(t) ? t : {};
function Up(t) {
  const e = ue(t), r = ue(ue(e.properties).req ?? e), n = ue(ue(r.properties).prompt).maxLength;
  return typeof n == "number" && Number.isInteger(n) && n > 0 ? n : void 0;
}
function Mp(t, e) {
  const r = Up(e), n = Array.from(String(t ?? "")).length;
  if (r !== void 0 && n > r)
    throw new Error(`Topview's Canvas route accepts up to ${r.toLocaleString("en-US")} prompt characters; this prompt has ${n.toLocaleString("en-US")}. CineGen preserved the full prompt and did not submit a generation. Do not shorten or rewrite it without the user's approval.`);
}
function zi(t) {
  const e = ue(ue(t.raw).parameters ?? t.parameters), r = ue(e.supportHybridUploadsRef);
  if (r.audios === !1 || r.maxAudios === 0) return !1;
  if (r.audios === !0 || Number(r.maxAudios) > 0 || Number(e.maxRefAudioCount) > 0) return !0;
  const n = ue(t.submitParameterOptions);
  return Object.hasOwn(n, "inputAudios") || Object.hasOwn(ue(t.defaultSubmitParameters), "inputAudios") ? !0 : [t.submitModel, t.displayName, t.name].some((i) => typeof i == "string" && /seedance[-\s]+2[.\-](?:5|0)(?:\b|[-\s])/i.test(i));
}
function Rn(t, e, r, n = !1) {
  if (e.omniReferenceTaskType === "edit") {
    if (n) return "canvas-mcp";
  ...
  const i = ue(t), o = ue(ue(i.properties).req ?? i);
  if (Object.hasOwn(ue(o.properties), "inputAudios")) return "mcp";
...
function cl(t) {
  return typeof t == "string" && /^seedance[ -]2[.-]5$/i.test(t.trim());
}
function Dp(t) {
  const e = t.params;
  return (t.videoMode ?? t.video_mode ?? (e == null ? void 0 : e.video_mode)) === "edit";
}
function $p(t, e) {
  if (!cl(t.model)) throw new Error("Topview Clip Edit is available for Seedance 2.5. Choose that model or switch back to generation.");
  if (e.filter((a) => a.role === "video" || a.role === "reference_video").length !== 1) throw new Error("Clip Edit needs exactly one source video. Connect it to Video to edit, then add optional image and audio references.");
  if (e.some((a) => a.role === "start_image" || a.role === "end_image")) throw new Error("Clip Edit uses the source video’s framing. Add images as references instead of first or last frames.");
  const n = t.params ?? {}, i = Number(String(t.resolution ?? n.resolution ?? 720).replace(/p$/i, ""));
  if (![480, 720, 1080].includes(i)) throw new Error("Seedance 2.5 Clip Edit supports 480p, 720p, or 1080p.");
  const o = t.generateAudio ?? n.generate_audio;
  return {
    model: "seedance-2.5",
...
const Fp = 407696;
function Bp(t) {
  const e = new DataView(t.buffer, t.byteOffset, t.byteLength), r = (o) => String.fromCharCode(...t.subarray(o, o + 4));
  function n(o, a) {
    const s = [];
    for (; o + 8 <= a && s.length < 1e4; ) {
      let l = e.getUint32(o), d = 8;
      if (l === 1) {
        if (o + 16 > a) break;
        l = e.getUint32(o + 8) * 2 ** 32 + e.getUint32(o + 12), d = 16;
      } else l === 0 && (l = a - o);
      if (!Number.isSafeInteger(l) || l < d || o + l > a) break;
      s.push({ type: r(o + 4), start: o + d, end: o + l }), o += l;
    }
    return s;
  }
  const i = n(0, t.length).find((o) => o.type === "moov");
  if (i)
    for (const o of n(i.start, i.end).filter((a) => a.type === "trak")) {
      const a = n(o.start, o.end), s = a.find((m) => m.type === "mdia"), l = s && n(s.start, s.end).find((m) => m.type === "hdlr");
      if (!l || l.start + 12 > l.end || r(l.start + 8) !== "vide") continue;
      const d = a.find((m) => m.type === "tkhd");
      if (!d) continue;
      const c = t[d.start], u = c === 0 ? 76 : c === 1 ? 88 : -1;
      if (u < 0 || d.start + u + 8 > d.end) continue;
      const p = e.getUint32(d.start + u) / 65536, f = e.getUint32(d.start + u + 4) / 65536;
      if (p > 0 && f > 0 && p <= 65535 && f <= 65535) return { width: p, height: f };
    }
}
function Vp(t) {
  const e = Bp(t);
  if (!e) throw new Error("Could not verify the Clip Edit source video. Use an MP4 or MOV with readable video dimensions. No generation was submitted.");
  if (e.width * e.height < Fp) throw new Error(`This source video is ${e.width}×${e.height}, below Topview’s Clip Edit minimum of 407,696 pixels per frame. Use a 720p or larger source video. Choosing 1080p output does not resize the input. No generation was submitted.`);
}
const Gp = [
...
function oa(t) {
  return Gp.every((e) => t.includes(e));
}
function we(t) {
  var r, n, i;
...
const si = "cinegen-canvas:";
function cr(t) {
  if (!(typeof t != "string" || !t.startsWith(si))) {
    try {
      const e = JSON.parse(atob(t.slice(si.length).replace(/-/g, "+").replace(/_/g, "/")));
...
function zp(t) {
  const e = si + btoa(JSON.stringify(t)).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  return cr(e), e;
}
function Ne(t) {
...
function Hp(t, e, r) {
  var d, c;
  const n = e.omniReferenceTaskType === "edit";
  if (n && r.filter((u) => Ne(u) === "video").length !== 1) throw new Error("Clip Edit needs exactly one source video.");
  const i = t.inputRoles ?? t.inputs ?? [], o = {};
  for (const u of r) {
    const p = `reference_${Ne(u)}`;
    if (o[p] = (o[p] ?? 0) + 1, !i.some((f) => f.role === p)) throw new Error(`Topview ${t.displayName ?? t.model} does not accept ${Ne(u)} references in this mode.`);
  }
  for (const u of i) {
    const p = o[u.role] ?? 0;
    if (p < (u.min ?? 0) || p > (u.max ?? 1 / 0)) throw new Error(`Topview ${t.displayName ?? t.model} allows ${u.min ?? 0}–${u.max} ${u.role.replace("reference_", "")} references in this mode.`);
  }
  for (const u of t.constraints ?? [])
    if (u.type === "require_any_roles" && !u.roles.some((p) => o[p]))
  ...
  const a = ((d = t.parametersSchema) == null ? void 0 : d.properties) ?? {}, s = { ...t.defaults }, l = {
    duration: n ? -1 : e.duration,
    resolution: e.resolution,
    aspectRatio: n ? "adaptive" : e.aspectRatio ?? e.ratio,
  ...
  for (const [u, p] of Object.entries(l)) {
    if (p === void 0 || p === "") continue;
    if (!Object.hasOwn(a, u)) throw new Error(`Topview Canvas does not expose the requested ${u} setting for this model.`);
    const f = a[u].enum ?? ((c = t.parameterEnums) == null ? void 0 : c[u]), m = a[u].type === "number" ? Number(String(p).replace(/p$/i, "")) : p;
    if (f && !f.includes(m)) throw new Error(`Topview Canvas does not support ${u}=${p}. Available: ${f.join(", ")}.`);
    s[u] = m;
  }
  for (const u of t.requiredParameters ?? [])
    if (s[u] === void 0) throw new Error(`Topview requires ${u} for this generation.`);
  return s;
}
async function aa(t) {
  var y, g, h;
  const { call: e, request: r, references: n } = t;
  Mp(r.prompt, t.submitSchema);
  let o = (g = (y = we(await e("list_topview_canvases", { limit: 50 })).canvases) == null ? void 0 : y.find((_) => _.name === "CineGen references")) == null ? void 0 : g.canvasId;
  ...
  const a = we(await e("get_topview_canvas_generation_capabilities", {
    ...
    include: ["schema"],
    refresh: !0
  })), s = (_) => String(_ ?? "").toLowerCase().replace(/[^a-z0-9]+/g, "-"), l = (h = a.capabilities) == null ? void 0 : h.find((_) => _.taskType === "video_edit" && [_.model, _.displayName].some((b) => s(b) === s(r.model)));
  if (!l || !a.capabilityVersion) throw new Error(`Topview Canvas does not currently expose ${r.model} for this reference mode.`);
  const d = Hp(l, r, n), u = (we(await e("get_topview_canvas_state", { canvasId: o, fields: ["nodes.geometry"] })).nodes ?? []).reduce((_, b) => {
    var v, T, S, x;
    return Math.max(
      _,
      Number(b.y ?? ((v = b.position) == null ? void 0 : v.y) ?? ((T = b.geometry) == null ? void 0 : T.y) ?? 0) + Number(b.height ?? ((S = b.measured) == null ? void 0 : S.height) ?? ((x = b.geometry) == null ? void 0 : x.height) ?? 400)
    );
  }, -80) + 80, p = [];
  for (const [_, b] of n.entries()) {
    ...
    if (r.omniReferenceTaskType === "edit" && Ne(b) === "video" && Vp(v.bytes), Ne(b) === "audio" && (!["mp3", "wav"].includes(v.format.toLowerCase()) || v.bytes.byteLength > 15 * 1024 * 1024))
    ...
    const S = Object.fromEntries(Object.entries(T.requiredHeaders ?? {}).filter((k) => typeof k[1] == "string")), x = await fetch(T.uploadUrl, {
      method: "PUT",
      headers: S,
    ...
    const E = we(await e("create_topview_canvas_media_node", {
      canvasId: o,
      mediaType: Ne(b),
      url: T.objectKey,
      mimeType: T.mimeType ?? v.mime,
      title: `${Ne(b)[0].toUpperCase()}${Ne(b).slice(1)} reference ${_ + 1}`,
      x: _ % 4 * 340,
      y: u + Math.floor(_ / 4) * 300,
      width: 300,
      height: 240
    }));
    if (!E.nodeId || E.consistencyStatus && E.consistencyStatus !== "projected") throw new Error("Topview has not finished preparing this reference. No generation was submitted.");
    p.push({ role: `reference_${Ne(b)}`, source: { kind: "canvas_node", nodeId: E.nodeId } });
  }
  const f = r.commandId ?? `cinegen-${crypto.randomUUID()}`, m = we(await e("submit_topview_canvas_generation_task", {
    ...
    parameters: d,
    inputs: p,
    layout: { x: 0, y: u + Math.ceil(n.length / 4) * 300, width: 640, height: 400 }
  }));
  ...
  return {
    taskId: zp({ canvasId: o, nodeId: m.nodeId, taskId: m.taskId }),
    status: "running",
    model: r.model,
    ...d.duration > 0 ? { durationSec: d.duration } : {},
    canvasId: o
  };
}
...
const fl = ts(import.meta.url);
function pl(t) {
  return G.isPackaged ? t.replace("app.asar", "app.asar.unpacked") : t;
}
function ve() {
  const t = fl("ffmpeg-static");
  return pl(t);
}
function Hi() {
  const t = fl("ffprobe-static").path;
  return pl(t);
}
function ml() {
  if (G.isPackaged)
    return w.join(process.resourcesPath, "vendor", "fpcalc");
  const t = w.dirname(Si(import.meta.url));
  return w.resolve(t, "..", "vendor", "fpcalc", "fpcalc");
}
const gl = Ut(qe), Xp = 20 * 1e3, Jp = 300 * 1e3;
function Kp(t) {
...
async function Qp(t) {
  try {
    if (t.filePath) return await sa(t.filePath);
    const e = await R.mkdtemp(w.join(z.tmpdir(), "cinegen-probe-"));
    try {
      const r = w.join(e, `reference.${t.format}`);
      return await R.writeFile(r, t.bytes), await sa(r);
    } finally {
      await R.rm(e, { recursive: !0, force: !0 }).catch(() => {
      });
    }
  } catch {
    return;
...
async function hl(t, e) {
  const r = await R.mkdtemp(w.join(z.tmpdir(), "cinegen-resize-"));
  try {
    const n = t.filePath ?? w.join(r, `reference.${t.format}`);
    t.filePath || await R.writeFile(n, t.bytes);
...
], am = ["16:9", "9:16", "1:1", "4:3", "3:4"], sm = ["1K", "2K", "4K"], lm = ["16:9", "9:16", "1:1", "4:3", "3:4", "21:9"], dm = ["720", "1080"], um = Array.from({ length: 27 }, (t, e) => String(e + 4));
function ke(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function li(t, e = []) {
  return Array.isArray(t) ? t.forEach((r) => li(r, e)) : ke(t) && (e.push(t), Object.values(t).forEach((r) => li(r, e))), e;
}
function cm(t) {
  for (const e of li(t))
...
function pm(t, e) {
  ...
  return (Array.isArray(n) ? n : ke(n) ? ["values", "options", "enum", "allowedValues"].map((o) => n[o]).find(Array.isArray) ?? [] : []).map(fm).filter((o) => o != null);
}
function yl(t) {
    ...
    if (!(r.outputType !== "image" && r.outputType !== "video" && r.outputType !== "audio"))
      for (const n of cm(r.config)) {
        ...
        for (const s of ["aspectRatio", "resolution", "duration", "quality", "sound"]) {
          const l = pm(n, s);
          l.length && (a.accepts.add(s), a.options.set(s, yl([...a.options.get(s) ?? [], ...l])));
        }
        e.set(o, a);
      }
  return [...e.values()];
...
function la(t, e) {
  ...
  return yl(r).map((n) => ({ value: String(n), label: String(n) }));
}
function mm(t) {
...
function xt(t, e, r) {
  ...
  return n.length ? la(n, []) : t.live && !t.accepts.has(e) ? [] : la([], r);
}
function At(t, e, r) {
  var i;
...
function gm(t) {
  const e = xt(t, "aspectRatio", am), r = xt(t, "resolution", sm), n = xt(t, "quality", []), i = t.taskTypes.size === 0 || t.taskTypes.has("text_to_image"), o = t.taskTypes.size === 0 || t.taskTypes.has("image_edit"), a = [
  ...
  return o && a.push(
    ...
    fieldType: "select",
    default: At(e, "16:9", t.defaults.aspectRatio),
    options: e
  }), r.length && a.push({
    id: "resolution",
    portType: "text",
    ...
    falParam: "resolution",
    fieldType: "select",
    default: At(r, "2K", t.defaults.resolution),
    options: r
  }), n.length && a.push({
    id: "quality",
    portType: "text",
    ...
    falParam: "quality",
    fieldType: "select",
    default: At(n, "medium", t.defaults.quality),
    options: n
  }), {
    id: `topview/image/${t.submitModel ?? t.displayName}`,
    nodeType: `topview-image-${an(t.displayName)}`,
    name: t.displayName,
...
function hm(t) {
  const e = cl(t.submitModel ?? t.displayName), r = xt(t, "aspectRatio", lm), n = xt(t, "resolution", dm), i = xt(t, "duration", um), o = t.options.get("sound") ?? [], a = t.nativeAudio === !0 || o.some((p) => String(p).toLowerCase() === "on"), s = t.taskTypes.size === 0 || t.taskTypes.has("omni_reference"), l = s && (t.audioReferences ?? zi(t)), d = t.taskTypes.size === 0 || t.taskTypes.has("image_to_video"), c = [...t.taskTypes].map((p) => p.replaceAll("_", " ")).join(" · "), u = [
    { id: "prompt", portType: "text", label: "Prompt", required: !0, falParam: "prompt", fieldType: "port" }
  ];
  return e && u.push(
    {
    ...
    { id: "source_video", portType: "video", label: "Video to edit", required: !1, falParam: "input_video", fieldType: "port", mediaRole: "video", description: "Clip Edit source. Use a 720p or larger MP4/MOV; choose Clip Edit in Video mode." }
  ), s ? (u.push(
    ...
    { id: "image_url", portType: "media", label: "References", required: !1, falParam: "reference_images", fieldType: "port", multiple: !0, mediaRole: "image", description: l ? "Image, video, and audio references. Use audio_references for audio URLs without a file extension." : "Image and video references." },
    { id: "extra_images", portType: "media", label: "More References", required: !1, falParam: "image_urls", fieldType: "element-list", max: 30, mediaRole: "image", description: l ? "Additional images, videos, audio, or Elements." : "Additional images, videos, or Elements." }
  ), l && u.push({
    id: "audio_references",
    portType: "audio",
  ...
  })) : d && u.push({ id: "image_url", portType: "image", label: "Start Frame", required: !1, falParam: "image_url", fieldType: "port", mediaRole: "start_image" }), d && s && u.push(
    ...
    falParam: "duration",
    fieldType: "select",
    default: Number(At(i, "5", t.defaults.duration)),
    options: i
  }), r.length && u.push({
    id: "aspect_ratio",
    portType: "text",
    ...
    falParam: "aspect_ratio",
    fieldType: "select",
    default: At(r, "16:9", t.defaults.aspectRatio),
    options: r
  }), n.length && u.push({
    id: "resolution",
    portType: "text",
    ...
    falParam: "resolution",
    fieldType: "select",
    default: At(n, "720", t.defaults.resolution),
    options: n
  }), a && u.push({
    id: "generate_audio",
    portType: "number",
    ...
    nodeType: `topview-video-${an(t.displayName)}`,
    name: t.displayName,
    category: "video",
    description: `Topview video generation${l ? " with image, video, and audio input references" : ""}${a ? " with native audio" : ""}${c ? ` · ${c}` : ""}`,
    outputType: "video",
    provider: "topview",
    responseMapping: { path: "url" },
    inputs: u
...
function ym(t) {
  ...
  return e === "music" ? r.push(
    ...
    nodeType: `topview-audio-${an(t.displayName)}`,
    name: t.displayName,
    category: "audio",
...
function _m() {
  return [
    ...
    ...om.map(({ displayName: t, catalogType: e }) => ({
      displayName: t,
      outputType: "audio",
      catalogType: e,
...
function lb(t) {
  const e = _l(t), n = (e.length ? e : _m()).map((i) => i.outputType === "image" ? gm(i) : i.outputType === "video" ? hm(i) : ym(i));
  return Object.fromEntries([...n, ...Lp(t == null ? void 0 : t.tools, (t == null ? void 0 : t.authMode) === "api_key")].map((i) => [i.nodeType, i]));
}
const da = -1, wm = "[-\\u2010-\\u2015\\u2212]", bm = new RegExp(
  `duration\`?\\s*(?:must|should)\\s+be\\s*\`?\\s*${wm}\\s*1\\b`,
  "i"
), vm = /task\s+as\s+video\s+editing|duration\s+follows?\s+the\s+input\s+video/i;
function wl(t) {
  return /duration/i.test(t) ? bm.test(t) || vm.test(t) : !1;
}
const Tm = "https://mcp.topview.ai/mcp", Pn = "https://mcp.topview.ai", Em = "https://www.topview.ai/mcp_oauth/oauth/authorize", Sm = "https://www.topview.ai/mcp_oauth/oauth/token", Im = "https://www.topview.ai/mcp_oauth/oauth/register", ua = "https://www.topview.ai/mcp_oauth/oauth/userinfo", xm = 300 * 1e3, On = 1200 * 1e3, Am = 90 * 1e3, km = 30 * 1e3, qt = 45 * 1024 * 1024, ca = 50;
function $(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function Nn(t) {
...
class Rm {
  constructor() {
    this.root = w.join(G.getPath("userData"), "integrations", "topview");
  }
  availabilityError() {
    try {
      return Ar.isEncryptionAvailable() ? process.platform === "linux" && Ar.getSelectedStorageBackend() === "basic_text" ? "Topview sign-in requires a Linux secret store such as GNOME Keyring or KWallet." : void 0 : "Secure credential storage is unavailable on this device. Configure the operating-system keychain, then restart CineGen.";
    } catch {
  ...
  async read(e) {
    try {
      ...
      const n = Ar.decryptString(Buffer.from(r.data, "base64")), i = JSON.parse(n);
  ...
  async write(e, r) {
    ...
    const n = JSON.stringify({
      version: 1,
      data: Ar.encryptString(JSON.stringify(r)).toString("base64")
...
function lt(t, e = [], r = 0) {
  ...
  if (Array.isArray(t))
    for (const n of t) lt(n, e, r + 1);
  else if ($(t)) {
    e.push(t);
    for (const n of Object.values(t)) lt(n, e, r + 1);
  }
  return e;
}
function Vr(t, e = [], r = 0) {
  ...
  else if (Array.isArray(t)) for (const n of t) Vr(n, e, r + 1);
  else if ($(t)) for (const n of Object.values(t)) Vr(n, e, r + 1);
  return e;
}
function ie(t) {
  const e = [t];
...
function gt(t) {
  ...
  return e && /^https?:\/\//i.test(e) ? e : Vr(t).find((r) => /^https?:\/\//i.test(r) && (/\.(?:mp4|mov|webm|png|jpe?g|webp|avif)(?:[?#]|$)/i.test(r) || /cloudfront|cdn|output|result/i.test(r)));
}
function ma(t) {
  const e = re(t, [
    "fileId",
...
function Zt(t) {
  return (re(t, ["status", "taskStatus", "task_status", "state"]) ?? "").toLowerCase();
}
function vl(t, e) {
  if (!$(e)) return t;
  ...
  return r.includes("array") && Array.isArray(t) ? t.map((n) => vl(n, e.items)) : r.includes("object") && $(t) ? Tl(e, t) : t;
}
function Tl(t, e) {
  if (!$(t)) return { ...e };
  ...
  for (const [l, d] of Object.entries(e))
    a && !Object.hasOwn(o, l) || (s[l] = vl(d, o[l]));
  return s;
}
function Nm(t, e) {
  var i;
  const r = $((i = t.inputSchema) == null ? void 0 : i.properties) ? t.inputSchema.properties : {}, n = Tl(t.inputSchema, e);
...
function Cm(t) {
  const r = (El(t, /^(?:boards|list|items|records|data|rows)$/i) ?? []).filter($).map((i) => ({
...
function qm(t) {
  return (El(t, /^models$/i) ?? []).filter($);
}
function Sl(t, e) {
  const r = t.submitParameterOptions;
  if ($(r)) {
    const n = r[e];
    if (Array.isArray(n)) return n.map(Un);
    if ($(n)) {
      for (const i of ["values", "options", "enum", "allowedValues"])
        if (Array.isArray(n[i])) return n[i].map(Un);
    }
  }
  if (Array.isArray(r)) {
...
function fr(t, e) {
  ...
  return Object.hasOwn(r, e) || Wi(t).includes(e) || Lm(t).includes(e);
}
function ui(t) {
  if (t.nativeAudio === !1 || t.supportsNativeAudio === !1) return !1;
  if (t.nativeAudio === !0 || t.supportsNativeAudio === !0) return !0;
  const e = Sl(t, "sound");
  if (e.length) return Al(e, "on") !== void 0;
  if (($(t.defaultSubmitParameters) ? t.defaultSubmitParameters : {}).sound === "on" || fr(t, "sound")) return !0;
}
function kt(t) {
...
function Il(t, e, r = !1) {
  ...
  if (i && i !== "auto") {
    const s = n.find((l) => kt(l).some((d) => d.toLowerCase() === i.toLowerCase()));
    if (!s)
  ...
  const o = re(t, ["preferredSubmitModel", "preferred_submit_model"]), a = n.find((s) => kt(s).includes(o ?? "")) ?? n.find((s) => s.preferred === !0) ?? n[0];
  if (r && ui(a) === !1)
    throw new Error(`Topview's default model "${kt(a)[0] ?? "selected"}" does not support native sound. Disable sound or explicitly choose another model.`);
  return a;
}
function Um(t) {
...
function ga(t) {
  var a;
  const e = t.references.length ? "image_edit" : "text_to_image", r = Il(t.config, t.params.model), n = (a = kt(r)[0]) == null ? void 0 : a.trim();
  ...
  for (const [s, l, d] of [
    ...
    fr(r, s) && (o[s] = ci({ model: r, field: s, requested: l, fallback: d }));
  for (const s of Wi(r))
...
const Mm = "Keep the frame free of on-screen text, captions, and subtitles.";
function xl(t) {
...
function ci(t) {
  ...
  const o = Sl(t.model, t.field);
  if (o.length) {
    const s = Al(o, i);
    if (s === void 0) {
      if (!r && !n) return o[0];
      throw new Error(`Topview model "${kt(t.model)[0] ?? "selected"}" does not allow ${t.field}=${String(i)}. Allowed values: ${o.map(String).join(", ")}.`);
    }
    return s;
  }
  const a = Dm(t.model, t.field);
  if (a.min !== void 0 || a.max !== void 0 || a.step !== void 0) {
    ...
    if (!Number.isFinite(s) || a.min !== void 0 && s < a.min || a.max !== void 0 && s > a.max || a.step !== void 0 && a.min !== void 0 && Math.abs((s - a.min) / a.step - Math.round((s - a.min) / a.step)) > 1e-9)
      throw new Error(`Topview model "${kt(t.model)[0] ?? "selected"}" does not allow ${t.field}=${String(i)}.`);
    return s;
  }
  return i;
}
...
function Mn(t) {
  var p, f;
  const e = Il(t.config, t.params.model, t.params.generateAudio === !0), r = String(e.submitModel ?? "").trim();
  if (!r) throw new Error("Topview returned a video model without a submit identifier.");
  const n = $(e.defaultSubmitParameters) ? e.defaultSubmitParameters : {}, i = new Set(Wi(e)), o = t.params.durationSec === void 0 ? void 0 : Math.round(t.params.durationSec);
  if (o !== void 0 && (!Number.isFinite(o) || o === 0 || o < -1))
    throw new Error("Topview video duration must be a positive whole number of seconds.");
  let a = xl(t.params.prompt);
  const s = {
    ...n,
    taskType: t.taskType,
    model: r,
  ...
  const l = (m, y, g) => {
    ...
    const _ = ci({ model: e, field: m, requested: y, fallback: g, required: i.has(m) });
    _ !== void 0 && (s[m] = _);
  };
  if (l("resolution", t.params.resolution === void 0 ? void 0 : Number.parseInt(t.params.resolution, 10), 720), t.inheritInputVideoDuration || o === da ? s.duration = da : fr(e, "duration") && l("duration", o, 5), l("generatingCount", void 0, 1), t.taskType !== "image_to_video" ? l("aspectRatio", (p = t.params.aspectRatio) == null ? void 0 : p.trim(), "16:9") : (t.params.aspectRatio !== void 0 || i.has("aspectRatio")) && l("aspectRatio", (f = t.params.aspectRatio) == null ? void 0 : f.trim(), "16:9"), ui(e) !== !1 && (fr(e, "sound") || t.params.generateAudio === !0))
    s.sound = ci({
      model: e,
      field: "sound",
  ...
  if (t.taskType === "omni_reference") {
    ...
    for (const T of t.references)
      if (T.role === "video") {
        const S = `Video${++y}`;
        _.push({ fileId: T.fileId, name: S }), v.push(`<<<${S}>>> is a supplied motion and timing reference.`);
      } else if (T.role === "audio") {
        const S = `Audio${++g}`;
        b.push({ fileId: T.fileId, name: S }), v.push(`<<<${S}>>> is a supplied audio reference.`);
      } else {
        const S = `Image${++m}`;
        h.push({ fileId: T.fileId, name: S });
...
${a}`, s.prompt = a, h.length && (s.inputImages = h), _.length && (s.inputVideos = _), b.length && (s.inputAudios = b);
  ...
  const c = Number(s.duration ?? n.duration), u = Number.isFinite(c) && c > 0 ? c : void 0;
  return { req: s, model: r, ...u !== void 0 ? { durationSec: u } : {} };
}
const Fm = {
  "image/bmp": "bmp",
  "image/jpeg": "jpg",
...
function Dn(t, e, r) {
  const n = (r ?? "").split(";", 1)[0].trim().toLowerCase(), i = n ? Fm[n] : void 0, o = (() => {
    try {
  ...
  if (!s || !ha(e).has(s)) {
    ...
    throw new Error(`Topview received an unsupported ${l} reference format. Supported formats: ${[...ha(e)].join(", ")}.`);
  }
  if (n && !i)
...
function Vm(t) {
  const e = t.startsWith("[") && t.endsWith("]") ? t.slice(1, -1) : t, r = Ei(e.split("%", 1)[0]);
  if (r === 4) return ya(e);
  if (r !== 6) return !1;
  const n = Bm(e);
  return n ? n.slice(0, 10).every((o) => o === 0) && n[10] === 255 && n[11] === 255 ? ya(n.slice(12).join(".")) : !(n[0] < 32 || n[0] > 63 || n[0] === 32 && n[1] === 1 && (n[2] & 254) === 0 || n[0] === 32 && n[1] === 1 && n[2] === 0 && n[3] === 2 || n[0] === 32 && n[1] === 1 && n[2] === 13 && n[3] === 184 || n[0] === 32 && n[1] === 1 && (n[2] & 240) === 16 || n[0] === 32 && n[1] === 1 && (n[2] & 240) === 32 || n[0] === 32 && n[1] === 2 || n[0] === 63 && (n[1] & 240) === 240) : !1;
}
async function Gm(t) {
  ...
  const n = Ei(e), i = n ? [{ address: e, family: n }] : await qd(e, { all: !0, verbatim: !0 });
  if (!i.length || i.some((o) => !Vm(o.address)))
...
async function jl(t, e = 0) {
  ...
  return new Promise((i, o) => {
    const a = Ld({
      ...
      method: "GET",
      servername: Ei(r.hostname.replace(/^\[|\]$/g, "")) ? void 0 : r.hostname,
      headers: {
      ...
      if (l >= 300 && l < 400) {
        ...
        jl(new URL(p, r).href, e + 1).then(i, o);
        return;
      }
      if (l < 200 || l >= 300) {
      ...
      s.on("data", (p) => {
        ...
        c.push(Buffer.from(p));
      }), s.once("error", o), s.once("end", () => i({
        bytes: Buffer.concat(c),
...
async function $n(t, e) {
  ...
  if (r.startsWith("data:")) {
    ...
    if (s.length > qt) throw new Error("This reference exceeds CineGen's 45 MB Topview upload safety limit.");
    const l = Dn("", e, a[1]);
    return { bytes: s, format: l, contentType: pr[l] };
  }
  let n;
  ...
  if (n) {
    ...
    if (a.size > qt) throw new Error("This reference exceeds CineGen's 45 MB Topview upload safety limit.");
    const s = Dn(n, e);
    return { bytes: await R.readFile(n), format: s, contentType: pr[s], filePath: n };
  }
  const i = await jl(r), o = Dn(i.finalUrl, e, i.contentType);
  return { bytes: i.bytes, format: o, contentType: pr[o] };
}
const zm = 407696, Gr = 409600;
function Rl(t) {
  const e = an(t);
  return e.includes("seedance-2-5") ? zm : e.includes("seedance-2") ? Gr : void 0;
}
function pi(t) {
  return t.toLocaleString("en-US");
}
function Hm(t) {
  const e = Rl(t.submitModel);
  if (e === void 0) return;
  const r = t.width * t.height;
  if (!(r >= e))
    return `This reference video is ${t.width}x${t.height}, which is ${pi(r)} pixels per frame. "${t.submitModel}" requires at least ${pi(e)}. Re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice), then attach it again.`;
}
function _a(t) {
  var e;
  if (/video\s+pixel\s+count/i.test(t)) {
    const r = (e = /greater\s+than\s+or\s+equal\s+to\s+(\d+)/i.exec(t)) == null ? void 0 : e[1], n = r ? Number.parseInt(r, 10) : Gr;
    return `Seedance rejected a reference video for being too small. It needs at least ${pi(n)} pixels per frame, so re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice) and attach it again. A 640x360 clip is the usual cause.`;
  }
  if (wl(t))
...
function Wm(t) {
  var r;
  const e = (r = lt(t).find((n) => $(n.headers))) == null ? void 0 : r.headers;
...
class Xm {
  ...
  async tokenExchange(e, r) {
    ...
    return n.set("client_id", r.client_id), r.client_secret && n.set("client_secret", r.client_secret), Ln(Sm, {
      method: "POST",
  ...
  async teamConnection() {
    ...
    try {
      const n = JSON.parse(await R.readFile(
        w.join(G.getPath("home"), ".topview", "credentials.json"),
        "utf8"
      ));
  ...
  async mcpRequest(e, r, n) {
    var a;
    const i = new AbortController(), o = setTimeout(() => i.abort(), Am);
    ...
    try {
      const s = await fetch(Tm, {
        ...
        signal: i.signal
      }), l = await s.text(), d = (s.headers.get("content-type") || "").includes("text/event-stream") ? Pm(l, r.id) : l ? (() => {
        try {
          return JSON.parse(l);
        } catch {
          return l;
      ...
      if (!s.ok) throw di(s.status, d, "Topview MCP request failed.");
      if ($(d) && d.error !== void 0) throw di(400, d.error, "Topview MCP returned an error.");
      return {
        payload: $(d) ? d : {},
  ...
  async session() {
    ...
    for (let l = 0; l < ca; l += 1) {
      ...
      i = d.sessionId || i;
      const c = $(d.payload.result) ? d.payload.result : {}, u = Array.isArray(c.tools) ? c.tools.filter((f) => $(f) && typeof f.name == "string") : [];
      for (const f of u) s.set(f.name, f);
      const p = c.nextCursor;
      if (typeof p != "string" || !p)
        return { token: e, sessionId: i, tools: [...s.values()] };
      if (a.has(p)) throw new Error("Topview returned a repeated MCP tools cursor.");
      a.add(p), o = p;
    }
    throw new Error(`Topview returned more than ${ca} MCP tool pages.`);
  }
  async callTool(e, r, n) {
    if (r === "topview_query_task" && cr(n.taskId))
      return Wp((s, l) => this.callTool(e, s, l), String(n.taskId));
    const i = e.tools.find((s) => s.name === r);
    if (!i) throw new Error(`Your Topview account does not currently expose ${r}.`);
    r === "topview_generate_video" && Rn(i.inputSchema, n);
    ...
    if ($(a) && a.isError === !0) {
      const s = ie(a), d = (re(s, [
        ...
        "failure_reason",
        "message"
      ]) ?? Vr(a).join(" ").slice(0, 700)).trim() || `Topview could not run ${r}.`, c = _a(d);
      throw new Error(c ? `${d}
...
${c}` : d);
  ...
  async chooseBoard(e) {
    ...
    const i = await this.callTool(e, "topview_create_board", { name: "CineGen" }), o = re(ie(i), ["boardId", "board_id", "id"]);
  ...
  async uploadReference(e, r, n) {
    ...
    let i = await $n(r.value, r.role), o;
    if (r.role === "video" && n && Rl(n) !== void 0) {
      const f = await Qp(i), m = f && Hm({ submitModel: n, ...f });
      if (f && m) {
        const y = Yp(f, Gr);
        try {
          const g = await Zp(i, y);
          if (g.length > qt)
            throw new Error("The resized reference exceeds CineGen's 45 MB Topview upload safety limit.");
          i = { bytes: g, format: "mp4", contentType: pr.mp4 }, o = `Upscaled reference video ${f.width}x${f.height} → ${y.width}x${y.height}`, console.info(
            `[topview] resized Seedance reference video ${f.width}x${f.height} -> ${y.width}x${y.height}`
          );
        } catch (g) {
          throw console.warn("[topview] could not resize undersized Seedance reference video", g), new Error(`${m}
...
CineGen could not create the temporary resized copy automatically.`);
        ...
        try {
          const y = await em(i, Gr);
          if (y.length > qt)
            throw new Error("The prepared reference exceeds CineGen's 45 MB Topview upload safety limit.");
          i = { bytes: y, format: "mp4", contentType: pr.mp4 }, o = "Prepared reference video at Seedance-compatible resolution", console.info("[topview] prepared Seedance reference video with the area-based compatibility fallback");
        } catch (y) {
    ...
    const a = await this.callTool(e, "ta_upload_credential", {
      format: i.format,
      needAccelerateUrl: !1
    }), s = ie(a), l = re(s, ["fileId", "file_id"]), d = re(s, ["uploadUrl", "upload_url", "accelerateUrl", "accelerate_url"]);
    if (!l || !d) throw new Error("Topview did not return a usable upload destination for an element.");
    const c = (re(s, ["method", "httpMethod", "http_method"]) || "PUT").toUpperCase(), u = await fetch(d, {
      method: c,
      headers: { ...Wm(s), ...i.contentType ? { "Content-Type": i.contentType } : {} },
      body: i.bytes
    });
    if (!u.ok) throw new Error(`Topview could not upload an element reference (${u.status}).`);
    const p = await this.callTool(e, "ta_upload_check_file", { fileId: l });
    if (Om(ie(p)) === !1) throw new Error("Topview could not verify an uploaded element reference.");
  ...
  async accountStatus() {
    ...
    try {
      ...
      const n = await this.accessToken(), i = await Ln(ua, {
        method: "GET",
      ...
      ].filter((d) => !o.tools.some((c) => c.name === d));
      if (s.length)
      ...
      if (o.tools.some((d) => d.name === "topview_get_credit"))
        try {
          l = pa(ie(await this.callTool(o, "topview_get_credit", {}))) ?? l;
        } catch {
        }
      return {
        connected: !0,
      ...
      return { connected: !1, configured: !0, error: Cn(r, "Topview connection expired.") };
    }
  }
  async modelCatalog() {
  ...
  async authLogin() {
    ...
    const i = `http://127.0.0.1:${n.port}/oauth/callback`, o = Nn(X.randomBytes(48)), a = Nn(X.createHash("sha256").update(o).digest()), s = Nn(X.randomBytes(32));
    let l, d;
    ...
    }), u = setTimeout(() => d == null ? void 0 : d(new Error("Topview sign-in timed out. Try connecting again.")), xm);
    (p = u.unref) == null || p.call(u);
    try {
      const f = await Ln(Im, {
        method: "POST",
      ...
      y.search = new URLSearchParams({
        ...
        resource: Pn
      }).toString(), await Kr.openExternal(y.href);
      const g = await c, h = g.searchParams.get("error_description") || g.searchParams.get("error");
      ...
      try {
        ...
        T.ok && $(S) && await this.store.write("profile", S);
      } catch {
      }
      return fa(l, !0, "You can close this window and return to CineGen."), this.accountStatus();
    } catch (f) {
      throw l && !l.writableEnded && fa(l, !1, Cn(f, "Topview sign-in did not complete.")), f;
    } finally {
      clearTimeout(u), r.close();
    }
  }
  async authLogout() {
    await Promise.all([
  ...
  async submitVideoRequest(e, r) {
    ...
    try {
      ...
      const o = Mn({ ...r, inheritInputVideoDuration: !0 });
      return {
        built: o,
  ...
  async submitWithSession(e, r) {
    var f, m, y;
    if (qp(r == null ? void 0 : r.model)) {
      const g = ul(r.model, e.tools.map((x) => x.name));
      if (g) throw new Error(g);
      const h = fi(r.medias);
      ia(r.model, r.prompt ?? "", h);
      const _ = [];
      for (const x of h) _.push(await this.uploadReference(e, x));
      const b = await this.chooseBoard(e), v = ia(r.model, r.prompt ?? "", _, b), T = ie(await this.callTool(e, "topview_avatar_video", v.request)), S = re(T, ["taskId", "task_id"]);
      if (!S) throw new Error("Topview did not return a receipt. Check Topview before starting another generation.");
      return { result: { taskId: S, taskType: v.taskType, boardId: b, model: r.model }, documents: T };
    }
    this.validateGenerateParams(r);
    const n = fi(r.medias);
    if (Dp({ ...r })) {
      const g = $p({ ...r }, n);
      Rn(void 0, g, !1, oa(e.tools.map((_) => _.name)));
      const h = await aa({
        call: (_, b) => this.callTool(e, _, b),
        request: g,
        references: n,
        submitSchema: (f = e.tools.find((_) => _.name === "submit_topview_canvas_generation_task")) == null ? void 0 : f.inputSchema,
        load: async (_) => {
          const b = await $n(_.value, _.role);
          return { bytes: b.bytes, format: b.format, mime: b.contentType };
        }
      });
      return { result: {
        taskId: h.taskId,
    ...
    const i = $m(r.medias), o = await this.chooseBoard(e), a = ie(await this.callTool(e, "topview_get_generation_config", {
      type: "video",
      taskType: i
    })), s = Mn({
      config: a,
      ...
      params: r,
      boardId: o,
      references: n.map((g, h) => ({ ...g, fileId: `preflight-${h + 1}` }))
    });
    if (Rn(
      (m = e.tools.find((g) => g.name === "topview_generate_video")) == null ? void 0 : m.inputSchema,
      s.req,
      !1,
      oa(e.tools.map((g) => g.name))
    ) === "canvas-mcp") {
      const g = await aa({
        call: (h, _) => this.callTool(e, h, _),
        references: n,
        submitSchema: (y = e.tools.find((h) => h.name === "submit_topview_canvas_generation_task")) == null ? void 0 : y.inputSchema,
        request: { ...s.req, prompt: r.prompt, ...r.generateAudio !== void 0 ? { sound: r.generateAudio } : {} },
        load: async (h) => {
          const _ = await $n(h.value, h.role);
          return { bytes: _.bytes, format: _.format, mime: _.contentType };
        }
      });
      return { result: {
        taskId: g.taskId,
    ...
    const d = [];
    for (const g of n)
      d.push(await this.uploadReference(e, g, s.model));
    const { built: c, documents: u } = await this.submitVideoRequest(e, {
      config: a,
      taskType: i,
      params: r,
      references: d,
      boardId: o
    }), p = re(u, ["taskId", "task_id", "generationId", "generation_id"]);
    ...
    return {
      result: {
        ...
        ...d.some((g) => g.preparation) ? {
          referencePreparation: d.flatMap((g) => g.preparation ?? []).join("; ")
        } : {}
      },
      documents: u
    };
  ...
  validateQueryParams(e) {
    ...
    if (!cr(e.taskId) && (typeof e.boardId != "string" || !e.boardId.trim()))
  ...
  async queryWithSession(e, r) {
    ...
    const n = await this.callTool(e, "topview_query_task", {
      ...
      needCloudFrontUrl: !0,
      shortenUrls: !1
    }), i = ie(n), o = Zt(i), a = gt(i), s = /fail|error|cancel/.test(o), l = !!a || /success|complete|done/.test(o), d = s ? "fail" : l && a ? "success" : l ? "fail" : /^(init|created|queued)$/.test(o) ? "init" : "running", c = re(i, ["boardTaskId", "board_task_id"]), u = re(i, [
      "errorMsg",
      "error_msg",
      "errorMessage",
      "error_message",
      "failureReason",
      "failure_reason"
    ]), p = u ? _a(u) : void 0, f = d !== "fail" ? void 0 : u ? p ? `${u}

${p}` : u : l ? "Topview completed the task without returning a video URL." : "Topview could not complete this video.";
    return {
      ...r,
      taskId: r.taskId.trim(),
      status: d,
      ...a ? { url: a } : {},
      ...f ? { error: f } : {},
      ...cr(r.taskId) ? {} : { boardUrl: `https://www.topview.ai/board/${encodeURIComponent(r.boardId)}${c ? `?boardResultId=${encodeURIComponent(c)}` : ""}` }
    };
  }
  async query(e) {
  ...
  async generate(e) {
    const r = await this.session(), n = await this.submitWithSession(r, e), i = gt(n.documents);
    if (i)
      return {
        url: i,
        mediaType: "video",
    ...
    const o = Zt(n.documents);
    if (/fail|error|cancel/.test(o))
      throw new Error(re(n.documents, ["errorMsg", "error_msg", "errorMessage", "error_message"]) ?? "Topview could not complete this video.");
    const a = Date.now() + On;
    for (; Date.now() < a; ) {
  ...
  async generateImage(e) {
    ...
    const r = await this.session(), n = Um(e.medias), i = n.length ? "image_edit" : "text_to_image", o = await this.chooseBoard(r), a = ie(await this.callTool(r, "topview_get_generation_config", {
      type: "image",
      taskType: i
    }));
    ga({
    ...
    const l = ga({ config: a, params: e, references: s, boardId: o }), d = await this.callTool(r, "topview_generate_image", l.req), c = ie(d), u = re(c, ["taskId", "task_id", "generationId", "generation_id"]), p = gt(c);
    if (p)
      return {
        url: p,
        mediaType: "image",
        taskId: u,
        model: l.model,
        referenceValue: ma(c) ?? await this.reusableGeneratedImageReference(r, p),
        boardUrl: `https://www.topview.ai/board/${encodeURIComponent(o)}`
      };
    if (!u) throw new Error("Topview did not return a task ID for this image generation.");
    const f = Zt(c);
    if (/fail|error|cancel/.test(f))
      throw new Error(re(c, ["errorMsg", "error_msg", "errorMessage", "error_message"]) ?? "Topview could not complete this image.");
    const m = Date.now() + On;
    for (; Date.now() < m; ) {
      ...
      const y = await this.callTool(r, "topview_query_task", {
        ...
        taskId: u,
        needCloudFrontUrl: !0
      }), g = ie(y), h = gt(g), _ = Zt(g);
      if (h) {
        const b = re(g, ["boardTaskId", "board_task_id"]);
        return {
          url: h,
          mediaType: "image",
          taskId: u,
          model: l.model,
          referenceValue: ma(g) ?? await this.reusableGeneratedImageReference(r, h),
          boardUrl: `https://www.topview.ai/board/${encodeURIComponent(o)}${b ? `?boardResultId=${encodeURIComponent(b)}` : ""}`
        };
      }
      if (/fail|error|cancel/.test(_))
        throw new Error(re(g, ["errorMsg", "error_msg", "errorMessage", "error_message"]) ?? "Topview could not complete this image.");
    }
    throw new Error(`Topview is still processing image task ${u}. Open your Topview board to check it; do not submit the same render again.`);
  }
  async generateAudio(e) {
    ...
    const d = re(l, ["taskId", "task_id", "generationId", "generation_id"]), c = gt(l);
    if (c) return { url: c, mediaType: "audio", taskId: d, model: e.model, boardUrl: `https://www.topview.ai/board/${encodeURIComponent(n)}` };
    if (!d) throw new Error("Topview did not return a task ID for this audio generation.");
    const u = Date.now() + On;
    for (; Date.now() < u; ) {
      ...
      const f = gt(l);
      if (f) return { url: f, mediaType: "audio", taskId: d, model: e.model, boardUrl: `https://www.topview.ai/board/${encodeURIComponent(n)}` };
      if (/fail|error|cancel/.test(Zt(l)))
        throw new Error(re(l, ["errorMsg", "error_msg", "errorMessage", "error_message"]) ?? "Topview could not complete this audio generation.");
    }
    throw new Error(`Topview is still processing audio task ${d}. Open your Topview board to check it; do not submit the same task again.`);
  }
}
let wa;
function Pl() {
  return wa ?? (wa = new Xm()), wa;
}
function Jm() {
  return Pl().teamConnection();
}
function Km() {
  const t = Pl();
...
const ba = {
  ...
  high: { crf: 16, scale: 1 }
}, le = /* @__PURE__ */ new Map(), mr = /* @__PURE__ */ new Map();
function Ym(t, e) {
  for (const r of Z.getAllWindows())
...
async function Zm(t, e) {
  const r = le.get(t);
  if (!r) return;
  const n = ve(), i = ba[e.preset || "standard"] || ba.standard, o = e.fps || 30, a = e.outputPath || w.join(process.cwd(), `export_${t}.mp4`);
  ...
  const c = s.map((p, f) => `[v${f}]`).join(""), u = s.map((p, f) => `[a${f}]`).join("");
  return d.push(
    `${c}${u}concat=n=${s.length}:v=1:a=1[outv][outa]`
    ...
    (g = m.stderr) == null || g.on("data", (h) => {
      ...
      if (b) {
        const v = Qm(b, e.totalDuration);
        if (v !== null) {
          const T = le.get(t);
          T && (le.set(t, { ...T, progress: v }), Ym(t, v));
        }
      }
      ...
      mr.delete(t);
      const _ = le.get(t);
      _ && le.set(t, { ..._, status: "failed", error: h.message }), f(h);
    });
  });
}
function eg() {
  I.handle("export:start", async (t, e) => {
    ...
    return le.set(i.id, i), Zm(i.id, e).catch((o) => {
...
function Fn(t) {
  const e = w.extname(t).toLowerCase();
  return tg[e] ?? "application/octet-stream";
}
function va(t) {
  try {
    const e = new URL(t);
...
function ig() {
  I.handle(
    ...
    async (t, e, r) => {
      if (!r) {
        const a = w.join(G.getPath("userData"), "media", "elements");
        await R.mkdir(a, { recursive: !0 });
        const s = w.join(a, rg(e.name));
        return await R.writeFile(s, Buffer.from(e.buffer)), { url: `local-media://file${s}` };
      }
    ...
    async (t, e, r) => {
      ...
      const i = await ng(n);
      try {
        const o = await R.readFile(i), s = `${w.basename(n, w.extname(n))}.m4a`, l = Fn(i), d = new Blob([o], { type: l }), c = new File([d], s, { type: l });
        return { url: await K.fal.storage.upload(c) };
      } finally {
    ...
    async (t, e, r) => {
      ...
      const n = va(e);
      if (n) {
        const i = await R.readFile(n), o = w.basename(n), a = Fn(n), s = new Blob([i], { type: a }), l = new File([s], o, { type: a });
      ...
      if (e.startsWith("http://") || e.startsWith("https://")) {
        ...
        const s = await R.readFile(a), l = w.basename(a), d = Fn(a), c = new Blob([s], { type: d }), u = new File([c], l, { type: d }), p = await K.fal.storage.upload(u);
...
class sg {
  constructor(e) {
    Ol(e);
    const r = w.join(Xe(e), "project.db");
    this.db = new Ti(r), this.db.pragma("journal_mode = WAL"), this.db.pragma("foreign_keys = ON"), this.initSchema();
  }
  /**
   ...
   */
  initSchema() {
    this.db.exec(og), this.db.exec(ag);
  }
  /**
...
function Cl(t) {
  ...
  if (!r) return null;
  const n = e.type === "character" || e.type === "location" || e.type === "prop" || e.type === "vehicle" ? e.type : "character", i = typeof e.folderId == "string" && e.folderId ? e.folderId : typeof e.folder_id == "string" && e.folder_id ? e.folder_id : void 0, o = ug(e.variations), a = typeof e.activeVariationId == "string" && o.some((l) => l.id === e.activeVariationId) ? e.activeVariationId : (s = o[0]) == null ? void 0 : s.id;
  return {
    id: r,
    name: typeof e.name == "string" ? e.name : "Untitled",
    type: n,
    description: typeof e.description == "string" ? e.description : "",
    voice: n === "character" ? lg(e.voice) : void 0,
    images: ql(e.images),
...
function Ll(t) {
  if (!t || typeof t != "object") return Nl();
  const e = t, r = Array.isArray(e.folders) ? e.folders.map(cg).filter((o) => o !== null) : [], n = new Set(r.map((o) => o.id)), i = Array.isArray(e.elements) ? e.elements.map(Cl).filter((o) => o !== null) : [];
  return {
    version: 1,
    folders: r,
...
function fg(t, e) {
  const r = Nl(), n = new Map(r.elements.map((o) => [o.id, o])), i = [...r.folders];
  for (const o of e) {
    const a = o.elements.map(Cl).filter((l) => l !== null);
...
function hg(t) {
  const e = w.join(Ta(t.id), "project.db"), r = w.join(Ta(t.id), "project.json");
  if (t.useSqlite || D.existsSync(e))
    try {
      const n = new Ti(e, { readonly: !0 }), i = n.prepare("SELECT * FROM elements").all();
...
async function yg() {
  ...
  let e = [];
  try {
    const i = JSON.parse(await R.readFile(mg(), "utf-8"));
    e = Array.isArray(i.projects) ? i.projects : [];
  } catch {
    e = [];
  }
  const r = e.map(hg), n = fg(null, r);
  return await mi(n), n;
}
function _g() {
  I.handle(
    ...
    async (t, e) => {
      ...
      if (e != null && e.projectId && e.projectName) {
        const n = pg(r, e.projectId, e.projectName);
        n !== r && (await mi(n), r = n);
      }
      return r;
    }
...
function bg(t, e, r) {
  const n = wg(r.persona), i = r.persona ? Ml[r.persona] : void 0, o = [];
  let a = 0;
  if (e.length === 0)
    a += t.words.length > 0 ? 3 : 1;
  else {
    const s = t.text.toLowerCase(), l = `${t.assetName} ${t.text} ${t.words.map((c) => c.word).join(" ")}`.toLowerCase();
    let d = 0;
    for (const c of e)
      l.includes(c) && (d += 1, a += s.includes(c) ? n.termInText : n.termElsewhere);
    d > 0 && o.push(`matched ${e.slice(0, 4).join(", ")}`);
  }
  return t.timelinePlacements.some((s) => s.timelineId === r.activeTimelineId) && r.activeTimelineId && (a += n.activeTimeline, o.push("already on the active timeline")), t.words.length > 0 && (a += n.wordTiming), t.emotion && (a += n.hasEmotion), t.delivery && (a += n.hasDelivery, o.push("has vocal delivery notes")), i && (Sa(t.energy, i.preferredEnergy) && (a += n.energyMatch, o.push(`${t.energy} energy fits ${r.persona}`)), Sa(t.pace, i.preferredPace) && (a += n.paceMatch, o.push(`${t.pace} pace fits ${r.persona}`)), t.emotion && i.emotionBias.some((s) => t.emotion.toLowerCase().includes(s)) && (a += n.emotionBias, o.push(`${t.emotion} emotion favored by ${r.persona}`))), t.emotion && r.queryEmotions.some((s) => t.emotion.toLowerCase().includes(s) || s.includes(t.emotion.toLowerCase())) && (a += n.emotionQueryMatch, o.push(`emotion (${t.emotion}) matches the query`)), t.notable && t.notable.length > 0 && (a += n.notableSignal * t.notable.length, o.push(`notable: ${t.notable.slice(0, 2).join("; ")}`)), { score: a, reasons: o };
}
function vg(t) {
...
function xg(t) {
  ...
  return Ig.filter((r) => e.includes(r));
}
function Ag(t, e, r = 24) {
  const n = typeof r == "number" ? { limit: r } : r, i = n.limit ?? 24, o = Sg(e), a = {
    activeTimelineId: t.activeTimelineId,
    persona: n.persona,
    queryEmotions: xg(e)
  };
  return t.moments.map((s) => ({ moment: s, ...bg(s, o, a) })).filter((s) => s.score > 0).sort((s, l) => l.score - s.score || s.moment.sourceStart - l.moment.sourceStart).slice(0, i).map(({ moment: s, score: l, reasons: d }) => ({
    id: s.id,
    assetId: s.assetId,
...
const Be = "google/gemini-2.5-flash";
function ht(t, e, r) {
...
async function sn(t, e) {
  ...
  if (e.startsWith("data:")) {
    ...
    if (!s) return null;
    const l = s[1] || "application/octet-stream", d = s[3] || "", c = s[2] ? Buffer.from(d, "base64") : Buffer.from(decodeURIComponent(d), "utf8"), u = new Blob([c], { type: l }), p = new File([u], `auto-segment.${l.split("/")[1] || "bin"}`, { type: l });
    return K.fal.config({ credentials: t }), K.fal.storage.upload(p);
  }
  const r = jg(e);
  if (!r) return null;
  const n = await R.readFile(r), i = kg(r), o = new Blob([n], { type: i }), a = new File([o], w.basename(r), { type: i });
...
function Ia(t, e) {
  const n = (Array.isArray(t.objects) ? t.objects : Array.isArray(t.detections) ? t.detections : Array.isArray(t.items) ? t.items : Array.isArray(t.regions) ? t.regions : Array.isArray(t.subjects) ? t.subjects : typeof t.label == "string" || typeof t.name == "string" || typeof t.object == "string" ? [t] : []).map((o) => {
    ...
    if (!l) return null;
    let d = null, c = null, u = null, p = null;
    const f = Array.isArray(a.box) ? a.box : Array.isArray(a.cxcywh) ? a.cxcywh : null;
    f && f.length >= 4 && (d = Q(f[0]), c = Q(f[1]), u = Q(f[2]), p = Q(f[3]));
    ...
    if ((d === null || c === null || u === null || p === null) && m && m.length >= 4) {
      const x = Q(m[0]), E = Q(m[1]), k = Q(m[2]), O = Q(m[3]);
      [x, E, k, O].every((N) => N !== null) && (d = (x + k) / 2, c = (E + O) / 2, u = k - x, p = O - E);
    }
    const y = Array.isArray(a.box_3d) ? a.box_3d : Array.isArray(a.box3d) ? a.box3d : null;
    if ((d === null || c === null || u === null || p === null) && y && y.length >= 6) {
      const x = Q(y[0]), E = Q(y[1]), k = Q(y[3]), O = Q(y[4]), N = Q(y[5]);
      [x, E, k, O, N].every((B) => B !== null) && (d = x, c = E, u = Math.max(k, O), p = Math.max(O, N));
    }
    if (d === null || c === null || u === null || p === null) {
      const x = Q(a.center_x ?? a.cx ?? a.mid_x), E = Q(a.center_y ?? a.cy ?? a.mid_y), k = Q(a.width ?? a.w), O = Q(a.height ?? a.h);
      [x, E, k, O].every((N) => N !== null) && (d = x, c = E, u = k, p = O);
    }
    if (d === null || c === null || u === null || p === null) {
      const x = Q(a.x_min ?? a.left), E = Q(a.y_min ?? a.top), k = Q(a.x_max ?? a.right), O = Q(a.y_max ?? a.bottom);
      [x, E, k, O].every((N) => N !== null) && (d = (x + k) / 2, c = (E + O) / 2, u = k - x, p = O - E);
    }
    if ([d, c, u, p].some((x) => x === null || !Number.isFinite(x))) return null;
    const g = ht(u, 0.02, 1), h = ht(p, 0.02, 1), _ = [
      ht(d, g / 2, 1 - g / 2),
      ht(c, h / 2, 1 - h / 2),
      g,
      h
    ], b = Q(a.score ?? a.confidence ?? a.probability), v = b !== null ? ht(b, 0, 1) : 0.75, T = Q(a.priority ?? a.salience ?? a.importance), S = T !== null ? ht(T, 0, 1) : v;
    return {
      label: l,
      box: _,
      score: v,
  ...
  for (const o of n)
    if (i.some((s) => {
      const l = s.label.toLowerCase() === o.label.toLowerCase(), d = Math.abs(s.box[0] - o.box[0]), c = Math.abs(s.box[1] - o.box[1]), u = Math.abs(s.box[2] - o.box[2]), p = Math.abs(s.box[3] - o.box[3]);
      return l && d < 0.06 && c < 0.06 && u < 0.08 && p < 0.08;
...
async function $l(t) {
  ...
  const e = (await Promise.all(
    t.framePaths.slice(0, 6).map((u) => sn(t.apiKey, u).catch(() => null))
  )).filter((u) => !!u);
  if (e.length === 0)
    return {
      assetId: t.assetId,
      status: "missing",
      model: ((a = t.model) == null ? void 0 : a.trim()) || Be,
  ...
  const n = (await K.fal.subscribe("fal-ai/any-llm/vision", {
    input: {
      model: ((s = t.model) == null ? void 0 : s.trim()) || Be,
      prompt: [
...
`),
    ...
    logs: !0
  })).data, i = ye(n.output) || ye(n.text) || "", o = Dl(i);
  if (!o)
    return {
      assetId: t.assetId,
      status: "failed",
      model: ((l = t.model) == null ? void 0 : l.trim()) || Be,
  ...
  try {
    ...
    return {
      assetId: t.assetId,
      status: "ready",
      summary: typeof u.summary == "string" ? u.summary.trim() : void 0,
      tone: Array.isArray(u.tone) ? u.tone.filter((p) => typeof p == "string") : void 0,
      pacing: typeof u.pacing == "string" ? u.pacing.trim() : void 0,
      shotTypes: Array.isArray(u.shotTypes) ? u.shotTypes.filter((p) => typeof p == "string") : void 0,
      subjects: Array.isArray(u.subjects) ? u.subjects.filter((p) => typeof p == "string") : void 0,
      brollIdeas: Array.isArray(u.brollIdeas) ? u.brollIdeas.filter((p) => typeof p == "string") : void 0,
      confidence: typeof u.confidence == "number" && Number.isFinite(u.confidence) ? u.confidence : void 0,
      updatedAt: (/* @__PURE__ */ new Date()).toISOString(),
      model: ((d = t.model) == null ? void 0 : d.trim()) || Be,
    ...
    return {
      assetId: t.assetId,
      status: "failed",
      model: ((c = t.model) == null ? void 0 : c.trim()) || Be,
...
async function Fl(t) {
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await Rg(t.apiKey, t.videoPath).catch(() => null);
  if (!e)
...
async function Pg(t) {
  var o;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await sn(t.apiKey, t.imagePath).catch(() => null);
  if (!e)
    throw new Error("Could not upload the image file for analysis.");
  K.fal.config({ credentials: t.apiKey });
  const n = (await K.fal.subscribe("fal-ai/any-llm/vision", {
    input: {
      model: ((o = t.model) == null ? void 0 : o.trim()) || Be,
...
async function Og(t) {
  var a, s;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = Math.min(12, Math.max(1, Math.round(t.maxObjects ?? 6))), r = await sn(t.apiKey, t.imagePath).catch(() => null);
  if (!r)
    return {
      status: "missing",
      model: ((a = t.model) == null ? void 0 : a.trim()) || Be,
  ...
  const n = ((s = t.model) == null ? void 0 : s.trim()) || Be, i = [
...
`);
  try {
    const l = await xa(t.apiKey, r, n, e, i), d = l ? Ia(l, e) : [];
    if (d.length > 0)
    ...
    const c = await xa(t.apiKey, r, n, e, o), u = c ? Ia(c, e) : [];
    return u.length > 0 ? (console.info("[vision:auto-seg] Retry object proposals", {
      model: n,
      count: u.length,
      objects: u,
      context: t.context ?? null
      ...
      retryKeys: c ? Object.keys(c).slice(0, 12) : [],
      primaryPreview: l ? JSON.stringify(l).slice(0, 1e3) : "",
      retryPreview: c ? JSON.stringify(c).slice(0, 1e3) : "",
      context: t.context ?? null
    }), {
      status: "ready",
      model: n,
...
function Ng() {
  I.handle("vision:index-asset", async (t, e) => $l(e)), I.handle("vision:detect-objects", async (t, e) => Og(e));
}
const Cg = "anthropic/claude-sonnet-4.6";
function oe(t) {
  const e = Number(t);
...
async function Lt(t) {
  ...
  K.fal.config({ credentials: t.apiKey });
  const e = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || Cg,
    prompt: t.prompt,
...
function Fg(t, e) {
  ...
  return {
    brief: {
      ...
      variantCount: Mg(r.variantCount, e.variantCount),
      persona: Ug(r.persona),
    ...
    clarifyingQuestions: $g(r.clarifyingQuestions)
  };
}
function Bg(t, e, r) {
  const n = { ...t, ...e ?? {} };
...
function Vg(t) {
  if (!t || typeof t != "object") return null;
  const e = t, r = Aa(e.source_start), n = Aa(e.source_end);
...
function Gg(t, e) {
  if (!t || typeof t != "object") return null;
  const r = t, n = Array.isArray(r.segments) ? r.segments.map(Vg).filter((i) => !!i) : [];
...
function zg(t) {
  ...
  return Array.isArray(e.variants) ? e.variants.flatMap((r, n) => {
    var a;
    if (!r || typeof r != "object") return [];
    const i = r, o = Array.isArray(i.proposals) ? i.proposals.map((s) => Gg(s, `AI Cut ${n + 1}`)).filter((s) => !!s) : [];
    return o.length === 0 ? [] : [{
...
function Hg(t, e) {
  ...
  for (const s of n) {
    ...
    d && i.set(d, {
      ...
      strengths: Array.isArray(l.strengths) ? l.strengths.filter((c) => typeof c == "string") : [],
      cautions: Array.isArray(l.cautions) ? l.cautions.filter((c) => typeof c == "string") : [],
  ...
  return a.sort((s, l) => {
    const d = o.indexOf(s.id), c = o.indexOf(l.id);
    return d === -1 && c === -1 ? l.scorecard.overall - s.scorecard.overall : d === -1 ? 1 : c === -1 ? -1 : d - c;
  }), a;
}
function Wg(t) {
...
async function Kg(t) {
  var o;
  const e = Dg(t.request, t.index), r = [
...
`),
  ...
  try {
    ...
    return { ...Fg(a, e), usage: n.usage };
  } catch {
    return { brief: e, clarifyingQuestions: [], usage: n.usage };
  }
}
async function ka(t, e, r, n, i = {}) {
  const o = [e, r.storyGoal, r.hook, r.tone, r.audience].join(" ");
  let a = Ag(t, o, { limit: 20, persona: r.persona });
  if (i.rerank && i.apiKey && a.length > 1)
    try {
      const l = vg({ query: o, brief: r, candidates: a }), d = await Lt({
        apiKey: i.apiKey,
        model: i.model,
...
async function Yg(t) {
  ...
  const e = (c, u) => {
    ...
    if (!p) return null;
    try {
      const f = JSON.parse(p), y = zg({ variants: [f] })[0];
      return y ? {
        variant: y,
        usage: u
      } : null;
    ...
    const p = [
      `Repair this malformed cut-variant response into valid JSON for variant ${u + 1}.`,
      "Return JSON only with this shape:",
...
`), f = await Lt({
  ...
  for (let c = 0; c < a.length; c += 1) {
    const p = [
      ...
      `Generate exactly one editorial variant using this strategy: ${a[c]}`,
...
${JSON.stringify(l.map((g) => ({ title: g.title, strategy: g.strategy, summary: g.summary })), null, 2)}` : "",
      ...
      "",
      "Retrieved moments:",
      Bl(t.retrievalSummary.topMoments),
      "",
      "Reference timelines:",
...
`),
    ...
    const y = await r(f.message, c);
    s = hr(s, y.usage), y.variant && l.push({
      ...y.variant,
      id: `variant_${c + 1}`
    });
  }
...
async function Qg(t) {
  ...
  const e = [
    ...
    Bl(t.retrievalSummary.topMoments.slice(0, 10)),
    "",
    "Variants:",
...
async function Zg(t) {
  ...
  n = hr(n, i.usage);
  const o = Bg(i.brief, t.briefOverride, t.questionAnswers), a = await ka(e, r, o, []);
  if (!t.confirmedBrief)
    return {
      stage: "brief",
  ...
  if (n = hr(n, d.usage), d.variants.length === 0)
    return {
      stage: "brief",
...
const Vl = "http://127.0.0.1:11434";
function eh() {
  return Z.getAllWindows().find((t) => !t.isDestroyed());
}
...
`)) >= 0; ) {
      ...
      if (m = m.slice(b + 1), !!v)
        try {
          const T = JSON.parse(v), S = T.message, x = typeof (S == null ? void 0 : S.content) == "string" ? S.content : "";
          if (x)
            for (const E of x)
              c ? (u += E, u.endsWith("</think>") && (c = !1, u = "")) : (u += E, u === "<think>" ? (c = !0, u = "") : "<think>".startsWith(u) || (s += u, a == null || a.webContents.send("llm:local-stream", { requestId: t, token: u }), u = ""));
          T.done && (l = oe(T.prompt_eval_count) ?? 0, d = oe(T.eval_count) ?? 0);
        } catch {
        }
    }
  }
  return u && !c && (s += u, a == null || a.webContents.send("llm:local-stream", { requestId: t, token: u })), a == null || a.webContents.send("llm:local-stream", { requestId: t, done: !0 }), {
    message: s.trim(),
...
async function rh() {
  try {
    const t = await fetch(`${Vl}/api/tags`);
...
function nh() {
  I.handle("llm:chat", async (t, e) => {
    ...
    const n = Array.isArray(e.messages) ? e.messages : [], i = Lg(n);
    ...
    const r = e.requestId || crypto.randomUUID(), n = await th(r, e);
    return {
      message: n.message,
      ...n.usage ? { usage: n.usage } : {}
    };
  }), I.handle("llm:local-models", async () => rh()), I.handle("llm:run-cut-workflow", async (t, e) => Zg(e));
}
const ih = 18, ja = 24 * 1024 * 1024;
function Gl(t = []) {
  if (t.length > ih) throw new Error("Too many canvas previews attached. Select fewer nodes.");
  let e = 0;
  return t.map((r) => {
    if (typeof r.dataUrl != "string" || r.dataUrl.length > ja * 4 / 3 + 100) throw new Error("Canvas preview is too large.");
    const n = /^data:(image\/(?:jpeg|png|webp));base64,([A-Za-z0-9+/]+={0,2})$/.exec(r.dataUrl);
    if (!n) throw new Error("Canvas preview must be a JPEG, PNG, or WebP image.");
    const i = Buffer.from(n[2], "base64");
    if (e += i.length, e > ja) throw new Error("Canvas previews are too large. Select fewer nodes.");
    const o = n[1];
    if (!(o === "image/jpeg" ? i.subarray(0, 3).equals(Buffer.from([255, 216, 255])) : o === "image/png" ? i.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])) : i.toString("ascii", 0, 4) === "RIFF" && i.toString("ascii", 8, 12) === "WEBP")) throw new Error("Canvas preview contains invalid image data.");
    return { label: String(r.label).slice(0, 1e3), mime: o, base64: n[2], bytes: i };
  });
}
async function zl(t = []) {
  const e = Gl(t);
  if (!e.length) return { refs: [], cleanup: async () => {
  } };
  const r = await Nd(w.join(z.tmpdir(), "cinegen-assistant-")), n = () => Cd(r, { recursive: !0, force: !0 });
  try {
    const i = [];
    for (const [o, a] of e.entries()) {
      const s = w.join(r, `${o + 1}.${a.mime.split("/")[1]}`);
      await Za(s, a.bytes, { mode: 384 }), i.push({ label: a.label, kind: "asset", mediaType: "image", mediaPath: s, ephemeral: !1 });
    }
    return { refs: i, cleanup: n };
  } catch (i) {
    throw await n(), i;
  }
}
function oh(t, e) {
  return JSON.stringify({ type: "user", message: { role: "user", content: [
    { type: "text", text: t },
    ...Gl(e).flatMap((r, n) => [
      { type: "text", text: `Attached image ${n + 1}: ${r.label}` },
...
async function Wl() {
  if (Nr !== void 0) return Nr;
  for (const t of ah)
    try {
      const { stdout: e } = await Hl(t, ["--version"], {
        env: Ji(),
        timeout: 8e3
      });
...
function gh() {
  return w.join(G.getPath("userData"), "claude-code-workspace");
}
function hh(t) {
...
async function _h(t, e) {
  ...
  const n = ((_ = e.model) == null ? void 0 : _.trim()) || "sonnet", i = !!e.resumeSessionId && !e.injectProjectContext && !((b = e.images) != null && b.length), o = hh(e), a = i ? e.userMessage.trim() : yh(e, o), s = (v = e.images) != null && v.length ? oh(a, e.images) : void 0, l = [
    "-p",
    ...s ? ["--input-format", "stream-json"] : [a],
    "--output-format",
    "stream-json",
    "--verbose",
    "--max-turns",
  ...
  if (o ? (l.push("--safe-mode", "--effort", "low", "--include-partial-messages"), !i && ((T = e.systemPrompt) != null && T.trim()) && l.push("--system-prompt", e.systemPrompt.trim())) : l.push("--include-partial-messages"), i && e.resumeSessionId) {
    if (l.push("--resume", e.resumeSessionId), !o) {
      const E = [(S = e.systemPrompt) == null ? void 0 : S.trim(), lh].filter(Boolean).join(`

`);
      l.push("--append-system-prompt", E);
    }
  } else if (!o && e.injectProjectContext && ((x = e.systemPrompt) != null && x.trim())) {
...
` : "", k = e.purpose === "enhance-prompt" ? dh : sh;
    l.push("--append-system-prompt", `${E}${e.systemPrompt.trim()}

${k}`);
  }
  const d = fh(), c = o ? gh() : void 0;
  c && await st(c, { recursive: !0 });
  let u = "", p = "", f, m = !1, y = !1, g, h;
  return new Promise((E, k) => {
    var W, J, j;
    const O = ae(r, l, {
      env: Ji(),
      ...c ? { cwd: c } : {},
      stdio: [s ? "pipe" : "ignore", "pipe", "pipe"]
    });
    s && ((W = O.stdin) == null || W.end(s)), nt = { child: O, requestId: t };
    let N = "", B = !1;
    const H = (L) => {
      B || (B = !0, nt = null, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, done: !0 }), L(), O.killed || O.kill());
    };
    (J = O.stdout) == null || J.on("data", (L) => {
      N += L.toString();
      let U;
      for (; (U = N.indexOf(`
`)) >= 0; ) {
        const P = N.slice(0, U).trim();
        if (N = N.slice(U + 1), !!P)
          try {
            const A = JSON.parse(P);
            if (A.type === "system" && A.subtype === "init" && typeof A.session_id == "string" && (f = A.session_id), A.type === "assistant" && A.error === "authentication_failed" && (m = !0), A.type === "result") {
              h = A;
              const V = Pa(A);
              V && !u.trim() && (u = V, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: V }));
              const Y = u.trim();
              if (Y && !m && !Y.includes("Not logged in")) {
                H(() => E({ message: Y, sessionId: f, usage: g, resumed: i }));
                return;
              }
            }
            const q = Ra(A);
            ...
            if (A.type === "stream_event") {
              y = !0, u += C, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: C });
              continue;
            }
            A.type === "assistant" && !y ? (u = C, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: C })) : A.type === "result" && !u.trim() && (u = C, d == null || d.webContents.send("llm:claude-code-stream", { requestId: t, token: C }));
          } catch {
          }
      }
    }), (j = O.stderr) == null || j.on("data", (L) => {
      p += L.toString();
    }), O.on("error", (L) => {
      H(() => k(L));
    }), O.on("close", (L) => {
      H(() => {
        const U = u.trim();
        if (m || U.includes("Not logged in")) {
          k(new Error("Claude Code is not logged in. Open Terminal, run `claude`, and sign in with your subscription."));
          return;
        }
        if (U) {
          E({ message: U, sessionId: f, usage: g, resumed: i });
          return;
        }
        k(new Error(mh(L, p, h)));
      });
    });
  });
}
function wh() {
  I.handle("llm:claude-code-detect", async () => {
    ...
    const r = e.requestId || X.randomUUID(), n = await _h(r, e);
    return {
      message: n.message,
...
async function ln(t) {
  if (Vn.has(t))
    return Vn.get(t) ?? null;
  for (const e of bh[t])
    try {
      const { stdout: r } = await Ki(e, ["--version"], {
        env: Ir(),
        timeout: 8e3
      });
      if (r.trim())
...
async function vh(t, e) {
  ...
  let n = "";
  try {
    const i = await Ki(e, r, { env: Ir(), timeout: 8e3 });
    n = `${i.stdout}
${i.stderr}`;
  } catch (i) {
    const o = i;
...
function Sh() {
  I.handle("llm:cli-detect", async () => ({ providers: await Th() }));
}
function Ih(t) {
  var r;
...
`).trim();
  return i ? Ih(i) : `Codex exited with code ${e ?? "unknown"}`;
}
function Ah(t) {
...
let se = null;
function kh() {
  return w.join(G.getPath("userData"), "codex-workspace");
}
function jh(t) {
  return t.purpose === "json-job" ? !0 : t.purpose === "copilot" || t.purpose === "enhance-prompt" ? !1 : !t.injectProjectContext && !t.resumeSessionId && !(t.messages && t.messages.length > 0);
}
...
` : "", l = t.purpose === "enhance-prompt" ? Yl : Kl;
      r.push(`${s}${t.systemPrompt.trim()}

${l}`);
    } else
      r.push(t.systemPrompt.trim());
  const n = (t.messages ?? []).filter((s) => s.content.trim()), i = n.length > 0 ? hi(n) : `${t.userMessage.trim()}
...
async function Nh(t, e) {
  var _;
  const r = await ln("codex");
  if (!r)
    throw new Error("Codex CLI is not installed. Install it from https://developers.openai.com/codex");
  if (!e.userMessage.trim())
    throw new Error("No chat message provided.");
  const n = Ah(e.model), i = !!e.resumeSessionId && !e.injectProjectContext && !((_ = e.images) != null && _.length), o = jh(e), a = i ? e.userMessage.trim() : Rh(e, o), s = kh();
  await st(s, { recursive: !0 });
  const l = [
    "exec",
    "--json",
    "-s",
  ...
  n && l.push("-m", n);
  const d = await zl(e.images);
  for (const b of d.refs) l.push("--image", b.mediaPath);
  d.refs.length && !o && l.push("--"), i && e.resumeSessionId && l.push("resume", e.resumeSessionId), o || l.push(a);
  const c = Jl();
  let u = "", p = "", f = "", m = !1, y, g, h = "";
  try {
    return await new Promise((b, v) => {
      ...
      o && ((x = T.stdin) == null || x.write(a), (E = T.stdin) == null || E.end()), se = { child: T, requestId: t, provider: "codex" };
      let S = "";
      (k = T.stdout) == null || k.on("data", (N) => {
        S += N.toString();
        let B;
        for (; (B = S.indexOf(`
`)) >= 0; ) {
          const H = S.slice(0, B).trim();
          if (S = S.slice(B + 1), !!H)
            try {
              const W = JSON.parse(H);
              W.type === "thread.started" && typeof W.thread_id == "string" && (y = W.thread_id);
              const J = Ph(W);
              if (J && (g = J), W.type === "turn.failed") {
                m = !0;
                const L = W.error;
                f = (L == null ? void 0 : L.message) ?? "Codex turn failed.";
              } else W.type === "error" && typeof W.message == "string" && (f = W.message);
              const j = Oh(W);
              if (j) {
                const L = j.startsWith(h) ? j.slice(h.length) : j;
                h = j, u = j, L && (c == null || c.webContents.send("llm:codex-stream", { requestId: t, token: L }));
              }
            } catch {
            }
        }
      }), (O = T.stderr) == null || O.on("data", (N) => {
        p += N.toString();
      }), T.on("error", (N) => {
        (se == null ? void 0 : se.requestId) === t && (se = null), v(N);
      }), T.on("close", (N) => {
        (se == null ? void 0 : se.requestId) === t && (se = null), c == null || c.webContents.send("llm:codex-stream", { requestId: t, done: !0 });
        const B = u.trim();
        if (!B || N !== 0 || m) {
          v(new Error(xh(f || p, N)));
          return;
        }
        b({ message: B, sessionId: y, usage: g, resumed: i });
      });
    });
  } finally {
    await d.cleanup();
...
function Ch() {
  I.handle("llm:codex-chat", async (t, e) => {
    const r = e.requestId || X.randomUUID(), n = await Nh(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
...
function Lh(t) {
  ...
  const r = e, n = r.prompt_tokens_details && typeof r.prompt_tokens_details == "object" && !Array.isArray(r.prompt_tokens_details) ? r.prompt_tokens_details : {}, i = er(r.prompt_tokens ?? r.input_tokens), o = er(r.completion_tokens ?? r.output_tokens), a = er(n.cached_tokens), s = er(n.cache_write_tokens), l = er(r.total_tokens) || i + o;
  if (!(i <= 0 && o <= 0 && l <= 0))
    return { promptTokens: i, completionTokens: o, totalTokens: l, cachedTokens: a, cacheWriteTokens: s };
}
function Uh(t) {
  const e = t.promptTokens > qh ? Oa.long : Oa.short, r = Math.min(t.cachedTokens, t.promptTokens), n = Math.min(t.cacheWriteTokens, Math.max(0, t.promptTokens - r)), o = (Math.max(0, t.promptTokens - r - n) * e.input + r * e.cached + n * e.cacheWrite + t.completionTokens * e.output) / 1e6;
  return { ...t, cost: Math.round(o * 1e8) / 1e8 };
}
const Mh = "https://api.openai.com/v1/chat/completions", Dh = "gpt-5.6-luna", $h = 6e4;
function Fh(t, e = []) {
...
function Bh(t) {
  ...
  r && e.push({ role: "system", content: r }), e.push({ role: "user", content: Fh(t.userMessage, t.imageUrls) });
  const n = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || Dh,
    messages: e,
    reasoning_effort: t.reasoningEffort ?? "low",
    max_completion_tokens: Number.isFinite(t.maxCompletionTokens) ? Math.max(1, Math.floor(t.maxCompletionTokens)) : $h
  };
...
async function zh(t) {
  ...
  if (!i.ok)
    throw new Error(Vh(a, `OpenAI request failed (${i.status}).`));
  const s = Lh(a);
  return {
    message: Gh(a),
    ...s ? { usage: Uh(s) } : {}
...
const Wh = "http://localhost:3000", Ql = "https://cinegen-film.vercel.app", Yi = "persist:cinegen-team-workspace", Xh = 8e3, yi = "__CINEGEN_TEAM_PROVIDER__", ze = { origin: Ql, source: "hosted" }, zn = { origin: Wh, source: "local-web" };
let Ce = null, yt = null, tr = null;
function zr() {
  return {
    supported: !0,
    scope: "workspace",
...
async function dn(t, e, r, n) {
  const i = new AbortController(), o = setTimeout(() => i.abort(), Xh);
  try {
    const a = `${t.origin}/api/rpc/${encodeURIComponent(e)}/${encodeURIComponent(r)}`, s = {
      ...
      redirect: "error",
      signal: i.signal
    }, l = t.source === "hosted" ? await Ja.fromPartition(Yi).fetch(a, s) : await fetch(a, s);
    return { response: l, payload: await Jh(l) };
  } finally {
    clearTimeout(o);
  }
}
...
async function yr(t) {
  try {
    const { response: e, payload: r } = await dn(t, "providers", "status", []);
    return !e.ok || !(r != null && r.ok) || !r.result ? null : Zl(r.result, t);
  } catch {
    return null;
  }
}
async function un() {
  if (Ce) {
    const r = await yr(Ce);
    if (r) return { target: Ce, status: r };
    Ce = null;
  }
  const t = await yr(ze);
  if (t)
    return Ce = ze, { target: ze, status: t };
  const e = await yr(zn);
  return e ? (Ce = zn, { target: zn, status: e }) : null;
}
async function Kh(t, e, r) {
  var a;
  const n = await un();
  if (!n)
    throw new Error("Connect CineGen Desktop to the hosted team workspace in Settings first.");
  const { response: i, payload: o } = await dn(n.target, t, e, r);
  if (!i.ok || !(o != null && o.ok))
...
async function Na(t, e) {
  ...
  if (!r)
    throw new Error("Connect CineGen Desktop to the hosted team workspace in Settings first.");
  const { response: n, payload: i } = await dn(r.target, "providers", t, [e]);
...
async function Yh() {
  ...
  if (!e || e.target.source !== "hosted") {
    const a = await ed();
    (i = a.desktop) != null && i.connected && a.desktop.source === "hosted" && (e = { target: ze, status: a });
  }
  if (!e || e.target.source !== "hosted")
  ...
  const { response: r, payload: n } = await dn(e.target, "topview", "importTeamConnection", [t]);
...
async function Qh(t) {
  return Kh("llm", "openaiChat", [{ ...t, apiKey: yi }]);
}
async function ed() {
  const t = await yr(ze);
  if (t)
    return Ce = ze, t;
  if (tr)
    return yt && !yt.isDestroyed() && yt.focus(), tr;
  const e = new Z({
    width: 720,
    height: 820,
    minWidth: 520,
  ...
  yt = e;
  const r = Ql, n = new Promise((i, o) => {
    let a = !1, s = !1, l = null, d = null;
    const c = () => {
      l && clearInterval(l), d && clearTimeout(d), l = null, d = null;
    }, u = (m) => {
      a || (a = !0, c(), i(m));
    }, p = (m) => {
      a || (a = !0, c(), o(m));
      ...
      if (!(s || a)) {
        ...
        try {
          const m = await yr(ze);
          if (a || !m) return;
          Ce = ze, u(m), e.isDestroyed() || e.close();
        } finally {
          s = !1;
        }
      }
    ...
    e.webContents.on("did-finish-load", () => void f()), e.on("closed", () => {
      yt === e && (yt = null), l && (clearInterval(l), l = null), !a && (async () => {
        for (; s && !a; )
          await new Promise((m) => setTimeout(m, 50));
        a || (await f(), a || u(zr()));
      })();
    }), l = setInterval(() => void f(), 1e3), d = setTimeout(() => {
      u(zr()), e.isDestroyed() || e.close();
...
function Zh() {
  I.handle("team-providers:status", async () => {
    const t = await un();
    return (t == null ? void 0 : t.status) ?? zr();
  }), I.handle("team-providers:connect", () => ed()), I.handle("team-providers:disconnect", async () => (await Ja.fromPartition(Yi).clearStorageData({ storages: ["cookies", "indexdb", "localstorage"] }), Ce = null, zr())), I.handle("team-providers:save", async (t, e) => Na("save", e)), I.handle("team-providers:remove", async (t, e) => Na("remove", e)), I.handle("team-providers:share-topview", () => Yh());
}
function ey(t) {
  const e = w.extname(t).toLowerCase();
...
function ty(t) {
  ...
  const r = Hh(e) ?? (e.startsWith("/") || /^[A-Za-z]:[\\/]/.test(e) ? e : null);
  if (!r || !D.existsSync(r)) return null;
  const n = D.readFileSync(r);
  return `data:${ey(r)};base64,${n.toString("base64")}`;
}
function ry(t) {
  return Array.isArray(t) ? t.flatMap((e) => {
...
function ny() {
  I.handle("llm:openai-chat", async (t, e) => {
    const r = e && typeof e == "object" && !Array.isArray(e) ? e : {}, n = typeof r.apiKey == "string" ? r.apiKey : "", i = typeof r.userMessage == "string" ? r.userMessage : "", o = ry(r.imageUrls);
    return n === yi ? Qh({
      ...r,
      apiKey: yi,
      userMessage: i,
    ...
    if (!d.ok) {
      ...
      try {
        ...
        (u = f.error) != null && u.message && (p = f.error.message);
      } catch {
      }
      throw new Error(p);
    }
...
const iy = /* @__PURE__ */ new Set(["authCancel", "authLogin", "authStatus", "accountStatus", "connect", "disconnect", "voices", "generate", "job", "design", "saveVoice", "enhance"]);
function oy() {
  I.handle("elevenlabs:request", async (t, e, r, n) => {
    var a;
    if (!iy.has(e) || typeof n != "string" || !n || n.length > 12e3) throw new Error("Invalid ElevenLabs request.");
...
async function rd(t, e, r, n) {
  const i = ve(), o = Math.max(0, e), a = Math.max(0.1, Math.min(r, sy));
  try {
    return await td(i, [
      "-y",
      "-ss",
...
function Hn(t, e, r) {
  ...
  const n = w.extname(t) || (e.mediaType === "image" ? ".jpg" : ".mp4"), i = w.join(r, `${_i(e)}-source${n}`), o = id(t, i);
...
async function Qi(t, e) {
  ...
  for (const i of t) {
    ...
    if (i.trimStartSec !== void 0 && i.trimDurationSec !== void 0) {
      const c = w.join(r, `${_i(i)}.mp4`), u = await rd(
        o,
        i.trimStartSec,
        i.trimDurationSec,
        c
    ...
    const s = (i.framePaths ?? []).map((c) => jt(c)).find(Boolean);
    if (s) {
      const c = Hn(s, {
        ...i,
        mediaType: "image",
    ...
    const l = w.join(r, `${_i(i)}.jpg`), d = await Mr(o, i.trimStartSec ?? 0, l);
    d && n.push({
      label: i.label,
      kind: i.kind,
      mediaType: "image",
...
function ad(t) {
  ...
  if (e.startsWith("file://"))
    try {
      return jt(decodeURIComponent(new URL(e).pathname));
    } catch {
      return null;
    }
  return jt(e);
...
async function ly(t, e) {
  ...
  if (e.mode === "first-last") {
    const c = w.join(n, `${i}-first.jpg`), u = w.join(n, `${i}-last.jpg`), p = await Mr(r, o, c), f = await Mr(r, Math.max(o, a - 0.05), u), m = [], y = [];
  ...
  if (e.mode === "segment") {
    const c = w.join(n, `${i}-segment.mp4`), u = Math.max(0.1, a > o ? a - o : e.maxSegmentSec ?? 30), p = await rd(r, o, Math.min(u, e.maxSegmentSec ?? 30), c);
    if (!p) throw new Error("Failed to extract clip segment");
    return { paths: [p], roles: ["image"] };
  }
  const s = e.frameTimeSec ?? (a > o ? (o + a) / 2 : o), l = w.join(n, `${i}-frame.jpg`), d = await Mr(r, s, l);
...
const uy = 18e4, cy = 600 * 1e3;
async function sd(t) {
  var c;
  const e = await ln("gemini");
  if (!e)
    throw new Hr("Gemini CLI is not installed.");
  const r = jt(t.mediaPath);
  if (!r)
  ...
  let i = r, o = !1;
  if (nd(r)) {
    const u = w.extname(r) || ".mp4", p = w.join(n, `${X.randomUUID()}${u}`), f = id(r, p);
    if (!f)
      throw new Error("Could not stage the media file for Gemini analysis.");
    i = f, o = !0;
  }
  const a = ((c = t.model) == null ? void 0 : c.trim()) || "gemini-2.5-flash", l = [
    "--skip-trust",
    "-p",
    `@${i} ${t.prompt.trim()}`,
    "-o",
  ...
  return new Promise((u, p) => {
    ...
    let m = "", y = "", g = "", h = !1, _ = !1;
    const b = (E) => {
      h || (h = !0, clearTimeout(v), clearTimeout(T), d(), E());
    }, v = setTimeout(() => {
    ...
    (S = f.stdout) == null || S.on("data", (E) => {
      g += E.toString();
      let k;
      for (; (k = g.indexOf(`
`)) >= 0; ) {
        const O = g.slice(0, k).trim();
        if (g = g.slice(k + 1), !!O)
          try {
      ...
      const k = m.trim();
      if (!k) {
        const O = gi(y.trim()) || `Gemini CLI exited with code ${E ?? "unknown"}`;
        b(() => p(new Error(O)));
        return;
      }
      if (dy(k)) {
...
const Ca = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  analyzeMediaWithGeminiCli: sd,
  buildGeminiUserMessageWithVisualRefs: od,
  cleanupEphemeralVisualRefs: Zi,
  prepareClipReference: ly,
  prepareCopilotVisualRefs: Qi,
  resolveLocalSourcePath: ad
}, Symbol.toStringTag, { value: "Module" }));
let be = null;
const fy = 9e4, py = 18e4, my = 8e3;
function ld() {
  return w.join(G.getPath("userData"), "gemini-cli-workspace");
}
function gy() {
...
${t.purpose === "enhance-prompt" ? Yl : Kl}`);
    } else
...
async function qa(t, e, r) {
  var g, h;
  const n = await ln("gemini");
  if (!n)
    throw new Error("Gemini CLI is not installed. Install it with: npm install -g @google/gemini-cli");
  const i = ((g = e.model) == null ? void 0 : g.trim().replace(/^[^/]+\//, "")) || "gemini-2.5-flash", o = r.canResume ? yy(e) : hy(e), a = o.length > my, s = ld();
  await st(s, { recursive: !0 });
  const l = [
    "--skip-trust",
    ...a ? ["-p", ""] : ["-p", o],
    ...
    r.hasVisualRefs && !((h = e.images) != null && h.length) ? "yolo" : "default"
  ];
  if (r.hasVisualRefs) {
    l.push("--session-id", X.randomUUID());
    const _ = [...new Set(
      r.preparedVisualRefs.map((b) => w.dirname(b.mediaPath))
    )];
    for (const b of _)
      l.push("--include-directories", b);
  } else r.canResume && e.resumeSessionId && l.push("-r", e.resumeSessionId);
  const d = Jl();
  let c = "", u = "", p, f;
  const m = 900 * 1e3, y = r.hasVisualRefs ? py : fy;
  return new Promise((_, b) => {
    ...
    a && ((N = v.stdin) == null || N.write(o), (B = v.stdin) == null || B.end()), be = { child: v, requestId: t, provider: "gemini" };
    let T = "", S = !1, x = !1;
    const E = (J) => {
      S || (S = !0, clearTimeout(k), clearTimeout(O), Zi(r.preparedVisualRefs), J());
    }, k = setTimeout(() => {
      be = null, v.kill("SIGTERM"), E(() => b(new Error("Gemini CLI timed out after 15 minutes. Try again or switch models.")));
    }, m), O = setTimeout(() => {
      x || S || (be = null, v.kill("SIGTERM"), E(() => b(new Error(
    ...
    (H = v.stdout) == null || H.on("data", (J) => {
      T += J.toString();
      let j;
      for (; (j = T.indexOf(`
`)) >= 0; ) {
        const L = T.slice(0, j).trim();
        if (T = T.slice(j + 1), !!L)
          try {
            const U = JSON.parse(L);
            U.type === "init" && typeof U.session_id == "string" && (p = U.session_id);
            const P = _y(U);
            if (P && (f = P), U.type === "tool_use" && (d == null || d.webContents.send("llm:gemini-stream", {
              requestId: t,
              status: wy(U.tool_name)
            })), U.type === "message" && U.role === "assistant" && typeof U.content == "string") {
              const A = U.content;
              A && (x = !0, c += A, d == null || d.webContents.send("llm:gemini-stream", { requestId: t, token: A }));
            }
            if (U.type === "error" && typeof U.message == "string") {
              const A = U.message;
              u += A, !c.trim() && by(A) && (be = null, v.kill("SIGTERM"), E(() => b(new Error(gi(A)))));
            }
            if (U.type === "result" && U.status === "error") {
              const A = typeof U.error == "string" ? U.error : typeof U.message == "string" ? U.message : "Gemini CLI returned an error.";
              u += A;
            }
          } catch {
          }
      }
    }), (W = v.stderr) == null || W.on("data", (J) => {
      u += J.toString();
    }), v.on("error", (J) => {
      be = null, E(() => b(J));
    }), v.on("close", (J) => {
      be = null, d == null || d.webContents.send("llm:gemini-stream", { requestId: t, done: !0 });
      const j = c.trim();
      if (!j) {
        const L = gi(u.trim()) || `Gemini CLI exited with code ${J ?? "unknown"}`;
        E(() => b(new Error(L)));
        return;
      }
      E(() => _({
...
async function Ty(t, e) {
  if (!e.userMessage.trim())
    throw new Error("No chat message provided.");
  const r = ld(), n = gy();
  await st(r, { recursive: !0 }), await st(n, { recursive: !0 });
  const i = await zl(e.images);
  try {
    const o = [...i.refs, ...await Qi(e.visualRefs ?? [], n)];
    if ((e.visualRefs ?? []).length > 0 && o.length === 0)
    ...
    const a = o.length > 0, s = od(e.userMessage, o), l = e.messages ? [...e.messages] : void 0;
    if (a && (l != null && l.length)) {
      const u = l.findLastIndex((p) => p.role === "user");
      u >= 0 ? l[u] = { ...l[u], content: s } : l.push({ role: "user", content: s });
    }
    const d = {
      ...e,
      userMessage: s,
      messages: l
    }, c = !!e.resumeSessionId && !e.injectProjectContext && !a;
    try {
      return await qa(t, d, {
        canResume: c,
        hasVisualRefs: a,
        preparedVisualRefs: o
      });
    } catch (u) {
...
function Ey() {
  I.handle("llm:gemini-chat", async (t, e) => {
    const r = e.requestId || X.randomUUID(), n = await Ty(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
...
const Sy = `You are a music prompt engineer. Your job is to write a detailed, evocative text prompt that will be used to generate music with an AI music model (ElevenLabs/Suno).
...
function xy() {
  I.handle("music:generate-prompt", async (t, e) => {
    ...
    if (!r) throw new Error("No fal.ai API key provided.");
    K.fal.config({ credentials: r });
    const n = e.frameUrls && e.frameUrls.length > 0, i = Iy(e, !!n), o = {
...
function Ay() {
  I.handle("dialog:show-save", async (t, e) => {
    const r = Z.getFocusedWindow();
    if (!r) return null;
    const n = await so.showSaveDialog(r, {
    ...
    const r = Z.getFocusedWindow();
    if (!r) return null;
    const n = await so.showOpenDialog(r, {
      filters: e == null ? void 0 : e.filters,
  ...
  }), I.handle("shell:open-path", async (t, e) => await Kr.openPath(e));
}
const ky = {
...
function Mt(t, e) {
  const r = ky[e], n = Object.entries(t).filter(
    ([a]) => a !== "id" && (!r || r.has(a))
  );
...
function cd(t, e, r) {
  const { setClauses: n, values: i } = Mt(r, "projects");
...
function Wr(t, e, r) {
  const { setClauses: n, values: i } = Mt(r, "assets");
...
function Py(t, e, r) {
  const { setClauses: n, values: i } = Mt(r, "media_folders");
...
function Ny(t, e, r) {
  const { setClauses: n, values: i } = Mt(r, "timelines");
  ...
  t.transaction(() => {
    ...
    for (const n of r)
      t.run(
        ...
        [ir(), n.clip_id, n.time, n.property, n.value]
      );
  });
}
function By(t, e) {
...
function Jy(t, e, r) {
  const { setClauses: n, values: i } = Mt(r, "elements");
...
function Zy(t, e, r) {
  const { setClauses: n, values: i } = Mt(r, "export_jobs");
  return t.run(`UPDATE export_jobs SET ${n} WHERE id = ?`, [...i, e]);
}
function La(t, e) {
  const r = ud(t, e);
  if (!r) throw new Error(`Project not found: ${e}`);
  const n = fd(t, e), i = jy(t, e), o = zy(t, e), a = Wy(t, e), s = Yy(t, e), d = Oy(t, e).map((u) => {
    const p = qy(t, u.id), f = Uy(t, u.id), m = By(t, u.id), y = f.map((g) => ({
      ...g,
      keyframes: $y(t, g.id)
    }));
    return { ...u, tracks: p, clips: y, transitions: m };
  }), c = d.length > 0 ? d[0].id : "";
  return {
    project: r,
    assets: n,
...
function e_(t, e, r) {
  t.transaction(() => {
    ...
    for (const f of r.timelines) {
      if (l.has(f.id))
        Ny(t, f.id, { name: f.name, duration: f.duration });
      else {
        const { tracks: v, clips: T, transitions: S, ...x } = f;
        gd(t, x);
      }
      const m = new Set(
      ...
      ), h = new Set(f.clips.map((v) => v.id));
      for (const v of g)
        h.has(v) || Dy(t, v);
      for (const v of f.clips) {
        const { keyframes: T, ...S } = v;
        My(t, S), Fy(
          t,
          v.id,
          T.map(({ id: x, ...E }) => E)
    ...
    ), u = new Set(r.elements.map((f) => f.id));
    for (const f of c)
      u.has(f) || Ky(t, f);
    for (const f of r.elements)
      if (c.has(f.id)) {
        const { id: m, project_id: y, created_at: g, ...h } = f;
        Jy(t, f.id, { ...h, updated_at: gr() });
      } else
        Xy(t, f);
    const p = new Set(
...
function $e(t) {
  let e = He.get(t);
  return e || (e = new sg(t), He.set(t, e)), e;
}
function hd() {
  return w.join(ut(), "projects.json");
}
async function eo() {
  try {
    const t = await R.readFile(hd(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function to(t) {
  await R.mkdir(ut(), { recursive: !0 }), await R.writeFile(hd(), JSON.stringify(t, null, 2), "utf-8");
}
async function t_(t) {
  const e = await eo(), r = e.projects.findIndex((n) => n.id === t.id);
  r >= 0 ? e.projects[r] = t : e.projects.push(t), await to(e);
}
async function r_(t) {
  const e = await eo();
  e.projects = e.projects.filter((r) => r.id !== t), await to(e);
}
function n_() {
  I.handle("db:project:create", async (t, e) => {
    ...
    const r = $e(e), n = La(r, e);
    for (const i of n.assets)
      if (i.file_ref && !i.source_url) {
        const o = i.status;
        D.existsSync(i.file_ref) ? i.status === "offline" && (i.status = "online") : i.status = "offline", i.status !== o && Wr(r, i.id, { status: i.status });
      }
    return n;
  }), I.handle("db:project:save", async (t, e, r) => {
    const n = $e(e);
    e_(n, e, r);
    const i = gr(), o = await eo(), a = o.projects.find((s) => s.id === e);
    return a && (a.name = r.project.name, a.updatedAt = i, a.assetCount = r.assets.length, a.elementCount = r.elements.length, await to(o)), { ok: !0 };
    ...
    async (t, e, r) => {
      const n = $e(e);
      return cd(n, e, r), { ok: !0 };
    }
  ), I.handle("db:asset:insert", async (t, e) => {
    const r = $e(e.project_id);
    return pd(r, e), { ok: !0 };
  }), I.handle(
    "db:asset:update",
    async (t, e, r, n) => {
...
const o_ = /* @__PURE__ */ new Set([".mp4", ".mov", ".avi", ".mkv", ".webm", ".mxf", ".m4v"]), a_ = /* @__PURE__ */ new Set([".wav", ".mp3", ".aac", ".flac", ".ogg", ".m4a"]);
function Wn(t, e) {
  const r = w.extname(t).toLowerCase();
  return o_.has(r) ? "video" : a_.has(r) ? "audio" : r ? "image" : e;
}
function s_(t, e) {
  if (t)
    try {
...
function Dr(t) {
  const e = Xe(t.projectId), r = w.join(e, ".cache"), n = X.randomUUID(), i = {
    id: n,
...
async function Ua(t) {
  ...
  const i = Xe(e), o = w.join(i, "media", "generated");
  await R.mkdir(o, { recursive: !0 });
  const a = await l_(o, r);
  if (a)
    return Dr({
      assetId: r,
      projectId: e,
  ...
  const s = t.extension || s_(t.remoteUrl ?? t.localPathHint, n), l = w.join(o, `${r}${s}`), d = u_(t.localPathHint);
  if (d)
    return await c_(d, l), Dr({
      assetId: r,
      projectId: e,
      inputPath: l,
      type: Wn(l, n)
  ...
  const c = (f = t.remoteUrl) == null ? void 0 : f.trim();
  if (!c)
    return { error: "No downloadable URL or local file path for this asset." };
  const u = await fetch(c);
  if (!u.ok)
    throw new Error(`Failed to download (HTTP ${u.status}). The URL may have expired.`);
  const p = await u.arrayBuffer();
  return await R.writeFile(l, Buffer.from(p)), Dr({
    assetId: r,
    projectId: e,
    inputPath: l,
    type: Wn(l, n)
...
let he = null;
const Fe = /* @__PURE__ */ new Map(), or = /* @__PURE__ */ new Map(), f_ = w.dirname(Si(import.meta.url));
function yd() {
  let t = w.join(f_, "workers", "media-worker.js");
  return t.includes("app.asar") && (t = t.replace("app.asar", "app.asar.unpacked")), t;
}
function p_() {
  return he || (he = new rs(yd()), he.on("message", (t) => {
    switch (t.type) {
      case "ready":
      ...
      case "job:complete": {
        ...
        or.delete(t.jobId);
        const r = Fe.get(t.jobId);
        r && (r.resolve(t.result), Fe.delete(t.jobId));
        break;
      }
      case "job:error": {
        const e = or.get(t.jobId);
        ...
        or.delete(t.jobId);
        const r = Fe.get(t.jobId);
        r && (r.reject(new Error(t.error)), Fe.delete(t.jobId));
        break;
      }
    ...
    console.log(`[media-worker] Worker exited with code ${t}`), he = null;
    for (const [e, r] of Fe)
      r.reject(new Error("Worker exited")), Fe.delete(e);
  }), he.postMessage({
    type: "config",
    ffmpegPath: ve(),
    ffprobePath: Hi(),
...
function Se(t) {
  return t.type === "sync_compute_offset" || t.type === "sync_batch_match" ? m_(t) : new Promise((e, r) => {
    Fe.set(t.id, { resolve: e, reject: r }), or.set(t.id, { assetId: t.assetId, jobType: t.type }), p_().postMessage({ type: "job:submit", job: t });
  });
}
function m_(t) {
  return new Promise((e, r) => {
    ...
    n.on("message", (l) => {
      switch (l.type) {
        ...
        case "job:error":
          ...
          break;
        case "sync:batch-progress":
          for (const d of Z.getAllWindows())
...
function h_() {
  I.handle("media:import", async (t, e) => {
    const { filePaths: r, projectId: n, mode: i } = e, o = Xe(n), a = [], s = [];
    for (const l of r) {
      ...
      if (i === "copy") {
        ...
        await R.copyFile(l, y), c = y;
      }
      const u = g_(l), p = X.randomUUID();
      s.push({
        assetId: d,
        metadataJobId: p,
        inputPath: c,
        type: u,
        projectDir: o
      }), a.push({ assetId: d, jobId: p, filePath: c, type: u });
    }
    return setTimeout(() => {
      for (const l of s)
        Dr({
    ...
    const r = he;
    return r && r.postMessage({ type: "job:cancel", jobId: e }), Fe.delete(e), { ok: !0 };
  }), I.handle("media:extract-frame", async (t, e) => {
    const { inputPath: r, timeSec: n } = e, i = ve(), o = w.join(z.tmpdir(), `cinegen-frame-${X.randomUUID()}.jpg`);
    ...
    const n = r[1] === "jpeg" ? "jpg" : r[1], i = Buffer.from(r[2], "base64"), o = w.join(z.tmpdir(), `cinegen-frame-chat-${X.randomUUID()}.${n}`);
    ...
    const l = w.join(Xe(o), "media", "generated");
    await R.mkdir(l, { recursive: !0 });
    const d = w.join(l, `trim-${X.randomUUID()}.mp4`);
    return new Promise((c) => {
      ...
      qe(ve(), u, { timeout: Math.max(12e4, Math.ceil(s * 4e3)) }, (p) => {
        if (p || !D.existsSync(d)) {
          c(null);
          return;
        }
    ...
    const { inputPath: r, startTimeSec: n, durationSec: i } = e, o = ve(), a = w.join(z.tmpdir(), `cinegen-clip-${X.randomUUID()}.mp4`), s = Math.max(0, n), l = Math.max(0.1, i);
    return new Promise((d) => {
      ...
      qe(o, c, { timeout: Math.max(12e4, Math.ceil(l * 4e3)) }, (u, p, f) => {
        if (u || !D.existsSync(a)) {
          d(null);
          return;
        }
    ...
    } = e, d = Xe(n), c = w.join(d, ".cache");
    if (a) {
      const u = w.join(c, "thumbnails");
      D.mkdirSync(u, { recursive: !0 });
      const p = {
        id: X.randomUUID(),
        type: "generate_thumbnail",
        assetId: r,
        inputPath: i,
        outputPath: w.join(u, `${r}.jpg`),
        projectDir: d
      };
      Se(p).catch((f) => console.error("[media-import] Thumbnail failed:", f));
    }
    if (s) {
      const u = w.join(c, "waveforms");
      D.mkdirSync(u, { recursive: !0 });
      const p = {
        id: X.randomUUID(),
        type: "compute_waveform",
...
const w_ = ts(import.meta.url), b_ = w.dirname(Si(import.meta.url));
function v_() {
  return G.isPackaged ? w.join(process.resourcesPath, "native", "cinegen_avfoundation.node") : w.resolve(b_, "../native/avfoundation/build/Release/cinegen_avfoundation.node");
}
let ne = null, bi = null;
if (process.platform === "darwin")
  try {
    const t = v_();
    ne = w_(t), console.log("[native-video] AVFoundation addon loaded:", t);
  } catch (t) {
    bi = t instanceof Error ? t.message : String(t), console.error("[native-video] Failed to load AVFoundation addon:", bi);
  }
function De() {
  return ne != null;
}
...
function E_(t, e) {
  return ne ? ne.createSurface(t, e) : !1;
}
function Ma(t) {
  ne == null || ne.destroySurface(t);
}
function S_(t, e, r, n, i) {
  ne == null || ne.setSurfaceRect(t, e, r, n, i);
}
function Da(t, e) {
  ne == null || ne.setSurfaceHidden(t, e);
}
function $a(t) {
  ne == null || ne.clearSurface(t);
}
function I_(t, e) {
  ne == null || ne.syncSurface(t, e);
}
function x_() {
  I.handle("native-video:is-available", () => ({
    ...
    const r = Z.fromWebContents(t.sender);
    return !r || !De() ? !1 : E_(e, r.getNativeWindowHandle());
  }), I.on("native-video:set-surface-rect", (t, e) => {
    De() && S_(e.surfaceId, e.x, e.y, e.width, e.height);
  }), I.on("native-video:set-surface-hidden", (t, e) => {
    De() && Da(e.surfaceId, e.hidden);
  }), I.on("native-video:clear-surface", (t, e) => {
    De() && $a(e);
  }), I.on("native-video:sync-surface", (t, e) => {
    De() && I_(e.surfaceId, e.descriptors);
  }), I.on("native-video:destroy-surface", (t, e) => {
    De() && Ma(e);
  });
}
const A_ = "python3.12", _d = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), k_ = w.join(_d, ".venv", "bin", "python");
function j_(...t) {
  return G.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const R_ = j_("scripts", "whisperx", "cinegen_infer.py"), P_ = "fal-ai/whisper", Fa = "3", O_ = {
  ".mp4": "video/mp4",
  ".m4v": "video/mp4",
  ".mov": "video/quicktime",
  ".webm": "video/webm",
...
function N_(t) {
  const e = w.extname(t).toLowerCase();
  return O_[e] ?? "application/octet-stream";
}
function Xr(t) {
  const e = Number(t);
...
function wd(t) {
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    r || (r = {
      ...
      words: []
    }), r.words.push(o), r.end = o.end, r.text = C_(r.text, o.word), !r.speaker && o.speaker && (r.speaker = o.speaker);
    const a = t[i + 1], s = a ? Math.max(0, a.start - o.end) : 0, l = !!a && (a.speaker ?? null) !== (r.speaker ?? null), d = r.end - r.start, c = /[.!?]["')\]]*$/.test(o.word), u = s >= 0.85 || s >= 0.45 && /[,;:]$/.test(o.word), p = d >= 12;
    (!a || c || u || p || l) && n();
  }
  return n(), e;
}
function L_(t) {
  ...
  return e.length === 0 ? t : wd(e);
}
function U_(t) {
  const e = (t == null ? void 0 : t.data) ?? t, r = typeof (e == null ? void 0 : e.text) == "string" ? e.text : "", n = e == null ? void 0 : e.chunks, i = e, o = Array.isArray(n) ? n.flatMap((c) => {
    if (!c || typeof c != "object") return [];
    const u = typeof c.text == "string" ? c.text.trim() : "", p = c.timestamp, f = Array.isArray(p) ? Xr(p[0]) : void 0, m = Array.isArray(p) ? Xr(p[1]) : void 0, y = q_(c.speaker);
    return !u && f === void 0 && m === void 0 ? [] : [{ text: u, start: f, end: m, speaker: y }];
  }) : [], a = o.flatMap((c) => !c.text || c.start === void 0 || c.end === void 0 ? [] : [{
    word: c.text,
    start: c.start,
    end: c.end,
    ...c.speaker ? { speaker: c.speaker } : {}
  }]), s = a.length > 0 ? wd(a) : o.map((c) => ({
    text: c.text,
    start: c.start ?? 0,
    end: c.end ?? c.start ?? 0,
    ...c.speaker ? { speaker: c.speaker } : {}
  ...
  for (const c of d) {
    ...
    if (Array.isArray(c)) {
      const u = c.find((p) => typeof p == "string" && p.trim().length > 0);
      if (u) {
        l = u.trim();
        break;
      }
  ...
  return {
    text: r || s.map((c) => c.text).filter(Boolean).join(" "),
    segments: s,
    language: l
  };
}
...
function Je(t, e) {
  var r;
  (r = $_()) == null || r.webContents.send("transcription:progress", {
    jobId: t.jobId,
    assetId: t.assetId,
    engine: t.engine,
    ...e
...
async function F_(t) {
  try {
    const e = $e(t.projectId), r = fd(e, t.projectId).find((o) => o.id === t.assetId), i = {
...
async function ro(t) {
  t.status = "done", t.segments = L_(t.segments), t.fullText.trim() || (t.fullText = t.segments.map((e) => e.text).filter(Boolean).join(" ")), await F_(t), Je(t, {
    type: "done",
    text: t.fullText,
    segments: t.segments,
    language: t.language
...
function at(t, e) {
  t.status = "error", t.error = e, Je(t, { type: "error", error: e });
}
function B_(t, e) {
  const r = e.model ?? "large", n = e.language ?? "auto";
  t.model = r, (async () => {
    const i = w.join(z.tmpdir(), `cinegen-whisper-${t.jobId}.py`);
    await R.writeFile(i, D_, "utf-8");
    const o = ae(A_, [i, e.filePath, r, n], {
      stdio: ["ignore", "pipe", "pipe"]
    });
    t.status = "running", Je(t, { type: "status", status: "running" }), o.stdout.on("data", (a) => {
...
`)) {
        ...
        if (l)
          try {
            ...
            if (d.type === "segment") {
              ...
              t.segments.push(c), Je(t, { type: "segment", ...c });
      ...
      if (await R.unlink(i).catch(() => {
        ...
        at(t, `whisper process exited with code ${a}`);
        return;
      }
      await ro(t);
    }), o.on("error", async (a) => {
    ...
    at(t, i instanceof Error ? i.message : String(i));
  });
}
function V_(t, e) {
  t.model = "base";
  ...
  t.status = "running", Je(t, { type: "status", status: "running" });
  let o;
  i.stdout.on("data", (a) => {
    for (const s of a.toString().split(`
`)) {
      ...
      if (l)
        try {
          const d = JSON.parse(l);
          d.type === "progress" ? (d.output_text !== void 0 && (t.fullText = d.output_text), d.segments && (t.segments = d.segments), d.language !== void 0 && (t.language = d.language), Je(t, {
            type: "progress",
            stage: d.stage,
            message: d.message,
          ...
          })) : d.type === "done" ? (d.output_text !== void 0 && (t.fullText = d.output_text), d.segments && (t.segments = d.segments), d.language !== void 0 && (t.language = d.language), o = d.transcript_path) : d.type === "error" && at(t, d.error ?? "WhisperX error");
        } catch {
        }
    }
  }), i.stderr.on("data", () => {
    ...
    if (t.status !== "error") {
      if (a !== 0) {
        at(t, `whisperx process exited with code ${a}`);
        return;
      }
      if (o)
        try {
...
function G_(t, e) {
  (async () => {
    ...
    t.model = Fa, t.status = "running", Je(t, { type: "status", status: "running", stage: "uploading", message: "Preparing audio for cloud transcription" }), K.fal.config({ credentials: e.apiKey });
    const r = await M_(e.filePath);
    let n = "";
    try {
      const s = await R.readFile(r), d = `${w.basename(e.filePath, w.extname(e.filePath))}.m4a`, c = N_(r), u = new Blob([s], { type: c }), p = new File([u], d, { type: c });
      n = await K.fal.storage.upload(p);
    } finally {
      await R.unlink(r).catch(() => {
      });
    }
    Je(t, { type: "status", status: "running", stage: "transcribing", message: "Running cloud transcription" });
    const i = {
      audio_url: n,
      task: "transcribe",
      chunk_level: "word",
    ...
    }, o = await K.fal.subscribe(P_, { input: i, logs: !0 }), a = U_(o);
    t.fullText = a.text, t.segments = a.segments, t.language = a.language, await ro(t);
  })().catch((r) => {
    at(t, r instanceof Error ? r.message : String(r));
  });
}
function z_() {
  I.handle("transcription:start", async (t, e) => {
    const {
      ...
      apiKey: l
    } = e, d = `txn-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, c = {
      jobId: d,
      assetId: n,
      projectId: r,
      engine: s,
    ...
    return Ba.set(d, c), s === "whisperx-local" ? V_(c, { filePath: i, language: a }) : s === "whisper-cloud" ? G_(c, { filePath: i, language: a, apiKey: l }) : B_(c, { filePath: i, model: o, language: a }), { jobId: d };
...
const no = w.join(z.homedir(), "Desktop", "Coding", "ltx"), H_ = w.join(no, ".venv", "bin", "python"), W_ = w.join(no, "cinegen_infer.py"), io = w.join(z.homedir(), "Desktop", "Coding", "qwen-edit"), X_ = w.join(io, ".venv", "bin", "python"), J_ = w.join(io, "cinegen_infer.py"), bd = w.join(z.homedir(), "Desktop", "Coding", "layer-decompose"), K_ = w.join(bd, ".venv", "bin", "python"), vd = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), Y_ = w.join(vd, ".venv", "bin", "python");
function Td(...t) {
  return G.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const Q_ = Td("scripts", "layer-decompose", "cinegen_infer.py"), Z_ = Td("scripts", "whisperx", "cinegen_infer.py"), ew = {
...
function _t(t, e) {
  var r;
  (r = tw()) == null || r.webContents.send("local-model:progress", { jobId: t, ...e });
}
...
function rw() {
  I.handle("local-model:run", async (t, e) => {
    ...
    if (e.nodeType === "qwen-edit-local") {
      const s = String(r.prompt ?? ""), l = Number(r.num_inference_steps ?? 50), d = Number(r.guidance_scale ?? 1), c = Number(r.true_cfg_scale ?? 4), u = Number(r.seed ?? 42);
      let p = null;
      if (r.image_url) {
        const m = await Cr(String(r.image_url), n);
      ...
      const s = String(r.prompts ?? "").trim(), l = String(r.inpainter ?? "qwen-edit-local"), d = !!(r.reconstruct_bg ?? !0), c = Number(r.seed ?? 42);
      let u = null;
      if (r.image_url) {
        console.log("[layer-decompose] resolving image_url:", r.image_url);
        const m = await Cr(String(r.image_url), n);
        u = m.imagePath, a = m.tempPath, console.log("[layer-decompose] resolved to:", u);
      }
      if (!u) throw new Error("Layer Decompose requires an input image");
      const f = [
        Q_,
        "--image_path",
        u,
      ...
      if (r.audio_url) {
        ...
        const m = await Cr(String(r.audio_url), n);
        c = m.imagePath, a = m.tempPath, console.log("[whisperx] resolved to:", c);
      }
      if (!c) throw new Error("WhisperX requires an audio input");
      const u = [
        Z_,
        "--audio_path",
        c,
      ...
      l && u.push("--language", l), d || u.push("--no_diarize");
      const p = process.env.HF_TOKEN, f = { ...process.env };
      p && (f.HF_TOKEN = p), o = ae(Y_, u, {
        cwd: vd,
        stdio: ["ignore", "pipe", "pipe"],
        env: f
      });
    } else {
      const s = String(r.prompt ?? ""), l = String(r.resolution ?? "896x512"), { height: d, width: c } = ew[l] ?? { height: 512, width: 896 }, u = Number(r.frame_rate ?? 24), p = Number(r.duration_secs ?? 4), f = Math.round(p * u / 8) * 8 + 1, m = Math.max(9, f), y = Number(r.seed ?? 42), g = !!r.enhance_prompt;
      let h = null;
      if (r.image_url) {
        const b = await Cr(String(r.image_url), n);
      ...
      h && _.push("--image_path", h), g && _.push("--enhance_prompt"), o = ae(H_, _, {
        cwd: no,
        stdio: ["ignore", "pipe", "pipe"]
      });
    }
    return i.status = "running", _t(n, { type: "status", status: "running" }), o.stdout.on("data", (s) => {
...
`)) {
        ...
        if (d)
          try {
            ...
            c.type === "progress" ? (i.stage = c.stage, c.output_text !== void 0 && (i.outputText = c.output_text), c.segments && (i.segments = c.segments), c.language !== void 0 && (i.language = c.language), _t(n, {
              type: "progress",
              stage: c.stage,
              message: c.message,
              ...c.output_text !== void 0 && { output_text: c.output_text },
              ...c.segments && { segments: c.segments },
              ...c.language !== void 0 && { language: c.language }
            })) : c.type === "done" ? (i.status = "done", i.outputPath = c.output_path, i.outputText = c.output_text, i.transcriptPath = c.transcript_path, i.segments = c.segments, i.language = c.language, _t(n, {
              type: "done",
              output_path: c.output_path,
              ...c.output_text !== void 0 && { output_text: c.output_text },
              ...c.transcript_path !== void 0 && { transcript_path: c.transcript_path },
              ...c.segments && { segments: c.segments },
              ...c.language !== void 0 && { language: c.language },
              ...c.layers && { layers: c.layers },
              ...c.needs_inpainting !== void 0 && { needs_inpainting: c.needs_inpainting },
              ...c.combined_mask_path && { combined_mask_path: c.combined_mask_path }
            })) : c.type === "error" && (i.status = "error", i.error = c.error, _t(n, { type: "error", error: c.error }));
          } catch {
          }
      }
    }), o.stderr.on("data", () => {
    }), o.on("error", (s) => {
      i.status = "error", i.error = s.message, _t(n, { type: "error", error: s.message });
    }), o.on("close", (s) => {
      a && R.unlink(a).catch(() => {
      }), s !== 0 && i.status !== "done" && (i.status = "error", i.error = i.error ?? `Process exited with code ${s}`, _t(n, { type: "error", error: i.error }));
    }), { jobId: n };
...
const oo = w.join(z.homedir(), "Desktop", "Coding", "Sam3"), nw = w.join(oo, ".venv", "bin", "python"), iw = w.join(oo, "cinegen_server.py"), ow = 120 * 1e3, aw = 500, sw = 60;
class lw {
  constructor() {
  ...
  async start() {
    var e, r;
    return this.proc && !this.proc.killed ? this.port : (this.port = await this.findFreePort(), console.log(`[sam3] Starting server on port ${this.port}`), this.proc = ae(nw, [iw, "--port", String(this.port)], {
      cwd: oo,
      stdio: ["ignore", "pipe", "pipe"],
      env: {
        ...process.env,
  ...
  async waitForHealth() {
    ...
    for (let e = 0; e < sw; e++) {
      ...
      await new Promise((r) => setTimeout(r, aw));
    }
...
const ar = new lw();
function dw() {
  I.handle("sam3:start", async () => ({ port: await ar.ensureRunning() })), I.handle("sam3:stop", async () => {
    await ar.stop();
  }), I.handle("sam3:port", () => ({ port: ar.getPort(), running: ar.isRunning() }));
}
function uw() {
  ar.stop();
}
...
async function fw(t) {
  const e = t.workspaceDir ?? w.join(G.getPath("userData"), "gemini-cli-workspace"), r = await Qi(t.visualRefs, e);
  if (r.length === 0)
    throw new Error("Could not load the attached clip or asset files for visual analysis.");
  try {
    const n = [];
    for (const i of r) {
      const o = cw(t.prompt, i.label, i.mediaType), a = i.mediaType === "video" ? await Fl({
        apiKey: t.apiKey,
...
function pw() {
  I.handle("copilot:analyze-visual-refs", async (t, e) => fw(e));
}
const mw = 1, gw = -30, hw = 0.3;
function yw(t) {
  const e = [];
  let r = null;
  for (const n of t.split(/\r?\n/)) {
...
`);
  const n = r.map((i) => `[${Ga(i.start)}-${Ga(i.end)}] ${i.text}`).join(`
`);
  return [
...
function vw(t) {
  ...
  return (Array.isArray(r) ? r : r && typeof r == "object" && Array.isArray(r.segments) ? r.segments : []).flatMap((i) => {
    ...
    const o = i, a = Xn(o.start), s = Xn(o.end);
    return a === void 0 || s === void 0 || s <= a ? [] : [{
      start: a,
      end: s,
      delivery: wt(o.delivery),
      emotion: wt(o.emotion),
      energy: wt(o.energy),
      pace: wt(o.pace),
      notable: bw(o.notable),
      content: wt(o.content),
      shotType: wt(o.shotType),
      cutawayCandidate: typeof o.cutawayCandidate == "boolean" ? o.cutawayCandidate : void 0,
      confidence: Xn(o.confidence)
    }];
  });
}
function Tw(t) {
  return new Promise((e) => {
    const r = [
      ...
      t,
      "-af",
      `silencedetect=noise=${gw}dB:d=${hw}`,
      "-f",
      "null",
      "-"
    ], n = ae(ve(), r);
...
const Ed = "gemini-2.5-flash", Ew = "fal-ai/video-understanding";
async function Sw(t, e) {
  var n;
  const r = ((n = t.model) == null ? void 0 : n.trim()) || Ed;
  try {
    return { rawText: await sd({
...
async function Iw(t) {
  ...
  const e = {
    ...
    status: "failed",
    version: mw,
    model: ((r = t.model) == null ? void 0 : r.trim()) || Ed,
    silenceMap: [],
    segments: [],
  ...
  try {
    const n = await Tw(t.mediaPath).catch(() => ""), i = yw(n), o = _w({ assetName: t.assetName, transcript: t.transcript }), { rawText: a, model: s } = await Sw(t, o), l = vw(a);
    return {
      ...e,
      model: s,
      status: "ready",
...
function xw() {
  I.handle("acoustic:analyze-asset", async (t, e) => Iw(e));
}
const Aw = process.platform === "darwin" && !G.isPackaged;
Aw && (G.disableHardwareAcceleration(), G.commandLine.appendSwitch("disable-gpu-compositing"), console.log("[app] hardware acceleration disabled for macOS dev wake stability"));
G.commandLine.appendSwitch("disable-renderer-backgrounding");
G.commandLine.appendSwitch("disable-backgrounding-occluded-windows");
Ka.registerSchemesAsPrivileged([
  {
    scheme: "local-media",
    privileges: {
      standard: !0,
...
let ee = null, rr = null, te = null, Ve = null;
const kw = Date.now(), za = "cinegen-desktop", Ha = "CineGen", jw = ".cinegen-user-data-migrated.json", Wa = "CineGen", Rw = 700;
...
function Ow() {
  try {
    const t = G.getPath("appData"), e = w.join(t, za), r = w.join(t, Ha);
    return G.getPath("userData") !== r && G.setPath("userData", r), console.log("[app] userData path:", r), { preferredUserDataPath: r, legacyUserDataPath: e };
  } catch (t) {
    console.error("[app] failed to configure userData path:", t);
    const e = G.getPath("appData"), r = w.join(e, Ha), n = w.join(e, za);
...
Bd();
try {
  G.setName(Wa), process.platform === "darwin" && G.setAboutPanelOptions({
    applicationName: Wa,
...
async function Cw() {
  const { preferredUserDataPath: t, legacyUserDataPath: e } = Nw;
...
function Lw(t) {
  const e = w.extname(t).toLowerCase();
  return Pw[e] ?? "application/octet-stream";
}
function Uw(t, e) {
...
async function $w() {
  ...
  try {
    const s = await R.readFile(t, "utf-8"), l = JSON.parse(s), d = ((n = l.project) == null ? void 0 : n.id) || X.randomUUID(), c = ((i = l.project) == null ? void 0 : i.name) || "Migrated Project";
    ...
    await R.writeFile(r, JSON.stringify(u, null, 2), "utf-8"), console.log(`[migration] Migrated legacy project "${c}" to ${e}/${d}`);
  } catch (s) {
    console.error("[migration] Failed to migrate legacy data:", s);
  }
}
G.whenReady().then(async () => {
  if (await Cw(), process.platform === "darwin") {
    const r = qw();
    console.log("[dock] icon candidates:", r);
    for (const n of r)
      try {
        const i = xd.createFromPath(n);
        if (console.log("[dock] testing icon:", n, "empty?", i.isEmpty()), !i.isEmpty()) {
          await Promise.resolve(G.dock.setIcon(i)), console.log("[dock] applied icon:", n);
          break;
        }
      } catch (i) {
  ...
  Ka.handle("local-media", async (r) => {
    try {
      ...
      const o = i.size, a = Lw(n), s = Uw(r.headers, "range");
      ...
      if (s) {
        ...
        const f = p - u + 1, m = D.createReadStream(n, { start: u, end: p }), y = lo.toWeb(m);
        return new Response(y, {
          status: 206,
          headers: {
            "Content-Type": a,
            "Content-Length": String(f),
            "Content-Range": `bytes ${u}-${p}/${o}`,
            "Accept-Ranges": "bytes",
            "Cache-Control": "public, max-age=31536000, immutable",
            "Access-Control-Allow-Origin": "*"
          }
      ...
      const l = D.createReadStream(n), d = lo.toWeb(l);
  ...
  }), eu(), nu(() => ee), Fd(), vp(), $c(), Np(), Km(), eg(), ig(), _g(), nh(), wh(), Sh(), Ch(), ny(), Zh(), oy(), ay(), Ey(), xy(), Ay(), n_(), h_(), __(Se), Ng(), pw(), xw(), x_(), z_(), rw(), dw(), await $w(), I.handle("pm:open-project", async (r, n, i) => n === "__close__" ? (te == null || te.close(), te = null, { ok: !0 }) : ((!ee || ee.isDestroyed()) && (ee = mo()), ee.once("ready-to-show", () => {
    ee == null || ee.maximize(), ee == null || ee.show(), ee == null || ee.webContents.send("pm:open-project", n, i);
  }), ee.webContents.getURL() !== "" && (ee.maximize(), ee.show(), ee.webContents.send("pm:open-project", n, i)), te == null || te.close(), te = null, { ok: !0 })), I.handle("pm:open", async () => te && !te.isDestroyed() ? (te.focus(), { ok: !0 }) : (te = En(), te.on("closed", () => {
    te = null;
  }), { ok: !0 })), rr = Kd(), ee = mo();
  const t = 3e3;
  ee.once("ready-to-show", () => {
    const r = Date.now() - kw, n = Math.max(0, t - r);
    setTimeout(() => {
      rr == null || rr.close(), rr = null, te = En(), te.on("closed", () => {
        te = null;
      });
    }, n);
  }), G.on("activate", () => {
    Z.getAllWindows().length === 0 && (te = En(), te.on("closed", () => {
      te = null;
    }));
  });
  const e = (r) => {
    Ve && (clearTimeout(Ve), Ve = null), Ve = setTimeout(() => {
      Ve = null, console.log(`[app] Wake recovery triggered by ${r}`), Jd(r);
    }, Rw);
  };
  wn.on("resume", () => {
    Jn("resume"), e("resume");
  }), wn.on("unlock-screen", () => {
    Jn("unlock-screen"), e("unlock-screen");
  }), wn.on("suspend", () => {
    Jn("suspend");
  });
});
G.on("before-quit", () => {
  iu(), Ve && (clearTimeout(Ve), Ve = null), y_(), i_(), uw();
});
