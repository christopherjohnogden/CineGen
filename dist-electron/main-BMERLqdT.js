...
var U = (t, e, r) => cd(t, typeof e != "symbol" ? e + "" : e, r);
import { ipcMain as I, app as W, shell as hi, BrowserWindow as Q, screen as Fa, safeStorage as Sr, session as Ba, dialog as ro, protocol as Va, nativeImage as fd, powerMonitor as yn } from "electron";
import D, { existsSync as it, readFileSync as gr, mkdirSync as gi, writeFileSync as yi, copyFileSync as pd, chmodSync as md, renameSync as hd, rmSync as Ha } from "node:fs";
import w, { dirname as gd, join as Et } from "node:path";
import z, { homedir as Ga } from "node:os";
import { execFile as Ce, spawn as ae } from "node:child_process";
import { promisify as Lt } from "node:util";
import J, { randomUUID as yr, randomBytes as yd } from "node:crypto";
import j, { writeFile as _d, chmod as wd, mkdir as at } from "node:fs/promises";
import { Readable as no } from "node:stream";
import _i from "better-sqlite3";
import { createServer as za } from "node:http";
import { lookup as bd } from "node:dns/promises";
import { request as vd } from "node:https";
import Ed, { isIP as wi } from "node:net";
import { createRequire as Wa } from "node:module";
import { fileURLToPath as bi } from "node:url";
import { Worker as Xa } from "worker_threads";
function _r(t) {
...
function bt() {
  return {
    config: Et(Ga(), "Library", "Application Support", "Claude", "claude_desktop_config.json"),
    source: Et(W.getAppPath(), "dist-electron", "cinegen-mcp.cjs"),
    installed: Et(W.getPath("userData"), "mcp", "cinegen-mcp.cjs")
  };
}
function Ka() {
...
async function Id(t) {
  ...
  try {
    await Sd(process.execPath, ["-e", e, t], {
...
function wn() {
  const t = bt(), e = vi(t.config).config, r = Ja(e), n = it(t.source) && it(t.installed) && gr(t.source).equals(gr(t.installed));
  return {
    supported: process.platform === "darwin",
    configured: !!r,
    needsRepair: !!r && (!Td(r, Ka()) || !n),
...
function xd() {
  I.handle("claude-mcp:status", () => wn()), I.handle("claude-mcp:setup", async () => {
    _n();
    const t = bt();
    if (vi(t.config), !it(t.source)) throw new Error("This build is missing the MCP server. Install the latest CineGen Mac build.");
    await Id(t.source), gi(Et(W.getPath("userData"), "mcp"), { recursive: !0 }), yi(t.installed, gr(t.source), { mode: 384 });
    const e = io(t.config, Ka());
    return { ...wn(), ...e };
  }), I.handle("claude-mcp:remove", () => {
    _n();
    const t = io(bt().config, null);
    return { ...wn(), ...t };
  }), I.handle("claude-mcp:reveal", () => {
    if (_n(), !it(bt().config)) throw new Error("Connect Claude Desktop first to create its configuration.");
    hi.showItemInFolder(bt().config);
  });
}
const Ad = 1200, kd = 150, jd = 1e3, oo = 2800, Ya = /* @__PURE__ */ new WeakMap(), Qa = /* @__PURE__ */ new WeakMap(), Dr = /* @__PURE__ */ new WeakMap(), Za = /* @__PURE__ */ new WeakMap();
function Rd() {
...
const jt = Rd(), Jr = w.join(import.meta.dirname, "."), es = w.join(Jr, "../dist"), wr = process.env.VITE_DEV_SERVER_URL;
function ao(t) {
  return wr ? t.loadURL(`${wr}?pm=1`) : t.loadFile(w.join(es, "index.html"), { query: { pm: "1" } });
}
function so(t) {
  return wr ? t.loadURL(wr) : t.loadFile(w.join(es, "index.html"));
}
function bn(t, e) {
...
async function Pd(t, e, r) {
  if (!t.isDestroyed())
    try {
      ...
      await Gn(t, e, r, "blank renderer DOM after resume");
    } catch (n) {
      console.warn(`[window] ${e} health check failed after wake:`, n), await Gn(t, e, r, "resume health check failed");
    }
}
function Nd(t) {
  for (const e of Q.getAllWindows()) {
    ...
    const r = Ya.get(e);
    if (!r) continue;
    const n = Qa.get(e) ?? "window";
    ts(e), Za.set(e, Date.now() + oo + 1e3);
    let i = null;
    const o = setTimeout(() => {
      Ir(e, o), !e.isDestroyed() && (console.log(`[window] ${n} wake recovery started: ${t}`), e.webContents.invalidate(), e.webContents.executeJavaScript(
        `(() => {
    ...
    const a = setTimeout(() => {
      Ir(e, a), (async () => {
        try {
          await Pd(e, n, r), i && (clearTimeout(i), Ir(e, i), i = null);
        } catch (s) {
    ...
    bn(e, a), i = setTimeout(() => {
      Ir(e, i), !e.isDestroyed() && Gn(e, n, r, `hard reload after ${t}`).catch((s) => {
...
function vn() {
  const { width: t, height: e } = Fa.getPrimaryDisplay().workAreaSize, r = 900, n = 580, i = new Q({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
    ...
    ...jt ? { icon: jt } : {},
    webPreferences: {
      preload: w.join(Jr, "preload.js"),
      nodeIntegration: !1,
      contextIsolation: !0,
      backgroundThrottling: !1
    }
  });
  return rs(i, "project-manager", ao), ao(i), i;
}
function Cd() {
  const { width: t, height: e } = Fa.getPrimaryDisplay().workAreaSize, r = 800, n = 395, i = new Q({
    width: r,
    height: n,
    x: Math.round((t - r) / 2),
    y: Math.round((e - n) / 2),
  ...
  return i.loadFile(w.join(Jr, "splash.html")), i;
}
function lo() {
  const t = new Q({
    width: 1440,
  ...
  return rs(t, "main", so), so(t), wr && t.webContents.openDevTools({ mode: "detach" }), t;
}
function Ei() {
...
async function xr() {
  try {
    const t = await j.readFile(zn(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function En(t) {
  await os();
  const e = zn() + ".tmp";
  await j.writeFile(e, JSON.stringify(t, null, 2), "utf-8"), await j.rename(e, zn());
}
function qd(t, e) {
  const r = is(), n = {
    id: ns(),
...
function Ud(t) {
  ...
  if (!D.existsSync(e)) return null;
  try {
    const r = new _i(e, { readonly: !0 }), n = r.prepare(
...
function Md() {
  I.handle("project:list", async () => (await xr()).projects.map((e) => {
    const r = e.useSqlite ? Ud(e.id) : Ld(e.id);
    ...
    const n = ns(), i = qd(n, r);
    await os(), await j.mkdir(br(n), { recursive: !0 });
    const o = ut(n) + ".tmp";
    await j.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await j.rename(o, ut(n));
    const a = await xr();
    ...
    const r = await j.readFile(ut(e), "utf-8");
    return JSON.parse(r);
  }), I.handle("project:save", async (t, e, r) => {
    let n;
    try {
      const l = await j.readFile(ut(e), "utf-8");
      n = JSON.parse(l);
    } catch {
    ...
    await j.writeFile(o, JSON.stringify(i, null, 2), "utf-8"), await j.rename(o, ut(e));
    const a = await xr(), s = a.projects.find((l) => l.id === e);
    return s && (s.updatedAt = i.project.updatedAt, s.assetCount = Array.isArray(i.assets) ? i.assets.length : 0, s.elementCount = Array.isArray(i.elements) ? i.elements.length : 0, r.project && r.project.name && (s.name = r.project.name), await En(a)), i;
  }), I.handle("project:delete", async (t, e) => {
    await j.rm(br(e), { recursive: !0, force: !0 });
    const r = await xr();
    r.projects = r.projects.filter((n) => n.id !== e), await En(r);
  });
}
const uo = 12e4, as = Et(Ga(), "Documents", "CINEGEN"), Wn = Et(as, "mcp-bridge.json"), Rt = /* @__PURE__ */ new Map();
let Nr = !1, pe = null, Ar = "";
function ke(t, e, r) {
...
function $d(t, e, r) {
  return new Promise((n, i) => {
    const o = yr(), a = setTimeout(() => {
      Rt.delete(o), i(new Error(`CineGen did not answer "${e}" within ${Math.round(uo / 1e3)}s.`));
    }, uo);
...
function Fd(t) {
  I.on("mcp:ready", (e, r) => {
  ...
  }), Ar = yd(24).toString("hex"), pe = za((e, r) => {
    (async () => {
      try {
        const n = new URL(e.url ?? "/", "http://127.0.0.1");
        if (e.method === "GET" && n.pathname === "/health") {
          ke(r, 200, { ok: !0, app: "CineGen", appReady: !!t() });
          return;
        }
        const i = (e.headers.authorization ?? "").replace(/^Bearer\s+/i, "");
        if (i.length !== Ar.length || i !== Ar) {
          ke(r, 401, { ok: !1, error: "Bad or missing bridge token." });
          return;
        }
        if (e.method === "POST" && n.pathname === "/invoke") {
          const o = await Dd(e), a = typeof (o == null ? void 0 : o.tool) == "string" ? o.tool : "";
          if (!a) {
            ke(r, 400, { ok: !1, error: "No tool named." });
            return;
          }
          const s = t();
          if (!s || s.isDestroyed()) {
            ke(r, 503, { ok: !1, error: "CineGen has no open window. Open the app and try again." });
            return;
          }
          if (!Nr && a !== "cinegen_project") {
            ke(r, 200, { ok: !1, error: "No CineGen project is open. Open a project in the app, then try again." });
            return;
          }
          try {
            ke(r, 200, { ok: !0, result: await $d(s, a, o.args ?? {}) });
          } catch (l) {
            ke(r, 200, { ok: !1, error: l instanceof Error ? l.message : String(l) });
          }
          return;
        }
        ke(r, 404, { ok: !1, error: "Unknown path." });
      } catch (n) {
        ke(r, 400, { ok: !1, error: n instanceof Error ? n.message : String(n) });
      }
    })();
    ...
    try {
      gi(as, { recursive: !0 }), yi(
        Wn,
        `${JSON.stringify({ port: e, token: Ar, pid: process.pid, startedAt: (/* @__PURE__ */ new Date()).toISOString() }, null, 2)}
`,
        { mode: 384 }
...
function Bd() {
  ...
  Rt.clear(), pe == null || pe.close(), pe = null;
  try {
    Ha(Wn, { force: !0 });
  } catch {
  }
...
function ss() {
  return co || (co = 1, (function(t) {
    var e = Ht && Ht.__awaiter || function(i, o, a, s) {
      function l(d) {
...
function lt() {
  if (po) return ve;
  po = 1;
  var t = ve && ve.__awaiter || function(s, l, d, u) {
    function c(p) {
  ...
  Object.defineProperty(ve, "__esModule", { value: !0 }), ve.ValidationError = ve.ApiError = void 0, ve.defaultResponseHandler = o, ve.resultResponseHandler = a;
  const e = Ti(), r = "x-fal-request-id";
...
function Je() {
  ...
  var t = ce && ce.__awaiter || function(c, p, f, m) {
    ...
    return new (f || (f = Promise))(function(h, g) {
      ...
      function v(E) {
        E.done ? h(E.value) : y(E.value).then(_, b);
      }
      v((m = m.apply(c, p || [])).next());
    });
  };
  Object.defineProperty(ce, "__esModule", { value: !0 }), ce.ensureEndpointIdFormat = e, ce.parseEndpointId = n, ce.resolveEndpointPath = i, ce.isValidUrl = o, ce.throttle = a, ce.isReact = l, ce.isPlainObject = d, ce.sleep = u;
  function e(c) {
    if (c.split("/").length > 1)
      return c;
...
function Kr() {
  return ho || (ho = 1, (function(t) {
    var e = Gt && Gt.__awaiter || function(s, l, d, u) {
      function c(p) {
        return p instanceof d ? p : new d(function(f) {
          f(p);
        });
...
var zt = {};
const Hd = "@fal-ai/client", Gd = "1.9.4", zd = {
  name: Hd,
  version: Gd
};
var go;
function Si() {
  if (go) return zt;
  go = 1, Object.defineProperty(zt, "__esModule", { value: !0 }), zt.isBrowser = t, zt.getUserAgent = r;
  function t() {
...
function Er() {
  if (_o) return Re;
  _o = 1;
  var t = Re && Re.__awaiter || function(l, d, u, c) {
    function p(f) {
  ...
  Object.defineProperty(Re, "__esModule", { value: !0 }), Re.dispatchRequest = a, Re.buildUrl = s;
  const r = Kr(), n = Si(), i = Je(), o = typeof navigator < "u" && (navigator == null ? void 0 : navigator.userAgent) === "Cloudflare-Workers";
  function a(l) {
    return t(this, void 0, void 0, function* () {
      var d;
      const { targetUrl: u, input: c, config: p, options: f = {} } = l, { credentials: m, requestMiddleware: y, responseHandler: h, fetch: g } = p, _ = Object.assign(Object.assign({}, p.retry), f.retry || {}), b = () => t(this, void 0, void 0, function* () {
        var E, S, x;
        const T = (0, n.isBrowser)() ? {} : { "User-Agent": (0, n.getUserAgent)() }, A = typeof m == "function" ? m() : m, { method: C, url: N, headers: M } = yield y({
          method: ((S = (E = l.method) !== null && E !== void 0 ? E : f.method) !== null && S !== void 0 ? S : "post").toUpperCase(),
          url: u,
          headers: l.headers
        }), B = A ? { Authorization: `Key ${A}` } : {}, H = Object.assign(Object.assign(Object.assign(Object.assign({}, B), { Accept: "application/json", "Content-Type": "application/json" }), T), M ?? {}), { responseHandler: G, retry: R } = f, P = e(f, ["responseHandler", "retry"]), V = yield g(N, Object.assign(Object.assign(Object.assign(Object.assign({}, P), { method: C, headers: Object.assign(Object.assign({}, H), (x = P.headers) !== null && x !== void 0 ? x : {}) }), !o && { mode: "cors" }), { signal: f.signal, body: C.toLowerCase() !== "get" && c ? JSON.stringify(c) : void 0 }));
        return yield (G ?? h)(V);
      });
      let v;
      for (let E = 0; E <= _.maxRetries; E++)
        try {
          return yield b();
        } catch (S) {
          if (v = S, E === _.maxRetries || !(0, r.isRetryableError)(S, _.retryableStatusCodes) || ((d = f.signal) === null || d === void 0 ? void 0 : d.aborted))
            throw S;
          const T = (0, r.calculateBackoffDelay)(E, _.baseDelay, _.maxDelay, _.backoffMultiplier, _.enableJitter);
          yield (0, i.sleep)(T);
        }
      throw v;
    });
  ...
  return wo || (wo = 1, (function(t) {
    var e = Wt && Wt.__awaiter || function(m, y, h, g) {
      ...
      return new (h || (h = Promise))(function(b, v) {
        ...
        function x(T) {
          T.done ? b(T.value) : _(T.value).then(E, S);
        }
    ...
    function c(m, y, h) {
      return e(this, arguments, void 0, function* (g, _, b, v = 3) {
        ...
        const { fetch: E, responseHandler: S } = b;
        try {
          const x = yield E(g, {
            method: "PUT",
            body: _
    ...
    function p(m, y, h) {
      return e(this, void 0, void 0, function* () {
        const { fetch: g, responseHandler: _ } = y, b = m.type || "application/octet-stream", { upload_url: v, file_url: E } = yield u(m, y, b, h), S = 10 * 1024 * 1024, x = Math.ceil(m.size / S), T = new URL(v), A = [];
        for (let M = 0; M < x; M++) {
          const B = M * S, H = Math.min(B + S, m.size), G = m.slice(B, H), R = M + 1, P = `${T.origin}${T.pathname}/${R}${T.search}`;
          A.push(yield c(P, G, y));
        }
        const C = `${T.origin}${T.pathname}/complete${T.search}`, N = yield g(C, {
          method: "POST",
          headers: {
    ...
    function f({ config: m }) {
      const y = {
        upload: (h, g) => e(this, void 0, void 0, function* () {
          ...
          const b = h.type || "application/octet-stream", { fetch: v, responseHandler: E } = m, { upload_url: S, file_url: x } = yield d(h, m, b, _), T = yield v(S, {
            method: "PUT",
            body: h,
            headers: {
        ...
        transformInput: (h) => e(this, void 0, void 0, function* () {
          ...
          if ((0, i.isPlainObject)(h)) {
            const _ = Object.entries(h).map((v) => e(this, [v], void 0, function* ([E, S]) {
              return [E, yield y.transformInput(S)];
...
function Wd() {
  if (bo) return kr;
  bo = 1, Object.defineProperty(kr, "__esModule", {
    value: !0
  });
  function t(n) {
    let i, o, a, s, l, d, u;
...
` && ++h, g = !1);
        let _ = -1, b = s, v;
        for (let E = a; _ < 0 && E < y; ++E)
          v = o[E], v === ":" && b < 0 ? b = E - h : v === "\r" ? (g = !0, _ = E - h) : v === `
` && (_ = E - h);
        if (_ < 0) {
          a = y - h, s = b;
          break;
    ...
    function f(m, y, h, g) {
      ...
      const E = y + v, S = g - v, x = m.slice(E, E + S).toString();
      if (b === "data")
        u += x ? "".concat(x, `
`) : `
`;
  ...
  return kr.createParser = t, kr;
}
var Xt = {}, vo;
function ds() {
  return vo || (vo = 1, (function(t) {
    var e = Xt && Xt.__awaiter || function(a, s, l, d) {
      function u(c) {
...
function us() {
  ...
  var t = ge && ge.__awaiter || function(p, f, m, y) {
    ...
    function _(A) {
      return function(C) {
        return Promise.resolve(C).then(A, x);
      };
    }
    function b(A, C) {
      y[A] && (h[A] = function(N) {
    ...
    function E(A) {
      A.value instanceof e ? Promise.resolve(A.value.v).then(S, x) : T(g[0][2], A);
    }
    function S(A) {
      v("next", A);
    }
    ...
    function T(A, C) {
      A(C), g.shift(), g.length && v(g[0][0], g[0][1]);
    }
  };
  Object.defineProperty(ge, "__esModule", { value: !0 }), ge.FalStream = void 0, ge.createStreamingClient = c;
  const n = /* @__PURE__ */ Wd(), i = ds(), o = Er(), a = lt(), s = Je(), l = "text/event-stream", d = 15 * 1e3;
  class u {
    constructor(f, m, y) {
      var h;
      this.listeners = /* @__PURE__ */ new Map(), this.buffer = [], this.currentData = void 0, this.lastEventTimestamp = 0, this.streamClosed = !1, this._requestId = null, this.abortController = new AbortController(), this.start = () => t(this, void 0, void 0, function* () {
        var g, _, b;
        const { endpointId: v, options: E } = this, { input: S, method: x = "post", connectionMode: T = "server", tokenProvider: A } = E;
        try {
          if (T === "client") {
            const C = (0, s.ensureEndpointIdFormat)(v), N = (g = (0, s.resolveEndpointPath)(v, void 0, "/stream")) !== null && g !== void 0 ? g : "", B = yield (A ? () => A(`${C}${N}`) : () => (console.warn('[fal.stream] Using the default token provider is deprecated. Please provide a `tokenProvider` function when using `connectionMode: "client"`. See https://docs.fal.ai/fal-client/authentication for more information.'), (0, i.getTemporaryAuthToken)(v, this.config)))(), { fetch: H } = this.config, G = new URL(this.url);
            G.searchParams.set("fal_jwt_token", B);
            const R = yield H(G.toString(), {
              method: x.toUpperCase(),
              headers: {
                accept: (_ = E.accept) !== null && _ !== void 0 ? _ : l,
                "content-type": "application/json"
              },
          ...
          return yield (0, o.dispatchRequest)({
            ...
            options: {
              headers: {
                accept: (b = E.accept) !== null && b !== void 0 ? b : l
              },
              responseHandler: (C) => t(this, void 0, void 0, function* () {
                return this._requestId = C.headers.get("x-fal-request-id"), yield this.handleResponse(C);
              }),
              signal: this.abortController.signal
            }
          });
        ...
        const S = new TextDecoder("utf-8"), x = g.body.getReader(), T = (0, n.createParser)((N) => {
          if (N.type === "event") {
            const M = N.data;
            try {
        ...
        }), A = (b = this.options.timeout) !== null && b !== void 0 ? b : d, C = () => t(this, void 0, void 0, function* () {
          const { value: N, done: M } = yield x.read();
          this.lastEventTimestamp = Date.now(), T.feed(S.decode(N)), Date.now() - this.lastEventTimestamp > A && this.emit("error", new a.ApiError({
            message: `Event stream timed out after ${(A / 1e3).toFixed(0)} seconds with no messages.`,
            status: 408,
            requestId: this._requestId || void 0
          })), M ? this.emit("done", this.currentData) : C().catch(this.handleError);
        });
        C().catch(this.handleError);
      }), this.handleError = (g) => {
        var _;
...
function Xd() {
  if (To) return je;
  To = 1;
  var t = je && je.__awaiter || function(f, m, y, h) {
    function g(_) {
    ...
    return new (y || (y = Promise))(function(_, b) {
      ...
      function S(x) {
        x.done ? _(x.value) : g(x.value).then(v, E);
      }
      S((h = h.apply(f, m || [])).next());
    });
  }, e = je && je.__rest || function(f, m) {
    var y = {};
  ...
  Object.defineProperty(je, "__esModule", { value: !0 }), je.createQueueClient = void 0;
  const r = Ti(), n = Er(), i = lt(), o = Kr(), a = ls(), s = us(), l = Je(), d = 500, u = {
    maxRetries: 3,
    baseDelay: 1e3,
    maxDelay: 6e4,
    ...
    const y = {
      submit(h, g) {
        return t(this, void 0, void 0, function* () {
          const { webhookUrl: _, priority: b, hint: v, startTimeout: E, headers: S, storageSettings: x } = g, T = e(g, ["webhookUrl", "priority", "hint", "startTimeout", "headers", "storageSettings"]), A = g.input ? yield m.transformInput(g.input) : void 0, C = Object.fromEntries(Object.entries(S ?? {}).map(([N, M]) => [
            N.toLowerCase(),
            M
          ]));
          return (0, n.dispatchRequest)({
            method: g.method,
            targetUrl: (0, n.buildUrl)(h, Object.assign(Object.assign({}, T), { subdomain: "queue", query: _ ? { fal_webhook: _ } : void 0 })),
            headers: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, C), (0, a.buildObjectLifecycleHeaders)(x)), { [r.QUEUE_PRIORITY_HEADER]: b ?? "normal" }), v && { [r.RUNNER_HINT_HEADER]: v }), (0, r.buildTimeoutHeaders)(E)),
            input: A,
            config: f,
            options: {
      ...
      status(h, g) {
        return t(this, arguments, void 0, function* (_, { requestId: b, logs: v = !1, abortSignal: E }) {
      ...
      streamStatus(h, g) {
        return t(this, arguments, void 0, function* (_, { requestId: b, logs: v = !1, connectionMode: E }) {
          const S = (0, l.parseEndpointId)(_), x = S.namespace ? `${S.namespace}/` : "", T = {
            logs: v ? "1" : "0"
          }, A = (0, n.buildUrl)(`${x}${S.owner}/${S.alias}`, {
            subdomain: "queue",
      ...
      subscribeToStatus(h, g) {
        return t(this, void 0, void 0, function* () {
          ...
          if (g.mode === "streaming") {
            ...
            b && (v = setTimeout(() => {
              throw S.abort(), y.cancel(h, { requestId: _ }).catch(E), new Error(`Client timed out waiting for the request to complete after ${b}ms`);
            }, b)), S.on("data", (A) => {
            ...
            const T = yield S.done();
            return v && clearTimeout(v), T;
          }
          return new Promise((S, x) => {
            var T;
            let A;
            const C = "pollInterval" in g && typeof g.pollInterval == "number" && (T = g.pollInterval) !== null && T !== void 0 ? T : d, N = () => {
              v && clearTimeout(v), A && clearTimeout(A);
            };
            b && (v = setTimeout(() => {
              N(), y.cancel(h, { requestId: _ }).catch(E), x(new Error(`Client timed out waiting for the request to complete after ${b}ms`));
            }, b));
      ...
      result(h, g) {
        return t(this, arguments, void 0, function* (_, { requestId: b, abortSignal: v }) {
          const E = (0, l.parseEndpointId)(_), S = E.namespace ? `${E.namespace}/` : "";
          return (0, n.dispatchRequest)({
            method: "get",
            targetUrl: (0, n.buildUrl)(`${S}${E.owner}/${E.alias}`, {
              subdomain: "queue",
              path: `/requests/${b}`
            }),
  ...
  return je.createQueueClient = p, je;
}
var ft = {};
function Jd(t) {
  const e = t.length;
...
const Yd = new TextEncoder(), Qd = 50;
function Zd(t, e, r) {
  Yd.encodeInto(t, e.subarray(r));
}
function eu(t, e, r) {
  t.length > Qd ? Zd(t, e, r) : Kd(t, e, r);
}
const tu = 4096;
function cs(t, e, r) {
  let n = e;
  ...
  for (; n < i; ) {
    ...
    if ((s & 128) === 0)
      ...
      o.push(s);
    o.length >= tu && (a += String.fromCharCode(...o), o.length = 0);
  }
  return o.length > 0 && (a += String.fromCharCode(...o)), a;
}
const ru = new TextDecoder(), nu = 200;
function iu(t, e, r) {
  const n = t.subarray(e, e + r);
  return ru.decode(n);
}
function ou(t, e, r) {
  return r > nu ? iu(t, e, r) : cs(t, e, r);
}
class rr {
  constructor(e, r) {
    U(this, "type");
...
const ms = -1, lu = 4294967296 - 1, du = 17179869184 - 1;
function hs({ sec: t, nsec: e }) {
  if (t >= 0 && e >= 0 && t <= du)
    if (e === 0 && t <= lu) {
    ...
    return n.setUint32(0, e), fs(n, 4, t), r;
  }
}
function gs(t) {
...
function _s(t) {
  ...
  switch (t.byteLength) {
    ...
    case 12: {
      const r = ps(e, 4), n = e.getUint32(0);
      return { sec: r, nsec: n };
    }
    default:
...
U(Xr, "defaultCodec", new Xr());
let vr = Xr;
function cu(t) {
  return t instanceof ArrayBuffer || typeof SharedArrayBuffer < "u" && t instanceof SharedArrayBuffer;
}
function Xn(t) {
  return t instanceof Uint8Array ? t : ArrayBuffer.isView(t) ? new Uint8Array(t.buffer, t.byteOffset, t.byteLength) : cu(t) ? new Uint8Array(t) : Uint8Array.from(t);
}
const fu = 100, pu = 2048;
class Yr {
  constructor(e) {
    ...
    U(this, "bytes");
    U(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? vr.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.maxDepth = (e == null ? void 0 : e.maxDepth) ?? fu, this.initialBufferSize = (e == null ? void 0 : e.initialBufferSize) ?? pu, this.sortKeys = (e == null ? void 0 : e.sortKeys) ?? !1, this.forceFloat32 = (e == null ? void 0 : e.forceFloat32) ?? !1, this.ignoreUndefined = (e == null ? void 0 : e.ignoreUndefined) ?? !1, this.forceIntegerToFloat = (e == null ? void 0 : e.forceIntegerToFloat) ?? !1, this.pos = 0, this.view = new DataView(new ArrayBuffer(this.initialBufferSize)), this.bytes = new Uint8Array(this.view.buffer);
  }
  clone() {
    return new Yr({
  ...
  encodeString(e) {
    const n = Jd(e);
    this.ensureBufferSizeToWrite(5 + n), this.writeStringHeader(n), eu(e, this.bytes, this.pos), this.pos += n;
  }
  encodeObject(e, r) {
  ...
  writeU64(e) {
    this.ensureBufferSizeToWrite(8), au(this.view, this.pos, e), this.pos += 8;
  }
  writeI64(e) {
    this.ensureBufferSizeToWrite(8), fs(this.view, this.pos, e), this.pos += 8;
  }
  writeBigUint64(e) {
...
function mu(t, e) {
  return new Yr(e).encodeSharedRef(t);
}
function In(t) {
...
class yu {
  ...
  decode(e, r, n) {
    ...
    if (i != null)
      return this.hit++, i;
    this.miss++;
    const o = cs(e, r, n), a = Uint8Array.prototype.slice.call(e, r, r + n);
    return this.store(a, o), o;
  }
}
const Jn = "array", ar = "map_key", bs = "map_value", _u = (t) => {
...
class wu {
  ...
  pushArrayState(e) {
    ...
    r.type = Jn, r.position = 0, r.size = e, r.array = new Array(e);
  }
  pushMapState(e) {
    const r = this.getUninitializedStateFromPool();
...
const Kt = -1, xi = new DataView(new ArrayBuffer(0)), bu = new Uint8Array(xi.buffer);
try {
  xi.getInt8(0);
} catch (t) {
...
const So = new RangeError("Insufficient data"), vu = new yu();
class Ke {
  constructor(e) {
    U(this, "extensionCodec");
    U(this, "context");
    ...
    U(this, "stack", new wu());
    U(this, "entered", !1);
    this.extensionCodec = (e == null ? void 0 : e.extensionCodec) ?? vr.defaultCodec, this.context = e == null ? void 0 : e.context, this.useBigInt64 = (e == null ? void 0 : e.useBigInt64) ?? !1, this.rawStrings = (e == null ? void 0 : e.rawStrings) ?? !1, this.maxStrLength = (e == null ? void 0 : e.maxStrLength) ?? Jt, this.maxBinLength = (e == null ? void 0 : e.maxBinLength) ?? Jt, this.maxArrayLength = (e == null ? void 0 : e.maxArrayLength) ?? Jt, this.maxMapLength = (e == null ? void 0 : e.maxMapLength) ?? Jt, this.maxExtLength = (e == null ? void 0 : e.maxExtLength) ?? Jt, this.keyDecoder = (e == null ? void 0 : e.keyDecoder) !== void 0 ? e.keyDecoder : vu, this.mapKeyConverter = (e == null ? void 0 : e.mapKeyConverter) ?? _u;
  }
  clone() {
    return new Ke({
      extensionCodec: this.extensionCodec,
  ...
  appendBuffer(e) {
    if (this.headByte === Kt && !this.hasRemaining(1))
      ...
      const r = this.bytes.subarray(this.pos), n = Xn(e), i = new Uint8Array(r.length + n.length);
  ...
  *decodeMulti(e) {
  ...
  doDecodeSync() {
    e: for (; ; ) {
      ...
      if (e >= 224)
        ...
        throw new le(`Unrecognized type byte: ${In(e)}`);
      this.complete();
      const n = this.stack;
      for (; n.length > 0; ) {
        const i = n.top();
        if (i.type === Jn)
          ...
          i.key = this.mapKeyConverter(r), i.type = bs;
          continue e;
  ...
  readArraySize() {
    ...
    switch (e) {
      ...
      default: {
        if (e < 160)
          return e - 144;
        throw new le(`Unrecognized array type byte: ${In(e)}`);
      }
    }
  }
  pushMapState(e) {
  ...
  decodeUtf8String(e, r) {
    ...
    const n = this.pos + r;
    let i;
    return this.stateIsMapKey() && ((o = this.keyDecoder) != null && o.canBeCached(e)) ? i = this.keyDecoder.decode(this.bytes, n, e) : i = ou(this.bytes, n, e), this.pos += r + e, i;
  }
  stateIsMapKey() {
  ...
  readU64() {
    const e = su(this.view, this.pos);
    return this.pos += 8, e;
  }
  readI64() {
    const e = ps(this.view, this.pos);
    return this.pos += 8, e;
  }
  readU64AsBigInt() {
...
const ju = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  ExtensionCodec: vr,
  decode: Eu,
  decodeArrayStream: Au,
  decodeAsync: xu,
  decodeMulti: Tu,
  decodeMultiStream: ku,
  decodeTimestampExtension: ws,
  decodeTimestampToTimeSpec: _s,
  encode: mu,
  encodeDateToTimeSpec: gs,
  encodeTimeSpecToTimestamp: hs,
  encodeTimestampExtension: ys
}, Symbol.toStringTag, { value: "Module" })), Ru = /* @__PURE__ */ Vd(ju);
var fe = {}, Io;
function Ou() {
  if (Io) return fe;
  Io = 1, Object.defineProperty(fe, "__esModule", { value: !0 });
  function t(k) {
  ...
  let r = {}, n = () => !0, i = () => ({}), o = (k) => k, a = (k, O, q, L) => k.apply(q, L) && O.apply(q, L), s = (k, O, q, [L, X]) => O.call(q, k.call(q, L, X), X), l = (k, O) => Object.freeze(Object.create(k, O));
  function d(k, O, q) {
    return k.reduce((L, X) => function(...ne) {
      return q(L, X, this, ne);
    }, O);
  }
  function u(k) {
    return l(this, { fn: t(k) });
  }
  let c = {}, p = u.bind(c), f = (k) => p((O, q) => !!~k(O, q) && O), m = {}, y = u.bind(m);
  function h(k, O) {
    return O.filter((q) => k.isPrototypeOf(q));
  }
  function g(k, O, ...q) {
    let L = d(h(m, q).map((ne) => ne.fn), n, a), X = d(h(c, q).map((ne) => ne.fn), o, s);
    return l(this, {
      from: t(k),
      to: t(O),
      guards: t(L),
  ...
  let _ = {}, b = {}, v = g.bind(_), E = g.bind(b, null);
  function S(k, O, q) {
    return G(O, k, q, this.immediates) || k;
  }
  function x(k) {
    let O = /* @__PURE__ */ new Map();
    for (let q of k)
      O.has(q.from) || O.set(q.from, []), O.get(q.from).push(q);
    return O;
  }
  let T = { enter: o };
  function A(...k) {
    let O = h(_, k), q = h(b, k), L = {
      final: t(k.length === 0),
      transitions: t(x(O))
    };
    return q.length && (L.immediates = t(q), L.enter = t(S)), l(T, L);
  }
  let C = {
    enter(k, O, q) {
      let L = this.fn.call(O, O.context, q);
      return B.isPrototypeOf(L) ? l(N, {
        machine: t(L),
        transitions: t(this.transitions)
      }).enter(k, O, q) : (L.then((X) => O.send({ type: "done", data: X })).catch((X) => O.send({ type: "error", error: X })), k);
    }
  }, N = {
    enter(k, O, q) {
      if (O.child = V(this.machine, (L) => {
        O.onChange(L), O.child == L && L.machine.state.value.final && (delete O.child, O.send({ type: "done", data: L.context }));
      }, O.context, q), O.child.machine.state.value.final) {
        let L = O.child.context;
        return delete O.child, G(O, k, { type: "done", data: L }, this.transitions.get("done"));
      }
      return k;
    }
  };
  ...
  function H(k, O, q = i) {
    return typeof k != "string" && (q = O || i, O = k, k = Object.keys(O)[0]), r._create && r._create(k, O), l(B, {
      context: t(q),
      current: t(k),
      states: t(O)
    });
  ...
  function G(k, O, q, L) {
    let { context: X } = k;
    for (let { to: ne, guards: Ae, reducers: de } of L)
      if (Ae(X, q)) {
        k.context = de.call(k, X, q);
        let Le = O.original || O, Mt = l(Le, {
          current: t(ne),
          original: { value: Le }
        });
        return r._onEnter && r._onEnter(O, ne, k.context, X, q), Mt.state.value.enter(Mt, k, q);
      }
  }
  function R(k, O) {
    let q = O.type || O, { machine: L } = k, { value: X, name: ne } = L.state;
    return X.transitions.has(q) ? G(k, L, O, X.transitions.get(q)) || L : (r._send && r._send(q, ne), L);
  }
  let P = {
    send(k) {
  ...
  function V(k, O, q, L) {
    let X = Object.create(P, {
      machine: e(k),
      context: e(k.context(q, L)),
      onChange: t(O)
    });
    return X.send = X.send.bind(X), X.machine = X.machine.state.value.enter(X.machine, X, L), X;
  }
  return fe.action = f, fe.createMachine = H, fe.d = r, fe.guard = y, fe.immediate = E, fe.interpret = V, fe.invoke = M, fe.reduce = p, fe.state = A, fe.transition = v, fe;
}
var xo;
function Pu() {
  if (xo) return ft;
  xo = 1;
  var t = ft && ft.__awaiter || function(R, P, V, k) {
    function O(q) {
      return q instanceof V ? q : new V(function(L) {
        L(q);
      });
    }
    return new (V || (V = Promise))(function(q, L) {
      function X(de) {
        try {
          Ae(k.next(de));
        } catch (Le) {
      ...
      function Ae(de) {
        de.done ? q(de.value) : O(de.value).then(X, ne);
      }
      Ae((k = k.apply(R, P || [])).next());
    });
  };
  Object.defineProperty(ft, "__esModule", { value: !0 }), ft.createRealtimeClient = G;
  const e = Ru, r = Ou(), n = ds(), i = lt(), o = Si(), a = Je(), s = () => ({
  ...
  function u(R, P) {
    return Object.assign(Object.assign({}, R), { enqueuedMessage: P.message });
  }
  function c(R) {
  ...
  function p(R, P) {
    return R.websocket && R.websocket.readyState === WebSocket.OPEN ? (P.message instanceof Uint8Array || typeof P.message == "string" ? R.websocket.send(P.message) : R.websocket.send((0, e.encode)(P.message)), Object.assign(Object.assign({}, R), { enqueuedMessage: void 0 })) : Object.assign(Object.assign({}, R), { enqueuedMessage: P.message });
  }
  function f(R) {
    return Object.assign(Object.assign({}, R), { token: void 0 });
  }
  function m(R, P) {
    return Object.assign(Object.assign({}, R), { token: P.token });
  }
  function y(R, P) {
    return Object.assign(Object.assign({}, R), { websocket: P.websocket });
  }
  ...
  function g(R, { token: P, maxBuffering: V, path: k }) {
    var O;
    if (V !== void 0 && (V < 1 || V > 60))
      throw new Error("The `maxBuffering` must be between 1 and 60 (inclusive)");
    const q = new URLSearchParams({
      fal_jwt_token: P
    });
    V !== void 0 && q.set("max_buffering", V.toFixed(0));
    const L = (0, a.ensureEndpointIdFormat)(R), X = (O = (0, a.resolveEndpointPath)(R, k, "/realtime")) !== null && O !== void 0 ? O : "";
    return `wss://fal.run/${L}${X}?${q.toString()}`;
  }
  const _ = 128;
  function b(R) {
  ...
  const v = {
    NORMAL_CLOSURE: 1e3
  }, E = /* @__PURE__ */ new Map(), S = /* @__PURE__ */ new Map();
  function x(R, P, V) {
    if (!E.has(R)) {
      const k = (0, r.interpret)(h, V);
      E.set(R, Object.assign(Object.assign({}, k), { throttledSend: P > 0 ? (0, a.throttle)(k.send, P, !0) : k.send }));
    }
    return E.get(R);
  }
  const T = () => {
  ...
  function M(R) {
    return t(this, void 0, void 0, function* () {
      if (typeof R == "string")
        return JSON.parse(R);
      const P = (V) => t(this, void 0, void 0, function* () {
        return V instanceof Uint8Array ? V : V instanceof Blob ? new Uint8Array(yield V.arrayBuffer()) : new Uint8Array(V);
      });
      return R instanceof ArrayBuffer || R instanceof Uint8Array ? (0, e.decode)(yield P(R)) : R instanceof Blob ? (0, e.decode)(yield P(R)) : R;
    });
  }
  function B(R) {
    return R instanceof Uint8Array ? R : (0, e.encode)(R);
  }
  function H({ data: R, decodeMessage: P, onResult: V, onError: k, send: O }) {
    const q = (L) => {
      if (b(L)) {
        O({
          type: "unauthorized",
    ...
    Promise.resolve(P ? P(R) : R).then(q).catch((L) => {
      var X;
      k(new i.ApiError({
  ...
  function G({ config: R }) {
    return {
      connect(P, V) {
        ...
        if (k && !(0, o.isBrowser)())
          return A;
        const Mt = ne ?? ((Ye) => B(Ye)), dn = Ae ?? ((Ye) => M(Ye));
        let Tr, to, Dt, $t = 0;
        S.set(O, {
          decodeMessage: dn,
          onError: V.onError,
        ...
        const un = () => S.get(O), Ft = x(O, X, ({ context: Ye, machine: Qe, send: Ue }) => {
          var cn;
          const { enqueuedMessage: fn, token: pn, websocket: mn } = Ye;
          if (to = fn, Qe.current === "active" && fn && (mn == null ? void 0 : mn.readyState) === WebSocket.OPEN && Ue({ type: "send", message: fn }), Qe.current === "authRequired" && pn === void 0 && Tr !== Qe.current) {
            Ue({ type: "initiateAuth" }), $t++;
            const Ie = $t, ue = (0, a.ensureEndpointIdFormat)(P), be = (cn = (0, a.resolveEndpointPath)(P, L, "/realtime")) !== null && cn !== void 0 ? cn : "", Ze = de ? () => de(`${ue}${be}`) : () => (console.warn("[fal.realtime] Using the default token provider is deprecated. Please provide a `tokenProvider` function to `fal.realtime.connect()`. See https://docs.fal.ai/model-apis/client#client-side-usage-with-token-provider for more information."), (0, n.getTemporaryAuthToken)(P, R)), Bt = de ? Le : n.TOKEN_EXPIRATION_SECONDS, hn = Bt !== void 0 ? () => {
              clearTimeout(Dt);
          ...
          if (Qe.current === "connecting" && Tr !== Qe.current && pn !== void 0) {
            const Ie = new WebSocket(g(P, { token: pn, maxBuffering: q, path: L }));
            Ie.onopen = () => {
              var ue, be;
              Ue({ type: "connected", websocket: Ie });
              const Ze = (be = (ue = Ft.context) === null || ue === void 0 ? void 0 : ue.enqueuedMessage) !== null && be !== void 0 ? be : to;
              Ze && (Ie.send(Mt(Ze)), Ft.context = Object.assign(Object.assign({}, Ft.context), { enqueuedMessage: void 0 }));
            }, Ie.onclose = (ue) => {
              if (ue.code !== v.NORMAL_CLOSURE) {
                const { onError: be = T } = un();
                be(new i.ApiError({
                  message: `Error closing the connection: ${ue.reason}`,
                  status: ue.code
                }));
              }
              Ue({ type: "connectionClosed", code: ue.code });
            }, Ie.onerror = (ue) => {
              const { onError: be = T } = un();
              be(new i.ApiError({ message: "Unknown error", status: 500 }));
            }, Ie.onmessage = (ue) => {
              const { decodeMessage: be = dn, onResult: Ze, onError: Bt = T } = un();
              H({
                data: ue.data,
                decodeMessage: be,
                onResult: Ze,
          ...
          Tr === "active" && Qe.current !== "active" && (clearTimeout(Dt), Dt = void 0), Tr = Qe.current;
        });
        return {
          send: (Ye) => {
            Ft.throttledSend({
...
function ko() {
  if (Ao) return ct;
  Ao = 1;
  var t = ct && ct.__awaiter || function(u, c, p, f) {
    function m(y) {
  ...
  Object.defineProperty(ct, "__esModule", { value: !0 }), ct.createFalClient = d;
  const e = Ii(), r = Ti(), n = Xd(), i = Pu(), o = Er(), a = lt(), s = ls(), l = us();
  function d(u = {}) {
...
function Nu() {
  if (jo) return Yt;
  jo = 1, Object.defineProperty(Yt, "__esModule", { value: !0 }), Yt.isQueueStatus = t, Yt.isCompletedQueueStatus = e;
  function t(r) {
...
function Cu() {
  return Ro || (Ro = 1, (function(t) {
    var e = et && et.__createBinding || (Object.create ? (function(d, u, c, p) {
      p === void 0 && (p = c);
    ...
    })), r = et && et.__exportStar || function(d, u) {
...
const qu = /* @__PURE__ */ JSON.parse('[{"display_name":"3D Rigging","job_set_type":"3d_rigging","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height_meters","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true}]},{"display_name":"Brain Activity","job_set_type":"brain_activity","type":"text","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Bytedance Image Upscale","job_set_type":"bytedance_image_upscale","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"4k","required":false,"enum":["2k","4k"]}]},{"display_name":"Bytedance Video Upscale","job_set_type":"bytedance_video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"fps","type":"integer","default":24,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"model_version","type":"string","default":"standard","required":false,"enum":["standard","pro"]},{"name":"preset","type":"string","default":"common","required":false,"enum":["common","aigc","short_series","ugc","old_film"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1080p","2k","4k"]}]},{"display_name":"Cinematic Studio 2.5","job_set_type":"cinematic_studio_2_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"auto","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio 3.0","job_set_type":"cinematic_studio_3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Cinematic Studio Image","job_set_type":"cinematic_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"string","default":null,"required":true},{"name":"camera_lens_id","type":"string","default":null,"required":true},{"name":"camera_model_id","type":"string","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Cinematic Studio Soul Cast","job_set_type":"cinematic_studio_soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinematic Studio Soul Location","job_set_type":"cinematic_studio_soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Cinematic Studio Video","job_set_type":"cinematic_studio_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"slow_motion","type":"boolean","default":false,"required":false},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Cinematic Studio Video 3.5","job_set_type":"cinematic_studio_video_3_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_focal_length_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"camera_style","type":"object","default":null,"required":false},{"name":"color_grading","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"light_scheme","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"style_id","type":"object","default":null,"required":false},{"name":"style_prompt","type":"object","default":null,"required":false}]},{"display_name":"Cinema Studio 4.0","job_set_type":"cinematic_studio_video_4_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"camera_aperture_id","type":"object","default":null,"required":false},{"name":"camera_lens_id","type":"object","default":null,"required":false},{"name":"camera_model_id","type":"object","default":null,"required":false},{"name":"color_palette","type":"object","default":null,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"era_id","type":"object","default":null,"required":false},{"name":"extension_mode","type":"object","default":null,"required":false},{"name":"film_era","type":"null","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"null","default":null,"required":false},{"name":"genre_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"light","type":"object","default":null,"required":false},{"name":"light_custom","type":"object","default":null,"required":false},{"name":"light_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"model","type":"string","default":"default","required":false,"enum":["default","video_edit","video_extension"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"pacing_id","type":"object","default":null,"required":false},{"name":"preset_id","type":"null","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"prompt_language","type":"string","default":"zh","required":false,"enum":["en","zh"]},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"speedramp","type":"object","default":"auto","required":false},{"name":"use_blur","type":"boolean","default":false,"required":false},{"name":"use_eye_mask","type":"boolean","default":false,"required":false},{"name":"use_transparency","type":"boolean","default":false,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Cinematic Studio Video V2","job_set_type":"cinematic_studio_video_v2","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"cfg_scale","type":"number","default":0.5,"required":false},{"name":"duration","type":"integer","default":5,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","western","suspense","intimate","spectacle"]},{"name":"kling_element_ids","type":"array","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std"]},{"name":"multi_prompt","type":"array","default":null,"required":false},{"name":"multi_shot_mode","type":"string","default":"custom","required":false,"enum":["auto","custom"]},{"name":"multi_shots","type":"boolean","default":false,"required":false},{"name":"preset_id","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]},{"name":"speedramp","type":"object","default":"auto","required":false}]},{"display_name":"Clipify","job_set_type":"clipify","type":"video","params":[{"name":"clip_aspect","type":"string","default":"9:16","required":false,"enum":["9:16","1:1","16:9"]},{"name":"clips_num","type":"integer","default":10,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"max_height","type":"integer","default":1080,"required":false},{"name":"segment_seconds","type":"integer","default":10,"required":false},{"name":"subtitle_case","type":"string","default":"as-is","required":false,"enum":["lower","upper","as-is"]},{"name":"subtitle_font","type":"string","default":"notosans","required":false},{"name":"subtitle_highlight_hex","type":"string","default":"#FFE84D","required":false},{"name":"subtitle_position","type":"string","default":"bottom","required":false,"enum":["bottom","center","top"]},{"name":"track_face_crop","type":"boolean","default":true,"required":false},{"name":"urls","type":"array","default":null,"required":true}]},{"display_name":"Draw To Video","job_set_type":"draw_to_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"ref_image","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"sketch","type":"object","default":null,"required":true},{"name":"video","type":"object","default":null,"required":true}]},{"display_name":"dubbing","job_set_type":"dubbing","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"target_language","type":"string","default":null,"required":true,"enum":["eng","cmn","fra","hin","ita","jpn","kor","por","rus","tur","spa","deu","ara","pol","ind","fil","swe","fin"]}]},{"display_name":"Explainer Video","job_set_type":"explainer_video","type":"video","params":[{"name":"height","type":"integer","default":null,"required":true},{"name":"items","type":"array","default":null,"required":true},{"name":"subtitles","type":"object","default":null,"required":false},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"FLUX.2","job_set_type":"flux_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"pro","required":false,"enum":["pro","flex","max"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"FLUX.2 Pro Outpaint","job_set_type":"flux_2_pro_outpaint","type":"image","params":[{"name":"expand_bottom","type":"integer","default":0,"required":false},{"name":"expand_left","type":"integer","default":0,"required":false},{"name":"expand_right","type":"integer","default":0,"required":false},{"name":"expand_top","type":"integer","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"FLUX 3 Video","job_set_type":"flux_3_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","2:1","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Flux Kontext","job_set_type":"flux_kontext","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Gemini Omni Flash","job_set_type":"gemini_omni","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"integer","default":8,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false}]},{"display_name":"GPT Image 2","job_set_type":"gpt_image_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"high","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Grok Image","job_set_type":"grok_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","1:2","2:1","3:2","2:3","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","quality"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Grok Video","job_set_type":"grok_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Grok Video 1.5","job_set_type":"grok_video_v15","type":"video","params":[{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Happy Horse Video","job_set_type":"happy_horse_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Hunyuan 3D v3.1 Text to 3D","job_set_type":"hunyuan3d_v3_1_text_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Hunyuan3D v3 Image to 3D","job_set_type":"hunyuan3d_v3_image_to_3d","type":"3d","params":[{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"face_count","type":"integer","default":500000,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"generate_type","type":"string","default":"Normal","required":false,"enum":["Normal","LowPoly","Geometry"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"polygon_type","type":"string","default":"triangle","required":false,"enum":["triangle","quadrilateral"]}]},{"display_name":"Image Auto","job_set_type":"image_auto","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Image Background Remover","job_set_type":"image_background_remover","type":"image","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Image Decompose","job_set_type":"image_decompose","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"granular","required":false,"enum":["granular","standard"]}]},{"display_name":"Image to 3D","job_set_type":"image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Inworld Text to Speech","job_set_type":"inworld_text_to_speech","type":"audio","params":[{"name":"prompt","type":"string","default":null,"required":true},{"name":"voice","type":"string","default":null,"required":true}]},{"display_name":"Kimodo","job_set_type":"kimodo","type":"3d","params":[{"name":"diffusion_steps","type":"integer","default":10,"required":false},{"name":"duration","type":"object","default":null,"required":false},{"name":"durations","type":"object","default":null,"required":false},{"name":"enhancer","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_version","type":"string","default":"ardy-core","required":false,"enum":["ardy-core","ardy-core-h8"]},{"name":"prompt","type":"object","default":null,"required":false},{"name":"prompts","type":"object","default":null,"required":false},{"name":"seed","type":"integer","default":42,"required":false}]},{"display_name":"Kling O1 Image","job_set_type":"kling_omni_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","auto","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Kling 2.6 Video","job_set_type":"kling2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"boolean","default":true,"required":false}]},{"display_name":"Kling v3.0","job_set_type":"kling3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["pro","std","4k"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sound","type":"string","default":"on","required":false,"enum":["on","off"]}]},{"display_name":"Kling 3.0 Motion Control","job_set_type":"kling3_0_motion_control","type":"video","params":[{"name":"background_source","type":"string","default":"input_image","required":false,"enum":["input_image","input_video"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","pro"]}]},{"display_name":"Kling 3.0 Turbo","job_set_type":"kling3_0_turbo","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"LLM Generation","job_set_type":"llm_text","type":"video","params":[{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":null,"required":true},{"name":"reasoning_effort","type":"object","default":null,"required":false},{"name":"system_prompt","type":"string","default":"","required":false},{"name":"user_prompt","type":"string","default":"","required":false}]},{"display_name":"Marketing Studio Image","job_set_type":"marketing_studio_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Marketing Studio Video","job_set_type":"marketing_studio_video","type":"video","params":[{"name":"ad_reference_id","type":"object","default":null,"required":false},{"name":"aspect_ratio","type":"string","default":"9:16","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"avatar_ids","type":"array","default":null,"required":false},{"name":"avatars","type":"array","default":null,"required":false},{"name":"duration","type":"integer","default":15,"required":false},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"hook_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"ugc","required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"setting_id","type":"object","default":null,"required":false},{"name":"specific_mode","type":"string","default":"default","required":false,"enum":["default","web_product","from_storyboard"]},{"name":"storyboard_id","type":"object","default":null,"required":false},{"name":"web_product_ids","type":"array","default":null,"required":false},{"name":"web_product_type","type":"object","default":null,"required":false}]},{"display_name":"Meshy 5 Remesh","job_set_type":"meshy_v5_remesh","type":"3d","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"model_url","type":"string","default":null,"required":true},{"name":"origin_at","type":"object","default":null,"required":false},{"name":"resize_height","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Meshy 6 Text to 3D","job_set_type":"meshy_v6_text_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"boolean","default":false,"required":false},{"name":"enable_prompt_expansion","type":"boolean","default":false,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"mode","type":"string","default":"full","required":false},{"name":"model_type","type":"string","default":"standard","required":false},{"name":"pose_mode","type":"string","default":"","required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"boolean","default":true,"required":false},{"name":"symmetry_mode","type":"string","default":"auto","required":false},{"name":"target_polycount","type":"integer","default":30000,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"string","default":"triangle","required":false}]},{"display_name":"MiniMax H3","job_set_type":"minimax_h3","type":"video","params":[{"name":"aigc_watermark","type":"boolean","default":false,"required":false},{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"duration","type":"integer","default":4,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"integer","default":null,"required":true},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":"","required":false},{"name":"resolution","type":"string","default":"2K","required":false,"enum":["768P","2K"]},{"name":"width","type":"integer","default":null,"required":true}]},{"display_name":"Minimax Hailuo","job_set_type":"minimax_hailuo","type":"video","params":[{"name":"duration","type":"string","default":6,"required":false,"enum":["6","10"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"model","type":"string","default":"minimax-2.3","required":false,"enum":["minimax","minimax-fast","minimax-2.3","minimax-2.3-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"768","required":false,"enum":["512","768","1080"]}]},{"display_name":"Mirelo Text to Audio","job_set_type":"mirelo_text_to_audio","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"MS Image","job_set_type":"ms_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"avatars","type":"array","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"brand_kit_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"product_ids","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"low","required":false,"enum":["low","medium","high"]},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Multi-Image to 3D","job_set_type":"multi_image_to_3d","type":"3d","params":[{"name":"animation_action_id","type":"object","default":null,"required":false},{"name":"enable_animation","type":"boolean","default":false,"required":false},{"name":"enable_pbr","type":"object","default":null,"required":false},{"name":"enable_rigging","type":"boolean","default":false,"required":false},{"name":"enable_safety_checker","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"pose_mode","type":"object","default":null,"required":false},{"name":"rigging_height_meters","type":"object","default":null,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"should_remesh","type":"object","default":null,"required":false},{"name":"should_texture","type":"boolean","default":false,"required":false},{"name":"symmetry_mode","type":"object","default":null,"required":false},{"name":"target_polycount","type":"object","default":null,"required":false},{"name":"texture_image_url","type":"object","default":null,"required":false},{"name":"texture_prompt","type":"object","default":null,"required":false},{"name":"topology","type":"object","default":null,"required":false}]},{"display_name":"Nano Banana","job_set_type":"nano_banana","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_ai_stylist","type":"image","params":[{"name":"background_preset_id","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"outfit_preset_ids","type":"array","default":null,"required":false},{"name":"pose_preset_id","type":"object","default":null,"required":false},{"name":"user_outfit_ids","type":"array","default":null,"required":false}]},{"display_name":"Nano Banana 2 Lite","job_set_type":"nano_banana_2_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false},{"name":"thinking","type":"string","default":"HIGH","required":false,"enum":["MINIMAL","HIGH"]}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_relight","type":"image","params":[{"name":"brightness","type":"integer","default":null,"required":true},{"name":"color","type":"string","default":null,"required":true},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"light_quality","type":"string","default":null,"required":true,"enum":["hard","sharp","soft"]},{"name":"light_source","type":"string","default":null,"required":true,"enum":["mdl","mdr","mul","mur","bml","fml","fmr","bmm","mml","mmr","fmm","bmr","mdm","mum","bdr","fdl","bur","ful","bdl","fdr","bul","fur","bdm","fdm","bum","fum"]},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_shots","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_images","type":"array","default":null,"required":true}]},{"display_name":"Nano Banana Pro","job_set_type":"nano_banana_2_skin_enhancer","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"preset_id","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false}]},{"display_name":"Nano Banana 2","job_set_type":"nano_banana_flash","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k","4k"]}]},{"display_name":"OpenAI Hazel","job_set_type":"openai_hazel","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:2","2:3","auto"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"medium","required":false,"enum":["low","medium","high"]}]},{"display_name":"Outpaint","job_set_type":"outpaint","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"21:9","required":false,"enum":["auto","1:1","3:2","2:3","4:3","3:4","4:5","5:4","9:16","16:9","21:9"]},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Qwen Audio 3.0 TTS Flash","job_set_type":"qwen_audio_tts","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"instruction","type":"object","default":null,"required":false},{"name":"language","type":"object","default":null,"required":false},{"name":"pitch_rate","type":"number","default":1,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","22050","24000","44100","48000"]},{"name":"seed","type":"integer","default":0,"required":false},{"name":"speech_rate","type":"number","default":1,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"integer","default":50,"required":false}]},{"display_name":"Angles","job_set_type":"qwen_camera_control","type":"image","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"height","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"move_forward_level","type":"integer","default":0,"required":false},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"rotate_degree","type":"integer","default":0,"required":false},{"name":"vertical_angle","type":"integer","default":0,"required":false},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Recraft V4.1","job_set_type":"recraft_v4_1","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","3:4","4:3","4:5","5:4","3:2","2:3","16:9","9:16","21:9"]},{"name":"background_color","type":"object","default":null,"required":false},{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"colors","type":"array","default":null,"required":false},{"name":"model_type","type":"string","default":"standard","required":false,"enum":["standard","vector","utility","utility_vector"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"1k","required":false,"enum":["1k","2k"]}]},{"display_name":"Reframe","job_set_type":"reframe","type":"video","params":[{"name":"aspect_ratio","type":"string","default":null,"required":true,"enum":["21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"3D Objects","job_set_type":"sam_3_3d","type":"3d","params":[{"name":"detection_threshold","type":"object","default":null,"required":false},{"name":"export_textured_glb","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"3D Body","job_set_type":"sam_3_3d_body","type":"3d","params":[{"name":"export_meshes","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"include_3d_keypoints","type":"boolean","default":true,"required":false},{"name":"include_mhr_params","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Remove Background","job_set_type":"sam_3_video","type":"video","params":[{"name":"apply_mask","type":"boolean","default":true,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"frames_count","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false}]},{"display_name":"Seed Audio 1.0","job_set_type":"seed_audio","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"expression_intensity","type":"integer","default":5,"required":false},{"name":"format","type":"string","default":"wav","required":false,"enum":["wav","mp3","pcm","ogg_opus"]},{"name":"loudness_rate","type":"integer","default":0,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"mood","type":"number","default":0,"required":false},{"name":"pitch_rate","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":24000,"required":false,"enum":["8000","16000","24000","32000","44100","48000"]},{"name":"speech_rate","type":"integer","default":0,"required":false},{"name":"voice_id","type":"object","default":null,"required":false},{"name":"voice_style","type":"object","default":null,"required":false},{"name":"voice_type","type":"object","default":null,"required":false},{"name":"voices","type":"array","default":null,"required":false}]},{"display_name":"Seedance 2.0","job_set_type":"seedance_2_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"std","required":false,"enum":["std","fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p","4k"]}]},{"display_name":"Seedance 2.0 Mini","job_set_type":"seedance_2_0_mini","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"genre","type":"string","default":"auto","required":false,"enum":["auto","action","horror","comedy","noir","drama","epic"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p"]}]},{"display_name":"Seedance 2.5","job_set_type":"seedance_2_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"audio_references","type":"array","default":null,"required":false},{"name":"bitrate_mode","type":"string","default":"standard","required":false,"enum":["standard","high"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"end_image","type":"object","default":null,"required":false},{"name":"extension_mode","type":"string","default":null,"required":false,"enum":["backward","forward"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"image_references","type":"array","default":null,"required":false},{"name":"mode","type":"string","default":"t2v","required":false,"enum":["t2v","omni_reference","video_edit","video_extension"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]},{"name":"start_image","type":"object","default":null,"required":false},{"name":"video_references","type":"array","default":null,"required":false}]},{"display_name":"Seedance 1.5 Pro","job_set_type":"seedance1_5","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["auto","16:9","9:16","4:3","3:4","1:1","21:9"]},{"name":"duration","type":"string","default":4,"required":false,"enum":["4","8","12"]},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Seedream 4.5","job_set_type":"seedream_v4_5","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["auto","1:1","4:3","16:9","3:2","21:9","3:4","9:16","2:3"]},{"name":"input_images","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Lite","job_set_type":"seedream_v5_lite","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","21:9"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high"]}]},{"display_name":"Seedream 5.0 Pro","job_set_type":"seedream_v5_pro","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16","3:2","2:3","21:9"]},{"name":"height","type":"object","default":null,"required":false},{"name":"is_inpaint","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"remove_bg","type":"boolean","default":false,"required":false},{"name":"resolution","type":"string","default":"2k","required":false,"enum":["1k","1.5k","2k"]},{"name":"width","type":"object","default":null,"required":false}]},{"display_name":"Sonilo Music","job_set_type":"sonilo_music","type":"audio","params":[{"name":"duration","type":"number","default":null,"required":true},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Soul Cast","job_set_type":"soul_cast","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","5:4","4:5","21:9","9:21"]},{"name":"budget","type":"integer","default":50,"required":false},{"name":"prompt","type":"object","default":null,"required":false}]},{"display_name":"soul_cinema_studio","job_set_type":"soul_cinema_studio","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"enhance_prompt","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"style_id","type":"object","default":null,"required":false}]},{"display_name":"Soul Cinematic","job_set_type":"soul_cinematic","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]}]},{"display_name":"Soul Location","job_set_type":"soul_location","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3","21:9","9:21"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Sync Lipsync 3","job_set_type":"sync_so","type":"video","params":[{"name":"active_speaker_detection","type":"boolean","default":false,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_audio","type":"object","default":null,"required":true},{"name":"input_video","type":"object","default":null,"required":true},{"name":"occlusion_detection_enabled","type":"boolean","default":false,"required":false},{"name":"sync_mode","type":"string","default":"bounce","required":false,"enum":["bounce","loop","cut_off","silence","remap"]},{"name":"temperature","type":"number","default":0.5,"required":false}]},{"display_name":"Higgsfield Soul 2.0","job_set_type":"text2image_soul_v2","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","16:9","9:16","4:3","3:4","3:2","2:3"]},{"name":"custom_reference_id","type":"object","default":null,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"2k","required":false,"enum":["1.5k","2k"]},{"name":"seed","type":"object","default":null,"required":false}]},{"display_name":"Text to Speech V2","job_set_type":"text2speech_v2","type":"audio","params":[{"name":"batch_size","type":"integer","default":1,"required":false},{"name":"emotion","type":"object","default":null,"required":false},{"name":"format","type":"string","default":"mp3","required":false,"enum":["mp3","wav"]},{"name":"language_boost","type":"string","default":"auto","required":false,"enum":["auto","af","ar","bg","ca","cs","da","de","el","en","es","fa","fi","fil","fr","he","hi","hr","hu","id","it","ja","ko","ms","nl","nn","no","pl","pt","ro","ru","sk","sl","sv","ta","th","tr","uk","vi","yue","zh"]},{"name":"model","type":"string","default":null,"required":true,"enum":["elevenlabs","minimax","seed_speech","vibe_voice","cozy_voice"]},{"name":"pitch","type":"integer","default":0,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"sample_rate","type":"string","default":32000,"required":false,"enum":["8000","16000","22050","24000","32000","44100"]},{"name":"speed","type":"number","default":1,"required":false},{"name":"stability","type":"object","default":null,"required":false},{"name":"text_normalization","type":"boolean","default":false,"required":false},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":null,"required":true,"enum":["preset","element"]},{"name":"volume","type":"number","default":1,"required":false}]},{"display_name":"Topaz","job_set_type":"topaz_image","type":"image","params":[{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"Standard V2","required":false,"enum":["Standard V2","Low Resolution V2","CGI","High Fidelity V2","Text Refine"]},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"sharpen","type":"number","default":0,"required":false}]},{"display_name":"Topaz","job_set_type":"topaz_image_generative","type":"image","params":[{"name":"autoprompt","type":"boolean","default":true,"required":false},{"name":"creativity","type":"integer","default":1,"required":false},{"name":"denoise","type":"number","default":0,"required":false},{"name":"face_enhancement","type":"boolean","default":false,"required":false},{"name":"face_enhancement_creativity","type":"number","default":0,"required":false},{"name":"face_enhancement_strength","type":"number","default":0,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"Redefine","required":false,"enum":["Standard MAX","Redefine","Recovery","Recovery V2"]},{"name":"output_height","type":"integer","default":null,"required":true},{"name":"output_width","type":"integer","default":null,"required":true},{"name":"prompt","type":"string","default":"","required":false},{"name":"sharpen","type":"number","default":0,"required":false},{"name":"texture","type":"integer","default":1,"required":false}]},{"display_name":"Topaz","job_set_type":"topaz_video","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","21:9","16:9","4:3","1:1","3:4","9:16"]},{"name":"duration","type":"object","default":null,"required":false},{"name":"enhancement","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"frame_interpolation","type":"object","default":null,"required":false},{"name":"frame_rate","type":"number","default":30,"required":false},{"name":"frames_count","type":"object","default":null,"required":false},{"name":"input_height","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":false},{"name":"input_video_size","type":"integer","default":0,"required":false},{"name":"input_width","type":"object","default":null,"required":false},{"name":"resolution","type":"string","default":"1080p","required":false,"enum":["1080p","2160p"]}]},{"display_name":"Text to 3D","job_set_type":"tripo_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"negative_prompt","type":"object","default":null,"required":false},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]}]},{"display_name":"Tripo H3.1 Image to 3D","job_set_type":"tripo_h3_1_image_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Tripo H3.1 Multiview to 3D","job_set_type":"tripo_h3_1_multiview_to_3d","type":"3d","params":[{"name":"auto_size","type":"boolean","default":false,"required":false},{"name":"face_limit","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"geometry_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"medias","type":"array","default":null,"required":true},{"name":"orientation","type":"string","default":"default","required":false,"enum":["default","align_image"]},{"name":"pbr","type":"boolean","default":true,"required":false},{"name":"quad","type":"boolean","default":false,"required":false},{"name":"seed","type":"object","default":null,"required":false},{"name":"texture","type":"boolean","default":true,"required":false},{"name":"texture_alignment","type":"string","default":"original_image","required":false,"enum":["original_image","geometry"]},{"name":"texture_quality","type":"string","default":"standard","required":false,"enum":["standard","detailed"]},{"name":"texture_seed","type":"object","default":null,"required":false}]},{"display_name":"Google Veo 3","job_set_type":"veo3","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"input_image","type":"object","default":null,"required":true},{"name":"model","type":"string","default":"veo-3-fast","required":false,"enum":["veo-3-preview","veo-3-fast"]},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Google Veo 3.1","job_set_type":"veo3_1","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"input_image","type":"object","default":null,"required":false},{"name":"model","type":"string","default":"veo-3-1-fast","required":false,"enum":["veo-3-1-preview","veo-3-1-fast"]},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"basic","required":false,"enum":["basic","high","ultra"]}]},{"display_name":"Google Veo 3.1 Lite","job_set_type":"veo3_1_lite","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","auto"]},{"name":"duration","type":"string","default":8,"required":false,"enum":["4","6","8"]},{"name":"generate_audio","type":"boolean","default":false,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true}]},{"display_name":"Video Background Remover","job_set_type":"video_background_remover","type":"video","params":[{"name":"medias","type":"array","default":null,"required":true}]},{"display_name":"Video Deflicker","job_set_type":"video_deflicker","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"Video Upscale","job_set_type":"video_upscale","type":"video","params":[{"name":"duration","type":"object","default":null,"required":false},{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true}]},{"display_name":"voice_change","job_set_type":"voice_change","type":"video","params":[{"name":"folder_id","type":"object","default":null,"required":false},{"name":"input_video","type":"object","default":null,"required":true},{"name":"voice_id","type":"string","default":null,"required":true},{"name":"voice_type","type":"string","default":"preset","required":false,"enum":["preset","element"]}]},{"display_name":"Wan 2.6 Video","job_set_type":"wan2_6","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1"]},{"name":"duration","type":"string","default":5,"required":false,"enum":["5","10","15"]},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"quality","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 2.7","job_set_type":"wan2_7","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"16:9","required":false,"enum":["16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["720p","1080p"]}]},{"display_name":"Wan 3.0","job_set_type":"wan3_0","type":"video","params":[{"name":"aspect_ratio","type":"string","default":"auto","required":false,"enum":["auto","16:9","9:16","1:1","4:3","3:4"]},{"name":"duration","type":"integer","default":5,"required":false},{"name":"enable_thinking","type":"boolean","default":false,"required":false},{"name":"generate_audio","type":"boolean","default":true,"required":false},{"name":"medias","type":"array","default":null,"required":false},{"name":"prompt","type":"string","default":null,"required":true},{"name":"resolution","type":"string","default":"720p","required":false,"enum":["480p","720p","1080p"]}]},{"display_name":"Z Image","job_set_type":"z_image","type":"image","params":[{"name":"aspect_ratio","type":"string","default":"1:1","required":false,"enum":["1:1","4:3","3:4","16:9","9:16"]},{"name":"prompt","type":"string","default":null,"required":true}]}]'), Lu = {
  ...
  "sketch",
  "texture_image_url"
]), $u = /* @__PURE__ */ new Set(["input_images"]), Fu = /* @__PURE__ */ new Set(["input_video", "video"]), Bu = /* @__PURE__ */ new Set(["input_audio"]);
function ki(t) {
  return t.split(/[_-]+/).filter(Boolean).map((e) => e.charAt(0).toUpperCase() + e.slice(1)).join(" ");
}
function Vu(t) {
  return Mu[t] ?? `hf-${t.replaceAll("_", "-")}`;
}
function Hu(t) {
...
const Ju = ["prompt", "user_prompt", "instruction"];
function Ku(t) {
  for (const e of Ju) {
...
function Qu(t = vs) {
  const e = {};
  for (const r of t) {
    const n = Vu(r.job_set_type), i = Hu(r.type);
    if (e[n]) throw new Error(`Duplicate Higgsfield node type: ${n}`);
    const o = Ku(r.params.flatMap((s) => [
      Wu(r, zu(r, s)),
      ...Xu(r, s)
    ])), a = o.findIndex((s) => s.id === "prompt") + 1;
    o.splice(a, 0, ...Yu(r)), e[n] = {
      id: r.job_set_type,
      nodeType: n,
      name: r.display_name,
      category: i,
      ...
      inputs: o,
      outputType: i,
      outputs: [{ id: i, portType: i, label: i === "model3d" ? "3D Model" : ki(i) }],
      provider: "higgsfield",
...
const Zu = {
  ...
  video: "--video",
  audio: "--audio"
}, Ts = /^[A-Za-z][A-Za-z0-9_]*$/, Ss = /* @__PURE__ */ new Set(["json", "wait", "no_color"]);
function ec(t) {
  const e = ["generate", "create", t.model], r = {
    ...Es(t.model, { ...t.extra, ...t.params }) ?? {}
  }, n = (o, a) => {
    if (a == null) return;
    if (!Ts.test(o) || Ss.has(o))
  ...
  for (const o of t.medias ?? [])
    o.value && e.push(Zu[o.role], o.value);
...
const tc = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;
function rc(t) {
...
function Ri(t) {
  ...
  const i = nc(n), o = i.job_id ?? i.id ?? i.jobId;
  return {
...
`);
  ...
  return (r = e.match(tc)) == null ? void 0 : r[0];
}
function ze(t) {
  return !!t && typeof t == "object" && !Array.isArray(t);
}
const ic = /* @__PURE__ */ new Set(["params", "prompt", "input_images", "inputs", "extra", "request"]), Po = [
  "url",
  "urls",
  "video_url",
  "image_url",
...
function wt(t, e = 0) {
  ...
  for (const [n, i] of Object.entries(t))
    ic.has(n) || Po.includes(n) || No.includes(n) || n === "result_json" || (ze(i) || Array.isArray(i)) && r.push(...wt(i, e + 1));
  return [...new Set(r)];
}
const oc = /https?:\/\/[^\s"'<>\\]+/gi;
function Is(t) {
...
function Oi(t, e) {
  var l;
  const r = wt(t.parsed), n = r[0], i = Cr(t.parsed), o = t.record.duration ?? ((l = t.record.output) == null ? void 0 : l.duration), a = e.mediaType, s = r.map((d) => ({ kind: a, url: d }));
...
function As(t) {
  return t instanceof Kn ? { stdout: t.stdout, stderr: t.stderr } : { stdout: "", stderr: "" };
}
...
const Co = 1260 * 1e3, uc = 9e4, qo = 4, xn = "Higgsfield CLI not found. Install @higgsfield/cli, then run higgsfield auth login — or connect Higgsfield in Settings.";
function cc(t) {
...
function fc(t, e, r) {
  return new Promise((n, i) => {
    var d, u;
    const o = ae(t, e, { env: dc() });
    let a = "", s = "";
    const l = setTimeout(() => {
      o.kill("SIGTERM"), i(new Kn("Higgsfield CLI timed out", a, s));
    }, r);
...
async function qe(t, e = 6e4) {
  const r = t.includes("--json") ? t : [...t, "--json"], n = lc(), i = pt ? [pt, ...n.filter((l) => l !== pt)] : n;
  if (i.length === 0) throw new Error(xn);
  let o;
  for (const l of i)
    try {
      const d = await fc(l, r, e);
      return pt = l, d;
    } catch (d) {
      if (cc(d)) {
        pt === l && (pt = null), o = d;
...
function mc(t, e, r) {
  const n = e ? ks[e.split(";", 1)[0].trim().toLowerCase()] : void 0;
  if (n) return n;
  const i = w.extname(t.split(/[?#]/, 1)[0]).toLowerCase();
  return i && i.length <= 5 ? i : js(r);
}
async function hc(t) {
  ...
  for (const [n, i] of t.entries()) {
    ...
    if (/^https?:\/\//i.test(o)) {
      const a = await fetch(o, { signal: AbortSignal.timeout(pc) });
      if (!a.ok) throw new Error(`Failed to download media input (HTTP ${a.status}): ${o}`);
      const s = mc(o, a.headers.get("content-type"), i.role), l = w.join(z.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${s}`);
      ...
      const s = o.slice(5, a), l = s.replace(/;base64$/i, ""), d = o.slice(a + 1), u = /;base64$/i.test(s) ? Buffer.from(d, "base64") : Buffer.from(decodeURIComponent(d)), c = ks[l.toLowerCase()] ?? js(i.role), p = w.join(z.tmpdir(), `cinegen-hf-media-${Date.now()}-${n}${c}`);
      await D.promises.writeFile(p, u), e.push(p), r.push({ ...i, value: p });
    } else
...
async function Zn(t) {
  const { medias: e, tempPaths: r } = await hc(t.medias);
  let n;
  try {
    n = await gc({ ...t, medias: e });
  } finally {
    for (const o of r)
      D.promises.unlink(o).catch(() => {
      });
  }
  if (ji(n, t.mediaType)) return n;
...
async function gc(t) {
  ...
  for (let n = 1; n <= 3; n++)
    try {
      const i = await qe(e, uc), o = await Qn(i, t);
      if (o) return o;
      const a = Oo(i);
      if (a) return { jobId: a };
      ...
      r = i;
      const { stdout: o, stderr: a } = As(i), s = await Qn(o, t);
      if (s) return s;
      const l = Oo(o, a, Tt(i));
      if (l) return { jobId: l };
      if (!Yn(Tt(i)) || n === 3) throw i;
      await Rs(1500 * n);
    }
...
async function Os(t, e) {
  try {
    const r = await Ns({ size: 50 }), n = yc(r, t);
    if (!n) return;
    const i = Oi({
...
async function Pi(t, e) {
  const r = await qe(["generate", "get", t], 2e4), n = Ri(r);
  ...
  const o = Is(r);
  return o[0] ? { ...i, url: o[0], urls: o, outputs: o.map((a) => ({ kind: e.mediaType, url: a })) } : Os(t, e);
}
async function _c(t) {
  try {
    return await qe(
    ...
    if (!/unknown|unexpected|unrecognized/i.test(Tt(e))) throw e;
    return qe(
...
async function Ps(t, e) {
  ...
  for (let i = 1; i <= qo; i++)
    try {
      ...
      r = o;
      const { stdout: a } = As(o), s = await Qn(a, e);
      if (s) return s;
      try {
        const u = await Pi(t, e);
        if (u) return u;
      } catch (u) {
        if (!Yn(Tt(u))) throw u;
      }
      const l = Tt(o);
      if (!(Yn(l) || /timed out/i.test(l) || /still running/i.test(l) || /without a media URL/i.test(l))) throw o;
      if (i === qo) break;
      await Rs(2e3 * i);
    }
  ...
  if (n) return n;
  throw new Error(
    `${Tt(r)} The job was submitted (${t}) and may still finish on Higgsfield.`
  );
}
function Lo(t) {
...
async function Ns(t) {
  ...
  try {
    ...
    return i < 0 ? [] : Lo(JSON.parse(n.slice(i)));
  }
}
async function wc(t, e) {
  const r = await Pi(t, e);
  return r && ji(r, e.mediaType) ? r : Ps(t, e);
}
function bc(t, e) {
  const r = ["generate", "cost", t];
  for (const [n, i] of Object.entries(Es(t, e) ?? {}))
    i == null || i === "" || !Ts.test(n) || Ss.has(n) || (typeof i == "string" ? r.push(`--${n}`, i) : typeof i == "number" && Number.isFinite(i) ? r.push(`--${n}`, String(i)) : typeof i == "boolean" && r.push(`--${n}`, i ? "true" : "false"));
...
function Tc() {
  I.handle("higgsfield:account-status", async () => Mo(await Uo())), I.handle("higgsfield:quick-edit", async (t, e) => {
    const { prepareClipReference: r, resolveLocalSourcePath: n } = await Promise.resolve().then(() => Aa);
    ...
    if (e.drawnFramePath && e.referenceMode === "frame")
      ...
      try {
        ...
        console.log("[higgsfield:quick-edit] extracted refs:", s.paths), i = Do({
    ...
    const { resolveLocalSourcePath: r } = await Promise.resolve().then(() => Aa);
    if (e.jobId) {
      const i = { model: e.model, mediaType: e.outputType };
      if (e.wait === !1) {
        const o = await Pi(e.jobId, i);
        if (o && ji(o, e.outputType)) return o;
        throw new Error("Higgsfield job is still running");
      }
      return wc(e.jobId, i);
    }
  ...
  }), I.handle("higgsfield:generate-list", async (t, e) => Ns(e)), I.handle("higgsfield:generate-cost", async (t, e) => Ec(e == null ? void 0 : e.model, (e == null ? void 0 : e.params) ?? {})), I.handle("higgsfield:auth-login", async () => {
    try {
...
}, Ac = "notrius/ltx-2.5-serverless:cu130@sha256:73d1621ef915ae6a149f2a32f6c317dfc89f12075ed4b3abd7df707420267205", Ni = "https://rest.runpod.io/v1", kc = "https://api.runpod.io/v2", Cs = "https://api.runpod.io/graphql", Ci = 8e3, An = 256 * 1024, jc = 1800, Rc = 200, ei = 15e3, qs = 6500, Oc = 12e4, qi = 12e3, Ls = 14 * 1024 * 1024, Pc = 100 * 1024 * 1024, $o = 1024 * 1024, Fo = 4, Us = Object.freeze(["sdxl", "qwen-image-edit"]), Nc = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABACAIAAAAlC+aJAAAAYElEQVR4nO3PQQ0AIBDAMMD4WUcEj4ZkVbDtmVk/OzrgVQNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgNaA1oDWgPaBRFyAf0dnk7yAAAAAElFTkSuQmCC", Cc = "balanced", Bo = Object.freeze({
...
async function Ms(t, e, r = {}, n = ei) {
  ...
  let s;
  const l = new Li(n), d = new Promise((c, p) => {
    s = setTimeout(() => {
      i.abort(l), p(l);
    }, n);
  }), u = (async () => {
    const c = await t(e, { ...r, signal: i.signal }), p = await Lc(c);
...
function Uc(t) {
  try {
    return new URL(t).pathname === "/health" ? qs : ei;
  } catch {
    return ei;
  }
}
...
async function Se(t, e, r, n, i = [200, 201, 202, 204], o = Uc(e)) {
  let a;
  try {
    a = await Ms(t, e, r, o);
  } catch (d) {
    throw d instanceof Li ? new F(`${n} RunPod did not respond before the request timed out.`, "PROVIDER_TIMEOUT", 504) : new F(d instanceof Error ? d.message : n, "PROVIDER_UNREACHABLE", 502);
  }
  const { response: s, payload: l } = a;
  if (!i.includes(s.status))
    throw new F(Zr(l, `${n} (${s.status})`), "PROVIDER_ERROR", s.status);
  return l;
}
function Ui(t, e = !1) {
  return {
...
async function Dc(t, e, r) {
  var a;
  const n = new AbortController(), i = setTimeout(() => n.abort(), jc);
  let o;
  try {
    const s = new URL(`${kc}/pods/${encodeURIComponent(r)}/logs`);
    ...
    for (; !c.done && u.length < An && (u += d.decode(c.value, { stream: !0 }), !(u.length >= An)); ) {
      let p;
      const f = new Promise((m) => {
        p = setTimeout(() => m({ quiet: !0 }), Rc);
      });
      if (c = await Promise.race([o.read(), f]), clearTimeout(p), c != null && c.quiet)
        break;
    }
    return u += d.decode(), Mc(u.slice(0, An));
  } catch {
    return [];
  } finally {
...
async function Vo(t, e, r, n) {
  const i = `mutation { secretCreate(input: { name: ${JSON.stringify(r)}, value: ${JSON.stringify(n)}, description: "Temporary CineGen LTX-2.5 session credential" }) { id name } }`, o = new URL(Cs);
  o.searchParams.set("api_key", e);
  const a = await Se(t, o.toString(), {
    method: "POST",
  ...
  if (s.length || typeof (d == null ? void 0 : d.id) != "string")
    throw new F(Zr(a, "RunPod could not create the encrypted session secret."));
  return d.id;
}
async function Ds(t, e, r) {
  const n = `mutation { secretDelete(id: ${JSON.stringify(r)}) }`, i = new URL(Cs);
  i.searchParams.set("api_key", e), await Se(t, i.toString(), {
    method: "POST",
    headers: { "Content-Type": "application/json", Accept: "application/json" },
...
function zc(t) {
  return `https://${t}-${Ci}.proxy.runpod.net`;
}
function Mi(t, e) {
  const r = Qr(e, "RunPod session ID"), n = new URL(Te(t, "RunPod session URL"));
  if (n.protocol !== "https:" || n.username || n.password || n.hostname !== `${r}-${Ci}.proxy.runpod.net`)
...
function Jc(t) {
  ...
  const e = [];
  for (const r of t) {
    if (typeof r != "string" || !Us.includes(r))
...
async function Yc(t, e = fetch) {
  const r = Te(t.runpodKey, "RunPod API key"), n = Te(t.huggingFaceToken, "Hugging Face read token");
  if (!/^hf_[A-Za-z0-9]+$/.test(n))
    throw new F("Enter a valid Hugging Face read token.", "INVALID_HUGGINGFACE_TOKEN", 422);
  const i = Xc(t.gpuProfile), o = Jc(t.imageModels), a = Wc(), s = `cinegen_ltx25_hf_${a}`, l = `cinegen_ltx25_session_${a}`, d = `${crypto.randomUUID()}${crypto.randomUUID()}`.replace(/-/g, ""), u = [];
  try {
    u.push(await Vo(e, r, s, n)), u.push(await Vo(e, r, l, d));
    const c = await Se(e, `${Ni}/pods`, {
      method: "POST",
      headers: Ui(r, !0),
      body: JSON.stringify({
        ...
        containerDiskInGb: Kc(i.config, o),
        volumeInGb: 0,
        ports: [`${Ci}/http`],
        supportPublicIp: !0,
        interruptible: !1,
    ...
    }, "RunPod could not create the LTX-2.5 session Pod."), p = $s(c);
    if (!p.id)
    ...
    throw await Promise.allSettled(u.map((p) => Ds(e, r, p))), c;
  }
}
async function Qc(t, e = fetch) {
  const r = Te(t.runpodKey, "RunPod API key"), n = Te(t.podAuthToken, "RunPod session token"), i = Mi(t.podUrl, t.podId);
  let o;
  try {
    o = await Se(e, `${Ni}/pods/${i.podId}`, {
      headers: Ui(r)
  ...
  if (a.desiredStatus === "RUNNING")
    try {
      const { response: c, payload: p } = await Ms(e, `${i.url}/health`, {
      ...
      s = c instanceof Li ? { kind: "timeout" } : { kind: "unreachable" };
    }
  const l = await Dc(e, r, i.podId), d = Bc(l);
  if (!d && $c(l))
    try {
      const c = await Fs({
        runpodKey: r,
  ...
  let u;
  return (s == null ? void 0 : s.kind) === "timeout" ? u = "RunPod reports the Pod is running, but its private gateway did not answer within 7 seconds. It may still be starting; check again shortly. Billing continues while the Pod runs." : (s == null ? void 0 : s.kind) === "response" && (s.status === 502 || s.status === 504) ? u = `RunPod reports the Pod is running, but its private gateway returned ${s.status}. The container may still be starting; check again shortly. Billing continues while the Pod runs.` : u = Hc(s == null ? void 0 : s.body) ?? Gc(l), !u && (s == null ? void 0 : s.kind) === "unreachable" ? u = "RunPod reports the Pod is running, but its private gateway is not reachable yet. The container may still be starting; check again shortly. Billing continues while the Pod runs." : !u && (s == null ? void 0 : s.kind) === "response" && s.status >= 400 && (u = `RunPod reports the Pod is running, but its private gateway returned HTTP ${s.status}. Check again shortly. Billing continues while the Pod runs.`), {
    status: "downloading",
    phase: "downloading",
    podId: i.podId,
    podUrl: i.url,
...
async function Fs(t, e = fetch) {
  const r = Te(t.runpodKey, "RunPod API key"), n = Qr(t.podId, "RunPod session ID");
  await Se(e, `${Ni}/pods/${n}`, {
    method: "DELETE",
    headers: Ui(r)
  ...
  return (await Promise.allSettled(i.map((s) => Ds(e, r, s)))).filter((s) => s.status === "rejected").length ? { ok: !0, warning: "The Pod was deleted and billing stopped, but one temporary RunPod secret could not be removed." } : { ok: !0 };
}
function Zc(t, e) {
...
function ef(t) {
  const e = JSON.parse(JSON.stringify(Sc)), r = Te(t.prompt, "Video prompt");
  if (r.length > qi)
    throw new F("The LTX-2.5 video prompt is too long.", "PROMPT_TOO_LONG", 422);
  const n = Math.min(20, Math.max(1, Math.round(Number(t.durationSec) || 5))), i = ["16:9", "9:16", "1:1"].includes(t.aspectRatio ?? "") ? t.aspectRatio : "16:9", o = t.resolution === "480p" || t.resolution === "1080p" ? t.resolution : "720p", a = Zc(i, o);
  return e["398:376"].inputs.value = r, e[395].inputs.image = "cinegen-source.png", e["398:362"].inputs.value = n, e["398:372"].inputs.value = a.width, e["398:360"].inputs.value = a.height, e["398:361"].inputs.value = 24, e["398:380"].inputs.sampling_mode = "on", e["398:380"].inputs["sampling_mode.seed"] = Math.floor(Math.random() * 999999998) + 1, e["398:383"].inputs.value = t.generateAudio !== !1, e["398:363"].inputs.value = Bs(t).length === 0, e["398:338"].inputs.noise_seed = Math.floor(Math.random() * 999999999999998) + 1, e["398:339"].inputs.noise_seed = Math.floor(Math.random() * 999999999999998) + 1, e;
}
function tf(t) {
  const e = Bs(t), r = e.find((a) => a.startsWith("data:image/")) ?? (e.length ? "" : Nc), n = /^data:(image\/(?:png|jpeg|webp|gif|bmp|avif));base64,([A-Za-z0-9+/]*={0,2})$/i.exec(r);
...
function nf(t, e) {
  ...
  if (Math.floor(r[2].length * 0.75) - n > Ls)
...
function sf(t, e, r, n, i, o) {
  const a = JSON.parse(JSON.stringify(Ic));
...
function lf(t, e, r, n) {
  const i = JSON.parse(JSON.stringify(xc));
...
function df(t) {
  const e = Vs(t.model), r = af(t.prompt, "Image prompt"), n = typeof t.negativePrompt == "string" ? t.negativePrompt.trim().slice(0, qi) : e === "sdxl" ? "text, watermark, logo, low quality, distorted" : "", i = Go(t.width, 1024), o = Go(t.height, 1024), a = rf(t.seed), s = of(t, e), l = e === "sdxl" ? sf(t, r, n, i, o, a) : lf(r, n, a, s);
  return {
    model: e,
    label: en(e),
    workflow: l,
...
function uf(t, e) {
  if (e === "videos" || e === "video" || Di(t).startsWith("video/"))
    return !0;
  const n = t.filename ?? t.name;
  return typeof n == "string" && /\.(?:mp4|webm|mov|mkv|avi|m4v)(?:$|[?#])/i.test(n);
}
function cf(t, e) {
  if (e === "images" || e === "image" || Di(t).startsWith("image/"))
    return !0;
  const n = t.filename ?? t.name;
...
function ff(t, e) {
  ...
  for (const { record: n, parentKey: i } of tn(t)) {
    ...
    if (u)
      return { data: u, mediaType: $r(n), durationSec: e, model: "LTX-2.5" };
  }
...
function pf(t, e) {
  ...
  for (const { record: i, parentKey: o } of tn(t)) {
...
function Gs(t, e = "video", r = "LTX-2.5") {
  for (const { record: n, parentKey: i } of tn(t)) {
    if (i !== "artifact")
      continue;
...
function hf(t, e, r = "video", n = "LTX-2.5") {
...
async function zs(t, e, r, n, i = "video", o = "LTX-2.5") {
  const a = new Uint8Array(n.byteSize);
  let s;
  const l = [];
  for (let c = 0; c < n.byteSize; c += $o) {
    const p = Math.min($o, n.byteSize - c);
    l.push({ offset: c, length: p });
  }
  for (let c = 0; c < l.length; c += Fo) {
    const p = l.slice(c, c + Fo), f = await Promise.all(p.map(async ({ offset: m, length: y }) => {
      ...
      const g = await Se(t, h.toString(), {
        headers: { Authorization: `Bearer ${r}`, Accept: "application/json" }
      }, `CineGen could not download the ${o} ${i} chunk.`);
      if (!g || typeof g != "object")
        throw new F(`${o} returned an invalid ${i} chunk.`, "INVALID_PROVIDER_RESPONSE", 502);
      const _ = typeof g.id == "string" ? g.id : "", b = Number(g.offset), v = Number(g.byteSize ?? g.byte_size), E = typeof (g.mediaType ?? g.media_type) == "string" ? String(g.mediaType ?? g.media_type).trim() : "", S = mf(g.data, o, i);
      if (_ !== n.id || b !== m || v !== n.byteSize || n.mediaType && E !== n.mediaType || S.byteLength !== y)
  ...
  const d = hf(s ?? new Uint8Array(), n.mediaType, i, o), u = gf(a, i);
  return await yf(t, e, r, n.id), { data: u, mediaType: d };
}
function Ws(t) {
  if (!t || typeof t != "object")
    return !1;
...
async function Xs(t, e, r) {
  const n = await Se(t, `${e.url}/health`, {
    headers: { Authorization: `Bearer ${r}`, Accept: "application/json" }
...
async function _f(t, e, r, n) {
  const i = await Xs(t, e, r), o = i == null ? void 0 : i.capabilities, a = Array.isArray(i == null ? void 0 : i.installedModels) ? i.installedModels : [];
  ...
  if (!a.includes(n))
    throw new F(
      `${en(n)} was not installed when this Pod was created. Start a new session with that image model selected.`,
...
async function vf(t, e, r, n) {
  try {
    const i = await Se(t, `${e.url}/status/${n}`, {
      headers: { Authorization: `Bearer ${r}`, Accept: "application/json" }
...
async function Js(t, e, r, n, i, o, a) {
  var c;
  const s = wf(), l = ((c = i == null ? void 0 : i.capabilities) == null ? void 0 : c.idempotentSubmissions) === !0, d = JSON.stringify({ input: { ...o, cinegen_job_id: s } }), u = () => Se(t, `${e.url}/run`, {
    method: "POST",
...
async function Ef(t, e = fetch) {
  const r = Mi(t.podUrl, t.podId), n = Te(t.podAuthToken, "RunPod session token"), i = { Authorization: `Bearer ${n}`, Accept: "application/json", "Content-Type": "application/json" };
  if (t.jobId) {
    const d = Qr(t.jobId, "RunPod generation job ID"), u = await Se(e, `${r.url}/status/${d}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    ...
    if (c === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: Zr(u, "LTX-2.5 generation failed.") };
    if (c === "COMPLETED") {
      const p = Math.min(20, Math.max(1, Math.round(Number(u.durationSec) || 5)));
      try {
        const f = Gs(u.output), m = f ? { ...await zs(e, r, n, f), durationSec: p, model: "LTX-2.5" } : ff(u.output, p);
  ...
  const o = Math.min(20, Math.max(1, Math.round(Number(t.input.durationSec) || 5))), a = {
    workflow: ef(t.input),
    images: [{ name: "cinegen-source.png", image: tf(t.input) }],
    cinegen_duration_sec: o,
    cinegen_task: "ltx-2.5"
  }, s = await Xs(e, r, n), { jobId: l } = await Js(e, r, n, i, s, a, "CineGen could not submit the LTX-2.5 generation.");
  return { jobId: l, status: "queued", phase: "rendering", message: "LTX-2.5 generation queued." };
}
async function Tf(t, e = fetch) {
  var l;
  const r = Mi(t.podUrl, t.podId), n = Te(t.podAuthToken, "RunPod session token"), i = { Authorization: `Bearer ${n}`, Accept: "application/json", "Content-Type": "application/json" };
  if (t.jobId) {
    const d = Qr(t.jobId, "RunPod generation job ID"), u = await Se(e, `${r.url}/status/${d}`, {
      headers: { Authorization: `Bearer ${n}`, Accept: "application/json" }
    }, "CineGen could not read the session image generation status."), c = Ho(u == null ? void 0 : u.task), p = Ho(t.model ?? ((l = t.input) == null ? void 0 : l.model));
    if (c && p && c !== p)
    ...
    const m = en(f), y = String(u.status ?? "").toUpperCase();
    if (y === "IN_QUEUE")
    ...
    if (y === "FAILED")
      return { jobId: d, status: "failed", phase: "error", error: Zr(u, `${m} generation failed.`) };
    if (y === "COMPLETED")
      try {
        const h = Gs(u.output, "image", m), g = h ? { ...await zs(e, r, n, h, "image", m), model: m } : pf(u.output, f);
  ...
  const o = df(t.input), a = await _f(e, r, n, o.model), { jobId: s } = await Js(e, r, n, i, a, {
    workflow: o.workflow,
    images: o.images,
...
const Sf = Yc, If = Qc, xf = Fs, Af = Ef, kf = Tf, vt = "https://api.kie.ai/api/v1", jf = 3e3, Rf = 120, Of = {
...
function Pf(t) {
  for (const [e, r] of Object.entries(Of))
    if (t.startsWith(e)) return r;
}
async function Nf(t, e, r) {
  const n = Pf(t), i = n ?? `${vt}/jobs/createTask`, o = n ? { ...e, callBackUrl: "" } : { model: t, input: e, callBackUrl: "" }, a = await fetch(i, {
    method: "POST",
    headers: {
...
async function Cf(t, e) {
  for (let r = 0; r < Rf; r++) {
    await new Promise((a) => setTimeout(a, jf));
...
function sr(t, e, r = !1) {
  if (typeof t == "string") {
    const a = Ks(t).trim(), s = r ? Df(a, e) : e;
...
function Ff(t, e, r) {
  const n = [], i = {}, o = t === "seedance_2_5" ? "image" : $f(r);
  for (const [a, s] of Object.entries(e)) {
    if (s == null) continue;
...
async function Vf(t, e, r) {
  const n = await Zn(Ff(t, e, r));
  return Bf(n);
}
const zo = "https://api.runpod.ai/v2", Hf = 3e3, Gf = 120;
async function zf(t, e, r) {
  ...
  for (let o = 0; o < Gf; o++) {
    await new Promise((l) => setTimeout(l, Hf));
    const a = await fetch(`${zo}/${t}/status/${i}`, {
    ...
    if (s.status === "COMPLETED") {
      ...
      if (d && !d.startsWith("http") && !d.startsWith("local-media://")) {
        const u = d.includes(",") ? d.split(",")[1] : d, c = w.join(z.tmpdir(), `cinegen-runpod-${Date.now()}.png`);
...
const St = 14 * 1024 * 1024, Xo = 100 * 1024 * 1024, Jo = 100 * 1024 * 1024, Jf = 45e3;
function $i(t) {
...
async function Ys(t) {
  ...
  if (/^https?:\/\//i.test(t)) {
    const e = Kf(t), r = new AbortController(), n = setTimeout(() => r.abort(), Jf);
    let i;
    try {
...
async function Qf(t) {
  ...
  return {
    ...t,
    referenceImages: e ? [await Ys(e)] : void 0
  };
}
function Zf(t, e) {
  var a;
...
async function ep(t) {
  ...
  const r = await Promise.all(e.map(async (n, i) => {
    const o = n.trim().startsWith("data:") ? n : await Ys(n.trim());
    return Zf(o, i);
  }));
  return {
    ...t,
...
async function tp(t) {
  ...
  if (!r || r.length > Math.ceil(Xo / 3) * 4 + 8)
  ...
  if (n.byteLength > Xo) throw new Error("The LTX-2.5 video is larger than CineGen can import automatically.");
...
async function rp(t) {
  ...
  if (!n || n.length > Math.ceil(Jo / 3) * 4 + 8 || !/^[A-Za-z0-9+/]*={0,2}$/.test(n) || n.length % 4 === 1)
    throw new Error("RunPod returned an invalid or oversized image file.");
  const i = Buffer.from(n, "base64");
  if (!i.length || i.byteLength > Jo)
    throw new Error("RunPod returned an invalid or oversized image file.");
  const o = $i(i), a = o === "image/png" ? ".png" : o === "image/jpeg" ? ".jpg" : o === "image/webp" ? ".webp" : void 0;
...
async function Yo(t) {
  const e = decodeURIComponent(t.replace("local-media://file", "")), r = await j.readFile(e), n = np(e), i = new Blob([r], { type: n }), o = new File([i], w.basename(e), { type: n });
  return K.fal.storage.upload(o);
}
async function Fr(t) {
  const e = {};
  for (const [r, n] of Object.entries(t))
    typeof n == "string" && n.startsWith("local-media://file") ? e[r] = await Yo(n) : Array.isArray(n) ? e[r] = await Promise.all(
      n.map(async (i) => typeof i == "string" && i.startsWith("local-media://file") ? Yo(i) : i && typeof i == "object" && !Array.isArray(i) ? Fr(i) : i)
    ) : n && typeof n == "object" && !Array.isArray(n) ? e[r] = await Fr(n) : e[r] = n;
  return e;
}
async function Qo(t, e, r) {
  var n;
  ti(r), console.log("[fal] Calling model:", t, "with input:", JSON.stringify(e, null, 2));
  try {
...
function ip() {
  I.handle("workflow:run", async (e, r) => {
    const {
      ...
      outputType: c,
      inputs: p
    } = r, { ALL_MODELS: f, resolveVideoModelEndpoint: m, sanitizeVideoInputsForEndpoint: y } = await import("./models-De1bktfC.js"), h = f[u] ?? Object.values(f).find(
      (T) => T.id === u || T.altId === u || T.nodeType === u
    );
    if (!h) {
      if (u.startsWith("fal-ai/")) {
        const T = n;
        if (!T) throw new Error("No fal.ai API key provided. Add one in Settings.");
        ti(T);
        const A = await Fr(p), C = await Qo(u, A, T);
        return C.data ?? C;
      }
    ...
    const g = h.provider;
    let _ = p;
    g !== "higgsfield" && (n && ti(n), _ = await Fr(p));
    let b = u.includes("/") ? u : h.id;
    const v = h.nodeType ?? u, E = Object.keys(_).some(
      (T) => T === "image_url" || T === "start_image_url" || T === "image_urls" || T === "imageUrl"
    );
    b = m(v, h, {
      hasImageInputs: E,
      quality: _.quality
    ...
    if (g === "kie") {
      const T = i;
      if (!T) throw new Error("No kie.ai API key provided. Add one in Settings.");
      S = await qf(b, _, T);
    } else if (g === "pod") {
      if (!s) throw new Error("No pod URL configured. Start your pod and set the URL in Settings.");
      const T = h.podRoute ?? b;
      S = await Wf(s, T, _);
    } else if (g === "runpod") {
      const T = o;
      if (!T) throw new Error("No RunPod API key provided. Add one in Settings.");
      const A = a || h.runpodEndpointId || "";
      S = await zf(A, _, T);
    } else if (g === "higgsfield") {
      const T = h.outputType;
      S = await Vf(b, _, c ?? (T === "video" ? "video" : T === "audio" ? "audio" : T === "text" ? "text" : T === "3d" || T === "model3d" || T === "model" ? "3d" : "image"));
    } else {
      const T = n;
      if (!T) throw new Error("No fal.ai API key provided. Add one in Settings.");
      S = await Qo(b, _, T);
  ...
  I.handle("workflow:poll-job", async (e, r) => {
    ...
    return n;
  }), I.handle("pod:start", async (e, r) => await Wo(r.runpodKey, r.podId, "start")), I.handle("pod:stop", async (e, r) => await Wo(r.runpodKey, r.podId, "stop")), I.handle("pod:status", async (e, r) => await Xf(r.runpodKey, r.podId)), I.handle("pod:setup-ltx25", async (e, r) => await Sf(r)), I.handle("pod:status-ltx25", async (e, r) => await If(r)), I.handle("pod:terminate-ltx25", async (e, r) => await xf(r)), I.handle("pod:generate-ltx25", async (e, r) => {
    const n = r.input ? await Qf(r.input) : void 0;
    return await tp(await Af({ ...r, input: n }));
  }), I.handle("pod:generate-session-image", async (e, r) => {
    const n = r.input ? await ep(r.input) : void 0;
    return await rp(await kf({ ...r, input: n }));
  });
}
const Ot = Lt(Ce), Qs = "https://mcp.artlist.io/mcp", Pt = "artlist", op = 1200 * 1e3, ap = [
...
function dp(t) {
  ...
  for (const d of i) {
    const u = tt(d, ["url", "videoUrl", "video_url", "downloadUrl", "download_url", "mediaUrl", "media_url"]);
    ...
    return {
      ...
      ...tt(d, ["generationId", "generation_id", "id"]) ? { generationId: tt(d, ["generationId", "generation_id", "id"]) } : {},
      ...tt(d, ["accountUrl", "account_url", "sessionUrl", "session_url"]) ? { accountUrl: tt(d, ["accountUrl", "account_url", "sessionUrl", "session_url"]) } : {},
      ...tt(d, ["model", "modelId", "model_id"]) ? { model: tt(d, ["model", "modelId", "model_id"]) } : {}
    };
  }
  const o = (l = n.join(`
...
async function dr(t) {
  try {
    const { stdout: e, stderr: r } = await Ot(t, ["mcp", "get", Pt], {
      env: Nt(),
      timeout: 2e4
    }), n = `${e}
...
async function up(t) {
  (await dr(t)).configured || await Ot(t, [
    "mcp",
    "add",
    "--transport",
    "http",
...
function fp(t, e = process.platform, r = "/tmp/cinegen-artlist-login.command") {
  if (e === "darwin") {
    const n = [
      "#!/bin/zsh",
...
`);
  ...
  return { file: t, args: ["mcp", "login", Pt], detached: !1 };
}
function pp(t) {
...
async function hp(t, e) {
  ...
  const n = lp(e), { stdout: i } = await Ot(t, [
    "-p",
    n,
    "--output-format",
    "json",
...
function gp() {
  I.handle("artlist:account-status", async () => {
    const t = await jr();
    return t ? dr(t) : { connected: !1, configured: !1, error: "Claude Code is required for the Artlist MCP connection." };
    ...
    try {
      e.script && (await _d(e.script.path, e.script.contents, { mode: 448 }), await wd(e.script.path, 448)), await Ot(e.file, e.args, {
        env: Nt(),
        timeout: e.detached ? 2e4 : 300 * 1e3,
        maxBuffer: 2 * 1024 * 1024
      });
    } catch (r) {
      throw new Error(pp(r));
    }
    return e.detached ? mp(t) : dr(t);
  }), I.handle("artlist:auth-logout", async () => {
    const t = await jr();
    t && await Ot(t, ["mcp", "logout", Pt], {
      env: Nt(),
      timeout: 2e4
    });
    ...
    if (!(await dr(r)).connected) throw new Error("Connect your Artlist account in Settings before generating.");
    if (!((i = e == null ? void 0 : e.prompt) != null && i.trim())) throw new Error("Artlist generation requires a prompt.");
    return hp(r, e);
  });
}
const Ne = (t) => t && typeof t == "object" && !Array.isArray(t) ? t : {};
function Fi(t) {
  const e = Ne(Ne(t.raw).parameters ?? t.parameters), r = Ne(e.supportHybridUploadsRef);
  if (r.audios === !1 || r.maxAudios === 0) return !1;
  if (r.audios === !0 || Number(r.maxAudios) > 0 || Number(e.maxRefAudioCount) > 0) return !0;
  const n = Ne(t.submitParameterOptions);
  return Object.hasOwn(n, "inputAudios") || Object.hasOwn(Ne(t.defaultSubmitParameters), "inputAudios") ? !0 : [t.submitModel, t.displayName, t.name].some((i) => typeof i == "string" && /seedance[-\s]+2[.\-](?:5|0)(?:\b|[-\s])/i.test(i));
}
function Zo(t, e, r, n = !1) {
  if (e.taskType !== "omni_reference" || !Array.isArray(e.inputAudios) || !e.inputAudios.length) return "mcp";
  const i = Ne(t), o = Ne(Ne(i.properties).req ?? i);
  if (Object.hasOwn(Ne(o.properties), "inputAudios")) return "mcp";
...
function _p(t) {
  return yp.every((e) => t.includes(e));
}
function ye(t) {
  var r, n, i;
  ...
  if (e.structuredContent) return ye(e.structuredContent);
  ...
  return e.result && typeof e.result == "object" ? ye(e.result) : e;
}
const ri = "cinegen-canvas:";
function qr(t) {
  if (!(typeof t != "string" || !t.startsWith(ri))) {
    try {
      const e = JSON.parse(atob(t.slice(ri.length).replace(/-/g, "+").replace(/_/g, "/")));
...
function wp(t) {
  return ri + btoa(JSON.stringify(t)).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
function rt(t) {
...
function bp(t, e, r) {
  ...
  for (const u of r) {
    const c = `reference_${rt(u)}`;
    if (i[c] = (i[c] ?? 0) + 1, !n.some((p) => p.role === c)) throw new Error(`Topview ${t.displayName ?? t.model} does not accept ${rt(u)} references in this mode.`);
  }
  for (const u of n) {
    const c = i[u.role] ?? 0;
    if (c < (u.min ?? 0) || c > (u.max ?? 1 / 0)) throw new Error(`Topview ${t.displayName ?? t.model} allows ${u.min ?? 0}–${u.max} ${u.role.replace("reference_", "")} references in this mode.`);
  }
  for (const u of t.constraints ?? [])
    if (u.type === "require_any_roles" && !u.roles.some((c) => i[c]))
  ...
  for (const [u, c] of Object.entries(s)) {
    ...
    if (!Object.hasOwn(o, u)) throw new Error(`Topview Canvas does not expose the requested ${u} setting for this model.`);
    const p = o[u].enum ?? ((d = t.parameterEnums) == null ? void 0 : d[u]), f = o[u].type === "number" ? Number(String(c).replace(/p$/i, "")) : c;
    if (p && !p.includes(f)) throw new Error(`Topview Canvas does not support ${u}=${c}. Available: ${p.join(", ")}.`);
    a[u] = f;
  }
  for (const u of t.requiredParameters ?? [])
    if (a[u] === void 0) throw new Error(`Topview requires ${u} for this generation.`);
  return a;
}
async function vp(t) {
  var y, h, g;
  const { call: e, request: r, references: n } = t;
  let o = (h = (y = ye(await e("list_topview_canvases", { limit: 50 })).canvases) == null ? void 0 : y.find((_) => _.name === "CineGen references")) == null ? void 0 : h.canvasId;
  if (o || (o = ye(await e("create_topview_canvas", { name: "CineGen references" })).canvasId), typeof o != "string" || !o) throw new Error("Topview did not return the CineGen Canvas ID.");
  const a = ye(await e("get_topview_canvas_generation_capabilities", {
    canvasId: o,
    mediaType: "video",
    include: ["schema"],
    refresh: !0
  ...
  const d = bp(l, r, n), c = (ye(await e("get_topview_canvas_state", { canvasId: o, fields: ["nodes.geometry"] })).nodes ?? []).reduce((_, b) => {
    var v, E, S, x;
    return Math.max(
      _,
      Number(b.y ?? ((v = b.position) == null ? void 0 : v.y) ?? ((E = b.geometry) == null ? void 0 : E.y) ?? 0) + Number(b.height ?? ((S = b.measured) == null ? void 0 : S.height) ?? ((x = b.geometry) == null ? void 0 : x.height) ?? 400)
    );
  }, -80) + 80, p = [];
  for (const [_, b] of n.entries()) {
    ...
    const E = ye(await e("prepare_topview_canvas_media_upload", {
      canvasId: o,
      fileName: `reference-${_ + 1}.${v.format}`,
      fileSize: v.bytes.byteLength
    }));
    if (!/^https:\/\//.test(E.uploadUrl ?? "") || !E.objectKey) throw new Error("Topview did not return a usable Canvas upload destination.");
    const S = Object.fromEntries(Object.entries(E.requiredHeaders ?? {}).filter((A) => typeof A[1] == "string")), x = await fetch(E.uploadUrl, {
      method: "PUT",
      headers: S,
      body: new Blob([new Uint8Array(v.bytes)], { type: E.mimeType ?? v.mime }),
      redirect: "manual",
      signal: AbortSignal.timeout(9e4)
    });
    if (!x.ok) throw new Error(`Topview reference upload failed (HTTP ${x.status}). No generation was submitted.`);
    const T = ye(await e("create_topview_canvas_media_node", {
      canvasId: o,
      mediaType: rt(b),
      url: E.objectKey,
      mimeType: E.mimeType ?? v.mime,
    ...
    if (!T.nodeId || T.consistencyStatus && T.consistencyStatus !== "projected") throw new Error("Topview has not finished preparing this reference. No generation was submitted.");
    p.push({ role: `reference_${rt(b)}`, source: { kind: "canvas_node", nodeId: T.nodeId } });
  }
  const f = r.commandId ?? `cinegen-${crypto.randomUUID()}`, m = ye(await e("submit_topview_canvas_generation_task", {
    canvasId: o,
    mediaType: "video",
    taskType: l.taskType,
    model: l.model,
...
async function Ep(t, e) {
  ...
  const n = ye(await t("refresh_topview_canvas_generation_task", { ...r, include: [] })), i = String(n.status ?? "").toLowerCase();
  ...
  const a = (s = ye(await t("download_topview_canvas_nodes", { canvasId: r.canvasId, nodeIds: [r.nodeId] })).downloadArtifacts) == null ? void 0 : s.find((l) => l.nodeId === r.nodeId && /^https:\/\//.test(l.url ?? ""));
  return a ? { taskId: e, status: "success", videoUrl: a.url } : { taskId: e, status: "running" };
}
const Zs = Wa(import.meta.url);
function el(t) {
  return W.isPackaged ? t.replace("app.asar", "app.asar.unpacked") : t;
}
function we() {
  const t = Zs("ffmpeg-static");
  return el(t);
}
function Bi() {
  const t = Zs("ffprobe-static").path;
  return el(t);
}
function tl() {
  if (W.isPackaged)
    return w.join(process.resourcesPath, "vendor", "fpcalc");
  const t = w.dirname(bi(import.meta.url));
  return w.resolve(t, "..", "vendor", "fpcalc", "fpcalc");
}
const rl = Lt(Ce), Tp = 20 * 1e3, Sp = 300 * 1e3;
function Ip(t) {
...
function kp(t, e) {
  return nl(t, `scale=${e.width}:${e.height}:flags=lanczos`);
}
function jp(t, e) {
  const r = `max(1,sqrt(${e}/(iw*ih)))`, n = `ceil(iw*${r}/2)*2`, i = `ceil(ih*${r}/2)*2`;
  return nl(t, `scale='${n}':'${i}':flags=lanczos`);
}
const Rp = [
  "GPT Image 2",
  "Nano Banana 2",
...
], Np = ["16:9", "9:16", "1:1", "4:3", "3:4"], Cp = ["1K", "2K", "4K"], qp = ["16:9", "9:16", "1:1", "4:3", "3:4", "21:9"], Lp = ["720", "1080"], Up = Array.from({ length: 27 }, (t, e) => String(e + 4));
function xe(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function ni(t, e = []) {
  return Array.isArray(t) ? t.forEach((r) => ni(r, e)) : xe(t) && (e.push(t), Object.values(t).forEach((r) => ni(r, e))), e;
}
function Mp(t) {
  for (const e of ni(t))
    if (Array.isArray(e.models)) return e.models.filter(xe);
  return [];
}
function Dp(t) {
  return xe(t) ? t.value ?? t.id ?? t.name ?? t.label : t;
}
function $p(t, e) {
  const r = t.submitParameterOptions, n = xe(r) ? r[e] : Array.isArray(r) ? r.find((o) => xe(o) && (o.name === e || o.key === e || o.field === e)) : void 0;
  return (Array.isArray(n) ? n : xe(n) ? ["values", "options", "enum", "allowedValues"].map((o) => n[o]).find(Array.isArray) ?? [] : []).map(Dp).filter((o) => o != null);
}
function il(t) {
  ...
  for (const r of [])
    if (!(r.outputType !== "image" && r.outputType !== "video" && r.outputType !== "audio"))
      for (const n of Mp(r.config)) {
        ...
        a.catalogType ?? (a.catalogType = r.catalogType ?? r.taskType), a.taskTypes.add(r.taskType), r.taskType === "omni_reference" && (a.audioReferences = Fi(n)), typeof n.submitModel == "string" && (a.submitModel = n.submitModel), xe(n.defaultSubmitParameters) && (a.defaults = { ...a.defaults, ...n.defaultSubmitParameters }), (n.nativeAudio === !0 || n.supportsNativeAudio === !0) && (a.nativeAudio = !0), (n.nativeAudio === !1 || n.supportsNativeAudio === !1) && (a.nativeAudio ?? (a.nativeAudio = !1));
        for (const s of Bp(n)) a.accepts.add(s);
        for (const s of Object.keys(xe(n.defaultSubmitParameters) ? n.defaultSubmitParameters : {}))
          a.accepts.add(s);
        for (const s of ["aspectRatio", "resolution", "duration", "quality", "sound"]) {
          const l = $p(n, s);
          l.length && (a.accepts.add(s), a.options.set(s, il([...a.options.get(s) ?? [], ...l])));
        }
        e.set(o, a);
      }
  return [...e.values()];
...
function ta(t, e) {
  ...
  return il(r).map((n) => ({ value: String(n), label: String(n) }));
}
function Bp(t) {
  const e = t.requiredSubmitFields;
  return xe(e) ? Object.entries(e).filter(([, r]) => r !== !1).map(([r]) => r) : Array.isArray(e) ? e.flatMap((r) => {
...
function It(t, e, r) {
  ...
  return n.length ? ta(n, []) : t.live && !t.accepts.has(e) ? [] : ta([], r);
}
function xt(t, e, r) {
  var i;
...
function Vp(t) {
  const e = It(t, "aspectRatio", Np), r = It(t, "resolution", Cp), n = It(t, "quality", []), i = t.taskTypes.size === 0 || t.taskTypes.has("text_to_image"), o = t.taskTypes.size === 0 || t.taskTypes.has("image_edit"), a = [
  ...
  return o && a.push(
    ...
    nodeType: `topview-image-${rn(t.displayName)}`,
    name: t.displayName,
...
function Hp(t) {
  const e = It(t, "aspectRatio", qp), r = It(t, "resolution", Lp), n = It(t, "duration", Up), i = t.options.get("sound") ?? [], o = t.nativeAudio === !0 || i.some((c) => String(c).toLowerCase() === "on"), a = t.taskTypes.size === 0 || t.taskTypes.has("omni_reference"), s = a && (t.audioReferences ?? Fi(t)), l = t.taskTypes.size === 0 || t.taskTypes.has("image_to_video"), d = [...t.taskTypes].map((c) => c.replaceAll("_", " ")).join(" · "), u = [
    { id: "prompt", portType: "text", label: "Prompt", required: !0, falParam: "prompt", fieldType: "port" }
  ];
  return a ? (u.push(
  ...
  })) : l && u.push({ id: "image_url", portType: "image", label: "Start Frame", required: !1, falParam: "image_url", fieldType: "port", mediaRole: "start_image" }), l && a && u.push(
    ...
    nodeType: `topview-video-${rn(t.displayName)}`,
    name: t.displayName,
    category: "video",
    description: `Topview video generation${s ? " with image, video, and audio input references" : ""}${o ? " with native audio" : ""}${d ? ` · ${d}` : ""}`,
    outputType: "video",
    provider: "topview",
    responseMapping: { path: "url" },
    inputs: u
...
function Gp(t) {
  ...
  return e === "music" ? r.push(
    ...
    nodeType: `topview-audio-${rn(t.displayName)}`,
    name: t.displayName,
    category: "audio",
...
function zp() {
  return [
    ...
    ...Pp.map(({ displayName: t, catalogType: e }) => ({
      displayName: t,
      outputType: "audio",
      catalogType: e,
...
function Aw(t) {
  const e = Fp(), r = e.length ? e : zp();
  return Object.fromEntries(r.map((n) => {
    const i = n.outputType === "image" ? Vp(n) : n.outputType === "video" ? Hp(n) : Gp(n);
    return [i.nodeType, i];
  }));
}
const ra = -1, Wp = "[-\\u2010-\\u2015\\u2212]", Xp = new RegExp(
  `duration\`?\\s*(?:must|should)\\s+be\\s*\`?\\s*${Wp}\\s*1\\b`,
  "i"
), Jp = /task\s+as\s+video\s+editing|duration\s+follows?\s+the\s+input\s+video/i;
function ol(t) {
  return /duration/i.test(t) ? Xp.test(t) || Jp.test(t) : !1;
}
const Kp = "https://mcp.topview.ai/mcp", kn = "https://mcp.topview.ai", Yp = "https://www.topview.ai/mcp_oauth/oauth/authorize", Qp = "https://www.topview.ai/mcp_oauth/oauth/token", Zp = "https://www.topview.ai/mcp_oauth/oauth/register", na = "https://www.topview.ai/mcp_oauth/oauth/userinfo", em = 300 * 1e3, jn = 1200 * 1e3, tm = 90 * 1e3, rm = 30 * 1e3, Ct = 45 * 1024 * 1024, ia = 50;
function $(t) {
  return typeof t == "object" && t !== null && !Array.isArray(t);
}
function Rn(t) {
...
class im {
  ...
  availabilityError() {
    try {
      return Sr.isEncryptionAvailable() ? process.platform === "linux" && Sr.getSelectedStorageBackend() === "basic_text" ? "Topview sign-in requires a Linux secret store such as GNOME Keyring or KWallet." : void 0 : "Secure credential storage is unavailable on this device. Configure the operating-system keychain, then restart CineGen.";
    } catch {
  ...
  async read(e) {
    try {
      ...
      const n = Sr.decryptString(Buffer.from(r.data, "base64")), i = JSON.parse(n);
  ...
  async write(e, r) {
    ...
    const n = JSON.stringify({
      version: 1,
      data: Sr.encryptString(JSON.stringify(r)).toString("base64")
...
function sl(t, e) {
  ...
  return r.includes("array") && Array.isArray(t) ? t.map((n) => sl(n, e.items)) : r.includes("object") && $(t) ? ll(e, t) : t;
}
function ll(t, e) {
  if (!$(t)) return { ...e };
  ...
  for (const [l, d] of Object.entries(e))
    a && !Object.hasOwn(o, l) || (s[l] = sl(d, o[l]));
  return s;
}
function sm(t, e) {
  var i;
  const r = $((i = t.inputSchema) == null ? void 0 : i.properties) ? t.inputSchema.properties : {}, n = ll(t.inputSchema, e);
...
function lm(t) {
  const r = (dl(t, /^(?:boards|list|items|records|data|rows)$/i) ?? []).filter($).map((i) => ({
...
function ur(t, e) {
  ...
  return Object.hasOwn(r, e) || Vi(t).includes(e) || um(t).includes(e);
}
function oi(t) {
  if (t.nativeAudio === !1 || t.supportsNativeAudio === !1) return !1;
  if (t.nativeAudio === !0 || t.supportsNativeAudio === !0) return !0;
  const e = ul(t, "sound");
  if (e.length) return pl(e, "on") !== void 0;
  if (($(t.defaultSubmitParameters) ? t.defaultSubmitParameters : {}).sound === "on" || ur(t, "sound")) return !0;
}
function At(t) {
...
function cl(t, e, r = !1) {
  ...
  if (i && i !== "auto") {
    const s = n.find((l) => At(l).some((d) => d.toLowerCase() === i.toLowerCase()));
    if (!s)
...
function la(t) {
  var a;
  const e = t.references.length ? "image_edit" : "text_to_image", r = cl(t.config, t.params.model), n = (a = At(r)[0]) == null ? void 0 : a.trim();
  ...
  for (const [s, l, d] of [
    ...
    ur(r, s) && (o[s] = ai({ model: r, field: s, requested: l, fallback: d }));
  for (const s of Vi(r))
...
const fm = "Keep the frame free of on-screen text, captions, and subtitles.";
function fl(t) {
...
function qn(t) {
  var p, f;
  const e = cl(t.config, t.params.model, t.params.generateAudio === !0), r = String(e.submitModel ?? "").trim();
  if (!r) throw new Error("Topview returned a video model without a submit identifier.");
  const n = $(e.defaultSubmitParameters) ? e.defaultSubmitParameters : {}, i = new Set(Vi(e)), o = t.params.durationSec === void 0 ? void 0 : Math.round(t.params.durationSec);
  if (o !== void 0 && (!Number.isFinite(o) || o === 0 || o < -1))
    throw new Error("Topview video duration must be a positive whole number of seconds.");
  let a = fl(t.params.prompt);
  const s = {
    ...n,
    taskType: t.taskType,
    model: r,
  ...
  const l = (m, y, h) => {
    ...
    const _ = ai({ model: e, field: m, requested: y, fallback: h, required: i.has(m) });
    _ !== void 0 && (s[m] = _);
  };
  if (l("resolution", t.params.resolution === void 0 ? void 0 : Number.parseInt(t.params.resolution, 10), 720), t.inheritInputVideoDuration || o === ra ? s.duration = ra : ur(e, "duration") && l("duration", o, 5), l("generatingCount", void 0, 1), t.taskType !== "image_to_video" ? l("aspectRatio", (p = t.params.aspectRatio) == null ? void 0 : p.trim(), "16:9") : (t.params.aspectRatio !== void 0 || i.has("aspectRatio")) && l("aspectRatio", (f = t.params.aspectRatio) == null ? void 0 : f.trim(), "16:9"), oi(e) !== !1 && (ur(e, "sound") || t.params.generateAudio === !0))
    s.sound = ai({
      model: e,
      field: "sound",
  ...
  if (t.taskType === "omni_reference") {
    ...
    for (const E of t.references)
      if (E.role === "video") {
        const S = `Video${++y}`;
        _.push({ fileId: E.fileId, name: S }), v.push(`<<<${S}>>> is a supplied motion and timing reference.`);
      } else if (E.role === "audio") {
        const S = `Audio${++h}`;
        b.push({ fileId: E.fileId, name: S }), v.push(`<<<${S}>>> is a supplied audio reference.`);
      } else {
        const S = `Image${++m}`;
        g.push({ fileId: E.fileId, name: S });
        const x = E.role === "start_image" ? "the requested opening-frame reference" : E.role === "end_image" ? "the requested closing-frame reference" : "a supplied visual reference";
        v.push(`<<<${S}>>> is ${x}.`);
      }
    if (b.length && !Fi(e))
...
function Ln(t, e, r) {
  const n = (r ?? "").split(";", 1)[0].trim().toLowerCase(), i = n ? hm[n] : void 0, o = (() => {
    try {
  ...
  if (!s || !da(e).has(s)) {
    ...
    throw new Error(`Topview received an unsupported ${l} reference format. Supported formats: ${[...da(e)].join(", ")}.`);
  }
  if (n && !i)
...
function ym(t) {
  const e = t.startsWith("[") && t.endsWith("]") ? t.slice(1, -1) : t, r = wi(e.split("%", 1)[0]);
  if (r === 4) return ua(e);
  if (r !== 6) return !1;
  const n = gm(e);
  return n ? n.slice(0, 10).every((o) => o === 0) && n[10] === 255 && n[11] === 255 ? ua(n.slice(12).join(".")) : !(n[0] < 32 || n[0] > 63 || n[0] === 32 && n[1] === 1 && (n[2] & 254) === 0 || n[0] === 32 && n[1] === 1 && n[2] === 0 && n[3] === 2 || n[0] === 32 && n[1] === 1 && n[2] === 13 && n[3] === 184 || n[0] === 32 && n[1] === 1 && (n[2] & 240) === 16 || n[0] === 32 && n[1] === 1 && (n[2] & 240) === 32 || n[0] === 32 && n[1] === 2 || n[0] === 63 && (n[1] & 240) === 240) : !1;
}
async function _m(t) {
  ...
  const n = wi(e), i = n ? [{ address: e, family: n }] : await bd(e, { all: !0, verbatim: !0 });
  if (!i.length || i.some((o) => !ym(o.address)))
...
async function gl(t, e = 0) {
  ...
  return new Promise((i, o) => {
    const a = vd({
      ...
      method: "GET",
      servername: wi(r.hostname.replace(/^\[|\]$/g, "")) ? void 0 : r.hostname,
      headers: {
      ...
      if (l >= 300 && l < 400) {
        ...
        gl(new URL(p, r).href, e + 1).then(i, o);
        return;
      }
      if (l < 200 || l >= 300) {
...
async function ca(t, e) {
  ...
  const i = await gl(r), o = Ln(i.finalUrl, e, i.contentType);
...
function bm(t) {
  const e = yl(t.submitModel);
  if (e === void 0) return;
  const r = t.width * t.height;
  if (!(r >= e))
    return `This reference video is ${t.width}x${t.height}, which is ${si(r)} pixels per frame. "${t.submitModel}" requires at least ${si(e)}. Re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice), then attach it again.`;
}
function fa(t) {
  var e;
  if (/video\s+pixel\s+count/i.test(t)) {
    ...
    return `Seedance rejected a reference video for being too small. It needs at least ${si(n)} pixels per frame, so re-encode the clip at 854x480 or larger for 16:9 (960x540 is a safe choice) and attach it again. A 640x360 clip is the usual cause.`;
  }
  if (ol(t))
...
class Em {
  ...
  async tokenExchange(e, r) {
    ...
    return n.set("client_id", r.client_id), r.client_secret && n.set("client_secret", r.client_secret), Nn(Qp, {
      method: "POST",
  ...
  async mcpRequest(e, r, n) {
    var a;
    const i = new AbortController(), o = setTimeout(() => i.abort(), tm);
    (a = o.unref) == null || a.call(o);
    try {
      const s = await fetch(Kp, {
        method: "POST",
        headers: {
        ...
        signal: i.signal
      }), l = await s.text(), d = (s.headers.get("content-type") || "").includes("text/event-stream") ? om(l, r.id) : l ? (() => {
        try {
          return JSON.parse(l);
        } catch {
          return l;
      ...
      if (!s.ok) throw ii(s.status, d, "Topview MCP request failed.");
      if ($(d) && d.error !== void 0) throw ii(400, d.error, "Topview MCP returned an error.");
      return {
        payload: $(d) ? d : {},
  ...
  async session() {
    ...
    for (let l = 0; l < ia; l += 1) {
      ...
      i = d.sessionId || i;
      const u = $(d.payload.result) ? d.payload.result : {}, c = Array.isArray(u.tools) ? u.tools.filter((f) => $(f) && typeof f.name == "string") : [];
  ...
  async callTool(e, r, n) {
    ...
    if ($(a) && a.isError === !0) {
      const s = ie(a), d = (te(s, [
        ...
        "failure_reason",
        "message"
      ]) ?? Br(a).join(" ").slice(0, 700)).trim() || `Topview could not run ${r}.`, u = fa(d);
      throw new Error(u ? `${d}
...
${u}` : d);
  ...
  async uploadReference(e, r, n) {
    ...
    let i = await ca(r.value, r.role), o;
    if (r.role === "video" && n && yl(n) !== void 0) {
      const f = await Ap(i), m = f && bm({ submitModel: n, ...f });
      if (f && m) {
        const y = xp(f, Vr);
        try {
          const h = await kp(i, y);
          if (h.length > Ct)
...
CineGen could not create the temporary resized copy automatically.`);
    ...
    const a = await this.callTool(e, "ta_upload_credential", {
      format: i.format,
      needAccelerateUrl: !1
    }), s = ie(a), l = te(s, ["fileId", "file_id"]), d = te(s, ["uploadUrl", "upload_url", "accelerateUrl", "accelerate_url"]);
    if (!l || !d) throw new Error("Topview did not return a usable upload destination for an element.");
    const u = (te(s, ["method", "httpMethod", "http_method"]) || "PUT").toUpperCase(), c = await fetch(d, {
      method: u,
      headers: { ...vm(s), ...i.contentType ? { "Content-Type": i.contentType } : {} },
      body: i.bytes
    });
    ...
    if (am(ie(p)) === !1) throw new Error("Topview could not verify an uploaded element reference.");
  ...
  async accountStatus() {
    ...
    try {
      ...
      const n = await this.accessToken(), i = await Nn(na, {
        method: "GET",
      ...
      if (o.tools.some((d) => d.name === "topview_get_credit"))
        try {
          l = aa(ie(await this.callTool(o, "topview_get_credit", {}))) ?? l;
        } catch {
        }
      return {
        connected: !0,
      ...
      return { connected: !1, configured: !0, error: On(r, "Topview connection expired.") };
    }
  }
  async modelCatalog() {
    ...
    return {
      ...
      toolSchemas: Object.fromEntries(e.tools.filter((i) => ["topview_get_generation_config", "topview_generate_video", "topview_generate_audio", "topview_generate_music", "topview_generate_voice", "topview_clone_voice", "topview_query_task"].includes(i.name)).map((i) => [i.name, i.inputSchema])),
  ...
  async authLogin() {
    ...
    }), c = setTimeout(() => d == null ? void 0 : d(new Error("Topview sign-in timed out. Try connecting again.")), em);
    ...
    try {
      ...
      y.search = new URLSearchParams({
        ...
        resource: kn
      }).toString(), await hi.openExternal(y.href);
      ...
      try {
        ...
        E.ok && $(S) && await this.store.write("profile", S);
      } catch {
      }
      return oa(l, !0, "You can close this window and return to CineGen."), this.accountStatus();
    } catch (f) {
      throw l && !l.writableEnded && oa(l, !1, On(f, "Topview sign-in did not complete.")), f;
    } finally {
  ...
  async submitWithSession(e, r) {
    ...
    const n = ml(r.medias), i = mm(r.medias), o = await this.chooseBoard(e), a = ie(await this.callTool(e, "topview_get_generation_config", {
      type: "video",
      taskType: i
    })), s = qn({
      config: a,
      ...
      params: r,
      boardId: o,
      references: n.map((m, y) => ({ ...m, fileId: `preflight-${y + 1}` }))
    });
    if (Zo(
      (f = e.tools.find((m) => m.name === "topview_generate_video")) == null ? void 0 : f.inputSchema,
      s.req,
      !1,
      _p(e.tools.map((m) => m.name))
    ) === "canvas-mcp") {
      const m = await vp({
        call: (y, h) => this.callTool(e, y, h),
        references: n,
        request: { ...s.req, prompt: r.prompt, ...r.generateAudio !== void 0 ? { sound: r.generateAudio } : {} },
        load: async (y) => {
          const h = await ca(y.value, y.role);
          return { bytes: h.bytes, format: h.format, mime: h.contentType };
        }
      });
      return { result: {
        taskId: m.taskId,
    ...
    const d = [];
    for (const m of n)
      d.push(await this.uploadReference(e, m, s.model));
    ...
    return {
      result: {
        ...
        ...d.some((m) => m.preparation) ? {
          referencePreparation: d.flatMap((m) => m.preparation ?? []).join("; ")
        } : {}
      },
      documents: c
    };
  ...
  async queryWithSession(e, r) {
    ...
    const n = await this.callTool(e, "topview_query_task", {
      ...
      needCloudFrontUrl: !0
    }), i = ie(n), o = Qt(i), a = mt(i), s = /fail|error|cancel/.test(o), l = !!a || /success|complete|done/.test(o), d = s ? "fail" : l && a ? "success" : l ? "fail" : /^(init|created|queued)$/.test(o) ? "init" : "running", u = te(i, ["boardTaskId", "board_task_id"]), c = te(i, [
      "errorMsg",
      "error_msg",
      "errorMessage",
      "error_message",
      "failureReason",
      "failure_reason"
    ]), p = c ? fa(c) : void 0, f = d !== "fail" ? void 0 : c ? p ? `${c}
...
${p}` : c : l ? "Topview completed the task without returning a video URL." : "Topview could not complete this video.";
    return {
      ...
      ...a ? { url: a } : {},
      ...f ? { error: f } : {},
      ...qr(r.taskId) ? {} : { boardUrl: `https://www.topview.ai/board/${encodeURIComponent(r.boardId)}${u ? `?boardResultId=${encodeURIComponent(u)}` : ""}` }
    };
  }
  async query(e) {
  ...
  async generateImage(e) {
    ...
    const r = await this.session(), n = cm(e.medias), i = n.length ? "image_edit" : "text_to_image", o = await this.chooseBoard(r), a = ie(await this.callTool(r, "topview_get_generation_config", {
      type: "image",
      taskType: i
    }));
    la({
    ...
    const l = la({ config: a, params: e, references: s, boardId: o }), d = await this.callTool(r, "topview_generate_image", l.req), u = ie(d), c = te(u, ["taskId", "task_id", "generationId", "generation_id"]), p = mt(u);
    if (p)
      return {
        url: p,
        mediaType: "image",
        taskId: c,
        model: l.model,
        referenceValue: sa(u) ?? await this.reusableGeneratedImageReference(r, p),
    ...
    if (/fail|error|cancel/.test(f))
      throw new Error(te(u, ["errorMsg", "error_msg", "errorMessage", "error_message"]) ?? "Topview could not complete this image.");
    const m = Date.now() + jn;
    for (; Date.now() < m; ) {
      ...
      if (g) {
        ...
        return {
          ...
          taskId: c,
          model: l.model,
          referenceValue: sa(h) ?? await this.reusableGeneratedImageReference(r, g),
  ...
  async generateAudio(e) {
    ...
    const d = te(l, ["taskId", "task_id", "generationId", "generation_id"]), u = mt(l);
...
let pa;
function _l() {
  return pa ?? (pa = new Em()), pa;
}
function Tm() {
  return _l().teamConnection();
}
function Sm() {
  const t = _l();
...
const ma = {
  ...
  high: { crf: 16, scale: 1 }
}, se = /* @__PURE__ */ new Map(), fr = /* @__PURE__ */ new Map();
function Im(t, e) {
...
async function Am(t, e) {
  const r = se.get(t);
  if (!r) return;
  const n = we(), i = ma[e.preset || "standard"] || ma.standard, o = e.fps || 30, a = e.outputPath || w.join(process.cwd(), `export_${t}.mp4`);
  ...
  return d.push(
    ...
    (h = m.stderr) == null || h.on("data", (g) => {
      ...
      if (b) {
        const v = xm(b, e.totalDuration);
        if (v !== null) {
          const E = se.get(t);
          E && (se.set(t, { ...E, progress: v }), Im(t, v));
        }
      }
...
function km() {
  I.handle("export:start", async (t, e) => {
    ...
    return se.set(i.id, i), Am(i.id, e).catch((o) => {
...
function Un(t) {
  const e = w.extname(t).toLowerCase();
  return jm[e] ?? "application/octet-stream";
}
function ha(t) {
  try {
    const e = new URL(t);
...
function Pm() {
  I.handle(
    ...
    async (t, e, r) => {
      ...
      const i = await Om(n);
      try {
        const o = await j.readFile(i), s = `${w.basename(n, w.extname(n))}.m4a`, l = Un(i), d = new Blob([o], { type: l }), u = new File([d], s, { type: l });
    ...
    async (t, e, r) => {
      ...
      if (e.startsWith("http://") || e.startsWith("https://")) {
        ...
        const s = await j.readFile(a), l = w.basename(a), d = Un(a), u = new Blob([s], { type: d }), c = new File([u], l, { type: d }), p = await K.fal.storage.upload(c);
...
class qm {
  constructor(e) {
    ...
    this.db = new _i(r), this.db.pragma("journal_mode = WAL"), this.db.pragma("foreign_keys = ON"), this.initSchema();
  }
  /**
   ...
   */
  initSchema() {
    this.db.exec(Nm), this.db.exec(Cm);
  }
  /**
...
function vl(t) {
  ...
  if (!r) return null;
  const n = e.type === "character" || e.type === "location" || e.type === "prop" || e.type === "vehicle" ? e.type : "character", i = typeof e.folderId == "string" && e.folderId ? e.folderId : typeof e.folder_id == "string" && e.folder_id ? e.folder_id : void 0, o = Um(e.variations), a = typeof e.activeVariationId == "string" && o.some((l) => l.id === e.activeVariationId) ? e.activeVariationId : (s = o[0]) == null ? void 0 : s.id;
  return {
    id: r,
...
function Tl(t) {
  if (!t || typeof t != "object") return bl();
  const e = t, r = Array.isArray(e.folders) ? e.folders.map(Mm).filter((o) => o !== null) : [], n = new Set(r.map((o) => o.id)), i = Array.isArray(e.elements) ? e.elements.map(vl).filter((o) => o !== null) : [];
  return {
    version: 1,
    folders: r,
...
function Dm(t, e) {
  const r = bl(), n = new Map(r.elements.map((o) => [o.id, o])), i = [...r.folders];
  for (const o of e) {
    const a = o.elements.map(vl).filter((l) => l !== null);
...
function Sl() {
  return w.join(dt(), "elements-library.json");
}
function Fm() {
  return w.join(dt(), "projects.json");
}
function ga(t) {
  return w.join(dt(), t);
}
async function li(t) {
  await j.mkdir(dt(), { recursive: !0 });
  const e = Sl(), r = `${e}.tmp`;
  await j.writeFile(r, JSON.stringify(t, null, 2), "utf-8"), await j.rename(r, e);
}
async function Bm() {
  try {
    const t = await j.readFile(Sl(), "utf-8");
    return Tl(JSON.parse(t));
  } catch {
    return null;
  }
}
function Vm(t) {
  const e = w.join(ga(t.id), "project.db"), r = w.join(ga(t.id), "project.json");
  if (t.useSqlite || D.existsSync(e))
    try {
      const n = new _i(e, { readonly: !0 }), i = n.prepare("SELECT * FROM elements").all();
...
async function Hm() {
  ...
  let e = [];
  try {
    const i = JSON.parse(await j.readFile(Fm(), "utf-8"));
    e = Array.isArray(i.projects) ? i.projects : [];
  } catch {
    e = [];
  }
  const r = e.map(Vm), n = Dm(null, r);
  return await li(n), n;
}
function Gm() {
  I.handle(
    ...
    async (t, e) => {
      ...
      if (e != null && e.projectId && e.projectName) {
        const n = $m(r, e.projectId, e.projectName);
        n !== r && (await li(n), r = n);
      }
      return r;
    }
...
function Wm(t, e, r) {
  const n = zm(r.persona), i = r.persona ? Il[r.persona] : void 0, o = [];
  let a = 0;
  if (e.length === 0)
  ...
  return t.timelinePlacements.some((s) => s.timelineId === r.activeTimelineId) && r.activeTimelineId && (a += n.activeTimeline, o.push("already on the active timeline")), t.words.length > 0 && (a += n.wordTiming), t.emotion && (a += n.hasEmotion), t.delivery && (a += n.hasDelivery, o.push("has vocal delivery notes")), i && (_a(t.energy, i.preferredEnergy) && (a += n.energyMatch, o.push(`${t.energy} energy fits ${r.persona}`)), _a(t.pace, i.preferredPace) && (a += n.paceMatch, o.push(`${t.pace} pace fits ${r.persona}`)), t.emotion && i.emotionBias.some((s) => t.emotion.toLowerCase().includes(s)) && (a += n.emotionBias, o.push(`${t.emotion} emotion favored by ${r.persona}`))), t.emotion && r.queryEmotions.some((s) => t.emotion.toLowerCase().includes(s) || s.includes(t.emotion.toLowerCase())) && (a += n.emotionQueryMatch, o.push(`emotion (${t.emotion}) matches the query`)), t.notable && t.notable.length > 0 && (a += n.notableSignal * t.notable.length, o.push(`notable: ${t.notable.slice(0, 2).join("; ")}`)), { score: a, reasons: o };
}
function Xm(t) {
...
function Zm(t) {
  ...
  return Qm.filter((r) => e.includes(r));
}
function eh(t, e, r = 24) {
  const n = typeof r == "number" ? { limit: r } : r, i = n.limit ?? 24, o = Ym(e), a = {
    activeTimelineId: t.activeTimelineId,
    persona: n.persona,
    queryEmotions: Zm(e)
  };
  return t.moments.map((s) => ({ moment: s, ...Wm(s, o, a) })).filter((s) => s.score > 0).sort((s, l) => l.score - s.score || s.moment.sourceStart - l.moment.sourceStart).slice(0, i).map(({ moment: s, score: l, reasons: d }) => ({
    id: s.id,
    assetId: s.assetId,
...
async function nn(t, e) {
  ...
  const r = rh(e);
  if (!r) return null;
  const n = await j.readFile(r), i = th(r), o = new Blob([n], { type: i }), a = new File([o], w.basename(r), { type: i });
...
function wa(t, e) {
  const n = (Array.isArray(t.objects) ? t.objects : Array.isArray(t.detections) ? t.detections : Array.isArray(t.items) ? t.items : Array.isArray(t.regions) ? t.regions : Array.isArray(t.subjects) ? t.subjects : typeof t.label == "string" || typeof t.name == "string" || typeof t.object == "string" ? [t] : []).map((o) => {
    ...
    if ((d === null || u === null || c === null || p === null) && m && m.length >= 4) {
      const x = Y(m[0]), T = Y(m[1]), A = Y(m[2]), C = Y(m[3]);
      [x, T, A, C].every((N) => N !== null) && (d = (x + A) / 2, u = (T + C) / 2, c = A - x, p = C - T);
    }
    const y = Array.isArray(a.box_3d) ? a.box_3d : Array.isArray(a.box3d) ? a.box3d : null;
    if ((d === null || u === null || c === null || p === null) && y && y.length >= 6) {
      const x = Y(y[0]), T = Y(y[1]), A = Y(y[3]), C = Y(y[4]), N = Y(y[5]);
      [x, T, A, C, N].every((M) => M !== null) && (d = x, u = T, c = Math.max(A, C), p = Math.max(C, N));
    }
    if (d === null || u === null || c === null || p === null) {
      const x = Y(a.center_x ?? a.cx ?? a.mid_x), T = Y(a.center_y ?? a.cy ?? a.mid_y), A = Y(a.width ?? a.w), C = Y(a.height ?? a.h);
      [x, T, A, C].every((N) => N !== null) && (d = x, u = T, c = A, p = C);
    }
    if (d === null || u === null || c === null || p === null) {
      const x = Y(a.x_min ?? a.left), T = Y(a.y_min ?? a.top), A = Y(a.x_max ?? a.right), C = Y(a.y_max ?? a.bottom);
      [x, T, A, C].every((N) => N !== null) && (d = (x + A) / 2, u = (T + C) / 2, c = A - x, p = C - T);
    }
    ...
    const h = ht(c, 0.02, 1), g = ht(p, 0.02, 1), _ = [
      ...
      h,
      g
    ], b = Y(a.score ?? a.confidence ?? a.probability), v = b !== null ? ht(b, 0, 1) : 0.75, E = Y(a.priority ?? a.salience ?? a.importance), S = E !== null ? ht(E, 0, 1) : v;
    return {
      label: l,
      box: _,
      score: v,
...
async function Al(t) {
  ...
  const e = (await Promise.all(
    t.framePaths.slice(0, 6).map((c) => nn(t.apiKey, c).catch(() => null))
  )).filter((c) => !!c);
  if (e.length === 0)
    return {
      assetId: t.assetId,
...
`),
    ...
    logs: !0
  })).data, i = he(n.output) || he(n.text) || "", o = xl(i);
  if (!o)
    return {
      assetId: t.assetId,
      status: "failed",
...
async function kl(t) {
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await nh(t.apiKey, t.videoPath).catch(() => null);
  if (!e)
...
async function ih(t) {
  var o;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = await nn(t.apiKey, t.imagePath).catch(() => null);
  if (!e)
...
async function oh(t) {
  var a, s;
  if (!t.apiKey) throw new Error("No fal.ai API key provided.");
  const e = Math.min(12, Math.max(1, Math.round(t.maxObjects ?? 6))), r = await nn(t.apiKey, t.imagePath).catch(() => null);
  if (!r)
    return {
      status: "missing",
      model: ((a = t.model) == null ? void 0 : a.trim()) || Fe,
...
`);
  try {
    const l = await ba(t.apiKey, r, n, e, i), d = l ? wa(l, e) : [];
    if (d.length > 0)
    ...
    const u = await ba(t.apiKey, r, n, e, o), c = u ? wa(u, e) : [];
...
function ah() {
  I.handle("vision:index-asset", async (t, e) => Al(e)), I.handle("vision:detect-objects", async (t, e) => oh(e));
}
const sh = "anthropic/claude-sonnet-4.6";
function oe(t) {
  const e = Number(t);
...
async function qt(t) {
  ...
  K.fal.config({ credentials: t.apiKey });
  const e = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || sh,
    prompt: t.prompt,
...
function mh(t, e) {
  ...
  return {
    brief: {
      ...
      variantCount: ch(r.variantCount, e.variantCount),
      persona: uh(r.persona),
    ...
    clarifyingQuestions: ph(r.clarifyingQuestions)
  };
}
function hh(t, e, r) {
  const n = { ...t, ...e ?? {} };
...
function gh(t) {
  if (!t || typeof t != "object") return null;
  const e = t, r = va(e.source_start), n = va(e.source_end);
...
function yh(t, e) {
  if (!t || typeof t != "object") return null;
  const r = t, n = Array.isArray(r.segments) ? r.segments.map(gh).filter((i) => !!i) : [];
...
function _h(t) {
  ...
  return Array.isArray(e.variants) ? e.variants.flatMap((r, n) => {
    var a;
    if (!r || typeof r != "object") return [];
    const i = r, o = Array.isArray(i.proposals) ? i.proposals.map((s) => yh(s, `AI Cut ${n + 1}`)).filter((s) => !!s) : [];
    return o.length === 0 ? [] : [{
...
async function Ea(t, e, r, n, i = {}) {
  ...
  let a = eh(t, o, { limit: 20, persona: r.persona });
  if (i.rerank && i.apiKey && a.length > 1)
    try {
      const l = Xm({ query: o, brief: r, candidates: a }), d = await qt({
        apiKey: i.apiKey,
        model: i.model,
...
async function Sh(t) {
  ...
  const e = (u, c) => {
    ...
    if (!p) return null;
    try {
      const f = JSON.parse(p), y = _h({ variants: [f] })[0];
      return y ? {
        variant: y,
        usage: c
      } : null;
...
${JSON.stringify(l.map((h) => ({ title: h.title, strategy: h.strategy, summary: h.summary })), null, 2)}` : "",
      ...
      "",
      "Retrieved moments:",
      jl(t.retrievalSummary.topMoments),
      "",
      "Reference timelines:",
...
async function Ih(t) {
  ...
  const e = [
    ...
    jl(t.retrievalSummary.topMoments.slice(0, 10)),
    "",
    "Variants:",
...
async function xh(t) {
  ...
  n = mr(n, i.usage);
  const o = hh(i.brief, t.briefOverride, t.questionAnswers), a = await Ea(e, r, o, []);
  if (!t.confirmedBrief)
    return {
      stage: "brief",
  ...
  if (n = mr(n, d.usage), d.variants.length === 0)
    return {
      stage: "brief",
...
`)) >= 0; ) {
      ...
      if (m = m.slice(b + 1), !!v)
        try {
          const E = JSON.parse(v), S = E.message, x = typeof (S == null ? void 0 : S.content) == "string" ? S.content : "";
          if (x)
            for (const T of x)
              u ? (c += T, c.endsWith("</think>") && (u = !1, c = "")) : (c += T, c === "<think>" ? (u = !0, c = "") : "<think>".startsWith(c) || (s += c, a == null || a.webContents.send("llm:local-stream", { requestId: t, token: c }), c = ""));
          E.done && (l = oe(E.prompt_eval_count) ?? 0, d = oe(E.eval_count) ?? 0);
        } catch {
        }
    }
  }
...
async function jh() {
  try {
    const t = await fetch(`${Rl}/api/tags`);
...
function Rh() {
  I.handle("llm:chat", async (t, e) => {
    ...
    const n = Array.isArray(e.messages) ? e.messages : [], i = dh(n);
    ...
    const r = e.requestId || crypto.randomUUID(), n = await kh(r, e);
    return {
      message: n.message,
      ...n.usage ? { usage: n.usage } : {}
    };
  }), I.handle("llm:local-models", async () => jh()), I.handle("llm:run-cut-workflow", async (t, e) => xh(e));
}
const Ol = Lt(Ce), Oh = [
  w.join(z.homedir(), ".local/bin/claude"),
  "/opt/homebrew/bin/claude",
...
async function Pl() {
  ...
  for (const t of Oh)
    try {
      const { stdout: e } = await Ol(t, ["--version"], {
        env: Gi(),
        timeout: 8e3
      });
...
async function Vh(t, e) {
  ...
  const n = ((h = e.model) == null ? void 0 : h.trim()) || "sonnet", i = !!e.resumeSessionId && !e.injectProjectContext, o = Fh(e), a = [
    "-p",
    i ? e.userMessage.trim() : Bh(e, o),
    "--output-format",
    "stream-json",
    "--verbose",
    "--max-turns",
  ...
  if (o ? (a.push("--safe-mode", "--effort", "low", "--include-partial-messages"), !i && ((g = e.systemPrompt) != null && g.trim()) && a.push("--system-prompt", e.systemPrompt.trim())) : a.push("--include-partial-messages"), i && e.resumeSessionId) {
    if (a.push("--resume", e.resumeSessionId), !o) {
      const v = [(_ = e.systemPrompt) == null ? void 0 : _.trim(), Nh].filter(Boolean).join(`
...
` : "", E = e.purpose === "enhance-prompt" ? Ch : Ph;
    a.push("--append-system-prompt", `${v}${e.systemPrompt.trim()}

${E}`);
  }
  const s = Uh(), l = o ? $h() : void 0;
  ...
  return new Promise((v, E) => {
    ...
    nt = { child: S, requestId: t };
    let x = "", T = !1;
    const A = (M) => {
      T || (T = !0, nt = null, s == null || s.webContents.send("llm:claude-code-stream", { requestId: t, done: !0 }), M(), S.killed || S.kill());
    };
    (C = S.stdout) == null || C.on("data", (M) => {
      x += M.toString();
      let B;
      for (; (B = x.indexOf(`
`)) >= 0; ) {
        const H = x.slice(0, B).trim();
        if (x = x.slice(B + 1), !!H)
          try {
            ...
            if (G.type === "system" && G.subtype === "init" && typeof G.session_id == "string" && (c = G.session_id), G.type === "assistant" && G.error === "authentication_failed" && (p = !0), G.type === "result") {
              y = G;
              const V = Sa(G);
              V && !d.trim() && (d = V, s == null || s.webContents.send("llm:claude-code-stream", { requestId: t, token: V }));
              const k = d.trim();
            ...
            G.type === "assistant" && !f ? (d = P, s == null || s.webContents.send("llm:claude-code-stream", { requestId: t, token: P })) : G.type === "result" && !d.trim() && (d = P, s == null || s.webContents.send("llm:claude-code-stream", { requestId: t, token: P }));
          } catch {
          }
      }
    }), (N = S.stderr) == null || N.on("data", (M) => {
      u += M.toString();
    }), S.on("error", (M) => {
      A(() => E(M));
    }), S.on("close", (M) => {
      A(() => {
        ...
        if (p || B.includes("Not logged in")) {
          E(new Error("Claude Code is not logged in. Open Terminal, run `claude`, and sign in with your subscription."));
          return;
        }
        if (B) {
...
function Hh() {
  I.handle("llm:claude-code-detect", async () => {
    ...
    const r = e.requestId || J.randomUUID(), n = await Vh(r, e);
    return {
      message: n.message,
...
async function an(t) {
  ...
  for (const e of Gh[t])
    try {
      const { stdout: r } = await Nl(e, ["--version"], {
        env: on(),
        timeout: 8e3
      });
      if (r.trim())
...
function Xh() {
  I.handle("llm:cli-detect", async () => ({ providers: await zh() }));
}
function Jh(t, e) {
...
` : "", l = t.purpose === "enhance-prompt" ? Ul : Ll;
      r.push(`${s}${t.systemPrompt.trim()}

${l}`);
    } else
      r.push(t.systemPrompt.trim());
  const n = (t.messages ?? []).filter((s) => s.content.trim()), i = n.length > 0 ? ui(n) : `${t.userMessage.trim()}
...
async function tg(t, e) {
  ...
  if (!e.userMessage.trim())
    throw new Error("No chat message provided.");
  const n = ((y = e.model) == null ? void 0 : y.trim()) || "gpt-5.3-codex", i = !!e.resumeSessionId && !e.injectProjectContext, o = Yh(e), a = i ? e.userMessage.trim() : Qh(e, o), s = o ? Kh() : void 0;
  s && await at(s, { recursive: !0 });
  ...
  return new Promise((h, g) => {
    ...
    o && ((v = _.stdin) == null || v.write(a), (E = _.stdin) == null || E.end()), Oe = { child: _, requestId: t, provider: "codex" };
    let b = "";
    (S = _.stdout) == null || S.on("data", (T) => {
      b += T.toString();
      let A;
      for (; (A = b.indexOf(`
`)) >= 0; ) {
        const C = b.slice(0, A).trim();
        if (b = b.slice(A + 1), !!C)
          try {
            ...
            if (M && (f = M), N.type === "turn.failed") {
              const H = N.error;
              c += (H == null ? void 0 : H.message) ?? "Codex turn failed.";
            }
            const B = eg(N);
            if (B) {
              const H = B.startsWith(m) ? B.slice(m.length) : B;
              m = B, u = B, H && (d == null || d.webContents.send("llm:codex-stream", { requestId: t, token: H }));
            }
          } catch {
          }
      }
    }), (x = _.stderr) == null || x.on("data", (T) => {
      c += T.toString();
    }), _.on("error", (T) => {
      Oe = null, g(T);
    }), _.on("close", (T) => {
      Oe = null, d == null || d.webContents.send("llm:codex-stream", { requestId: t, done: !0 });
      const A = u.trim();
      if (!A) {
        g(new Error(Jh(c, T)));
        return;
...
function rg() {
  I.handle("llm:codex-chat", async (t, e) => {
    const r = e.requestId || J.randomUUID(), n = await tg(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
    ...
    (Oe == null ? void 0 : Oe.requestId) !== e || Oe.provider !== "codex" || (Oe.child.kill("SIGTERM"), Oe = null);
  });
}
const ng = 272e3, Ia = {
...
function og(t) {
  const e = t.promptTokens > ng ? Ia.long : Ia.short, r = Math.min(t.cachedTokens, t.promptTokens), n = Math.min(t.cacheWriteTokens, Math.max(0, t.promptTokens - r)), o = (Math.max(0, t.promptTokens - r - n) * e.input + r * e.cached + n * e.cacheWrite + t.completionTokens * e.output) / 1e6;
  return { ...t, cost: Math.round(o * 1e8) / 1e8 };
}
const ag = "https://api.openai.com/v1/chat/completions", sg = "gpt-5.6-luna", lg = 6e4;
function dg(t, e = []) {
...
function ug(t) {
  ...
  r && e.push({ role: "system", content: r }), e.push({ role: "user", content: dg(t.userMessage, t.imageUrls) });
  const n = {
    model: ((o = t.model) == null ? void 0 : o.trim()) || sg,
    messages: e,
    reasoning_effort: t.reasoningEffort ?? "low",
    max_completion_tokens: Number.isFinite(t.maxCompletionTokens) ? Math.max(1, Math.floor(t.maxCompletionTokens)) : lg
  };
...
async function pg(t) {
  ...
  if (!i.ok)
    throw new Error(cg(a, `OpenAI request failed (${i.status}).`));
  const s = ig(a);
  return {
    message: fg(a),
    ...s ? { usage: og(s) } : {}
...
const hg = "http://localhost:3000", Ml = "https://cinegen-film.vercel.app", zi = "persist:cinegen-team-workspace", gg = 8e3, ci = "__CINEGEN_TEAM_PROVIDER__", He = { origin: Ml, source: "hosted" }, $n = { origin: hg, source: "local-web" };
let Pe = null, gt = null, er = null;
function Hr() {
  return {
    supported: !0,
    scope: "workspace",
...
async function sn(t, e, r, n) {
  const i = new AbortController(), o = setTimeout(() => i.abort(), gg);
  try {
    const a = `${t.origin}/api/rpc/${encodeURIComponent(e)}/${encodeURIComponent(r)}`, s = {
      ...
      redirect: "error",
      signal: i.signal
    }, l = t.source === "hosted" ? await Ba.fromPartition(zi).fetch(a, s) : await fetch(a, s);
    return { response: l, payload: await yg(l) };
  } finally {
    clearTimeout(o);
  }
}
...
async function hr(t) {
  try {
    ...
    return !e.ok || !(r != null && r.ok) || !r.result ? null : Dl(r.result, t);
  } catch {
    return null;
  }
}
async function ln() {
  ...
  const t = await hr(He);
  if (t)
    return Pe = He, { target: He, status: t };
  const e = await hr($n);
  return e ? (Pe = $n, { target: $n, status: e }) : null;
}
async function _g(t, e, r) {
  var a;
  const n = await ln();
  if (!n)
...
async function wg() {
  ...
  if (!e || e.target.source !== "hosted") {
    const a = await $l();
    (i = a.desktop) != null && i.connected && a.desktop.source === "hosted" && (e = { target: He, status: a });
  }
  if (!e || e.target.source !== "hosted")
...
async function bg(t) {
  return _g("llm", "openaiChat", [{ ...t, apiKey: ci }]);
}
async function $l() {
  const t = await hr(He);
  if (t)
    ...
    const u = () => {
      ...
      if (!(s || a)) {
        ...
        try {
          const m = await hr(He);
          if (a || !m) return;
          Pe = He, c(m), e.isDestroyed() || e.close();
        } finally {
          s = !1;
        }
      }
...
function vg() {
  I.handle("team-providers:status", async () => {
    const t = await ln();
    return (t == null ? void 0 : t.status) ?? Hr();
  }), I.handle("team-providers:connect", () => $l()), I.handle("team-providers:disconnect", async () => (await Ba.fromPartition(zi).clearStorageData({ storages: ["cookies", "indexdb", "localstorage"] }), Pe = null, Hr())), I.handle("team-providers:save", async (t, e) => xa("save", e)), I.handle("team-providers:remove", async (t, e) => xa("remove", e)), I.handle("team-providers:share-topview", () => wg());
}
function Eg(t) {
  const e = w.extname(t).toLowerCase();
...
function Tg(t) {
  ...
  const r = mg(e) ?? (e.startsWith("/") || /^[A-Za-z]:[\\/]/.test(e) ? e : null);
  if (!r || !D.existsSync(r)) return null;
  const n = D.readFileSync(r);
  return `data:${Eg(r)};base64,${n.toString("base64")}`;
}
function Sg(t) {
  return Array.isArray(t) ? t.flatMap((e) => {
...
function Ig() {
  I.handle("llm:openai-chat", async (t, e) => {
    const r = e && typeof e == "object" && !Array.isArray(e) ? e : {}, n = typeof r.apiKey == "string" ? r.apiKey : "", i = typeof r.userMessage == "string" ? r.userMessage : "", o = Sg(r.imageUrls);
    return n === ci ? bg({
      ...r,
      apiKey: ci,
      userMessage: i,
...
async function Bl(t, e, r, n) {
  const i = we(), o = Math.max(0, e), a = Math.max(0.1, Math.min(r, xg));
  try {
    return await Fl(i, [
      "-y",
      "-ss",
...
function Fn(t, e, r) {
  ...
  const n = w.extname(t) || (e.mediaType === "image" ? ".jpg" : ".mp4"), i = w.join(r, `${fi(e)}-source${n}`), o = Hl(t, i);
...
async function Wi(t, e) {
  ...
  for (const i of t) {
    ...
    if (i.trimStartSec !== void 0 && i.trimDurationSec !== void 0) {
      const u = w.join(r, `${fi(i)}.mp4`), c = await Bl(
        o,
        i.trimStartSec,
        i.trimDurationSec,
        u
    ...
    const s = (i.framePaths ?? []).map((u) => kt(u)).find(Boolean);
    if (s) {
      const u = Fn(s, {
        ...i,
        mediaType: "image",
    ...
    const l = w.join(r, `${fi(i)}.jpg`), d = await Ur(o, i.trimStartSec ?? 0, l);
    d && n.push({
      label: i.label,
      kind: i.kind,
      mediaType: "image",
...
async function Ag(t, e) {
  ...
  if (e.mode === "first-last") {
    const u = w.join(n, `${i}-first.jpg`), c = w.join(n, `${i}-last.jpg`), p = await Ur(r, o, u), f = await Ur(r, Math.max(o, a - 0.05), c), m = [], y = [];
  ...
  if (e.mode === "segment") {
    const u = w.join(n, `${i}-segment.mp4`), c = Math.max(0.1, a > o ? a - o : e.maxSegmentSec ?? 30), p = await Bl(r, o, Math.min(c, e.maxSegmentSec ?? 30), u);
    if (!p) throw new Error("Failed to extract clip segment");
    return { paths: [p], roles: ["image"] };
  }
  const s = e.frameTimeSec ?? (a > o ? (o + a) / 2 : o), l = w.join(n, `${i}-frame.jpg`), d = await Ur(r, s, l);
...
const jg = 18e4, Rg = 600 * 1e3;
async function Wl(t) {
  var u;
  const e = await an("gemini");
  if (!e)
    throw new Gr("Gemini CLI is not installed.");
  ...
  let i = r, o = !1;
  if (Vl(r)) {
    const c = w.extname(r) || ".mp4", p = w.join(n, `${J.randomUUID()}${c}`), f = Hl(r, p);
    if (!f)
  ...
  return new Promise((c, p) => {
    var S, x;
    const f = ae(e, l, { env: Cl(), cwd: n, stdio: ["ignore", "pipe", "pipe"] });
    let m = "", y = "", h = "", g = !1, _ = !1;
    const b = (T) => {
      g || (g = !0, clearTimeout(v), clearTimeout(E), d(), T());
    }, v = setTimeout(() => {
    ...
    (S = f.stdout) == null || S.on("data", (T) => {
      h += T.toString();
      let A;
      for (; (A = h.indexOf(`
`)) >= 0; ) {
        const C = h.slice(0, A).trim();
        if (h = h.slice(A + 1), !!C)
          try {
            const N = JSON.parse(C);
    ...
    }), (x = f.stderr) == null || x.on("data", (T) => {
      y += T.toString();
    }), f.on("error", (T) => b(() => p(T))), f.on("close", (T) => {
      const A = m.trim();
      if (!A) {
        const C = di(y.trim()) || `Gemini CLI exited with code ${T ?? "unknown"}`;
        b(() => p(new Error(C)));
        return;
      }
      if (kg(A)) {
        b(() => p(new Gr("Gemini CLI declined to analyze the media.")));
        return;
      }
      b(() => c(A));
    });
...
const Aa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  GeminiMediaUnavailableError: Gr,
  analyzeMediaWithGeminiCli: Wl,
  buildGeminiUserMessageWithVisualRefs: Gl,
  cleanupEphemeralVisualRefs: Xi,
  prepareClipReference: Ag,
  prepareCopilotVisualRefs: Wi,
  resolveLocalSourcePath: zl
}, Symbol.toStringTag, { value: "Module" }));
let _e = null;
const Og = 9e4, Pg = 18e4, Ng = 8e3;
function Xl() {
...
${t.purpose === "enhance-prompt" ? Ul : Ll}`);
    } else
...
async function ka(t, e, r) {
  ...
  const i = ((h = e.model) == null ? void 0 : h.trim().replace(/^[^/]+\//, "")) || "gemini-2.5-flash", o = r.canResume ? Lg(e) : qg(e), a = o.length > Ng, s = Xl();
  ...
  const d = ql();
  let u = "", c = "", p, f;
  const m = 900 * 1e3, y = r.hasVisualRefs ? Pg : Og;
  return new Promise((g, _) => {
    ...
    a && ((C = b.stdin) == null || C.write(o), (N = b.stdin) == null || N.end()), _e = { child: b, requestId: t, provider: "gemini" };
    let v = "", E = !1, S = !1;
    const x = (H) => {
      E || (E = !0, clearTimeout(T), clearTimeout(A), Xi(r.preparedVisualRefs), H());
    }, T = setTimeout(() => {
      _e = null, b.kill("SIGTERM"), x(() => _(new Error("Gemini CLI timed out after 15 minutes. Try again or switch models.")));
    }, m), A = setTimeout(() => {
      S || E || (_e = null, b.kill("SIGTERM"), x(() => _(new Error(
...
`)) >= 0; ) {
        ...
        if (v = v.slice(G + 1), !!R)
          try {
            ...
            const V = Ug(P);
            if (V && (f = V), P.type === "tool_use" && (d == null || d.webContents.send("llm:gemini-stream", {
              requestId: t,
              status: Mg(P.tool_name)
            })), P.type === "message" && P.role === "assistant" && typeof P.content == "string") {
              const k = P.content;
              k && (S = !0, u += k, d == null || d.webContents.send("llm:gemini-stream", { requestId: t, token: k }));
            }
            if (P.type === "error" && typeof P.message == "string") {
              const k = P.message;
              c += k, !u.trim() && Dg(k) && (_e = null, b.kill("SIGTERM"), x(() => _(new Error(di(k)))));
            }
            if (P.type === "result" && P.status === "error") {
              const k = typeof P.error == "string" ? P.error : typeof P.message == "string" ? P.message : "Gemini CLI returned an error.";
              c += k;
            }
          } catch {
          }
      }
    }), (B = b.stderr) == null || B.on("data", (H) => {
      c += H.toString();
    }), b.on("error", (H) => {
      _e = null, x(() => _(H));
    }), b.on("close", (H) => {
      _e = null, d == null || d.webContents.send("llm:gemini-stream", { requestId: t, done: !0 });
      const G = u.trim();
      if (!G) {
        const R = di(c.trim()) || `Gemini CLI exited with code ${H ?? "unknown"}`;
        x(() => _(new Error(R)));
        return;
      }
      x(() => g({
...
async function Fg(t, e) {
  ...
  const i = await Wi(e.visualRefs ?? [], n);
  ...
  const o = i.length > 0, a = {
    ...e,
    userMessage: Gl(e.userMessage, i)
...
function Bg() {
  I.handle("llm:gemini-chat", async (t, e) => {
    const r = e.requestId || J.randomUUID(), n = await Fg(r, e);
    return {
      message: n.message,
      sessionId: n.sessionId,
      resumed: n.resumed,
    ...
    (_e == null ? void 0 : _e.requestId) !== e || _e.provider !== "gemini" || (_e.child.kill("SIGTERM"), _e = null);
  });
}
const Vg = `You are a music prompt engineer. Your job is to write a detailed, evocative text prompt that will be used to generate music with an AI music model (ElevenLabs/Suno).
...
function Gg() {
  I.handle("music:generate-prompt", async (t, e) => {
    ...
    if (!r) throw new Error("No fal.ai API key provided.");
    K.fal.config({ credentials: r });
    const n = e.frameUrls && e.frameUrls.length > 0, i = Hg(e, !!n), o = {
...
function zg() {
  I.handle("dialog:show-save", async (t, e) => {
    ...
    if (!r) return null;
    const n = await ro.showSaveDialog(r, {
    ...
    const r = Q.getFocusedWindow();
    if (!r) return null;
    const n = await ro.showOpenDialog(r, {
      filters: e == null ? void 0 : e.filters,
  ...
  }), I.handle("shell:open-path", async (t, e) => await hi.openPath(e));
}
const Wg = {
...
function Ut(t, e) {
  const r = Wg[e], n = Object.entries(t).filter(
    ([a]) => a !== "id" && (!r || r.has(a))
  );
...
function ja(t, e) {
  ...
  const n = Ql(t, e), i = Xg(t, e), o = uy(t, e), a = fy(t, e), s = gy(t, e), d = Yg(t, e).map((c) => {
    const p = ey(t, c.id), f = ry(t, c.id), m = sy(t, c.id), y = f.map((h) => ({
      ...h,
      keyframes: oy(t, h.id)
    }));
...
function wy(t, e, r) {
  t.transaction(() => {
    ...
    for (const f of r.timelines) {
      if (l.has(f.id))
        Qg(t, f.id, { name: f.name, duration: f.duration });
      else {
        const { tracks: v, clips: E, transitions: S, ...x } = f;
        td(t, x);
      }
      const m = new Set(
...
function De(t) {
  let e = Ge.get(t);
  return e || (e = new qm(t), Ge.set(t, e)), e;
}
function rd() {
  return w.join(dt(), "projects.json");
}
async function Ji() {
  try {
    const t = await j.readFile(rd(), "utf-8");
    return JSON.parse(t);
  } catch {
    return { projects: [] };
  }
...
async function Ki(t) {
  await j.mkdir(dt(), { recursive: !0 }), await j.writeFile(rd(), JSON.stringify(t, null, 2), "utf-8");
}
async function by(t) {
  const e = await Ji(), r = e.projects.findIndex((n) => n.id === t.id);
  r >= 0 ? e.projects[r] = t : e.projects.push(t), await Ki(e);
}
async function vy(t) {
  const e = await Ji();
  e.projects = e.projects.filter((r) => r.id !== t), await Ki(e);
}
function Ey() {
  I.handle("db:project:create", async (t, e) => {
    ...
    const n = De(e);
    wy(n, e, r);
    const i = pr(), o = await Ji(), a = o.projects.find((s) => s.id === e);
    return a && (a.name = r.project.name, a.updatedAt = i, a.assetCount = r.assets.length, a.elementCount = r.elements.length, await Ki(o)), { ok: !0 };
  }), I.handle("db:project:delete", async (t, e) => {
    const r = Ge.get(e);
    r && (r.close(), Ge.delete(e));
    const n = We(e);
    try {
    ...
    return await vy(e), { ok: !0 };
  }), I.handle("db:project:close", async (t, e) => {
    const r = Ge.get(e);
    return r && (r.close(), Ge.delete(e)), { ok: !0 };
  }), I.handle(
    "db:project:update",
    async (t, e, r) => {
      const n = De(e);
      return Yl(n, e, r), { ok: !0 };
    }
...
const Sy = /* @__PURE__ */ new Set([".mp4", ".mov", ".avi", ".mkv", ".webm", ".mxf", ".m4v"]), Iy = /* @__PURE__ */ new Set([".wav", ".mp3", ".aac", ".flac", ".ogg", ".m4a"]);
function Bn(t, e) {
  const r = w.extname(t).toLowerCase();
  return Sy.has(r) ? "video" : Iy.has(r) ? "audio" : r ? "image" : e;
}
function xy(t, e) {
  if (t)
    try {
...
function Mr(t) {
  ...
  if (Ee(i).catch((o) => console.error("[generated-asset-persist] Metadata failed:", o)), t.type === "audio" || t.type === "video") {
...
async function Ra(t) {
  ...
  const s = t.extension || xy(t.remoteUrl ?? t.localPathHint, n), l = w.join(o, `${r}${s}`), d = jy(t.localPathHint);
  if (d)
    return await Ry(d, l), Mr({
      assetId: r,
      projectId: e,
      inputPath: l,
      type: Bn(l, n)
...
let me = null;
const $e = /* @__PURE__ */ new Map(), ir = /* @__PURE__ */ new Map(), Oy = w.dirname(bi(import.meta.url));
function nd() {
  let t = w.join(Oy, "workers", "media-worker.js");
  return t.includes("app.asar") && (t = t.replace("app.asar", "app.asar.unpacked")), t;
}
function Py() {
  return me || (me = new Xa(nd()), me.on("message", (t) => {
    switch (t.type) {
      case "ready":
...
function Ee(t) {
  return t.type === "sync_compute_offset" || t.type === "sync_batch_match" ? Ny(t) : new Promise((e, r) => {
    $e.set(t.id, { resolve: e, reject: r }), ir.set(t.id, { assetId: t.assetId, jobType: t.type }), Py().postMessage({ type: "job:submit", job: t });
  });
}
function Ny(t) {
  return new Promise((e, r) => {
...
function qy() {
  I.handle("media:import", async (t, e) => {
  ...
  }), I.handle("media:submit-job", async (t, e) => Ee(e)), I.handle("media:cancel-job", async (t, e) => {
    const r = me;
    return r && r.postMessage({ type: "job:cancel", jobId: e }), $e.delete(e), { ok: !0 };
  }), I.handle("media:extract-frame", async (t, e) => {
    const { inputPath: r, timeSec: n } = e, i = we(), o = w.join(z.tmpdir(), `cinegen-frame-${J.randomUUID()}.jpg`);
    ...
    const d = w.join(l, `trim-${J.randomUUID()}.mp4`);
    return new Promise((u) => {
      ...
      Ce(we(), c, { timeout: Math.max(12e4, Math.ceil(s * 4e3)) }, (p) => {
        if (p || !D.existsSync(d)) {
          u(null);
          return;
        }
    ...
    const { inputPath: r, startTimeSec: n, durationSec: i } = e, o = we(), a = w.join(z.tmpdir(), `cinegen-clip-${J.randomUUID()}.mp4`), s = Math.max(0, n), l = Math.max(0.1, i);
    return new Promise((d) => {
      ...
      Ce(o, u, { timeout: Math.max(12e4, Math.ceil(l * 4e3)) }, (c, p, f) => {
        if (c || !D.existsSync(a)) {
          d(null);
          return;
        }
    ...
    if (a) {
      ...
      Ee(p).catch((f) => console.error("[media-import] Thumbnail failed:", f));
    }
    if (s) {
      ...
      Ee(p).catch((f) => console.error("[media-import] Waveform failed:", f));
    }
    if (l) {
      ...
      Ee(p).catch((f) => console.error("[media-import] Filmstrip failed:", f));
    }
    if (o) {
      ...
      Ee(p).catch((f) => console.error("[media-import] Proxy failed:", f));
    }
    return { ok: !0 };
  }), I.handle(
...
const My = Wa(import.meta.url), Dy = w.dirname(bi(import.meta.url));
function $y() {
  return W.isPackaged ? w.join(process.resourcesPath, "native", "cinegen_avfoundation.node") : w.resolve(Dy, "../native/avfoundation/build/Release/cinegen_avfoundation.node");
}
let re = null, mi = null;
if (process.platform === "darwin")
  try {
    const t = $y();
    re = My(t), console.log("[native-video] AVFoundation addon loaded:", t);
  } catch (t) {
    mi = t instanceof Error ? t.message : String(t), console.error("[native-video] Failed to load AVFoundation addon:", mi);
  }
function Me() {
  return re != null;
}
...
function Gy() {
  I.handle("native-video:is-available", () => ({
    ...
    return !r || !Me() ? !1 : By(e, r.getNativeWindowHandle());
  }), I.on("native-video:set-surface-rect", (t, e) => {
    Me() && Vy(e.surfaceId, e.x, e.y, e.width, e.height);
  }), I.on("native-video:set-surface-hidden", (t, e) => {
    Me() && Pa(e.surfaceId, e.hidden);
  }), I.on("native-video:clear-surface", (t, e) => {
    Me() && Na(e);
  }), I.on("native-video:sync-surface", (t, e) => {
    Me() && Hy(e.surfaceId, e.descriptors);
  }), I.on("native-video:destroy-surface", (t, e) => {
    Me() && Oa(e);
  });
}
const zy = "python3.12", id = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), Wy = w.join(id, ".venv", "bin", "python");
function Xy(...t) {
  return W.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const Jy = Xy("scripts", "whisperx", "cinegen_infer.py"), Ky = "fal-ai/whisper", Ca = "3", Yy = {
  ".mp4": "video/mp4",
  ".m4v": "video/mp4",
  ".mov": "video/quicktime",
  ".webm": "video/webm",
...
function Qy(t) {
  const e = w.extname(t).toLowerCase();
  return Yy[e] ?? "application/octet-stream";
}
function Wr(t) {
  const e = Number(t);
...
function od(t) {
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    r || (r = {
      ...
      words: []
    }), r.words.push(o), r.end = o.end, r.text = Zy(r.text, o.word), !r.speaker && o.speaker && (r.speaker = o.speaker);
...
function r_(t) {
  const e = (t == null ? void 0 : t.data) ?? t, r = typeof (e == null ? void 0 : e.text) == "string" ? e.text : "", n = e == null ? void 0 : e.chunks, i = e, o = Array.isArray(n) ? n.flatMap((u) => {
    ...
    const c = typeof u.text == "string" ? u.text.trim() : "", p = u.timestamp, f = Array.isArray(p) ? Wr(p[0]) : void 0, m = Array.isArray(p) ? Wr(p[1]) : void 0, y = e_(u.speaker);
...
function Xe(t, e) {
  var r;
  (r = o_()) == null || r.webContents.send("transcription:progress", {
    jobId: t.jobId,
    assetId: t.assetId,
    engine: t.engine,
    ...e
...
async function a_(t) {
  try {
    const e = De(t.projectId), r = Ql(e, t.projectId).find((o) => o.id === t.assetId), i = {
...
async function Yi(t) {
  t.status = "done", t.segments = t_(t.segments), t.fullText.trim() || (t.fullText = t.segments.map((e) => e.text).filter(Boolean).join(" ")), await a_(t), Xe(t, {
    type: "done",
    text: t.fullText,
    segments: t.segments,
    language: t.language
...
function s_(t, e) {
  ...
  t.model = r, (async () => {
    ...
    await j.writeFile(i, i_, "utf-8");
    const o = ae(zy, [i, e.filePath, r, n], {
      stdio: ["ignore", "pipe", "pipe"]
    });
        ...
        if (l)
          try {
            ...
            if (d.type === "segment") {
              ...
              t.segments.push(u), Xe(t, { type: "segment", ...u });
...
`)) {
      ...
      if (l)
        try {
          const d = JSON.parse(l);
          d.type === "progress" ? (d.output_text !== void 0 && (t.fullText = d.output_text), d.segments && (t.segments = d.segments), d.language !== void 0 && (t.language = d.language), Xe(t, {
            type: "progress",
            stage: d.stage,
            message: d.message,
          ...
          })) : d.type === "done" ? (d.output_text !== void 0 && (t.fullText = d.output_text), d.segments && (t.segments = d.segments), d.language !== void 0 && (t.language = d.language), o = d.transcript_path) : d.type === "error" && ot(t, d.error ?? "WhisperX error");
        } catch {
        }
    }
  }), i.stderr.on("data", () => {
...
function d_(t, e) {
  (async () => {
    ...
    t.model = Ca, t.status = "running", Xe(t, { type: "status", status: "running", stage: "uploading", message: "Preparing audio for cloud transcription" }), K.fal.config({ credentials: e.apiKey });
    const r = await n_(e.filePath);
    let n = "";
    try {
      const s = await j.readFile(r), d = `${w.basename(e.filePath, w.extname(e.filePath))}.m4a`, u = Qy(r), c = new Blob([s], { type: u }), p = new File([c], d, { type: u });
    ...
    }, o = await K.fal.subscribe(Ky, { input: i, logs: !0 }), a = r_(o);
    t.fullText = a.text, t.segments = a.segments, t.language = a.language, await Yi(t);
  })().catch((r) => {
...
function u_() {
  I.handle("transcription:start", async (t, e) => {
    ...
    return qa.set(d, u), s === "whisperx-local" ? l_(u, { filePath: i, language: a }) : s === "whisper-cloud" ? d_(u, { filePath: i, language: a, apiKey: l }) : s_(u, { filePath: i, model: o, language: a }), { jobId: d };
...
const Qi = w.join(z.homedir(), "Desktop", "Coding", "ltx"), c_ = w.join(Qi, ".venv", "bin", "python"), f_ = w.join(Qi, "cinegen_infer.py"), Zi = w.join(z.homedir(), "Desktop", "Coding", "qwen-edit"), p_ = w.join(Zi, ".venv", "bin", "python"), m_ = w.join(Zi, "cinegen_infer.py"), ad = w.join(z.homedir(), "Desktop", "Coding", "layer-decompose"), h_ = w.join(ad, ".venv", "bin", "python"), sd = w.join(z.homedir(), "Desktop", "Coding", "whisperx"), g_ = w.join(sd, ".venv", "bin", "python");
function ld(...t) {
  return W.isPackaged ? w.join(process.resourcesPath, ...t) : w.join(process.cwd(), ...t);
}
const y_ = ld("scripts", "layer-decompose", "cinegen_infer.py"), __ = ld("scripts", "whisperx", "cinegen_infer.py"), w_ = {
...
function yt(t, e) {
  var r;
  (r = b_()) == null || r.webContents.send("local-model:progress", { jobId: t, ...e });
}
...
function v_() {
  I.handle("local-model:run", async (t, e) => {
    ...
    if (e.nodeType === "qwen-edit-local") {
      ...
      let p = null;
      if (r.image_url) {
        const m = await Pr(String(r.image_url), n);
      ...
      if (r.audio_url) {
        ...
        const m = await Pr(String(r.audio_url), n);
      ...
      p && (f.HF_TOKEN = p), o = ae(g_, c, {
        cwd: sd,
        stdio: ["ignore", "pipe", "pipe"],
        env: f
      });
    } else {
      const s = String(r.prompt ?? ""), l = String(r.resolution ?? "896x512"), { height: d, width: u } = w_[l] ?? { height: 512, width: 896 }, c = Number(r.frame_rate ?? 24), p = Number(r.duration_secs ?? 4), f = Math.round(p * c / 8) * 8 + 1, m = Math.max(9, f), y = Number(r.seed ?? 42), h = !!r.enhance_prompt;
      let g = null;
      if (r.image_url) {
        const b = await Pr(String(r.image_url), n);
      ...
      g && _.push("--image_path", g), h && _.push("--enhance_prompt"), o = ae(c_, _, {
        cwd: Qi,
        stdio: ["ignore", "pipe", "pipe"]
      });
    }
...
`)) {
        ...
        if (d)
          try {
            ...
            u.type === "progress" ? (i.stage = u.stage, u.output_text !== void 0 && (i.outputText = u.output_text), u.segments && (i.segments = u.segments), u.language !== void 0 && (i.language = u.language), yt(n, {
              type: "progress",
              stage: u.stage,
              message: u.message,
            ...
            })) : u.type === "done" ? (i.status = "done", i.outputPath = u.output_path, i.outputText = u.output_text, i.transcriptPath = u.transcript_path, i.segments = u.segments, i.language = u.language, yt(n, {
              type: "done",
            ...
            })) : u.type === "error" && (i.status = "error", i.error = u.error, yt(n, { type: "error", error: u.error }));
          } catch {
          }
      }
    }), o.stderr.on("data", () => {
...
const eo = w.join(z.homedir(), "Desktop", "Coding", "Sam3"), E_ = w.join(eo, ".venv", "bin", "python"), T_ = w.join(eo, "cinegen_server.py"), S_ = 120 * 1e3, I_ = 500, x_ = 60;
class A_ {
  constructor() {
  ...
  async start() {
    var e, r;
    return this.proc && !this.proc.killed ? this.port : (this.port = await this.findFreePort(), console.log(`[sam3] Starting server on port ${this.port}`), this.proc = ae(E_, [T_, "--port", String(this.port)], {
      cwd: eo,
      stdio: ["ignore", "pipe", "pipe"],
      env: {
        ...process.env,
  ...
  async waitForHealth() {
    ...
    for (let e = 0; e < x_; e++) {
      ...
      await new Promise((r) => setTimeout(r, I_));
    }
...
async function O_(t) {
  const e = t.workspaceDir ?? w.join(W.getPath("userData"), "gemini-cli-workspace"), r = await Wi(t.visualRefs, e);
  if (r.length === 0)
    throw new Error("Could not load the attached clip or asset files for visual analysis.");
  try {
    const n = [];
    for (const i of r) {
      const o = R_(t.prompt, i.label, i.mediaType), a = i.mediaType === "video" ? await kl({
        apiKey: t.apiKey,
...
function P_() {
  I.handle("copilot:analyze-visual-refs", async (t, e) => O_(e));
}
const N_ = 1, C_ = -30, q_ = 0.3;
function L_(t) {
  const e = [];
  let r = null;
  for (const n of t.split(/\r?\n/)) {
...
`);
  const n = r.map((i) => `[${Ua(i.start)}-${Ua(i.end)}] ${i.text}`).join(`
`);
  return [
...
function $_(t) {
  ...
  return (Array.isArray(r) ? r : r && typeof r == "object" && Array.isArray(r.segments) ? r.segments : []).flatMap((i) => {
    ...
    const o = i, a = Vn(o.start), s = Vn(o.end);
...
function F_(t) {
  return new Promise((e) => {
    const r = [
      ...
      t,
      "-af",
      `silencedetect=noise=${C_}dB:d=${q_}`,
      "-f",
      "null",
      "-"
    ], n = ae(we(), r);
...
const dd = "gemini-2.5-flash", B_ = "fal-ai/video-understanding";
async function V_(t, e) {
  var n;
  const r = ((n = t.model) == null ? void 0 : n.trim()) || dd;
  try {
    return { rawText: await Wl({
      mediaPath: t.mediaPath,
      prompt: e,
      model: r
    }), model: r };
  } catch (i) {
    if (!(i instanceof Gr)) throw i;
    if (!t.apiKey)
      throw new Error("Gemini CLI could not analyze this clip and no fal.ai API key is set for fallback.");
    return { rawText: await kl({
      apiKey: t.apiKey,
...
async function H_(t) {
  ...
  const e = {
    ...
    status: "failed",
    version: N_,
    model: ((r = t.model) == null ? void 0 : r.trim()) || dd,
    silenceMap: [],
    segments: [],
  ...
  try {
    const n = await F_(t.mediaPath).catch(() => ""), i = L_(n), o = U_({ assetName: t.assetName, transcript: t.transcript }), { rawText: a, model: s } = await V_(t, o), l = $_(a);
    return {
      ...e,
      model: s,
      status: "ready",
...
function G_() {
  I.handle("acoustic:analyze-asset", async (t, e) => H_(e));
}
const z_ = process.platform === "darwin" && !W.isPackaged;
z_ && (W.disableHardwareAcceleration(), W.commandLine.appendSwitch("disable-gpu-compositing"), console.log("[app] hardware acceleration disabled for macOS dev wake stability"));
...
Va.registerSchemesAsPrivileged([
  {
    scheme: "local-media",
    privileges: {
      standard: !0,
...
const W_ = Date.now(), Ma = "cinegen-desktop", Da = "CineGen", X_ = ".cinegen-user-data-migrated.json", $a = "CineGen", J_ = 700;
...
function Y_() {
  try {
    const t = W.getPath("appData"), e = w.join(t, Ma), r = w.join(t, Da);
    ...
    const e = W.getPath("appData"), r = w.join(e, Da), n = w.join(e, Ma);
    return { preferredUserDataPath: r, legacyUserDataPath: n };
  }
}
const Q_ = Y_();
try {
  W.setName($a), process.platform === "darwin" && W.setAboutPanelOptions({
    applicationName: $a,
...
async function Z_() {
  const { preferredUserDataPath: t, legacyUserDataPath: e } = Q_;
...
function tw(t) {
  const e = w.extname(t).toLowerCase();
  return K_[e] ?? "application/octet-stream";
}
function rw(t, e) {
...
async function ow() {
  ...
  try {
    const s = await j.readFile(t, "utf-8"), l = JSON.parse(s), d = ((n = l.project) == null ? void 0 : n.id) || J.randomUUID(), u = ((i = l.project) == null ? void 0 : i.name) || "Migrated Project";
...
W.whenReady().then(async () => {
  if (await Z_(), process.platform === "darwin") {
    const r = ew();
    console.log("[dock] icon candidates:", r);
    for (const n of r)
      try {
        const i = fd.createFromPath(n);
  ...
  Va.handle("local-media", async (r) => {
    try {
      ...
      const o = i.size, a = tw(n), s = rw(r.headers, "range");
      ...
      if (s) {
        ...
        const f = p - c + 1, m = D.createReadStream(n, { start: c, end: p }), y = no.toWeb(m);
      ...
      const l = D.createReadStream(n), d = no.toWeb(l);
  ...
  }), Md(), Fd(() => Z), xd(), ip(), Tc(), gp(), Sm(), km(), Pm(), Gm(), Rh(), Hh(), Xh(), rg(), Ig(), vg(), Bg(), Gg(), zg(), Ey(), qy(), Uy(Ee), ah(), P_(), G_(), Gy(), u_(), v_(), k_(), await ow(), I.handle("pm:open-project", async (r, n, i) => n === "__close__" ? (ee == null || ee.close(), ee = null, { ok: !0 }) : ((!Z || Z.isDestroyed()) && (Z = lo()), Z.once("ready-to-show", () => {
  ...
  Z.once("ready-to-show", () => {
    const r = Date.now() - W_, n = Math.max(0, t - r);
    setTimeout(() => {
  ...
  const e = (r) => {
    Be && (clearTimeout(Be), Be = null), Be = setTimeout(() => {
      Be = null, console.log(`[app] Wake recovery triggered by ${r}`), Nd(r);
    }, J_);
  };
  yn.on("resume", () => {
    Hn("resume"), e("resume");
...
W.on("before-quit", () => {
  Bd(), Be && (clearTimeout(Be), Be = null), Ly(), Ty(), j_();
});
