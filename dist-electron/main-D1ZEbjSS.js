...
function buildCostArgs(model, params) {
  const args = ["generate", "cost", model];
  for (const [name2, value] of Object.entries(pickKnownHiggsfieldParams(model, params) ?? {})) {
    if (value === void 0 || value === null || value === "") continue;
    if (!PARAM_NAME_PATTERN.test(name2) || RESERVED_PARAM_NAMES.has(name2)) continue;
    if (typeof value === "string") args.push(`--${name2}`, value);
    else if (typeof value === "number" && Number.isFinite(value)) args.push(`--${name2}`, String(value));
    else if (typeof value === "boolean") args.push(`--${name2}`, value ? "true" : "false");
  }
  args.push("--json");
  return args;
}
function creditsFromCostStdout(stdout) {
  const trimmed = stdout.trim();
...
async function estimateHiggsfieldCost(model, params = {}) {
  if (!model || !/^[A-Za-z][A-Za-z0-9_-]*$/.test(model)) return null;
  try {
    return creditsFromCostStdout(await runHiggsfieldCli(buildCostArgs(model, params), 2e4));
  } catch {
    return null;
  }
}
...
function registerHiggsfieldHandlers() {
  ...
  ipcMain.handle("higgsfield:generate-cost", async (_event, params) => {
    return estimateHiggsfieldCost(params == null ? void 0 : params.model, (params == null ? void 0 : params.params) ?? {});
  });
...
function registerWorkflowHandlers() {
  ipcMain.handle("workflow:run", async (_event, params) => {
    ...
    const { ALL_MODELS, resolveVideoModelEndpoint, sanitizeVideoInputsForEndpoint } = await import("./models-BufbdHFI.js");
...
function registerMediaImportHandlers() {
  ...
  ipcMain.handle("media:trim-video", async (_event, params) => {
    ...
    const outputDir = path.join(projectDir$1(projectId), "media", "generated");
    await fs$1.mkdir(outputDir, { recursive: true });
    const outputPath = path.join(outputDir, `trim-${crypto$1.randomUUID()}.mp4`);
    return new Promise((resolve) => {
      const args = [
        "-y",
      ...
      execFile(getFfmpegPath(), args, { timeout: Math.max(12e4, Math.ceil(duration * 4e3)) }, (err) => {
        if (err || !fs.existsSync(outputPath)) {
          resolve(null);
          return;
        }
