import { H as HIGGSFIELD_MODEL_REGISTRY } from "./main-C5ME3J1x.js";
...
const KLING_V3_QUALITY_OPTS = [
  { value: "standard", label: "Standard (720p)" },
  { value: "pro", label: "Pro (1080p)" },
  { value: "4k", label: "4K" }
];
const SORA2_QUALITY_OPTS = [
  { value: "pro", label: "Pro (up to 1080p)" },
  { value: "standard", label: "Standard (720p)" }
];
const LTX23_QUALITY_OPTS = [
  { value: "pro", label: "Pro" },
  { value: "fast", label: "Fast" }
];
function isKlingV3NodeType(nodeType) {
  return nodeType === "kling-3-text" || nodeType === "kling-3-image";
}
function resolveKlingV3ModelId(mode, quality) {
  const tier = quality === "standard" || quality === "4k" ? quality : "pro";
  return `fal-ai/kling-video/v3/${tier}/${mode}`;
}
function usesEndpointQualityRouting(nodeType) {
  return isKlingV3NodeType(nodeType) || nodeType === "sora-2" || nodeType === "ltx-2-3-text" || nodeType === "ltx-2-3-image";
}
function resolveVideoModelEndpoint(nodeType, modelDef, opts) {
  ...
  if (nodeType === "sora-2") {
    return quality === "standard" ? "fal-ai/sora-2/image-to-video" : "fal-ai/sora-2/image-to-video/pro";
  }
  if (nodeType === "ltx-2-3-text") {
    return quality === "fast" ? "fal-ai/ltx-2.3/text-to-video/fast" : "fal-ai/ltx-2.3/text-to-video";
  }
  if (nodeType === "ltx-2-3-image") {
    return quality === "fast" ? "fal-ai/ltx-2.3/image-to-video/fast" : "fal-ai/ltx-2.3/image-to-video";
  }
  return opts.hasImageInputs && modelDef.altId ? modelDef.altId : modelDef.id;
}
function sanitizeVideoInputsForEndpoint(nodeType, effectiveModelId, inputs) {
...
function optionValues(model, field) {
  ...
  const entry = isRecord(options) ? options[field] : Array.isArray(options) ? options.find((candidate) => isRecord(candidate) && (candidate.name === field || candidate.key === field || candidate.field === field)) : void 0;
  const raw = Array.isArray(entry) ? entry : isRecord(entry) ? ["values", "options", "enum", "allowedValues"].map((key) => entry[key]).find(Array.isArray) ?? [] : [];
  return raw.map(optionValue).filter((entry2) => entry2 !== void 0 && entry2 !== null);
}
...
const LOCAL_MODEL_REGISTRY = {
  ...
  "qwen-edit-local": {
    ...
    inputs: [
      ...
      { id: "guidance_scale", portType: "number", label: "Guidance", required: false, falParam: "guidance_scale", fieldType: "range", default: 1, min: 0.5, max: 5, step: 0.5 },
      { id: "true_cfg_scale", portType: "number", label: "True CFG", required: false, falParam: "true_cfg_scale", fieldType: "range", default: 4, min: 1, max: 10, step: 0.5 },
      { id: "seed", portType: "number", label: "Seed", required: false, falParam: "seed", fieldType: "number", default: 42 }
    ]
  },
  "layer-decompose": {
...
const POD_MODEL_REGISTRY = {
  ...
  "openrouter-llm": {
    ...
    inputs: [
      ...
      { id: "max_tokens", portType: "number", label: "Max Tokens", required: false, falParam: "max_tokens", fieldType: "number", default: 1024, min: 1, max: 128e3, step: 1 },
      { id: "reasoning", portType: "number", label: "Reasoning", required: false, falParam: "reasoning", fieldType: "toggle", default: false }
    ]
  }
};
const TOPVIEW_LEGACY_MODEL_REGISTRY = {
  "topview-image-auto": {
...
const ALL_MODELS = { ...buildTopviewModelRegistry(), ...TOPVIEW_LEGACY_MODEL_REGISTRY, ...HIGGSFIELD_MODEL_REGISTRY, ...MODEL_REGISTRY, ...KIE_MODEL_REGISTRY, ...LOCAL_MODEL_REGISTRY, ...RUNPOD_MODEL_REGISTRY, ...POD_MODEL_REGISTRY };
export {
  ALL_MODELS,
