...
import { captureGenerationMetadata, generationInputReferences, savedGenerationReferences } from '../../src/lib/studio/generation-metadata';
import { MEDIA_TOOL_METADATA } from '../../mcp/display-tools.mjs';
import { prepareTrackedGeneration, trackGenerationTool } from './tracked-generation';
...
export const generationTools = [
  ...
  trackGenerationTool,
].map(tool => tool.name === 'cinegen_generate' ? { ...tool, _meta: MEDIA_TOOL_METADATA,
  description: tool.description + ' Returns the live CineGen prism viewer immediately while queued/running. Do not wait for completion before showing it. For a task already started directly in Topview, use cinegen_track_generation instead; never submit it again.' } : tool);
...
function publicJob(j:Job) {
  const provenance=j.generationMetadata;
  return {requestId:j.args.requestId,projectId:j.args.projectId,nodeId:j.nodeId,status:j.status==='registering'?'running':j.status,provider:j.args.provider??'fal',createdAt:j.createdAt,url:j.url??null,error:j.error??null,
    kind:j.prepared?.model.outputType,model:provenance?.model||j.prepared?.model.name,title:j.args.title,
    prompt:provenance?.prompt??j.prepared?.config?.prompt,
    references:provenance?.references?savedGenerationReferences(provenance.references,[]):j.prepared?.config?generationInputReferences({assets:[],elements:[]},j.prepared.config,j.prepared.model,{nodes:[],edges:[]},j.nodeId):undefined,
    resolution:provenance?.resolution||j.prepared?.config?.resolution,aspectRatio:provenance?.aspectRatio||j.prepared?.config?.aspect_ratio};
}
...
export class GenerationJob extends DurableObject {
  ...
  private async handleRequest(request:Request) {
    ...
    const tracking=new URL(request.url).pathname==='/track';
    if(tracking && body.args.provider===undefined)body.args.provider='topview';
    if(job) {
      if(Boolean(job.tracked)!==tracking || JSON.stringify(job.args)!==JSON.stringify(body.args)) return Response.json({error:'This requestId was already used with different inputs.'},{status:409});
      if(tracking && job.status==='needs_attention' && body.identity.refreshToken) {
        job.identity=body.identity;job.attempts=0;job.status=job.sourceUrl?'saving':job.trackingRegistered?'running':'registering';
        delete job.error;
        await this.ctx.storage.put('job',job);await this.ctx.storage.setAlarm(Date.now()+100);
      }
      return Response.json(publicJob(job));
    }
    const tracked=tracking?prepareTrackedGeneration(body.args):undefined;
    if(tracked) body.prepared=tracked.prepared;
    else if(body.args.provider) prepareProviderGeneration(body.args, body.prepared ? [body.prepared.model] : undefined);
    else prepareGeneration(body.args); // Only legacy callers/jobs use fal; the public MCP always supplies provider.
    job={identity:body.identity,args:body.args,nodeId:crypto.randomUUID(),createdAt:tracked&&body.args.startedAt?body.args.startedAt:new Date().toISOString(),status:tracked?'registering':'queued',attempts:0,...(body.prepared?{prepared:body.prepared}:{}),
      ...(tracked?{tracked:true,registeredAt:new Date().toISOString(),generationMetadata:tracked.provenance,providerTask:tracked.providerTask}:{})};
    await this.ctx.storage.setAlarm(Date.now()+100);
    await this.ctx.storage.put('job',job);
    return Response.json(publicJob(job));
  }
  async alarm() {
    ...
    try {
      ...
      if(job.status==='submitting') {job.status='needs_attention';job.error='Submission was interrupted. Check your provider history before starting another generation to avoid a duplicate charge.';}
      else if(job.status==='queued'||job.status==='registering') {
        ...
        const node=createWorkflowNodeFromSpec({nodeType:prepared.model.nodeType,label:job.args.title||prepared.model.name,config:{...prepared.config,__studioGenerated:true,__studioCreatedAt:job.createdAt,__studioOutputType:prepared.model.outputType,__studioPrompt:prepared.config.prompt??'',__studioPromptBody:prepared.config.prompt??'',...(references.length?{__studioVideoMode:prepared.config.video_mode==='edit'?'edit':'references',__studioAttachedRefs:references}:{})}},{x:space.nodes.length*40,y:0});
        if(job.tracked) for(const field of prepared.model.inputs) if(!Object.hasOwn(prepared.config,field.id)) delete node.data.config[field.id];
        node.id=job.nodeId;node.data.result={status:'running',progressStartedAt:Date.parse(job.createdAt)};
        ...
        if(job.tracked) {
          // Register a monitor only. This branch must never reach submission.
          job.status='running';job.trackingRegistered=true;
        } else {
        ...
        if(job.args.provider==='topview' || job.args.provider==='higgsfield') {
          const task=job.tracked?prepareTrackedGeneration(job.args).providerTask:job.providerTask;
          const result=await providerRpc(auth.token,job.args.provider,'generate',task);
          if(result.status==='fail') {job.status='failed';job.error=result.error||'Provider generation failed.';}
      ...
      if(job.status==='saving') {
        ...
        if(job.tracked)node.data.config.__studioGenerationMetadata={...job.generationMetadata,outputUrl:job.url};
        if(space.id===state.activeSpaceId)state.nodes=space.nodes;
        if(!state.assets.some(a=>a.id===job.nodeId))state.assets.push({id:job.nodeId,name:job.args.title||`${prepared.model.name} generation`,type:prepared.model.outputType as 'image'|'video',url:job.url,fileRef:job.url,thumbnailUrl:prepared.model.outputType==='image'?job.url:undefined,createdAt:job.createdAt,
          metadata:{generatedVia:job.tracked?'tracked-generation':'studio-generation',sourceNodeId:job.nodeId,generation:job.tracked?{...job.generationMetadata,sourceNodeId:job.nodeId,outputUrl:job.url}:{...captureGenerationMetadata({...node,data:{...node.data,config:prepared.config}},prepared.model,{nodes:[],edges:[]},state,job.url),
      ...
      if(job.status==='running'&&Date.now()-Date.parse(job.registeredAt??job.createdAt)>24*3600000){job.status='needs_attention';job.error='Generation has been pending for more than 24 hours. Check the provider before retrying.';}
      job.attempts=0;
    } catch(e) {
