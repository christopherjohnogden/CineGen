  ...
  async alarm() {
    ...
    try {
      ...
      if(job.status==='submitting') {job.status='needs_attention';job.error='Submission was interrupted. Check your provider history before starting another generation to avoid a duplicate charge.';}
        ...
        const node=createWorkflowNodeFromSpec({nodeType:prepared.model.nodeType,label:prepared.model.name,config:{...prepared.config,__studioGenerated:true,__studioCreatedAt:job.createdAt,__studioOutputType:prepared.model.outputType,__studioPrompt:prepared.config.prompt??'',__studioPromptBody:prepared.config.prompt??'',...(references.length?{__studioVideoMode:'references',__studioAttachedRefs:references}:{})}},{x:space.nodes.length*40,y:0});
