...
const defaultHandler={async fetch(request:Request,env:Env):Promise<Response>{
  ...
  if(url.pathname==='/health') return Response.json({status:'ready',service:'cinegen-remote',tools:remoteTools.length+generationTools.length+1,version:'1.8.4',displayTools:remoteTools.filter(t=>isDisplayTool(t.name)).map(t=>t.name)});
