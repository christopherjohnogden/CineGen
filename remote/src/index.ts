    ...
    try {
      ...
      if(params.name==='cinegen_list_models') {
        ...
        if(params.name==='cinegen_generate') {
          ...
          if (prepared.model.outputType === 'video' && prepared.model.inputs.some(field => field.id === 'prompt' && field.required)) {
        ...
        }, async (audioRequest) => providerRpc(store.token, 'elevenlabs', 'generate', { ...audioRequest, projectId: safeId(projectId) }));
...
const defaultHandler={async fetch(request:Request,env:Env):Promise<Response>{
  ...
  if(url.pathname==='/health') return Response.json({status:'ready',service:'cinegen-remote',tools:remoteTools.length+generationTools.length+1,version:'1.8.3',displayTools:remoteTools.filter(t=>isDisplayTool(t.name)).map(t=>t.name)});
