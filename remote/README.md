...
## Creative library and inline displays (server 1.6.2)
...
Cloud calls require `projectId`. Display tools return readable text and `structuredContent` and advertise `ui://cinegen/media-viewer-v7.html` through `_meta.ui.resourceUri` and `openai/outputTemplate`. Both transports implement resources/list and resources/read. The stdio server version is 0.6.2. Cached v1/v2/v3/v4/v5/v6 resource URIs also return the fixed viewer.
