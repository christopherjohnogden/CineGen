...
## Creative library and inline displays (server 1.6.5)

Active video jobs display an animated gold prism in the player until output is ready. Queued, starting and saving jobs retain their actual status labels; prepared or failed jobs do not animate. Reduced-motion preferences show a still prism.
...
Cloud calls require `projectId`. Display tools return readable text and `structuredContent` and advertise `ui://cinegen/media-viewer-v10.html` through `_meta.ui.resourceUri` and `openai/outputTemplate`. Both transports implement resources/list and resources/read. The stdio server version is 0.6.5. Cached v1/v2/v3/v4/v5/v6/v7/v8/v9 resource URIs also return the fixed viewer.
