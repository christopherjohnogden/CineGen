...
export function createAudioEnhancer(env: EnhanceEnv, workspace: string) {
  return async (value: unknown) => {
    ...
    if (typeof p.text !== 'string' || !p.text.trim() || p.text.length > 12000) throw new SiteHttpError(400, 'Enter up to 12,000 characters to enhance. Your text has not been shortened.');
    if (!env.AI) throw new SiteHttpError(503, 'The prompt enhancer is temporarily unavailable. Your original text is unchanged.');
    const input = JSON.stringify({ kind, text: p.text });
    ...
    try {
      const prompt = audioEnhancePrompt(kind, p.text);
